# Capability: Cascading Query Wizard

_The cascading query wizard provides a structured, step-by-step selection interface for querying lineage data. Users navigate through System → Schema → Table → Field levels, with each selection filtering the next. An advanced search fallback preserves the original free-text search experience._
