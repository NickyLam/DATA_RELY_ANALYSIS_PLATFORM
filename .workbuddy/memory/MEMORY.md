...
## 项目概述
Oracle 数据库血缘分析系统，解析 .tab/.prc/.sql/.ctl 文件，生成表级/字段级血缘关系。
支持双数据源：Oracle（RRP_ORACLE）+ 数仓脚本（AI指标血缘分析_数仓&管架）。
...
- `core/parser_protocol.py` — FileParser Protocol + ParseOutput 统一产出
- `core/parser_registry.py` — ParserRegistry 文件扩展名→解析器路由
- `core/adapters/` — Oracle适配器（oracle_tab_adapter.py, oracle_prc_adapter.py）
- `core/warehouse/` — 数仓脚本解析器包
  - `warehouse_parser.py` — 主入口（两阶段解析：DDL→DML→CTL）
  - `schema_resolver.py` — ${xxx_schema} 变量替换（9种schema变量）
  - `temp_table_filter.py` — 临时表过滤
  - `ddl_parser.py` — DDL解析器
  - `dml_parser.py` — DML解析器
  - `ctl_parser.py` — CTL文件解析器
- `app/main.py` — FastAPI 应用入口
- `app/services/parser_service.py` — 解析服务（含ParserRegistry注入）
- `app/services/caliber_service.py` — 口径查询服务
- `app/config.py` — 配置（含多数据源datasource_configs）
...
## 2026-06-17：SQL注释泄漏导致幽灵表名（对私担保客户）

### 根因
旧版 DML 解析器未正确剥离 SQL 行注释 `--个人客户,对私担保客户`，将注释文本误捕获为表名。
运行中服务内存数据为旧版污染数据；SQLite 缓存 `output/lineage.db` 数据已正确（0污染）。

### 修复
**文件**: `core/warehouse/dml_parser.py` → `_is_valid_table_name()`（第805-826行）
- 新增防御性非 ASCII 过滤：`if not name.isascii(): return False`
- 合法 Oracle/数仓表名仅含 ASCII，含中文/CJK 即为注释泄漏，一律拒绝
- `_strip_sql_comments()` 本身已正确工作，此为第二道防线

### 验证结果
- 6 个污染表名（含"对私担保客户"）全部被拒绝
- 7 个合法表名（IML/IOL/ICL/MSL schema + schema变量）全部通过
- 端到端：`icl_cmm_indv_cust_basic_info.sql` → 10表级上游 + CUST_ID←IML.PTY_INDV.PARTY_ID(0.95) + 0污染

### 服务重启注意
- 运行中服务需重启以从干净 SQLite 加载：`./stop.sh && ./start.sh`（~8秒）
- 或设置 `ADMIN_API_KEY` 后调用 `POST /api/system/reparse`（全量重解析6-10分钟）
- `output/lineage_data.pkl`/`.json` 缓存文件已不存在，仅 SQLite 后端
