...
t0 = time.time()
try:
    from app.dependencies import get_parser_service, get_lineage_service, get_caliber_service
...
if parser_service:
    ...
    if data:
        ...
        check("存储过程数据加载", proc_count >= 1700, f"{proc_count} 个存储过程 (期望 ~1754)")
...
try:
    ...
    check(f"解析前10个.tab文件", parsed_count >= 8, f"成功 {parsed_count}/10, 共 {total_fields} 个字段")
    
    # 验证数据服务中的表结构
    if data:
        tables = data.get('tables', [])
        check(f"数据中表结构完整", len(tables) >= 3000, f"{len(tables)} 张表有结构信息")
...
for input_val, expected in bare_cases:
    ...
    check(f"bare_table('{input_val}')", result == expected, f"期望={expected}, 实际={result}")
...
try:
    ...
    if lineage_service:
        ...
        if result:
            ...
            check(f"表级血缘节点", len(nodes) >= 0, f"{len(nodes)} 个节点")
            check(f"表级血缘边", len(edges) >= 0, f"{len(edges)} 条边")
            
            # 检查节点ID唯一性
            node_ids = [n.get('id', '') for n in nodes]
            dup_nodes = len(node_ids) - len(set(node_ids))
            check(f"节点ID无重复", dup_nodes == 0, f"重复 {dup_nodes} 个")
            
            # 检查边连接完整性
            node_id_set = set(node_ids)
            orphan_edges = sum(1 for e in edges if e.get('source_table') not in node_id_set or e.get('target_table') not in node_id_set)
            check(f"边连接完整性", orphan_edges == 0, f"孤立边 {orphan_edges} 条")
            
            # 检查自环
            self_loops = sum(1 for e in edges if e.get('source_table') == e.get('target_table'))
            check(f"无自环边", self_loops == 0, f"发现 {self_loops} 个自环")
        
        # 测试字段级血缘查询
        result_field = lineage_service.query_lineage(TEST_TABLE, field=TEST_FIELD, mode='upstream', depth=3, include_fields=True)
        ...
        if result_field:
            ...
            check(f"字段级血缘节点", len(fm_nodes) >= 0, f"{len(fm_nodes)} 个节点")
            check(f"字段级血缘边", len(fm_edges) >= 0, f"{len(fm_edges)} 条边")
            check(f"字段映射", len(fm_mappings) >= 0, f"{len(fm_mappings)} 条映射")
            
            # 检查字段映射重复
            seen_fm = set()
            dup_fm = 0
            for fm in fm_mappings:
                key = (fm.get('source_table'), fm.get('source_column'), fm.get('target_table'), fm.get('target_column'), fm.get('procedure'))
                if key in seen_fm:
                    dup_fm += 1
                seen_fm.add(key)
            check(f"字段映射无重复", dup_fm == 0, f"重复 {dup_fm} 条")
...
try:
    ...
    if caliber_service:
        ...
        check(f"query_caliber({TEST_TABLE}.{TEST_FIELD})", result.get('success', False) or len(result.get('chains', [])) > 0)
        
        chains = result.get('chains', [])
        check(f"口径链路数量", len(chains) > 0, f"{len(chains)} 条链路")
        ...
        if chains:
            ...
            for chain in chains:
                ...
                seen_in_chain = set()
                for step in steps:
                    key = (step.get('source_table',''), step.get('source_column',''),
                           step.get('target_table',''), step.get('target_column',''),
                           step.get('procedure',''))
                    if key in seen_in_chain:
                        dup_in_chain += 1
                    seen_in_chain.add(key)
            check(f"链路内部无自环重复", dup_in_chain == 0, f"链路内重复 {dup_in_chain} 步")
            ...
            if cross_dup_steps > 0:
                check(f"跨链路共享步骤(DAG正常)", True, f"{cross_dup_steps} 步共享于多条链路")
            else:
                check(f"口径步骤无重复", True)
            ...
            for chain in chains:
                steps = chain.get('steps', [])
                sig = tuple((s.get('source_table',''), s.get('target_table',''), s.get('target_column','')) for s in steps)
                if sig in chain_sigs:
                    dup_chains += 1
                chain_sigs.add(sig)
            check(f"口径链路无重复", dup_chains == 0, f"重复 {dup_chains} 条")
            ...
            check(f"步骤编号完整性", True, f"{steps_with_num}/{len(all_steps)} 步有编号")
        
        # 测试搜索功能
        search_results = caliber_service.search_indicators(TEST_FIELD, limit=10)
        check(f"指标搜索: {TEST_FIELD}", len(search_results) > 0, f"{len(search_results)} 个结果")
        ...
        for r in search_results:
            ...
            if key in seen_search:
                dup_search += 1
            seen_search.add(key)
        check(f"搜索结果无重复", dup_search == 0, f"重复 {dup_search} 个")
        
        # 测试获取字段列表
        fields_with_caliber = caliber_service.get_fields_with_caliber(TEST_TABLE)
        check(f"获取口径字段列表", len(fields_with_caliber) > 0, f"{len(fields_with_caliber)} 个字段")
...
try:
    if caliber_service:
        ...
        check(f"build_summary_card", card_result is not None)
        
        if card_result and card_result.get('success') and card_result.get('data'):
            card_data = card_result['data']
            check(f"summary_card有指标名", 'indicator' in card_data, f"indicator={card_data.get('indicator')}")
            check(f"summary_card有query_time_ms", card_data.get('query_time_ms', 0) > 0, f"{card_data.get('query_time_ms')}ms")
            if 'stats' in card_data:
                check(f"summary_card有统计信息", True, f"stats keys: {list(card_data['stats'].keys())[:5]}")
            if 'data_quality_flags' in card_data:
                check(f"summary_card有质量标记", True, f"flags: {card_data['data_quality_flags']}")
        else:
            check(f"summary_card数据完整性", False, f"success={card_result.get('success') if card_result else None}")
        ...
        check(f"build_pipeline_view", pipeline_result is not None)
        ...
        if pipeline_result and pipeline_result.get('success') and pipeline_result.get('data'):
            ...
            check(f"pipeline节点数量", len(nodes) > 0, f"{len(nodes)} 个节点")
            check(f"pipeline边数量", len(edges) >= 0, f"{len(edges)} 条边")
            
            # 检查节点ID唯一性
            node_ids = [n.get('id', '') for n in nodes]
            dup_node_ids = len(node_ids) - len(set(node_ids))
            check(f"pipeline节点ID无重复", dup_node_ids == 0, f"重复 {dup_node_ids} 个")
            
            # 检查边连接完整性（实际字段名是 source/target，不是 source_id/target_id）
            node_id_set = set(node_ids)
            orphan_edges = sum(1 for e in edges if e.get('source') not in node_id_set or e.get('target') not in node_id_set)
            check(f"pipeline边连接完整性", orphan_edges == 0, f"孤立边 {orphan_edges} 条")
        else:
            check(f"pipeline数据完整性", False)
        ...
        check(f"build_step_detail(step_num=1)", step_result is not None)
        
        if step_result and step_result.get('success') and step_result.get('data'):
            step_data = step_result['data']
            check(f"step_detail有step_num", 'step_num' in step_data, f"step_num={step_data.get('step_num')}")
            check(f"step_detail有raw_sql", len(step_data.get('raw_sql', '')) > 0, f"长度 {len(step_data.get('raw_sql', ''))}")
            expressions = step_data.get('expressions', [])
            check(f"step_detail有表达式", len(expressions) >= 0, f"{len(expressions)} 个表达式")
            if step_data.get('cte_functions'):
                check(f"step_detail有CTE/函数", True, f"{len(step_data['cte_functions'])} 个")
        else:
            check(f"step_detail数据完整性", False)
...
try:
    ...
    if caliber_result and caliber_result.get('chains'):
        ...
        check(f"Markdown导出", len(md_content) > 100, f"长度 {len(md_content)}")
        check(f"Markdown包含标题", "# " in md_content or "## " in md_content)
        
        html_content = exporter.export_html(summary_card, pipeline_view=pipeline_data, include_sql=True)
        check(f"HTML导出", len(html_content) > 100, f"长度 {len(html_content)}")
        check(f"HTML包含html标签", "<html" in html_content or "<!DOCTYPE" in html_content)
    else:
...
try:
    ...
    check(f"总API路由数量", len(all_api_routes) >= 5, f"{len(all_api_routes)} 个API路由")
...
try:
    if caliber_service:
        ...
        if len(results) >= 2:
            check(f"BFS结果一致性(3次调用)", all(x == results[0] for x in results),
                  f"链数量: {results}")
        else:
