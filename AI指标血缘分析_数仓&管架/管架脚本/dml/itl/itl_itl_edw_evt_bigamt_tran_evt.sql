...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd itl_itl_edw_evt_bigamt_tran_evt
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
--alter table ${itl_schema}.itl_edw_evt_bigamt_tran_evt drop partition p_${retain_day};
alter table ${itl_schema}.itl_edw_evt_bigamt_tran_evt drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${itl_schema}.itl_edw_evt_bigamt_tran_evt add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${itl_schema}.itl_edw_evt_bigamt_tran_evt partition for (to_date('${batch_date}','yyyymmdd')) (
    etl_dt  -- 数据日期
    ,evt_id  -- 事件编号
    ,lp_id  -- 法人编号
...
select
    ...
    ,replace(replace(t1.spec_prmssn_prtcptr_id,chr(13),''),chr(10),'')  -- 特许参与者编号
    ...
    ,replace(replace(t1.recv_bank_bank_no,chr(13),''),chr(10),'')  -- 接收行行号
    ...
    ,replace(replace(t1.init_prtcpt_org_id,chr(13),''),chr(10),'')  -- 原发起参与机构编号
    ...
    ,replace(replace(t1.reg_main_acct_num,chr(13),''),chr(10),'')  -- 挂账或维护入账账号
    ...
    ,replace(replace(t1.clarify_enter_acct_num,chr(13),''),chr(10),'')  -- 清分入账账号
    ,replace(replace(t1.clarify_flow_num,chr(13),''),chr(10),'')  -- 清分流水号
    ...
    ,t1.modif_tm  -- 修改时间
    ,replace(replace(t1.cc_bank_draft_id,chr(13),''),chr(10),'')  -- 城商行汇票编号
    ...
    ,replace(replace(t1.exch_bus_tran_chn_cd,chr(13),''),chr(10),'')  -- 汇兑业务交易渠道代码
    ,t1.modif_dt  -- 修改日期
    ,replace(replace(t1.bus_flow_num,chr(13),''),chr(10),'')  -- 业务流水号
    ,replace(replace(t1.mgmt_org_id,chr(13),''),chr(10),'')  -- 管理机构编号
    ,t1.comm_fee_amt  -- 手续费用金额
    ,t1.remit_tran_fee_amt  -- 汇划费用金额
    ,t1.todos  -- 工本费
...
from ${msl_schema}.msl_edw_evt_bigamt_tran_evt t1    --大额交易事件
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${itl_schema}',tabname => 'itl_edw_evt_bigamt_tran_evt',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);