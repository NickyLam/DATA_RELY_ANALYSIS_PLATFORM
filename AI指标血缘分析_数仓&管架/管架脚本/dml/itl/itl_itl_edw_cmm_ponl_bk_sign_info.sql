...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd itl_itl_edw_cmm_ponl_bk_sign_info
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
--alter table ${itl_schema}.itl_edw_cmm_ponl_bk_sign_info drop partition p_${retain_day};
alter table ${itl_schema}.itl_edw_cmm_ponl_bk_sign_info drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${itl_schema}.itl_edw_cmm_ponl_bk_sign_info add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${itl_schema}.itl_edw_cmm_ponl_bk_sign_info partition for (to_date('${batch_date}','yyyymmdd')) (
    etl_dt  -- 数据日期
    ,lp_id  -- 法人编号
    ,cust_id  -- 客户编号
    ,user_id  -- 用户编号
...
from ${msl_schema}.msl_edw_cmm_ponl_bk_sign_info t1    --个人网银签约信息
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${itl_schema}',tabname => 'itl_edw_cmm_ponl_bk_sign_info',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);