...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd itl_mtl_wind_financialbalancesheetdetails
CreateDate: 20180515
Logs:
...
-- alter table ${itl_schema}.mtl_wind_financialbalancesheetdetails drop partition p_${retain_day};
alter table ${itl_schema}.mtl_wind_financialbalancesheetdetails drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${itl_schema}.mtl_wind_financialbalancesheetdetails add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${itl_schema}.mtl_wind_financialbalancesheetdetails partition for (to_date('${batch_date}','yyyymmdd')) (
     object_id -- 对象ID
...
from ${msl_schema}.msl_wind_financialbalancesheetdetails
where 1 = 1
 ;
commit;
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${itl_schema}',tabname => 'mtl_wind_financialbalancesheetdetails',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);