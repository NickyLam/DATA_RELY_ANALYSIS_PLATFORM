...
truncate table ${itl_schema}.itl_edw_pams_jxbb_dkftpmx;
...
BEGIN
  FOR rec IN (SELECT DISTINCT etl_dt FROM ${msl_schema}.msl_edw_pams_jxbb_dkftpmx) 
  LOOP
    BEGIN
      EXECUTE IMMEDIATE 
        'ALTER TABLE ${itl_schema}.itl_edw_pams_jxbb_dkftpmx ADD PARTITION p_' || TO_CHAR(rec.etl_dt, 'yyyymmdd') ||
...
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${itl_schema}.itl_edw_pams_jxbb_dkftpmx (
    tjrq -- 统计日期
    ,khm -- 客户名称
    ,khh -- 客户号
