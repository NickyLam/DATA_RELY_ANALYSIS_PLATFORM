...
truncate table ${itl_schema}.itl_edw_pams_jxbb_ckftphz;
...
BEGIN
  FOR rec IN (SELECT DISTINCT etl_dt FROM ${msl_schema}.msl_edw_pams_jxbb_ckftphz) 
  LOOP
    BEGIN
      EXECUTE IMMEDIATE 
        'ALTER TABLE ${itl_schema}.itl_edw_pams_jxbb_ckftphz ADD PARTITION p_' || TO_CHAR(rec.etl_dt, 'yyyymmdd') ||
...
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${itl_schema}.itl_edw_pams_jxbb_ckftphz (
    tjrq -- 统计日期
    ,kmh -- 科目号
    ,kmmc -- 科目名称
    ,cpmc -- 产品名称
