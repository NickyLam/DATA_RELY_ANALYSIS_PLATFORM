...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd itl_itl_edw_pams_jxbb_ckftphz_recal
CreateDate: 20180515
Logs:
...
--alter table ${itl_schema}.itl_edw_pams_jxbb_ckftphz_recal drop partition p_${retain_day};
alter table ${itl_schema}.itl_edw_pams_jxbb_ckftphz_recal drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${itl_schema}.itl_edw_pams_jxbb_ckftphz_recal add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${itl_schema}.itl_edw_pams_jxbb_ckftphz_recal partition for (to_date('${batch_date}','yyyymmdd')) (
    tjrq -- 统计日期
    ,kmh -- 科目号
    ,kmmc -- 科目名称
    ,cpmc -- 产品名称
...
from ${msl_schema}.msl_edw_pams_jxbb_ckftphz_recal
where 1=1
 ;
commit;
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${itl_schema}',tabname => 'itl_edw_pams_jxbb_ckftphz_recal',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);