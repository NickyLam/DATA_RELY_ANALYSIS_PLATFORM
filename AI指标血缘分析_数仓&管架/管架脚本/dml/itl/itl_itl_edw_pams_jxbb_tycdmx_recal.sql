...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd itl_itl_edw_pams_jxbb_tycdmx_recal
CreateDate: 20180515
Logs:
...
--alter table ${itl_schema}.itl_edw_pams_jxbb_tycdmx_recal drop partition p_${retain_day};
alter table ${itl_schema}.itl_edw_pams_jxbb_tycdmx_recal drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${itl_schema}.itl_edw_pams_jxbb_tycdmx_recal add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${itl_schema}.itl_edw_pams_jxbb_tycdmx_recal partition for (to_date('${batch_date}','yyyymmdd')) (
    tjrq -- 统计日期
    ,jxdxdh -- 绩效对象代号
    ,khdxdh -- 考核对象代号
    ,jgkhdxdh -- 机构考核对象代号
...
select
    ...
    ,nvl(trim(cjdrgjgkhh), ' ') as cjdrgjgkhh -- 成交单认购机构客户号
    ,nvl(trim(cjdrgjg), ' ') as cjdrgjg -- 成交单认购机构
    ,nvl(trim(sjrgfkhh), ' ') as sjrgfkhh -- 实际认购方客户号
    ,nvl(trim(sjrgfqc), ' ') as sjrgfqc -- 实际认购方全称
    ,nvl(trim(tycb), 0) as tycb -- 摊余成本
    ,nvl(trim(recal_dt), 0) as recal_dt -- 重算日期
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间
from ${msl_schema}.msl_edw_pams_jxbb_tycdmx_recal
where 1=1
 ;
commit;
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${itl_schema}',tabname => 'itl_edw_pams_jxbb_tycdmx_recal',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);