...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd itl_itl_edw_pcls_nowoverdue_yxyd
CreateDate: 20180515
Logs:
...
--alter table ${itl_schema}.itl_edw_pcls_nowoverdue_yxyd drop partition p_${retain_day};
alter table ${itl_schema}.itl_edw_pcls_nowoverdue_yxyd drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${itl_schema}.itl_edw_pcls_nowoverdue_yxyd add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${itl_schema}.itl_edw_pcls_nowoverdue_yxyd partition for (to_date('${batch_date}','yyyymmdd')) (
    datecreated -- 日期
    ,loan_bal -- 余额
    ,loan_cnt -- 在贷客户数
...
from ${msl_schema}.msl_edw_pcls_nowoverdue_yxyd
where 1=1
 ;
commit;
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${itl_schema}',tabname => 'itl_edw_pcls_nowoverdue_yxyd',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);