...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd itl_itl_edw_atms_rpt_open_rate_dev_date
CreateDate: 20180515
Logs:
...
alter table ${itl_schema}.itl_edw_atms_rpt_open_rate_dev_date drop partition p_${retain_day};
alter table ${itl_schema}.itl_edw_atms_rpt_open_rate_dev_date drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${itl_schema}.itl_edw_atms_rpt_open_rate_dev_date add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${itl_schema}.itl_edw_atms_rpt_open_rate_dev_date partition for (to_date('${batch_date}','yyyymmdd')) (
    dev_no -- 设备号
...
select
    ...
    ,nvl(trim(date_time), ' ') as date_time -- 日期 yyyyMMdd
    ,nvl(trim(open_rate_year), ' ') as open_rate_year -- 记录年份
    ,nvl(trim(open_rate_month), ' ') as open_rate_month -- 记录月份
    ,nvl(trim(open_rate_day), ' ') as open_rate_day -- 记录日期
    ...
    ,nvl(trim(vcomm_failure_time), 0) as vcomm_failure_time -- V通讯故障时间
    ,nvl(trim(suspected_crash_time), 0) as suspected_crash_time -- 疑似死机时间
    ,nvl(trim(is_from_dev), ' ') as is_from_dev -- 是否通过客户端开机率报文计算。0：是；1：否
...
from ${msl_schema}.msl_edw_atms_rpt_open_rate_dev_date
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${itl_schema}',tabname => 'itl_edw_atms_rpt_open_rate_dev_date',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);