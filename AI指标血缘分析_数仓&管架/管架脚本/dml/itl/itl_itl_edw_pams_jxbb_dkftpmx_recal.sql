...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd itl_itl_edw_pams_jxbb_dkftpmx_recal
CreateDate: 20180515
Logs:
...
--alter table ${itl_schema}.itl_edw_pams_jxbb_dkftpmx_recal drop partition p_${retain_day};
alter table ${itl_schema}.itl_edw_pams_jxbb_dkftpmx_recal drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${itl_schema}.itl_edw_pams_jxbb_dkftpmx_recal add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${itl_schema}.itl_edw_pams_jxbb_dkftpmx_recal partition for (to_date('${batch_date}','yyyymmdd')) (
    tjrq -- 统计日期
    ,khm -- 客户名
    ,khh -- 客户号
...
select
    ...
    ,nvl(trim(bwbs), ' ') as bwbs -- 表外标识
    ,nvl(trim(gyljrywbz), ' ') as gyljrywbz -- 供应链金融业务标志
    ,nvl(trim(recal_dt), 0) as recal_dt -- 重算日期
    ,nvl(trim(ljtzysje), 0) as ljtzysje -- 累计调整营收金额
    ,nvl(trim(tzhljftpjsy), 0) as tzhljftpjsy -- 调整后累计ftp净收益
    ,nvl(trim(ljtzfyje), 0) as ljtzfyje -- 累计调整费用金额
    ,nvl(trim(jjljftpjsy), 0) as jjljftpjsy -- 计奖累计ftp净收益
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间
from ${msl_schema}.msl_edw_pams_jxbb_dkftpmx_recal
where 1=1
 ;
commit;
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${itl_schema}',tabname => 'itl_edw_pams_jxbb_dkftpmx_recal',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);