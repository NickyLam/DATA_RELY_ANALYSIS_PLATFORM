...
truncate table ${itl_schema}.itl_edw_pams_v_yjzb_jg;
...
BEGIN
  FOR rec IN (SELECT DISTINCT etl_dt FROM ${msl_schema}.msl_edw_pams_v_yjzb_jg) 
  LOOP
    BEGIN
      EXECUTE IMMEDIATE 
        'ALTER TABLE ${itl_schema}.itl_edw_pams_v_yjzb_jg ADD PARTITION p_' || TO_CHAR(rec.etl_dt, 'yyyymmdd') ||
...
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${itl_schema}.itl_edw_pams_v_yjzb_jg(
    tjrq -- 统计日期
    ,zbdh -- 指标代号
    ,sdbs -- 时段标识
    ,tjkj -- 统计口径
