...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd itl_itl_ccrw_u_ip_result_icrw_mcss
CreateDate: 20180515
Logs:
...
--alter table ${itl_schema}.itl_ccrw_u_ip_result_icrw_mcss drop partition p_${retain_day};
alter table ${itl_schema}.itl_ccrw_u_ip_result_icrw_mcss drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${itl_schema}.itl_ccrw_u_ip_result_icrw_mcss add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${itl_schema}.itl_ccrw_u_ip_result_icrw_mcss partition for (to_date('${batch_date}','yyyymmdd')) (
    result_id -- 指标结果ID
    ,index_mx_id -- 实际指标ID
...
select
    ...
    ,nvl(trim(tw_sub_bal), 0) as tw_sub_bal -- 比上两周
    ,nvl(trim(tw_sub_zf), 0) as tw_sub_zf -- 比上两周增幅
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间
from ${msl_schema}.msl_ccrw_u_ip_result_icrw_mcss
where 1=1
 ;
commit;
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${itl_schema}',tabname => 'itl_ccrw_u_ip_result_icrw_mcss',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);