...
truncate table ${itl_schema}.itl_edw_pams_jxbb_tycdmx;
...
BEGIN
  FOR rec IN (SELECT DISTINCT etl_dt FROM ${msl_schema}.msl_edw_pams_jxbb_tycdmx) 
  LOOP
    BEGIN
      EXECUTE IMMEDIATE 
        'ALTER TABLE ${itl_schema}.itl_edw_pams_jxbb_tycdmx ADD PARTITION p_' || TO_CHAR(rec.etl_dt, 'yyyymmdd') ||
...
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${itl_schema}.itl_edw_pams_jxbb_tycdmx (
    tjrq -- 统计日期
    ,jxdxdh -- 绩效对象代号
    ,khdxdh -- 考核对象代号
    ,jgkhdxdh -- 机构考核对象代号
