...
INSERT /*+ append */INTO ${idl_schema}.mckb_cust_mgr_distr_tmp_01
               (ETL_DT       -- 数据日期
               ,PED_NO       -- 周期编号
               ,PED_NAME     -- 周期名称
               ,ORG_NO       -- 机构编号
               ...
               ,LP_CLS_NAME  -- 法人分类名称
               --20260206 新增 
               ,LP_CLS_PROD  -- 产品分类  1 是IPC/2 是数据贷
                )
     -- 好企贷 IPC 
     --累计
        SELECT 
        ...
        SELECT TO_DATE('${batch_date}','yyyymmdd') AS ETL_DT
              ...
              ,'当周'                              AS PED_NAME
              ,SUBSTR(BD.OPERATEORGID,1,3)         AS ORG_NO
              ,BD.OPERATEUSERID                    AS MGR_NO -- 推荐人编号 
              ,T4.NAME                             AS MGR_NAME    -- 客户经理名称
              ,TRUNC(T4.CREATE_DATE)               AS CREATE_DATE -- 系统开通权限时间
              ,SUM(BD.BUSINESSSUM)                 AS DISTR_AMT
              ,'1'                                 AS LP_CLS_ID
...
INSERT /*+ append */INTO ${idl_schema}.mckb_cust_mgr_distr_tmp_01
              ...
              ,ORG_NO                -- 机构编号
              ,MGR_NO                -- 客户经理编号
              ,MGR_NAME              -- 推荐人用户编号     -- 20260206 新增 
              ,CREATE_DATE           -- 推荐人名称         -- 20260206 新增 
              ,DISTR_AMT             -- 放款金额
              ,LP_CLS_ID             -- 法人分类编号
              ,LP_CLS_NAME           -- 法人分类名称
        ...
        SELECT 
              ...
              ,T1.ORG_NO         AS ORG_NO
              ,T1.MGR_NO         AS MGR_NO
              ,T1.MGR_NAME       AS MGR_NAME     -- 推荐人用户编号
              ,T1.CREATE_DATE    AS CREATE_DATE  -- 推荐人名称   
...
INSERT /*+ append */INTO ${idl_schema}.mckb_cust_mgr_distr_tmp_02
                (ETL_DT                   -- 数据日期
                ,PED_NO                   -- 周期编号
                ,PED_NAME                 -- 周期名称
                ,RANK                     -- 排名
        ...
        SELECT TO_DATE('${batch_date}', 'yyyymmdd') AS ETL_DT --数据日期
               ...
               ,T1.CREATE_DATE      AS CREATE_DATE --系统开通权限时间
               ,T1.LP_CLS_PROD      AS LP_CLS_PROD    -- 产品分类   20260206 新增 
