/*
Purpose:    自助设备（ATM/CRS）交易量/交易额D层-业务分析实时表(WD041001/WD041002):数据来源于中台
Author:     Sunline/郑沛隆
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_mcyy_bu_analysis_atm_bus
Createdate: 20210511
Logs:
...
alter table ${idl_schema}.mcyy_bu_analysis truncate subpartition p_${batch_date}_atm_bus;
...
alter table ${idl_schema}.mcyy_bu_analysis add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'))(
                                              subpartition p_${batch_date}_atm_bus values ('ATM_BUS')
                                              )
;
alter table ${idl_schema}.mcyy_bu_analysis modify partition p_${batch_date} 
                                             add subpartition p_${batch_date}_atm_bus values ('ATM_BUS')
;
    ...
    --当日数据初始化
     (SELECT t4.index_no
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      WHERE  t1.index_no = 'WD041001') dim_tab) t1
      LEFT   JOIN (SELECT (CASE
                  ...
                  FROM   (SELECT COUNT(1) AS sum_count
                                ,a.tran_org_id AS org_no
                                ,'ATM_YWLX002' AS bu_type
                          ...
                          AND    (intnal_proc_cd <> 'up9999' OR
                                ...
                                substr(a.main_acct_id
                                         ...
                                         ,6) IN
                                (SELECT pinblock
                                    FROM   msl_mpcs_a50ubcardbin)))
                          AND    b.chnlid = 'ATM'
                          AND    substr(a.tran_tm, 1,8) ='${batch_date}'
                          AND    b.trnname IN ('ATM查询余额'
                                              ,'ATM单位结算卡对手账户查询'
                                              ,'ATM转入卡号户名查询'
                                              ,'ATM卡密码修改')
                          ...
                          GROUP  BY a.tran_org_id
                          
                          UNION ALL
                          SELECT COUNT(1) AS sum_count
                                ,a.tran_org_id AS org_no
                                ,'ATM_YWLX001' AS bu_type
                          FROM   itl_edw_evt_atmp_unionpay_tran_flow a
                          LEFT   JOIN msl_mpcs_a50ttrncdmap b
                          ON     substr(a.intnal_tran_cd
                                       ,4) = substr(b.trncd
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      ON     t2.dim_class = t3.dim_class
                      AND    t3.dim_class_name IS NOT NULL
                      WHERE  t1.index_no = 'WD041002') dim_tab) t1
      LEFT   JOIN (SELECT (CASE
                             WHEN substr(org_no
                                        ,1
                                        ,2) = '89'
                        ...
                        ,bu_type
                        ,SUM(sum_count) sum_count
                  FROM   (SELECT SUM(a.tran_amt) AS sum_count
                                ,a.tran_org_id AS org_no
                                ,'ATM_YWLX002' AS bu_type
                          FROM   itl_edw_evt_atmp_unionpay_tran_flow a
                          LEFT   JOIN msl_mpcs_a50ttrncdmap b
                          ...
                          GROUP  BY a.tran_org_id
                          UNION ALL
                          SELECT SUM(a.tran_amt) AS sum_count
                                ,a.tran_org_id AS org_no
                                ,'ATM_YWLX001' AS bu_type
                          FROM   itl_edw_evt_atmp_unionpay_tran_flow a
                          LEFT   JOIN msl_mpcs_a50ttrncdmap b
