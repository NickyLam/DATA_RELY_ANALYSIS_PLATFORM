/*
Purpose:    集中授权处理中任务数D层-业务分析表(JZ010104):数据来源于营运预警新柜面系统（NCTS）
Author:     Sunline/郑沛隆
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_mcyy_bu_analysis_jz010104_ncts
Createdate: 20210112
Logs:

-- 生成的IDL层表 ：mcyy_bu_analysis
-- 以下为依赖了上游的表 :
-- itl_edw_ncts_ab_auth_tasktabledtl --授权任务流水表
-- 以下为依赖的参数表 :
...
INTO ${idl_schema}.mcyy_bu_analysis
    ...
    --当日数据初始化
     (SELECT t4.index_no
            ...
            , --当日累计
              rank() over(PARTITION BY decode(length(t1.super_org_no), '1', 1, '3', 2, '6', 3) ORDER BY coalesce(t2.handlecount, 0) DESC) AS index_ranking
            , --当前排名
             t4.unit
            , -- 单位
             t4.frequency
      ...
      LEFT   JOIN (SELECT (CASE
                             WHEN substr(i1.txorgno
                                        ,1
                                        ,2) = '89'
                                  OR i1.txorgno = '800001' THEN
                              '000000'
                             WHEN i1.txorgno LIKE '%001'
                                  AND substr(i1.txorgno
                                            ,1
                                            ,2) != '89'
                                  AND i1.txorgno != '800001' THEN
                              substr(i1.txorgno
                                    ,1
                                    ,3)
                             ELSE
                              i1.txorgno
                         END) as txorgno
                        ,COUNT(DISTINCT i1.authserno) AS handlecount
                  
                  FROM   ${itl_schema}.itl_edw_ncts_ab_auth_tasktabledtl i1
                  WHERE  to_char(i1.txdate
                                ,'yyyymmdd') = '${batch_date}'
                  AND    i1.authmodel = '2'
                  AND    i1.authstatus != '3'
                  --AND		 i1.etl_dt = to_date('${batch_date}','yyyymmdd')
                    GROUP  BY (CASE
                                WHEN substr(i1.txorgno
                                           ,1
                                           ,2) = '89'
    ...
    tmp_lm_data AS
     (SELECT t1.index_no
      ...
      ON     t1.index_no = t2.index_no
      AND    t1.org_no = t2.org_no
      AND    t2.etl_dt = trunc(to_date('${batch_date}','yyyymmdd'),'mm') - 1),
    --取上季数据来做月比较类计算
    tmp_lq_data AS
     (SELECT t1.index_no
            , ---- 指标编码
