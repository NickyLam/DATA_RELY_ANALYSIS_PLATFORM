/*
Purpose:    STM交易量/交易额D层-业务分析表(WD040802/WD040803):数据来源于智能网点
Author:     Sunline/郑沛隆
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_mcyy_bu_analysis_stm_bus
Createdate: 20210511
Logs:

-- 生成的IDL层表 ：mcyy_bu_analysis
-- 以下为依赖了上游的表 :
--ITL_EDW_IB_LOG_BUSINESS_LOG
-- 以下为依赖的参数表 :
...
alter table ${idl_schema}.mcyy_bu_analysis truncate subpartition p_${batch_date}_stm_bus;
...
alter table ${idl_schema}.mcyy_bu_analysis add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'))(
                                              subpartition p_${batch_date}_stm_bus values ('STM_BUS')
                                              )
;
alter table ${idl_schema}.mcyy_bu_analysis modify partition p_${batch_date} 
                                             add subpartition p_${batch_date}_stm_bus values ('STM_BUS')
;
    ...
    --当日数据初始化
     (SELECT t4.index_no
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      WHERE  t1.index_no = 'WD040802') dim_tab) t1
      LEFT   JOIN (SELECT (CASE
                  ...
                  FROM   (
SELECT t1.TX_ORG_NUM AS org_no --交易机构号
      ,COUNT(DISTINCT TX_SEQ_NUM) AS sum_count --业务笔数
      ,SUM(TX_AMT) AS sum_amount --业务金额
      ,'STM_YWLX002' AS bu_type
FROM   MSL_NIBS_IB_LOG_BUSINESS_LOG t1
WHERE  T1.APP_NUM = 'SS-STM'
AND    T1.TRANSTATE = 'S'
AND    T1.CHANNELDATE=to_date('${batch_date}','yyyymmdd')
AND    NVL(T1.TX_AMT,0) = 0
GROUP  BY t1.TX_ORG_NUM
UNION ALL
SELECT t1.TX_ORG_NUM AS org_no --交易机构号
      ,COUNT(DISTINCT TX_SEQ_NUM) AS sum_count --业务笔数
      ,SUM(TX_AMT) AS sum_amount --业务金额
      ,'STM_YWLX001' AS bu_type
FROM   MSL_NIBS_IB_LOG_BUSINESS_LOG t1
WHERE  T1.APP_NUM = 'SS-STM'
AND    T1.TRANSTATE = 'S'
AND    T1.CHANNELDATE=to_date('${batch_date}','yyyymmdd')
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      ON     t2.dim_class = t3.dim_class
                      AND    t3.dim_class_name IS NOT NULL
                      WHERE  t1.index_no = 'WD040803') dim_tab) t1
      LEFT   JOIN (SELECT (CASE
                             WHEN substr(org_no
                                        ,1
                                        ,2) = '89'
