/*
Purpose:    流程银行等待中任务数JZ020103-D层业务分析表:数据来源于流程银行PBSS
Author:     Sunline/谢婉宜
...
drop table ${idl_schema}.mbar_jz020103_${batch_date}_tm purge ;

whenever sqlerror exit sql.sqlcode;
create table ${idl_schema}.mbar_jz020103_${batch_date}_tm
...
AND    index_no ='JZ020103'
ORDER  BY sum_end_time,index_no;

 CURSOR CUR_TABLE_LIST IS
   SELECT TABLE_NAME
   			FROM ALL_TABLES
  WHERE TABLE_NAME LIKE UPPER('mbar_jz020103_%')
...
INSERT /*+ append */
INTO ${idl_schema}.mbar_jz020103_${batch_date}_tm
    (etl_dt --数据日期
    ,index_no --指标编码
    ,index_name --指标名称
    ,org_no --机构编码
    ...
    --3、xx分行营运管理部，数据归到分行
     ...
     (
      --集中作业处理中任务数等于总任务数 - 退票数 - 已处理任务数
      SELECT t.org_no AS task_branch_id, t.totaltask AS totaltask
      
      FROM   tmp_run_log
      LEFT   JOIN (SELECT t1.accu_index_value_d - t2.accu_index_value_d AS totaltask
                         ,t1.org_no
                         ,t1.sum_time
                   FROM   ${idl_schema}.mcyy_bu_analysis_realtime t1
                   LEFT   JOIN ${idl_schema}.mcyy_bu_analysis_realtime t2
                   ON     t1.org_no = t2.org_no
                   AND    t1.sum_time = t2.sum_time
                   WHERE  t1.index_no = 'JZ020101'
                   AND    t2.index_no = 'JZ020102') t
      ON     t.sum_time = to_date(tmp_run_log.sum_end_time
                                 ,'yyyymmdd hh24:mi:ss')),
    ...
    temp_now_data AS
     (SELECT t5.etl_dt
            ...
            , --上级机构编码,
             t2.totaltask AS accu_index_value_d --当日累计
...
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        dbms_output.put_line('循环执行实时脚本idl_mcyy_bu_analysis_realtime_jz020103_pbss出错' || SQLERRM);
...
select * from ${idl_schema}.mbar_jz020103_${batch_date}_tm;
