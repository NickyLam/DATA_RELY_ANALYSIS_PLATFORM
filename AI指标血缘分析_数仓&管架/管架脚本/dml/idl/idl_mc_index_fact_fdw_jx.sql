...
select t1.INDEX_NO	                                     --指标编号
      ...
      ,case when t2.unit='%' then round(t1.INDEX_VAL,4) else t1.INDEX_VAL end as INDEX_VAL --指标值
      ,t1.ETL_DT	                                       --ETL处理日期
      ,t1.ETL_TIMESTAMP	 
from
...
union all
select to_char(t1.zbdh)	as index_no                                     --指标编号
      ,t2.jgdh	as org_no                                       --机构编号
      ,null as BIZ_STRIP_LINE_CD	                             --业务条线代码
      ,null as DIM_CD1	                                       --维度代码1
      ,null as DIM_CD2		                                     --维度代码2
      ,null as DIM_CD3		                                     --维度代码3
      ,null as BATCH_FREQ	                                   --批次频度
      ,decode(t1.sdbs,'1','001','2','002','4','004')  as INDEX_MEASURE                   --指标度量
      ,decode(t1.bz,'01','CNY','0A','BWB','BWB')  as CURR_CD                         --币种代码
      ,case when t3.unit='%' then round(t1.ZBZ,4) else t1.ZBZ end as ZBZ  --指标值
      ,t1.ETL_DT	                                       --ETL处理日期
      ,t1.ETL_TIMESTAMP	 
from itl_edw_pams_v_yjzb_jg t1
inner join itl_edw_pams_khdx_jg t2
on t2.khdxdh=t1.khdxdh
  and t2.etl_dt=(case when to_date('${batch_date}','yyyymmdd')<=to_date('20250612','yyyymmdd') then to_date('20250612','yyyymmdd') else to_date('${batch_date}','yyyymmdd') end)
inner join mc_index_define t3
on t3.source_system='绩效系统'
  and nvl(t3.etl_dt,to_date('${batch_date}','yyyymmdd'))<>date'2025-04-29'
  and trim(t1.zbdh) = trim(t3.index_no)
  and t3.index_no_mcs in ('JX02000132','JX02000133','JX02000134','JX02000135')
where t1.etl_dt in (to_date('${batch_date}','yyyymmdd') ,to_date('${batch_date}','yyyymmdd') - 1 ,trunc(to_date('${batch_date}','yyyymmdd'),'mm') -1  ,trunc(add_months(to_date('${batch_date}','yyyymmdd'),-1),'mm') -1 )
  and t1.sdbs in ('1','2','4')
  and t1.tjkj='1'
;
...
select    to_date('${batch_date}','yyyymmdd') as etl_dt
 ...
 where nvl(t1.etl_dt,to_date('${batch_date}','yyyymmdd'))<>date'2025-04-29' 
   and (t1.source_system in ( '财务集市（绩效）')
   ...
   OR (t1.index_no_mcs in ('JX02000132','JX02000133','JX02000134','JX02000135'))
;
 commit;
...
left join mc_index_fact_fdw_temp01_jx t2
      ...
      and t1.CURR_NO    = t2.CURR_NO
      and t1.MEASURE_NO = t2.MEASURE_NO
      and t2.INDEX_NO ='JX01000111'   -- 贷款总额
where t1.INDEX_NO ='JX01000116'
    and t1.ETL_DT = to_date('${batch_date}','yyyymmdd')
    group by t1.INDEX_NO, t1.ORG_NO, t1.ETL_DT, t1.CURR_NO, t1.MEASURE_NO, t1.index_value
;
commit;
...
select  t1.ETL_DT
       ...
       ,coalesce(t6.RATIO_INDEX ,t1.RATIO_INDEX) as  RATIO_INDEX
       ,t1.RATIO_ORG
       ,t1.UNIT as UNIT
       ,t1.FREQUENCY
       ,t1.MEASURE_NO
...
left join ${idl_schema}.mc_index_fact_fdw_temp02_jx t6 --占比
on  t1.INDEX_NO   = t6.INDEX_NO
      and t1.ETL_DT     = t6.ETL_DT
      and t1.ORG_NO     = t6.ORG_NO
      and t1.CURR_NO    = t6.CURR_NO
      and t1.MEASURE_NO = t6.MEASURE_NO
;
commit ;
