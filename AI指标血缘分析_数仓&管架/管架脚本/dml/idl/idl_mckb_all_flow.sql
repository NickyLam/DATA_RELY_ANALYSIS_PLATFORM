/*
Purpose:    全流程看板T+1:数据来源于综合信贷系统
Author:     Sunline/郑沛隆
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_mckb_all_flow
Createdate: 20250213
Logs:
-- 模块 
--第1组：业绩数据
...
-- 新增 产品分类编号 字段 
LP_CLS_PROD 1 是IPC/2 是数据贷/0 是合计
-- 新增 字段 
PREP_TEL_CNT         -- 待电调
PREP_ACTL_CNT        -- 待实调
...
个人-续贷   203
个人-续贷复议 204 
-- 20260303 修改 IPC 审批金额及审批户数 口径 按照蔡浪威及李旻翀的确认
*/
set timing on
-- 0.1 alter parallel
...
drop table ${idl_schema}.mckb_all_flow_tmp_01 purge ;
drop table ${idl_schema}.mckb_all_flow_tmp_02 purge ;
drop table ${idl_schema}.mckb_all_flow_tmp_03 purge ;
drop table ${idl_schema}.mckb_all_flow_tmp_04 purge ;
drop table ${idl_schema}.mckb_all_flow_tmp_05 purge ;
-- 20260112 新增 
drop table ${idl_schema}.mckb_all_flow_tmp_06 purge ;
drop table ${idl_schema}.mckb_all_flow_tmp_07 purge ;
drop table ${idl_schema}.mckb_all_flow_tmp_08 purge ;
drop table ${idl_schema}.mckb_all_flow_tmp_09 purge ;
drop table ${idl_schema}.mckb_all_flow_tmp_10 purge ;
drop table ${idl_schema}.mckb_all_flow_tmp_11 purge ;
drop table ${idl_schema}.mckb_all_flow_tmp_12 purge ;
drop table ${idl_schema}.mckb_all_flow_tmp_result purge ;

alter table ${idl_schema}.mckb_all_flow add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

whenever sqlerror exit sql.sqlcode; 
create table  ${idl_schema}.mckb_all_flow_tmp_result compress
AS 
select * from ${idl_schema}.mckb_all_flow
where 1=2 ;

--第1组：业绩数据
create table ${idl_schema}.mckb_all_flow_tmp_01 as (   
       --累计
        select 
...
--全行汇总          
INSERT INTO ${idl_schema}.mckb_all_flow_tmp_01
            SELECT 
                    ETL_DT                        AS ETL_DT
                    ,PED_NO                       AS PED_NO
                    ...
                    ,LP_CLS_PROD                  AS LP_CLS_PROD -- 产品分类编号
            FROM    MCKB_ALL_FLOW_TMP_01 
            GROUP BY PED_NO
                    ,PED_NAME
                    ,LP_CLS_ID
                    ,LP_CLS_NAME
...
INSERT INTO ${idl_schema}.mckb_all_flow_tmp_01
            SELECT 
                   ...
                   ,T1.ORG_NO                       AS ORG_NO
                   ,SUM(T1.ACVMNT_DATA_ACCT_CNT)    AS ACVMNT_DATA_ACCT_CNT     -- 业绩数据户数
                   ,SUM(T1.ACVMNT_DATA_AMT)         AS ACVMNT_DATA_AMT          -- 业绩数据金额
                   ,SUM(T1.ACVMNT_DATA_NET_INCREMT) AS ACVMNT_DATA_NET_INCREMT  -- 累计净增就是当前的余额
                   ,'0'                             AS LP_CLS_ID
                   ,'合计'                          AS LP_CLS_NAME
                   ,LP_CLS_PROD                     AS LP_CLS_PROD
           FROM     MCKB_ALL_FLOW_TMP_01 T1
           GROUP BY T1.PED_NO
                   ,T1.PED_NAME
                   ,T1.ETL_DT
                   ,T1.ORG_NO
...
--第2组：进件/审批通过数据
-- 20260308 IPC 新增 201基线逻辑 
create table ${idl_schema}.mckb_all_flow_tmp_02 as (
               ...
               --累计
                select      
                 ...
                 inner join msl_hgls_loan_branch_website t2
                   ...
                   and t2. ISDEL = 0
                 where t1.isdel = 0
                   and t1.PRD_TYPE in ('18','32') --18个人标准产品,22企业,32个人特色产品/201 基线
                 ...
                 inner join msl_hgls_loan_branch_website t2
                   ...
                   and t2. ISDEL = 0
                 where t1.isdel = 0
                   and t1.PRD_TYPE in ('201') --18个人标准产品,22企业,32个人特色产品/201 基线
                   and trunc(t1.req_date)<=to_date('${batch_date}','yyyymmdd')
                 group by t2.org_num
                 ...
                 inner join msl_hgls_loan_branch_website t2
                   ...
                   and t2. ISDEL = 0
                 where t1.isdel = 0
                   and t1.PRD_TYPE in ('201') --18个人标准产品,22企业,32个人特色产品
                   and trunc(t1.req_date)=to_date('${batch_date}','yyyymmdd')
                 group by t2.org_num
                  union all
                 --月
                 ...
                 inner join msl_hgls_loan_branch_website t2
                   ...
                   and t2.ISDEL = 0
                 where t1.isdel = 0
                   and t1.PRD_TYPE in ('18','32') --18个人标准产品,22企业,32个人特色产品
                ...
                select      
                 ...
                 inner join msl_hgls_loan_branch_website t2
                   ...
                   and t2. ISDEL = 0
                 where t1.isdel = 0
                   and t1.PRD_TYPE ='22' --18个人标准产品,22企业,32个人特色产品
                   and trunc(t1.req_date)<=to_date('${batch_date}','yyyymmdd')
                 group by t2.org_num
                 union all
                 --日
...
--全行汇总          
INSERT INTO ${idl_schema}.mckb_all_flow_tmp_02
            SELECT 
                   ETL_DT                 AS ETL_DT
                   ,PED_NO                AS PED_NO
                   ,PED_NAME              AS PED_NAME
...
INSERT INTO ${idl_schema}.mckb_all_flow_tmp_02
     ...
     commit;
--第3组：进件通过数
create table ${idl_schema}.mckb_all_flow_tmp_03 as (
--累计
select 
        to_date('${batch_date}','yyyymmdd') as etl_dt
       ,'099' as ped_no
...
--全行汇总          
INSERT INTO ${idl_schema}.mckb_all_flow_tmp_03
        SELECT 
                 ETL_DT               AS ETL_DT
                 ,PED_NO              AS PED_NO
                 ,PED_NAME            AS PED_NAME
...
INSERT INTO ${idl_schema}.mckb_all_flow_tmp_03
     ...
     commit;
--第4组：进件拒绝数
create table ${idl_schema}.mckb_all_flow_tmp_04 as (
--累计
select 
to_date('${batch_date}','yyyymmdd') as etl_dt
       ,'099' as ped_no
...
--全行汇总          
INSERT INTO ${idl_schema}.mckb_all_flow_tmp_04
        SELECT 
                 ETL_DT                 AS ETL_DT
                 ,PED_NO                AS PED_NO
                 ,PED_NAME              AS PED_NAME
...
INSERT INTO ${idl_schema}.mckb_all_flow_tmp_04
     ...
     COMMIT;
--第5组：系统审批金额
-- 20260308  按照蔡浪威 修改 IPC 口径，201基线的状态应该为112
create table ${idl_schema}.mckb_all_flow_tmp_05 as (
        SELECT 
              to_date('${batch_date}','yyyymmdd') as etl_dt
              ,T3.PED_NO                          AS PED_NO
              ,T3.PED_NAME                        AS PED_NAME
        ...
        FROM (
              ...
              left join (select lra.loan_id
                          ...
                          inner join (select loan_id
                                      ...
                                      where lra2.audit_status = '5' -- 贷款调查通过
             ...
             where lr.isdel = 0
                   ...
                   and lr.audit_status in (4, 6) -- 4人工质检通过/6人工审批通过
                   and trunc(lr.req_date)<=to_date('${batch_date}','yyyymmdd')
             group by lbw.org_num
             UNION ALL 
             select to_date('${batch_date}','yyyymmdd') as etl_dt
             ...
             where lr.isdel = 0
                   ...
                   and lr.audit_status in (112) -- 112 信息补录完成
                   and trunc(lr.req_date)<=to_date('${batch_date}','yyyymmdd')
             group by lbw.org_num
             ...
             where lr.isdel = 0
                   and lr.prd_type in (201)  -- 18个人标准产品,32个人特色产品/201 基线 /22企业
                   and lr.audit_status in (112) -- 信息补录完成 
                   and trunc(lr.req_date)=to_date('${batch_date}','yyyymmdd')
             group by lbw.org_num
             union all
             --月
...
--全行汇总          
INSERT INTO ${idl_schema}.mckb_all_flow_tmp_05
          SELECT 
                 ETL_DT            AS ETL_DT
                 ,PED_NO           AS PED_NO
                 ,PED_NAME         AS PED_NAME
...
--第6组：调查 待电调/待实调
create table ${idl_schema}.mckb_all_flow_tmp_06 as (
         -- 累计 IPC 
        SELECT      
                TO_DATE('${batch_date}','yyyymmdd') AS ETL_DT
                ,'099'                              AS PED_NO
 ...
 commit;
--全行汇总          
        INSERT INTO ${idl_schema}.mckb_all_flow_tmp_06
            SELECT 
                    ETL_DT             AS ETL_DT
                   ,PED_NO             AS PED_NO
                   ,PED_NAME           AS PED_NAME
...
--第7组：调查通过
CREATE TABLE ${idl_schema}.MCKB_ALL_FLOW_TMP_07 AS (
         --累计 IPC
        SELECT   TO_DATE('${batch_date}','yyyymmdd') AS ETL_DT
                ,'099'                               AS PED_NO
                ,'累计'                              AS PED_NAME
...
--全行汇总          
        INSERT INTO ${idl_schema}.MCKB_ALL_FLOW_TMP_07
            SELECT 
                    ETL_DT             AS ETL_DT
                   ,PED_NO             AS PED_NO
                   ,PED_NAME           AS PED_NAME
...
--第8组：调查拒绝
CREATE TABLE ${idl_schema}.MCKB_ALL_FLOW_TMP_08 AS (
         -- 累计 IPC
        -- 1.准入和初审在查询时间区间内通过
        -- 2.进件当前状态为拒绝
        -- 3.没有调查通过记录或已被打回
...
;
 commit;
--全行汇总          
        INSERT INTO ${idl_schema}.MCKB_ALL_FLOW_TMP_08
            SELECT 
                    ETL_DT             AS ETL_DT
                   ,PED_NO             AS PED_NO
                   ,PED_NAME           AS PED_NAME
...
--第9组：质检通过 
CREATE TABLE ${idl_schema}.MCKB_ALL_FLOW_TMP_09 AS (
        -- 累计 IPC
        SELECT    
                      TO_DATE('${batch_date}','yyyymmdd') AS ETL_DT
                     ,'099'                               AS PED_NO
...
commit;
--全行汇总          
        INSERT INTO ${idl_schema}.MCKB_ALL_FLOW_TMP_09
            SELECT 
                    ETL_DT             AS ETL_DT
                   ,PED_NO             AS PED_NO
                   ,PED_NAME           AS PED_NAME
...
--第10组：质检流程中 
CREATE TABLE ${idl_schema}.MCKB_ALL_FLOW_TMP_10 AS (
        -- 累计 IPC 
        SELECT    
                     TO_DATE('${batch_date}','yyyymmdd') AS ETL_DT
                    ,'099'                               AS PED_NO
...
;
 commit;
--全行汇总          
        INSERT INTO ${idl_schema}.MCKB_ALL_FLOW_TMP_10
            SELECT 
                    ETL_DT              AS ETL_DT
                   ,PED_NO              AS PED_NO
                   ,PED_NAME            AS PED_NAME
...
--第11组：签约客户数
CREATE TABLE ${idl_schema}.MCKB_ALL_FLOW_TMP_11 AS (
...
;
 ...
 commit;
--全行汇总          
        INSERT INTO ${idl_schema}.MCKB_ALL_FLOW_TMP_11
            SELECT 
                    ETL_DT              AS ETL_DT
                   ,PED_NO              AS PED_NO
                   ,PED_NAME            AS PED_NAME
...
CREATE TABLE ${idl_schema}.MCKB_ALL_FLOW_TMP_12 AS (
...
;
 ...
 commit;
--全行汇总          
        INSERT INTO ${idl_schema}.MCKB_ALL_FLOW_TMP_12
            SELECT 
                    ETL_DT              AS ETL_DT
                   ,PED_NO              AS PED_NO
                   ,PED_NAME            AS PED_NAME
...
INSERT /*+ append */INTO ${idl_schema}.mckb_all_flow_tmp_result
    (ETL_DT      -- 数据日期
    ,PED_NO      -- 周期编号
    ,PED_NAME    -- 周期名称
    ,GROUPING    -- 分组
    ...
    ,ETL_TIMESTAMP --ETL处理时间戳
     ...
     SELECT to_date('${batch_date}','yyyymmdd') as etl_dt --数据日期
            ...
            ,CASE WHEN T1.PED_NO='099' THEN NVL(T2.ACVMNT_DATA_NET_INCREMT,0)
                  ELSE NVL(T9.ACVMNT_DATA_NET_INCREMT,0)-NVL(T8.ACVMNT_DATA_NET_INCREMT,0) END AS ACVMNT_DATA_NET_INCREMT --业绩数据净增
        ...
        LEFT JOIN MCKB_ALL_FLOW_TMP_01 T9
...
alter table ${idl_schema}.mckb_all_flow truncate partition p_${batch_date} reuse storage;

-- 3.3 exchage tm table and target table
alter table ${idl_schema}.mckb_all_flow exchange partition p_${batch_date} with table mckb_all_flow_tmp_result;

-- 4.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${idl_schema}.mckb_all_flow to ${idl_schema};
...
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'mckb_all_flow', partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);
