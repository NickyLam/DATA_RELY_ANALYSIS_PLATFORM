/*
Purpose:    对公开户数D层-业务分析表(WD030101):数据来源于核心系统（CBSS）,包括电子账户（IATS）
Author:     Sunline/郑沛隆
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_mcyy_bu_analysis_wd030101_cbss
Createdate: 20210112
Logs:
...
alter table ${idl_schema}.mcyy_bu_analysis truncate subpartition p_${batch_date}_cbss;
...
alter table ${idl_schema}.mcyy_bu_analysis add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'))(
                                              subpartition p_${batch_date}_cbss values ('CBSS')
                                              )
;
alter table ${idl_schema}.mcyy_bu_analysis modify partition p_${batch_date} 
                                             add subpartition p_${batch_date}_cbss values ('CBSS')
;
    ...
    --当日数据初始化
     (SELECT t4.index_no
      ...
      LEFT   JOIN (SELECT SUM(index_value) AS index_value
                  ...
                  WHERE  E.custtp NOT IN ('1','3','7') --对公标识
                  AND    a.opendt = '${batch_date}'
                  AND    a.accstp = '0'
                  AND    d.acctno is not null
                  and    a.acctno not like '34%' --剔除大额存单
