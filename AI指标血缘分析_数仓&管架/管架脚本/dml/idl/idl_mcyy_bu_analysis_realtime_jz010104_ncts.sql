/*
Purpose:    集中授权处理中(处理中、等待中)任务数D层-业务分析实时表(JZ010104):数据来源于营运预警新柜面系统（NCTS）
Author:     Sunline/郑沛隆
...
drop table ${idl_schema}.mbar_jz010104_${batch_date}_tm purge ;

whenever sqlerror exit sql.sqlcode;
create table ${idl_schema}.mbar_jz010104_${batch_date}_tm
...
AND    index_no ='JZ010104'
ORDER  BY sum_end_time,index_no;

 CURSOR CUR_TABLE_LIST IS
   SELECT TABLE_NAME
   			FROM ALL_TABLES
  WHERE TABLE_NAME LIKE UPPER('mbar_jz010104_%')
...
INSERT /*+ append */
INTO ${idl_schema}.mbar_jz010104_${batch_date}_tm
    (etl_dt --数据日期
    ,index_no --指标编码
    ,index_name --指标名称
    ,org_no --机构编码
    ...
    --3、xx分行营运管理部，数据归到分行
     ...
     /*(SELECT COUNT(DISTINCT t1.authserno) AS handlecount
            ,(CASE
      ...
      GROUP  BY (CASE
                ...
                END)),*/
      (
    SELECT COUNT(DISTINCT t1.PRINAME) AS handlecount
            ,(CASE
...
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        dbms_output.put_line('循环执行实时脚本idl_mcyy_bu_analysis_realtime_jz010104_ncts出错' || SQLERRM);
...
select * from ${idl_schema}.mbar_jz010104_${batch_date}_tm;
