/*
Purpose:    高中低风险问题单笔数合计-D层人员及风险分析表:数据来源于营运预警ORWS
Author:     Sunline/谢婉宜
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_mcyy_human_risk_orws_fx000102
Createdate: 20210426
Logs:
--1、89开头的虚拟机构，数据归纳到总行
...
alter table ${idl_schema}.mcyy_human_risk truncate subpartition p_${batch_date}_orws1;
commit;

-- 2.2 add today partition
whenever sqlerror continue none;
alter table ${idl_schema}.mcyy_human_risk add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'))(
                                              subpartition p_${batch_date}_orws1 values ('ORWS1')
                                             )
;
alter table ${idl_schema}.mcyy_human_risk modify partition p_${batch_date} 
                                             add subpartition p_${batch_date}_orws1 values ('ORWS1')
;
