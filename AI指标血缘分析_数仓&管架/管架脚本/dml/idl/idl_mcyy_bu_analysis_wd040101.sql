...
Author:     Sunline/郑沛隆
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_mcyy_bu_analysis_wd040101
Createdate: 20210118
Logs:
...
alter table ${idl_schema}.mcyy_bu_analysis add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'))(
                                              subpartition p_${batch_date}_upps values ('UPPS')
                                              )
;
alter table ${idl_schema}.mcyy_bu_analysis modify partition p_${batch_date} 
                                             add subpartition p_${batch_date}_upps values ('UPPS')
;
...
INTO ${idl_schema}.mcyy_bu_analysis
    ...
    --当日数据初始化
     (SELECT t4.index_no
      ...
      LEFT   JOIN (SELECT SUM(sum_count) AS sum_count
                        ,(CASE
