/*
Purpose:    集中授权已处理任务数D层-业务分析表(JZ010102):数据来源于营运预警新柜面系统（NCTS）
Author:     Sunline/郑沛隆
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_mcyy_bu_analysis_jz010102_ncts
Createdate: 20210112
Logs:
...
INTO ${idl_schema}.mcyy_bu_analysis
    ...
    --当日数据初始化
     (SELECT t4.index_no
            ...
            , --当日累计             
             rank() over(PARTITION BY decode(length(t1.super_org_no), '1', 1, '3', 2, '6', 3) ORDER BY coalesce(t2.passcount, 0) DESC) AS index_ranking
            , --当前排名
             t4.unit
            , -- 单位
             t4.frequency
      ...
      LEFT   JOIN (SELECT (CASE
                             ...
                             ELSE
                              i1.txorgno
                         END) as txorgno
                        ,COUNT(DISTINCT i1.authserno) AS passcount
