/*
Purpose:    超级网银 交易量/交易额D层-业务分析实时表(WD041201/WD041202):数据来源于中台
Author:     Sunline/谢婉宜
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_mcyy_bu_analysis_super_olbk
Createdate: 20210527
Logs:

-- 生成的IDL层表 ：mcyy_bu_analysis
-- 以下为依赖了上游的表 :
-- itl_edw_mpcs_a10tibpsdetaillog 
-- 以下为依赖的参数表 :
...
alter table ${idl_schema}.mcyy_bu_analysis truncate subpartition p_${batch_date}_super_olbk;
...
alter table ${idl_schema}.mcyy_bu_analysis add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'))(
                                              subpartition p_${batch_date}_super_olbk values ('SUPER_OLBK')
                                              )
;
alter table ${idl_schema}.mcyy_bu_analysis modify partition p_${batch_date} 
                                             add subpartition p_${batch_date}_super_olbk values ('SUPER_OLBK')
;
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      WHERE  t1.index_no = 'WD041201') dim_tab) t1
...
FROM   (SELECT a.open_acct_brac_id AS org_no
      ,COUNT(1) AS sum_count
      ,'CJWY_YWLX002' AS bu_type
FROM   itl_edw_evt_super_olbk_tran_evt a
WHERE  ((a.pbc_tran_code IN ('ibps.101.001.02'
                            ,'ibps.101.001.01') AND
       a.present_wdraw_flg IN ('0'
                                ,'1')) OR
       (a.pbc_tran_code IN ('ibps.103.001.02'
                            ,'ibps.101.003.01') AND
       a.present_wdraw_flg IN ('0'
                                ,'1')))
AND    a.pbc_bus_status_cd IN ('PR03'
                              ,'PR04')
AND    a.send_bank_dt = to_date('${batch_date}'
                               ,'yyyymmdd')
AND    a.tran_amt = 0
GROUP  BY a.open_acct_brac_id
UNION ALL
SELECT a.open_acct_brac_id AS org_no
      ,COUNT(1) AS sum_count
      ,'CJWY_YWLX001' AS bu_type
FROM   itl_edw_evt_super_olbk_tran_evt a
WHERE  ((a.pbc_tran_code IN ('ibps.101.001.02'
                            ,'ibps.101.001.01') AND
       a.present_wdraw_flg IN ('0'
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      ON     t2.dim_class = t3.dim_class
                      AND    t3.dim_class_name IS NOT NULL
                      WHERE  t1.index_no = 'WD041202') dim_tab) t1
      LEFT   JOIN (SELECT (CASE
           WHEN substr(org_no
                      ,1
                      ,2) = '89'
