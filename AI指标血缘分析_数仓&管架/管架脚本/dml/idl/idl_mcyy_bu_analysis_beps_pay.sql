/*
Purpose:    小额支付交易量/交易额D层-业务分析实时表(WD040401/WD040402):数据来源于中台
Author:     Sunline/谢婉宜
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_mcyy_bu_analysis_beps_pay
Createdate: 20210527
Logs:

-- 生成的IDL层表 ：mcyy_bu_analysis
-- 以下为依赖了上游的表 :
-- itl_edw_mpcs_a08tbetrx 
-- 以下为依赖的参数表 :
...
alter table ${idl_schema}.mcyy_bu_analysis truncate subpartition p_${batch_date}_beps_pay;
...
alter table ${idl_schema}.mcyy_bu_analysis add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'))(
                                              subpartition p_${batch_date}_beps_pay values ('BEPS_PAY')
                                              )
;
alter table ${idl_schema}.mcyy_bu_analysis modify partition p_${batch_date} 
                                             add subpartition p_${batch_date}_beps_pay values ('BEPS_PAY')
;
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      WHERE  t1.index_no = 'WD040401') dim_tab) t1
...
FROM   (SELECT a.proc_org_id AS org_no
      ,COUNT(1) AS sum_count
      ,'DXE_YWLX002' AS bu_type
FROM   itl_edw_evt_beps_tran_evt a
WHERE  ((a.scd_gener_msg_type_id IN
       ...
       a.nostro_flg IN ('0'
                         ,'1')) OR
       (a.scd_gener_msg_type_id IN ('beps.127.001.01') AND
       a.nostro_flg = '1'))
AND    a.proc_status_cd IN ('6'
                           ,'7'
                           ,'1'
                           ,'T'
                           ,'O'
...
AND    a.tran_amt = 0
GROUP  BY a.proc_org_id
UNION ALL
SELECT a.proc_org_id AS org_no
      ,COUNT(1) AS sum_count
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      ON     t2.dim_class = t3.dim_class
                      AND    t3.dim_class_name IS NOT NULL
                      WHERE  t1.index_no = 'WD040402') dim_tab) t1
      LEFT   JOIN (SELECT (CASE
                             WHEN substr(org_no
                                        ,1
                                        ,2) = '89'
