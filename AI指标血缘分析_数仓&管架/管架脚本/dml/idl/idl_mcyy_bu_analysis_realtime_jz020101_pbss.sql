/*
Purpose:    流程银行总任务数JZ020101-D层业务分析表:数据来源于流程银行SCPS
Author:     Sunline/郑沛隆
...
drop table ${idl_schema}.mbar_jz020101_${batch_date}_tm purge ;

whenever sqlerror exit sql.sqlcode;
create table ${idl_schema}.mbar_jz020101_${batch_date}_tm
...
AND    index_no ='JZ020101'
ORDER  BY sum_end_time,index_no;

 CURSOR CUR_TABLE_LIST IS
   SELECT TABLE_NAME
   			FROM ALL_TABLES
  WHERE TABLE_NAME LIKE UPPER('mbar_jz020101_%')
...
INSERT /*+ append */
INTO ${idl_schema}.mbar_jz020101_${batch_date}_tm
    (etl_dt --数据日期
    ,index_no --指标编码
    ,index_name --指标名称
    ,org_no --机构编码
    ...
    tmp_initza_data AS
      ...
      --3、xx分行营运管理部，数据归到分行
     (SELECT COUNT(DISTINCT a.task_id) AS totaltask, (CASE
...
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        dbms_output.put_line('循环执行实时脚本idl_mcyy_bu_analysis_realtime_jz020101_pbss出错' || SQLERRM);
...
select * from ${idl_schema}.mbar_jz020101_${batch_date}_tm;
