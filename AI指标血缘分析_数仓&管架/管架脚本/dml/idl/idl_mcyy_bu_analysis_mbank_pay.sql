/*
Purpose:    手机银行交易量/交易额D层-业务分析实时表(WD040701/WD040702):数据来源于中台
Author:     Sunline/郑沛隆
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_mcyy_bu_analysis_mbank_pay
Createdate: 20210601
Logs:
...
-- itl_edw_cmm_ponl_bk_sign_info
-- itl_edw_ref_tran_bank_code_para
...
alter table ${idl_schema}.mcyy_bu_analysis truncate subpartition p_${batch_date}_mbank_pay;
...
alter table ${idl_schema}.mcyy_bu_analysis add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'))(
                                              subpartition p_${batch_date}_mbank_pay values ('MBANK_PAY')
                                              )
;
alter table ${idl_schema}.mcyy_bu_analysis modify partition p_${batch_date} 
                                             add subpartition p_${batch_date}_mbank_pay values ('MBANK_PAY')
;
    ...
    tmp_mbank_vlm_bus AS
     (
      ...
      AND    (
            ...
            --转账汇款 立即收钱、电子零钱包充值、分享转账、普通转出、普通转出多笔拆包、售汇、结汇、电子零钱包提现、主动归集
            
             t1.tran_code IN ('NMBMPSX1006'
                              ,'NMBMPSX1009'
                              ,'NMBMPSX1011'
                              ,'NMBMPSX1013'
                              ,'NMBMPSX1014'
            ...
            --投资理财 手机理财理财产品购买、理财产品实时赎回、大额存单认购、智能储蓄产品申购、财富宝提前支取、智能储蓄提前支取、个人定期通知全额支取、个人定期通知部分支取、基金购买、基金赎回、基金赎回、大额存单修改、大额存单提前支取、七兴宝产品购买、七兴宝产品赎回、大额存单认购、新单承保、私募下单、新单承保(新)、新单承保（旧）、新单承保(新预留)、个人智能存款(新)、个人智能存款提取(新)                 
             OR t1.tran_code IN ('NMBMPSX1017'
                                 ,'NMBMPSX1018'
                                 ,'NMBMPSX1019'
                                 ,'NMBMPSX1020'
                                 ,'NMBMPSX1022'
      ...
      LEFT   JOIN itl_edw_cmm_ponl_bk_sign_info t2
      ON     (t1.unify_cust_id = t2.cust_id)
      LEFT   JOIN itl_edw_ref_tran_bank_code_para t3
      ON     (t1.tran_order = t3.tran_code)
      LEFT   JOIN TMP_CUST_BASIC_INFO t4
      ON     t4.CUST_ID = t1.unify_cust_id
      WHERE  (t1.parent_flow_id = t1.flow_num)
      AND    t1.tran_order <> 'batchTransAuthResult'
      AND    t1.tran_chn_cd = 'TBM'
      AND    t3.fin_tran_flg = '0'
      AND    t1.tran_dt = to_date('${batch_date}'
                                 ,'yyyymmdd')
      GROUP  BY t4.BELONG_ORG_ID
      /*union all
      select count(1) 交易笔数
        from cbsprd.cpr_login_flow@link_ts_ibs a  --cpr_login_flow
       where a.clf_channel = 'TBM'
         and clf_state = '0'
         and a.clf_date >= '20210101'
         and clf_date <= '20210331'*/
      UNION ALL
      --个人手机银行资金交易量
      SELECT t2.BELONG_ORG_ID AS org_no
            ,COUNT(1) AS sum_count
            ,'SJYH_YWLX001' AS bu_type
      FROM   itl_edw_evt_priv_onl_bank_tran_flow t1
      LEFT   JOIN TMP_CUST_BASIC_INFO t2
      ON     t2.CUST_ID = t1.whole_unify_cust_id
      WHERE  t1.tran_dt = to_date('${batch_date}'
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      WHERE  t1.index_no = 'WD040701') dim_tab) t1
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      ON     t2.dim_class = t3.dim_class
                      AND    t3.dim_class_name IS NOT NULL
                      WHERE  t1.index_no = 'WD040702') dim_tab) t1
      LEFT   JOIN (SELECT (CASE
                             WHEN substr(org_no
                                        ,1
                                        ,2) = '89'
