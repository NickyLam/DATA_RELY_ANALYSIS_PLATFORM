...
drop table ${idl_schema}.mbar_wd040102_${batch_date}_tm purge ;

whenever sqlerror exit sql.sqlcode;
create table ${idl_schema}.mbar_wd040102_${batch_date}_tm
...
AND    index_no ='WD040102'
ORDER  BY sum_end_time,index_no;

 CURSOR CUR_TABLE_LIST IS
   SELECT TABLE_NAME
   			FROM ALL_TABLES
  WHERE TABLE_NAME LIKE UPPER('mbar_wd040102_%')
...
INSERT /*+ append */
INTO ${idl_schema}.mbar_wd040102_${batch_date}_tm
    (etl_dt --数据日期
    ,index_no --指标编码
    ,index_name --指标名称
    ,org_no --机构编码
    ...
    --3、xx分行营运管理部，数据归到分行
     (SELECT SUM(t1.sum_amount) AS account_sum
            ,(CASE
      ...
      LEFT   JOIN (
                  ...
                  SELECT t1.brnnbr AS org_no --交易机构号
                         ,SUM(settlmtamt) AS sum_amount --业务金额
                  ...
                  SELECT t1.trans_org_no AS org_no --交易机构号
      						...
      						,t2.create_time AS from_date
  								FROM ${msl_schema}.msl_ppps_t_txn_credit t2
 										WHERE t2.status = 'SUCCESS' AND trunc(t2.create_time) = to_date('${batch_date}','yyyymmdd')
 										GROUP BY t2.trans_org_no ,t2.create_time
		  ) t1
...
select * from ${idl_schema}.mbar_wd040102_${batch_date}_tm 
...
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        dbms_output.put_line('循环执行实时脚本idl_mcyy_bu_analysis_realtime_wd040102出错' || SQLERRM);
