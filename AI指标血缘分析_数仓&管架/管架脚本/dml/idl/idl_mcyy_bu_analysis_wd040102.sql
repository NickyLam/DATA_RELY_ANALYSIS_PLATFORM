/*
Purpose:    支付业务交易额D层-业务分析实时表(WD040102):数据来源于统一支付、银联
Author:     Sunline/郑沛隆
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_mcyy_bu_analysis_WD040102
Createdate: 20210118
Logs:
...
-- 以下为依赖了上游的表 :
-- itl_mpcs_a50ubcardjour -- ATMP/银联前置交易流水表 itl_edw_evt_atmp_unionpay_tran_flow
-- itl_edw_upps_order_header --  订单信息表
-- itl_edw_upps_t_txn_credit -- 平台贷记类交易交互流水表
-- 以下为依赖的参数表 :
...
--alter table ${idl_schema}.mcyy_bu_analysis truncate subpartition p_${batch_date}_upps;
...
INTO ${idl_schema}.mcyy_bu_analysis
    ...
    --当日数据初始化
     (SELECT t4.index_no
      ...
      LEFT   JOIN (SELECT SUM(sum_amount) AS sum_amount
                        ,(CASE
                  ...
                  FROM   (
                          ...
                          --银联
                          SELECT t1.tran_org_id AS org_no --交易机构号
                                 ,SUM(t1.tran_amt) AS sum_amount --业务金额
                          FROM   ${itl_schema}.itl_edw_evt_atmp_unionpay_tran_flow t1
                          WHERE  t1.tran_status_cd IN ('1'
                                                      ,'2'
                                                      ,'9')
                         -- AND    t1.etl_dt = to_date('${batch_date}'
                          --                          ,'YYYYMMDD')
                          AND    substr(tran_tm, 1,6) = '${batch_date}'
                          AND    substr(t1.intnal_tran_cd
                                       ,7
                                       ,1) <> 'C'
                          GROUP  BY t1.tran_org_id
                          ...
                          FROM   ${itl_schema}.itl_edw_upps_order_header t1
                          WHERE  t1.status_id = 'ORDER_COMPLETED'
                          --AND    t1.etl_dt =to_date('${batch_date}','yyyymmdd')
                          AND    to_char(t1.created_stamp
                                        ,'yyyymmdd') = '${batch_date}'
                          GROUP  BY t1.product_store_id
                          UNION ALL
                          SELECT t2.trans_org_no AS org_no --交易机构号
                                 
                                ,SUM(t2.amount) AS sum_amount --业务金额                   
                          FROM   ${itl_schema}.itl_edw_upps_t_txn_credit t2
                          WHERE  t2.status = 'SUCCESS'
                          --AND    t2.etl_dt =to_date('${batch_date}','yyyymmdd')
                          AND    t2.trade_type = 'TRANSFER'
                          AND    t2.txn_date = '${batch_date}'
                          GROUP  BY t2.trans_org_no) t1
                  GROUP  BY (CASE
                                WHEN substr(t1.org_no
                                           ,1
                                           ,2) = '89'
                                ...
                                ELSE
                                 t1.org_no
                            END)) t2
      ON     t1.org_no = t2.txorgno
      INNER  JOIN ${idl_schema}.mcyy_index_define t4
      ON     'WD040102' = t4.index_no_mcs),
    --取上日数据来做日比较类计算
    tmp_ld_data AS
     (SELECT t1.index_no
            , ---- 指标编码
