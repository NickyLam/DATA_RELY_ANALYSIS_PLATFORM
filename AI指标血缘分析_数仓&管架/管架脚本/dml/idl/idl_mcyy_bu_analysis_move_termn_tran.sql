/*
Purpose:    移动终端PAD交易量/交易额D层-业务分析实时表(WD040901/WD040902):数据来源于移动作业平台
Author:     Sunline/郑沛隆
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_mcyy_bu_analysis_move_termn_tran
Createdate: 20210525
Logs:
...
MSL_NIBS_IB_UPM_MENU_INFO
--下面的表都废弃：
-- itl_edw_mmps_mobile_sign_submit
--ITL_EDW_MMPS_mmp_mobile_client_sign
--itl_edw_mmps_mmp_volume_active
--itl_edw_mmps_mmp_reset_password
--itl_edw_mmps_mmp_change_password
--itl_edw_mmps_mmp_certificateloss
--itl_edw_mmps_mmp_individuallossrepeal
-- 以下为依赖的参数表 :
...
alter table ${idl_schema}.mcyy_bu_analysis truncate subpartition p_${batch_date}_move_termn_tran;
...
alter table ${idl_schema}.mcyy_bu_analysis add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'))(
                                              subpartition p_${batch_date}_move_termn_tran values ('MOVE_TERMN_TRAN')
                                              )
;
alter table ${idl_schema}.mcyy_bu_analysis modify partition p_${batch_date} 
                                             add subpartition p_${batch_date}_move_termn_tran values ('MOVE_TERMN_TRAN')
;
     ...
     (SELECT t4.index_no
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      WHERE  t1.index_no = 'WD040901') dim_tab) t1
      LEFT   JOIN (SELECT (CASE
                             ...
                             ELSE
                              org_no
                         END) AS org_no
                        ,'PAD_YWLX002' AS bu_type
                        ,sum(index_value) sum_count
                  FROM   (
                  SELECT tx_org_num as org_no, count(distinct b.CORE_TRAN_FLOW_NUM) as index_value
  									FROM MSL_NIBS_IB_LOG_BUSINESS_LOG b
 										INNER JOIN MSL_NIBS_IB_UPM_MENU_INFO m
    									ON b.CHANNELTRANCODE = m.MENUNUM
 										WHERE b.APP_NUM = 'MT' AND m.TRANFLAG in ('0', '2') AND b.ISAGENT = 'N' AND
       								b.TRANSTATE = 'S' and p_workdate = to_date('${batch_date}','yyyymmdd') and nvl(tx_amt, 0) <= 0 group by tx_org_num)
                  GROUP  BY (CASE
                                WHEN substr(org_no
                                           ,1
                                           ,2) = '89'
...
,(CASE
          ...
          ,'MOVE_TERMN_TRAN' source_sys --来源系统
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      ON     t2.dim_class = t3.dim_class
                      AND    t3.dim_class_name IS NOT NULL
                      WHERE  t1.index_no = 'WD040902') dim_tab) t1
      LEFT   JOIN (SELECT (CASE
                             WHEN substr(org_no
                                        ,1
                                        ,2) = '89'
