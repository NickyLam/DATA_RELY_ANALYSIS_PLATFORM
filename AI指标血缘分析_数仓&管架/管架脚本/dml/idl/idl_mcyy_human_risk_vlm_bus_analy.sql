/*
Purpose:    产能、业务量分析D层-业务分析表:营运预警
Author:     Sunline/郑沛隆
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_mcyy_human_risk_vlm_bus_analy
Createdate: 20210428

-- 以下为依赖了上游的表 :
itl_EDW_orws_t_ghbr_bv_statistics
itl_EDW_orws_t_ghbr_bv_statistics_SYS
itl_edw_cmm_teller_info
itl_edw_orws_t_ghbr_bv_details
Logs:
...
alter table ${idl_schema}.mcyy_human_risk truncate subpartition p_${batch_date}_vlm_bus_analy;
...
alter table ${idl_schema}.mcyy_human_risk add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'))(
                                              subpartition p_${batch_date}_vlm_bus_analy values ('VLM_BUS_ANALY')
                                              )
;
alter table ${idl_schema}.mcyy_human_risk modify partition p_${batch_date} 
                                             add subpartition p_${batch_date}_vlm_bus_analy values ('VLM_BUS_ANALY')
;
...
whenever sqlerror exit sql.sqlcode;
 INSERT INTO ${idl_schema}.mcyy_human_risk
    ...
    WITH tmp_vlm_bus_data AS
     (SELECT 'YWL_CNLX001' AS bu_type --原始
            ,SUM(t1.total_txnvol - t1.auto_txnvol - t1.ma_txnvol) AS index_value --业务笔数
            ,t1.display_num AS org_no --柜员号 
            ,t1.display_name AS org_name --柜员名

            ,(CASE
                 WHEN substr(t1.organ_num
                            ,1
                            ,2) = '89'
                      OR t1.organ_num = '800001' THEN
                  '-000000'
                 ELSE
                  t1.organ_num
             END) AS super_org_no --机构号
      FROM   itl_EDW_orws_t_ghbr_bv_statistics t1
      LEFT   JOIN ITL_EDW_CMM_TELLER_INFO t2
      ON     t1.display_num = T2.TELLER_ID
      AND    T2.TELLER_TYPE_CD='TELLER_USER'
      WHERE  t1.data_date = to_date('${batch_date}'
                                      ,'yyyyMMdd') 
      AND    t1.data_type = '1'
      AND    t1.is_shield = '2'
      AND    t1.period_type = 1
      --AND    T2.TELLER_ID IS NOT NULL
      GROUP  BY t1.display_num, t1.organ_num,t1.display_name
      UNION ALL
      SELECT 'YWL_CNLX002' AS bu_type --折标
            ,SUM(t1.total_weight_txnvol - t1.auto_weight_txnvol -
                 t1.ma_weight_txnvol) AS index_value
            ,t1.display_num AS org_no --柜员号 
           ,t1.display_name AS org_name --柜员名
            ,(CASE
                 WHEN substr(t1.organ_num
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      WHERE  t1.index_no = 'WD010201') dim_tab) t1 --可以考虑放到with子句
...
whenever sqlerror exit sql.sqlcode;
 INSERT INTO ${idl_schema}.mcyy_human_risk
    ...
    WITH tmp_vlm_bus_data AS
     (SELECT fun_code_conv(t2.sys_name
                    ,'WD010301') AS bu_type --原始
      ,SUM(t2.sys_txnvol) AS index_value
      ,t1.display_num AS org_no --柜员号 
            ,t1.display_name AS org_name --柜员号 
...
FROM   itl_EDW_orws_t_ghbr_bv_statistics t1
 ...
 ON     t1.display_num = t3.TELLER_ID
 AND    t3.TELLER_TYPE_CD='TELLER_USER'
LEFT   JOIN itl_EDW_orws_t_ghbr_bv_statistics_sys t2
ON     t1.id = t2.statistics_id
WHERE  t1.data_type = '1'
AND    t1.is_shield = '2'
AND    t1.period_type = 1
--AND    t3.TELLER_ID IS NOT NULL
AND    t1.data_date = to_date('${batch_date}'
                             ,'yyyyMMdd')
GROUP  BY t1.display_num, t2.sys_name, t1.organ_num,t1.display_name
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      ON     t2.dim_class = t3.dim_class
                      AND    t3.dim_class_name IS NOT NULL
                      WHERE  t1.index_no = 'WD010301') dim_tab) t1 --可以考虑放到with子句
      LEFT   JOIN tmp_vlm_bus_data_person t2
      ON     t1.org_no = t2.org_no
      AND    t2.bu_type = t1.bu_type --前端需要不发生数据也生成分总行，所以这里写在ON里不用where过滤
      INNER  JOIN ${idl_schema}.mcyy_index_define t4
...
whenever sqlerror exit sql.sqlcode;
 INSERT INTO ${idl_schema}.mcyy_human_risk
    ...
    WITH tmp_vlm_bus_data AS
     (SELECT fun_code_conv(t2.sys_name
                    ,'WD010302') AS bu_type --折标
      ,SUM(t2.SYS_WEIGHT_TXNVOL) AS index_value
      ,t1.display_num AS org_no --柜员号 
            ,t1.display_name AS org_name --柜员号 
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      ON     t2.dim_class = t3.dim_class
                      AND    t3.dim_class_name IS NOT NULL
                      WHERE  t1.index_no = 'WD010302') dim_tab) t1 --可以考虑放到with子句
      LEFT   JOIN tmp_vlm_bus_data_person t2
      ON     t1.org_no = t2.org_no
      AND    t2.bu_type = t1.bu_type --前端需要不发生数据也生成分总行，所以这里写在ON里不用where过滤
      INNER  JOIN ${idl_schema}.mcyy_index_define t4
...
INSERT INTO ${idl_schema}.mcyy_human_risk
    ...
    WITH tmp_vlm_bus_data_d AS
     (SELECT 'YWL_CNLX001' AS bu_type --原始
            ,SUM(total_txnvol - auto_txnvol - ma_txnvol) /
             COUNT(DISTINCT t1.display_num) AS index_value --业务笔数
            ,(CASE
                 WHEN substr(organ_num
                            ,1
                            ,2) = '89'
                      OR organ_num = '800001' THEN
                  '-000000'
                 ELSE
                  organ_num
             END) AS org_no --机构号
      ...
      ON     t1.display_num = T2.TELLER_ID
      AND    T2.TELLER_TYPE_CD='TELLER_USER'
      WHERE  data_date = to_date('${batch_date}'
                                ,'yyyyMMdd')
      AND    data_type = '1'
      AND    is_shield = '2'
      AND    period_type = 1
      ...
      SELECT 'YWL_CNLX001' AS bu_type --原始
            ,SUM(total_txnvol - auto_txnvol - ma_txnvol) /
             COUNT(DISTINCT t1.display_num) AS index_value --业务笔数
            ,substr(organ_num,1,3) AS org_no --机构号
      FROM   itl_EDW_orws_t_ghbr_bv_statistics t1
      LEFT   JOIN ITL_EDW_CMM_TELLER_INFO t2
      ON     t1.display_num = T2.TELLER_ID
      AND    T2.TELLER_TYPE_CD='TELLER_USER'
      ...
      AND    period_type = 1
      --AND    T2.TELLER_ID IS NOT NULL
      AND    (substr(organ_num,1,2) <> '89' AND organ_num <> '800001')
      GROUP  BY substr(organ_num,1,3)
      UNION ALL
      --全行
       SELECT 'YWL_CNLX001' AS bu_type --原始
            ,SUM(total_txnvol - auto_txnvol - ma_txnvol) /
             COUNT(DISTINCT t1.display_num) AS index_value --业务笔数
            ,'000000' AS org_no --机构号
      FROM   itl_EDW_orws_t_ghbr_bv_statistics t1
      LEFT   JOIN ITL_EDW_CMM_TELLER_INFO t2
      ON     t1.display_num = T2.TELLER_ID
      AND    T2.TELLER_TYPE_CD='TELLER_USER'
      ...
      UNION ALL
      SELECT 'YWL_CNLX002' AS bu_type --折标
            ,SUM(total_weight_txnvol - auto_weight_txnvol -
                 ma_weight_txnvol) / COUNT(DISTINCT t1.display_num) AS index_value --业务笔数        
            ,(CASE
                 WHEN substr(organ_num
                            ,1
                            ,2) = '89'
...
tmp_vlm_bus_data_m AS
     (SELECT 'YWL_CNLX001' AS bu_type --原始
      ...
      ON     t1.display_num = T2.TELLER_ID
      AND    T2.TELLER_TYPE_CD='TELLER_USER'
      WHERE  data_date >=  trunc(to_date('${batch_date}'
                                  ,'yyyyMMdd')
                          ,'MM')
      AND    data_date <=to_date('${batch_date}'
                                ,'yyyyMMdd')
      AND    data_type = '1'
      AND    is_shield = '2'
      AND    period_type = 1
    ...
    --当日数据初始化
     (SELECT t4.index_no
            ...
            , --柜员名称
             t1.super_org_no AS super_org_no
            , --柜员所属支行机构编码
             coalesce(t2.index_value,0)  AS accu_index_value_d
            , --当日累计
             CASE
                 WHEN t1.org_no = '000000' THEN
                 ...
                 ELSE
                  coalesce(t5.index_value
                          ,0)
             END AS accu_index_value_m
            , --当月累计
                         CASE
                 WHEN t1.org_no = '000000' THEN
                  SUM(coalesce(t6.index_value
                              ,0)) over(PARTITION BY t4.index_no
                                       ,t1.bu_type)
                 WHEN length(t1.org_no) = 3 THEN
                  SUM(coalesce(t6.index_value
                              ,0)) over(PARTITION BY substr(t1.org_no
                                       ...
                                       ,t1.bu_type)
                 ELSE
                  coalesce(t6.index_value
                          ,0)
             END AS accu_index_value_q
            , --当季累计
                         CASE
                 WHEN t1.org_no = '000000' THEN
                  SUM(coalesce(t7.index_value
                              ,0)) over(PARTITION BY t4.index_no
                                       ,t1.bu_type)
                 WHEN length(t1.org_no) = 3 THEN
                  SUM(coalesce(t7.index_value
                              ,0)) over(PARTITION BY substr(t1.org_no
                                       ...
                                       ,t1.bu_type)
                 ELSE
                  coalesce(t7.index_value
                          ,0)
             END AS accu_index_value_y
            , --当年累计
             NULL AS index_ranking
            , --当前排名
             t4.unit
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      ON     t2.dim_class = t3.dim_class
                      AND    t3.dim_class_name IS NOT NULL
                      WHERE  t1.index_no = 'WD010202') dim_tab) t1 --可以考虑放到with子句
      LEFT   JOIN tmp_vlm_bus_data_d t2
      ON     t1.org_no = t2.org_no
      AND    t2.bu_type = t1.bu_type --前端需要不发生数据也生成分总行，所以这里写在ON里不用where过滤
     LEFT   JOIN tmp_vlm_bus_data_m t5
      ON     t1.org_no = t5.org_no
      AND    t5.bu_type = t1.bu_type
     LEFT   JOIN tmp_vlm_bus_data_q t6
      ON     t1.org_no = t6.org_no
      AND    t6.bu_type = t1.bu_type
     LEFT   JOIN tmp_vlm_bus_data_y t7
      ON     t1.org_no = t7.org_no
      AND    t7.bu_type = t1.bu_type
      INNER  JOIN ${idl_schema}.mcyy_index_define t4
      ON     'WD010202' = t4.index_no_mcs),
    --取上日数据来做日比较类计算
    tmp_ld_data AS
     (SELECT t1.index_no
            , ---- 指标编码
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      ON     t2.dim_class = t3.dim_class
                      AND    t3.dim_class_name IS NOT NULL
                      WHERE  t1.index_no = 'WD010303') dim_tab) t1 --可以考虑放到with子句
      LEFT   JOIN (
      SELECT 'GM_YWLX001' AS bu_type --原始产能
      ,SUM(nvl(t1.txn_amt
              ,0)) AS index_value     
      ,(CASE
           WHEN substr(t1.blng_org_id
                      ,1
                      ,2) = '89'
                OR t1.blng_org_id = '800001' THEN
            '-000000'
           ELSE
            t1.blng_org_id
       END) AS org_no --机构号
FROM   itl_edw_orws_t_ghbr_bv_details t1
LEFT   JOIN ITL_EDW_CMM_TELLER_INFO t2 --参数平台：柜员信息表
ON     t1.oper_teller_id = T2.TELLER_ID
AND    T2.TELLER_TYPE_CD='TELLER_USER' --C：柜台柜员
WHERE t1.txn_dt = to_date('${batch_date}'
                          ,'yyyyMMdd')
--AND    T2.TELLER_ID IS NOT NULL                          
AND    t1.txn_amt >0
AND    t1.txn_amt <=1000000000000
AND    t1.TXN_NUM <> '130B5'
AND    t1.data_src_cd IN ( 'IBSS'
,'SCPS'
,'ISBS'
,'IFMS'
,'ICMS'
...
GROUP  BY (CASE
    ...
    SELECT to_date('${batch_date}'
          ...
          ,SUM(mcyy_human_risk_tmp.rate_last_period) -- 同比
          ,(CASE
          						 WHEN SUM(mcyy_human_risk_tmp.rate_up_day) > 0
               					 AND SUM(mcyy_human_risk_tmp.rate_up_day_per) = 0 THEN
            							1
          						 ELSE
            						SUM(mcyy_human_risk_tmp.rate_up_day_per)
       									END) -- 比上日百分比
                      ,(CASE
          						 WHEN SUM(mcyy_human_risk_tmp.rate_last_month) > 0
               					 AND SUM(mcyy_human_risk_tmp.rate_last_month_per) = 0 THEN
            							1
          						 ELSE 
          						 SUM(mcyy_human_risk_tmp.rate_last_month_per) 
          						 end)  -- 比上月百分比
                      ,(CASE
          						 WHEN SUM(mcyy_human_risk_tmp.rate_last_quater) > 0
               					 AND SUM(mcyy_human_risk_tmp.rate_last_quater_per) = 0 THEN
            							1
          						 ELSE 
          						 SUM(mcyy_human_risk_tmp.rate_last_quater_per) 
          						 end)  -- 比上季百分比
          						 ,(CASE
          						 WHEN SUM(mcyy_human_risk_tmp.rate_last_year) > 0
               					 AND SUM(mcyy_human_risk_tmp.rate_last_year_per) = 0 THEN
            							1
          						 ELSE 
          						 SUM(mcyy_human_risk_tmp.rate_last_year_per) 
          						 end)-- 比上年百分比
          ,SUM(mcyy_human_risk_tmp.rate_last_period_per) -- 同比百分比
          ,mcyy_human_risk_tmp.index_ranking -- 当前排名
          ,mcyy_human_risk_tmp.index_ranking_cha -- 排名变动
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      ON     t2.dim_class = t3.dim_class
                      AND    t3.dim_class_name IS NOT NULL
                      WHERE  t1.index_no = 'WD010304') dim_tab) t1 --可以考虑放到with子句
      LEFT   JOIN (
      SELECT 'GM_YWLX001' AS bu_type --原始产能/资金业务
      ,(CASE
           WHEN substr(t1.blng_org_id
                      ,1
                      ,2) = '89'
           ...
           ELSE
            t1.blng_org_id
       END) AS org_no --机构号
      ,COUNT(1) AS index_value --资金业务量笔数
FROM   itl_edw_orws_t_ghbr_bv_details t1
LEFT   JOIN ITL_EDW_CMM_TELLER_INFO t2 --参数平台：柜员信息表
ON     t1.oper_teller_id = T2.TELLER_ID
AND    T2.TELLER_TYPE_CD='TELLER_USER' --C：柜台柜员
...
SELECT 'GM_YWLX002' AS bu_type --原始产能/非资金业务 
      ,(CASE
           WHEN substr(m.super_org_no
                      ,1
                      ,2) = '89'
                OR m.super_org_no = '800001' THEN
            '-000000'
           ELSE
            m.super_org_no
       END) AS org_no --机构号
      ,CASE WHEN coalesce(m.index_value, 0) - coalesce(p.index_value, 0) < 0 THEN 0 ELSE coalesce(m.index_value, 0) - coalesce(p.index_value, 0) END AS index_value --非资金业务量笔数
FROM   (SELECT SUM(t2.sys_txnvol) AS index_value
              ,t1.organ_num AS super_org_no --机构号
        FROM   itl_edw_orws_t_ghbr_bv_statistics t1
        LEFT   JOIN ITL_EDW_CMM_TELLER_INFO t3
        ON     t1.display_num = t3.TELLER_ID
        AND    t3.TELLER_TYPE_CD='TELLER_USER'
        ...
        AND    t1.data_date = to_date('${batch_date}'
                                     ,'yyyyMMdd')
        GROUP  BY  t1.organ_num) m --总业务量笔数
LEFT   JOIN (SELECT t1.blng_org_id AS super_org_no --机构号
                   ,COUNT(1) AS index_value --资金业务量笔数
             FROM   itl_edw_orws_t_ghbr_bv_details t1
             LEFT   JOIN ITL_EDW_CMM_TELLER_INFO t2 --参数平台：柜员信息表
             ON     t1.oper_teller_id = T2.TELLER_ID
             ...
             AND    t1.data_src_cd IN ('IBSS'
                                      ...
                                      ,'IBMS'
                                      ,'CTMS')
             GROUP  BY t1.blng_org_id) p --资金业务量笔数
ON    m.super_org_no = p.super_org_no ) t2
      ON     t1.org_no = t2.org_no
      AND    t2.bu_type = t1.bu_type --前端需要不发生数据也生成分总行，所以这里写在ON里不用where过滤
      INNER  JOIN ${idl_schema}.mcyy_index_define t4
      ON     'WD010304' = t4.index_no_mcs),
    --取上日数据来做日比较类计算
    tmp_ld_data AS
     (SELECT t1.index_no
            , ---- 指标编码
    ...
    SELECT to_date('${batch_date}'
          ...
          ,'VLM_BUS_ANALY' source_sys --来源系统
          ,mcyy_human_risk_tmp.bu_type --业务品种
     ,ratio_to_report(sum(mcyy_human_risk_tmp.accu_index_value_d)) over(PARTITION BY mcyy_human_risk_tmp.index_no, mcyy_human_risk_tmp.org_no) day_ratio_index --日占比
         ,ratio_to_report(sum(mcyy_human_risk_tmp.accu_index_value_m)) over(PARTITION BY mcyy_human_risk_tmp.index_no, mcyy_human_risk_tmp.org_no) mon_ratio_index --月占比
         ,ratio_to_report(sum(mcyy_human_risk_tmp.accu_index_value_q)) over(PARTITION BY mcyy_human_risk_tmp.index_no, mcyy_human_risk_tmp.org_no) quar_ratio_index --季占比
         ,ratio_to_report(sum(mcyy_human_risk_tmp.accu_index_value_y)) over(PARTITION BY mcyy_human_risk_tmp.index_no, mcyy_human_risk_tmp.org_no) year_ratio_index --年占比
