/*
Purpose:    集中授权总任务数D层-业务分析表(JZ010101):数据来源于营运预警新柜面系统（NCTS）
Author:     Sunline/郑沛隆
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_mcyy_bu_analysis_jz010101_ncts
Createdate: 20210112
Logs:
...
alter table ${idl_schema}.mcyy_bu_analysis truncate subpartition p_${batch_date}_ncts;
...
alter table ${idl_schema}.mcyy_bu_analysis add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'))(
                                              subpartition p_${batch_date}_ncts values ('NCTS')
                                              )
;
alter table ${idl_schema}.mcyy_bu_analysis modify partition p_${batch_date} 
                                             add subpartition p_${batch_date}_ncts values ('NCTS')
;
