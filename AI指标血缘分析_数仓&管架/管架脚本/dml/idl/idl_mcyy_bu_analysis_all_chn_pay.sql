/*
Purpose:    全渠道交易量/交易额D层-业务分析实时表(WD040201/WD040202):组合指标
Author:     Sunline/郑沛隆
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_mcyy_bu_analysis_all_chn_pay
Createdate: 20210601
Logs:
...
alter table ${idl_schema}.mcyy_bu_analysis truncate subpartition p_${batch_date}_all_chn_pay;
...
alter table ${idl_schema}.mcyy_bu_analysis add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'))(
                                              subpartition p_${batch_date}_all_chn_pay values ('ALL_CHN_PAY')
                                              )
;
alter table ${idl_schema}.mcyy_bu_analysis modify partition p_${batch_date} 
                                             add subpartition p_${batch_date}_all_chn_pay values ('ALL_CHN_PAY')
;
...
INTO ${idl_schema}.mcyy_bu_analysis
  ...
  WITH tmp_all_chn_vlm_bus AS
   (
    --柜面交易量
    SELECT org_no AS org_no,
            sum(accu_index_value_d) AS sum_count,
            'QQD_YWLX001' as bu_type
      FROM mcyy_human_risk t
     WHERE t.index_no = 'WD010304' --柜面业务量(资金+非资金)
       AND t.etl_dt = to_date('${batch_date}', 'yyyymmdd')
       and length(t.super_org_no) = 3
     group by org_no
    UNION ALL
    --自助渠道交易量
    SELECT org_no AS org_no,
           sum(accu_index_value_d) AS sum_count,
           'QQD_YWLX002' as bu_type
      FROM mcyy_bu_analysis t
     WHERE t.index_no IN ('WD040301',
                          'WD040401',
                          'WD041201',
                          'WD040601',
                          'WD040701',
  ...
  tmp_td_initza AS
  --当日数据初始化
   (SELECT t4.index_no, --指标编码
           t4.index_name_mcs AS index_name, --指标名称
           t1.org_no AS org_no, --机构编码
           t1.org_name AS org_name, --机构名称
           t1.super_org_no AS super_org_no, --上级机构编码
           CASE
             WHEN t1.org_no = '000000' THEN
              SUM(coalesce(t2.sum_count, 0))
              over(PARTITION BY t4.index_no, t1.bu_type)
           ...
           rank() over(PARTITION BY decode(length(t1.super_org_no), '1', 1, '3', 2, '6', 3) ORDER BY coalesce(t2.sum_count, 0) DESC) AS index_ranking, --当前排名
           t4.unit, -- 单位
           t4.frequency, -- 频度
      ...
      FROM (SELECT *
              FROM mcyy_orga_para org_tab,
                   (SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                     ...
                     WHERE t1.index_no = 'WD040201') dim_tab) t1
      LEFT JOIN (SELECT (CASE
                         WHEN substr(org_no, 1, 2) = '89' OR
                              org_no = '800001' THEN
                          '-000000'
                         ELSE
                          org_no
     ...
     INNER JOIN ${idl_schema}.mcyy_index_define t4
        ON 'WD040201' = t4.index_no_mcs),
  --取上日数据来做日比较类计算
  tmp_ld_data AS
   (SELECT t1.index_no, ---- 指标编码
           t1.index_name, -- 指标名称
           t1.org_no, -- 机构编码
           t1.org_name, -- 机构名称
           ...
           CASE
             WHEN trunc(to_date('${batch_date}', 'yyyymmdd'), 'mm') =
                  to_date('${batch_date}', 'yyyymmdd') THEN
              t1.accu_index_value_d
             ELSE
              coalesce(t1.accu_index_value_d, 0) +
              coalesce(t2.accu_index_value_m, 0)
           END AS accu_index_value_m --当月累计
           --当季初取当日数据作为当季累计，否则在上日的数据基础上累加
          ,
           CASE
             WHEN trunc(to_date('${batch_date}', 'yyyymmdd'), 'Q') =
                  to_date('${batch_date}', 'yyyymmdd') THEN
              t1.accu_index_value_d
             ELSE
              coalesce(t1.accu_index_value_d, 0) +
              coalesce(t2.accu_index_value_q, 0)
           END AS accu_index_value_q --当季累计 
           --当年初取当日数据作为当年累计，否则在上日的数据基础上累加
          ,
           CASE
             WHEN trunc(to_date('${batch_date}', 'yyyymmdd'), 'yy') =
                  to_date('${batch_date}', 'yyyymmdd') THEN
              t1.accu_index_value_d
             ELSE
              coalesce(t1.accu_index_value_d, 0) +
              coalesce(t2.accu_index_value_y, 0)
           END AS accu_index_value_y --当年累计
          ,
           coalesce(t1.accu_index_value_d, 0) -
           coalesce(t2.accu_index_value_d, 0) AS rate_up_day --比上日   
          ,
           CASE
             WHEN coalesce(t2.accu_index_value_d, 0) = 0 THEN
              0
             ELSE
              round((coalesce(t1.accu_index_value_d, 0) -
  ...
  tmp_lm_data AS
   (SELECT t1.index_no, ---- 指标编码
           ...
           t1.measure_no, -- 度量编号
           t1.index_measure, -- 度量名称
           coalesce(t1.accu_index_value_m, 0) -
           coalesce(t2.accu_index_value_m, 0) AS rate_last_month --比上月  
          ,
           CASE
             WHEN coalesce(t2.accu_index_value_m, 0) = 0 THEN
              0
             ELSE
              round((coalesce(t1.accu_index_value_m, 0) -
                    coalesce(t2.accu_index_value_m, 0)) /
                    coalesce(t2.accu_index_value_m, 0),
                    6)
  ...
  tmp_lq_data AS
   (SELECT t1.index_no, ---- 指标编码
           ...
           t1.measure_no, -- 度量编号
           t1.index_measure, -- 度量名称
           coalesce(t1.accu_index_value_q, 0) -
           coalesce(t2.accu_index_value_q, 0) AS rate_last_quater --比上季 
          ,
           CASE
             WHEN coalesce(t2.accu_index_value_q, 0) = 0 THEN
              0
             ELSE
              round((coalesce(t1.accu_index_value_q, 0) -
                    coalesce(t2.accu_index_value_q, 0)) /
                    coalesce(t2.accu_index_value_q, 0),
                    6)
  ...
  tmp_ly_data AS
   (SELECT t1.index_no, ---- 指标编码
           ...
           t1.measure_no, -- 度量编号
           t1.index_measure, -- 度量名称
           coalesce(t1.accu_index_value_y, 0) -
           coalesce(t2.accu_index_value_y, 0) AS rate_last_year --比上年
          ,
           CASE
             WHEN coalesce(t2.accu_index_value_y, 0) = 0 THEN
              0
             ELSE
              round((coalesce(t1.accu_index_value_y, 0) -
                    coalesce(t2.accu_index_value_y, 0)) /
                    coalesce(t2.accu_index_value_y, 0),
                    6)
  ...
  tmp_yoy_data AS
   (SELECT t1.index_no, ---- 指标编码
           ...
           t1.index_measure, -- 度量名称
           coalesce(t1.accu_index_value_d, 0) -
           coalesce(t2.accu_index_value_d, 0) AS rate_last_period --同比
          ,
           CASE
             WHEN coalesce(t2.accu_index_value_d, 0) = 0 THEN
              0
  ...
  SELECT to_date('${batch_date}', 'yyyymmdd') etl_dt -- 数据日期
        ,
         to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') etl_timestamp -- ETL处理时间戳
        ,
...
INTO ${idl_schema}.mcyy_bu_analysis
  ...
  WITH tmp_all_chn_bus_lmt AS
   (
    --柜面交易额
    SELECT org_no AS org_no,
            sum(accu_index_value_d) AS sum_amount,
            'QQD_YWLX001' as bu_type
      FROM mcyy_human_risk t
     WHERE t.index_no = 'WD010303' --柜面业务额(资金+非资金)
       AND t.etl_dt = to_date('${batch_date}', 'yyyymmdd')
       and length(t.super_org_no) = 3
     group by org_no
    UNION ALL
  ...
  --当日数据初始化
   (SELECT t4.index_no, --指标编码
           ...
           END AS accu_index_value_d, --当日累计
           rank() over(PARTITION BY decode(length(t1.super_org_no), '1', 1, '3', 2, '6', 3) ORDER BY coalesce(t2.sum_amount, 0) DESC) AS index_ranking, --当前排名
           t4.unit, -- 单位
           t4.frequency, -- 频度
           NULL measure_no, --- 度量编号
           t4.index_measure -- 度量名称
              ...
              FROM mcyy_orga_para org_tab,
                   (SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      LEFT JOIN mcyy_dim_define t3
                        ON t2.dim_class = t3.dim_class
                       AND t3.dim_class_name IS NOT NULL
                     WHERE t1.index_no = 'WD040202') dim_tab) t1
      LEFT JOIN (SELECT (CASE
                         WHEN substr(org_no, 1, 2) = '89' OR
                              org_no = '800001' THEN
                          '-000000'
