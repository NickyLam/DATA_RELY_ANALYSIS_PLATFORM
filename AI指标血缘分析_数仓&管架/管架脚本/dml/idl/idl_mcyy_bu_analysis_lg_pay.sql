/*
Purpose:    大额支付交易量/交易额D层-业务分析实时表(WD040301/WD040302):数据来源于中台
Author:     Sunline/谢婉宜
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_mcyy_bu_analysis_lg_pay
Createdate: 20210527
Logs:

-- 生成的IDL层表 ：mcyy_bu_analysis
-- 以下为依赖了上游的表 :
-- itl_edw_mpcs_a08thvtrx 
-- 以下为依赖的参数表 :
...
alter table ${idl_schema}.mcyy_bu_analysis truncate subpartition p_${batch_date}_lg_pay;
...
alter table ${idl_schema}.mcyy_bu_analysis add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'))(
                                              subpartition p_${batch_date}_lg_pay values ('LG_PAY')
                                              )
;
alter table ${idl_schema}.mcyy_bu_analysis modify partition p_${batch_date} 
                                             add subpartition p_${batch_date}_lg_pay values ('LG_PAY')
;
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      WHERE  t1.index_no = 'WD040301') dim_tab) t1
...
FROM   (SELECT a.mgmt_org_id AS org_no
      ,COUNT(1) AS sum_count
      ,'DXE_YWLX002' AS bu_type
FROM   itl_edw_evt_bigamt_tran_evt a
WHERE  ((a.scd_gener_msg_type_id IN
       ('hvps.111.001.01'
         ,'hvps.112.001.01') AND a.nostro_flg = '0' AND
       a.proc_status_cd IN ('T')) OR
       (a.scd_gener_msg_type_id IN
       ('hvps.111.001.01'
         ,'hvps.112.001.01') AND a.nostro_flg = '1' AND
       a.proc_status_cd IN ('6'
                             ,'7')) OR
       (a.scd_gener_msg_type_id LIKE '%141%' AND a.nostro_flg = '1' AND
       a.proc_status_cd IN ('4'
                             ,'6'
                             ,'7') AND
       debit_crdt_cd IN ('C'
                          ,'D')))
AND    a.tran_dt = to_date('${batch_date}'
                          ,'yyyymmdd')
AND    a.tran_amt = 0
GROUP  BY a.mgmt_org_id
UNION ALL
SELECT a.mgmt_org_id AS org_no
      ,COUNT(1) AS sum_count
      ,'DXE_YWLX001' AS bu_type
FROM   itl_edw_evt_bigamt_tran_evt a
WHERE  ((a.scd_gener_msg_type_id IN
       ('hvps.111.001.01'
         ,'hvps.112.001.01') AND a.nostro_flg = '0' AND
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      ON     t2.dim_class = t3.dim_class
                      AND    t3.dim_class_name IS NOT NULL
                      WHERE  t1.index_no = 'WD040302') dim_tab) t1
      LEFT   JOIN (SELECT (CASE
                             WHEN substr(org_no
                                        ,1
                                        ,2) = '89'
