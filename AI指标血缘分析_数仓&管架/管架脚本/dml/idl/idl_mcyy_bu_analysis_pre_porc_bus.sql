/*
Purpose:    预受理业务量D层-业务分析表(WD041101):数据来源于新柜面
Author:     Sunline/郑沛隆
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_mcyy_bu_analysis_pre_porc_bus
Createdate: 20210511
Logs:

-- 生成的IDL层表 ：mcyy_bu_analysis
-- 以下为依赖了上游的表 :
--ITL_EDW_EVT_IBS_PRE_PROC_FLOW 
--itl_edw_evt_scps_corp_bus_flow
...
alter table ${idl_schema}.mcyy_bu_analysis truncate subpartition p_${batch_date}_pre_porc_bus;
...
alter table ${idl_schema}.mcyy_bu_analysis add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'))(
                                              subpartition p_${batch_date}_pre_porc_bus values ('PRE_PORC_BUS')
                                              )
;
alter table ${idl_schema}.mcyy_bu_analysis modify partition p_${batch_date} 
                                             add subpartition p_${batch_date}_pre_porc_bus values ('PRE_PORC_BUS')
;
...
INTO ${idl_schema}.mcyy_bu_analysis
    ...
    --当日数据初始化
     (SELECT t4.index_no
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      WHERE  t1.index_no = 'WD041101') dim_tab) t1
      LEFT   JOIN (SELECT (CASE
                  ...
                  FROM
                  --新一代改造    
                  /*(SELECT NVL(NVL(t2.BELONG_ORG_ID,T3.APPOINTBRCODE),'-000000') AS org_no
                                ,fun_code_conv(biztype
                                              ,'WD041101') AS bu_type
                                ,COUNT(1) AS sum_count --业务笔数
                          FROM   itl_edw_ncts_bt_acp_acceptmain t1
                          LEFT  JOIN TMP_CUST_BASIC_INFO t2
                          ON     t1.custno = t2.cust_id
                          LEFT JOIN msl_pbss_flw_t_corp_reg t3
                          ON T3.pre_seqno=T1.REFTRADESERNO                         
                          WHERE  t1.createdate =
                                 to_date('${batch_date}'
                                        ,'yyyymmdd')
                          AND T1.STATUS NOT IN ('2','3','4')
                          GROUP  BY NVL(NVL(t2.BELONG_ORG_ID,T3.APPOINTBRCODE),'-000000'), biztype
                          UNION
                          SELECT NVL(NVL(t2.BELONG_ORG_ID,T3.APPOINTBRCODE),'-000000') AS org_no
                                ,fun_code_conv(biztype
                                              ,'WD041101') AS bu_type
                                ,COUNT(1) AS sum_count --业务笔数
                          FROM   itl_edw_ncts_bt_acp_acceptmain_new t1
                          INNER  JOIN TMP_CUST_BASIC_INFO t2
                          ON     t1.custno = t2.cust_id
                          LEFT JOIN msl_pbss_flw_t_corp_reg t3
                          ON T3.pre_seqno=T1.REFTRADESERNO 
                          WHERE  t1.createdate =
                           ...
                           AND T1.STATUS NOT IN ('2','3','4')
                          GROUP  BY NVL(NVL(t2.BELONG_ORG_ID,T3.APPOINTBRCODE),'-000000'), biztype)*/
                          (SELECT NVL(NVL(t2.BELONG_ORG_ID,T3.ORG_ID),'-000000') AS org_no
                                ,fun_code_conv(t1.BUS_TYPE_CD
                                              ,'WD041101') AS bu_type
                                ,COUNT(1) AS sum_count --业务笔数
                          FROM   ITL_EDW_EVT_IBS_PRE_PROC_FLOW t1 
                          LEFT  JOIN TMP_CUST_BASIC_INFO t2
                          ON     t1.CUST_ID = t2.cust_id
                          LEFT   JOIN ITL_EDW_EVT_SCPS_CORP_BUS_FLOW T3
                          ON  t1.tran_flow_num=t3.TASK_NO
                          AND T3.ETL_DT=to_date('${batch_date}','yyyymmdd')
                          WHERE  t1.ETL_DT =to_date('${batch_date}','yyyymmdd')
                           AND   T1.PRE_PROC_STATUS_CD NOT IN ('2','3','4')
                           AND   TRUNC(T1.CREATE_TM) = to_date('${batch_date}','yyyymmdd')
                          GROUP  BY NVL(NVL(t2.BELONG_ORG_ID,T3.ORG_ID),'-000000'), t1.BUS_TYPE_CD)                          
                     GROUP  BY (CASE
                             WHEN substr(org_no
                                        ,1
                                        ,2) = '89'
       ...
       ON t1.org_no = t2.org_no AND t1.bu_type = t2.bu_type INNER JOIN ${idl_schema}.mcyy_index_define t4 ON
     'WD041101' = t4.index_no_mcs),
    --取上日数据来做日比较类计算
    tmp_ld_data AS
     (SELECT t1.index_no
            , ---- 指标编码
    ...
    tmp_lm_data AS
     (SELECT t1.index_no
      ...
      AND    t2.etl_dt = trunc(to_date('${batch_date}'
                                      ,'yyyymmdd')
                              ,'mm') - 1 AND t1.bu_type = t2.bu_type),
    --取上季数据来做月比较类计算
    tmp_lq_data AS
     (SELECT t1.index_no
            , ---- 指标编码
    ...
    tmp_ly_data AS
     (SELECT t1.index_no
      ...
      AND    t2.etl_dt = trunc(to_date('${batch_date}'
                                      ,'yyyymmdd')
                              ,'yy') - 1 AND t1.bu_type = t2.bu_type),
    tmp_yoy_data AS
     (SELECT t1.index_no
            , ---- 指标编码
             t1.index_name
