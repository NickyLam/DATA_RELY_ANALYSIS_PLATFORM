/*
Purpose:    STM设备运行情况-业务分析实时表(WD040806):数据来源于自助设备系统
Author:     Sunline/郑沛隆
...
INTO ${idl_schema}.mcyy_bu_analysis_realtime
    ...
    --2、800001营运管理部，数据归到总行
     /*(SELECT (CASE
            ...
            ,t5.name AS manager_name
             --设备运行情况 SBYXQK 码值转换
            ,FUN_CODE_CONV(t1.dev_run_status,'WD040806') AS bu_type
      ...
      LEFT   JOIN (SELECT d.dev_no, listagg(o.name,',') as name 
                  FROM   msl_atms_dev_responsor_table d, msl_atms_op_table o
                  WHERE  to_char(d.reponser_no) = o.no
                  AND    d.catalog = 1
                  AND    d.grade = 1
     ...
     */
     --新一代后更新口径
     (SELECT (CASE
                 WHEN substr(t.ascrbranch
                            ,1
                            ,2) = '89'
                      OR t.ascrbranch = '800001' THEN
                  '000000'
                 ELSE
                  t.ascrbranch
             END) AS org_no
            ,COUNT(DISTINCT t.devicenum) AS index_value
            ,t.devicenum AS dev_no
            ,listagg(u.teller_name,',') AS manager_name
             --设备运行情况 SBYXQK 码值转换
            ,FUN_CODE_CONV(t.operstatus,'WD040806') AS bu_type
            ,to_date(t.modifdate||' '||t.modiftime
                    ,'yyyymmdd hh24:mi:ss') AS last_time
      FROM   msl_nibs_ib_dev_device_info t
      LEFT   JOIN  itl_edw_cmm_teller_info u
      ON u.teller_id in(t.adminuserone,t.adminusertwo)
      WHERE  t.devicetypenum = '06'  --STM设备
      AND    t.devicestate != '3'  --设备使用状态 : 0.领用、1：启用、2：停用、3：删除、9登记
      AND    t.devicesatus <> '0'  --设备状态标识 0-待启用，1-正常
      AND    substr(t.modifdate
                   ,1
                   ,8) = '${batch_date}'
      AND    to_date(t.modifdate||' '||t.modiftime
                    ,'yyyymmdd hh24:mi:ss') <
             (SELECT to_date(tmp_run_log.sum_end_time
                             ,'yyyymmdd hh24:mi:ss')
               FROM   tmp_run_log)
      GROUP  BY t.devicenum
               ,t.operstatus
               ,to_date(t.modifdate||' '||t.modiftime
                    ,'yyyymmdd hh24:mi:ss')
               ,(CASE
      ...
      FROM   (SELECT *
              FROM   mcyy_orga_para org_tab
                    ,(SELECT t1.index_no, t3.dim_class || dim_no AS bu_type
                      ...
                      WHERE  t1.index_no = 'WD040806') dim_tab) t1 -- 机构树表
...
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        dbms_output.put_line('循环执行实时脚本idl_mcyy_human_risk_realtime_wd040806出错' || SQLERRM);
