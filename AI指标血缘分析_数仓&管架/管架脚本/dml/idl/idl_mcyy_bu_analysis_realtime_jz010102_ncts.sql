/*
Purpose:    集中授权已处理任务数D层-业务分析实时表(JZ010102):数据来源于营运预警新柜面系统（NCTS）
Author:     Sunline/郑沛隆
Usage:      由ETL调度配置，每隔5分钟从${idl_schema}.mcyy_realtime_run_log获取时间点对业务表进行关联准实时统计
Createdate: 20210112
Logs:
20210220 mod by zhengpeilong :集中授权已处理和未处理只更新状态不更新时间，且前端显示只需要用到当日累计，所以修改为每次查询最新。
...
drop table ${idl_schema}.mbar_jz010102_${batch_date}_tm purge ;

whenever sqlerror exit sql.sqlcode;
create table ${idl_schema}.mbar_jz010102_${batch_date}_tm
...
AND    index_no ='JZ010102'
ORDER  BY sum_end_time,index_no;

 CURSOR CUR_TABLE_LIST IS
   SELECT TABLE_NAME
   			FROM ALL_TABLES
  WHERE TABLE_NAME LIKE UPPER('mbar_jz010102_%')
...
INSERT /*+ append */
INTO ${idl_schema}.mbar_jz010102_${batch_date}_tm
    (etl_dt --数据日期
    ,index_no --指标编码
    ,index_name --指标名称
    ,org_no --机构编码
    ...
    --3、xx分行营运管理部，数据归到分行
    /* (SELECT COUNT(DISTINCT t1.authserno) AS passcount
            ,(CASE
      ...
      GROUP  BY (CASE
                ...
                END)),*/
    (
    /*SELECT COUNT(DISTINCT t1.PRINAME) AS passcount
            ,(CASE
       ...
       FROM    msl_scps_workitem t1
			 inner join msl_scps_bp_translist_tb t2
 				...
 				where t1.STATE  IN ('6','7','9')
 				 and trunc(t1.INPUT_TIME) = to_date('${batch_date}', 'yyyymmdd')
      	 AND   t1.INPUT_TIME < (select to_date(tmp_run_log.sum_end_time
                                   ,'yyyymmdd hh24:mi:ss') from tmp_run_log)
         AND NOT EXISTS 
         (SELECT 1 FROM msl_scps_workitem t3 
         WHERE T3.PRINAME=T1.PRINAME 
         AND T3.STATE NOT IN ('6','7','9') 
         AND trunc(t3.INPUT_TIME) = to_date('${batch_date}', 'yyyymmdd'))--已处理是新增一条记录，不会更新原来记录
      GROUP  BY (CASE
                 WHEN substr(t2.begin_orgno
                            ,1
                            ,2) = '89'
    ...
    SELECT (CASE
                 ...
                 ELSE
                  a.begin_orgno
             END) AS org_no
            ,COUNT(DISTINCT a.task_id) AS passcount 
...
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        dbms_output.put_line('循环执行实时脚本idl_mcyy_bu_analysis_realtime_jz010102_ncts出错' || SQLERRM);
...
select * from ${idl_schema}.mbar_jz010102_${batch_date}_tm;
