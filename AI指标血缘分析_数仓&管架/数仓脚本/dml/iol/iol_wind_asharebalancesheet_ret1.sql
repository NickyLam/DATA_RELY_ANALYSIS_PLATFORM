...
begin
	for tb in (select partition_name,table_name,substr(partition_name,3) as etl_dt 
	            ...
	            where table_name = upper('wind_asharebalancesheet_bak${batch_date}')
	              and partition_name <> 'P_19000101') loop
	             
  select count(*) into v_flag
    from all_tab_partitions 
   where table_owner = upper('IOL')
     and table_name = upper('wind_asharebalancesheet')
     and partition_name = tb.partition_name;
     
  if v_flag <> 0 then
     execute immediate 'alter table wind_asharebalancesheet drop partition '|| tb.partition_name ;
     
  end if;
  
     execute immediate 'alter table wind_asharebalancesheet add partition ' || tb.partition_name || ' values (to_date(' || tb.etl_dt || ',''yyyymmdd''))';

insert /*+ append */ into ${iol_schema}.wind_asharebalancesheet(
    object_id -- 对象ID
...
select
    ...
    ,ann_dt -- 公告日期
    ,report_period -- 报告期
    ,nvl(trim(statement_type), 0) as statement_type -- 报表类型
    ,crncy_code -- 货币代码
    ,monetary_cap -- 货币资金
    ,tradable_fin_assets -- 交易性金融资产
    ,notes_rcv -- 应收票据
...
from wind_asharebalancesheet_bak${batch_date}
