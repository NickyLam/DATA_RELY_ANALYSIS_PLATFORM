...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_rptm_rtm_trade_aft_credit
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.rptm_rtm_trade_aft_credit_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.rptm_rtm_trade_aft_credit
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.rptm_rtm_trade_aft_credit_op purge;
drop table ${iol_schema}.rptm_rtm_trade_aft_credit_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.rptm_rtm_trade_aft_credit_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.rptm_rtm_trade_aft_credit where 0=1;

create table ${iol_schema}.rptm_rtm_trade_aft_credit_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.rptm_rtm_trade_aft_credit where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.rptm_rtm_trade_aft_credit_cl(
            id -- 
            ,bus_id -- 
            ,data_state -- 
            ,active_time -- 
        ...
        into ${iol_schema}.rptm_rtm_trade_aft_credit_op(
            id -- 
            ,bus_id -- 
            ,data_state -- 
            ,active_time -- 
...
select
    ...
    ,nvl(n.date_month, o.date_month) as date_month -- 
...
from (select * from ${iol_schema}.rptm_rtm_trade_aft_credit_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.rptm_rtm_trade_aft_credit where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
from ${iol_schema}.rptm_rtm_trade_aft_credit_bk o
    left join ${iol_schema}.rptm_rtm_trade_aft_credit_op n
        on

            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.rptm_rtm_trade_aft_credit_cl d
        on
...
--truncate table ${iol_schema}.rptm_rtm_trade_aft_credit;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('rptm_rtm_trade_aft_credit') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.rptm_rtm_trade_aft_credit drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.rptm_rtm_trade_aft_credit add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.rptm_rtm_trade_aft_credit exchange partition p_${batch_date} with table ${iol_schema}.rptm_rtm_trade_aft_credit_cl;
alter table ${iol_schema}.rptm_rtm_trade_aft_credit exchange partition p_20991231 with table ${iol_schema}.rptm_rtm_trade_aft_credit_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.rptm_rtm_trade_aft_credit_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'rptm_rtm_trade_aft_credit',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
