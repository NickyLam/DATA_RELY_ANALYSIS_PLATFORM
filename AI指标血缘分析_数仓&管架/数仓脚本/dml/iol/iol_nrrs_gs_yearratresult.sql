...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_nrrs_gs_yearratresult
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.nrrs_gs_yearratresult_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.nrrs_gs_yearratresult
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.nrrs_gs_yearratresult_op purge;
drop table ${iol_schema}.nrrs_gs_yearratresult_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.nrrs_gs_yearratresult_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.nrrs_gs_yearratresult where 0=1;

create table ${iol_schema}.nrrs_gs_yearratresult_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.nrrs_gs_yearratresult where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.nrrs_gs_yearratresult_cl(
            modelcode -- 体系类型代号
            ,index1 -- 对象代码1
            ,index2 -- 对象代码2
            ,index3 -- 对象代码3
        ...
        into ${iol_schema}.nrrs_gs_yearratresult_op(
            modelcode -- 体系类型代号
            ,index1 -- 对象代码1
            ,index2 -- 对象代码2
            ,index3 -- 对象代码3
...
select
    ...
    ,nvl(n.year, o.year) as year -- 评级年月
    ...
    ,nvl(n.state, o.state) as state -- 评级状态
    ...
    ,nvl(n.ratcount, o.ratcount) as ratcount -- 重新审核次数
    ,nvl(n.overturn, o.overturn) as overturn -- 是否推翻
    ,nvl(n.overthing, o.overthing) as overthing -- 推翻理由
...
from (select * from ${iol_schema}.nrrs_gs_yearratresult_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.nrrs_gs_yearratresult where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
from ${iol_schema}.nrrs_gs_yearratresult_bk o
    left join ${iol_schema}.nrrs_gs_yearratresult_op n
        on
            o.modelcode = n.modelcode
            and o.index1 = n.index1
            and o.index2 = n.index2
    ...
    left join ${iol_schema}.nrrs_gs_yearratresult_cl d
        on
...
--truncate table ${iol_schema}.nrrs_gs_yearratresult;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('nrrs_gs_yearratresult') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.nrrs_gs_yearratresult drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.nrrs_gs_yearratresult add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.nrrs_gs_yearratresult exchange partition p_${batch_date} with table ${iol_schema}.nrrs_gs_yearratresult_cl;
alter table ${iol_schema}.nrrs_gs_yearratresult exchange partition p_20991231 with table ${iol_schema}.nrrs_gs_yearratresult_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.nrrs_gs_yearratresult_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'nrrs_gs_yearratresult',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
