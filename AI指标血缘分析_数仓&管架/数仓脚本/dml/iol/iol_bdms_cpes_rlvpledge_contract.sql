...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_bdms_cpes_rlvpledge_contract
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.bdms_cpes_rlvpledge_contract_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.bdms_cpes_rlvpledge_contract
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.bdms_cpes_rlvpledge_contract_op purge;
drop table ${iol_schema}.bdms_cpes_rlvpledge_contract_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.bdms_cpes_rlvpledge_contract_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.bdms_cpes_rlvpledge_contract where 0=1;

create table ${iol_schema}.bdms_cpes_rlvpledge_contract_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.bdms_cpes_rlvpledge_contract where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.bdms_cpes_rlvpledge_contract_cl(
            id -- ID
            ...
            ,manager_no -- 客户经理
            ,department_no -- 部门编号
            ,contract_status -- 审批状态: 00   未处理 01   审批中 02   审核成功 03   审核失败 04   审核拒绝
            ,message_status -- 报文处理状态: 01 已发送 02 收到确认 21 收到确认失败
        ...
        into ${iol_schema}.bdms_cpes_rlvpledge_contract_op(
            id -- ID
            ,buss_flag -- 业务方向： 01 申请 02 签收
            ,contract_no -- 批次号
            ,product_no -- 产品号
...
select
    ...
    ,nvl(n.contract_status, o.contract_status) as contract_status -- 审批状态: 00   未处理 01   审批中 02   审核成功 03   审核失败 04   审核拒绝
    ,nvl(n.message_status, o.message_status) as message_status -- 报文处理状态: 01 已发送 02 收到确认 21 收到确认失败
...
from (select * from ${iol_schema}.bdms_cpes_rlvpledge_contract_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.bdms_cpes_rlvpledge_contract where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.id = n.id
where (
        o.id is null
...
select
    ...
    ,o.contract_status -- 审批状态: 00   未处理 01   审批中 02   审核成功 03   审核失败 04   审核拒绝
    ,o.message_status -- 报文处理状态: 01 已发送 02 收到确认 21 收到确认失败
...
from ${iol_schema}.bdms_cpes_rlvpledge_contract_bk o
    left join ${iol_schema}.bdms_cpes_rlvpledge_contract_op n
        on
            o.id = n.id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.bdms_cpes_rlvpledge_contract_cl d
        on
            o.id = d.id
...
--truncate table ${iol_schema}.bdms_cpes_rlvpledge_contract;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('bdms_cpes_rlvpledge_contract') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.bdms_cpes_rlvpledge_contract drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.bdms_cpes_rlvpledge_contract add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.bdms_cpes_rlvpledge_contract exchange partition p_${batch_date} with table ${iol_schema}.bdms_cpes_rlvpledge_contract_cl;
alter table ${iol_schema}.bdms_cpes_rlvpledge_contract exchange partition p_20991231 with table ${iol_schema}.bdms_cpes_rlvpledge_contract_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.bdms_cpes_rlvpledge_contract_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'bdms_cpes_rlvpledge_contract',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
