...
drop table ${iol_schema}.ibms_ttrd_accounting_secu_obj_comp_ex purge;
alter table ${iol_schema}.ibms_ttrd_accounting_secu_obj_comp add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.2 truncate target table
whenever sqlerror exit sql.sqlcode;
truncate table ${iol_schema}.ibms_ttrd_accounting_secu_obj_comp;

-- 2.3 insert data to ex table
create table ${iol_schema}.ibms_ttrd_accounting_secu_obj_comp_ex nologging
compress
as
select * from ${iol_schema}.ibms_ttrd_accounting_secu_obj_comp where 0=1;

insert /*+ append */ into ${iol_schema}.ibms_ttrd_accounting_secu_obj_comp_ex(
    obj_id -- 
    ,tsk_id -- 
    ,beg_date -- 
    ,end_date -- 
...
from ${itl_schema}.ibms_ttrd_accounting_secu_obj_comp
where etl_dt = to_date('${batch_date}', 'yyyymmdd')
;

-- 2.4 exchage ex table and target table
alter table ${iol_schema}.ibms_ttrd_accounting_secu_obj_comp exchange partition p_${batch_date} with table ${iol_schema}.ibms_ttrd_accounting_secu_obj_comp_ex;
...
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ibms_ttrd_accounting_secu_obj_comp',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);