...
drop table ${iol_schema}.ncbs_rb_tran_hist_ex purge;
alter table ${iol_schema}.ncbs_rb_tran_hist add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.2 truncate target table batch_date partition
whenever sqlerror exit sql.sqlcode;
alter table ${iol_schema}.ncbs_rb_tran_hist truncate partition p_${batch_date};

-- 2.3 insert data to ex table
create table ${iol_schema}.ncbs_rb_tran_hist_ex nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ncbs_rb_tran_hist where 0=1;

insert /*+ append */ into ${iol_schema}.ncbs_rb_tran_hist_ex(
    acct_seq_no -- 账户子账号
    ,acct_status -- 账户状态
    ,amt_type -- 金额类型
...
from ${itl_schema}.ncbs_rb_tran_hist
where etl_dt = to_date('${batch_date}', 'yyyymmdd')
;

-- 2.4 exchage ex table and target table
alter table ${iol_schema}.ncbs_rb_tran_hist exchange partition p_${batch_date} with table ${iol_schema}.ncbs_rb_tran_hist_ex;
...
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ncbs_rb_tran_hist',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);