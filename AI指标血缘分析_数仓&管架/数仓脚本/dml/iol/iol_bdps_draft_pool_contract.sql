...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_bdps_draft_pool_contract
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.bdps_draft_pool_contract_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.bdps_draft_pool_contract
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.bdps_draft_pool_contract_op purge;
drop table ${iol_schema}.bdps_draft_pool_contract_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.bdps_draft_pool_contract_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.bdps_draft_pool_contract where 0=1;

create table ${iol_schema}.bdps_draft_pool_contract_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.bdps_draft_pool_contract where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.bdps_draft_pool_contract_cl(
            id -- 
            ,protocol_no -- 
            ,contract_type -- 
            ,draft_attr -- 
            ...
            ,income_accno -- 
            ,account_id -- 客户账号id
            ,register_status -- 票交所登记状态:0-未登记(为空默认未登记);1-已发送质押登记;2-已发送解质押登记
            ,bsnssq -- 全局流水号
            ,transq -- 业务流水号
            ,start_dt -- 开始时间
            ,end_dt -- 结束时间
        ...
        into ${iol_schema}.bdps_draft_pool_contract_op(
            id -- 
            ,protocol_no -- 
            ,contract_type -- 
            ,draft_attr -- 
...
select
    nvl(n.id, o.id) as id -- 
    ,nvl(n.protocol_no, o.protocol_no) as protocol_no -- 
    ...
    ,nvl(n.bail_acct_id, o.bail_acct_id) as bail_acct_id -- 
    ...
    ,nvl(n.is_collztn_draft_pool, o.is_collztn_draft_pool) as is_collztn_draft_pool -- 
    ...
    ,nvl(n.dis_brhid, o.dis_brhid) as dis_brhid -- 
    ,nvl(n.pay_type, o.pay_type) as pay_type -- 
    ,nvl(n.intrst_rate, o.intrst_rate) as intrst_rate -- 
    ,nvl(n.income_brhid, o.income_brhid) as income_brhid -- 
    ,nvl(n.income_accno, o.income_accno) as income_accno -- 
    ,nvl(n.account_id, o.account_id) as account_id -- 客户账号id
    ,nvl(n.register_status, o.register_status) as register_status -- 票交所登记状态:0-未登记(为空默认未登记);1-已发送质押登记;2-已发送解质押登记
...
from (select * from ${iol_schema}.bdps_draft_pool_contract_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.bdps_draft_pool_contract where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.id = n.id
where (
        o.id is null
    ...
    or (
        ...
        or o.bail_acct_id <> n.bail_acct_id
        ...
        or o.is_collztn_draft_pool <> n.is_collztn_draft_pool
        ...
        or o.dis_brhid <> n.dis_brhid
        or o.pay_type <> n.pay_type
        or o.intrst_rate <> n.intrst_rate
        or o.income_brhid <> n.income_brhid
        or o.income_accno <> n.income_accno
...
select
    ...
    ,o.income_accno -- 
    ,o.account_id -- 客户账号id
    ,o.register_status -- 票交所登记状态:0-未登记(为空默认未登记);1-已发送质押登记;2-已发送解质押登记
    ,o.bsnssq -- 全局流水号
    ,o.transq -- 业务流水号
    ,o.start_dt -- 开始时间
...
from ${iol_schema}.bdps_draft_pool_contract_bk o
    left join ${iol_schema}.bdps_draft_pool_contract_op n
        on
            o.id = n.id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.bdps_draft_pool_contract_cl d
        on
            o.id = d.id
...
--truncate table ${iol_schema}.bdps_draft_pool_contract;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('bdps_draft_pool_contract') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.bdps_draft_pool_contract drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.bdps_draft_pool_contract add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.bdps_draft_pool_contract exchange partition p_${batch_date} with table ${iol_schema}.bdps_draft_pool_contract_cl;
alter table ${iol_schema}.bdps_draft_pool_contract exchange partition p_20991231 with table ${iol_schema}.bdps_draft_pool_contract_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.bdps_draft_pool_contract_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'bdps_draft_pool_contract',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
