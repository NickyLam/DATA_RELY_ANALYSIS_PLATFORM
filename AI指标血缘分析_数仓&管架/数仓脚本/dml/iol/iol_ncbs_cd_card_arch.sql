...
create table ${iol_schema}.ncbs_cd_card_arch_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.ncbs_cd_card_arch
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.ncbs_cd_card_arch_op purge;
drop table ${iol_schema}.ncbs_cd_card_arch_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.ncbs_cd_card_arch_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ncbs_cd_card_arch where 0=1;

create table ${iol_schema}.ncbs_cd_card_arch_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ncbs_cd_card_arch where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.ncbs_cd_card_arch_cl(
            acct_seq_no -- 账户子账号
            ,card_no -- 卡号
            ,client_no -- 客户编号
            ,prod_type -- 产品编号
        ...
        into ${iol_schema}.ncbs_cd_card_arch_op(
            acct_seq_no -- 账户子账号
            ,card_no -- 卡号
            ,client_no -- 客户编号
            ,prod_type -- 产品编号
...
select
    ...
    ,nvl(n.card_close_reason, o.card_close_reason) as card_close_reason -- 销卡原因
    ...
    ,nvl(n.card_close_date, o.card_close_date) as card_close_date -- 销卡日期
    ...
    ,nvl(n.close_user_id, o.close_user_id) as close_user_id -- 关闭柜员
    ...
    ,nvl(n.card_cvn2, o.card_cvn2) as card_cvn2 -- 卡片CVN信息（存储的等效二磁信息，包含D的）
    ,nvl(n.card_cvv2, o.card_cvv2) as card_cvv2 -- 
    ,nvl(n.card_cvv2_mac, o.card_cvv2_mac) as card_cvv2_mac -- 
    ,case when
            n.card_no is null
        then o.start_dt
...
from (select * from ${iol_schema}.ncbs_cd_card_arch_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.ncbs_cd_card_arch where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.card_no = n.card_no
where (
        o.card_no is null
    ...
    or (
        ...
        or o.card_close_reason <> n.card_close_reason
        ...
        or o.card_close_date <> n.card_close_date
        ...
        or o.close_user_id <> n.close_user_id
        ...
        or o.card_cvn2 <> n.card_cvn2
        or o.card_cvv2 <> n.card_cvv2
        or o.card_cvv2_mac <> n.card_cvv2_mac
    )
;
commit;
...
select
    ...
    ,o.card_cvn2 -- 卡片CVN信息（存储的等效二磁信息，包含D的）
    ,o.card_cvv2 -- 
    ,o.card_cvv2_mac -- 
    ,o.start_dt -- 开始时间
...
from ${iol_schema}.ncbs_cd_card_arch_bk o
    left join ${iol_schema}.ncbs_cd_card_arch_op n
        on
            o.card_no = n.card_no
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.ncbs_cd_card_arch_cl d
        on
            o.card_no = d.card_no
...
--truncate table ${iol_schema}.ncbs_cd_card_arch;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('ncbs_cd_card_arch') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.ncbs_cd_card_arch drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.ncbs_cd_card_arch add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.ncbs_cd_card_arch exchange partition p_${batch_date} with table ${iol_schema}.ncbs_cd_card_arch_cl;
alter table ${iol_schema}.ncbs_cd_card_arch exchange partition p_20991231 with table ${iol_schema}.ncbs_cd_card_arch_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.ncbs_cd_card_arch_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ncbs_cd_card_arch',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
