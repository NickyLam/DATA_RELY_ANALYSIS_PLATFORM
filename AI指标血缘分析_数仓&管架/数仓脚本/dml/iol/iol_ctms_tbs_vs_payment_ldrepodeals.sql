...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_ctms_tbs_vs_payment_ldrepodeals
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.ctms_tbs_vs_payment_ldrepodeals_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.ctms_tbs_vs_payment_ldrepodeals;

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.ctms_tbs_vs_payment_ldrepodeals_op purge;
drop table ${iol_schema}.ctms_tbs_vs_payment_ldrepodeals_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.ctms_tbs_vs_payment_ldrepodeals_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ctms_tbs_vs_payment_ldrepodeals where 0=1;

create table ${iol_schema}.ctms_tbs_vs_payment_ldrepodeals_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ctms_tbs_vs_payment_ldrepodeals where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.ctms_tbs_vs_payment_ldrepodeals_cl(
            deal_id -- 引用表ID
            ,deal_tablename -- 引用表名
            ,aspclient_id -- 部门编号
            ,bondscode -- 债券代码
        ...
        into ${iol_schema}.ctms_tbs_vs_payment_ldrepodeals_op(
            deal_id -- 引用表ID
            ,deal_tablename -- 引用表名
            ,aspclient_id -- 部门编号
            ,bondscode -- 债券代码
...
from (select * from ${iol_schema}.ctms_tbs_vs_payment_ldrepodeals_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.ctms_tbs_vs_payment_ldrepodeals where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.deal_id = n.deal_id
...
select
    ...
    ,o.id_mark -- 增删标志
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间
from ${iol_schema}.ctms_tbs_vs_payment_ldrepodeals_bk o
    left join ${iol_schema}.ctms_tbs_vs_payment_ldrepodeals_op n
        on
            o.deal_id = n.deal_id
            and o.deal_tablename = n.deal_tablename
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.ctms_tbs_vs_payment_ldrepodeals_cl d
        on
            o.deal_id = d.deal_id
...
-- truncate table ${iol_schema}.ctms_tbs_vs_payment_ldrepodeals;

-- 4.2 exchange partition
alter table ${iol_schema}.ctms_tbs_vs_payment_ldrepodeals exchange partition p_19000101 with table ${iol_schema}.ctms_tbs_vs_payment_ldrepodeals_cl;
alter table ${iol_schema}.ctms_tbs_vs_payment_ldrepodeals exchange partition p_20991231 with table ${iol_schema}.ctms_tbs_vs_payment_ldrepodeals_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.ctms_tbs_vs_payment_ldrepodeals_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ctms_tbs_vs_payment_ldrepodeals',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
