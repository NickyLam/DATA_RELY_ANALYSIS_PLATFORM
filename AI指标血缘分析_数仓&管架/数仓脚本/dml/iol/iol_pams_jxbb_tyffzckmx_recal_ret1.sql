...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_pams_jxbb_tyffzckmx_recal_ret1
CreateDate: 20250630
*/
...
begin
  for tb in (select partition_name,table_name,substr(partition_name,3) as etl_dt
               from user_tab_partitions
                   where table_name = upper('pams_jxbb_tyffzckmx_recal_bak${batch_date}')
  ...
  select count(*) into v_flag
   ...
   where table_owner = upper('iol')
     and table_name = upper('pams_jxbb_tyffzckmx_recal')
     and partition_name = tb.partition_name;

  if v_flag <> 0 then
    execute immediate 'alter table pams_jxbb_tyffzckmx_recal drop partition '|| tb.partition_name ;
  end if;

    execute immediate 'alter table pams_jxbb_tyffzckmx_recal add partition ' || tb.partition_name || ' values (to_date(' || tb.etl_dt || ',''yyyymmdd''))';


-- 回插所有数据

insert /*+ append */ into ${iol_schema}.pams_jxbb_tyffzckmx_recal (
	tjrq -- 数据入库日期
	,jxdxdh -- 绩效对象代号
	,ywbh -- 业务编号
	,jrgjdm -- 金融工具代码
...
select
	...
	,lxsrzzs as lxsrzzs -- 税额
	,bnmmsy as bnmmsy -- 半年买卖损益
	,ljmmsy as ljmmsy -- 累计买卖损益
	,yzbwlx as yzbwlx -- 已转表外利息
	,bjkmh as bjkmh -- 本金科目号
	,bjkmmc as bjkmmc -- 本金科目名称
	,ftpll as ftpll -- 准备金FTP利率
	,zhnrjye as zhnrjye -- 年日均
	,zhjrjye as zhjrjye -- 账户季日均
	,zhyrjye as zhyrjye -- 账户月日均
	...
	,ftpsynlj as ftpsynlj -- ftp净收入(年累计)
	...
	,hsfxjqzcje as hsfxjqzcje -- 缓释后的风险加权资产金额
	,tzsy as tzsy -- 调整收益
...
from ${iol_schema}.pams_jxbb_tyffzckmx_recal_bak${batch_date}
