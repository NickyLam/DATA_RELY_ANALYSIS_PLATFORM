...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_mims_si_ship
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.mims_si_ship_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.mims_si_ship;

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.mims_si_ship_op purge;
drop table ${iol_schema}.mims_si_ship_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.mims_si_ship_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.mims_si_ship where 0=1;

create table ${iol_schema}.mims_si_ship_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.mims_si_ship where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.mims_si_ship_cl(
            sccode -- 
            ,identyno -- 
            ,registno -- 
            ,shipcerno -- 
        ...
        into ${iol_schema}.mims_si_ship_op(
            sccode -- 
            ,identyno -- 
            ,registno -- 
            ,shipcerno -- 
...
from (select * from ${iol_schema}.mims_si_ship_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.mims_si_ship where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.sccode = n.sccode
where (
        o.sccode is null
...
select
    ...
    ,o.id_mark -- 增删标志
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间
from ${iol_schema}.mims_si_ship_bk o
    left join ${iol_schema}.mims_si_ship_op n
        on
            o.sccode = n.sccode
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.mims_si_ship_cl d
        on
            o.sccode = d.sccode
...
-- truncate table ${iol_schema}.mims_si_ship;

-- 4.2 exchange partition
alter table ${iol_schema}.mims_si_ship exchange partition p_19000101 with table ${iol_schema}.mims_si_ship_cl;
alter table ${iol_schema}.mims_si_ship exchange partition p_20991231 with table ${iol_schema}.mims_si_ship_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.mims_si_ship_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'mims_si_ship',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
