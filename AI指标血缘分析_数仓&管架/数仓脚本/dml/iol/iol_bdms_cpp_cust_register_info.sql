...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_bdms_cpp_cust_register_info
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.bdms_cpp_cust_register_info_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.bdms_cpp_cust_register_info
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.bdms_cpp_cust_register_info_op purge;
drop table ${iol_schema}.bdms_cpp_cust_register_info_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.bdms_cpp_cust_register_info_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.bdms_cpp_cust_register_info where 0=1;

create table ${iol_schema}.bdms_cpp_cust_register_info_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.bdms_cpp_cust_register_info where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.bdms_cpp_cust_register_info_cl(
            id -- ID
            ,busi_id -- 业务表ID
            ,busi_type -- 业务类型： 300 登记 301 解除登记
            ,opr_type -- 操作类型： AT01 新增 AT02 修改 AT99 查询
            ,reg_brh_no -- 登记机构代码
            ,cpp_cust_name -- 申请人名称
            ,cpp_social_no -- 申请人社会信用代码
            ,cpp_corp_scale -- 规模： SC00 大型企业 SC01 中型企业 SC02 小型企业 SC03 微小企业 SC04 其他
            ,cpp_ind_cls -- 行业分类：详见概述分册
            ,cpp_arc_flag -- 是否涉农企业： 0 否 1 是
            ,cpp_grn_flag -- 是否绿色企业： 0 否 1 是
            ,cpp_sci_flag -- 是否科技企业： 0 否 1 是
            ,cpp_pop_flag -- 是否民营企业： 0 否 1 是
            ,cpp_province -- 贴现申请人省份：参见附录省份代码
            ,reg_status -- 登记状态： DRS01 正常 DRS02 失效
            ,process_code -- 处理码
            ,process_msg -- 处理信息
            ,last_upd_opr -- 最后操作员
        ...
        into ${iol_schema}.bdms_cpp_cust_register_info_op(
            id -- ID
            ,busi_id -- 业务表ID
            ,busi_type -- 业务类型： 300 登记 301 解除登记
            ,opr_type -- 操作类型： AT01 新增 AT02 修改 AT99 查询
...
select
    ...
    ,nvl(n.busi_type, o.busi_type) as busi_type -- 业务类型： 300 登记 301 解除登记
    ,nvl(n.opr_type, o.opr_type) as opr_type -- 操作类型： AT01 新增 AT02 修改 AT99 查询
    ,nvl(n.reg_brh_no, o.reg_brh_no) as reg_brh_no -- 登记机构代码
    ,nvl(n.cpp_cust_name, o.cpp_cust_name) as cpp_cust_name -- 申请人名称
    ,nvl(n.cpp_social_no, o.cpp_social_no) as cpp_social_no -- 申请人社会信用代码
    ,nvl(n.cpp_corp_scale, o.cpp_corp_scale) as cpp_corp_scale -- 规模： SC00 大型企业 SC01 中型企业 SC02 小型企业 SC03 微小企业 SC04 其他
    ,nvl(n.cpp_ind_cls, o.cpp_ind_cls) as cpp_ind_cls -- 行业分类：详见概述分册
    ...
    ,nvl(n.cpp_province, o.cpp_province) as cpp_province -- 贴现申请人省份：参见附录省份代码
    ,nvl(n.reg_status, o.reg_status) as reg_status -- 登记状态： DRS01 正常 DRS02 失效
...
from (select * from ${iol_schema}.bdms_cpp_cust_register_info_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.bdms_cpp_cust_register_info where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.id = n.id
where (
        o.id is null
    ...
    or (
        ...
        or o.opr_type <> n.opr_type
        or o.reg_brh_no <> n.reg_brh_no
...
select
    o.id -- ID
    ,o.busi_id -- 业务表ID
    ,o.busi_type -- 业务类型： 300 登记 301 解除登记
    ,o.opr_type -- 操作类型： AT01 新增 AT02 修改 AT99 查询
    ,o.reg_brh_no -- 登记机构代码
    ,o.cpp_cust_name -- 申请人名称
    ,o.cpp_social_no -- 申请人社会信用代码
    ,o.cpp_corp_scale -- 规模： SC00 大型企业 SC01 中型企业 SC02 小型企业 SC03 微小企业 SC04 其他
    ,o.cpp_ind_cls -- 行业分类：详见概述分册
    ,o.cpp_arc_flag -- 是否涉农企业： 0 否 1 是
    ,o.cpp_grn_flag -- 是否绿色企业： 0 否 1 是
    ,o.cpp_sci_flag -- 是否科技企业： 0 否 1 是
    ,o.cpp_pop_flag -- 是否民营企业： 0 否 1 是
    ,o.cpp_province -- 贴现申请人省份：参见附录省份代码
    ,o.reg_status -- 登记状态： DRS01 正常 DRS02 失效
    ,o.process_code -- 处理码
    ,o.process_msg -- 处理信息
...
from ${iol_schema}.bdms_cpp_cust_register_info_bk o
    left join ${iol_schema}.bdms_cpp_cust_register_info_op n
        on
            o.id = n.id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.bdms_cpp_cust_register_info_cl d
        on
            o.id = d.id
...
--truncate table ${iol_schema}.bdms_cpp_cust_register_info;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('bdms_cpp_cust_register_info') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.bdms_cpp_cust_register_info drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.bdms_cpp_cust_register_info add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.bdms_cpp_cust_register_info exchange partition p_${batch_date} with table ${iol_schema}.bdms_cpp_cust_register_info_cl;
alter table ${iol_schema}.bdms_cpp_cust_register_info exchange partition p_20991231 with table ${iol_schema}.bdms_cpp_cust_register_info_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.bdms_cpp_cust_register_info_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'bdms_cpp_cust_register_info',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
