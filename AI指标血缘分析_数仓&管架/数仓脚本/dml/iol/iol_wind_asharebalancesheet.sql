...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_wind_asharebalancesheet
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
drop table ${iol_schema}.wind_asharebalancesheet_ex purge;
alter table ${iol_schema}.wind_asharebalancesheet add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.2 truncate target table batch_date partition
whenever sqlerror exit sql.sqlcode;
alter table ${iol_schema}.wind_asharebalancesheet truncate partition p_${batch_date};

-- 2.3 insert data to ex table
create table ${iol_schema}.wind_asharebalancesheet_ex nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.wind_asharebalancesheet where 0=1;

insert /*+ append */ into ${iol_schema}.wind_asharebalancesheet_ex(
    object_id -- 对象ID
...
from ${itl_schema}.wind_asharebalancesheet
where etl_dt = to_date('${batch_date}', 'yyyymmdd')
;

-- 2.4 exchage ex table and target table
alter table ${iol_schema}.wind_asharebalancesheet exchange partition p_${batch_date} with table ${iol_schema}.wind_asharebalancesheet_ex;
...
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'wind_asharebalancesheet',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);