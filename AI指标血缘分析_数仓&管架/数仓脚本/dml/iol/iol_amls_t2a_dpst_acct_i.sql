...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_amls_t2a_dpst_acct_i
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.amls_t2a_dpst_acct_i_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.amls_t2a_dpst_acct_i
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.amls_t2a_dpst_acct_i_op purge;
drop table ${iol_schema}.amls_t2a_dpst_acct_i_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.amls_t2a_dpst_acct_i_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.amls_t2a_dpst_acct_i where 0=1;

create table ${iol_schema}.amls_t2a_dpst_acct_i_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.amls_t2a_dpst_acct_i where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.amls_t2a_dpst_acct_i_cl(
            acct_id -- 账户编号
            ,cust_id -- 客户编号
            ,cust_name -- 客户名称
            ,org_id -- 开户机构编号
            ...
            ,card_style -- 借贷记卡标识
            ,main_acct_id -- 主账号
            ,acc_type1 -- 个人账户种类(11：代表I类账户；12：代表II类账户；13：代表III类账户；14：代表信用卡账户)
            ,is_merch -- 银行卡收单账户标识(11：是；12：否)
            ,is_ebank -- 账户网银标识(11：开通；12：未开通)
            ,mobile_bank_phone -- 手机银行电话号码
            ,open_chnl -- 开户渠道
            ,agent_flag -- 是否代理开户(代理11；本人12；批量开户13)
            ,agent_tel -- 代理人联系方式
            ,last_occur_dt -- ATM机具所属行行号
        ...
        into ${iol_schema}.amls_t2a_dpst_acct_i_op(
            acct_id -- 账户编号
            ,cust_id -- 客户编号
            ,cust_name -- 客户名称
            ,org_id -- 开户机构编号
...
select
    ...
    ,nvl(n.acc_type1, o.acc_type1) as acc_type1 -- 个人账户种类(11：代表I类账户；12：代表II类账户；13：代表III类账户；14：代表信用卡账户)
    ,nvl(n.is_merch, o.is_merch) as is_merch -- 银行卡收单账户标识(11：是；12：否)
    ,nvl(n.is_ebank, o.is_ebank) as is_ebank -- 账户网银标识(11：开通；12：未开通)
    ...
    ,nvl(n.last_occur_dt, o.last_occur_dt) as last_occur_dt -- ATM机具所属行行号
...
from (select * from ${iol_schema}.amls_t2a_dpst_acct_i_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.amls_t2a_dpst_acct_i where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.acct_id = n.acct_id
where (
        o.acct_id is null
...
select
    ...
    ,o.card_style -- 借贷记卡标识
    ,o.main_acct_id -- 主账号
    ,o.acc_type1 -- 个人账户种类(11：代表I类账户；12：代表II类账户；13：代表III类账户；14：代表信用卡账户)
    ,o.is_merch -- 银行卡收单账户标识(11：是；12：否)
    ,o.is_ebank -- 账户网银标识(11：开通；12：未开通)
    ,o.mobile_bank_phone -- 手机银行电话号码
    ,o.open_chnl -- 开户渠道
    ,o.agent_flag -- 是否代理开户(代理11；本人12；批量开户13)
    ,o.agent_tel -- 代理人联系方式
    ,o.last_occur_dt -- ATM机具所属行行号
...
from ${iol_schema}.amls_t2a_dpst_acct_i_bk o
    left join ${iol_schema}.amls_t2a_dpst_acct_i_op n
        on
            o.acct_id = n.acct_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.amls_t2a_dpst_acct_i_cl d
        on
            o.acct_id = d.acct_id
...
--truncate table ${iol_schema}.amls_t2a_dpst_acct_i;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('amls_t2a_dpst_acct_i') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.amls_t2a_dpst_acct_i drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.amls_t2a_dpst_acct_i add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.amls_t2a_dpst_acct_i exchange partition p_${batch_date} with table ${iol_schema}.amls_t2a_dpst_acct_i_cl;
alter table ${iol_schema}.amls_t2a_dpst_acct_i exchange partition p_20991231 with table ${iol_schema}.amls_t2a_dpst_acct_i_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.amls_t2a_dpst_acct_i_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'amls_t2a_dpst_acct_i',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
