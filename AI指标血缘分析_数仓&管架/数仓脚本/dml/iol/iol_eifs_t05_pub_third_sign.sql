...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_eifs_t05_pub_third_sign
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.eifs_t05_pub_third_sign_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.eifs_t05_pub_third_sign
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.eifs_t05_pub_third_sign_op purge;
drop table ${iol_schema}.eifs_t05_pub_third_sign_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.eifs_t05_pub_third_sign_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.eifs_t05_pub_third_sign where 0=1;

create table ${iol_schema}.eifs_t05_pub_third_sign_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.eifs_t05_pub_third_sign where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.eifs_t05_pub_third_sign_cl(
            ...
            ,settle_act -- 清算账号
            ,entente_dt -- 协约日期
            ,settle_bank_id -- 清算银行号
            ,settle_bank_name -- 清算银行名称
        ...
        into ${iol_schema}.eifs_t05_pub_third_sign_op(
            sign_contract_id -- 签约id
            ,agreement_id -- 签约编号
            ,agreement_item_seq_id -- 协议项编号
            ,sp_id -- 第三方代码
...
select
    ...
    ,nvl(n.settle_act, o.settle_act) as settle_act -- 清算账号
    ,nvl(n.entente_dt, o.entente_dt) as entente_dt -- 协约日期
    ,nvl(n.settle_bank_id, o.settle_bank_id) as settle_bank_id -- 清算银行号
    ,nvl(n.settle_bank_name, o.settle_bank_name) as settle_bank_name -- 清算银行名称
...
from (select * from ${iol_schema}.eifs_t05_pub_third_sign_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.eifs_t05_pub_third_sign where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
where (
    ...
    or (
        ...
        or o.settle_bank_id <> n.settle_bank_id
...
select
    ...
    ,o.settle_act -- 清算账号
    ,o.entente_dt -- 协约日期
    ,o.settle_bank_id -- 清算银行号
    ,o.settle_bank_name -- 清算银行名称
...
from ${iol_schema}.eifs_t05_pub_third_sign_bk o
    left join ${iol_schema}.eifs_t05_pub_third_sign_op n
        on
            o.sign_contract_id = n.sign_contract_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.eifs_t05_pub_third_sign_cl d
        on
...
--truncate table ${iol_schema}.eifs_t05_pub_third_sign;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('eifs_t05_pub_third_sign') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.eifs_t05_pub_third_sign drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.eifs_t05_pub_third_sign add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.eifs_t05_pub_third_sign exchange partition p_${batch_date} with table ${iol_schema}.eifs_t05_pub_third_sign_cl;
alter table ${iol_schema}.eifs_t05_pub_third_sign exchange partition p_20991231 with table ${iol_schema}.eifs_t05_pub_third_sign_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.eifs_t05_pub_third_sign_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'eifs_t05_pub_third_sign',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
