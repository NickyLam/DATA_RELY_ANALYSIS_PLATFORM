...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_nfss_tcs_tbprddaily
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.nfss_tcs_tbprddaily_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.nfss_tcs_tbprddaily;

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.nfss_tcs_tbprddaily_op purge;
drop table ${iol_schema}.nfss_tcs_tbprddaily_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.nfss_tcs_tbprddaily_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.nfss_tcs_tbprddaily where 0=1;

create table ${iol_schema}.nfss_tcs_tbprddaily_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.nfss_tcs_tbprddaily where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.nfss_tcs_tbprddaily_cl(
            iss_date -- 发布日期
            ,cfm_date -- 确认日期(当天日期)
            ,prd_code -- 产品代码
            ,ta_code -- TA代码
        ...
        into ${iol_schema}.nfss_tcs_tbprddaily_op(
            iss_date -- 发布日期
            ,cfm_date -- 确认日期(当天日期)
            ,prd_code -- 产品代码
            ,ta_code -- TA代码
...
from (select * from ${iol_schema}.nfss_tcs_tbprddaily_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.nfss_tcs_tbprddaily where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
select
    ...
    ,o.id_mark -- 增删标志
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间
from ${iol_schema}.nfss_tcs_tbprddaily_bk o
    left join ${iol_schema}.nfss_tcs_tbprddaily_op n
        on
            o.iss_date = n.iss_date
            and o.cfm_date = n.cfm_date
            and o.prd_code = n.prd_code
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.nfss_tcs_tbprddaily_cl d
        on
...
-- truncate table ${iol_schema}.nfss_tcs_tbprddaily;

-- 4.2 exchange partition
alter table ${iol_schema}.nfss_tcs_tbprddaily exchange partition p_19000101 with table ${iol_schema}.nfss_tcs_tbprddaily_cl;
alter table ${iol_schema}.nfss_tcs_tbprddaily exchange partition p_20991231 with table ${iol_schema}.nfss_tcs_tbprddaily_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.nfss_tcs_tbprddaily_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'nfss_tcs_tbprddaily',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
