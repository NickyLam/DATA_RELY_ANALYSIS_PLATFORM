...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_mrms_tbl_df_order_txn
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.mrms_tbl_df_order_txn_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.mrms_tbl_df_order_txn
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.mrms_tbl_df_order_txn_op purge;
drop table ${iol_schema}.mrms_tbl_df_order_txn_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.mrms_tbl_df_order_txn_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.mrms_tbl_df_order_txn where 0=1;

create table ${iol_schema}.mrms_tbl_df_order_txn_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.mrms_tbl_df_order_txn where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.mrms_tbl_df_order_txn_cl(
            key_rsp -- 流水号
            ,batch_no -- 批次号
            ,file_name -- 文件名
        ...
        into ${iol_schema}.mrms_tbl_df_order_txn_op(
            key_rsp -- 流水号
            ,batch_no -- 批次号
            ,file_name -- 文件名
            ,up_file_name -- 上传中台文件名
...
from (select * from ${iol_schema}.mrms_tbl_df_order_txn_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.mrms_tbl_df_order_txn where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.key_rsp = n.key_rsp
where (
        o.key_rsp is null
...
from ${iol_schema}.mrms_tbl_df_order_txn_bk o
    left join ${iol_schema}.mrms_tbl_df_order_txn_op n
        on
            o.key_rsp = n.key_rsp
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.mrms_tbl_df_order_txn_cl d
        on
            o.key_rsp = d.key_rsp
...
--truncate table ${iol_schema}.mrms_tbl_df_order_txn;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('mrms_tbl_df_order_txn') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.mrms_tbl_df_order_txn drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.mrms_tbl_df_order_txn add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.mrms_tbl_df_order_txn exchange partition p_${batch_date} with table ${iol_schema}.mrms_tbl_df_order_txn_cl;
alter table ${iol_schema}.mrms_tbl_df_order_txn exchange partition p_20991231 with table ${iol_schema}.mrms_tbl_df_order_txn_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.mrms_tbl_df_order_txn_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'mrms_tbl_df_order_txn',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
