...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_rcds_src_dw_agt_ln_ac_base_info
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
drop table ${iol_schema}.rcds_src_dw_agt_ln_ac_base_info_ex purge;
alter table ${iol_schema}.rcds_src_dw_agt_ln_ac_base_info add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.2 truncate target table
whenever sqlerror exit sql.sqlcode;
truncate table ${iol_schema}.rcds_src_dw_agt_ln_ac_base_info;

-- 2.3 insert data to ex table
create table ${iol_schema}.rcds_src_dw_agt_ln_ac_base_info_ex nologging
compress
as
select * from ${iol_schema}.rcds_src_dw_agt_ln_ac_base_info where 0=1;

insert /*+ append */ into ${iol_schema}.rcds_src_dw_agt_ln_ac_base_info_ex(
    data_src_cd -- 数据来源代码
    ,del_flg -- 删除标志
...
from ${itl_schema}.rcds_src_dw_agt_ln_ac_base_info
where etl_dt = to_date('${batch_date}', 'yyyymmdd')
;

-- 2.4 exchage ex table and target table
alter table ${iol_schema}.rcds_src_dw_agt_ln_ac_base_info exchange partition p_${batch_date} with table ${iol_schema}.rcds_src_dw_agt_ln_ac_base_info_ex;
...
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'rcds_src_dw_agt_ln_ac_base_info',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);