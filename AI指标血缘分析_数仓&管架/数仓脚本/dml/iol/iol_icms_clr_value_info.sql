...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_icms_clr_value_info
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.icms_clr_value_info_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.icms_clr_value_info
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.icms_clr_value_info_op purge;
drop table ${iol_schema}.icms_clr_value_info_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.icms_clr_value_info_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.icms_clr_value_info where 0=1;

create table ${iol_schema}.icms_clr_value_info_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.icms_clr_value_info where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.icms_clr_value_info_cl(
            clrid -- 押品编号
            ...
            ,outevalmethod -- 评估方法
            ,outevalflag -- 是否有外部预评估报告
            ,outevalamt1 -- 外部预评估报告的评估价值
            ,outevaldate -- 外部评估基准日
            ,outevalamt -- 外部正式评估报告的评估价值
            ,evalamt -- 内部评估价值
            ,evalamt2 -- 申请评估确认价值
            ,businessinsid -- 流程编号（入库初评-出入库任务流水号、价值重估为-押品变更任务流水号、自动重估该字段为空）
            ,confmamt -- 我行确认价值
            ,condate -- 评估认定日期
        ...
        into ${iol_schema}.icms_clr_value_info_op(
            clrid -- 押品编号
            ,evalmode -- 评估方式 01-外部评估、02-内部评估、03-外部和内部评估
            ,evaldate -- 估值日期
            ,curreny -- 币种
...
select
    ...
    ,nvl(n.outevalmethod, o.outevalmethod) as outevalmethod -- 评估方法
    ,nvl(n.outevalflag, o.outevalflag) as outevalflag -- 是否有外部预评估报告
    ,nvl(n.outevalamt1, o.outevalamt1) as outevalamt1 -- 外部预评估报告的评估价值
    ,nvl(n.outevaldate, o.outevaldate) as outevaldate -- 外部评估基准日
    ...
    ,nvl(n.businessinsid, o.businessinsid) as businessinsid -- 流程编号（入库初评-出入库任务流水号、价值重估为-押品变更任务流水号、自动重估该字段为空）
    ...
    ,nvl(n.status, o.status) as status -- 流程状态 1 流程中 0正常
...
from (select * from ${iol_schema}.icms_clr_value_info_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.icms_clr_value_info where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.clrid = n.clrid
where (
        o.clrid is null
...
select
    ...
    ,o.outevalmethod -- 评估方法
    ,o.outevalflag -- 是否有外部预评估报告
    ,o.outevalamt1 -- 外部预评估报告的评估价值
    ,o.outevaldate -- 外部评估基准日
    ,o.outevalamt -- 外部正式评估报告的评估价值
    ,o.evalamt -- 内部评估价值
    ,o.evalamt2 -- 申请评估确认价值
    ,o.businessinsid -- 流程编号（入库初评-出入库任务流水号、价值重估为-押品变更任务流水号、自动重估该字段为空）
    ,o.confmamt -- 我行确认价值
    ,o.condate -- 评估认定日期
...
from ${iol_schema}.icms_clr_value_info_bk o
    left join ${iol_schema}.icms_clr_value_info_op n
        on
            o.clrid = n.clrid
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.icms_clr_value_info_cl d
        on
            o.clrid = d.clrid
...
--truncate table ${iol_schema}.icms_clr_value_info;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('icms_clr_value_info') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.icms_clr_value_info drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.icms_clr_value_info add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.icms_clr_value_info exchange partition p_${batch_date} with table ${iol_schema}.icms_clr_value_info_cl;
alter table ${iol_schema}.icms_clr_value_info exchange partition p_20991231 with table ${iol_schema}.icms_clr_value_info_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.icms_clr_value_info_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'icms_clr_value_info',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
