...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_rsts_rcd_ir_grade_a_score_input
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.rsts_rcd_ir_grade_a_score_input_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.rsts_rcd_ir_grade_a_score_input
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.rsts_rcd_ir_grade_a_score_input_op purge;
drop table ${iol_schema}.rsts_rcd_ir_grade_a_score_input_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.rsts_rcd_ir_grade_a_score_input_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.rsts_rcd_ir_grade_a_score_input where 0=1;

create table ${iol_schema}.rsts_rcd_ir_grade_a_score_input_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.rsts_rcd_ir_grade_a_score_input where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.rsts_rcd_ir_grade_a_score_input_cl(
        ...
        into ${iol_schema}.rsts_rcd_ir_grade_a_score_input_op(
            grade_key_id -- 申请评分流水号
            ,serialno -- 申请流水号
            ,data_time -- 数据记录时间
            ,a_ln_cm_cnt_npf_secure_p -- 未结清抵押贷款的账户数/所有未结清贷款的账户数
...
from (select * from ${iol_schema}.rsts_rcd_ir_grade_a_score_input_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.rsts_rcd_ir_grade_a_score_input where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
from ${iol_schema}.rsts_rcd_ir_grade_a_score_input_bk o
    left join ${iol_schema}.rsts_rcd_ir_grade_a_score_input_op n
        on
            o.grade_key_id = n.grade_key_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.rsts_rcd_ir_grade_a_score_input_cl d
        on
...
--truncate table ${iol_schema}.rsts_rcd_ir_grade_a_score_input;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('rsts_rcd_ir_grade_a_score_input') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.rsts_rcd_ir_grade_a_score_input drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.rsts_rcd_ir_grade_a_score_input add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.rsts_rcd_ir_grade_a_score_input exchange partition p_${batch_date} with table ${iol_schema}.rsts_rcd_ir_grade_a_score_input_cl;
alter table ${iol_schema}.rsts_rcd_ir_grade_a_score_input exchange partition p_20991231 with table ${iol_schema}.rsts_rcd_ir_grade_a_score_input_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.rsts_rcd_ir_grade_a_score_input_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'rsts_rcd_ir_grade_a_score_input',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
