...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_ncbs_cl_irregular_schedule_hist
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.ncbs_cl_irregular_schedule_hist_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.ncbs_cl_irregular_schedule_hist
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.ncbs_cl_irregular_schedule_hist_op purge;
drop table ${iol_schema}.ncbs_cl_irregular_schedule_hist_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.ncbs_cl_irregular_schedule_hist_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ncbs_cl_irregular_schedule_hist where 0=1;

create table ${iol_schema}.ncbs_cl_irregular_schedule_hist_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ncbs_cl_irregular_schedule_hist where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.ncbs_cl_irregular_schedule_hist_cl(
            amt_type -- 金额类型
            ,client_no -- 客户编号
            ,internal_key -- 账户内部键值
            ,period_freq -- 频率id
        ...
        into ${iol_schema}.ncbs_cl_irregular_schedule_hist_op(
            amt_type -- 金额类型
            ,client_no -- 客户编号
            ,internal_key -- 账户内部键值
            ,period_freq -- 频率id
...
from (select * from ${iol_schema}.ncbs_cl_irregular_schedule_hist_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.ncbs_cl_irregular_schedule_hist where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
from ${iol_schema}.ncbs_cl_irregular_schedule_hist_bk o
    left join ${iol_schema}.ncbs_cl_irregular_schedule_hist_op n
        on
            o.internal_key = n.internal_key
            and o.sched_seq_no = n.sched_seq_no
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.ncbs_cl_irregular_schedule_hist_cl d
        on
...
--truncate table ${iol_schema}.ncbs_cl_irregular_schedule_hist;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('ncbs_cl_irregular_schedule_hist') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.ncbs_cl_irregular_schedule_hist drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.ncbs_cl_irregular_schedule_hist add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.ncbs_cl_irregular_schedule_hist exchange partition p_${batch_date} with table ${iol_schema}.ncbs_cl_irregular_schedule_hist_cl;
alter table ${iol_schema}.ncbs_cl_irregular_schedule_hist exchange partition p_20991231 with table ${iol_schema}.ncbs_cl_irregular_schedule_hist_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.ncbs_cl_irregular_schedule_hist_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ncbs_cl_irregular_schedule_hist',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
