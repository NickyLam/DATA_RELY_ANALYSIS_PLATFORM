...
drop table ${iol_schema}.rwas_rpt_asset_manager_result_ex purge;
alter table ${iol_schema}.rwas_rpt_asset_manager_result add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.2 truncate target table batch_date partition
whenever sqlerror exit sql.sqlcode;
alter table ${iol_schema}.rwas_rpt_asset_manager_result truncate partition p_${batch_date};

-- 2.3 insert data to ex table
create table ${iol_schema}.rwas_rpt_asset_manager_result_ex nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.rwas_rpt_asset_manager_result where 0=1;

insert /*+ append */ into ${iol_schema}.rwas_rpt_asset_manager_result_ex(
    data_date -- 数据日期
    ,pk_col -- PK_COL
    ,loan_ref_no -- 债项编号
    ,fund_cd -- 资产管理产品编号
...
from ${itl_schema}.rwas_rpt_asset_manager_result
where etl_dt = to_date('${batch_date}', 'yyyymmdd')
;

-- 2.4 exchage ex table and target table
alter table ${iol_schema}.rwas_rpt_asset_manager_result exchange partition p_${batch_date} with table ${iol_schema}.rwas_rpt_asset_manager_result_ex;
...
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'rwas_rpt_asset_manager_result',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);