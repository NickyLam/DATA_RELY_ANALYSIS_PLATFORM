...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_tbps_cpr_trade_flow
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.tbps_cpr_trade_flow_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.tbps_cpr_trade_flow
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.tbps_cpr_trade_flow_op purge;
drop table ${iol_schema}.tbps_cpr_trade_flow_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.tbps_cpr_trade_flow_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.tbps_cpr_trade_flow where 0=1;

create table ${iol_schema}.tbps_cpr_trade_flow_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.tbps_cpr_trade_flow where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.tbps_cpr_trade_flow_cl(
            ...
            ,ctf_ecifname -- 客户姓名
            ,ctf_menuid -- 
            ,ctf_state -- 交易状态(90:交易成功;99:交易失败;33:通讯异常;50:交易初始状态;10:待授权;11:授权中;12:授权完成)
        ...
        into ${iol_schema}.tbps_cpr_trade_flow_op(
            ctf_trade_flowno -- 流水号
            ,ctf_transtime -- 交易时间
            ,ctf_transdate -- 交易日期
            ,ctf_transcode -- 交易码
...
select
    ...
    ,nvl(n.ctf_state, o.ctf_state) as ctf_state -- 交易状态(90:交易成功;99:交易失败;33:通讯异常;50:交易初始状态;10:待授权;11:授权中;12:授权完成)
    ...
    ,nvl(n.ctf_senddate, o.ctf_senddate) as ctf_senddate -- 摘要
    ,nvl(n.ctf_sendtime, o.ctf_sendtime) as ctf_sendtime -- 交易真正提交核心的时间
    ,nvl(n.ctf_totalcount, o.ctf_totalcount) as ctf_totalcount -- 
    ,nvl(n.ctf_remark, o.ctf_remark) as ctf_remark -- 
...
from (select * from ${iol_schema}.tbps_cpr_trade_flow_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.tbps_cpr_trade_flow where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
select
    ...
    ,o.ctf_ecifname -- 客户姓名
    ,o.ctf_menuid -- 
    ,o.ctf_state -- 交易状态(90:交易成功;99:交易失败;33:通讯异常;50:交易初始状态;10:待授权;11:授权中;12:授权完成)
...
from ${iol_schema}.tbps_cpr_trade_flow_bk o
    left join ${iol_schema}.tbps_cpr_trade_flow_op n
        on
            o.ctf_trade_flowno = n.ctf_trade_flowno
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.tbps_cpr_trade_flow_cl d
        on
...
--truncate table ${iol_schema}.tbps_cpr_trade_flow;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('tbps_cpr_trade_flow') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.tbps_cpr_trade_flow drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.tbps_cpr_trade_flow add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.tbps_cpr_trade_flow exchange partition p_${batch_date} with table ${iol_schema}.tbps_cpr_trade_flow_cl;
alter table ${iol_schema}.tbps_cpr_trade_flow exchange partition p_20991231 with table ${iol_schema}.tbps_cpr_trade_flow_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.tbps_cpr_trade_flow_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'tbps_cpr_trade_flow',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
