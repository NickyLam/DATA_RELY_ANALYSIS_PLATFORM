...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_ifms_tbsharedetail4
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.ifms_tbsharedetail4_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.ifms_tbsharedetail4;

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.ifms_tbsharedetail4_op purge;
drop table ${iol_schema}.ifms_tbsharedetail4_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.ifms_tbsharedetail4_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ifms_tbsharedetail4 where 0=1;

create table ${iol_schema}.ifms_tbsharedetail4_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ifms_tbsharedetail4 where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.ifms_tbsharedetail4_cl(
            in_client_no -- 
            ,seller_code -- 
            ,bank_no -- 
            ,client_no -- 
        ...
        into ${iol_schema}.ifms_tbsharedetail4_op(
            in_client_no -- 
            ,seller_code -- 
            ,bank_no -- 
            ,client_no -- 
...
from (select * from ${iol_schema}.ifms_tbsharedetail4_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.ifms_tbsharedetail4 where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
select
    ...
    ,o.id_mark -- 增删标志
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间
from ${iol_schema}.ifms_tbsharedetail4_bk o
    left join ${iol_schema}.ifms_tbsharedetail4_op n
        on
            o.in_client_no = n.in_client_no
            and o.seller_code = n.seller_code
            and o.bank_no = n.bank_no
    ...
    left join ${iol_schema}.ifms_tbsharedetail4_cl d
        on
...
-- truncate table ${iol_schema}.ifms_tbsharedetail4;

-- 4.2 exchange partition
alter table ${iol_schema}.ifms_tbsharedetail4 exchange partition p_19000101 with table ${iol_schema}.ifms_tbsharedetail4_cl;
alter table ${iol_schema}.ifms_tbsharedetail4 exchange partition p_20991231 with table ${iol_schema}.ifms_tbsharedetail4_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.ifms_tbsharedetail4_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ifms_tbsharedetail4',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
