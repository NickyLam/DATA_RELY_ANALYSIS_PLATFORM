...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_ncbs_rb_notran_list
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.ncbs_rb_notran_list_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.ncbs_rb_notran_list
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.ncbs_rb_notran_list_op purge;
drop table ${iol_schema}.ncbs_rb_notran_list_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.ncbs_rb_notran_list_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ncbs_rb_notran_list where 0=1;

create table ${iol_schema}.ncbs_rb_notran_list_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ncbs_rb_notran_list where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.ncbs_rb_notran_list_cl(
            acct_status -- 账户状态
        ...
        into ${iol_schema}.ncbs_rb_notran_list_op(
            acct_status -- 账户状态
            ,base_acct_no -- 交易账号/卡号
            ,client_name -- 客户名称
            ,client_no -- 客户编号
...
select
    ...
    ,nvl(n.add_flag, o.add_flag) as add_flag -- 添加标识
...
from (select * from ${iol_schema}.ncbs_rb_notran_list_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.ncbs_rb_notran_list where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
from ${iol_schema}.ncbs_rb_notran_list_bk o
    left join ${iol_schema}.ncbs_rb_notran_list_op n
        on
            o.batch_no = n.batch_no
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.ncbs_rb_notran_list_cl d
        on
...
--truncate table ${iol_schema}.ncbs_rb_notran_list;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('ncbs_rb_notran_list') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.ncbs_rb_notran_list drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.ncbs_rb_notran_list add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.ncbs_rb_notran_list exchange partition p_${batch_date} with table ${iol_schema}.ncbs_rb_notran_list_cl;
alter table ${iol_schema}.ncbs_rb_notran_list exchange partition p_20991231 with table ${iol_schema}.ncbs_rb_notran_list_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.ncbs_rb_notran_list_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ncbs_rb_notran_list',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
