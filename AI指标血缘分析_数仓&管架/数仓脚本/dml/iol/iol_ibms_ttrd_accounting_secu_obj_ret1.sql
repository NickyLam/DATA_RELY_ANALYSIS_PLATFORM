...
truncate table ibms_ttrd_accounting_secu_obj;
...
begin
	for tb in (select partition_name,table_name,substr(partition_name,3) as etl_dt 
	            ...
	            where table_name = upper('ibms_ttrd_accounting_secu_obj_bak_${batch_date}')
	              and partition_name <> 'P_19000101') loop
	             
  select count(*) into v_flag
    from all_tab_partitions 
   where table_owner = upper('IOL')
     and table_name = upper('ibms_ttrd_accounting_secu_obj')
     and partition_name = tb.partition_name;
     
  if v_flag <> 0 then
     execute immediate 'alter table ibms_ttrd_accounting_secu_obj drop partition '|| tb.partition_name ;
     
  end if;
  
     execute immediate 'alter table ibms_ttrd_accounting_secu_obj add partition ' || tb.partition_name || ' values (to_date(' || tb.etl_dt || ',''yyyymmdd''))';

insert /*+ append */ into ${iol_schema}.ibms_ttrd_accounting_secu_obj(
    obj_id -- 对象id
    ,tsk_id -- 任务id
    ,beg_date -- 开始日期
    ,end_date -- 结束日期
...
from ibms_ttrd_accounting_secu_obj_bak_${batch_date}
