...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_ncbs_rb_pcp_agreement
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.ncbs_rb_pcp_agreement_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.ncbs_rb_pcp_agreement
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.ncbs_rb_pcp_agreement_op purge;
drop table ${iol_schema}.ncbs_rb_pcp_agreement_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.ncbs_rb_pcp_agreement_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ncbs_rb_pcp_agreement where 0=1;

create table ${iol_schema}.ncbs_rb_pcp_agreement_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ncbs_rb_pcp_agreement where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.ncbs_rb_pcp_agreement_cl(
            acct_seq_no -- 账户子账号
            ...
            ,approval_no -- 审批单号
            ,merge_cycle_flag -- 合并结息标识y-合并n-不合并
            ,start_dt -- 开始时间
            ,end_dt -- 结束时间
            ,id_mark -- 增删标志
        ...
        into ${iol_schema}.ncbs_rb_pcp_agreement_op(
            acct_seq_no -- 账户子账号
            ,base_acct_no -- 交易账号/卡号
            ,client_no -- 客户编号
            ,internal_key -- 账户内部键值
...
select
    ...
    ,nvl(n.agree_int_rate, o.agree_int_rate) as agree_int_rate -- 协议利率
    ...
    ,nvl(n.pcp_prod_type, o.pcp_prod_type) as pcp_prod_type -- 资金池产品类型
    ...
    ,nvl(n.merge_cycle_flag, o.merge_cycle_flag) as merge_cycle_flag -- 合并结息标识y-合并n-不合并
    ,case when
...
from (select * from ${iol_schema}.ncbs_rb_pcp_agreement_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.ncbs_rb_pcp_agreement where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
where (
    ...
    or (
        ...
        or o.acct_int_flag <> n.acct_int_flag
        or o.agreement_status <> n.agreement_status
        or o.cash_pool_attr <> n.cash_pool_attr
        ...
        or o.agree_int_rate <> n.agree_int_rate
        ...
        or o.pcp_prod_type <> n.pcp_prod_type
...
select
    ...
    ,o.prod_type -- 产品编号
    ,o.user_id -- 交易柜员编号
    ,o.acct_int_flag -- 计息标志
    ,o.agreement_id -- 协议编号
    ,o.agreement_status -- 协议状态
    ,o.cash_pool_attr -- 资金池属性
    ,o.company -- 法人
    ...
    ,o.approval_no -- 审批单号
    ,o.merge_cycle_flag -- 合并结息标识y-合并n-不合并
    ,o.start_dt -- 开始时间
...
from ${iol_schema}.ncbs_rb_pcp_agreement_bk o
    left join ${iol_schema}.ncbs_rb_pcp_agreement_op n
        on
            o.agreement_id = n.agreement_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.ncbs_rb_pcp_agreement_cl d
        on
...
--truncate table ${iol_schema}.ncbs_rb_pcp_agreement;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('ncbs_rb_pcp_agreement') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.ncbs_rb_pcp_agreement drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.ncbs_rb_pcp_agreement add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.ncbs_rb_pcp_agreement exchange partition p_${batch_date} with table ${iol_schema}.ncbs_rb_pcp_agreement_cl;
alter table ${iol_schema}.ncbs_rb_pcp_agreement exchange partition p_20991231 with table ${iol_schema}.ncbs_rb_pcp_agreement_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.ncbs_rb_pcp_agreement_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ncbs_rb_pcp_agreement',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
