...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_bdms_cpes_pay_confirm_contract
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.bdms_cpes_pay_confirm_contract_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.bdms_cpes_pay_confirm_contract;

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.bdms_cpes_pay_confirm_contract_op purge;
drop table ${iol_schema}.bdms_cpes_pay_confirm_contract_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.bdms_cpes_pay_confirm_contract_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.bdms_cpes_pay_confirm_contract where 0=1;

create table ${iol_schema}.bdms_cpes_pay_confirm_contract_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.bdms_cpes_pay_confirm_contract where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.bdms_cpes_pay_confirm_contract_cl(
            id -- 
            ,contract_no -- 批次号
            ,product_no -- 产品号
            ...
            ,branch_no -- 机构号
            ,apply_date -- 业务申请日期
            ,pay_apply_name -- 付款确认申请人名称
            ,pay_apply_brh_no -- 付款确认申请人机构号
            ,pay_confirm_type -- 付款确认类型： VM01 影像验证 VM02 实物验证
            ,add_flag -- 补充标识： VN01 自动新建 VN02 手动新建 VN03 应答发起补录影像 VN04 应答发起实物验证
            ,department_no -- 所属部门号
            ,manage_no -- 客户经理号
        ...
        into ${iol_schema}.bdms_cpes_pay_confirm_contract_op(
            id -- 
            ,contract_no -- 批次号
            ,product_no -- 产品号
            ,buss_flag -- 业务类型： 01 申请 02 签收
...
select
    ...
    ,nvl(n.pay_apply_name, o.pay_apply_name) as pay_apply_name -- 付款确认申请人名称
    ,nvl(n.pay_apply_brh_no, o.pay_apply_brh_no) as pay_apply_brh_no -- 付款确认申请人机构号
    ,nvl(n.pay_confirm_type, o.pay_confirm_type) as pay_confirm_type -- 付款确认类型： VM01 影像验证 VM02 实物验证
    ,nvl(n.add_flag, o.add_flag) as add_flag -- 补充标识： VN01 自动新建 VN02 手动新建 VN03 应答发起补录影像 VN04 应答发起实物验证
...
from (select * from ${iol_schema}.bdms_cpes_pay_confirm_contract_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.bdms_cpes_pay_confirm_contract where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.id = n.id
where (
        o.id is null
    ...
    or (
        ...
        or o.pay_apply_name <> n.pay_apply_name
        or o.pay_apply_brh_no <> n.pay_apply_brh_no
        or o.pay_confirm_type <> n.pay_confirm_type
        or o.add_flag <> n.add_flag
...
select
    ...
    ,o.branch_no -- 机构号
    ,o.apply_date -- 业务申请日期
    ,o.pay_apply_name -- 付款确认申请人名称
    ,o.pay_apply_brh_no -- 付款确认申请人机构号
    ,o.pay_confirm_type -- 付款确认类型： VM01 影像验证 VM02 实物验证
    ,o.add_flag -- 补充标识： VN01 自动新建 VN02 手动新建 VN03 应答发起补录影像 VN04 应答发起实物验证
    ...
    ,o.id_mark -- 增删标志
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间
from ${iol_schema}.bdms_cpes_pay_confirm_contract_bk o
    left join ${iol_schema}.bdms_cpes_pay_confirm_contract_op n
        on
            o.id = n.id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.bdms_cpes_pay_confirm_contract_cl d
        on
            o.id = d.id
...
-- truncate table ${iol_schema}.bdms_cpes_pay_confirm_contract;

-- 4.2 exchange partition
alter table ${iol_schema}.bdms_cpes_pay_confirm_contract exchange partition p_19000101 with table ${iol_schema}.bdms_cpes_pay_confirm_contract_cl;
alter table ${iol_schema}.bdms_cpes_pay_confirm_contract exchange partition p_20991231 with table ${iol_schema}.bdms_cpes_pay_confirm_contract_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.bdms_cpes_pay_confirm_contract_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'bdms_cpes_pay_confirm_contract',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
