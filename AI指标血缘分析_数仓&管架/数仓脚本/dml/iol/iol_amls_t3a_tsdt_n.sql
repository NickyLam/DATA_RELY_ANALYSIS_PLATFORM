...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_amls_t3a_tsdt_n
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.amls_t3a_tsdt_n_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.amls_t3a_tsdt_n
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.amls_t3a_tsdt_n_op purge;
drop table ${iol_schema}.amls_t3a_tsdt_n_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.amls_t3a_tsdt_n_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.amls_t3a_tsdt_n where 0=1;

create table ${iol_schema}.amls_t3a_tsdt_n_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.amls_t3a_tsdt_n where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.amls_t3a_tsdt_n_cl(
            rpt_id -- 报告编号
            ,stat_dt -- 数据日期
            ,cbif_seq -- 客户序号
            ,crcd -- 大额交易特征代码
        ...
        into ${iol_schema}.amls_t3a_tsdt_n_op(
            rpt_id -- 报告编号
            ,stat_dt -- 数据日期
            ,cbif_seq -- 客户序号
            ,crcd -- 大额交易特征代码
...
from (select * from ${iol_schema}.amls_t3a_tsdt_n_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.amls_t3a_tsdt_n where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.rpt_id = n.rpt_id
...
from ${iol_schema}.amls_t3a_tsdt_n_bk o
    left join ${iol_schema}.amls_t3a_tsdt_n_op n
        on
            o.rpt_id = n.rpt_id
            and o.cbif_seq = n.cbif_seq
            and o.crcd = n.crcd
    ...
    left join ${iol_schema}.amls_t3a_tsdt_n_cl d
        on
            o.rpt_id = d.rpt_id
...
--truncate table ${iol_schema}.amls_t3a_tsdt_n;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('amls_t3a_tsdt_n') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.amls_t3a_tsdt_n drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.amls_t3a_tsdt_n add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.amls_t3a_tsdt_n exchange partition p_${batch_date} with table ${iol_schema}.amls_t3a_tsdt_n_cl;
alter table ${iol_schema}.amls_t3a_tsdt_n exchange partition p_20991231 with table ${iol_schema}.amls_t3a_tsdt_n_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.amls_t3a_tsdt_n_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'amls_t3a_tsdt_n',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
