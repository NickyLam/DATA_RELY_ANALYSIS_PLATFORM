...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_rwas_rpt_ibout
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
drop table ${iol_schema}.rwas_rpt_ibout_ex purge;
alter table ${iol_schema}.rwas_rpt_ibout add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.2 truncate target table batch_date partition
whenever sqlerror exit sql.sqlcode;
alter table ${iol_schema}.rwas_rpt_ibout truncate partition p_${batch_date};

-- 2.3 insert data to ex table
create table ${iol_schema}.rwas_rpt_ibout_ex nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.rwas_rpt_ibout where 0=1;

insert /*+ append */ into ${iol_schema}.rwas_rpt_ibout_ex(
    data_date -- 数据日期
    ,loan_ref_no -- 债项编号
    ,loan_ref_desc -- 债项描述
    ,contract_no -- 合同编号
...
from ${itl_schema}.rwas_rpt_ibout
where etl_dt = to_date('${batch_date}', 'yyyymmdd')
;

-- 2.4 exchage ex table and target table
alter table ${iol_schema}.rwas_rpt_ibout exchange partition p_${batch_date} with table ${iol_schema}.rwas_rpt_ibout_ex;
...
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'rwas_rpt_ibout',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);