...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_rsts_cpr_rating_update
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.rsts_cpr_rating_update_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.rsts_cpr_rating_update
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.rsts_cpr_rating_update_op purge;
drop table ${iol_schema}.rsts_cpr_rating_update_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.rsts_cpr_rating_update_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.rsts_cpr_rating_update where 0=1;

create table ${iol_schema}.rsts_cpr_rating_update_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.rsts_cpr_rating_update where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.rsts_cpr_rating_update_cl(
            uuid -- 指标加工明细ID
            ,serial_no -- 评级流水号
            ,rating_type -- 评级类型
            ,cust_no -- 客户编号
        ...
        into ${iol_schema}.rsts_cpr_rating_update_op(
            uuid -- 指标加工明细ID
            ,serial_no -- 评级流水号
            ,rating_type -- 评级类型
            ,cust_no -- 客户编号
...
from (select * from ${iol_schema}.rsts_cpr_rating_update_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.rsts_cpr_rating_update where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.uuid = n.uuid
where (
        o.uuid is null
...
from ${iol_schema}.rsts_cpr_rating_update_bk o
    left join ${iol_schema}.rsts_cpr_rating_update_op n
        on
            o.uuid = n.uuid
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.rsts_cpr_rating_update_cl d
        on
            o.uuid = d.uuid
...
--truncate table ${iol_schema}.rsts_cpr_rating_update;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('rsts_cpr_rating_update') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.rsts_cpr_rating_update drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.rsts_cpr_rating_update add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.rsts_cpr_rating_update exchange partition p_${batch_date} with table ${iol_schema}.rsts_cpr_rating_update_cl;
alter table ${iol_schema}.rsts_cpr_rating_update exchange partition p_20991231 with table ${iol_schema}.rsts_cpr_rating_update_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.rsts_cpr_rating_update_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'rsts_cpr_rating_update',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
