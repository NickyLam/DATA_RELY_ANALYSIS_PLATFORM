...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_ppps_t_txn_debit
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
drop table ${iol_schema}.ppps_t_txn_debit_ex purge;
alter table ${iol_schema}.ppps_t_txn_debit add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.2 truncate target table batch_date partition
whenever sqlerror exit sql.sqlcode;
alter table ${iol_schema}.ppps_t_txn_debit truncate partition p_${batch_date};

-- 2.3 insert data to ex table
create table ${iol_schema}.ppps_t_txn_debit_ex nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ppps_t_txn_debit where 0=1;

insert /*+ append */ into ${iol_schema}.ppps_t_txn_debit_ex(
    id -- 自增主键
    ,global_no -- 全局流水号
    ,txn_no -- 平台流水号
    ...
    ,advance_flag -- 是否垫资00：不垫支 01：垫支
    ,init_mcht_no -- 源发起渠道编号
    ,sys_comm_no -- 业务流水号
    ,book_type -- 00：渠道账 01：客户账
    ,pmc_ret_no -- 通道响应流水号
    ,pmc_ret_date -- 通道响应日期
    ,pmc_ret_time -- 通道响应时间
    ,pmc_ret_status -- 
    ...
    ,real_payee_acct_no -- 实际收款人账号
    ,real_payee_acct_name -- 实际收款人名称
    ,real_payee_acct_type -- 实际收款人账户类型
    ,real_payee_host_type -- 实际收款人所属核心类型
    ,sch_id -- 学校编号
    ,stud_name -- 学生姓名
    ,one_card_solu_sch_num -- 校园卡卡号
    ,sch_card_typ -- 
    ,curr_pay_noti_sing_id -- 缴费通知单id
    ,transaction_id -- 订单号
...
from ${itl_schema}.ppps_t_txn_debit
where etl_dt = to_date('${batch_date}', 'yyyymmdd')
;

-- 2.4 exchage ex table and target table
alter table ${iol_schema}.ppps_t_txn_debit exchange partition p_${batch_date} with table ${iol_schema}.ppps_t_txn_debit_ex;
...
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ppps_t_txn_debit',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);