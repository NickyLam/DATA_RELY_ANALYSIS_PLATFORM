...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_icms_clr_asset_property_parking
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.icms_clr_asset_property_parking_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.icms_clr_asset_property_parking
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.icms_clr_asset_property_parking_op purge;
drop table ${iol_schema}.icms_clr_asset_property_parking_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.icms_clr_asset_property_parking_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.icms_clr_asset_property_parking where 0=1;

create table ${iol_schema}.icms_clr_asset_property_parking_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.icms_clr_asset_property_parking where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.icms_clr_asset_property_parking_cl(
            clrid -- 押品编号
            ,isindepcard -- 是否有独立产权证
            ,parktype -- 车库/车位类型
            ,contno -- 房地产买卖合同编号
            ,tradedate -- 购买日期
            ,tradeprice -- 购买总价
        ...
        into ${iol_schema}.icms_clr_asset_property_parking_op(
            clrid -- 押品编号
            ,isindepcard -- 是否有独立产权证
            ,parktype -- 车库/车位类型
            ,contno -- 房地产买卖合同编号
...
select
    ...
    ,nvl(n.isindepcard, o.isindepcard) as isindepcard -- 是否有独立产权证
    ,nvl(n.parktype, o.parktype) as parktype -- 车库/车位类型
    ,nvl(n.contno, o.contno) as contno -- 房地产买卖合同编号
    ,nvl(n.tradedate, o.tradedate) as tradedate -- 购买日期
    ,nvl(n.tradeprice, o.tradeprice) as tradeprice -- 购买总价
    ...
    ,nvl(n.address, o.address) as address -- 房产证或不动产证地址
    ...
    ,nvl(n.landno, o.landno) as landno -- 房产证号
    ...
    ,nvl(n.landnumber, o.landnumber) as landnumber -- 土地证书号
...
from (select * from ${iol_schema}.icms_clr_asset_property_parking_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.icms_clr_asset_property_parking where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.clrid = n.clrid
where (
        o.clrid is null
    ...
    or (
        o.isindepcard <> n.isindepcard
        or o.parktype <> n.parktype
...
from ${iol_schema}.icms_clr_asset_property_parking_bk o
    left join ${iol_schema}.icms_clr_asset_property_parking_op n
        on
            o.clrid = n.clrid
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.icms_clr_asset_property_parking_cl d
        on
            o.clrid = d.clrid
...
--truncate table ${iol_schema}.icms_clr_asset_property_parking;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('icms_clr_asset_property_parking') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.icms_clr_asset_property_parking drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.icms_clr_asset_property_parking add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.icms_clr_asset_property_parking exchange partition p_${batch_date} with table ${iol_schema}.icms_clr_asset_property_parking_cl;
alter table ${iol_schema}.icms_clr_asset_property_parking exchange partition p_20991231 with table ${iol_schema}.icms_clr_asset_property_parking_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.icms_clr_asset_property_parking_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'icms_clr_asset_property_parking',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
