...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_ibms_tbnd_tag
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.ibms_tbnd_tag_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.ibms_tbnd_tag;

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.ibms_tbnd_tag_op purge;
drop table ${iol_schema}.ibms_tbnd_tag_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.ibms_tbnd_tag_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ibms_tbnd_tag where 0=1;

create table ${iol_schema}.ibms_tbnd_tag_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ibms_tbnd_tag where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.ibms_tbnd_tag_cl(
            i_code -- 交易代码
            ,a_type -- 资产类型
            ,m_type -- 市场类型
            ,sh_code -- 上海交易所代码
            ...
            ,currency -- 币种
            ,country -- 国家
            ,q_type -- 报价方式，CP：净价方式；DP：全价方式；YD：到期收益率
            ,b_name -- 名称
            ,p_type -- 产品类型
            ,p_class -- 产品分类
            ,b_par_value -- 债券面值
            ...
            ,a_type_bench -- 标的资产类型
            ,m_type_bench -- 标的市场类型
            ,b_issue_mode -- 发行方式，1：面值发行；2：贴现发行
            ,b_coupon_type -- 息票类型，1：固定利率；2：浮动利率；3：零息票利率
            ,b_cash_times -- 付息次数，每年的付息次数，如：1（年）、2（半年）、4（3个月）等，如果一次还本付息则为0
            ,b_embopt_type -- 含权标识，4位含权标识（1、0）表示：第一位：是否可转股；第二位：是否可赎回；第三位：是否可回售；第四位：是否可转为相关债券（固息转浮息、浮息转固息）
            ,b_amortizing -- 本金偿还，0：本金不提前偿还；1：本金提前偿还
            ,b_as_type -- 资产证券化产品类型，ABS；MBS；(空)
            ,b_issuer -- 发行人
            ,b_warrantor -- 担保人
            ,b_seniority -- 清偿等级，表示对债权类金融工具进行清算时，偿还债务的优先次序，标准定义（由高到低）：Senior、SubTier1、SubUpperTier2、SubLowerTier2、SubTier3；简化定义：Senior（普通）、Subordinate（次级）。注：目前采用简化定义
            ,b_fpml -- FPML
            ,b_coupon -- 票面利率
            ,b_name_full -- 债券全称
            ,b_actual_mtr_date -- 用户指定到期日
            ,d_code -- 债券内部代码
            ,b_p_class -- 债券分类
        ...
        into ${iol_schema}.ibms_tbnd_tag_op(
            i_code -- 交易代码
            ,a_type -- 资产类型
            ,m_type -- 市场类型
            ,sh_code -- 上海交易所代码
...
select
    ...
    ,nvl(n.sh_code, o.sh_code) as sh_code -- 上海交易所代码
    ,nvl(n.sz_code, o.sz_code) as sz_code -- 深圳交易所代码
    ,nvl(n.yh_code, o.yh_code) as yh_code -- 银行间市场代码
    ,nvl(n.currency, o.currency) as currency -- 币种
    ,nvl(n.country, o.country) as country -- 国家
    ,nvl(n.q_type, o.q_type) as q_type -- 报价方式，CP：净价方式；DP：全价方式；YD：到期收益率
    ,nvl(n.b_name, o.b_name) as b_name -- 名称
    ,nvl(n.p_type, o.p_type) as p_type -- 产品类型
    ,nvl(n.p_class, o.p_class) as p_class -- 产品分类
    ,nvl(n.b_par_value, o.b_par_value) as b_par_value -- 债券面值
    ,nvl(n.b_issue_price, o.b_issue_price) as b_issue_price -- 发行价格
    ,nvl(n.b_issue_date, o.b_issue_date) as b_issue_date -- 发行日期
    ,nvl(n.b_list_date, o.b_list_date) as b_list_date -- 上市日期
    ,nvl(n.b_start_date, o.b_start_date) as b_start_date -- 起息日期
    ,nvl(n.b_mtr_date, o.b_mtr_date) as b_mtr_date -- 到期日期
    ,nvl(n.b_term, o.b_term) as b_term -- 期限
    ,nvl(n.b_daycount, o.b_daycount) as b_daycount -- 日期计算方式
    ,nvl(n.i_code_bench, o.i_code_bench) as i_code_bench -- 标的交易代码
    ,nvl(n.a_type_bench, o.a_type_bench) as a_type_bench -- 标的资产类型
    ,nvl(n.m_type_bench, o.m_type_bench) as m_type_bench -- 标的市场类型
    ,nvl(n.b_issue_mode, o.b_issue_mode) as b_issue_mode -- 发行方式，1：面值发行；2：贴现发行
    ,nvl(n.b_coupon_type, o.b_coupon_type) as b_coupon_type -- 息票类型，1：固定利率；2：浮动利率；3：零息票利率
    ,nvl(n.b_cash_times, o.b_cash_times) as b_cash_times -- 付息次数，每年的付息次数，如：1（年）、2（半年）、4（3个月）等，如果一次还本付息则为0
    ,nvl(n.b_embopt_type, o.b_embopt_type) as b_embopt_type -- 含权标识，4位含权标识（1、0）表示：第一位：是否可转股；第二位：是否可赎回；第三位：是否可回售；第四位：是否可转为相关债券（固息转浮息、浮息转固息）
    ,nvl(n.b_amortizing, o.b_amortizing) as b_amortizing -- 本金偿还，0：本金不提前偿还；1：本金提前偿还
    ,nvl(n.b_as_type, o.b_as_type) as b_as_type -- 资产证券化产品类型，ABS；MBS；(空)
    ,nvl(n.b_issuer, o.b_issuer) as b_issuer -- 发行人
    ,nvl(n.b_warrantor, o.b_warrantor) as b_warrantor -- 担保人
    ,nvl(n.b_seniority, o.b_seniority) as b_seniority -- 清偿等级，表示对债权类金融工具进行清算时，偿还债务的优先次序，标准定义（由高到低）：Senior、SubTier1、SubUpperTier2、SubLowerTier2、SubTier3；简化定义：Senior（普通）、Subordinate（次级）。注：目前采用简化定义
    ,nvl(n.b_fpml, o.b_fpml) as b_fpml -- FPML
    ,nvl(n.b_coupon, o.b_coupon) as b_coupon -- 票面利率
    ,nvl(n.b_name_full, o.b_name_full) as b_name_full -- 债券全称
    ,nvl(n.b_actual_mtr_date, o.b_actual_mtr_date) as b_actual_mtr_date -- 用户指定到期日
    ,nvl(n.d_code, o.d_code) as d_code -- 债券内部代码
    ,nvl(n.b_p_class, o.b_p_class) as b_p_class -- 债券分类
    ,nvl(n.b_actual_issue_amount, o.b_actual_issue_amount) as b_actual_issue_amount -- 实际发行量
    ,nvl(n.chinesespell, o.chinesespell) as chinesespell -- 中文拼音
    ,nvl(n.b_coupon_prec, o.b_coupon_prec) as b_coupon_prec -- 票面精度
    ,nvl(n.host_market, o.host_market) as host_market -- 托管场所
    ,nvl(n.bj_market, o.bj_market) as bj_market -- 一级市场簿记场所
    ,nvl(n.issuer_id, o.issuer_id) as issuer_id -- 发行机构ID
    ,nvl(n.warrantor_id, o.warrantor_id) as warrantor_id -- 担保机构ID
    ...
    ,nvl(n.update_user, o.update_user) as update_user -- 修改用户
    ,nvl(n.account_user, o.account_user) as account_user -- 复核用户
    ...
    ,nvl(n.pipe_id, o.pipe_id) as pipe_id -- 管道编号
    ...
    ,nvl(n.tag_version, o.tag_version) as tag_version -- Tag版本号
...
from (select * from ${iol_schema}.ibms_tbnd_tag_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.ibms_tbnd_tag where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.i_code = n.i_code
            and o.a_type = n.a_type
            and o.m_type = n.m_type
            and o.tag_version = n.tag_version
where (
        o.i_code is null
        and o.a_type is null
        and o.m_type is null
        and o.tag_version is null
    )
    or (
        n.i_code is null
        and n.a_type is null
...
select
    ...
    ,o.currency -- 币种
    ,o.country -- 国家
    ,o.q_type -- 报价方式，CP：净价方式；DP：全价方式；YD：到期收益率
    ,o.b_name -- 名称
    ,o.p_type -- 产品类型
    ,o.p_class -- 产品分类
    ,o.b_par_value -- 债券面值
    ...
    ,o.b_term -- 期限
    ,o.b_daycount -- 日期计算方式
    ,o.i_code_bench -- 标的交易代码
    ,o.a_type_bench -- 标的资产类型
    ,o.m_type_bench -- 标的市场类型
    ,o.b_issue_mode -- 发行方式，1：面值发行；2：贴现发行
    ,o.b_coupon_type -- 息票类型，1：固定利率；2：浮动利率；3：零息票利率
    ,o.b_cash_times -- 付息次数，每年的付息次数，如：1（年）、2（半年）、4（3个月）等，如果一次还本付息则为0
    ,o.b_embopt_type -- 含权标识，4位含权标识（1、0）表示：第一位：是否可转股；第二位：是否可赎回；第三位：是否可回售；第四位：是否可转为相关债券（固息转浮息、浮息转固息）
    ,o.b_amortizing -- 本金偿还，0：本金不提前偿还；1：本金提前偿还
    ,o.b_as_type -- 资产证券化产品类型，ABS；MBS；(空)
    ,o.b_issuer -- 发行人
    ,o.b_warrantor -- 担保人
    ,o.b_seniority -- 清偿等级，表示对债权类金融工具进行清算时，偿还债务的优先次序，标准定义（由高到低）：Senior、SubTier1、SubUpperTier2、SubLowerTier2、SubTier3；简化定义：Senior（普通）、Subordinate（次级）。注：目前采用简化定义
    ,o.b_fpml -- FPML
    ,o.b_coupon -- 票面利率
    ,o.b_name_full -- 债券全称
    ,o.b_actual_mtr_date -- 用户指定到期日
    ,o.d_code -- 债券内部代码
    ,o.b_p_class -- 债券分类
    ,o.b_actual_issue_amount -- 实际发行量
    ,o.chinesespell -- 中文拼音
    ,o.b_coupon_prec -- 票面精度
    ,o.host_market -- 托管场所
    ,o.bj_market -- 一级市场簿记场所
    ,o.issuer_id -- 发行机构ID
    ,o.warrantor_id -- 担保机构ID
    ,o.is_delete -- 是否删除
    ...
    ,o.id_mark -- 增删标志
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间
from ${iol_schema}.ibms_tbnd_tag_bk o
    left join ${iol_schema}.ibms_tbnd_tag_op n
        on
            o.i_code = n.i_code
            and o.a_type = n.a_type
            and o.m_type = n.m_type
    ...
    left join ${iol_schema}.ibms_tbnd_tag_cl d
        on
            o.i_code = d.i_code
            and o.a_type = d.a_type
            and o.m_type = d.m_type
            and o.tag_version = d.tag_version
...
-- truncate table ${iol_schema}.ibms_tbnd_tag;

-- 4.2 exchange partition
alter table ${iol_schema}.ibms_tbnd_tag exchange partition p_19000101 with table ${iol_schema}.ibms_tbnd_tag_cl;
alter table ${iol_schema}.ibms_tbnd_tag exchange partition p_20991231 with table ${iol_schema}.ibms_tbnd_tag_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.ibms_tbnd_tag_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ibms_tbnd_tag',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
