...
create table ${iol_schema}.ctms_tbs_v_bondsdeals_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.ctms_tbs_v_bondsdeals
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.ctms_tbs_v_bondsdeals_op purge;
drop table ${iol_schema}.ctms_tbs_v_bondsdeals_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.ctms_tbs_v_bondsdeals_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ctms_tbs_v_bondsdeals where 0=1;

create table ${iol_schema}.ctms_tbs_v_bondsdeals_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ctms_tbs_v_bondsdeals where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.ctms_tbs_v_bondsdeals_cl(
            deal_id -- 引用表id
            ,deal_tablename -- 引用表名
            ,aspclient_id -- 部门编号
            ,bondscode -- 债券代码
        ...
        into ${iol_schema}.ctms_tbs_v_bondsdeals_op(
            deal_id -- 引用表id
            ,deal_tablename -- 引用表名
            ,aspclient_id -- 部门编号
            ,bondscode -- 债券代码
...
select
    ...
    ,nvl(n.bondsdeals_id_grand, o.bondsdeals_id_grand) as bondsdeals_id_grand -- 原始交易id
    ,nvl(n.stock_id, o.stock_id) as stock_id -- 股票代码
    ,nvl(n.convert_price, o.convert_price) as convert_price -- 转股价格
    ,nvl(n.stock_price, o.stock_price) as stock_price -- 正股价格
    ,nvl(n.convert_quantity, o.convert_quantity) as convert_quantity -- 转股数量
    ,nvl(n.dn_dealer, o.dn_dealer) as dn_dealer -- 本币交易员
    ,nvl(n.currency, o.currency) as currency -- 币种
    ,nvl(n.tradetime, o.tradetime) as tradetime -- 交易时间
    ,case when
            n.deal_id is null
...
from (select * from ${iol_schema}.ctms_tbs_v_bondsdeals_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.ctms_tbs_v_bondsdeals where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.deal_id = n.deal_id
...
where (
    ...
    or (
        ...
        or o.bondsdeals_id_grand <> n.bondsdeals_id_grand
        or o.stock_id <> n.stock_id
        or o.convert_price <> n.convert_price
        or o.stock_price <> n.stock_price
        or o.convert_quantity <> n.convert_quantity
...
select
    ...
    ,o.bondsdeals_id_grand -- 原始交易id
    ,o.stock_id -- 股票代码
    ,o.convert_price -- 转股价格
    ,o.stock_price -- 正股价格
    ,o.convert_quantity -- 转股数量
    ,o.dn_dealer -- 本币交易员
    ,o.currency -- 币种
    ,o.tradetime -- 交易时间
    ,o.start_dt -- 开始时间
...
from ${iol_schema}.ctms_tbs_v_bondsdeals_bk o
    left join ${iol_schema}.ctms_tbs_v_bondsdeals_op n
        on
            o.deal_id = n.deal_id
            and o.deal_tablename = n.deal_tablename
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.ctms_tbs_v_bondsdeals_cl d
        on
            o.deal_id = d.deal_id
...
--truncate table ${iol_schema}.ctms_tbs_v_bondsdeals;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('ctms_tbs_v_bondsdeals') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.ctms_tbs_v_bondsdeals drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.ctms_tbs_v_bondsdeals add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.ctms_tbs_v_bondsdeals exchange partition p_${batch_date} with table ${iol_schema}.ctms_tbs_v_bondsdeals_cl;
alter table ${iol_schema}.ctms_tbs_v_bondsdeals exchange partition p_20991231 with table ${iol_schema}.ctms_tbs_v_bondsdeals_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.ctms_tbs_v_bondsdeals_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ctms_tbs_v_bondsdeals',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
