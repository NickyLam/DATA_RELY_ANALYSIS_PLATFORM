...
begin
	for tb in (select partition_name,table_name,substr(partition_name,3) as etl_dt 
	            ...
	            where table_name = upper('rptm_rtm_rp_his_legal_bak${batch_date}')
	              and partition_name <> 'P_19000101') loop
	             
  select count(*) into v_flag
    from all_tab_partitions 
   where table_owner = upper('IOL')
     and table_name = upper('rptm_rtm_rp_his_legal')
     and partition_name = tb.partition_name;
     
  if v_flag <> 0 then
     execute immediate 'alter table rptm_rtm_rp_his_legal drop partition '|| tb.partition_name ;
     
  end if;
  
     execute immediate 'alter table rptm_rtm_rp_his_legal add partition ' || tb.partition_name || ' values (to_date(' || tb.etl_dt || ',''yyyymmdd''))';
end loop;
end;
/
insert /*+ append */ into ${iol_schema}.rptm_rtm_rp_his_legal(
    id -- ID
    ,bus_id -- 业务主键
...
from rptm_rtm_rp_his_legal_bak${batch_date}
