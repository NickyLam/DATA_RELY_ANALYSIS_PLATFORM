...
begin
	for tb in (select partition_name,table_name,substr(partition_name,3) as etl_dt 
	            ...
	            where table_name = upper('ncbs_rb_exchange_tran_hist_bak${batch_date}')
	              and partition_name <> 'P_19000101') loop
	             
  select count(*) into v_flag
    from all_tab_partitions 
   where table_owner = upper('IOL')
     and table_name = upper('ncbs_rb_exchange_tran_hist')
     and partition_name = tb.partition_name;
     
  if v_flag <> 0 then
     execute immediate 'alter table ncbs_rb_exchange_tran_hist drop partition '|| tb.partition_name ;
     
  end if;
  
     execute immediate 'alter table ncbs_rb_exchange_tran_hist add partition ' || tb.partition_name || ' values (to_date(' || tb.etl_dt || ',''yyyymmdd''))';

insert /*+ append */ into ${iol_schema}.ncbs_rb_exchange_tran_hist(
...
select
    ...
    ,sell_unc_rate -- 卖出平盘汇率
    ,buy_unc_rate -- 买入平盘汇率
    ,' ' as oversea_talent_proof -- 海外人才证明
    ,' ' as oversea_talent_proof_type -- 人才证明类型
    ,' ' as oversea_talent_remark -- 海外人才备注
    ,etl_dt as etl_dt -- ETL处理日期
    ,etl_timestamp as etl_timestamp -- ETL处理时间
from ncbs_rb_exchange_tran_hist_bak${batch_date}
