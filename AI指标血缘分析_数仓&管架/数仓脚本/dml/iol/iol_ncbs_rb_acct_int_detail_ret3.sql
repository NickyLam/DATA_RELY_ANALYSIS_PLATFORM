...
begin
  ...
  for tb in (select table_name, partition_name, substr(partition_name, 3) as etl_dt
              ...
              where table_name = upper('ncbs_rb_acct_int_detail_bak_20240815')
                and substr(partition_name, 3) <> '19000101'
                and substr(partition_name, 3) >= '20230821'
				and substr(partition_name, 3) < '20231016'
            ) loop
    ...
    -- 判断分区是否存在
	  select count(1)
	   ...
	   where table_name = upper('ncbs_rb_acct_int_detail')
	     and table_owner = 'IOL'
	  ...
	  if v_flag <> 0 then
	    ...
	    execute immediate 'alter table iol.ncbs_rb_acct_int_detail drop partition ' || tb.partition_name;

    end if;

      execute immediate 'alter table iol.ncbs_rb_acct_int_detail add partition ' || tb.partition_name || ' values (to_date(' || tb.etl_dt ||',''yyyymmdd''))';
	 
  end loop;
  
   dbms_output.put_line('ncbs_rb_acct_int_detail');
end;

/

insert /*+ append */ into ${iol_schema}.ncbs_rb_acct_int_detail(
    agg -- 积数
    ,client_no -- 客户编号
    ,int_type -- 利率类型
    ,internal_key -- 账户内部键值
...
select
    ...
    ,delay_int_amount -- 延迟付息累计供核算金额
    ,delay_int_amount_prev -- 延迟付息累计供核算金额-上日
    ,0 as delay_int_amount_last_prev -- 
    ,0 as month_total_amount -- 
    ,0 as last_month_total_amount -- 
    ,etl_dt as etl_dt -- ETL处理日期
    ,etl_timestamp as etl_timestamp -- ETL处理时间
from ncbs_rb_acct_int_detail_bak_20240815 t1
 where t1.etl_dt >= to_date('20230821', 'yyyymmdd')
and t1.etl_dt < to_date('20231016', 'yyyymmdd');
 commit;
