...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_pams_jxbb_xwqydkmxb_recal_ret1
CreateDate: 20250801
*/
...
begin
  for tb in (select partition_name,table_name,substr(partition_name,3) as etl_dt
               from user_tab_partitions
                   where table_name = upper('pams_jxbb_xwqydkmxb_recal_bak${batch_date}')
  ...
  select count(*) into v_flag
   ...
   where table_owner = upper('iol')
     and table_name = upper('pams_jxbb_xwqydkmxb_recal')
     and partition_name = tb.partition_name;

  if v_flag <> 0 then
    execute immediate 'alter table pams_jxbb_xwqydkmxb_recal drop partition '|| tb.partition_name ;
  end if;

    execute immediate 'alter table pams_jxbb_xwqydkmxb_recal add partition ' || tb.partition_name || ' values (to_date(' || tb.etl_dt || ',''yyyymmdd''))';


-- 回插所有数据

insert /*+ append */ into ${iol_schema}.pams_jxbb_xwqydkmxb_recal (
	tjrq -- 统计日期
	,jxdxdh -- 绩效对象代号
	,jgkhdxdh -- 机构考核对象代号
	,jgdh -- 机构代号
...
select
	...
	,ssjgdh as ssjgdh -- 所属机构代号
	,ssjgmc as ssjgmc -- 所属机构名称
	,fpje as fpje -- 分配金额
	,zlbl as zlbl -- 认领比例
	,fphzcye as fphzcye -- 分配后支出余额
	,fphzcyrj as fphzcyrj -- 分配后支出月日均
	,fphzcjrj as fphzcjrj -- 分配后支出季日均
	,fphzcnrj as fphzcnrj -- 分配后支出年日均
	,fphyqye as fphyqye -- 分配后预期余额
...
from ${iol_schema}.pams_jxbb_xwqydkmxb_recal_bak${batch_date}
