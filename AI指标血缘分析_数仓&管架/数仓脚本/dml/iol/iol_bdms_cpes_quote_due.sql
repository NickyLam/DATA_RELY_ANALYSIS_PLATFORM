...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_bdms_cpes_quote_due
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.bdms_cpes_quote_due_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.bdms_cpes_quote_due;

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.bdms_cpes_quote_due_op purge;
drop table ${iol_schema}.bdms_cpes_quote_due_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.bdms_cpes_quote_due_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.bdms_cpes_quote_due where 0=1;

create table ${iol_schema}.bdms_cpes_quote_due_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.bdms_cpes_quote_due where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.bdms_cpes_quote_due_cl(
            id -- 
            ,contract_no -- 协议号
            ,busi_date -- 业务日期
            ,org_cpes_quote_contract_id -- 原报价协议ID
            ,product_no -- 产品号
        ...
        into ${iol_schema}.bdms_cpes_quote_due_op(
            id -- 
            ,contract_no -- 协议号
            ,busi_date -- 业务日期
            ,org_cpes_quote_contract_id -- 原报价协议ID
...
select
    ...
    ,nvl(n.org_cpes_quote_contract_id, o.org_cpes_quote_contract_id) as org_cpes_quote_contract_id -- 原报价协议ID
...
from (select * from ${iol_schema}.bdms_cpes_quote_due_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.bdms_cpes_quote_due where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.id = n.id
where (
        o.id is null
    ...
    or (
        ...
        or o.org_cpes_quote_contract_id <> n.org_cpes_quote_contract_id
...
select
    ...
    ,o.contract_no -- 协议号
    ,o.busi_date -- 业务日期
    ,o.org_cpes_quote_contract_id -- 原报价协议ID
    ,o.product_no -- 产品号
    ...
    ,o.id_mark -- 增删标志
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间
from ${iol_schema}.bdms_cpes_quote_due_bk o
    left join ${iol_schema}.bdms_cpes_quote_due_op n
        on
            o.id = n.id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.bdms_cpes_quote_due_cl d
        on
            o.id = d.id
...
-- truncate table ${iol_schema}.bdms_cpes_quote_due;

-- 4.2 exchange partition
alter table ${iol_schema}.bdms_cpes_quote_due exchange partition p_19000101 with table ${iol_schema}.bdms_cpes_quote_due_cl;
alter table ${iol_schema}.bdms_cpes_quote_due exchange partition p_20991231 with table ${iol_schema}.bdms_cpes_quote_due_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.bdms_cpes_quote_due_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'bdms_cpes_quote_due',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
