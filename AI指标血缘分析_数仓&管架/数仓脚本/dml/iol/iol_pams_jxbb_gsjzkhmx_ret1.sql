...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_pams_jxbb_gsjzkhmx_ret1
CreateDate: 20250812
*/
...
begin
  for tb in (select partition_name,table_name,substr(partition_name,3) as etl_dt
               from user_tab_partitions
                   where table_name = upper('pams_jxbb_gsjzkhmx_bak${batch_date}')
  ...
  select count(*) into v_flag
   ...
   where table_owner = upper('iol')
     and table_name = upper('pams_jxbb_gsjzkhmx')
     and partition_name = tb.partition_name;

  if v_flag <> 0 then
    execute immediate 'alter table pams_jxbb_gsjzkhmx drop partition '|| tb.partition_name ;
  end if;

    execute immediate 'alter table pams_jxbb_gsjzkhmx add partition ' || tb.partition_name || ' values (to_date(' || tb.etl_dt || ',''yyyymmdd''))';


-- 回插所有数据

insert /*+ append */ into ${iol_schema}.pams_jxbb_gsjzkhmx (
	tjrq -- 统计日期
	,hydh -- 行员代号
	,hymc -- 行员名称
	,jgdh -- 机构代号
...
from ${iol_schema}.pams_jxbb_gsjzkhmx_bak${batch_date}
