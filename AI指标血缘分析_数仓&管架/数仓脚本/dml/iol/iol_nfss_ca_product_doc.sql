...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_nfss_ca_product_doc
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.nfss_ca_product_doc_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.nfss_ca_product_doc;

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.nfss_ca_product_doc_op purge;
drop table ${iol_schema}.nfss_ca_product_doc_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.nfss_ca_product_doc_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.nfss_ca_product_doc where 0=1;

create table ${iol_schema}.nfss_ca_product_doc_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.nfss_ca_product_doc where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.nfss_ca_product_doc_cl(
            id -- 主键
        ...
        into ${iol_schema}.nfss_ca_product_doc_op(
            id -- 主键
            ,prod_id -- 关联表主键ID，例产品表ID
            ,prd_cd -- 产品编码
            ,doc_id -- 文件ID
...
from (select * from ${iol_schema}.nfss_ca_product_doc_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.nfss_ca_product_doc where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.id = n.id
where (
        o.id is null
...
select
    ...
    ,o.id_mark -- 增删标志
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间
from ${iol_schema}.nfss_ca_product_doc_bk o
    left join ${iol_schema}.nfss_ca_product_doc_op n
        on
            o.id = n.id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.nfss_ca_product_doc_cl d
        on
            o.id = d.id
...
-- truncate table ${iol_schema}.nfss_ca_product_doc;

-- 4.2 exchange partition
alter table ${iol_schema}.nfss_ca_product_doc exchange partition p_19000101 with table ${iol_schema}.nfss_ca_product_doc_cl;
alter table ${iol_schema}.nfss_ca_product_doc exchange partition p_20991231 with table ${iol_schema}.nfss_ca_product_doc_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.nfss_ca_product_doc_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'nfss_ca_product_doc',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
