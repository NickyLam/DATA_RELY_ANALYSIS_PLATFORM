...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_mpcs_a92sharedatahis
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.mpcs_a92sharedatahis_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.mpcs_a92sharedatahis;

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.mpcs_a92sharedatahis_op purge;
drop table ${iol_schema}.mpcs_a92sharedatahis_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.mpcs_a92sharedatahis_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.mpcs_a92sharedatahis where 0=1;

create table ${iol_schema}.mpcs_a92sharedatahis_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.mpcs_a92sharedatahis where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.mpcs_a92sharedatahis_cl(
            paysys -- 服务方简称
            ,instid -- 接入商户号
            ,shareid -- 份额ID
            ,brokeruserid -- 用户账户ID
        ...
        into ${iol_schema}.mpcs_a92sharedatahis_op(
            paysys -- 服务方简称
            ,instid -- 接入商户号
            ,shareid -- 份额ID
            ,brokeruserid -- 用户账户ID
...
select
    ...
    ,nvl(n.sharetypes, o.sharetypes) as sharetypes -- 收费类型
    ...
    ,nvl(n.dividendmethod, o.dividendmethod) as dividendmethod -- 分红方式
...
from (select * from ${iol_schema}.mpcs_a92sharedatahis_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.mpcs_a92sharedatahis where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.shareid = n.shareid
where (
        o.shareid is null
...
select
    ...
    ,o.id_mark -- 增删标志
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间
from ${iol_schema}.mpcs_a92sharedatahis_bk o
    left join ${iol_schema}.mpcs_a92sharedatahis_op n
        on
            o.shareid = n.shareid
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.mpcs_a92sharedatahis_cl d
        on
            o.shareid = d.shareid
...
-- truncate table ${iol_schema}.mpcs_a92sharedatahis;

-- 4.2 exchange partition
alter table ${iol_schema}.mpcs_a92sharedatahis exchange partition p_19000101 with table ${iol_schema}.mpcs_a92sharedatahis_cl;
alter table ${iol_schema}.mpcs_a92sharedatahis exchange partition p_20991231 with table ${iol_schema}.mpcs_a92sharedatahis_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.mpcs_a92sharedatahis_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'mpcs_a92sharedatahis',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
