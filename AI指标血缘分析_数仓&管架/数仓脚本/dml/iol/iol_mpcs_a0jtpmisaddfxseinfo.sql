...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_mpcs_a0jtpmisaddfxseinfo
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.mpcs_a0jtpmisaddfxseinfo_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.mpcs_a0jtpmisaddfxseinfo
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.mpcs_a0jtpmisaddfxseinfo_op purge;
drop table ${iol_schema}.mpcs_a0jtpmisaddfxseinfo_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.mpcs_a0jtpmisaddfxseinfo_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.mpcs_a0jtpmisaddfxseinfo where 0=1;

create table ${iol_schema}.mpcs_a0jtpmisaddfxseinfo_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.mpcs_a0jtpmisaddfxseinfo where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.mpcs_a0jtpmisaddfxseinfo_cl(
            mainseq -- 中台流水号
            ,transdt -- 交易日期
            ,businesstrace -- 行内业务序号
            ,status -- 交易状态 Z 初始状态 1 已应答
            ,trantype -- 交易类型 ZJL  占额度结汇录入 ZJB  占额度结汇补录 UJL  不占额度结汇录入 UJB  不占额度结汇补录 ZGL  占额度购汇录入 ZGB  占额度购汇补录 UGL  不占额度购汇录入 UGB  不占额度购汇补录
            ...
            ,add_idcode -- 补充证件号码
            ,person_name -- 姓名
            ,salefx_tx_code -- 结汇资金属性代码
            ,txccy -- 币种
            ,salefx_amt -- 结汇金额
            ,lcy_acctno_cny -- 结汇人民币账户
            ,salefx_settle_code -- 结汇资金形态代码
        ...
        into ${iol_schema}.mpcs_a0jtpmisaddfxseinfo_op(
            mainseq -- 中台流水号
            ,transdt -- 交易日期
            ,businesstrace -- 行内业务序号
            ,status -- 交易状态 Z 初始状态 1 已应答
...
select
    ...
    ,nvl(n.trantype, o.trantype) as trantype -- 交易类型 ZJL  占额度结汇录入 ZJB  占额度结汇补录 UJL  不占额度结汇录入 UJB  不占额度结汇补录 ZGL  占额度购汇录入 ZGB  占额度购汇补录 UGL  不占额度购汇录入 UGB  不占额度购汇补录
    ...
    ,nvl(n.salefx_tx_code, o.salefx_tx_code) as salefx_tx_code -- 结汇资金属性代码
    ,nvl(n.txccy, o.txccy) as txccy -- 币种
    ,nvl(n.salefx_amt, o.salefx_amt) as salefx_amt -- 结汇金额
    ,nvl(n.lcy_acctno_cny, o.lcy_acctno_cny) as lcy_acctno_cny -- 结汇人民币账户
    ,nvl(n.salefx_settle_code, o.salefx_settle_code) as salefx_settle_code -- 结汇资金形态代码
    ...
    ,nvl(n.salefx_amt_usd, o.salefx_amt_usd) as salefx_amt_usd -- 本次结汇金额折美元
...
from (select * from ${iol_schema}.mpcs_a0jtpmisaddfxseinfo_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.mpcs_a0jtpmisaddfxseinfo where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.mainseq = n.mainseq
...
where (
    ...
    or (
        ...
        or o.salefx_tx_code <> n.salefx_tx_code
        or o.txccy <> n.txccy
        or o.salefx_amt <> n.salefx_amt
        or o.lcy_acctno_cny <> n.lcy_acctno_cny
        or o.salefx_settle_code <> n.salefx_settle_code
        ...
        or o.salefx_amt_usd <> n.salefx_amt_usd
...
select
    ...
    ,o.trantype -- 交易类型 ZJL  占额度结汇录入 ZJB  占额度结汇补录 UJL  不占额度结汇录入 UJB  不占额度结汇补录 ZGL  占额度购汇录入 ZGB  占额度购汇补录 UGL  不占额度购汇录入 UGB  不占额度购汇补录
    ...
    ,o.add_idcode -- 补充证件号码
    ,o.person_name -- 姓名
    ,o.salefx_tx_code -- 结汇资金属性代码
    ,o.txccy -- 币种
    ,o.salefx_amt -- 结汇金额
    ,o.lcy_acctno_cny -- 结汇人民币账户
    ,o.salefx_settle_code -- 结汇资金形态代码
...
from ${iol_schema}.mpcs_a0jtpmisaddfxseinfo_bk o
    left join ${iol_schema}.mpcs_a0jtpmisaddfxseinfo_op n
        on
            o.mainseq = n.mainseq
            and o.transdt = n.transdt
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.mpcs_a0jtpmisaddfxseinfo_cl d
        on
            o.mainseq = d.mainseq
...
--truncate table ${iol_schema}.mpcs_a0jtpmisaddfxseinfo;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('mpcs_a0jtpmisaddfxseinfo') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.mpcs_a0jtpmisaddfxseinfo drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.mpcs_a0jtpmisaddfxseinfo add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.mpcs_a0jtpmisaddfxseinfo exchange partition p_${batch_date} with table ${iol_schema}.mpcs_a0jtpmisaddfxseinfo_cl;
alter table ${iol_schema}.mpcs_a0jtpmisaddfxseinfo exchange partition p_20991231 with table ${iol_schema}.mpcs_a0jtpmisaddfxseinfo_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.mpcs_a0jtpmisaddfxseinfo_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'mpcs_a0jtpmisaddfxseinfo',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
