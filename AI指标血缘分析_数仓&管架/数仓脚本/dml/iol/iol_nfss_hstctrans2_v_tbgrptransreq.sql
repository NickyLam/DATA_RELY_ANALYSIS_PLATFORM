...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_nfss_hstctrans2_v_tbgrptransreq
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
drop table ${iol_schema}.nfss_hstctrans2_v_tbgrptransreq_ex purge;
alter table ${iol_schema}.nfss_hstctrans2_v_tbgrptransreq add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.2 truncate target table
whenever sqlerror exit sql.sqlcode;
truncate table ${iol_schema}.nfss_hstctrans2_v_tbgrptransreq;

-- 2.3 insert data to ex table
create table ${iol_schema}.nfss_hstctrans2_v_tbgrptransreq_ex nologging
compress
as
select * from ${iol_schema}.nfss_hstctrans2_v_tbgrptransreq where 0=1;

insert /*+ append */ into ${iol_schema}.nfss_hstctrans2_v_tbgrptransreq_ex(
    serial_no -- 流水序号:流水号
    ,ex_serial -- 原始请求外部流水号
...
from ${itl_schema}.nfss_hstctrans2_v_tbgrptransreq
where etl_dt = to_date('${batch_date}', 'yyyymmdd')
;

-- 2.4 exchage ex table and target table
alter table ${iol_schema}.nfss_hstctrans2_v_tbgrptransreq exchange partition p_${batch_date} with table ${iol_schema}.nfss_hstctrans2_v_tbgrptransreq_ex;
...
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'nfss_hstctrans2_v_tbgrptransreq',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);