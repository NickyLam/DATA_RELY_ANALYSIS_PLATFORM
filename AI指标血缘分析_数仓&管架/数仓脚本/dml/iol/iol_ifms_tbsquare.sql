...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_ifms_tbsquare
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.ifms_tbsquare_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.ifms_tbsquare
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.ifms_tbsquare_op purge;
drop table ${iol_schema}.ifms_tbsquare_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.ifms_tbsquare_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ifms_tbsquare where 0=1;

create table ${iol_schema}.ifms_tbsquare_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ifms_tbsquare where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.ifms_tbsquare_cl(
            square_no -- 
            ,seq_no -- 
            ,trans_date -- 
            ,clear_date -- 
        ...
        into ${iol_schema}.ifms_tbsquare_op(
            square_no -- 
            ,seq_no -- 
            ,trans_date -- 
            ,clear_date -- 
...
from (select * from ${iol_schema}.ifms_tbsquare_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.ifms_tbsquare where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.square_no = n.square_no
            and o.in_client_no = n.in_client_no
            and o.bank_no = n.bank_no
            and o.bank_acc = n.bank_acc
            and o.prd_code = n.prd_code
where (
        o.square_no is null
        and o.in_client_no is null
...
from ${iol_schema}.ifms_tbsquare_bk o
    left join ${iol_schema}.ifms_tbsquare_op n
        on
            o.square_no = n.square_no
            and o.in_client_no = n.in_client_no
            and o.bank_no = n.bank_no
    ...
    left join ${iol_schema}.ifms_tbsquare_cl d
        on
            o.square_no = d.square_no
            and o.in_client_no = d.in_client_no
...
--truncate table ${iol_schema}.ifms_tbsquare;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('ifms_tbsquare') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.ifms_tbsquare drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.ifms_tbsquare add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.ifms_tbsquare exchange partition p_${batch_date} with table ${iol_schema}.ifms_tbsquare_cl;
alter table ${iol_schema}.ifms_tbsquare exchange partition p_20991231 with table ${iol_schema}.ifms_tbsquare_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.ifms_tbsquare_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ifms_tbsquare',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
