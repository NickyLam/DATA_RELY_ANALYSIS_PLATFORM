...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_wind_assetmanagementcashflow
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
drop table ${iol_schema}.wind_assetmanagementcashflow_ex purge;
alter table ${iol_schema}.wind_assetmanagementcashflow add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.2 truncate target table batch_date partition
whenever sqlerror exit sql.sqlcode;
alter table ${iol_schema}.wind_assetmanagementcashflow truncate partition p_${batch_date};

-- 2.3 insert data to ex table
create table ${iol_schema}.wind_assetmanagementcashflow_ex nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.wind_assetmanagementcashflow where 0=1;

insert /*+ append */ into ${iol_schema}.wind_assetmanagementcashflow_ex(
    object_id -- 对象ID
    ...
    ,agent_trading_sc_net_cash -- 代理买卖证券收到的现金净额
    ,melt_money_net_increase -- 融出资金净增加额
    ,etl_dt -- ETL处理日期
...
from ${itl_schema}.wind_assetmanagementcashflow
where etl_dt = to_date('${batch_date}', 'yyyymmdd')
;

-- 2.4 exchage ex table and target table
alter table ${iol_schema}.wind_assetmanagementcashflow exchange partition p_${batch_date} with table ${iol_schema}.wind_assetmanagementcashflow_ex;
...
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'wind_assetmanagementcashflow',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);