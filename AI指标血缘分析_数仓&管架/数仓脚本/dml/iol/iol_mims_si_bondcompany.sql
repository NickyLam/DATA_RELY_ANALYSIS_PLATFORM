...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_mims_si_bondcompany
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.mims_si_bondcompany_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.mims_si_bondcompany
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.mims_si_bondcompany_op purge;
drop table ${iol_schema}.mims_si_bondcompany_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.mims_si_bondcompany_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.mims_si_bondcompany where 0=1;

create table ${iol_schema}.mims_si_bondcompany_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.mims_si_bondcompany where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.mims_si_bondcompany_cl(
            sccode -- 押品编号
            ,vouchertype -- 保证人类型
            ,voucherno -- 保证人编号
            ,vouchername -- 保证人名称
        ...
        into ${iol_schema}.mims_si_bondcompany_op(
            sccode -- 押品编号
            ,vouchertype -- 保证人类型
            ,voucherno -- 保证人编号
            ,vouchername -- 保证人名称
...
select
    ...
    ,nvl(n.guarprice, o.guarprice) as guarprice -- 担保总额度
...
from (select * from ${iol_schema}.mims_si_bondcompany_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.mims_si_bondcompany where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.sccode = n.sccode
where (
        o.sccode is null
...
from ${iol_schema}.mims_si_bondcompany_bk o
    left join ${iol_schema}.mims_si_bondcompany_op n
        on
            o.sccode = n.sccode
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.mims_si_bondcompany_cl d
        on
            o.sccode = d.sccode
...
--truncate table ${iol_schema}.mims_si_bondcompany;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('mims_si_bondcompany') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.mims_si_bondcompany drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.mims_si_bondcompany add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.mims_si_bondcompany exchange partition p_${batch_date} with table ${iol_schema}.mims_si_bondcompany_cl;
alter table ${iol_schema}.mims_si_bondcompany exchange partition p_20991231 with table ${iol_schema}.mims_si_bondcompany_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.mims_si_bondcompany_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'mims_si_bondcompany',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
