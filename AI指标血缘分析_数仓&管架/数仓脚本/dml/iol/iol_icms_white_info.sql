...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_icms_white_info
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.icms_white_info_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.icms_white_info
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.icms_white_info_op purge;
drop table ${iol_schema}.icms_white_info_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.icms_white_info_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.icms_white_info where 0=1;

create table ${iol_schema}.icms_white_info_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.icms_white_info where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.icms_white_info_cl(
            serialno -- 白名单编号
            ,orgshort -- 机构中文简称
            ,status -- 状态
            ,isbelongterm -- 是否靠档计息
        ...
        into ${iol_schema}.icms_white_info_op(
            serialno -- 白名单编号
            ,orgshort -- 机构中文简称
            ,status -- 状态
            ,isbelongterm -- 是否靠档计息
...
select
    nvl(n.serialno, o.serialno) as serialno -- 白名单编号
    ...
    ,nvl(n.attribute4, o.attribute4) as attribute4 -- 备用信息4
    ...
    ,nvl(n.bypassaccount, o.bypassaccount) as bypassaccount -- 子户
    ...
    ,nvl(n.attribute3, o.attribute3) as attribute3 -- 备用信息3
    ...
    ,nvl(n.sinosurebillaccount, o.sinosurebillaccount) as sinosurebillaccount -- 保证金账户
...
from (select * from ${iol_schema}.icms_white_info_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.icms_white_info where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
from ${iol_schema}.icms_white_info_bk o
    left join ${iol_schema}.icms_white_info_op n
        on
            o.serialno = n.serialno
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.icms_white_info_cl d
        on
...
--truncate table ${iol_schema}.icms_white_info;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('icms_white_info') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.icms_white_info drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.icms_white_info add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.icms_white_info exchange partition p_${batch_date} with table ${iol_schema}.icms_white_info_cl;
alter table ${iol_schema}.icms_white_info exchange partition p_20991231 with table ${iol_schema}.icms_white_info_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.icms_white_info_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'icms_white_info',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
