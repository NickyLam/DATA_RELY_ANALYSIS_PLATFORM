...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_mrms_tbl_product_info_detail
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.mrms_tbl_product_info_detail_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.mrms_tbl_product_info_detail
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.mrms_tbl_product_info_detail_op purge;
drop table ${iol_schema}.mrms_tbl_product_info_detail_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.mrms_tbl_product_info_detail_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.mrms_tbl_product_info_detail where 0=1;

create table ${iol_schema}.mrms_tbl_product_info_detail_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.mrms_tbl_product_info_detail where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.mrms_tbl_product_info_detail_cl(
            id -- 主键ID
            ,product_no -- 商品编号
        ...
        into ${iol_schema}.mrms_tbl_product_info_detail_op(
            id -- 主键ID
            ,product_no -- 商品编号
            ,product_nm -- 商品名称(例如京东、苏宁易购)
            ,product_brand -- 商品品牌
...
from (select * from ${iol_schema}.mrms_tbl_product_info_detail_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.mrms_tbl_product_info_detail where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.id = n.id
where (
        o.id is null
...
from ${iol_schema}.mrms_tbl_product_info_detail_bk o
    left join ${iol_schema}.mrms_tbl_product_info_detail_op n
        on
            o.id = n.id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.mrms_tbl_product_info_detail_cl d
        on
            o.id = d.id
...
--truncate table ${iol_schema}.mrms_tbl_product_info_detail;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('mrms_tbl_product_info_detail') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.mrms_tbl_product_info_detail drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.mrms_tbl_product_info_detail add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.mrms_tbl_product_info_detail exchange partition p_${batch_date} with table ${iol_schema}.mrms_tbl_product_info_detail_cl;
alter table ${iol_schema}.mrms_tbl_product_info_detail exchange partition p_20991231 with table ${iol_schema}.mrms_tbl_product_info_detail_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.mrms_tbl_product_info_detail_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'mrms_tbl_product_info_detail',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
