...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_ctms_tbs_v_balance
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.ctms_tbs_v_balance_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.ctms_tbs_v_balance
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.ctms_tbs_v_balance_op purge;
drop table ${iol_schema}.ctms_tbs_v_balance_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.ctms_tbs_v_balance_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ctms_tbs_v_balance where 0=1;

create table ${iol_schema}.ctms_tbs_v_balance_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ctms_tbs_v_balance where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.ctms_tbs_v_balance_cl(
            baretrade_id -- 引用表ID
            ,baretradename -- 引用表名
            ,balance_id -- 引用表2ID
        ...
        into ${iol_schema}.ctms_tbs_v_balance_op(
            baretrade_id -- 引用表ID
            ,baretradename -- 引用表名
            ,balance_id -- 引用表2ID
            ,alterbalance_id -- 引用表3ID
...
select
    nvl(n.baretrade_id, o.baretrade_id) as baretrade_id -- 引用表ID
    ,nvl(n.baretradename, o.baretradename) as baretradename -- 引用表名
    ,nvl(n.balance_id, o.balance_id) as balance_id -- 引用表2ID
    ,nvl(n.alterbalance_id, o.alterbalance_id) as alterbalance_id -- 引用表3ID
    ...
    ,nvl(n.valuedate, o.valuedate) as valuedate -- 起息日期
    ,nvl(n.maturitydate, o.maturitydate) as maturitydate -- 到期日期
    ,nvl(n.lastmodified, o.lastmodified) as lastmodified -- 最后更新时间
    ,nvl(n.occuramount, o.occuramount) as occuramount -- 发生金额
    ,nvl(n.balance_id_prev, o.balance_id_prev) as balance_id_prev -- 上一笔的Balance_id
    ,nvl(n.rev_flag, o.rev_flag) as rev_flag -- 冲账标志
...
from (select * from ${iol_schema}.ctms_tbs_v_balance_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.ctms_tbs_v_balance where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
where (
    ...
    or (
        o.baretrade_id <> n.baretrade_id
        or o.baretradename <> n.baretradename
        or o.alterbalance_id <> n.alterbalance_id
...
select
    o.baretrade_id -- 引用表ID
    ,o.baretradename -- 引用表名
    ,o.balance_id -- 引用表2ID
    ,o.alterbalance_id -- 引用表3ID
    ,o.aspclient_id -- 部门ID
    ...
    ,o.occuramount -- 发生金额
    ,o.balance_id_prev -- 上一笔的Balance_id
    ,o.rev_flag -- 冲账标志
...
from ${iol_schema}.ctms_tbs_v_balance_bk o
    left join ${iol_schema}.ctms_tbs_v_balance_op n
        on
            o.balance_id = n.balance_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.ctms_tbs_v_balance_cl d
        on
...
--truncate table ${iol_schema}.ctms_tbs_v_balance;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('ctms_tbs_v_balance') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.ctms_tbs_v_balance drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.ctms_tbs_v_balance add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.ctms_tbs_v_balance exchange partition p_${batch_date} with table ${iol_schema}.ctms_tbs_v_balance_cl;
alter table ${iol_schema}.ctms_tbs_v_balance exchange partition p_20991231 with table ${iol_schema}.ctms_tbs_v_balance_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.ctms_tbs_v_balance_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ctms_tbs_v_balance',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
