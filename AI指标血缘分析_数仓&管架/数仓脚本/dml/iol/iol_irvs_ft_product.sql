...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_irvs_ft_product
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.irvs_ft_product_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.irvs_ft_product
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.irvs_ft_product_op purge;
drop table ${iol_schema}.irvs_ft_product_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.irvs_ft_product_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.irvs_ft_product where 0=1;

create table ${iol_schema}.irvs_ft_product_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.irvs_ft_product where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.irvs_ft_product_cl(
            product_id -- 主键序号
            ,product_name -- 产品名称
            ,product_code -- 产品代码
        ...
        into ${iol_schema}.irvs_ft_product_op(
            product_id -- 主键序号
            ,product_name -- 产品名称
            ,product_code -- 产品代码
            ,risk_grade -- 风险等级 R1:R1低风险  R2:R2中低风险 R3:R3中风险 R4:R4中高风险 R5:R5高风险
...
from (select * from ${iol_schema}.irvs_ft_product_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.irvs_ft_product where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
from ${iol_schema}.irvs_ft_product_bk o
    left join ${iol_schema}.irvs_ft_product_op n
        on
            o.product_id = n.product_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.irvs_ft_product_cl d
        on
...
--truncate table ${iol_schema}.irvs_ft_product;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('irvs_ft_product') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.irvs_ft_product drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.irvs_ft_product add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.irvs_ft_product exchange partition p_${batch_date} with table ${iol_schema}.irvs_ft_product_cl;
alter table ${iol_schema}.irvs_ft_product exchange partition p_20991231 with table ${iol_schema}.irvs_ft_product_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.irvs_ft_product_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'irvs_ft_product',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
