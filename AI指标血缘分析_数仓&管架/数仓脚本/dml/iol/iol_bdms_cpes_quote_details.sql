...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_bdms_cpes_quote_details
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.bdms_cpes_quote_details_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.bdms_cpes_quote_details
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.bdms_cpes_quote_details_op purge;
drop table ${iol_schema}.bdms_cpes_quote_details_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.bdms_cpes_quote_details_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.bdms_cpes_quote_details where 0=1;

create table ${iol_schema}.bdms_cpes_quote_details_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.bdms_cpes_quote_details where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.bdms_cpes_quote_details_cl(
            id -- ID
            ,contract_id -- 批次ID
            ,draft_id -- 票据表ID
            ,draft_amount -- 票面金额
            ...
            ,details_status -- 处理状态： 00 未处理 01 发送中 02 发送成功 03 返回成功 04 返回失败 05 通讯失败 06 撤回中 07 撤回成功 08 撤回失败
        ...
        into ${iol_schema}.bdms_cpes_quote_details_op(
            id -- ID
            ,contract_id -- 批次ID
            ,draft_id -- 票据表ID
            ,draft_amount -- 票面金额
...
select
    ...
    ,nvl(n.details_status, o.details_status) as details_status -- 处理状态： 00 未处理 01 发送中 02 发送成功 03 返回成功 04 返回失败 05 通讯失败 06 撤回中 07 撤回成功 08 撤回失败
...
from (select * from ${iol_schema}.bdms_cpes_quote_details_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.bdms_cpes_quote_details where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.id = n.id
where (
        o.id is null
...
select
    ...
    ,o.details_status -- 处理状态： 00 未处理 01 发送中 02 发送成功 03 返回成功 04 返回失败 05 通讯失败 06 撤回中 07 撤回成功 08 撤回失败
...
from ${iol_schema}.bdms_cpes_quote_details_bk o
    left join ${iol_schema}.bdms_cpes_quote_details_op n
        on
            o.id = n.id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.bdms_cpes_quote_details_cl d
        on
            o.id = d.id
...
--truncate table ${iol_schema}.bdms_cpes_quote_details;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('bdms_cpes_quote_details') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.bdms_cpes_quote_details drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.bdms_cpes_quote_details add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.bdms_cpes_quote_details exchange partition p_${batch_date} with table ${iol_schema}.bdms_cpes_quote_details_cl;
alter table ${iol_schema}.bdms_cpes_quote_details exchange partition p_20991231 with table ${iol_schema}.bdms_cpes_quote_details_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.bdms_cpes_quote_details_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'bdms_cpes_quote_details',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
