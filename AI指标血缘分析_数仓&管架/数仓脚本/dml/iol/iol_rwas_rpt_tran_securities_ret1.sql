...
begin
	for tb in (select partition_name,table_name,substr(partition_name,3) as etl_dt 
	            ...
	            where table_name = upper('rwas_rpt_tran_securities_BAK_${batch_date}')
	              and partition_name <> 'P_19000101') loop
	             
  select count(*) into v_flag
    from all_tab_partitions 
   where table_owner = upper('IOL')
     and table_name = upper('rwas_rpt_tran_securities')
     and partition_name = tb.partition_name;
     
  if v_flag <> 0 then
     execute immediate 'alter table rwas_rpt_tran_securities drop partition '|| tb.partition_name ;
     
  end if;
  
     execute immediate 'alter table rwas_rpt_tran_securities add partition ' || tb.partition_name || ' values (to_date(' || tb.etl_dt || ',''yyyymmdd''))';

insert /*+ append */ into ${iol_schema}.rwas_rpt_tran_securities(
    data_date -- 数据日期
    ,loan_ref_no -- 借据号
    ,sec_no -- 债券编号
    ,sec_name -- 债券名称
...
from rwas_rpt_tran_securities_BAK_${batch_date}
