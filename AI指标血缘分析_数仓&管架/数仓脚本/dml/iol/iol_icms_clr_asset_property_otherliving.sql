...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_icms_clr_asset_property_otherliving
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.icms_clr_asset_property_otherliving_bk nologging
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.icms_clr_asset_property_otherliving_op purge;
drop table ${iol_schema}.icms_clr_asset_property_otherliving_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.icms_clr_asset_property_otherliving_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.icms_clr_asset_property_otherliving where 0=1;

create table ${iol_schema}.icms_clr_asset_property_otherliving_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.icms_clr_asset_property_otherliving where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.icms_clr_asset_property_otherliving_cl(
            clrid -- 押品编号
            ,housesubtype -- 房产细类
            ,houseflag -- 现房/期房标识
            ,preregistnos -- 预购商品房预告登记证明号
            ,preroyno -- 预售许可证编号
        ...
        into ${iol_schema}.icms_clr_asset_property_otherliving_op(
            clrid -- 押品编号
            ,housesubtype -- 房产细类
            ,houseflag -- 现房/期房标识
            ,preregistnos -- 预购商品房预告登记证明号
...
select
    ...
    ,nvl(n.housesubtype, o.housesubtype) as housesubtype -- 房产细类
    ,nvl(n.houseflag, o.houseflag) as houseflag -- 现房/期房标识
    ,nvl(n.preregistnos, o.preregistnos) as preregistnos -- 预购商品房预告登记证明号
    ...
    ,nvl(n.warrantsno, o.warrantsno) as warrantsno -- 房产证号
...
from (select * from ${iol_schema}.icms_clr_asset_property_otherliving_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.icms_clr_asset_property_otherliving where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.clrid = n.clrid
where (
        o.clrid is null
...
from ${iol_schema}.icms_clr_asset_property_otherliving_bk o
    left join ${iol_schema}.icms_clr_asset_property_otherliving_op n
        on
            o.clrid = n.clrid
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.icms_clr_asset_property_otherliving_cl d
        on
            o.clrid = d.clrid
...
--truncate table ${iol_schema}.icms_clr_asset_property_otherliving;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('icms_clr_asset_property_otherliving') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.icms_clr_asset_property_otherliving drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.icms_clr_asset_property_otherliving add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.icms_clr_asset_property_otherliving exchange partition p_${batch_date} with table ${iol_schema}.icms_clr_asset_property_otherliving_cl;
alter table ${iol_schema}.icms_clr_asset_property_otherliving exchange partition p_20991231 with table ${iol_schema}.icms_clr_asset_property_otherliving_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.icms_clr_asset_property_otherliving_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'icms_clr_asset_property_otherliving',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
