...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_nrrs_gs_historyresult
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.nrrs_gs_historyresult_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.nrrs_gs_historyresult
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.nrrs_gs_historyresult_op purge;
drop table ${iol_schema}.nrrs_gs_historyresult_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.nrrs_gs_historyresult_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.nrrs_gs_historyresult where 0=1;

create table ${iol_schema}.nrrs_gs_historyresult_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.nrrs_gs_historyresult where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.nrrs_gs_historyresult_cl(
            ratdateend -- 评级到期时间
            ,pd -- 违约概率
            ,loansugg -- 授信限额
            ,operatorid -- 评级操作员
        ...
        into ${iol_schema}.nrrs_gs_historyresult_op(
            ratdateend -- 评级到期时间
            ,pd -- 违约概率
            ,loansugg -- 授信限额
            ,operatorid -- 评级操作员
...
select
    ...
    ,nvl(n.custtype, o.custtype) as custtype -- 客户分类
    ,nvl(n.overturn, o.overturn) as overturn -- 是否推翻
    ,nvl(n.modelevel, o.modelevel) as modelevel -- 模型等级
    ,nvl(n.modelscore, o.modelscore) as modelscore -- 模型得分
    ,nvl(n.audittype, o.audittype) as audittype -- 意见类型
    ,nvl(n.modellsh, o.modellsh) as modellsh -- 模型编号
    ,nvl(n.loanlevel, o.loanlevel) as loanlevel -- 债项等级
...
from (select * from ${iol_schema}.nrrs_gs_historyresult_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.nrrs_gs_historyresult where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.year = n.year
            and o.modelcode = n.modelcode
...
from ${iol_schema}.nrrs_gs_historyresult_bk o
    left join ${iol_schema}.nrrs_gs_historyresult_op n
        on
            o.year = n.year
            and o.modelcode = n.modelcode
            and o.index1 = n.index1
    ...
    left join ${iol_schema}.nrrs_gs_historyresult_cl d
        on
            o.year = d.year
            and o.modelcode = d.modelcode
...
--truncate table ${iol_schema}.nrrs_gs_historyresult;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('nrrs_gs_historyresult') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.nrrs_gs_historyresult drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.nrrs_gs_historyresult add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.nrrs_gs_historyresult exchange partition p_${batch_date} with table ${iol_schema}.nrrs_gs_historyresult_cl;
alter table ${iol_schema}.nrrs_gs_historyresult exchange partition p_20991231 with table ${iol_schema}.nrrs_gs_historyresult_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.nrrs_gs_historyresult_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'nrrs_gs_historyresult',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
