...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_wind_cbondincome
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
drop table ${iol_schema}.wind_cbondincome_ex purge;
alter table ${iol_schema}.wind_cbondincome add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.2 truncate target table batch_date partition
whenever sqlerror exit sql.sqlcode;
alter table ${iol_schema}.wind_cbondincome truncate partition p_${batch_date};

-- 2.3 insert data to ex table
create table ${iol_schema}.wind_cbondincome_ex nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.wind_cbondincome where 0=1;

insert /*+ append */ into ${iol_schema}.wind_cbondincome_ex(
    object_id -- 对象ID
...
from ${itl_schema}.wind_cbondincome
where etl_dt = to_date('${batch_date}', 'yyyymmdd')
;

-- 2.4 exchage ex table and target table
alter table ${iol_schema}.wind_cbondincome exchange partition p_${batch_date} with table ${iol_schema}.wind_cbondincome_ex;
...
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'wind_cbondincome',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);