...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_mims_si_businessbill
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.mims_si_businessbill_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.mims_si_businessbill;

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.mims_si_businessbill_op purge;
drop table ${iol_schema}.mims_si_businessbill_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.mims_si_businessbill_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.mims_si_businessbill where 0=1;

create table ${iol_schema}.mims_si_businessbill_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.mims_si_businessbill where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.mims_si_businessbill_cl(
            sccode -- 
            ,notecode -- 
            ,notetype -- 
            ,remitter -- 
            ...
            ,bankpastename -- 
            ,tdcurrency -- 
            ,acceptordepositno -- 承兑人开户行行号
            ,acceptordepositname -- 承兑人开户行行名
            ,sponsoraccount -- 出质人账号
            ,sponsordepositno -- 出质人开户行行号
            ,sponsordepositname -- 出质人开户行行名
            ,start_dt -- 开始时间
            ,end_dt -- 结束时间
            ,id_mark -- 增删标志
        ...
        into ${iol_schema}.mims_si_businessbill_op(
            sccode -- 
            ,notecode -- 
            ,notetype -- 
            ,remitter -- 
...
select
    ...
    ,nvl(n.acceptordepositno, o.acceptordepositno) as acceptordepositno -- 承兑人开户行行号
    ,nvl(n.acceptordepositname, o.acceptordepositname) as acceptordepositname -- 承兑人开户行行名
    ,nvl(n.sponsoraccount, o.sponsoraccount) as sponsoraccount -- 出质人账号
    ,nvl(n.sponsordepositno, o.sponsordepositno) as sponsordepositno -- 出质人开户行行号
    ,nvl(n.sponsordepositname, o.sponsordepositname) as sponsordepositname -- 出质人开户行行名
    ,case when
            n.sccode is null
        then o.start_dt
...
from (select * from ${iol_schema}.mims_si_businessbill_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.mims_si_businessbill where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.sccode = n.sccode
where (
        o.sccode is null
    ...
    or (
        ...
        or o.acceptordepositno <> n.acceptordepositno
        or o.acceptordepositname <> n.acceptordepositname
        or o.sponsoraccount <> n.sponsoraccount
        or o.sponsordepositno <> n.sponsordepositno
        or o.sponsordepositname <> n.sponsordepositname
    )
;
commit;
...
select
    ...
    ,o.bankpastename -- 
    ,o.tdcurrency -- 
    ,o.acceptordepositno -- 承兑人开户行行号
    ,o.acceptordepositname -- 承兑人开户行行名
    ,o.sponsoraccount -- 出质人账号
    ,o.sponsordepositno -- 出质人开户行行号
    ,o.sponsordepositname -- 出质人开户行行名
    ,o.start_dt -- 开始时间
    ...
    ,o.id_mark -- 增删标志
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间
from ${iol_schema}.mims_si_businessbill_bk o
    left join ${iol_schema}.mims_si_businessbill_op n
        on
            o.sccode = n.sccode
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.mims_si_businessbill_cl d
        on
            o.sccode = d.sccode
...
-- truncate table ${iol_schema}.mims_si_businessbill;

-- 4.2 exchange partition
alter table ${iol_schema}.mims_si_businessbill exchange partition p_19000101 with table ${iol_schema}.mims_si_businessbill_cl;
alter table ${iol_schema}.mims_si_businessbill exchange partition p_20991231 with table ${iol_schema}.mims_si_businessbill_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.mims_si_businessbill_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'mims_si_businessbill',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
