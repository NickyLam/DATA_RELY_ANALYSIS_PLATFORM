...
create table ${iol_schema}.wind_neeqsintroduction_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.wind_neeqsintroduction
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.wind_neeqsintroduction_op purge;
drop table ${iol_schema}.wind_neeqsintroduction_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.wind_neeqsintroduction_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.wind_neeqsintroduction where 0=1;

create table ${iol_schema}.wind_neeqsintroduction_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.wind_neeqsintroduction where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.wind_neeqsintroduction_cl(
            object_id -- 对象ID
        ...
        into ${iol_schema}.wind_neeqsintroduction_op(
            object_id -- 对象ID
            ,s_info_compcode -- 公司ID
            ,s_info_compname -- 公司名称
            ,s_info_compnameeng -- 英文名称
...
select
    ...
    ,nvl(n.s_info_compnameeng, o.s_info_compnameeng) as s_info_compnameeng -- 英文名称
    ,nvl(n.s_info_country, o.s_info_country) as s_info_country -- 国家及地区
    ...
    ,nvl(n.s_info_chineseintroduction, o.s_info_chineseintroduction) as s_info_chineseintroduction -- 公司简介
    ,case when
            n.s_info_compcode is null
        then o.start_dt
...
from (select * from ${iol_schema}.wind_neeqsintroduction_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.wind_neeqsintroduction where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
select
    ...
    ,o.s_info_compnameeng -- 英文名称
    ,o.s_info_country -- 国家及地区
    ...
    ,o.s_info_chineseintroduction -- 公司简介
    ,o.start_dt -- 开始时间
...
from ${iol_schema}.wind_neeqsintroduction_bk o
    left join ${iol_schema}.wind_neeqsintroduction_op n
        on
            o.s_info_compcode = n.s_info_compcode
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.wind_neeqsintroduction_cl d
        on
            o.s_info_compcode = d.s_info_compcode
...
--truncate table ${iol_schema}.wind_neeqsintroduction;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('wind_neeqsintroduction') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.wind_neeqsintroduction drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.wind_neeqsintroduction add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.wind_neeqsintroduction exchange partition p_${batch_date} with table ${iol_schema}.wind_neeqsintroduction_cl;
alter table ${iol_schema}.wind_neeqsintroduction exchange partition p_20991231 with table ${iol_schema}.wind_neeqsintroduction_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.wind_neeqsintroduction_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'wind_neeqsintroduction',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
