...
create table ${iol_schema}.ibms_vtrd_ncd_result_details_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.ibms_vtrd_ncd_result_details
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.ibms_vtrd_ncd_result_details_op purge;
drop table ${iol_schema}.ibms_vtrd_ncd_result_details_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.ibms_vtrd_ncd_result_details_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ibms_vtrd_ncd_result_details where 0=1;

create table ${iol_schema}.ibms_vtrd_ncd_result_details_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ibms_vtrd_ncd_result_details where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.ibms_vtrd_ncd_result_details_cl(
            seq_id -- 序列号
            ,sysordid -- 交易单号
            ,ref_sysordid -- 子交易单号
            ,i_code -- 存单代码
        ...
        into ${iol_schema}.ibms_vtrd_ncd_result_details_op(
            seq_id -- 序列号
            ,sysordid -- 交易单号
            ,ref_sysordid -- 子交易单号
            ,i_code -- 存单代码
...
select
    ...
    ,nvl(n.sales_org_name, o.sales_org_name) as sales_org_name -- 
    ,nvl(n.sales_ratio, o.sales_ratio) as sales_ratio -- 
    ,nvl(n.sales_org_ratio, o.sales_org_ratio) as sales_org_ratio -- 
    ,nvl(n.sales_name_rate, o.sales_name_rate) as sales_name_rate -- 
    ,nvl(n.sales_org_name_rate, o.sales_org_name_rate) as sales_org_name_rate -- 
...
from (select * from ${iol_schema}.ibms_vtrd_ncd_result_details_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.ibms_vtrd_ncd_result_details where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.seq_id = n.seq_id
where (
        o.seq_id is null
    ...
    or (
        ...
        or o.sales_ratio <> n.sales_ratio
        or o.sales_org_ratio <> n.sales_org_ratio
        or o.sales_name_rate <> n.sales_name_rate
        or o.sales_org_name_rate <> n.sales_org_name_rate
...
from ${iol_schema}.ibms_vtrd_ncd_result_details_bk o
    left join ${iol_schema}.ibms_vtrd_ncd_result_details_op n
        on
            o.seq_id = n.seq_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.ibms_vtrd_ncd_result_details_cl d
        on
            o.seq_id = d.seq_id
...
--truncate table ${iol_schema}.ibms_vtrd_ncd_result_details;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('ibms_vtrd_ncd_result_details') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.ibms_vtrd_ncd_result_details drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.ibms_vtrd_ncd_result_details add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.ibms_vtrd_ncd_result_details exchange partition p_${batch_date} with table ${iol_schema}.ibms_vtrd_ncd_result_details_cl;
alter table ${iol_schema}.ibms_vtrd_ncd_result_details exchange partition p_20991231 with table ${iol_schema}.ibms_vtrd_ncd_result_details_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.ibms_vtrd_ncd_result_details_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ibms_vtrd_ncd_result_details',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
