...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_rptm_rtm_trade_aft_ordinary_credit
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit_op purge;
drop table ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit where 0=1;

create table ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit_cl(
            id -- 
            ,bus_id -- 
            ,bus_dep -- 
            ,is_bank_business -- 
        ...
        into ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit_op(
            id -- 
            ,bus_id -- 
            ,bus_dep -- 
            ,is_bank_business -- 
...
select
    ...
    ,nvl(n.is_filings, o.is_filings) as is_filings -- 
    ,nvl(n.is_meeting, o.is_meeting) as is_meeting -- 
    ,nvl(n.major_status, o.major_status) as major_status -- 
    ,nvl(n.major_situation, o.major_situation) as major_situation -- 
    ,nvl(n.data_source, o.data_source) as data_source -- 
    ,nvl(n.this_credit_ye, o.this_credit_ye) as this_credit_ye -- 
    ,nvl(n.re_ca_type, o.re_ca_type) as re_ca_type -- 
    ,nvl(n.bda_amount, o.bda_amount) as bda_amount -- 
    ,nvl(n.meeting_time, o.meeting_time) as meeting_time -- 
    ,nvl(n.ccm_name, o.ccm_name) as ccm_name -- 
    ,nvl(n.dm_name, o.dm_name) as dm_name -- 
    ,case when
...
from (select * from ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.rptm_rtm_trade_aft_ordinary_credit where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
where (
    ...
    or (
        ...
        or o.is_filings <> n.is_filings
        or o.is_meeting <> n.is_meeting
        or o.major_status <> n.major_status
        or o.major_situation <> n.major_situation
        or o.data_source <> n.data_source
        or o.this_credit_ye <> n.this_credit_ye
        or o.re_ca_type <> n.re_ca_type
        or o.bda_amount <> n.bda_amount
        or o.meeting_time <> n.meeting_time
        or o.ccm_name <> n.ccm_name
        or o.dm_name <> n.dm_name
    )
;
commit;
...
from ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit_bk o
    left join ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit_op n
        on

            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit_cl d
        on
...
--truncate table ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('rptm_rtm_trade_aft_ordinary_credit') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit exchange partition p_${batch_date} with table ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit_cl;
alter table ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit exchange partition p_20991231 with table ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.rptm_rtm_trade_aft_ordinary_credit_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'rptm_rtm_trade_aft_ordinary_credit',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
