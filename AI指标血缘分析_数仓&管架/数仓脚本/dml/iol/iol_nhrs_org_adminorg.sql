...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_nhrs_org_adminorg
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.nhrs_org_adminorg_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.nhrs_org_adminorg
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.nhrs_org_adminorg_op purge;
drop table ${iol_schema}.nhrs_org_adminorg_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.nhrs_org_adminorg_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.nhrs_org_adminorg where 0=1;

create table ${iol_schema}.nhrs_org_adminorg_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.nhrs_org_adminorg where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.nhrs_org_adminorg_cl(
            code -- 
            ,creationtime -- 
            ,creator -- 
            ,dataoriginflag -- 
        ...
        into ${iol_schema}.nhrs_org_adminorg_op(
            code -- 
            ,creationtime -- 
            ,creator -- 
            ,dataoriginflag -- 
...
select
    ...
    ,nvl(n.devicemanage, o.devicemanage) as devicemanage -- 
    ...
    ,nvl(n.isreceivesenddoc, o.isreceivesenddoc) as isreceivesenddoc -- 
    ...
    ,nvl(n.pk_adminorg, o.pk_adminorg) as pk_adminorg -- 
    ...
    ,nvl(n.suppliesmanage, o.suppliesmanage) as suppliesmanage -- 
    ,nvl(n.ts, o.ts) as ts -- 
...
from (select * from ${iol_schema}.nhrs_org_adminorg_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.nhrs_org_adminorg where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.pk_adminorg = n.pk_adminorg
where (
        o.pk_adminorg is null
    )
    or (
        n.pk_adminorg is null
    )
    or (
        ...
        or o.def8 <> n.def8
        or o.def9 <> n.def9
        or o.devicemanage <> n.devicemanage
        ...
        or o.isreceivesenddoc <> n.isreceivesenddoc
        ...
        or o.suppliesmanage <> n.suppliesmanage
        or o.ts <> n.ts
...
from ${iol_schema}.nhrs_org_adminorg_bk o
    left join ${iol_schema}.nhrs_org_adminorg_op n
        on
            o.pk_adminorg = n.pk_adminorg
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.nhrs_org_adminorg_cl d
        on
            o.pk_adminorg = d.pk_adminorg
...
--truncate table ${iol_schema}.nhrs_org_adminorg;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('nhrs_org_adminorg') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.nhrs_org_adminorg drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.nhrs_org_adminorg add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.nhrs_org_adminorg exchange partition p_${batch_date} with table ${iol_schema}.nhrs_org_adminorg_cl;
alter table ${iol_schema}.nhrs_org_adminorg exchange partition p_20991231 with table ${iol_schema}.nhrs_org_adminorg_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.nhrs_org_adminorg_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'nhrs_org_adminorg',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
