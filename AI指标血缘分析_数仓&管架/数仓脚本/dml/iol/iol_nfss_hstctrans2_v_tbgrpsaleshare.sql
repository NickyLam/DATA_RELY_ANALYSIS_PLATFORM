...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_nfss_hstctrans2_v_tbgrpsaleshare
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare_op purge;
drop table ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare where 0=1;

create table ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare_cl(
            import_date -- 
            ,seller_code -- 
            ,bank_no -- 
            ,client_no -- 
        ...
        into ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare_op(
            import_date -- 
            ,seller_code -- 
            ,bank_no -- 
            ,client_no -- 
...
from (select * from ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.nfss_hstctrans2_v_tbgrpsaleshare where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
from ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare_bk o
    left join ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare_op n
        on
            o.import_date = n.import_date
            and o.seller_code = n.seller_code
            and o.bank_no = n.bank_no
    ...
    left join ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare_cl d
        on
...
--truncate table ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('nfss_hstctrans2_v_tbgrpsaleshare') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare exchange partition p_${batch_date} with table ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare_cl;
alter table ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare exchange partition p_20991231 with table ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.nfss_hstctrans2_v_tbgrpsaleshare_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'nfss_hstctrans2_v_tbgrpsaleshare',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
