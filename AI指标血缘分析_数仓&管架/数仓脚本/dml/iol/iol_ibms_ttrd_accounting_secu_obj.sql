...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_ibms_ttrd_accounting_secu_obj
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
drop table ${iol_schema}.ibms_ttrd_accounting_secu_obj_ex purge;
alter table ${iol_schema}.ibms_ttrd_accounting_secu_obj add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.2 truncate target table batch_date partition
whenever sqlerror exit sql.sqlcode;
alter table ${iol_schema}.ibms_ttrd_accounting_secu_obj truncate partition p_${batch_date};

-- 2.3 insert data to ex table
create table ${iol_schema}.ibms_ttrd_accounting_secu_obj_ex nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ibms_ttrd_accounting_secu_obj where 0=1;

insert /*+ append */ into ${iol_schema}.ibms_ttrd_accounting_secu_obj_ex(
    obj_id -- 对象id
    ,tsk_id -- 任务id
    ,beg_date -- 开始日期
    ,end_date -- 结束日期
...
from ${itl_schema}.ibms_ttrd_accounting_secu_obj
where etl_dt = to_date('${batch_date}', 'yyyymmdd')
;

-- 2.4 exchage ex table and target table
alter table ${iol_schema}.ibms_ttrd_accounting_secu_obj exchange partition p_${batch_date} with table ${iol_schema}.ibms_ttrd_accounting_secu_obj_ex;
...
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ibms_ttrd_accounting_secu_obj',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);