...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_isbs_dba
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.isbs_dba_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.isbs_dba;

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.isbs_dba_op purge;
drop table ${iol_schema}.isbs_dba_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.isbs_dba_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.isbs_dba where 0=1;

create table ${iol_schema}.isbs_dba_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.isbs_dba where 0=1;
...
from (select * from ${iol_schema}.isbs_dba_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.isbs_dba where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.inr = n.inr
where (
        o.inr is null
...
select
    ...
    ,o.id_mark -- 增删标志
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间
from ${iol_schema}.isbs_dba_bk o
    left join ${iol_schema}.isbs_dba_op n
        on
            o.inr = n.inr
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.isbs_dba_cl d
        on
            o.inr = d.inr
...
-- truncate table ${iol_schema}.isbs_dba;

-- 4.2 exchange partition
alter table ${iol_schema}.isbs_dba exchange partition p_19000101 with table ${iol_schema}.isbs_dba_cl;
alter table ${iol_schema}.isbs_dba exchange partition p_20991231 with table ${iol_schema}.isbs_dba_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.isbs_dba_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'isbs_dba',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
