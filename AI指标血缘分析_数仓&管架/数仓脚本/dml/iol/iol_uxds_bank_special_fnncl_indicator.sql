...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_uxds_bank_special_fnncl_indicator
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
drop table ${iol_schema}.uxds_bank_special_fnncl_indicator_ex purge;
alter table ${iol_schema}.uxds_bank_special_fnncl_indicator add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.2 truncate target table
whenever sqlerror exit sql.sqlcode;
truncate table ${iol_schema}.uxds_bank_special_fnncl_indicator;

-- 2.3 insert data to ex table
create table ${iol_schema}.uxds_bank_special_fnncl_indicator_ex nologging
compress
as
select * from ${iol_schema}.uxds_bank_special_fnncl_indicator where 0=1;

insert /*+ append */ into ${iol_schema}.uxds_bank_special_fnncl_indicator_ex(
    seq -- 记录唯一标识
    ,ctime -- 记录创建日期
    ,mtime -- 记录修改日期
    ,rtime -- 记录通讯到用户端日期
...
from ${itl_schema}.uxds_bank_special_fnncl_indicator
where etl_dt = to_date('${batch_date}', 'yyyymmdd')
;

-- 2.4 exchage ex table and target table
alter table ${iol_schema}.uxds_bank_special_fnncl_indicator exchange partition p_${batch_date} with table ${iol_schema}.uxds_bank_special_fnncl_indicator_ex;
...
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'uxds_bank_special_fnncl_indicator',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);