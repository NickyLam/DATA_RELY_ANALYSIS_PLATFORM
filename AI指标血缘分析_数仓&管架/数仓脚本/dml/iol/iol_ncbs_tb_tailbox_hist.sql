...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_ncbs_tb_tailbox_hist
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.ncbs_tb_tailbox_hist_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.ncbs_tb_tailbox_hist
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.ncbs_tb_tailbox_hist_op purge;
drop table ${iol_schema}.ncbs_tb_tailbox_hist_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.ncbs_tb_tailbox_hist_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ncbs_tb_tailbox_hist where 0=1;

create table ${iol_schema}.ncbs_tb_tailbox_hist_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ncbs_tb_tailbox_hist where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.ncbs_tb_tailbox_hist_cl(
        ...
        into ${iol_schema}.ncbs_tb_tailbox_hist_op(
            eod_voucher_equal -- 日终凭证碰库标志
            ,branch -- 机构编号
            ,user_id -- 交易柜员编号
            ,company -- 法人
...
from (select * from ${iol_schema}.ncbs_tb_tailbox_hist_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.ncbs_tb_tailbox_hist where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
from ${iol_schema}.ncbs_tb_tailbox_hist_bk o
    left join ${iol_schema}.ncbs_tb_tailbox_hist_op n
        on
            o.tailbox_id = n.tailbox_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.ncbs_tb_tailbox_hist_cl d
        on
...
--truncate table ${iol_schema}.ncbs_tb_tailbox_hist;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('ncbs_tb_tailbox_hist') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.ncbs_tb_tailbox_hist drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.ncbs_tb_tailbox_hist add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.ncbs_tb_tailbox_hist exchange partition p_${batch_date} with table ${iol_schema}.ncbs_tb_tailbox_hist_cl;
alter table ${iol_schema}.ncbs_tb_tailbox_hist exchange partition p_20991231 with table ${iol_schema}.ncbs_tb_tailbox_hist_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.ncbs_tb_tailbox_hist_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ncbs_tb_tailbox_hist',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
