...
begin
	for tb in (select partition_name,table_name,substr(partition_name,3) as etl_dt 
	            ...
	            where table_name = upper('pams_jxbb_ybdkftpmx_bak${batch_date}')
	              and partition_name <> 'P_19000101') loop
	             
  select count(*) into v_flag
    from all_tab_partitions 
   where table_owner = upper('IOL')
     and table_name = upper('pams_jxbb_ybdkftpmx')
     and partition_name = tb.partition_name;
     
  if v_flag <> 0 then
     execute immediate 'alter table pams_jxbb_ybdkftpmx drop partition '|| tb.partition_name ;
     
  end if;
  
     execute immediate 'alter table pams_jxbb_ybdkftpmx add partition ' || tb.partition_name || ' values (to_date(' || tb.etl_dt || ',''yyyymmdd''))';
end loop;
end;
/
insert /*+ append */ into ${iol_schema}.pams_jxbb_ybdkftpmx(
    tjrq -- 统计日期
    ,khm -- 客户名称
    ,khh -- 客户号
...
from pams_jxbb_ybdkftpmx_bak${batch_date};
