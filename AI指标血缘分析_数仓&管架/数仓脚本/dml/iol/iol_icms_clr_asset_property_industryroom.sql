...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_icms_clr_asset_property_industryroom
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.icms_clr_asset_property_industryroom_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.icms_clr_asset_property_industryroom
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.icms_clr_asset_property_industryroom_op purge;
drop table ${iol_schema}.icms_clr_asset_property_industryroom_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.icms_clr_asset_property_industryroom_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.icms_clr_asset_property_industryroom where 0=1;

create table ${iol_schema}.icms_clr_asset_property_industryroom_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.icms_clr_asset_property_industryroom where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.icms_clr_asset_property_industryroom_cl(
            clrid -- 押品编号
            ,isnewhouse -- 一手/二手标识
            ,istwotogether -- 是否两证合一
            ,warrantsno -- 房产证号
            ,developmode -- 工业房地产开发模式
        ...
        into ${iol_schema}.icms_clr_asset_property_industryroom_op(
            clrid -- 押品编号
            ,isnewhouse -- 一手/二手标识
            ,istwotogether -- 是否两证合一
            ,warrantsno -- 房产证号
...
select
    ...
    ,nvl(n.developmode, o.developmode) as developmode -- 工业房地产开发模式
...
from (select * from ${iol_schema}.icms_clr_asset_property_industryroom_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.icms_clr_asset_property_industryroom where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.clrid = n.clrid
where (
        o.clrid is null
...
from ${iol_schema}.icms_clr_asset_property_industryroom_bk o
    left join ${iol_schema}.icms_clr_asset_property_industryroom_op n
        on
            o.clrid = n.clrid
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.icms_clr_asset_property_industryroom_cl d
        on
            o.clrid = d.clrid
...
--truncate table ${iol_schema}.icms_clr_asset_property_industryroom;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('icms_clr_asset_property_industryroom') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.icms_clr_asset_property_industryroom drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.icms_clr_asset_property_industryroom add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.icms_clr_asset_property_industryroom exchange partition p_${batch_date} with table ${iol_schema}.icms_clr_asset_property_industryroom_cl;
alter table ${iol_schema}.icms_clr_asset_property_industryroom exchange partition p_20991231 with table ${iol_schema}.icms_clr_asset_property_industryroom_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.icms_clr_asset_property_industryroom_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'icms_clr_asset_property_industryroom',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
