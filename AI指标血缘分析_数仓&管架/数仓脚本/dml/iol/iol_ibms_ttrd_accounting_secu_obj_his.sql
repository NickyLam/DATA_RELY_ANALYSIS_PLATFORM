...
drop table ${iol_schema}.ibms_ttrd_accounting_secu_obj_his_ex purge;
alter table ${iol_schema}.ibms_ttrd_accounting_secu_obj_his add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.2 truncate target table
whenever sqlerror exit sql.sqlcode;
truncate table ${iol_schema}.ibms_ttrd_accounting_secu_obj_his;

-- 2.3 insert data to ex table
create table ${iol_schema}.ibms_ttrd_accounting_secu_obj_his_ex nologging
compress
as
select * from ${iol_schema}.ibms_ttrd_accounting_secu_obj_his where 0=1;

insert /*+ append */ into ${iol_schema}.ibms_ttrd_accounting_secu_obj_his_ex(
    obj_id -- 对象id
    ,tsk_id -- 任务id
    ,beg_date -- 开始日期
    ,end_date -- 结束日期
...
from ${itl_schema}.ibms_ttrd_accounting_secu_obj_his
where etl_dt = to_date('${batch_date}', 'yyyymmdd')
;

-- 2.4 exchage ex table and target table
alter table ${iol_schema}.ibms_ttrd_accounting_secu_obj_his exchange partition p_${batch_date} with table ${iol_schema}.ibms_ttrd_accounting_secu_obj_his_ex;
...
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ibms_ttrd_accounting_secu_obj_his',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);