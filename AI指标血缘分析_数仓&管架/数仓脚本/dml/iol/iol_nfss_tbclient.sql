...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_nfss_tbclient
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.nfss_tbclient_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.nfss_tbclient;

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.nfss_tbclient_op purge;
drop table ${iol_schema}.nfss_tbclient_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.nfss_tbclient_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.nfss_tbclient where 0=1;

create table ${iol_schema}.nfss_tbclient_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.nfss_tbclient where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.nfss_tbclient_cl(
            in_client_no -- 
            ,client_type -- 
            ,client_group -- 
            ,id_type -- 
        ...
        into ${iol_schema}.nfss_tbclient_op(
            in_client_no -- 
            ,client_type -- 
            ,client_group -- 
            ,id_type -- 
...
from (select * from ${iol_schema}.nfss_tbclient_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.nfss_tbclient where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
select
    ...
    ,o.id_mark -- 增删标志
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间
from ${iol_schema}.nfss_tbclient_bk o
    left join ${iol_schema}.nfss_tbclient_op n
        on
            o.in_client_no = n.in_client_no
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.nfss_tbclient_cl d
        on
...
-- truncate table ${iol_schema}.nfss_tbclient;

-- 4.2 exchange partition
alter table ${iol_schema}.nfss_tbclient exchange partition p_19000101 with table ${iol_schema}.nfss_tbclient_cl;
alter table ${iol_schema}.nfss_tbclient exchange partition p_20991231 with table ${iol_schema}.nfss_tbclient_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.nfss_tbclient_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'nfss_tbclient',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
