...
create table ${iol_schema}.nfss_tbhistransreq_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.nfss_tbhistransreq
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.nfss_tbhistransreq_op purge;
drop table ${iol_schema}.nfss_tbhistransreq_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.nfss_tbhistransreq_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.nfss_tbhistransreq where 0=1;

create table ${iol_schema}.nfss_tbhistransreq_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.nfss_tbhistransreq where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.nfss_tbhistransreq_cl(
            serial_no -- 流水号
            ,ex_serial -- 发起方流水号
            ,contract_no -- 合约编号
            ,trans_date -- 交易日期
        ...
        into ${iol_schema}.nfss_tbhistransreq_op(
            serial_no -- 流水号
            ,ex_serial -- 发起方流水号
            ,contract_no -- 合约编号
            ,trans_date -- 交易日期
...
select
    ...
    ,nvl(n.seller_code, o.seller_code) as seller_code -- 合作商代码
...
from (select * from ${iol_schema}.nfss_tbhistransreq_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.nfss_tbhistransreq where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
from ${iol_schema}.nfss_tbhistransreq_bk o
    left join ${iol_schema}.nfss_tbhistransreq_op n
        on
            o.serial_no = n.serial_no
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.nfss_tbhistransreq_cl d
        on
...
--truncate table ${iol_schema}.nfss_tbhistransreq;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('nfss_tbhistransreq') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.nfss_tbhistransreq drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.nfss_tbhistransreq add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.nfss_tbhistransreq exchange partition p_${batch_date} with table ${iol_schema}.nfss_tbhistransreq_cl;
alter table ${iol_schema}.nfss_tbhistransreq exchange partition p_20991231 with table ${iol_schema}.nfss_tbhistransreq_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.nfss_tbhistransreq_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'nfss_tbhistransreq',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
