...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_ppps_u_corp_info
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.ppps_u_corp_info_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.ppps_u_corp_info
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.ppps_u_corp_info_op purge;
drop table ${iol_schema}.ppps_u_corp_info_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.ppps_u_corp_info_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ppps_u_corp_info where 0=1;

create table ${iol_schema}.ppps_u_corp_info_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ppps_u_corp_info where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.ppps_u_corp_info_cl(
            issr_id -- 机构标识
            ,issr_name -- 机构名称
            ,permit_code -- 营业执照号码
            ...
            ,address -- 注册地址
            ,organ_code -- 组织机构代码证
            ,corp_id_type -- 法人证件类型 00-其他，01-身份证，02-军官士兵证，03-港澳台通行证，04-护照
            ,corp_id_no -- 法人证件号码
        ...
        into ${iol_schema}.ppps_u_corp_info_op(
            issr_id -- 机构标识
            ,issr_name -- 机构名称
            ,permit_code -- 营业执照号码
            ,permit_vaild_date -- 营业执照有效期 YYYYMMDD
...
select
    ...
    ,nvl(n.corp_id_type, o.corp_id_type) as corp_id_type -- 法人证件类型 00-其他，01-身份证，02-军官士兵证，03-港澳台通行证，04-护照
    ...
    ,nvl(n.delete_status, o.delete_status) as delete_status -- 0
...
from (select * from ${iol_schema}.ppps_u_corp_info_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.ppps_u_corp_info where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.issr_id = n.issr_id
where (
        o.issr_id is null
...
select
    ...
    ,o.corp_id_type -- 法人证件类型 00-其他，01-身份证，02-军官士兵证，03-港澳台通行证，04-护照
    ,o.corp_id_no -- 法人证件号码
...
from ${iol_schema}.ppps_u_corp_info_bk o
    left join ${iol_schema}.ppps_u_corp_info_op n
        on
            o.issr_id = n.issr_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.ppps_u_corp_info_cl d
        on
            o.issr_id = d.issr_id
...
--truncate table ${iol_schema}.ppps_u_corp_info;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('ppps_u_corp_info') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.ppps_u_corp_info drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.ppps_u_corp_info add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.ppps_u_corp_info exchange partition p_${batch_date} with table ${iol_schema}.ppps_u_corp_info_cl;
alter table ${iol_schema}.ppps_u_corp_info exchange partition p_20991231 with table ${iol_schema}.ppps_u_corp_info_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.ppps_u_corp_info_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ppps_u_corp_info',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
