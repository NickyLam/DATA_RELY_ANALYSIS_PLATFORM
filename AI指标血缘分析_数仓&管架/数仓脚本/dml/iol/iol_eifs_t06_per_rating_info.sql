...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_eifs_t06_per_rating_info
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.eifs_t06_per_rating_info_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.eifs_t06_per_rating_info
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.eifs_t06_per_rating_info_op purge;
drop table ${iol_schema}.eifs_t06_per_rating_info_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.eifs_t06_per_rating_info_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.eifs_t06_per_rating_info where 0=1;

create table ${iol_schema}.eifs_t06_per_rating_info_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.eifs_t06_per_rating_info where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.eifs_t06_per_rating_info_cl(
            grade_id -- 评级信息id
            ,party_id -- 参与人id
        ...
        into ${iol_schema}.eifs_t06_per_rating_info_op(
            grade_id -- 评级信息id
            ,party_id -- 参与人id
            ,ext_crdt_rating_cd -- 外部信用评级
            ,ext_crdt_rating_dt -- 外部信用评级日期
...
from (select * from ${iol_schema}.eifs_t06_per_rating_info_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.eifs_t06_per_rating_info where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
from ${iol_schema}.eifs_t06_per_rating_info_bk o
    left join ${iol_schema}.eifs_t06_per_rating_info_op n
        on
            o.grade_id = n.grade_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.eifs_t06_per_rating_info_cl d
        on
...
--truncate table ${iol_schema}.eifs_t06_per_rating_info;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('eifs_t06_per_rating_info') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.eifs_t06_per_rating_info drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.eifs_t06_per_rating_info add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.eifs_t06_per_rating_info exchange partition p_${batch_date} with table ${iol_schema}.eifs_t06_per_rating_info_cl;
alter table ${iol_schema}.eifs_t06_per_rating_info exchange partition p_20991231 with table ${iol_schema}.eifs_t06_per_rating_info_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.eifs_t06_per_rating_info_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'eifs_t06_per_rating_info',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
