...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_icms_xked_iqp_loan_prior
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.icms_xked_iqp_loan_prior_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.icms_xked_iqp_loan_prior
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.icms_xked_iqp_loan_prior_op purge;
drop table ${iol_schema}.icms_xked_iqp_loan_prior_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.icms_xked_iqp_loan_prior_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.icms_xked_iqp_loan_prior where 0=1;

create table ${iol_schema}.icms_xked_iqp_loan_prior_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.icms_xked_iqp_loan_prior where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.icms_xked_iqp_loan_prior_cl(
            serialno -- 业务流水号
            ,applyno -- 信贷申请流水号
            ,prdcode -- 产品编号
            ,prdname -- 产品名称
        ...
        into ${iol_schema}.icms_xked_iqp_loan_prior_op(
            serialno -- 业务流水号
            ,applyno -- 信贷申请流水号
            ,prdcode -- 产品编号
            ,prdname -- 产品名称
...
select
    ...
    ,nvl(n.applyterm, o.applyterm) as applyterm -- 申请期数
    ,nvl(n.applysum, o.applysum) as applysum -- 申请金额
    ,nvl(n.bano, o.bano) as bano -- 授信编号
...
from (select * from ${iol_schema}.icms_xked_iqp_loan_prior_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.icms_xked_iqp_loan_prior where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
from ${iol_schema}.icms_xked_iqp_loan_prior_bk o
    left join ${iol_schema}.icms_xked_iqp_loan_prior_op n
        on
            o.serialno = n.serialno
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.icms_xked_iqp_loan_prior_cl d
        on
...
--truncate table ${iol_schema}.icms_xked_iqp_loan_prior;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('icms_xked_iqp_loan_prior') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.icms_xked_iqp_loan_prior drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.icms_xked_iqp_loan_prior add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.icms_xked_iqp_loan_prior exchange partition p_${batch_date} with table ${iol_schema}.icms_xked_iqp_loan_prior_cl;
alter table ${iol_schema}.icms_xked_iqp_loan_prior exchange partition p_20991231 with table ${iol_schema}.icms_xked_iqp_loan_prior_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.icms_xked_iqp_loan_prior_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'icms_xked_iqp_loan_prior',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
