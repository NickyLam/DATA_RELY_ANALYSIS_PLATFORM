...
begin
	for tb in (select partition_name,table_name,substr(partition_name,3) as etl_dt 
	            ...
	            where table_name = upper('rwas_rwas_ws_exp_map_bak${batch_date}')
	              and partition_name <> 'P_19000101') loop
	             
  select count(*) into v_flag
    from all_tab_partitions 
   where table_owner = upper('IOL')
     and table_name = upper('rwas_rwas_ws_exp_map')
     and partition_name = tb.partition_name;
     
  if v_flag <> 0 then
     execute immediate 'alter table rwas_rwas_ws_exp_map drop partition '|| tb.partition_name ;
     
  end if;
  
     execute immediate 'alter table rwas_rwas_ws_exp_map add partition ' || tb.partition_name || ' values (to_date(' || tb.etl_dt || ',''yyyymmdd''))';

insert /*+ append */ into ${iol_schema}.rwas_rwas_ws_exp_map(
    scenarioid -- 场景标识
    ,timeid -- 时间标识
    ,reportid -- 报表编号标识
...
from rwas_rwas_ws_exp_map_bak${batch_date}
