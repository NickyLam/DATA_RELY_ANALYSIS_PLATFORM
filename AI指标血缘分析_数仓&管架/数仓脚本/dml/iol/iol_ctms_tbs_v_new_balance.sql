...
create table ${iol_schema}.ctms_tbs_v_new_balance_bk nologging
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.ctms_tbs_v_new_balance_op purge;
drop table ${iol_schema}.ctms_tbs_v_new_balance_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.ctms_tbs_v_new_balance_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ctms_tbs_v_new_balance where 0=1;

create table ${iol_schema}.ctms_tbs_v_new_balance_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ctms_tbs_v_new_balance where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.ctms_tbs_v_new_balance_cl(
            baretrade_id -- 
            ,baretradename -- 
            ,balance_id -- 
            ,alterbalance_id -- 
        ...
        into ${iol_schema}.ctms_tbs_v_new_balance_op(
            baretrade_id -- 
            ,baretradename -- 
            ,balance_id -- 
            ,alterbalance_id -- 
...
select
    nvl(n.baretrade_id, o.baretrade_id) as baretrade_id -- 
    ,nvl(n.baretradename, o.baretradename) as baretradename -- 
    ...
    ,nvl(n.arvinterestcost, o.arvinterestcost) as arvinterestcost -- 挂账金额
    ,nvl(n.arvinterestcost_subjectid, o.arvinterestcost_subjectid) as arvinterestcost_subjectid -- 挂账科目
    ,case when
...
from (select * from ${iol_schema}.ctms_tbs_v_new_balance_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.ctms_tbs_v_new_balance where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
where (
    ...
    or (
        ...
        or o.arvinterestcost <> n.arvinterestcost
        or o.arvinterestcost_subjectid <> n.arvinterestcost_subjectid
    )
;
commit;
...
select
    ...
    ,o.arvinterestcost -- 挂账金额
    ,o.arvinterestcost_subjectid -- 挂账科目
    ,o.start_dt -- 开始时间
...
from ${iol_schema}.ctms_tbs_v_new_balance_bk o
    left join ${iol_schema}.ctms_tbs_v_new_balance_op n
        on
            o.balance_id = n.balance_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.ctms_tbs_v_new_balance_cl d
        on
...
--truncate table ${iol_schema}.ctms_tbs_v_new_balance;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('ctms_tbs_v_new_balance') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.ctms_tbs_v_new_balance drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.ctms_tbs_v_new_balance add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.ctms_tbs_v_new_balance exchange partition p_${batch_date} with table ${iol_schema}.ctms_tbs_v_new_balance_cl;
alter table ${iol_schema}.ctms_tbs_v_new_balance exchange partition p_20991231 with table ${iol_schema}.ctms_tbs_v_new_balance_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.ctms_tbs_v_new_balance_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ctms_tbs_v_new_balance',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
