...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_ibms_ttrd_accounting_secu_chg
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.ibms_ttrd_accounting_secu_chg_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.ibms_ttrd_accounting_secu_chg;

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.ibms_ttrd_accounting_secu_chg_op purge;
drop table ${iol_schema}.ibms_ttrd_accounting_secu_chg_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.ibms_ttrd_accounting_secu_chg_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ibms_ttrd_accounting_secu_chg where 0=1;

create table ${iol_schema}.ibms_ttrd_accounting_secu_chg_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ibms_ttrd_accounting_secu_chg where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.ibms_ttrd_accounting_secu_chg_cl(
            chg_id -- 变动Id
        ...
        into ${iol_schema}.ibms_ttrd_accounting_secu_chg_op(
            chg_id -- 变动Id
            ,erase_ref_chg_id -- 撤销关联变动Id
            ,tsk_id -- 任务Id
            ,chg_date -- 变动日期
...
from (select * from ${iol_schema}.ibms_ttrd_accounting_secu_chg_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.ibms_ttrd_accounting_secu_chg where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.chg_id = n.chg_id
...
select
    ...
    ,o.id_mark -- 增删标志
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间
from ${iol_schema}.ibms_ttrd_accounting_secu_chg_bk o
    left join ${iol_schema}.ibms_ttrd_accounting_secu_chg_op n
        on
            o.chg_id = n.chg_id
            and o.tsk_id = n.tsk_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.ibms_ttrd_accounting_secu_chg_cl d
        on
            o.chg_id = d.chg_id
...
-- truncate table ${iol_schema}.ibms_ttrd_accounting_secu_chg;

-- 4.2 exchange partition
alter table ${iol_schema}.ibms_ttrd_accounting_secu_chg exchange partition p_19000101 with table ${iol_schema}.ibms_ttrd_accounting_secu_chg_cl;
alter table ${iol_schema}.ibms_ttrd_accounting_secu_chg exchange partition p_20991231 with table ${iol_schema}.ibms_ttrd_accounting_secu_chg_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.ibms_ttrd_accounting_secu_chg_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ibms_ttrd_accounting_secu_chg',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
