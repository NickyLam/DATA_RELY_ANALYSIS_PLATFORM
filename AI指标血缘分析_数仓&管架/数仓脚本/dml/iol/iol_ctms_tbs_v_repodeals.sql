...
create table ${iol_schema}.ctms_tbs_v_repodeals_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.ctms_tbs_v_repodeals
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.ctms_tbs_v_repodeals_op purge;
drop table ${iol_schema}.ctms_tbs_v_repodeals_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.ctms_tbs_v_repodeals_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ctms_tbs_v_repodeals where 0=1;

create table ${iol_schema}.ctms_tbs_v_repodeals_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ctms_tbs_v_repodeals where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.ctms_tbs_v_repodeals_cl(
            deal_id -- 引用表id
            ,deal_tablename -- 引用表名
            ,aspclient_id -- 部门编号
            ,bondscode -- 债券代码
        ...
        into ${iol_schema}.ctms_tbs_v_repodeals_op(
            deal_id -- 引用表id
            ,deal_tablename -- 引用表名
            ,aspclient_id -- 部门编号
            ,bondscode -- 债券代码
...
select
    ...
    ,nvl(n.first_price, o.first_price) as first_price -- 首次净价
    ,nvl(n.maturity_price, o.maturity_price) as maturity_price -- 到期净价
    ,nvl(n.repo_rate, o.repo_rate) as repo_rate -- 回购利率
    ...
    ,nvl(n.repodeals_id_grand, o.repodeals_id_grand) as repodeals_id_grand -- 原始交易id
...
from (select * from ${iol_schema}.ctms_tbs_v_repodeals_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.ctms_tbs_v_repodeals where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.deal_id = n.deal_id
...
where (
    ...
    or (
        ...
        or o.repodeals_id_grand <> n.repodeals_id_grand
...
from ${iol_schema}.ctms_tbs_v_repodeals_bk o
    left join ${iol_schema}.ctms_tbs_v_repodeals_op n
        on
            o.deal_id = n.deal_id
            and o.deal_tablename = n.deal_tablename
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.ctms_tbs_v_repodeals_cl d
        on
            o.deal_id = d.deal_id
...
--truncate table ${iol_schema}.ctms_tbs_v_repodeals;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('ctms_tbs_v_repodeals') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.ctms_tbs_v_repodeals drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.ctms_tbs_v_repodeals add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.ctms_tbs_v_repodeals exchange partition p_${batch_date} with table ${iol_schema}.ctms_tbs_v_repodeals_cl;
alter table ${iol_schema}.ctms_tbs_v_repodeals exchange partition p_20991231 with table ${iol_schema}.ctms_tbs_v_repodeals_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.ctms_tbs_v_repodeals_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ctms_tbs_v_repodeals',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
