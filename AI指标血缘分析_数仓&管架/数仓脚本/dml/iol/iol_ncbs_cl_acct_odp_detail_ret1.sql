...
begin
  for tb in (SELECT TO_CHAR(END_DT, 'yyyymmdd') as end_dt
               FROM (SELECT END_DT,
                       ...
                       FROM ncbs_cl_acct_odp_detail_bak${batch_date})
              WHERE RN = 1
 ) loop
  ...
  select count(partition_name) into v_var 
   ...
   where substr(partition_name,3) = tb.end_dt 
     and table_name = upper('ncbs_cl_acct_odp_detail');
  
  if v_var <> 0 then 
    execute immediate 'alter table ncbs_cl_acct_odp_detail drop partition p_' || tb.end_dt;
  end if;
  
    v_sql :='alter table ncbs_cl_acct_odp_detail add partition p_' || tb.end_dt || ' values(to_date(' || tb.end_dt || ',''yyyymmdd''))';
...
insert /*+ append */ into ${iol_schema}.ncbs_cl_acct_odp_detail(
            client_no -- 客户编号
            ,internal_key -- 账户内部键值
            ,user_id -- 交易柜员编号
            ,company -- 法人
...
select
            ...
            ,id_mark -- 增删标志
            ,etl_timestamp -- ETL处理时间戳
from ${iol_schema}.ncbs_cl_acct_odp_detail_bak${batch_date}
