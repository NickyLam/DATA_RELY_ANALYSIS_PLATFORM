...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_ibms_ttrd_instrument
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.ibms_ttrd_instrument_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.ibms_ttrd_instrument
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.ibms_ttrd_instrument_op purge;
drop table ${iol_schema}.ibms_ttrd_instrument_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.ibms_ttrd_instrument_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ibms_ttrd_instrument where 0=1;

create table ${iol_schema}.ibms_ttrd_instrument_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ibms_ttrd_instrument where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.ibms_ttrd_instrument_cl(
            i_code -- 金融工具代码
            ,a_type -- 资产类型
            ,m_type -- 市场类型
            ,currency -- 币种
        ...
        into ${iol_schema}.ibms_ttrd_instrument_op(
            i_code -- 金融工具代码
            ,a_type -- 资产类型
            ,m_type -- 市场类型
            ,currency -- 币种
...
select
    ...
    ,nvl(n.term_day, o.term_day) as term_day -- 期限天数
    ,nvl(n.remain_term_day, o.remain_term_day) as remain_term_day -- 剩余期限
    ,nvl(n.issue_volume, o.issue_volume) as issue_volume -- 发行数量
    ...
    ,nvl(n.acting_code, o.acting_code) as acting_code -- 记账科目号
    ,nvl(n.prod_code, o.prod_code) as prod_code -- 产品编码
    ,nvl(n.tax_code, o.tax_code) as tax_code -- 免税代码
    ,nvl(n.charge_item, o.charge_item) as charge_item -- 收费项
    ,nvl(n.fee_number, o.fee_number) as fee_number -- 费用编号
    ,nvl(n.update_time2, o.update_time2) as update_time2 -- 更新时间
    ,nvl(n.is_renewal, o.is_renewal) as is_renewal -- 续期标识
    ,case when
            n.i_code is null
...
from (select * from ${iol_schema}.ibms_ttrd_instrument_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.ibms_ttrd_instrument where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.i_code = n.i_code
...
from ${iol_schema}.ibms_ttrd_instrument_bk o
    left join ${iol_schema}.ibms_ttrd_instrument_op n
        on
            o.i_code = n.i_code
            and o.a_type = n.a_type
            and o.m_type = n.m_type
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.ibms_ttrd_instrument_cl d
        on
            o.i_code = d.i_code
...
--truncate table ${iol_schema}.ibms_ttrd_instrument;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('ibms_ttrd_instrument') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.ibms_ttrd_instrument drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.ibms_ttrd_instrument add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.ibms_ttrd_instrument exchange partition p_${batch_date} with table ${iol_schema}.ibms_ttrd_instrument_cl;
alter table ${iol_schema}.ibms_ttrd_instrument exchange partition p_20991231 with table ${iol_schema}.ibms_ttrd_instrument_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.ibms_ttrd_instrument_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ibms_ttrd_instrument',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
