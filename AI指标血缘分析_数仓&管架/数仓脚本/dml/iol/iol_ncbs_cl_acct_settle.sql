...
create table ${iol_schema}.ncbs_cl_acct_settle_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.ncbs_cl_acct_settle
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.ncbs_cl_acct_settle_op purge;
drop table ${iol_schema}.ncbs_cl_acct_settle_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.ncbs_cl_acct_settle_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ncbs_cl_acct_settle where 0=1;

create table ${iol_schema}.ncbs_cl_acct_settle_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.ncbs_cl_acct_settle where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.ncbs_cl_acct_settle_cl(
            amt_type -- 金额类型
            ,client_no -- 客户编号
            ,internal_key -- 账户内部键值
            ,tran_type -- 交易类型
        ...
        into ${iol_schema}.ncbs_cl_acct_settle_op(
            amt_type -- 金额类型
            ,client_no -- 客户编号
            ,internal_key -- 账户内部键值
            ,tran_type -- 交易类型
...
select
    ...
    ,nvl(n.receipt_no, o.receipt_no) as receipt_no -- 回收号
    ...
    ,nvl(n.hang_operate_type1, o.hang_operate_type1) as hang_operate_type1 -- 挂账操作类型
    ,nvl(n.acct_res_operate_type, o.acct_res_operate_type) as acct_res_operate_type -- 账户限制操作类型|账户限制操作类型|01-新增,02-修改,03-解限,04-追加,05-部分解限,06-续冻
    ,nvl(n.acct_nature_desc, o.acct_nature_desc) as acct_nature_desc -- 账户属性描述
...
from (select * from ${iol_schema}.ncbs_cl_acct_settle_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.ncbs_cl_acct_settle where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
...
where (
    ...
    or (
        ...
        or o.hang_operate_type1 <> n.hang_operate_type1
        or o.acct_res_operate_type <> n.acct_res_operate_type
        or o.acct_nature_desc <> n.acct_nature_desc
...
select
    ...
    ,o.settle_xrate -- 结算汇率
    ,o.hang_seq_no -- 挂账序列号
    ,o.hang_operate_type1 -- 挂账操作类型
    ,o.acct_res_operate_type -- 账户限制操作类型|账户限制操作类型|01-新增,02-修改,03-解限,04-追加,05-部分解限,06-续冻
    ,o.acct_nature_desc -- 账户属性描述
...
from ${iol_schema}.ncbs_cl_acct_settle_bk o
    left join ${iol_schema}.ncbs_cl_acct_settle_op n
        on
            o.settle_no = n.settle_no
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.ncbs_cl_acct_settle_cl d
        on
...
--truncate table ${iol_schema}.ncbs_cl_acct_settle;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('ncbs_cl_acct_settle') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.ncbs_cl_acct_settle drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.ncbs_cl_acct_settle add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.ncbs_cl_acct_settle exchange partition p_${batch_date} with table ${iol_schema}.ncbs_cl_acct_settle_cl;
alter table ${iol_schema}.ncbs_cl_acct_settle exchange partition p_20991231 with table ${iol_schema}.ncbs_cl_acct_settle_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.ncbs_cl_acct_settle_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'ncbs_cl_acct_settle',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
