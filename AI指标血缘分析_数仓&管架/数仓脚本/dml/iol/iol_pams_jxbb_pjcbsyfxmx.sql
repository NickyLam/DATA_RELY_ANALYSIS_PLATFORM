...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_pams_jxbb_pjcbsyfxmx
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.pams_jxbb_pjcbsyfxmx_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.pams_jxbb_pjcbsyfxmx
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.pams_jxbb_pjcbsyfxmx_op purge;
drop table ${iol_schema}.pams_jxbb_pjcbsyfxmx_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.pams_jxbb_pjcbsyfxmx_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.pams_jxbb_pjcbsyfxmx where 0=1;

create table ${iol_schema}.pams_jxbb_pjcbsyfxmx_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.pams_jxbb_pjcbsyfxmx where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.pams_jxbb_pjcbsyfxmx_cl(
            tjrq -- 统计日期
            ,jxdxdh -- 绩效对象代号
            ,khdxdh -- 考核对象代号
            ,jgkhdxdh -- 机构考核对象代号
        ...
        into ${iol_schema}.pams_jxbb_pjcbsyfxmx_op(
            tjrq -- 统计日期
            ,jxdxdh -- 绩效对象代号
            ,khdxdh -- 考核对象代号
            ,jgkhdxdh -- 机构考核对象代号
...
from (select * from ${iol_schema}.pams_jxbb_pjcbsyfxmx_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.pams_jxbb_pjcbsyfxmx where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.tjrq = n.tjrq
...
from ${iol_schema}.pams_jxbb_pjcbsyfxmx_bk o
    left join ${iol_schema}.pams_jxbb_pjcbsyfxmx_op n
        on
            o.tjrq = n.tjrq
            and o.jxdxdh = n.jxdxdh
            and o.khdxdh = n.khdxdh
    ...
    left join ${iol_schema}.pams_jxbb_pjcbsyfxmx_cl d
        on
            o.tjrq = d.tjrq
...
--truncate table ${iol_schema}.pams_jxbb_pjcbsyfxmx;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('pams_jxbb_pjcbsyfxmx') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.pams_jxbb_pjcbsyfxmx drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.pams_jxbb_pjcbsyfxmx add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.pams_jxbb_pjcbsyfxmx exchange partition p_${batch_date} with table ${iol_schema}.pams_jxbb_pjcbsyfxmx_cl;
alter table ${iol_schema}.pams_jxbb_pjcbsyfxmx exchange partition p_20991231 with table ${iol_schema}.pams_jxbb_pjcbsyfxmx_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.pams_jxbb_pjcbsyfxmx_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'pams_jxbb_pjcbsyfxmx',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
