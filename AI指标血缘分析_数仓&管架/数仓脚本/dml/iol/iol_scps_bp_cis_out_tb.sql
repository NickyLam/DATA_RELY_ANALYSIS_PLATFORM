...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_scps_bp_cis_out_tb
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.scps_bp_cis_out_tb_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.scps_bp_cis_out_tb
where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >=to_date('${batch_date}','yyyymmdd');

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.scps_bp_cis_out_tb_op purge;
drop table ${iol_schema}.scps_bp_cis_out_tb_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.scps_bp_cis_out_tb_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.scps_bp_cis_out_tb where 0=1;

create table ${iol_schema}.scps_bp_cis_out_tb_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.scps_bp_cis_out_tb where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.scps_bp_cis_out_tb_cl(
            task_id -- 任务号
            ,trans_date -- 交易日期（yyyyMMdd）
            ,trans_time -- 交易时间
            ,trans_state -- 交易最终状态 E 提出借记被人行拒绝,已删除 6已维护入客户账 V 已直接入客户账 G 已止付 H 已超期 R 被拒付 提入 E 借记回执被人行拒绝,已删除 R 已拒绝付款 V 已付款 P 已止付 H 已超期
            ,scene_code -- 业务场景码
            ,charge_id -- 授权柜员号
            ,br_code -- 记账机构号
        ...
        into ${iol_schema}.scps_bp_cis_out_tb_op(
            task_id -- 任务号
            ,trans_date -- 交易日期（yyyyMMdd）
            ,trans_time -- 交易时间
            ,trans_state -- 交易最终状态 E 提出借记被人行拒绝,已删除 6已维护入客户账 V 已直接入客户账 G 已止付 H 已超期 R 被拒付 提入 E 借记回执被人行拒绝,已删除 R 已拒绝付款 V 已付款 P 已止付 H 已超期
...
select
    ...
    ,nvl(n.trans_state, o.trans_state) as trans_state -- 交易最终状态 E 提出借记被人行拒绝,已删除 6已维护入客户账 V 已直接入客户账 G 已止付 H 已超期 R 被拒付 提入 E 借记回执被人行拒绝,已删除 R 已拒绝付款 V 已付款 P 已止付 H 已超期
    ...
    ,nvl(n.user_id, o.user_id) as user_id -- 柜员号
...
from (select * from ${iol_schema}.scps_bp_cis_out_tb_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.scps_bp_cis_out_tb where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.task_id = n.task_id
where (
        o.task_id is null
...
select
    ...
    ,o.trans_time -- 交易时间
    ,o.trans_state -- 交易最终状态 E 提出借记被人行拒绝,已删除 6已维护入客户账 V 已直接入客户账 G 已止付 H 已超期 R 被拒付 提入 E 借记回执被人行拒绝,已删除 R 已拒绝付款 V 已付款 P 已止付 H 已超期
    ,o.scene_code -- 业务场景码
    ,o.charge_id -- 授权柜员号
    ,o.br_code -- 记账机构号
...
from ${iol_schema}.scps_bp_cis_out_tb_bk o
    left join ${iol_schema}.scps_bp_cis_out_tb_op n
        on
            o.task_id = n.task_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.scps_bp_cis_out_tb_cl d
        on
            o.task_id = d.task_id
...
--truncate table ${iol_schema}.scps_bp_cis_out_tb;

-- 4.2 rebuild partition
declare
v_sql varchar2(100);
begin
for cur in (select partition_name from all_tab_partitions where table_owner=upper('${iol_schema}') and table_name=upper('scps_bp_cis_out_tb') and substr(partition_name,3) >= '${batch_date}' and substr(partition_name,3) < '20991231') loop
    v_sql := 'alter table ${iol_schema}.scps_bp_cis_out_tb drop partition ' || cur.partition_name;
    dbms_output.put_line(v_sql);
    execute immediate v_sql;
end loop;
end;
/
alter table ${iol_schema}.scps_bp_cis_out_tb add partition p_${batch_date} values(to_date('${batch_date}','YYYYMMDD'));

-- 4.3 exchange partition
alter table ${iol_schema}.scps_bp_cis_out_tb exchange partition p_${batch_date} with table ${iol_schema}.scps_bp_cis_out_tb_cl;
alter table ${iol_schema}.scps_bp_cis_out_tb exchange partition p_20991231 with table ${iol_schema}.scps_bp_cis_out_tb_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.scps_bp_cis_out_tb_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'scps_bp_cis_out_tb',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
