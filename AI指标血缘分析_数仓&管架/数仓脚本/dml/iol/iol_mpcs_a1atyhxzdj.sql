...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iol_mpcs_a1atyhxzdj
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建脚本
*/
...
create table ${iol_schema}.mpcs_a1atyhxzdj_bk nologging
compress ${option_switch} for query high
as
select *
from ${iol_schema}.mpcs_a1atyhxzdj;

-- 2.2 drop temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iol_schema}.mpcs_a1atyhxzdj_op purge;
drop table ${iol_schema}.mpcs_a1atyhxzdj_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iol_schema}.mpcs_a1atyhxzdj_op nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.mpcs_a1atyhxzdj where 0=1;

create table ${iol_schema}.mpcs_a1atyhxzdj_cl nologging
compress ${option_switch} for query high
as
select * from ${iol_schema}.mpcs_a1atyhxzdj where 0=1;
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iol_schema}.mpcs_a1atyhxzdj_cl(
            transdt -- 登记日期
            ,transtm -- 登记时间
            ,qqdbs -- 请求单标识
            ,rwlsh -- 任务流水号
        ...
        into ${iol_schema}.mpcs_a1atyhxzdj_op(
            transdt -- 登记日期
            ,transtm -- 登记时间
            ,qqdbs -- 请求单标识
            ,rwlsh -- 任务流水号
...
from (select * from ${iol_schema}.mpcs_a1atyhxzdj_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
    full join (select * from ${itl_schema}.mpcs_a1atyhxzdj where etl_dt = to_date('${batch_date}', 'yyyymmdd')) n
        on
            o.qqdbs = n.qqdbs
            and o.rwlsh = n.rwlsh
where (
...
select
    ...
    ,o.id_mark -- 增删标志
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间
from ${iol_schema}.mpcs_a1atyhxzdj_bk o
    left join ${iol_schema}.mpcs_a1atyhxzdj_op n
        on
            o.qqdbs = n.qqdbs
            and o.rwlsh = n.rwlsh
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.mpcs_a1atyhxzdj_cl d
        on
            o.qqdbs = d.qqdbs
            and o.rwlsh = d.rwlsh
...
-- truncate table ${iol_schema}.mpcs_a1atyhxzdj;

-- 4.2 exchange partition
alter table ${iol_schema}.mpcs_a1atyhxzdj exchange partition p_19000101 with table ${iol_schema}.mpcs_a1atyhxzdj_cl;
alter table ${iol_schema}.mpcs_a1atyhxzdj exchange partition p_20991231 with table ${iol_schema}.mpcs_a1atyhxzdj_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iol_schema}.mpcs_a1atyhxzdj_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iol_schema}',tabname => 'mpcs_a1atyhxzdj',partname => 'p_20991231', granularity => 'PARTITION', degree => 8, cascade => true);
