...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_oass_crss_business_contract
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
alter table ${idl_schema}.oass_crss_business_contract drop partition p_${retain_week};
alter table ${idl_schema}.oass_crss_business_contract drop partition p_${batch_date};


-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.oass_crss_business_contract add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.oass_crss_business_contract (
    etl_dt  -- 数据日期
    ,serialno  -- 
    ,relativeserialno  -- 
    ,artificialno  -- 
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'oass_crss_business_contract',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);