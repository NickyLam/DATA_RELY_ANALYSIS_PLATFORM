...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_ast_col_arch_land_use_info
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
--alter table ${idl_schema}.ast_col_arch_land_use_info drop partition p_${last_date};
alter table ${idl_schema}.ast_col_arch_land_use_info drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.ast_col_arch_land_use_info add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.ast_col_arch_land_use_info (
    etl_dt  -- 数据日期
    ,asset_id  -- 资产编号
    ,lp_id  -- 法人编号
...
select
    ...
    ,t1.land_use_right_area  -- 土地使用权面积
    ,replace(replace(t1.idle_land_type_cd,chr(13),''),chr(10),'')  -- 闲置土地类型代码
    ...
    ,replace(replace(t1.parcel_id,chr(13),''),chr(10),'')  -- 宗地编号
    ,t1.buy_dt  -- 购入日期
    ,t1.buy_amt  -- 购入金额
    ,replace(replace(t1.attachmen_flg,chr(13),''),chr(10),'')  -- 附着物标志
    ,replace(replace(t1.attachmen_type_cd,chr(13),''),chr(10),'')  -- 附着物类型代码
    ,t1.build_qtty  -- 建筑物数量
    ,replace(replace(t1.attachmen_owner_name,chr(13),''),chr(10),'')  -- 附着物所有人名称
    ,replace(replace(t1.attachmen_owner_type_cd,chr(13),''),chr(10),'')  -- 附着物所有人类型代码
    ,t1.attachmen_tot_area  -- 附着物总面积
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'ast_col_arch_land_use_info',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);