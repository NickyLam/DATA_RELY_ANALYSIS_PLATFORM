...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_oass_agt_loan_repay_plan_pd_h
CreateDate: 20221106
FileType:   DML
Logs:
*/
...
alter table ${idl_schema}.oass_agt_loan_repay_plan_pd_h drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.oass_agt_loan_repay_plan_pd_h add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.oass_agt_loan_repay_plan_pd_h (
etl_dt  --数据日期
,acct_id  --账户编号
,curr_pd  --当前期次
,cust_id  --客户编号
...
,t1.comp_int_bal as comp_int_bal --复利余额
...
from ${iml_schema}.agt_loan_repay_plan_pd_h t1    --贷款还款计划期次历史
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'oass_agt_loan_repay_plan_pd_h',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);
