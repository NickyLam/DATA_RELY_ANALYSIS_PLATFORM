...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_icrm_cmm_fx_cap_post
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
alter table ${idl_schema}.icrm_cmm_fx_cap_post drop partition p_${last_date};
alter table ${idl_schema}.icrm_cmm_fx_cap_post drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.icrm_cmm_fx_cap_post add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.icrm_cmm_fx_cap_post partition for (to_date('${batch_date}','yyyymmdd')) (
    etl_dt  -- 数据日期
    ,lp_id  -- 法人编号
    ,bal_id  -- 余额编号
...
select
    ...
    ,replace(replace(t1.main_asset_id,chr(13),''),chr(10),'')  -- 主资产编号
    ,replace(replace(t1.minor_asset_id,chr(13),''),chr(10),'')  -- 次资产编号
...
from ${icl_schema}.cmm_fx_cap_post t1    --外汇资金持仓
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'icrm_cmm_fx_cap_post',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);