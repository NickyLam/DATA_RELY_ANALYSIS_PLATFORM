...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_icrm_cmm_corp_cust_basic_info
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
alter table ${idl_schema}.icrm_cmm_corp_cust_basic_info drop partition p_${last_date};
alter table ${idl_schema}.icrm_cmm_corp_cust_basic_info drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.icrm_cmm_corp_cust_basic_info add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.icrm_cmm_corp_cust_basic_info (
    etl_dt  -- 数据日期
    ,lp_id  -- 法人编号
    ,cust_id  -- 客户编号
    ,cust_name  -- 客户名称
...
select
    ...
    ,replace(replace(t1.lp_org_name,chr(13),''),chr(10),'') as lp_org_name
    ,replace(replace(t1.lp_org_type_cd,chr(13),''),chr(10),'') as lp_org_type_cd
    ,replace(replace(t1.lp_org_cust_id,chr(13),''),chr(10),'') as lp_org_cust_id
    ...
    ,replace(replace(t1.strate_new_indus_cls_cd,chr(13),''),chr(10),'') as strate_new_indus_cls_cd
    ...
    ,replace(replace(t1.basic_acct_acct_num,chr(13),''),chr(10),'') as basic_acct_acct_num
    ...
    ,replace(replace(t1.green_bond_proj_flg,chr(13),''),chr(10),'') as green_bond_proj_flg
    ,replace(replace(t1.prod_mang_addr,chr(13),''),chr(10),'') as prod_mang_addr
    ,replace(replace(t1.prod_mang_addr_zip_cd,chr(13),''),chr(10),'') as prod_mang_addr_zip_cd
    ,replace(replace(t1.mang_site_cd,chr(13),''),chr(10),'') as mang_site_cd
    ...
    ,replace(replace(t1.orgnz_surviv_status_cd,chr(13),''),chr(10),'') as orgnz_surviv_status_cd
    ,replace(replace(t1.corp_idti_idf_type_cd,chr(13),''),chr(10),'') as corp_idti_idf_type_cd
    ,t1.major_contrior_cnt as major_contrior_cnt
    ,t1.actl_ctrler_cnt as actl_ctrler_cnt
    ,replace(replace(t1.fin_dept_phone,chr(13),''),chr(10),'') as fin_dept_phone
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'icrm_cmm_corp_cust_basic_info',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);