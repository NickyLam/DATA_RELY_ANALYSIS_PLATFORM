...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_oass_ast_col_gare_info
CreateDate: 20221106
FileType:   DML
Logs:
*/
...
alter table ${idl_schema}.oass_ast_col_gare_info drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.oass_ast_col_gare_info add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.oass_ast_col_gare_info (
etl_dt  --ETL处理日期
...
,replace(replace(t1.single_pro_cert_flg,chr(13),''),chr(10),'') as single_pro_cert_flg --独立产权证标志
,replace(replace(t1.gare_type_cd,chr(13),''),chr(10),'') as gare_type_cd --车库类型代码
...
from ${iml_schema}.ast_col_gare_info t1    --押品车库信息
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'oass_ast_col_gare_info',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);
