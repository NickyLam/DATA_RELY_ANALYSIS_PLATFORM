...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_oass_prd_trust_prod
CreateDate: 20221106
FileType:   DML
Logs:
*/
...
alter table ${idl_schema}.oass_prd_trust_prod drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.oass_prd_trust_prod add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.oass_prd_trust_prod (
etl_dt  --ETL处理日期
...
,replace(replace(t1.belong_cate_cd,chr(13),''),chr(10),'') as belong_cate_cd --归属类别代码
,replace(replace(t1.prod_tepla,chr(13),''),chr(10),'') as prod_tepla --产品模板
...
,t1.nv_dt as nv_dt --净值日期
,t1.nv_days as nv_days --净值天数
,t1.prod_fac_val as prod_fac_val --产品面值
,t1.issue_price as issue_price --发行价格
,replace(replace(t1.prod_sponsor_id,chr(13),''),chr(10),'') as prod_sponsor_id --产品发起人编号
,replace(replace(t1.prod_trustee_id,chr(13),''),chr(10),'') as prod_trustee_id --产品托管人编号
,replace(replace(t1.prod_mger_id,chr(13),''),chr(10),'') as prod_mger_id --产品管理人编号
,replace(replace(t1.prod_host_dept_id,chr(13),''),chr(10),'') as prod_host_dept_id --产品主办部门编号
...
,t1.int_closing_dt as int_closing_dt --利息截止日期
,t1.prft_exp_dt as prft_exp_dt --收益到期日期
,t1.aft_coll_close_exp_dt as aft_coll_close_exp_dt --募集后封闭到期日期
...
,t1.prod_curr_size as prod_curr_size --产品当前规模
,replace(replace(t1.allow_divd_way_cd,chr(13),''),chr(10),'') as allow_divd_way_cd --允许分红方式代码
...
,replace(replace(t1.lmt_ctrl_flg_cd,chr(13),''),chr(10),'') as lmt_ctrl_flg_cd --额度控制标志代码
...
,replace(replace(t1.chn_cd_comb,chr(13),''),chr(10),'') as chn_cd_comb --渠道代码组合
,replace(replace(t1.allow_cust_type_cd_comb,chr(13),''),chr(10),'') as allow_cust_type_cd_comb --允许客户类型代码组合
,replace(replace(t1.tepla_flg_cd,chr(13),''),chr(10),'') as tepla_flg_cd --模板标志代码
,replace(replace(t1.ctrl_flg_comb,chr(13),''),chr(10),'') as ctrl_flg_comb --控制标志组合
,replace(replace(t1.bta_ctrl_flg_comb,chr(13),''),chr(10),'') as bta_ctrl_flg_comb --BTA控制标志组合
,replace(replace(t1.charge_way_cd,chr(13),''),chr(10),'') as charge_way_cd --收费方式代码
,replace(replace(t1.out_charge_flg,chr(13),''),chr(10),'') as out_charge_flg --外收费标志
,replace(replace(t1.subscr_export_mode_cd,chr(13),''),chr(10),'') as subscr_export_mode_cd --认购导出模式代码
...
,replace(replace(t1.estim_level_cd,chr(13),''),chr(10),'') as estim_level_cd --评估等级代码
,replace(replace(t1.status_cd,chr(13),''),chr(10),'') as status_cd --状态代码
,replace(replace(t1.tran_flg_cd,chr(13),''),chr(10),'') as tran_flg_cd --转换标志代码
,t1.prod_curr_tot_lot as prod_curr_tot_lot --产品当前总份额
,t1.prod_acm_nv as prod_acm_nv --产品累计净值
,replace(replace(t1.curr_cd,chr(13),''),chr(10),'') as curr_cd --币种代码
,replace(replace(t1.prft_curr_cd,chr(13),''),chr(10),'') as prft_curr_cd --收益币种代码
,replace(replace(t1.ec_flg,chr(13),''),chr(10),'') as ec_flg --钞汇标志
,replace(replace(t1.discnt_way_cd,chr(13),''),chr(10),'') as discnt_way_cd --折扣方式代码
...
,t1.indv_lowt_reg_redem_lot as indv_lowt_reg_redem_lot --个人最低定赎份额
...
,t1.aip_fail_cnt as aip_fail_cnt --定投失败次数
,replace(replace(t1.sp_acct_id,chr(13),''),chr(10),'') as sp_acct_id --认申购账户编号
,replace(replace(t1.redem_acct_id,chr(13),''),chr(10),'') as redem_acct_id --赎回账户编号
,replace(replace(t1.comm_fee_assign_acct_id,chr(13),''),chr(10),'') as comm_fee_assign_acct_id --手续费分配账户编号
,replace(replace(t1.mgmt_fee_assign_acct_id,chr(13),''),chr(10),'') as mgmt_fee_assign_acct_id --管理费分配账户编号
,t1.redem_cap_avl_days as redem_cap_avl_days --赎回资金到帐天数
,t1.divd_cap_avl_days as divd_cap_avl_days --分红资金到帐天数
,t1.prod_exp_cap_avl_days as prod_exp_cap_avl_days --产品到期资金到帐天数
,t1.issue_fail_refund_days as issue_fail_refund_days --发行失败退款天数
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'oass_prd_trust_prod',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);
