...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_oass_ast_col_resd_build_info
CreateDate: 20221106
FileType:   DML
Logs:
*/
...
alter table ${idl_schema}.oass_ast_col_resd_build_info drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.oass_ast_col_resd_build_info add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.oass_ast_col_resd_build_info (
etl_dt  --ETL处理日期
,curt_house_flg  --现房标志
...
,replace(replace(t1.curt_house_flg,chr(13),''),chr(10),'') as curt_house_flg --现房标志
,replace(replace(t1.presell_lics_id,chr(13),''),chr(10),'') as presell_lics_id --预售许可证编号
,t1.expect_dlvy_tm as expect_dlvy_tm --预计交付时间
,t1.expect_rel_esat_wat_tm as expect_rel_esat_wat_tm --预计取得不动产权证时间
...
,replace(replace(t1.sell_recd_flg,chr(13),''),chr(10),'') as sell_recd_flg --销售备案标志
...
,replace(replace(t1.aptmt_cd,chr(13),''),chr(10),'') as aptmt_cd --套型代码
...
,t1.plot_ratio as plot_ratio --容积率
...
,t1.land_use_area as land_use_area --土地使用面积
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'oass_ast_col_resd_build_info',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);
