...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_ast_col_cnstring_proj_info
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
--alter table ${idl_schema}.ast_col_cnstring_proj_info drop partition p_${last_date};
alter table ${idl_schema}.ast_col_cnstring_proj_info drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.ast_col_cnstring_proj_info add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.ast_col_cnstring_proj_info (
    etl_dt  -- 数据日期
    ,asset_id  -- 资产编号
    ,lp_id  -- 法人编号
    ...
    ,cnstr_land_use_permit_id  -- 建设用地规划许可证编号
    ,cnstr_proj_plan_permit_id  -- 建设工程规划许可证编号
    ,proj_cnstr_lics_id  -- 建设工程施工许可证编号
    ,start_work_dt  -- 开工日期
...
select
    ...
    ,t1.land_area  -- 土地面积
    ,t1.land_tranf_fee_amt  -- 土地出让金金额
    ,replace(replace(t1.land_tranf_fee_dlvy_flg,chr(13),''),chr(10),'')  -- 土地出让金交付标志
    ,t1.attach_tranf_fee_amt  -- 应补出让金金额
    ,replace(replace(t1.land_usage_cd,chr(13),''),chr(10),'')  -- 土地用途代码
    ,replace(replace(t1.proj_proj_name,chr(13),''),chr(10),'')  -- 工程项目名称
    ,replace(replace(t1.cnstr_land_use_permit_id,chr(13),''),chr(10),'')  -- 建设用地规划许可证编号
    ,replace(replace(t1.cnstr_proj_plan_permit_id,chr(13),''),chr(10),'')  -- 建设工程规划许可证编号
    ,replace(replace(t1.proj_cnstr_lics_id,chr(13),''),chr(10),'')  -- 建设工程施工许可证编号
    ,t1.start_work_dt  -- 开工日期
    ,t1.expect_cmplt_dt  -- 预计竣工日期
    ,t1.proj_expect_tot_cost  -- 工程预计总造价
    ,t1.arch_area  -- 建筑面积
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'ast_col_cnstring_proj_info',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);