...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_aml_org_int_org
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
alter table ${idl_schema}.aml_org_int_org drop partition p_${last_date};
alter table ${idl_schema}.aml_org_int_org drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.aml_org_int_org add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.aml_org_int_org partition for (to_date('${batch_date}','yyyymmdd')) (
    org_id  -- 机构编号
    ,lp_id  -- 法人编号
    ,org_name  -- 机构名称
    ,org_abbr  -- 机构简称
...
select
    ...
    ,replace(replace(t2.org_status_cd,chr(13),''),chr(10),'')  -- 机构状态代码
    ,replace(replace(t4.org_status_cd,chr(13),''),chr(10),'')  -- 机构营业状态代码
...
left join iml.org_intnal_org_name_h t3  --内部机构名称历史
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'aml_org_int_org',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);