...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_gzfh_pams_dxgx_hyyjgx_dk
CreateDate: 20260130
FileType:   DML
Logs:
*/
...
alter table ${idl_schema}.gzfh_pams_dxgx_hyyjgx_dk drop partition p_${batch_date};

-- 2.1.1 truncate target table
whenever sqlerror exit sql.sqlcode;
truncate table ${idl_schema}.gzfh_pams_dxgx_hyyjgx_dk;

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.gzfh_pams_dxgx_hyyjgx_dk add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.gzfh_pams_dxgx_hyyjgx_dk (
tjrq  --数据日期
,jxdxdh  --绩效对象代号
,khdxdh  --考核对象代号
,fpjs  --分配角色
...
,replace(replace(dk.zhdh,chr(13),''),chr(10),'') as zhdh --账户代号
,replace(replace(dk.zzh,chr(13),''),chr(10),'') as zzh --子账户
,replace(replace(dk.jjh,chr(13),''),chr(10),'') as jjh --借据号
...
from ${iol_schema}.pams_dxgx_hyyjgx_dk t1    --对象关系-行员业绩关系-贷款
...
inner join iol.pams_jxdx_dkzh dk 
on t1.jxdxdh=dk.jxdxdh
	and (dk.start_dt <= to_date('${batch_date}', 'yyyymmdd') and dk.end_dt > to_date('${batch_date}', 'yyyymmdd'))
	and dk.id_mark <> 'D'
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'gzfh_pams_dxgx_hyyjgx_dk',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);
