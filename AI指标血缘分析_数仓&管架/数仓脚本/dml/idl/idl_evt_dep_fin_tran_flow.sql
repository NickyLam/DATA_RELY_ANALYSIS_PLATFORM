...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_evt_dep_fin_tran_flow
CreateDate: 20221107
FileType:   DML
Logs:
*/
...
alter table ${idl_schema}.evt_dep_fin_tran_flow drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.evt_dep_fin_tran_flow add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'evt_dep_fin_tran_flow',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);
