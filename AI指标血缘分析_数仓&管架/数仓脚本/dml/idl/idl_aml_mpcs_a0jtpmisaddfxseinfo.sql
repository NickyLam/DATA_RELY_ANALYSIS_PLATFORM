...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_aml_mpcs_a0jtpmisaddfxseinfo
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
alter table ${idl_schema}.aml_mpcs_a0jtpmisaddfxseinfo drop partition p_${last_date};
alter table ${idl_schema}.aml_mpcs_a0jtpmisaddfxseinfo drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.aml_mpcs_a0jtpmisaddfxseinfo add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.aml_mpcs_a0jtpmisaddfxseinfo partition for (to_date('${batch_date}','yyyymmdd')) (
    etl_dt  -- 数据日期
    ,mainseq  -- 中台流水号
    ,transdt  -- 交易日期
...
select
    ...
    ,replace(replace(t1.trantype,chr(13),''),chr(10),'')  -- 交易类型 ZJL  占额度结汇录入 ZJB  占额度结汇补录 UJL  不占额度结汇录入 UJB  不占额度结汇补录 ZGL  占额度购汇录入 ZGB  占额度购汇补录 UGL  不占额度购汇录入 UGB  不占额度购汇补录
    ...
    ,replace(replace(t1.salefx_tx_code,chr(13),''),chr(10),'')  -- 结汇资金属性代码
    ,replace(replace(t1.txccy,chr(13),''),chr(10),'')  -- 币种
    ,replace(replace(t1.salefx_amt,chr(13),''),chr(10),'')  -- 结汇金额
    ,replace(replace(t1.lcy_acctno_cny,chr(13),''),chr(10),'')  -- 结汇人民币账户
    ,replace(replace(t1.salefx_settle_code,chr(13),''),chr(10),'')  -- 结汇资金形态代码
    ...
    ,replace(replace(t1.salefx_amt_usd,chr(13),''),chr(10),'')  -- 本次结汇金额折美元
...
from ${iol_schema}.mpcs_a0jtpmisaddfxseinfo t1    --结汇录入补录表
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'aml_mpcs_a0jtpmisaddfxseinfo',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);