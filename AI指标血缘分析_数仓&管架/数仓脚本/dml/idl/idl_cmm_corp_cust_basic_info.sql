...
select
    ...
    ,replace(replace(t1.group_type_cd,chr(13),''),chr(10),'') as group_type_cd                       --集团类型代码
    ,replace(replace(t1.green_bond_proj_flg,chr(13),''),chr(10),'') as green_bond_proj_flg           --绿色债券项目标志
    ,replace(replace(t1.stock_cd,chr(13),''),chr(10),'') as stock_cd    --股票代码
    ,replace(replace(t1.dep_class_cust_flg,chr(13),''),chr(10),'') as dep_class_cust_flg                  --存款类客户标志 
    ,replace(replace(t1.loan_class_cust_flg,chr(13),''),chr(10),'') as loan_class_cust_flg                --贷款类客户标志 
    ,replace(replace(t1.guar_class_cust_flg,chr(13),''),chr(10),'') as guar_class_cust_flg                --担保客户标志代码 
    ,replace(replace(t1.anti_mon_lau_belong_org_id,chr(13),''),chr(10),'') as anti_mon_lau_belong_org_id  --反洗钱归属机构编号
