...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_ast_col_guar_corp_ensure
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
--alter table ${idl_schema}.ast_col_guar_corp_ensure drop partition p_${last_date};
alter table ${idl_schema}.ast_col_guar_corp_ensure drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.ast_col_guar_corp_ensure add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.ast_col_guar_corp_ensure (
    etl_dt  -- 数据日期
    ,asset_id  -- 资产编号
    ,lp_id  -- 法人编号
...
select
    ...
    ,replace(replace(t1.guartor_orgnz_cd,chr(13),''),chr(10),'')  -- 保证人组织机构代码
    ...
    ,t1.guar_corp_margin_amt  -- 担保公司保证金金额
    ,replace(replace(t1.other_comnt,chr(13),''),chr(10),'')  -- 其他说明
    ,t1.guar_tot_amt  -- 担保总额度
...
from ${iml_schema}.ast_col_guar_corp_ensure t1    --押品担保公司保证
;
commit;
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'ast_col_guar_corp_ensure',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);