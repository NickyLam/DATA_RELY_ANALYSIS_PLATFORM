...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_aml_isbs_dbq
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
alter table ${idl_schema}.aml_isbs_dbq drop partition p_${last_date};
alter table ${idl_schema}.aml_isbs_dbq drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.aml_isbs_dbq add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.aml_isbs_dbq partition for (to_date('${batch_date}','yyyymmdd')) (
    etl_dt  -- 数据日期
...
select
    ...
    ,replace(replace(t1.cusmno,chr(13),''),chr(10),'')  -- 报关单经营单位代码
    ,replace(replace(t1.customs,chr(13),''),chr(10),'')  -- 报关单信息
    ,replace(replace(t1.crtuser,chr(13),''),chr(10),'')  -- 填报人
    ,replace(replace(t1.inptelc,chr(13),''),chr(10),'')  -- 填报人电话
    ,t1.rptdate  -- 申报日期
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp  -- 数据处理时间
from ${iol_schema}.isbs_dbq t1    --境内汇款申请书－核销专用信息
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'aml_isbs_dbq',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);