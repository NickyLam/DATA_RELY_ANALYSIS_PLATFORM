...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_icrm_ctms_tbs_v_ldrepodeals
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
alter table ${idl_schema}.icrm_ctms_tbs_v_ldrepodeals drop partition p_${last_date};
alter table ${idl_schema}.icrm_ctms_tbs_v_ldrepodeals drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.icrm_ctms_tbs_v_ldrepodeals add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.icrm_ctms_tbs_v_ldrepodeals partition for (to_date('${batch_date}','yyyymmdd')) (
    etl_dt  -- 数据日期
    ,deal_id  -- 引用表ID
    ,deal_tablename  -- 引用表名
    ,aspclient_id  -- 部门编号
...
select
    ...
    ,replace(replace(t1.face_amount,chr(13),''),chr(10),'')  -- 质押券面额
    ,replace(replace(t1.repo_rate,chr(13),''),chr(10),'')  -- 质押比例
    ,t1.amount  -- 首期结算金额
    ...
    ,t1.trade_rate  -- 回购利率
    ,t1.repo_days  -- 回购天数
    ,t1.ldrepodeals_id_grand  -- 原始交易ID
    ,replace(replace(t1.repo_id,chr(13),''),chr(10),'')  -- 回购名称
    ,replace(replace(t1.counterparty_type,chr(13),''),chr(10),'')  -- 计息基准
    ,replace(replace(t1.clearing_type,chr(13),''),chr(10),'')  -- 清算类型
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp   --数据处理时间
from ${iol_schema}.ctms_tbs_v_ldrepodeals t1    --质押式回购交易
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'icrm_ctms_tbs_v_ldrepodeals',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);