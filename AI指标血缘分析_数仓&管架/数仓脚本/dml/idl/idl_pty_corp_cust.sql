...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_pty_corp_cust
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
--alter table ${idl_schema}.pty_corp_cust drop partition p_${last_date};
alter table ${idl_schema}.pty_corp_cust drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.pty_corp_cust add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.pty_corp_cust (
    etl_dt  -- 数据日期
    ,cust_id  -- 客户编号
    ,lp_id  -- 法人编号
    ,sorc_sys_cd  -- 源系统代码
...
select
    ...
    ,replace(replace(t1.corp_cust_type_cd,chr(13),''),chr(10),'')  -- 公司客户类型代码
    ,replace(replace(t1.orgnz_cd,chr(13),''),chr(10),'')  -- 组织机构代码
    ,replace(replace(t1.corp_name,chr(13),''),chr(10),'')  -- 公司名称
    ...
    ,replace(replace(t1.strtg_cust_flg,chr(13),''),chr(10),'')  -- 战略客户标志
    ,replace(replace(t1.off_shore_cust_flg,chr(13),''),chr(10),'')  -- 离岸客户标志
    ,replace(replace(t1.cust_sev_ugd_cls_cd,chr(13),''),chr(10),'')  -- 客户服务升级分类代码
    ,replace(replace(t1.weight_risk_asset_cust_cls_cd,chr(13),''),chr(10),'')  -- 加权风险资产客户分类代码
    ,replace(replace(t1.crdt_strategy_cd,chr(13),''),chr(10),'')  -- 授信策略代码
    ,replace(replace(t1.nb_corp_flg,chr(13),''),chr(10),'')  -- 新建企业标志
    ,replace(replace(t1.hxb_rela_tran_flg,chr(13),''),chr(10),'')  -- 我行关联交易标志
    ,replace(replace(t1.mc_dept_mplize_cust_flg,chr(13),''),chr(10),'')  -- 中小企业事业部专营客户标志
    ,replace(replace(t1.hxb_idtfy_small_bus_flg,chr(13),''),chr(10),'')  -- 我行认定小企业标志
    ...
    ,replace(replace(t1.fin_subsidy_inco_src_cd,chr(13),''),chr(10),'')  -- 财政补助收入来源代码
    ...
    ,replace(replace(t1.subj_org_name,chr(13),''),chr(10),'')  -- 隶属机构名称
    ,replace(replace(t1.cty_rg_cd,chr(13),''),chr(10),'')  -- 国家和地区代码
    ,replace(replace(t1.ctysd_corp_flg,chr(13),''),chr(10),'')  -- 农村企业标志
    ,replace(replace(t1.ta_cust_size,chr(13),''),chr(10),'')  -- 商圈客户规模
    ,replace(replace(t1.ta_cust_indus_status,chr(13),''),chr(10),'')  -- 商圈客户行业地位
    ,replace(replace(t1.ins_adj_type_cd,chr(13),''),chr(10),'')  -- 产业结构调整类型代码
    ,replace(replace(t1.itau_flg,chr(13),''),chr(10),'')  -- 工业转型升级标志
    ,replace(replace(t1.strtg_new_indus_type_cd,chr(13),''),chr(10),'')  -- 战略新兴产业类型代码
    ...
    ,replace(replace(t1.orgnz_type_subdv_cd,chr(13),''),chr(10),'')  -- 组织机构类型细分代码
    ,replace(replace(t1.org_status_cd,chr(13),''),chr(10),'')  -- 机构状态代码
    ,replace(replace(t1.orgnz_type_cd,chr(13),''),chr(10),'')  -- 组织机构类型代码
    ,replace(replace(t1.soci_crdt_cd,chr(13),''),chr(10),'')  -- 社会信用代码
    ,replace(replace(t1.strategy_camp_cust_no,chr(13),''),chr(10),'')  -- 策略营销客户号
    ,t1.single_lmt  -- 单一限额
    ,replace(replace(t1.corp_size_cd_intnal,chr(13),''),chr(10),'')  -- 企业规模代码_内部
    ,replace(replace(t1.green_crdt_cust_flg,chr(13),''),chr(10),'')  -- 绿色信贷客户标志
    ,replace(replace(t1.single_lp_flg,chr(13),''),chr(10),'')  -- 独立法人标志
    ,replace(replace(t1.cust_ownsp_type_cd,chr(13),''),chr(10),'')  -- 客户所有制类型代码
    ,replace(replace(t1.belong_rela_group_id,chr(13),''),chr(10),'')  -- 所属关联集团编号
    ,replace(replace(t1.araf_flg,chr(13),''),chr(10),'')  -- 三农标志
    ,replace(replace(t1.prtcptr_cate_cd,chr(13),''),chr(10),'')  -- 参与者类别代码
    ,t1.rgst_cap  -- 注册资金
    ,replace(replace(t1.bank_no,chr(13),''),chr(10),'')  -- 银行行号
    ,replace(replace(t1.bank_lev_cd,chr(13),''),chr(10),'')  -- 银行级别代码
    ,replace(replace(t1.ibank_no,chr(13),''),chr(10),'')  -- 银行联行号
    ,replace(replace(t1.cpes_corp_size_cd,chr(13),''),chr(10),'')  -- 票交所企业规模代码
    ,replace(replace(t1.cpes_indus_type_cd,chr(13),''),chr(10),'')  -- 票交所行业类型代码
    ,replace(replace(t1.edu_hea_flg,chr(13),''),chr(10),'')  -- 文教健康标志
    ,replace(replace(t1.inc_flg,chr(13),''),chr(10),'')  -- 普惠型标志
    ,replace(replace(t1.labor_inte_corp_flg,chr(13),''),chr(10),'')  -- 劳动密集型企业标志
...
from ${iml_schema}.pty_corp_cust t1    --公司贷款出账申请
;
commit;
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'pty_corp_cust',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);