...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_aml_isbs_dbg
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
alter table ${idl_schema}.aml_isbs_dbg drop partition p_${last_date};
alter table ${idl_schema}.aml_isbs_dbg drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.aml_isbs_dbg add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.aml_isbs_dbg partition for (to_date('${batch_date}','yyyymmdd')) (
    etl_dt  -- 数据日期
...
select
    ...
    ,replace(replace(t1.country,chr(13),''),chr(10),'')  -- 付款人常驻国家/地区编码
    ...
    ,replace(replace(t1.isref,chr(13),''),chr(10),'')  -- 是否保税货物项下收入
    ,replace(replace(t1.billno,chr(13),''),chr(10),'')  -- 外汇局批件号/备案表号/业务编号
...
from ${iol_schema}.isbs_dbg t1    --涉外收入申报单-申报信息
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'aml_isbs_dbg',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);