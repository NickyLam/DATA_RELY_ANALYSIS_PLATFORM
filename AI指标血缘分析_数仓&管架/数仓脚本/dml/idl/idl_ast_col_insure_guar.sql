...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_ast_col_insure_guar
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
--alter table ${idl_schema}.ast_col_insure_guar drop partition p_${last_date};
alter table ${idl_schema}.ast_col_insure_guar drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.ast_col_insure_guar add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.ast_col_insure_guar (
    etl_dt  -- 数据日期
    ,asset_id  -- 资产编号
    ,lp_id  -- 法人编号
...
select
    ...
    ,replace(replace(t1.guar_insure_policy_num,chr(13),''),chr(10),'')  -- 保证保险保单号码
...
from ${iml_schema}.ast_col_insure_guar t1    --押品保险保证
;
commit;
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'ast_col_insure_guar',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);