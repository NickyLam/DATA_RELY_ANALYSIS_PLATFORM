...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_oass_agt_loan_appl_indv_loan_attach_info_h
CreateDate: 20221106
FileType:   DML
Logs:
*/
...
alter table ${idl_schema}.oass_agt_loan_appl_indv_loan_attach_info_h drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.oass_agt_loan_appl_indv_loan_attach_info_h add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.oass_agt_loan_appl_indv_loan_attach_info_h (
etl_dt  --数据日期
,buy_cont_id  --购房合同编号
...
,t1.arch_area_price as arch_area_price --建筑面积单价
,t1.set_of_area_price as set_of_area_price --套内面积单价
...
,replace(replace(t1.seller_bus_lics_id,chr(13),''),chr(10),'') as seller_bus_lics_id --经销商营业执照编号
...
,replace(replace(t1.seller_bear_repo_duty_flg,chr(13),''),chr(10),'') as seller_bear_repo_duty_flg --经销商承担回购责任标志
...
,t1.entr_dep_amt as entr_dep_amt --委托存款金额
...
,replace(replace(t1.white_acct_flg,chr(13),''),chr(10),'') as white_acct_flg --白户标志
,replace(replace(t1.bar_flg,chr(13),''),chr(10),'') as bar_flg --随借随还标志
,replace(replace(t1.and_hxb_exist_incid_rela_flg,chr(13),''),chr(10),'') as and_hxb_exist_incid_rela_flg --与我行存在关联关系标志
,replace(replace(t1.hxb_open_supv_acct_flg,chr(13),''),chr(10),'') as hxb_open_supv_acct_flg --在我行开立监管账户标志
,t1.blon_loan_amort_exp_dt as blon_loan_amort_exp_dt --气球贷摊销到期日期
,replace(replace(t1.intd_blip_flg,chr(13),''),chr(10),'') as intd_blip_flg --引入影像标志
,replace(replace(t1.blip_flow_num,chr(13),''),chr(10),'') as blip_flow_num --影像流水号
,replace(replace(t1.blip_cmplt_upload_flg,chr(13),''),chr(10),'') as blip_cmplt_upload_flg --影像完成上传标志
,t1.sugst_loan_amt as sugst_loan_amt --建议贷款金额
,replace(replace(t1.redem_house_lon_final_risk_mgmt_rest_cd,chr(13),''),chr(10),'') as redem_house_lon_final_risk_mgmt_rest_cd --赎楼贷最终风控结果代码
...
,replace(replace(t1.recver_acct_id,chr(13),''),chr(10),'') as recver_acct_id --收款人帐户编号
,t1.grace_days as grace_days --宽限天数
,replace(replace(t1.open_acct_bind_mobile_no,chr(13),''),chr(10),'') as open_acct_bind_mobile_no --开户绑定手机号码
,replace(replace(t1.flow_type_cd,chr(13),''),chr(10),'') as flow_type_cd --流程类型代码
,replace(replace(t1.corp_lmt_ctrl_flg,chr(13),''),chr(10),'') as corp_lmt_ctrl_flg --公司额度管控标志
,t1.rtn_pric_ratio as rtn_pric_ratio --归还本金比例
,t1.rtn_pric_intrv as rtn_pric_intrv --归还本金间隔
,replace(replace(t1.invstg_opinion_descb,chr(13),''),chr(10),'') as invstg_opinion_descb --调查意见描述
,t1.crdt_level as crdt_level --信用等级
,t1.apv_end_tm as apv_end_tm --审批结束时间
,replace(replace(t1.chn_id,chr(13),''),chr(10),'') as chn_id --渠道编号
,replace(replace(t1.rest_advise_sucs_flg,chr(13),''),chr(10),'') as rest_advise_sucs_flg --结果通知成功标志
,t1.apv_tm as apv_tm --审批通过时间
...
from ${iml_schema}.agt_loan_appl_indv_loan_attach_info_h t1    --贷款申请个人贷款附属信息历史
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'oass_agt_loan_appl_indv_loan_attach_info_h',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);
