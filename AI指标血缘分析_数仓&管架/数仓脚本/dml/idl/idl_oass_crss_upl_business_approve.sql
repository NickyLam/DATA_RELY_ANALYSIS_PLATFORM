...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_oass_crss_upl_business_approve
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
alter table ${idl_schema}.oass_crss_upl_business_approve drop partition p_${retain_week};
alter table ${idl_schema}.oass_crss_upl_business_approve drop partition p_${batch_date};


-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.oass_crss_upl_business_approve add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.oass_crss_upl_business_approve (
    etl_dt  -- 数据日期
    ,serialno  -- 流水号
    ,relativeserialno  -- 相关申请流水号
    ,occurdate  -- 发生日期
    ,customerid  -- 客户编号
    ,customername  -- 客户名称
    ,businesstype  -- 业务品种
...
select
    ...
    ,replace(replace(t1.relativeserialno,chr(13),''),chr(10),'')  -- 相关申请流水号
    ...
    ,replace(replace(t1.paycurrency,chr(13),''),chr(10),'')  -- (new)付款币种
    ,replace(replace(t1.paydate,chr(13),''),chr(10),'')  -- (new)付款时间
    ,replace(replace(t1.flag4,chr(13),''),chr(10),'')  -- (new)是否4
    ,replace(replace(t1.fundsource,chr(13),''),chr(10),'')  -- 资金来源
    ,replace(replace(t1.operatetype,chr(13),''),chr(10),'')  -- 操作方式
    ,replace(replace(t1.approvetype,chr(13),''),chr(10),'')  -- 批复类型
    ...
    ,replace(replace(t1.creditcycle,chr(13),''),chr(10),'')  -- (new)额度是否循环
    ...
    ,replace(replace(t1.flag5,chr(13),''),chr(10),'')  -- (new)是否已登记合同
    ...
    ,replace(replace(t1.classifyresulteleven,chr(13),''),chr(10),'')  -- 风险分类
    ,replace(replace(t1.effectflag,chr(13),''),chr(10),'')  -- 生效标志
    ,replace(replace(t1.approvedate,chr(13),''),chr(10),'')  -- 审批通过日
    ,replace(replace(t1.creditmode,chr(13),''),chr(10),'')  -- 授信模式
...
from ${iol_schema}.crss_upl_business_approve t1    --微贷业务批复表
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'oass_crss_upl_business_approve',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);