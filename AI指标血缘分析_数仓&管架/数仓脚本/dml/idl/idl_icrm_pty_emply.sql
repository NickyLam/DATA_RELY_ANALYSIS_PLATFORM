...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_icrm_pty_emply
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
alter table ${idl_schema}.icrm_pty_emply drop partition p_${last_date};
alter table ${idl_schema}.icrm_pty_emply drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.icrm_pty_emply add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.icrm_pty_emply partition for (to_date('${batch_date}','yyyymmdd')) (
    etl_dt  -- 数据日期
    ,emply_id  -- 员工编号
    ,region_acct_num  -- 域账号
    ,first_name  -- 名字
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'icrm_pty_emply',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);