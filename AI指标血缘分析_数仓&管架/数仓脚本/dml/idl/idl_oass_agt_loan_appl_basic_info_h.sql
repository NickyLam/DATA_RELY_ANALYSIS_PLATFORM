...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_oass_agt_loan_appl_basic_info_h
CreateDate: 20221106
FileType:   DML
Logs:
*/
...
alter table ${idl_schema}.oass_agt_loan_appl_basic_info_h drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.oass_agt_loan_appl_basic_info_h add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.oass_agt_loan_appl_basic_info_h (
etl_dt  --数据日期
,rela_flow_num  --关联流水号
,risk_type_cd  --风险类型代码
...
,replace(replace(t1.info_type_cd,chr(13),''),chr(10),'') as info_type_cd --信息类型代码
...
,t1.appl_amt as appl_amt --申请金额
,replace(replace(t1.lmt_base_prod_id,chr(13),''),chr(10),'') as lmt_base_prod_id --额度基础产品编号
,replace(replace(t1.prod_id,chr(13),''),chr(10),'') as prod_id --产品编号
,replace(replace(t1.prod_policy_id,chr(13),''),chr(10),'') as prod_policy_id --产品政策编号
,replace(replace(t1.prod_policy_edit_id,chr(13),''),chr(10),'') as prod_policy_edit_id --产品政策版本编号
...
,replace(replace(t1.rela_regroup_prop_id,chr(13),''),chr(10),'') as rela_regroup_prop_id --关联重组方案编号
...
,replace(replace(t1.rgst_reply_flg,chr(13),''),chr(10),'') as rgst_reply_flg --登记批复标志
...
,replace(replace(t1.intd_bd_flg,chr(13),''),chr(10),'') as intd_bd_flg --引入大数据标志
...
from ${iml_schema}.agt_loan_appl_basic_info_h t1    --贷款申请基本信息历史
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'oass_agt_loan_appl_basic_info_h',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);
