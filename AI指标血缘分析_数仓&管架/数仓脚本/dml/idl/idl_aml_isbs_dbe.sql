...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_aml_isbs_dbe
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
alter table ${idl_schema}.aml_isbs_dbe drop partition p_${last_date};
alter table ${idl_schema}.aml_isbs_dbe drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.aml_isbs_dbe add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.aml_isbs_dbe partition for (to_date('${batch_date}','yyyymmdd')) (
    etl_dt  -- 数据日期
...
select
    ...
    ,replace(replace(t1.oppacc,chr(13),''),chr(10),'')  -- 收款人帐号
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp  -- 数据处理时间
from ${iol_schema}.isbs_dbe t1    --境内汇款申请书-基础信息
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'aml_isbs_dbe',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);