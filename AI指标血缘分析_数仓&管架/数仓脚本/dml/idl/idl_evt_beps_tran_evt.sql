...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_evt_beps_tran_evt
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
--alter table ${idl_schema}.evt_beps_tran_evt drop partition p_${last_date};
alter table ${idl_schema}.evt_beps_tran_evt drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror continue none;
alter table ${idl_schema}.evt_beps_tran_evt add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.evt_beps_tran_evt (
    etl_dt  -- 数据日期
    ,evt_id  -- 事件编号
    ,lp_id  -- 法人编号
...
select
    ...
    ,replace(replace(p1.pkg_seq_num,chr(13),''),chr(10),'')  -- 包序号
    ,p1.pkg_entr_dt  -- 包委托日期
    ,replace(replace(p1.pkg_type,chr(13),''),chr(10),'')  -- 包类型
    ...
    ,replace(replace(p1.recv_bank_no,chr(13),''),chr(10),'')  -- 接收行行号
    ,p1.init_entr_dt  -- 原委托日期
    ,replace(replace(p1.init_pay_tran_seq_num,chr(13),''),chr(10),'')  -- 原支付交易序号
    ,replace(replace(p1.init_bus_type_cd,chr(13),''),chr(10),'')  -- 原业务类型代码
    ,replace(replace(p1.bill_num,chr(13),''),chr(10),'')  -- 票据号码
    ,replace(replace(p1.offs_bal_node_type_cd,chr(13),''),chr(10),'')  -- 轧差节点类型代码
    ,p1.offs_bal_num_site  -- 轧差场次
    ,p1.offs_bal_dt_or_fs_dt  -- 轧差日期或终态日期
    ,replace(replace(p1.refund_rs_cd,chr(13),''),chr(10),'')  -- 退汇原因代码
    ,replace(replace(p1.proc_status_cd,chr(13),''),chr(10),'')  -- 处理状态代码
    ,replace(replace(p1.status_rest_cd,chr(13),''),chr(10),'')  -- 状态处理结果代码
    ,replace(replace(p1.pbc_bus_status_cd,chr(13),''),chr(10),'')  -- 人行业务状态代码
    ,replace(replace(p1.rtn_rcpt_code,chr(13),''),chr(10),'')  -- 回执码
    ,replace(replace(p1.proc_cd,chr(13),''),chr(10),'')  -- 处理代码
    ,replace(replace(p1.sys_type_cd,chr(13),''),chr(10),'')  -- 系统类型代码
    ,replace(replace(p1.node_type_cd,chr(13),''),chr(10),'')  -- 节点类型代码
    ,replace(replace(p1.rest_cd,chr(13),''),chr(10),'')  -- 处理结果代码
    ,replace(replace(p1.check_revs_flow_num,chr(13),''),chr(10),'')  -- 复核冲正流水号
    ,replace(replace(p1.rtn_rcpt_tenor_cd,chr(13),''),chr(10),'')  -- 回执期限代码
    ,p1.rtn_rcpt_dt  -- 回执日期
    ,replace(replace(p1.send_revs_flow_num,chr(13),''),chr(10),'')  -- 发送冲正流水号
    ,p1.clear_dt  -- 清算日期
    ,replace(replace(p1.err_code,chr(13),''),chr(10),'')  -- 错误编码
    ...
    ,replace(replace(p1.bank_int_out_line_flg,chr(13),''),chr(10),'')  -- 行内行外标志
    ,replace(replace(p1.draft_appl_form_num,chr(13),''),chr(10),'')  -- 汇票申请书号码
    ,replace(replace(p1.matn_enter_acct_num,chr(13),''),chr(10),'')  -- 维护入账账号
    ...
    ,p1.entr_vouch_dt  -- 委托凭证日期
    ,replace(replace(p1.entr_vouch_num,chr(13),''),chr(10),'')  -- 委托凭证号
    ...
    ,replace(replace(p1.exch_bus_cors_tran_chn_cd,chr(13),''),chr(10),'')  -- 汇兑业务对应交易渠道代码
    ,p1.recnt_modif_dt  -- 最近修改日期
    ,replace(replace(p1.bus_flow_num,chr(13),''),chr(10),'')  -- 业务流水号
    ,p1.comm_fee  -- 手续费
    ,p1.remit_tran_fee  -- 汇划费
    ,p1.todos  -- 工本费
    ,p1.init_amt  -- 原托金额
    ,p1.pay_amt  -- 支付金额
    ,p1.multi_pay_amt  -- 多付金额
    ,replace(replace(p1.mpr_host_flow_num,chr(13),''),chr(10),'')  -- 维护入账冲账主机流水号
    ,p1.mpr_host_dt  -- 维护入账冲账主机日期
    ,replace(replace(p1.mpr_teller_id,chr(13),''),chr(10),'')  -- 维护入账冲账柜员编号
    ,p1.recnt_modif_tm  -- 最近修改时间
    ,replace(replace(p1.proc_org_id,chr(13),''),chr(10),'')  -- 处理机构编号
    ,replace(replace(p1.rec_update_edit_num,chr(13),''),chr(10),'')  -- 记录更新版本号
    ,replace(replace(p1.rec_status_cd,chr(13),''),chr(10),'')  -- 记录状态代码
    ,replace(replace(p1.init_pkg_type,chr(13),''),chr(10),'')  -- 原包类型
    ,replace(replace(p1.init_pkg_init_clear_bk_num,chr(13),''),chr(10),'')  -- 原包发起清算行号
    ,p1.init_pkg_entr_dt  -- 原包委托日期
    ,replace(replace(p1.init_pkg_seq_num,chr(13),''),chr(10),'')  -- 原包序号
    ...
    ,replace(replace(p1.rgst_addit_data_dtl_name,chr(13),''),chr(10),'')  -- 登记附加数据明细表名称
    ,replace(replace(p1.on_acct_rs_cd,chr(13),''),chr(10),'')  -- 挂账原因代码
    ,replace(replace(p1.pkg_bank_int_seq_num,chr(13),''),chr(10),'')  -- 包行内序号
    ...
    ,replace(replace(p1.last_debit_rtn_rcpt_status_cd,chr(13),''),chr(10),'')  -- 上一借记回执状态代码
    ,replace(replace(p1.last_tran_status_cd,chr(13),''),chr(10),'')  -- 上一交易状态代码
    ,replace(replace(p1.acct_gen_cd,chr(13),''),chr(10),'')  -- 账户大类型代码
    ,replace(replace(p1.lmt_order_no,chr(13),''),chr(10),'')  -- 限额订单号
    ,replace(replace(p1.bind_flg,chr(13),''),chr(10),'')  -- 绑定标志
...
from ${iml_schema}.evt_beps_tran_evt  p1   --小额交易事件
where tran_dt = to_date('${batch_date}','yyyymmdd');
commit;

-- 4 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'evt_beps_tran_evt',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);