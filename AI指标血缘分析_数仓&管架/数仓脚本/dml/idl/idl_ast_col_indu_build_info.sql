...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_ast_col_indu_build_info
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
--alter table ${idl_schema}.ast_col_indu_build_info drop partition p_${last_date};
alter table ${idl_schema}.ast_col_indu_build_info drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.ast_col_indu_build_info add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.ast_col_indu_build_info (
    etl_dt  -- 数据日期
    ,asset_id  -- 资产编号
    ,lp_id  -- 法人编号
...
select
    ...
    ,replace(replace(t1.dev_mode_cd,chr(13),''),chr(10),'')  -- 开发模式代码
    ...
    ,replace(replace(t1.dplat_num,chr(13),''),chr(10),'')  -- 门牌号码
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'ast_col_indu_build_info',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);