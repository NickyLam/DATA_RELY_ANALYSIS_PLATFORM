...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_pty_emply
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
--alter table ${idl_schema}.pty_emply drop partition p_${last_date};
alter table ${idl_schema}.pty_emply drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.pty_emply add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.pty_emply (
    etl_dt  -- 数据日期
    ,emply_id  -- 员工编号
    ,region_acct_num  -- 域账号
    ,first_name  -- 名字
...
select
    ...
    ,replace(replace(t1.main_teller_id,chr(13),''),chr(10),'')  -- 主柜员编号
    ,replace(replace(t1.title_cd,chr(13),''),chr(10),'')  -- 职称代码
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'pty_emply',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);