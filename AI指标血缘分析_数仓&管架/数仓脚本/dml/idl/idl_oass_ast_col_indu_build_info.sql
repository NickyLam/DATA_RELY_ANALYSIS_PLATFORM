...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_oass_ast_col_indu_build_info
CreateDate: 20221106
FileType:   DML
Logs:
*/
...
alter table ${idl_schema}.oass_ast_col_indu_build_info drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.oass_ast_col_indu_build_info add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.oass_ast_col_indu_build_info (
etl_dt  --ETL处理日期
...
,replace(replace(t1.rel_esat_wat_id,chr(13),''),chr(10),'') as rel_esat_wat_id --不动产权证编号
,replace(replace(t1.dev_mode_cd,chr(13),''),chr(10),'') as dev_mode_cd --开发模式代码
...
,replace(replace(t1.dplat_num,chr(13),''),chr(10),'') as dplat_num --门牌号码
...
from ${iml_schema}.ast_col_indu_build_info t1    --押品工业用房信息
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'oass_ast_col_indu_build_info',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);
