...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd idl_ccrm_albs_bls_cust_all_aml
CreateDate: 20180515
FileType:   DML
Logs:
    zjj 2018-05-15 新建表本
...
alter table ${idl_schema}.ccrm_albs_bls_cust_all_aml drop partition p_${last_date};
alter table ${idl_schema}.ccrm_albs_bls_cust_all_aml drop partition p_${batch_date};

-- 2.2 add today partition
whenever sqlerror exit sql.sqlcode;
alter table ${idl_schema}.ccrm_albs_bls_cust_all_aml add  partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.3 insert data target table
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${idl_schema}.ccrm_albs_bls_cust_all_aml (
    etl_dt  -- 数据日期
    ,id  -- 主键ID
    ,customer_no  -- 客户号
    ,customer_name  -- 客户名称
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${idl_schema}',tabname => 'ccrm_albs_bls_cust_all_aml',partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);