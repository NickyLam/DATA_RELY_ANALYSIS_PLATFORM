...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_icl_cmm_dep_acct_tran_dtl_m_i
CreateDate: 20230111
FileName:   ${iel_data_path}/cmm_dep_acct_tran_dtl_m.i.${batch_date}.dat
IF_mark:    m_i
Logs:
' \
        query="select
...
where to_char(etl_dt,'yyyymm') = to_char(to_date('${batch_date}','yyyymmdd'),'yyyymm') and t1.entry_flg='1'" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/cmm_dep_acct_tran_dtl_m.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
