...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_agt_amt_h_f_w
CreateDate: 20180529
FileName:   ${iel_data_path}/agt_amt_h_w.f.${batch_date}.dat
IF_mark:    f_w
Logs:
...
where (start_dt <= to_date('${batch_date}', 'yyyymmdd') and
    ...
    or (end_dt <= to_date('${batch_date}', 'yyyymmdd') and
       end_dt >= to_date('${batch_date}', 'yyyymmdd') - 6) ;" \
        ...
        file="${iel_data_path}/agt_amt_h_w.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes