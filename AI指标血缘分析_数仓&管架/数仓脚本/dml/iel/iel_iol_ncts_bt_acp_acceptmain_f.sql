...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_ncts_bt_acp_acceptmain_f
CreateDate: 20180529
FileName:   ${iel_data_path}/ncts_bt_acp_acceptmain.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,replace(replace(t1.tellerphone,chr(13),''),chr(10),'') as tellerphone 
,t1.start_dt as start_dt 
,t1.end_dt as end_dt 
,replace(replace(t1.id_mark,chr(13),''),chr(10),'') as id_mark 
from ${iol_schema}.ncts_bt_acp_acceptmain t1 
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/ncts_bt_acp_acceptmain.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes