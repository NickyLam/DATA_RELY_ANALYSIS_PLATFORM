...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iof_ppps_u_corp_info_f
CreateDate: 20241011
FileName:   ${iel_data_path}/ppps_u_corp_info.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
from ${iol_schema}.ppps_u_corp_info t1
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/ppps_u_corp_info.f.${batch_date}.dat" \
        charset=utf8
        safe=yes
