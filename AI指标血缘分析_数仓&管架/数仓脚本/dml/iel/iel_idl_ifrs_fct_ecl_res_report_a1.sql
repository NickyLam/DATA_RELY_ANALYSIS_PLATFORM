...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_ifrs_fct_ecl_res_report_a1
CreateDate: 20250211
FileName:   ${iel_data_path}/ifrs_fct_ecl_res_report.i.${batch_date}.dat
IF_mark:    a
Logs:
' \
        query="select
...
where etl_dt between trunc(add_months(to_date('${batch_date}', 'yyyymmdd'), -1), 'mm') and trunc(to_date('${batch_date}', 'yyyymmdd'), 'mm') - 1; " \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/ifrs_fct_ecl_res_report.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
