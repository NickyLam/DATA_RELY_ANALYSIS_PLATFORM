...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_hdws_iml_evt_mpcs_event_dtl_i
CreateDate: 20180529
FileName:   ${iel_data_path}/hdws_iml_evt_mpcs_event_dtl.i.${batch_date}.dat
IF_mark:    i
Logs:
...
,NVL2(t1.data_src_cd,'EVT_MPCS_EVENT_DTL'||'_'||DECODE(T1.DATA_SRC_CD,'LHWD',UPPER(SUBSTR(T1.JOB_CD,1,4)),T1.DATA_SRC_CD),'EVT_MPCS_EVENT_DTL') as etl_task_name 
,t1.etl_dt as etl_dt
...
,replace(replace(t1.out_coa_id,chr(13),''),chr(10),'') as out_coa_id
,replace(replace(t1.red_blue_flg,chr(13),''),chr(10),'') as red_blue_flg
,replace(replace(t1.bank_id,chr(13),''),chr(10),'') as bank_id
,replace(replace(t1.syndc_id,chr(13),''),chr(10),'') as syndc_id
,t1.ghb_cntri_ratio as ghb_cntri_ratio
from ${idl_schema}.hdws_iml_evt_mpcs_event_dtl t1
where (etl_dt = to_date('${batch_date}','yyyymmdd')-1 and etl_task_name = 'EVT_MPCS_EVENT_DTL_01') OR (etl_dt = to_date('${batch_date}','yyyymmdd') and etl_task_name <> 'EVT_MPCS_EVENT_DTL_01');;" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/hdws_iml_evt_mpcs_event_dtl.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes