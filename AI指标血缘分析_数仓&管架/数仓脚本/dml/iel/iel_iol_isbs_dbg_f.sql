...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_isbs_dbg_f
CreateDate: 20180529
FileName:   ${iel_data_path}/isbs_dbg.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,replace(replace(t1.payattr,chr(13),''),chr(10),'') as payattr 
,t1.start_dt as start_dt 
,t1.end_dt as end_dt 
,replace(replace(t1.id_mark,chr(13),''),chr(10),'') as id_mark 
from ${iol_schema}.isbs_dbg t1 
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/isbs_dbg.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes