...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_evt_ups_agt_pay_flow_a
CreateDate: 20230804
FileName:   ${iel_data_path}/evt_ups_agt_pay_flow.a.${batch_date}.dat
IF_mark:    a
Logs:
' \
        query="select
...
,replace(replace(t1.exc_resv_bank_acct_id,chr(13),''),chr(10),'') as exc_resv_bank_acct_id
,replace(replace(t1.exc_resv_bank_acct_name,chr(13),''),chr(10),'') as exc_resv_bank_acct_name
,replace(replace(t1.payer_acct_org_id,chr(13),''),chr(10),'') as payer_acct_org_id
...
,replace(replace(t1.payer_rsrv_mobile_no,chr(13),''),chr(10),'') as payer_rsrv_mobile_no
...
from ${iml_schema}.evt_ups_agt_pay_flow t1
where etl_dt <= to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/evt_ups_agt_pay_flow.a.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
