...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_bdms_buy_details_f
CreateDate: 20180529
FileName:   ${iel_data_path}/bdms_buy_details.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,replace(replace(t.previous_hand,chr(13),''),chr(10),'') as previous_hand
,replace(replace(t.payment_date,chr(13),''),chr(10),'') as payment_date
,t.postpone_days as postpone_days
,t.payment_days as payment_days
...
,t.payment_maturity as payment_maturity
...
,t.payment_by_buyer as payment_by_buyer
,t.pay_amount as pay_amount
,t.risk_fees as risk_fees
,t.credit_line_id as credit_line_id
,t.credit_line as credit_line
,replace(replace(t.apply_date,chr(13),''),chr(10),'') as apply_date
,replace(replace(t.aoaccninf_actno,chr(13),''),chr(10),'') as aoaccninf_actno
,replace(replace(t.aoaccninf_ubank,chr(13),''),chr(10),'') as aoaccninf_ubank
...
,t.query_order as query_order
,replace(replace(t.query_content,chr(13),''),chr(10),'') as query_content
,replace(replace(t.query_type,chr(13),''),chr(10),'') as query_type
,replace(replace(t.check_result,chr(13),''),chr(10),'') as check_result
,replace(replace(t.check_content,chr(13),''),chr(10),'') as check_content
,t.query_check_id as query_check_id
,replace(replace(t.flaw_status,chr(13),''),chr(10),'') as flaw_status
...
,replace(replace(t.buy_status,chr(13),''),chr(10),'') as buy_status
...
from ${iol_schema}.bdms_buy_details t
where t.start_dt <= to_date('${batch_date}','yyyymmdd') and t.end_dt > to_date('${batch_date}','yyyymmdd');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/bdms_buy_details.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes