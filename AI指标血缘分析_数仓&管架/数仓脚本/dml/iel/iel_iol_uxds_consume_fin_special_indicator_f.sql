...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_uxds_consume_fin_special_indicator_f
CreateDate: 20251105
FileName:   ${iel_data_path}/uxds_consume_fin_special_indicator.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
from ${iol_schema}.uxds_consume_fin_special_indicator t1
where etl_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/uxds_consume_fin_special_indicator.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
