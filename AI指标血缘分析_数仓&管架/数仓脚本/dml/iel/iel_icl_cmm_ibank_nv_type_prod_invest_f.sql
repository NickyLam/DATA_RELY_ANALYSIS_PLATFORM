: '
Purpose:    unload config for sqluldr2
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_icl_cmm_ibank_nv_type_prod_invest_f
CreateDate: 20240425
FileName:   ${iel_data_path}/cmm_ibank_nv_type_prod_invest.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
to_date('${batch_date}','yyyymmdd') as etl_dt
,replace(replace(t1.lp_id,chr(13),''),chr(10),'') as lp_id
,replace(replace(t1.ext_secu_acct_id,chr(13),''),chr(10),'') as ext_secu_acct_id
,replace(replace(t1.intnal_secu_acct_id,chr(13),''),chr(10),'') as intnal_secu_acct_id
,replace(replace(t1.fin_instm_id,chr(13),''),chr(10),'') as fin_instm_id
,replace(replace(t1.asset_type_id,chr(13),''),chr(10),'') as asset_type_id
,replace(replace(t1.std_prod_id,chr(13),''),chr(10),'') as std_prod_id
,replace(replace(t1.market_type_id,chr(13),''),chr(10),'') as market_type_id
,replace(replace(t1.bus_id,chr(13),''),chr(10),'') as bus_id
,replace(replace(t1.comb_tran_num,chr(13),''),chr(10),'') as comb_tran_num
,replace(replace(t1.obj_id,chr(13),''),chr(10),'') as obj_id
,replace(replace(t1.crdt_fin_instm_id,chr(13),''),chr(10),'') as crdt_fin_instm_id
,replace(replace(t1.asset_uniq_idf_id,chr(13),''),chr(10),'') as asset_uniq_idf_id
,replace(replace(t1.prod_type_cd,chr(13),''),chr(10),'') as prod_type_cd
,replace(replace(t1.asset_type_name,chr(13),''),chr(10),'') as asset_type_name
,replace(replace(t1.class_crdt_flg,chr(13),''),chr(10),'') as class_crdt_flg
,replace(replace(t1.abs_flg,chr(13),''),chr(10),'') as abs_flg
,replace(replace(t1.acct_name,chr(13),''),chr(10),'') as acct_name
,replace(replace(t1.subj_id,chr(13),''),chr(10),'') as subj_id
,replace(replace(t1.int_subj_id,chr(13),''),chr(10),'') as int_subj_id
,replace(replace(t1.recvbl_uncol_int_subj_id,chr(13),''),chr(10),'') as recvbl_uncol_int_subj_id
,replace(replace(t1.int_adj_subj_id,chr(13),''),chr(10),'') as int_adj_subj_id
,replace(replace(t1.tran_market_id,chr(13),''),chr(10),'') as tran_market_id
,replace(replace(t1.exchg_acct_id,chr(13),''),chr(10),'') as exchg_acct_id
,replace(replace(t1.cntpty_cls_descb,chr(13),''),chr(10),'') as cntpty_cls_descb
,replace(replace(t1.bank_flg,chr(13),''),chr(10),'') as bank_flg
,replace(replace(t1.cty_cd,chr(13),''),chr(10),'') as cty_cd
,value_dt
,exp_dt
,subscr_dt
,replace(replace(t1.tenor_cd,chr(13),''),chr(10),'') as tenor_cd
,replace(replace(t1.int_accr_base_cd,chr(13),''),chr(10),'') as int_accr_base_cd
,replace(replace(t1.int_rat_adj_way_cd,chr(13),''),chr(10),'') as int_rat_adj_way_cd
,replace(replace(t1.curr_cd,chr(13),''),chr(10),'') as curr_cd
,fac_val_amt
,fac_val_int_rat
,replace(replace(t1.pay_int_ped_cd,chr(13),''),chr(10),'') as pay_int_ped_cd
,replace(replace(t1.auto_redt_flg,chr(13),''),chr(10),'') as auto_redt_flg
,actl_qtty
,actl_bal
,pric_bal
,currt_bal
,acru_int
,int_recvbl
,recvbl_uncol_pric
,recvbl_uncol_int
,int_adj_amt
,evha_val_chag
,replace(replace(t1.nv_prod_flg,chr(13),''),chr(10),'') as nv_prod_flg
,base_rat
,spd
,base_rat_mult
,td_nv
,book_bal
,curr_bal
,last_update_dt
,replace(replace(t1.cap_type_cd,chr(13),''),chr(10),'') as cap_type_cd
,replace(replace(t1.asset_four_cls_cd,chr(13),''),chr(10),'') as asset_four_cls_cd
,replace(replace(t1.asset_thd_cls_cd,chr(13),''),chr(10),'') as asset_thd_cls_cd
,replace(replace(t1.belong_org_id,chr(13),''),chr(10),'') as belong_org_id
,replace(replace(t1.uder_asset_dir_indus_categy_cd,chr(13),''),chr(10),'') as uder_asset_dir_indus_categy_cd
,replace(replace(t1.uder_bond_cd,chr(13),''),chr(10),'') as uder_bond_cd
,replace(replace(t1.uder_bond_name,chr(13),''),chr(10),'') as uder_bond_name
,replace(replace(t1.uder_bond_flg,chr(13),''),chr(10),'') as uder_bond_flg
,replace(replace(t1.uder_asset_type_id,chr(13),''),chr(10),'') as uder_asset_type_id
,replace(replace(t1.uder_bond_rating,chr(13),''),chr(10),'') as uder_bond_rating
,replace(replace(t1.uder_actl_finer_name,chr(13),''),chr(10),'') as uder_actl_finer_name
,uder_post_denom
,replace(replace(t1.uder_actl_finer_cust_id,chr(13),''),chr(10),'') as uder_actl_finer_cust_id
,replace(replace(t1.uder_actl_finer_belong_group,chr(13),''),chr(10),'') as uder_actl_finer_belong_group
,replace(replace(t1.uder_actl_finer_cust_char,chr(13),''),chr(10),'') as uder_actl_finer_cust_char
,replace(replace(t1.uder_coll_way_cd,chr(13),''),chr(10),'') as uder_coll_way_cd
,uder_cbond_estim_full_price
,uder_cbond_estim_net_price
,uder_csecu_full_price_evltion
,uder_csecu_net_price_evltion
,uder_csecu_coret_duran
,uder_csecu_bp_val
,uder_csecu_estim_cvty
,uder_estim_coret_duran
,uder_bp_val
,uder_estim_cvty
,replace(replace(t1.final_dir_type_cd,chr(13),''),chr(10),'') as final_dir_type_cd
,replace(replace(t1.final_dir_indus_gen,chr(13),''),chr(10),'') as final_dir_indus_gen
,replace(replace(t1.final_dir_indus_subclass,chr(13),''),chr(10),'') as final_dir_indus_subclass
,dir_ind_fund_part
,dir_debt_eqty_part
,dir_pe_part
,dir_pam_prod_part
,replace(replace(t1.extra_dimen_cd,chr(13),''),chr(10),'') as extra_dimen_cd
,stl_dt
,replace(replace(t1.ovdue_status,chr(13),''),chr(10),'') as ovdue_status
,replace(replace(t1.in_out_tab_flg_cd,chr(13),''),chr(10),'') as in_out_tab_flg_cd
,replace(replace(t1.ovdue_flg,chr(13),''),chr(10),'') as ovdue_flg
,replace(replace(t1.pric_ovdue_flg,chr(13),''),chr(10),'') as pric_ovdue_flg
,pric_ovdue_dt
,pric_ovdue_days
,replace(replace(t1.int_ovdue_flg,chr(13),''),chr(10),'') as int_ovdue_flg
,int_ovdue_dt
,int_ovdue_days
,replace(replace(t1.mger_id,chr(13),''),chr(10),'') as mger_id
,replace(replace(t1.mger_name,chr(13),''),chr(10),'') as mger_name
,replace(replace(t1.trustee_id,chr(13),''),chr(10),'') as trustee_id
,replace(replace(t1.trustee_name,chr(13),''),chr(10),'') as trustee_name

from ${icl_schema}.cmm_ibank_nv_type_prod_invest t1
where etl_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/cmm_ibank_nv_type_prod_invest.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
