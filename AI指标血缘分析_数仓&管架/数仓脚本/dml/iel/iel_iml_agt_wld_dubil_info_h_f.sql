...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_agt_wld_dubil_info_h_f
CreateDate: 20230608
FileName:   ${iel_data_path}/agt_wld_dubil_info_h.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
where start_dt <= to_date('${batch_date}','yyyymmdd')-1 and end_dt > to_date('${batch_date}','yyyymmdd')-1" \
        ...
        file="${iel_data_path}/agt_wld_dubil_info_h.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
