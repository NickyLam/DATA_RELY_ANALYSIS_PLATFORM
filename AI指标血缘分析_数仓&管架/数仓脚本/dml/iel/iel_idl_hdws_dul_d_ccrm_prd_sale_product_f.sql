...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_hdws_dul_d_ccrm_prd_sale_product_f
CreateDate: 20180529
FileName:   ${iel_data_path}/hdws_dul_d_ccrm_prd_sale_product.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,NVL2(t1.data_src_cd,'PRD_SALE_PRODUCT'||'_'||DECODE(T1.DATA_SRC_CD,'LHWD',UPPER(SUBSTR(T1.JOB_CD,1,4)),T1.DATA_SRC_CD),'PRD_SALE_PRODUCT') as etl_task_name
from ${idl_schema}.hdws_dul_d_ccrm_prd_sale_product t1
where etl_dt = to_date('${batch_date}','yyyymmdd');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/hdws_dul_d_ccrm_prd_sale_product.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes