...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_bdms_view_old_business_volume_f
CreateDate: 20250828
FileName:   ${iel_data_path}/bdms_view_old_business_volume.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
from ${iol_schema}.bdms_view_old_business_volume t1
where etl_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/bdms_view_old_business_volume.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
