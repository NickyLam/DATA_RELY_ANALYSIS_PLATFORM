...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_imf_evt_cust_wrtoff_tran_f
CreateDate: 20240829
FileName:   ${iel_data_path}/evt_cust_wrtoff_tran.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.wrtoff_seq_num,chr(13),''),chr(10),'') as wrtoff_seq_num
...
from ${iml_schema}.evt_cust_wrtoff_tran t1
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/evt_cust_wrtoff_tran.f.${batch_date}.dat" \
        charset=utf8
        safe=yes
