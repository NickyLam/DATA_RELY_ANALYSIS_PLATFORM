...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_icms_mybk_cs_extent_info_f
CreateDate: 20221114
FileName:   ${iel_data_path}/icms_mybk_cs_extent_info.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
from ${iol_schema}.icms_mybk_cs_extent_info t1
where etl_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/icms_mybk_cs_extent_info.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
