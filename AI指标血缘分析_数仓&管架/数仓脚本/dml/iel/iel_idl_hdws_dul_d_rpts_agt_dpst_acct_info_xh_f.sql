...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_hdws_dul_d_rpts_agt_dpst_acct_info_xh_f
CreateDate: 20180529
FileName:   ${iel_data_path}/hdws_dul_d_rpts_agt_dpst_acct_info_xh.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,NVL2(t1.data_src_cd,'D_RPTS_AGT_DPST_ACCT_INFO_XH'||'_'||DECODE(T1.DATA_SRC_CD,'LHWD',UPPER(SUBSTR(T1.JOB_CD,1,4)),T1.DATA_SRC_CD),'D_RPTS_AGT_DPST_ACCT_INFO_XH') as etl_task_name
,replace(replace(t1.sub_num,chr(13),''),chr(10),'') as sub_num
from ${idl_schema}.hdws_dul_d_rpts_agt_dpst_acct_info_xh t1
where etl_dt = to_date('${batch_date}','yyyymmdd');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/hdws_dul_d_rpts_agt_dpst_acct_info_xh.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes