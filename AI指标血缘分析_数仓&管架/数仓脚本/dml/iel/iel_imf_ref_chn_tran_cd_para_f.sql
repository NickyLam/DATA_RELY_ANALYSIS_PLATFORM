...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_imf_ref_chn_tran_cd_para_f
CreateDate: 20230525
FileName:   ${iel_data_path}/ref_chn_tran_cd_para.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.bank_int_proc_cd,chr(13),''),chr(10),'') as bank_int_proc_cd
,replace(replace(t1.obank_proc_cd,chr(13),''),chr(10),'') as obank_proc_cd
...
from ${iml_schema}.ref_chn_tran_cd_para t1
where create_dt <= to_date('${batch_date}','yyyymmdd') and id_mark<>'D'" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/ref_chn_tran_cd_para.f.${batch_date}.dat" \
        charset=utf8
        safe=yes
