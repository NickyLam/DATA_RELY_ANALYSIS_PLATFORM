...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_alss_ams_blacklist_detail_f
CreateDate: 20231103
FileName:   ${iel_data_path}/alss_ams_blacklist_detail.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.bla_iden_type,chr(13),''),chr(10),'') as bla_iden_type
...
from ${iol_schema}.alss_ams_blacklist_detail t1
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/alss_ams_blacklist_detail.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
