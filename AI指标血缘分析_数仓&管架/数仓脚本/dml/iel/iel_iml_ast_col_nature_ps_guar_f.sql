...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_ast_col_nature_ps_guar_f
CreateDate: 20221021
FileName:   ${iel_data_path}/ast_col_nature_ps_guar.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(guartor_cert_no,chr(13),''),chr(10),'')
,replace(replace(guartor_cert_type_cd,chr(13),''),chr(10),'')
...
from ${iml_schema}.ast_col_nature_ps_guar t1
where 1=1" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/ast_col_nature_ps_guar.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
