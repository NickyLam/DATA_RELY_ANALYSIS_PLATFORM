...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_noas_role_type_f
CreateDate: 20180529
FileName:   ${iel_data_path}/noas_role_type.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,replace(replace(t1.role_attribute,chr(13),''),chr(10),'') as role_attribute 
,t1.start_dt as start_dt 
,t1.end_dt as end_dt 
,replace(replace(t1.id_mark,chr(13),''),chr(10),'') as id_mark 
from ${iol_schema}.noas_role_type t1 
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/noas_role_type.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes