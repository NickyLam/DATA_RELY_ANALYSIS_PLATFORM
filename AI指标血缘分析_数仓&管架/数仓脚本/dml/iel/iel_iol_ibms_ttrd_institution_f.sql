...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_ibms_ttrd_institution_f
CreateDate: 20221013
FileName:   ${iel_data_path}/ibms_ttrd_institution.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,t1.rwa_code as rwa_code
,replace(replace(t1.rwa_name,chr(13),''),chr(10),'') as rwa_name
,replace(replace(t1.spv_manager,chr(13),''),chr(10),'') as spv_manager
...
,replace(replace(t1.i_level_m,chr(13),''),chr(10),'') as i_level_m

from ${iol_schema}.ibms_ttrd_institution t1
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/ibms_ttrd_institution.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
