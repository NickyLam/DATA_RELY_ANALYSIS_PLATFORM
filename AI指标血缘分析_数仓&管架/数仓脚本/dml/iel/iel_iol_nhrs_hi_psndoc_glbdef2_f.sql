...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_nhrs_hi_psndoc_glbdef2_f
CreateDate: 20240205
FileName:   ${iel_data_path}/nhrs_hi_psndoc_glbdef2.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
from ${iol_schema}.nhrs_hi_psndoc_glbdef2 t1
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/nhrs_hi_psndoc_glbdef2.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
