...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_rpt_rcrs_lhdk_intercept_result_i
CreateDate: 20180529
FileName:   ${iel_data_path}/rpt_rcrs_lhdk_intercept_result.i.${batch_date}.dat
IF_mark:    i
Logs:
...
,replace(replace(t1.rule_type,chr(13),''),chr(10),'') as rule_type
 from iol.rcrs_lhdk_intercept_result T1
where substr(replace(time,'-',''),1,8)='${batch_date}' and start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd');" \
        ...
        file="${iel_data_path}/rpt_rcrs_lhdk_intercept_result.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes