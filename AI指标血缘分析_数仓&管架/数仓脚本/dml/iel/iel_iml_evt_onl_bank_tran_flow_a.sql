...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_evt_onl_bank_tran_flow_a
CreateDate: 20180529
FileName:   ${iel_data_path}/evt_onl_bank_tran_flow.a.${batch_date}.dat
IF_mark:    a
Logs:
   zjj 2018-07-27 create template
' \
        query="select
to_date(substr(t.tran_dt,1,8),'yyyymmdd') as etl_dt
,replace(replace(t.evt_id,chr(13),''),chr(10),'') as evt_id
,replace(replace(t.lp_id,chr(13),''),chr(10),'') as lp_id
,replace(replace(t.flow_num,chr(13),''),chr(10),'') as flow_num
,replace(replace(t.tran_dt,chr(13),''),chr(10),'') as tran_dt
,replace(replace(t.tran_tm,chr(13),''),chr(10),'') as tran_tm
,replace(replace(t.tran_code,chr(13),''),chr(10),'') as tran_code
,replace(replace(t.onl_bank_tran_status_cd,chr(13),''),chr(10),'') as onl_bank_tran_status_cd
...
,replace(replace(t.splt_flow_num,chr(13),''),chr(10),'') as splt_flow_num
...
from iml.evt_onl_bank_tran_flow t
where substr(t.tran_dt,1,8) <= '${batch_date}' and substr(t.tran_dt,1,8) >= '20200101' " \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/evt_onl_bank_tran_flow.a.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes