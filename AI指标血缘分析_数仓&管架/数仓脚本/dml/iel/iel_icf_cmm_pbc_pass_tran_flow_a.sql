...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_icf_cmm_pbc_pass_tran_flow_a
CreateDate: 20250619
FileName:   ${iel_data_path}/cmm_pbc_pass_tran_flow.a.${batch_date}.dat
IF_mark:    a
Logs:
' \
        query="select
...
,revid_tm
,replace(replace(t1.pbc_pass_tran_type_cd,chr(13),''),chr(10),'') as pbc_pass_tran_type_cd

from ${icl_schema}.cmm_pbc_pass_tran_flow t1
where t1.tran_dt <= to_date('${batch_date}','yyyymmdd') " \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/cmm_pbc_pass_tran_flow.a.${batch_date}.dat" \
        charset=utf8
        safe=yes
