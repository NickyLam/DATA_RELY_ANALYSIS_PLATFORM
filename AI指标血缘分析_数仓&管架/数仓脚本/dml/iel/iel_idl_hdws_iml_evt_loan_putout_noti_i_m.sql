...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_hdws_iml_evt_loan_putout_noti_i_m
CreateDate: 20180529
FileName:   ${iel_data_path}/hdws_iml_evt_loan_putout_noti_m.i.${batch_date}.dat
IF_mark:    i_m
Logs:
...
from ${idl_schema}.hdws_iml_evt_loan_putout_noti t1
where etl_dt <= to_date('${batch_date}','yyyymmdd') and etl_dt >= trunc(to_date('${batch_date}','yyyymmdd'),'mm');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/hdws_iml_evt_loan_putout_noti_m.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes