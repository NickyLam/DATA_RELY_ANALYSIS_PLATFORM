...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_pams_jxbb_ckftpmx_gh_i
CreateDate: 20260415
FileName:   ${iel_data_path}/pams_jxbb_ckftpmx_gh.i.${batch_date}.dat
IF_mark:    i
Logs:
' \
        query="select
...
,txfpbl
,replace(replace(t1.shlllx,chr(13),''),chr(10),'') as shlllx
,ydshrq
,replace(replace(t1.tscpbz,chr(13),''),chr(10),'') as tscpbz
,xhczhll
,shll

from ${iol_schema}.pams_jxbb_ckftpmx_gh t1
where etl_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/pams_jxbb_ckftpmx_gh.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
