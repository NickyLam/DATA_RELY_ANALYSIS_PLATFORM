...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_hdws_iml_agt_loan_actl_repay_i_m
CreateDate: 20180529
FileName:   ${iel_data_path}/hdws_iml_agt_loan_actl_repay_m.i.${batch_date}.dat
IF_mark:    i_m
Logs:
...
where (etl_dt <= to_date('${batch_date}','yyyymmdd')-1 and etl_dt >= add_months(to_date('${batch_date}','yyyymmdd'),-1) and data_src_cd = 'LHWD') OR (etl_dt >= trunc(to_date('${batch_date}','yyyymmdd'),'mm') and etl_dt <= to_date('${batch_date}','yyyymmdd') and data_src_cd <> 'LHWD');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/hdws_iml_agt_loan_actl_repay_m.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes