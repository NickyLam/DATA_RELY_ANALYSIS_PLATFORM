...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_ilss_ghb_nsr_qykjzdlrxx_f
CreateDate: 20180529
FileName:   ${iel_data_path}/ilss_ghb_nsr_qykjzdlrxx.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,replace(replace(t.bys,chr(13),''),chr(10),'') as bys
,replace(replace(t.bnljs,chr(13),''),chr(10),'') as bnljs
...
from iol.ilss_ghb_nsr_qykjzdlrxx t
where t.etl_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/ilss_ghb_nsr_qykjzdlrxx.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes