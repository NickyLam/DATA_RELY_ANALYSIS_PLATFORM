: '
Purpose:    unload config for sqluldr2
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_evt_finc_tran_cfm_dtl_i_w
CreateDate: 20180529
FileName:   ${iel_data_path}/evt_finc_tran_cfm_dtl_w.i.${batch_date}.dat
IF_mark:    i_w
Logs:
   zjj 2018-07-27 create template
' \
        query="select 
to_date('${batch_date}','yyyymmdd') as etl_dt 
,replace(replace(t.evt_id,chr(13),''),chr(10),'') as evt_id
,replace(replace(t.lp_id,chr(13),''),chr(10),'') as lp_id
,replace(replace(t.dtl_cfm_flow_num,chr(13),''),chr(10),'') as dtl_cfm_flow_num
,replace(replace(t.ta_cfm_flow_num,chr(13),''),chr(10),'') as ta_cfm_flow_num
,replace(replace(t.seller_id,chr(13),''),chr(10),'') as seller_id
,replace(replace(t.bus_cd,chr(13),''),chr(10),'') as bus_cd
,t.cfm_dt as cfm_dt
,replace(replace(t.finc_acct_id,chr(13),''),chr(10),'') as finc_acct_id
,replace(replace(t.prod_id,chr(13),''),chr(10),'') as prod_id
,t.appl_dt as appl_dt
,replace(replace(t.appl_flow_num,chr(13),''),chr(10),'') as appl_flow_num
,t.init_lot_cfm_dt as init_lot_cfm_dt
,replace(replace(t.init_lot_cfm_flow_num,chr(13),''),chr(10),'') as init_lot_cfm_flow_num
,t.cfm_amt as cfm_amt
,t.cfm_lot as cfm_lot
,t.invest_prft as invest_prft
from ${iml_schema}.evt_finc_tran_cfm_dtl t
where etl_dt <= to_date('${batch_date}','yyyymmdd') and etl_dt >= to_date('${batch_date}','yyyymmdd') -6 ;" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/evt_finc_tran_cfm_dtl_w.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes