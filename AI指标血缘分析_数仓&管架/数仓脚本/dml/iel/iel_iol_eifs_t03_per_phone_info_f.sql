...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_eifs_t03_per_phone_info_f
CreateDate: 20240328
FileName:   ${iel_data_path}/eifs_t03_per_phone_info.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
from ${iol_schema}.eifs_t03_per_phone_info t1
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/eifs_t03_per_phone_info.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
