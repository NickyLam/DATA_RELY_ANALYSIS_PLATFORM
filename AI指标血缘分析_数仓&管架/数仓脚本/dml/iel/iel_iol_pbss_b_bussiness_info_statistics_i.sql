...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_pbss_b_bussiness_info_statistics_i
CreateDate: 20180529
FileName:   ${iel_data_path}/pbss_b_bussiness_info_statistics.i.${batch_date}.dat
IF_mark:    i
Logs:
   zjj 2018-07-27 create template
' \
        query="select to_date('${batch_date}','yyyymmdd') as etl_dt
      ,replace(replace(id,chr(13),''),chr(10),'') as id
      ,txn_dt
      ...
      ,end_dt
      ,id_mark
  from ${iol_schema}.pbss_b_bussiness_info_statistics t1 where start_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/pbss_b_bussiness_info_statistics.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes