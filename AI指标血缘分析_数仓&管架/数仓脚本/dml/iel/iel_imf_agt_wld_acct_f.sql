...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_imf_agt_wld_acct_f
CreateDate: 20230602
FileName:   ${iel_data_path}/agt_wld_acct.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.ext_cust_id,chr(13),''),chr(10),'') as ext_cust_id
,create_dt
,update_dt

from ${iml_schema}.agt_wld_acct t1
where create_dt <= to_date('${batch_date}','yyyymmdd')-1 and id_mark<>'D'" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/agt_wld_acct.f.${batch_date}.dat" \
        charset=utf8
        safe=yes
