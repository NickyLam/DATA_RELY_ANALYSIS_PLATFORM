...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_imf_ast_col_resd_build_info_f
CreateDate: 20251010
FileName:   ${iel_data_path}/ast_col_resd_build_info.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.curt_house_flg,chr(13),''),chr(10),'') as curt_house_flg
,replace(replace(t1.presell_lics_id,chr(13),''),chr(10),'') as presell_lics_id
,expect_dlvy_tm
,expect_rel_esat_wat_tm
...
,buy_dt
,buy_amt
,replace(replace(t1.sell_recd_flg,chr(13),''),chr(10),'') as sell_recd_flg
,replace(replace(t1.purch_estate_flg,chr(13),''),chr(10),'') as purch_estate_flg
,replace(replace(t1.uniq_housing_flg,chr(13),''),chr(10),'') as uniq_housing_flg
,arch_area
,usbl_area
,replace(replace(t1.build_year,chr(13),''),chr(10),'') as build_year
,prop_tenor
,build_age
,replace(replace(t1.aptmt_cd,chr(13),''),chr(10),'') as aptmt_cd
...
from ${iml_schema}.ast_col_resd_build_info t1
where create_dt <= to_date('${batch_date}','yyyymmdd') and id_mark<>'D'" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/ast_col_resd_build_info.f.${batch_date}.dat" \
        charset=utf8
        safe=yes
