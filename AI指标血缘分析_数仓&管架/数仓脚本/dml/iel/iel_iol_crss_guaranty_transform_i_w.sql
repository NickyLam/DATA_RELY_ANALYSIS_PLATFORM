: '
Purpose:    unload config for sqluldr2
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_crss_guaranty_transform_i_w
CreateDate: 20180529
FileName:   ${iel_data_path}/crss_guaranty_transform_w.i.${batch_date}.dat
IF_mark:    i_w
Logs:
   zjj 2018-07-27 create template
' \
        query="select replace(replace(serialno, chr(10), ''), chr(13), '') as serialno,
       replace(replace(relativeserialno, chr(10), ''), chr(13), '') as relativeserialno,
       replace(replace(objecttype, chr(10), ''), chr(13), '') as objecttype,
       replace(replace(artificialno, chr(10), ''), chr(13), '') as artificialno,
       replace(replace(customerid, chr(10), ''), chr(13), '') as customerid,
       replace(replace(customername, chr(10), ''), chr(13), '') as customername,
       replace(replace(occurtype, chr(10), ''), chr(13), '') as occurtype,
       replace(replace(businesstype, chr(10), ''), chr(13), '') as businesstype,
       replace(replace(businesscurrency, chr(10), ''), chr(13), '') as businesscurrency,
       replace(replace(businesssum, chr(10), ''), chr(13), '') as businesssum,
       replace(replace(balance, chr(10), ''), chr(13), '') as balance,
       replace(replace(transformreason, chr(10), ''), chr(13), '') as transformreason,
       replace(replace(manageorgid, chr(10), ''), chr(13), '') as manageorgid,
       replace(replace(manageuserid, chr(10), ''), chr(13), '') as manageuserid,
       replace(replace(inputdate, chr(10), ''), chr(13), '') as inputdate,
       replace(replace(updatedate, chr(10), ''), chr(13), '') as updatedate,
       replace(replace(pigeonholedate, chr(10), ''), chr(13), '') as pigeonholedate,
       start_dt,
       end_dt,
       id_mark,
       etl_timestamp
  from iol.crss_guaranty_transform
 where start_dt between to_date('${batch_date}', 'yyyymmdd') - 6 and to_date('${batch_date}', 'yyyymmdd');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/crss_guaranty_transform_w.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes