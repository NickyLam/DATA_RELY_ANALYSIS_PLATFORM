...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_hdws_iml_agt_loan_ctr_info_h_f
CreateDate: 20180529
FileName:   ${iel_data_path}/hdws_iml_agt_loan_ctr_info_h.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,NVL2(t1.data_src_cd,'AGT_LOAN_CTR_H'||'_'||DECODE(T1.DATA_SRC_CD,'LHWD',UPPER(SUBSTR(T1.JOB_CD,1,4)),T1.DATA_SRC_CD),'AGT_LOAN_CTR_H') as etl_task_name 
...
,replace(replace(t1.cntrpty_cust_nbr,chr(13),''),chr(10),'') as cntrpty_cust_nbr
,replace(replace(t1.cntrpty_pty_name,chr(13),''),chr(10),'') as cntrpty_pty_name
,replace(replace(t1.invt_mode_cd,chr(13),''),chr(10),'') as invt_mode_cd
,replace(replace(t1.actl_fin_pty_id,chr(13),''),chr(10),'') as actl_fin_pty_id
,replace(replace(t1.actl_fin_pty_name,chr(13),''),chr(10),'') as actl_fin_pty_name
,replace(replace(t1.txn_ast_name,chr(13),''),chr(10),'') as txn_ast_name
,replace(replace(t1.cnsm_srv_type_fin_flg,chr(13),''),chr(10),'') as cnsm_srv_type_fin_flg
,replace(replace(t1.dir_indu_fund_flg,chr(13),''),chr(10),'') as dir_indu_fund_flg
,replace(replace(t1.first_house_flg,chr(13),''),chr(10),'') as first_house_flg
,replace(replace(t1.circ_flg,chr(13),''),chr(10),'') as circ_flg
from ${idl_schema}.hdws_iml_agt_loan_ctr_info t1
where del_flg <> '1' and ((etl_dt = to_date('${batch_date}','yyyymmdd')-1 and data_src_cd = 'LHWD') OR (etl_dt = to_date('${batch_date}','yyyymmdd') and data_src_cd <> 'LHWD'));" \
        ...
        file="${iel_data_path}/hdws_iml_agt_loan_ctr_info_h.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes