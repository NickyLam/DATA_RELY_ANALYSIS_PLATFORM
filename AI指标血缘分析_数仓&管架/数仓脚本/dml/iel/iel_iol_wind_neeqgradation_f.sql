...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_wind_neeqgradation_f
CreateDate: 20180529
FileName:   ${iel_data_path}/wind_neeqgradation.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,replace(replace(t.wind_sec_code,chr(13),''),chr(10),'') as wind_sec_code
...
from iol.wind_neeqgradation t
where start_dt <=to_date('${batch_date}','yyyymmdd') and end_dt >to_date('${batch_date}','yyyymmdd') ;
" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/wind_neeqgradation.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes