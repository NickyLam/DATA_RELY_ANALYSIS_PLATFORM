...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_evt_ups_pay_flow_i
CreateDate: 20230810
FileName:   ${iel_data_path}/evt_ups_pay_flow.i.${batch_date}.dat
IF_mark:    i
Logs:
' \
        query="select
...
from ${iml_schema}.evt_ups_pay_flow t1
where etl_dt <= to_date('${batch_date}','yyyymmdd') and etl_dt > to_date('${batch_date}','yyyymmdd')-15" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/evt_ups_pay_flow.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
