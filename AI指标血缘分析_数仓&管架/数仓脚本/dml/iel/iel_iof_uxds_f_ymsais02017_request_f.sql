...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iof_uxds_f_ymsais02017_request_f
CreateDate: 20250318
FileName:   ${iel_data_path}/uxds_f_ymsais02017_request.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.mobnetcode,chr(13),''),chr(10),'') as mobnetcode
,replace(replace(t1.authorizationnum,chr(13),''),chr(10),'') as authorizationnum
,replace(replace(t1.genmonth,chr(13),''),chr(10),'') as genmonth
,replace(replace(t1.msgid,chr(13),''),chr(10),'') as msgid

from ${iol_schema}.uxds_f_ymsais02017_request t1
where etl_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/uxds_f_ymsais02017_request.f.${batch_date}.dat" \
        charset=utf8
        safe=yes
