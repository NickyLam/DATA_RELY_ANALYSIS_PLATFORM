...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_ilss_ghb_nsr_bgdjxx_f
CreateDate: 20180529
FileName:   ${iel_data_path}/ilss_ghb_nsr_bgdjxx.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,replace(replace(t.bgdjmxuuid,chr(13),''),chr(10),'') as bgdjmxuuid
,replace(replace(t.djxh,chr(13),''),chr(10),'') as djxh
,replace(replace(t.bgxm,chr(13),''),chr(10),'') as bgxm
,replace(replace(t.bgxm_dm,chr(13),''),chr(10),'') as bgxm_dm
...
from iol.ilss_ghb_nsr_bgdjxx t
where t.etl_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/ilss_ghb_nsr_bgdjxx.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes