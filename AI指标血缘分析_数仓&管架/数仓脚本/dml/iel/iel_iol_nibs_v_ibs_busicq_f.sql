...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_nibs_v_ibs_busicq_f
CreateDate: 20251112
FileName:   ${iel_data_path}/nibs_v_ibs_busicq.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.queuegettime,chr(13),''),chr(10),'') as queuegettime
,replace(replace(t1.queuecalltime,chr(13),''),chr(10),'') as queuecalltime
,transtarttime
,tranendtime
...
where etl_dt = to_date('${batch_date}','yyyymmdd') and channeldate = to_date('${batch_date}','yyyymmdd');" \
        ...
        file="${iel_data_path}/nibs_v_ibs_busicq.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
