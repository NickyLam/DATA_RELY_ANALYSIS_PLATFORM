...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_agt_bill_discount_dtl_f
CreateDate: 20221021
FileName:   ${iel_data_path}/agt_bill_discount_dtl.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(discount_dtl_id,chr(13),''),chr(10),'')
...
,replace(replace(crdt_main_type_cd,chr(13),''),chr(10),'')
,replace(replace(crdt_main_id,chr(13),''),chr(10),'')
,bill_intrv_std_amt
...
from ${iml_schema}.agt_bill_discount_dtl t1
where 1=1" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/agt_bill_discount_dtl.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
