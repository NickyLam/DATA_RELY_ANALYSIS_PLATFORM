...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_evt_onl_bank_tran_flow_i_w
CreateDate: 20180529
FileName:   ${iel_data_path}/evt_onl_bank_tran_flow_w.i.${batch_date}.dat
IF_mark:    i_w
Logs:
...
from ${iml_schema}.evt_onl_bank_tran_flow t
where substr(tran_dt,1,8)  <= ${batch_date} and substr(tran_dt,1,8) >= ${batch_date} -6  ;" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/evt_onl_bank_tran_flow_w.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes