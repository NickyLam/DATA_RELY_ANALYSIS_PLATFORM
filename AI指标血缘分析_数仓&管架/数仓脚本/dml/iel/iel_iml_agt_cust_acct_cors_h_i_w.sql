...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_agt_cust_acct_cors_h_i_w
CreateDate: 20180529
FileName:   ${iel_data_path}/agt_cust_acct_cors_h_w.i.${batch_date}.dat
IF_mark:    i_w
Logs:
...
where (start_dt <= to_date('${batch_date}','yyyymmdd') and  start_dt >= to_date('${batch_date}','yyyymmdd') -6)  or (end_dt <= to_date('${batch_date}','yyyymmdd') and  end_dt >= to_date('${batch_date}','yyyymmdd') -6) ;" \
        ...
        file="${iel_data_path}/agt_cust_acct_cors_h_w.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes