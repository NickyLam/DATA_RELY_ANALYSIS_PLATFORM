...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idf_aml_evt_mercht_clear_batch_dtl_i
CreateDate: 20240130
FileName:   ${iel_data_path}/aml_evt_mercht_clear_batch_dtl.i.${batch_date}.dat
IF_mark:    i
Logs:
' \
        query="select
...
where etl_dt <= to_date('${batch_date}','yyyymmdd')-1 and etl_dt > to_date('${batch_date}','yyyymmdd')-16" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/aml_evt_mercht_clear_batch_dtl.i.${batch_date}.dat" \
        charset=utf8
        safe=yes
