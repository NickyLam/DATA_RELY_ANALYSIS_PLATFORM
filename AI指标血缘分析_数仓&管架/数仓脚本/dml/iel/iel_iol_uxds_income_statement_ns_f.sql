...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_uxds_income_statement_ns_f
CreateDate: 20250605
FileName:   ${iel_data_path}/uxds_income_statement_ns.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
from ${iol_schema}.uxds_income_statement_ns t1
where etl_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/uxds_income_statement_ns.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
