...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_icms_tmp_zjbk_inbaseinfo_f
CreateDate: 20250107
FileName:   ${iel_data_path}/icms_tmp_zjbk_inbaseinfo.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.idnm,chr(13),''),chr(10),'') as idnm
...
from ${iol_schema}.icms_tmp_zjbk_inbaseinfo t1
where etl_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/icms_tmp_zjbk_inbaseinfo.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
