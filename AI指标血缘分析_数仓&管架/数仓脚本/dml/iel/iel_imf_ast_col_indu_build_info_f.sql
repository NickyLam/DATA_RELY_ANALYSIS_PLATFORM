...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_imf_ast_col_indu_build_info_f
CreateDate: 20251010
FileName:   ${iel_data_path}/ast_col_indu_build_info.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.dev_mode_cd,chr(13),''),chr(10),'') as dev_mode_cd
...
,replace(replace(t1.dplat_num,chr(13),''),chr(10),'') as dplat_num
...
from ${iml_schema}.ast_col_indu_build_info t1
where create_dt <= to_date('${batch_date}','yyyymmdd') and id_mark<>'D'" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/ast_col_indu_build_info.f.${batch_date}.dat" \
        charset=utf8
        safe=yes
