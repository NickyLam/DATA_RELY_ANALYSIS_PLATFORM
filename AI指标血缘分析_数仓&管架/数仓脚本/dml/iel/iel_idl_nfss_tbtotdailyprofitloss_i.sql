...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_nfss_tbtotdailyprofitloss_i
CreateDate: 20180529
FileName:   ${iel_data_path}/nfss_tbtotdailyprofitloss.i.${batch_date}.dat
IF_mark:    i
Logs:
...
where record_date>=to_char(to_date('${batch_date}','yyyymmdd')-15,'yyyymmdd') and record_date<=to_char(to_date('${batch_date}','yyyymmdd')-1,'yyyymmdd');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/nfss_tbtotdailyprofitloss.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes