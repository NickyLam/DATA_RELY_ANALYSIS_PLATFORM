...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_ccrm_wind_neeqgradation_f
CreateDate: 20180529
FileName:   ${iel_data_path}/ccrm_wind_neeqgradation.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,replace(replace(t1.wind_sec_code,chr(13),''),chr(10),'') as wind_sec_code
...
from ${iol_schema}.wind_neeqgradation t1
where start_dt <=to_date('${batch_date}','yyyymmdd') and end_dt >to_date('${batch_date}','yyyymmdd');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/ccrm_wind_neeqgradation.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes