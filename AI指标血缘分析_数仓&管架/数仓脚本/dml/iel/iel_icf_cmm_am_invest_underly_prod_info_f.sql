...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_icf_cmm_am_invest_underly_prod_info_f
CreateDate: 20230602
FileName:   ${iel_data_path}/cmm_am_invest_underly_prod_info.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.bank_int_prod_level2_cls_cd,chr(13),''),chr(10),'') as bank_int_prod_level2_cls_cd
,replace(replace(t1.bank_int_prod_level3_cls_cd,chr(13),''),chr(10),'') as bank_int_prod_level3_cls_cd
,replace(replace(t1.bank_int_prod_level4_cls_cd,chr(13),''),chr(10),'') as bank_int_prod_level4_cls_cd
,replace(replace(t1.bank_int_prod_level5_cls_cd,chr(13),''),chr(10),'') as bank_int_prod_level5_cls_cd
...
from ${icl_schema}.cmm_am_invest_underly_prod_info t1
where etl_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/cmm_am_invest_underly_prod_info.f.${batch_date}.dat" \
        charset=utf8
        safe=yes
