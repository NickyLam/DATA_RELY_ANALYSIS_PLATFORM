...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_evt_atmp_unionpay_tran_flow_a
CreateDate: 20180529
FileName:   ${iel_data_path}/evt_atmp_unionpay_tran_flow.a.${batch_date}.dat
IF_mark:    a
Logs:
...
,replace(replace(replace(t1.unionpay_exch_rat,chr(13),''),chr(10),''),chr(9),'') as unionpay_exch_rat 
,replace(replace(replace(t1.expns_acct_id,chr(13),''),chr(10),''),chr(9),'') as expns_acct_id 
,replace(replace(replace(t1.depot_acct_id,chr(13),''),chr(10),''),chr(9),'') as depot_acct_id 
,replace(replace(replace(t1.atmc_tran_flow_num,chr(13),''),chr(10),''),chr(9),'') as atmc_tran_flow_num 
,' ' as msg_head_info 
,replace(replace(replace(t1.tran_status_cd,chr(13),''),chr(10),''),chr(9),'') as tran_status_cd 
,replace(replace(replace(t1.err_cd,chr(13),''),chr(10),''),chr(9),'') as err_cd 
,replace(replace(replace(t1.err_info,chr(13),''),chr(10),''),chr(9),'') as err_info 
,replace(replace(replace(t1.termn_type_cd,chr(13),''),chr(10),''),chr(9),'') as termn_type_cd 
,replace(replace(replace(t1.init_way_cd,chr(13),''),chr(10),''),chr(9),'') as init_way_cd 
,replace(replace(replace(t1.mercht_cty_rg_cd,chr(13),''),chr(10),''),chr(9),'') as mercht_cty_rg_cd 
...
where etl_dt <= to_date('${batch_date}','yyyymmdd') " \
        ...
        file="${iel_data_path}/evt_atmp_unionpay_tran_flow.a.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes