...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_pams_jxbb_dkftpmx_bw_i
CreateDate: 20240722
FileName:   ${iel_data_path}/pams_jxbb_dkftpmx_bw.i.${batch_date}.dat
IF_mark:    i
Logs:
' \
        query="select
...
,txfpbl
,replace(replace(t1.zjtxbz,chr(13),''),chr(10),'') as zjtxbz
,dkje
,jrj
,jlx
,djftpzycb
...
from ${iol_schema}.pams_jxbb_dkftpmx_bw t1
where etl_dt <= to_date('${batch_date}','yyyymmdd') and etl_dt > to_date('${batch_date}','yyyymmdd')-15" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/pams_jxbb_dkftpmx_bw.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
