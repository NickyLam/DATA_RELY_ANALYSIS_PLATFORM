...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_ctms_tbs_v_withdrawdeals_f_w
CreateDate: 20180529
FileName:   ${iel_data_path}/ctms_tbs_v_withdrawdeals_w.f.${batch_date}.dat
IF_mark:    f_w
Logs:
...
,replace(replace(accruedinterest,chr(10),''),chr(13),'') as accruedinterest
,replace(replace(receivableinterest,chr(10),''),chr(13),'') as receivableinterest
,replace(replace(withdrawinterest,chr(10),''),chr(13),'') as withdrawinterest
,replace(replace(accruedinterest2,chr(10),''),chr(13),'') as accruedinterest2
...
,id_mark
,etl_timestamp
from iol.ctms_tbs_v_withdrawdeals
where start_dt between to_date('${batch_date}', 'yyyymmdd') - 6 and to_date('${batch_date}', 'yyyymmdd');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/ctms_tbs_v_withdrawdeals_w.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes