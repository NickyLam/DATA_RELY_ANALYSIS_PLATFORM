...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_agt_overs_remit_decl_info_h_f
CreateDate: 20231110
FileName:   ${iel_data_path}/agt_overs_remit_decl_info_h.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
from ${iml_schema}.agt_overs_remit_decl_info_h t1
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/agt_overs_remit_decl_info_h.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
