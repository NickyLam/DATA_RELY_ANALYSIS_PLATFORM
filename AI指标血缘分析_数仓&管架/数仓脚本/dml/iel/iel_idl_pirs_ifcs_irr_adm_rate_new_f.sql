...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_pirs_ifcs_irr_adm_rate_new_f
CreateDate: 20180529
FileName:   ${iel_data_path}/pirs_ifcs_irr_adm_rate_new.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,replace(replace(t1.fina_code,chr(13),''),chr(10),'') as fina_code
,replace(replace(t1.curr_cd,chr(13),''),chr(10),'') as curr_cd
,replace(replace(t1.amt_typ,chr(13),''),chr(10),'') as amt_typ
...
,replace(replace(t1.tranway_flg,chr(13),''),chr(10),'') as tranway_flg
,replace(replace(t1.cust_typ,chr(13),''),chr(10),'') as cust_typ
,replace(replace(t1.agreement_typ,chr(13),''),chr(10),'') as agreement_typ
...
from ${iol_schema}.ifcs_irr_adm_rate_new t1
where t1.start_dt <= to_date('${batch_date}','yyyymmdd') and t1.end_dt > to_date('${batch_date}','yyyymmdd');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/pirs_ifcs_irr_adm_rate_new.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes