...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_icl_cmm_unite_wl_dubil_info_f
CreateDate: 20251222
FileName:   ${iel_data_path}/cmm_unite_wl_dubil_info.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.regroup_flg,chr(13),''),chr(10),'') as regroup_flg
,replace(replace(t1.core_dubil_id,chr(13),''),chr(10),'') as core_dubil_id
,replace(replace(t1.repay_open_acct_bank_id,chr(13),''),chr(10),'') as repay_open_acct_bank_id
,replace(replace(t1.repay_open_acct_org_name,chr(13),''),chr(10),'') as repay_open_acct_org_name
,replace(replace(t1.cont_id,chr(13),''),chr(10),'') as cont_id

from ${icl_schema}.cmm_unite_wl_dubil_info t1
where etl_dt = to_date('${batch_date}','yyyymmdd')-1;" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/cmm_unite_wl_dubil_info.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
