...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iof_uxds_f_mbvg0026v2_request_f
CreateDate: 20250701
FileName:   ${iel_data_path}/uxds_f_mbvg0026v2_request.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.empowerno,chr(13),''),chr(10),'') as empowerno
,replace(replace(t1.genmonth,chr(13),''),chr(10),'') as genmonth

from ${iol_schema}.uxds_f_mbvg0026v2_request t1
where etl_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/uxds_f_mbvg0026v2_request.f.${batch_date}.dat" \
        charset=utf8
        safe=yes
