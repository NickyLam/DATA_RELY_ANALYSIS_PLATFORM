...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_agt_bill_blank_vouch_f
CreateDate: 20230525
FileName:   ${iel_data_path}/agt_bill_blank_vouch.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
from ${iml_schema}.agt_bill_blank_vouch t1
where create_dt <= to_date('${batch_date}','yyyymmdd') and id_mark<>'D'" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/agt_bill_blank_vouch.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
