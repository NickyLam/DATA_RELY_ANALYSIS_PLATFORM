...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_hdws_dul_d_east_pty_indv_party_info_f
CreateDate: 20180529
FileName:   ${iel_data_path}/hdws_dul_d_east_pty_indv_party_info.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,NVL2(t1.data_src_cd,'PTY_INDV_PARTY_INFO'||'_'||DECODE(T1.DATA_SRC_CD,'LHWD',UPPER(SUBSTR(T1.JOB_CD,1,4)),T1.DATA_SRC_CD),'PTY_INDV_PARTY_INFO') as etl_task_name 
,replace(replace(t1.pty_categ_cd,chr(13),''),chr(10),'') as pty_categ_cd
,replace(replace(t1.empl_typ_cd,chr(13),''),chr(10),'') as empl_typ_cd
,t1.chdr_qtty as chdr_qtty
,replace(replace(t1.own_local_e_state_flg,chr(13),''),chr(10),'') as own_local_e_state_flg
,replace(replace(t1.own_car_flg,chr(13),''),chr(10),'') as own_car_flg
,replace(replace(t1.sala_peop_flg,chr(13),''),chr(10),'') as sala_peop_flg
,replace(replace(t1.civil_serv_flg,chr(13),''),chr(10),'') as civil_serv_flg
,replace(replace(t1.self_cert_decl_flg,chr(13),''),chr(10),'') as self_cert_decl_flg
,replace(replace(t1.rvn_resd_iden_cd,chr(13),''),chr(10),'') as rvn_resd_iden_cd
,replace(replace(t1.rvn_resd_en_sur,chr(13),''),chr(10),'') as rvn_resd_en_sur
,replace(replace(t1.rvn_resd_en_name,chr(13),''),chr(10),'') as rvn_resd_en_name
from ${idl_schema}.hdws_dul_d_east_pty_indv_party_info t1
where etl_dt = to_date('${batch_date}','yyyymmdd');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/hdws_dul_d_east_pty_indv_party_info.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes