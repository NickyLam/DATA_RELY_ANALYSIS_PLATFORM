...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_pams_jxdx_tyzzh_f
CreateDate: 20240131
FileName:   ${iel_data_path}/pams_jxdx_tyzzh.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.khqd,chr(13),''),chr(10),'') as khqd
,replace(replace(t1.yhzhzl,chr(13),''),chr(10),'') as yhzhzl
,replace(replace(t1.zhbh,chr(13),''),chr(10),'') as zhbh

from ${iol_schema}.pams_jxdx_tyzzh t1
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/pams_jxdx_tyzzh.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
