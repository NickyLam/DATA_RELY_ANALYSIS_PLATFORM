...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_rpt_ibms_ttrd_set_instruction_his_i
CreateDate: 20180529
FileName:   ${iel_data_path}/rpt_ibms_ttrd_set_instruction_his.i.${batch_date}.dat
IF_mark:    i
Logs:
...
,replace(replace(t1.q_accname,chr(13),''),chr(10),'') as q_accname
,replace(replace(t1.q_secu_acct_id,chr(13),''),chr(10),'') as q_secu_acct_id
,replace(replace(t1.q_party_zzd_acct_code,chr(13),''),chr(10),'') as q_party_zzd_acct_code
,replace(replace(t1.q_p_type,chr(13),''),chr(10),'') as q_p_type
,replace(replace(t1.q_p_class,chr(13),''),chr(10),'') as q_p_class
,replace(replace(t1.q_currency,chr(13),''),chr(10),'') as q_currency
,replace(replace(t1.q_i_name,chr(13),''),chr(10),'') as q_i_name
,t1.q_i_id as q_i_id
,t1.q_settle_amount as q_settle_amount
,replace(replace(t1.q_two_effective_contract,chr(13),''),chr(10),'') as q_two_effective_contract
,replace(replace(t1.trade_orddate,chr(13),''),chr(10),'') as trade_orddate
,replace(replace(t1.trade_ids,chr(13),''),chr(10),'') as trade_ids
,replace(replace(t1.order_ids,chr(13),''),chr(10),'') as order_ids
,t1.trade_ref_type as trade_ref_type
,replace(replace(t1.q_description,chr(13),''),chr(10),'') as q_description
,t1.process_type as process_type
,replace(replace(t1.is_refreshable,chr(13),''),chr(10),'') as is_refreshable
 from iol.ibms_ttrd_set_instruction_his T1
where replace(real_set_date,'-','')='${batch_date}';" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/rpt_ibms_ttrd_set_instruction_his.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes