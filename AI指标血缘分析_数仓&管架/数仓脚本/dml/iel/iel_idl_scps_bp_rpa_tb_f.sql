...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_scps_bp_rpa_tb_f
CreateDate: 20180529
FileName:   ${iel_data_path}/scps_bp_rpa_tb.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,old_approve_no
,doc_id
from ${idl_schema}.scps_bp_rpa_tb t1
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd') and etl_dt = to_date('${batch_date}','yyyymmdd');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/scps_bp_rpa_tb.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes