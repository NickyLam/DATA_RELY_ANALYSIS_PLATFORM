...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_ctms_tbs_v_security_rating_f
CreateDate: 20221013
FileName:   ${iel_data_path}/ctms_tbs_v_security_rating.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
from ${iol_schema}.ctms_tbs_v_security_rating t1
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/ctms_tbs_v_security_rating.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
