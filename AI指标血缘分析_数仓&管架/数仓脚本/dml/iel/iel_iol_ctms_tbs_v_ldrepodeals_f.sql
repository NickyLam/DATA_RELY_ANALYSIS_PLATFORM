...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_ctms_tbs_v_ldrepodeals_f
CreateDate: 20221013
FileName:   ${iel_data_path}/ctms_tbs_v_ldrepodeals.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.repo_method,chr(13),''),chr(10),'') as repo_method
,replace(replace(t1.contract_no,chr(13),''),chr(10),'') as contract_no

from ${iol_schema}.ctms_tbs_v_ldrepodeals t1
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/ctms_tbs_v_ldrepodeals.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
