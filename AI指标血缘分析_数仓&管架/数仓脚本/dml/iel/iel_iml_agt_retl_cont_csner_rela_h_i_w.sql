...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_agt_retl_cont_csner_rela_h_i_w
CreateDate: 20180529
FileName:   ${iel_data_path}/agt_retl_cont_csner_rela_h_w.i.${batch_date}.dat
IF_mark:    i_w
Logs:
...
from ${iml_schema}.agt_retl_cont_csner_rela_h t
where (start_dt <= to_date('${batch_date}','yyyymmdd') and  start_dt >= to_date('${batch_date}','yyyymmdd') -6)  or (end_dt <= to_date('${batch_date}','yyyymmdd') and  end_dt >= to_date('${batch_date}','yyyymmdd') -6) ;" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/agt_retl_cont_csner_rela_h_w.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes