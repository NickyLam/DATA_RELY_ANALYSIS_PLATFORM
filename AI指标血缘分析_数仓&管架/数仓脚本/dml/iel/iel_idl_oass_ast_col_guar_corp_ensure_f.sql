...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_oass_ast_col_guar_corp_ensure_f
CreateDate: 20230116
FileName:   ${iel_data_path}/oass_ast_col_guar_corp_ensure.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
from ${idl_schema}.oass_ast_col_guar_corp_ensure t1
where etl_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/oass_ast_col_guar_corp_ensure.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
