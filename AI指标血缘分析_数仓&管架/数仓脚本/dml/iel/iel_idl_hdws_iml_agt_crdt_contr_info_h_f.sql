...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_hdws_iml_agt_crdt_contr_info_h_f
CreateDate: 20180529
FileName:   ${iel_data_path}/hdws_iml_agt_crdt_contr_info_h.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,replace(replace(t1.crdt_emply_id,chr(13),''),chr(10),'') as crdt_emply_id
...
,replace(replace(t1.crdt_contr_status_cd,chr(13),''),chr(10),'') as crdt_contr_status_cd
...
,replace(replace(t1.crdt_lmt_wrt_off_rsns,chr(13),''),chr(10),'') as crdt_lmt_wrt_off_rsns
,t1.usab_lmt as usab_lmt
,t1.occu_lmt as occu_lmt
,replace(replace(t1.crdt_contr_frz_flg,chr(13),''),chr(10),'') as crdt_contr_frz_flg
...
,replace(replace(t1.temp_lmt_flg,chr(13),''),chr(10),'') as temp_lmt_flg
,replace(replace(t1.csld_crdt_flg,chr(13),''),chr(10),'') as csld_crdt_flg
,replace(replace(t1.on_flg,chr(13),''),chr(10),'') as on_flg
,replace(replace(t1.grp_crdt_lmt_flg,chr(13),''),chr(10),'') as grp_crdt_lmt_flg
...
,t1.expos_usab_lmt as expos_usab_lmt
,replace(replace(t1.data_src_cd,chr(13),''),chr(10),'') as data_src_cd
,NVL2(t1.data_src_cd,'AGT_CRDT_CONTR_INFO_H'||'_'||DECODE(T1.DATA_SRC_CD,'LHWD',UPPER(SUBSTR(T1.JOB_CD,1,4)),T1.DATA_SRC_CD),'AGT_CRDT_CONTR_INFO_H') as etl_task_name 
,replace(replace(t1.auth_seq_num,chr(13),''),chr(10),'') as auth_seq_num
,replace(replace(t1.crdt_flow_typ_cd,chr(13),''),chr(10),'') as crdt_flow_typ_cd
,replace(replace(t1.east_status_cd,chr(13),''),chr(10),'') as east_status_cd
from ${idl_schema}.hdws_iml_agt_crdt_contr_info t1
where ((etl_dt = to_date('${batch_date}','yyyymmdd')-1 and data_src_cd = 'LHWD') OR (etl_dt = to_date('${batch_date}','yyyymmdd') and data_src_cd <> 'LHWD'))
  and del_flg <> '1';" \
        ...
        file="${iel_data_path}/hdws_iml_agt_crdt_contr_info_h.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes