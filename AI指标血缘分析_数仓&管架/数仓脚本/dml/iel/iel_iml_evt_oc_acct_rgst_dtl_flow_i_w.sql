...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_evt_oc_acct_rgst_dtl_flow_i_w
CreateDate: 20180529
FileName:   ${iel_data_path}/evt_oc_acct_rgst_dtl_flow_w.i.${batch_date}.dat
IF_mark:    i_w
Logs:
...
where etl_dt <= to_date('${batch_date}','yyyymmdd') and etl_dt >= to_date('${batch_date}','yyyymmdd') -6 and etl_dt >= to_date('${batch_date}', 'yyyymmdd') - 6 ;" \
        ...
        file="${iel_data_path}/evt_oc_acct_rgst_dtl_flow_w.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes