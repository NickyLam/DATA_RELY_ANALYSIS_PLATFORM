...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_pty_rp_lp_info_h_a1
CreateDate: 20251106
FileName:   ${iel_data_path}/pty_rp_lp_info_h.i.${batch_date}.dat
IF_mark:    i
Logs:
' \
        query="select
...
,replace(replace(t1.rela_party_from_id,chr(13),''),chr(10),'') as rela_party_from_id
,replace(replace(t1.rela_party_from_descb,chr(13),''),chr(10),'') as rela_party_from_descb
...
,replace(replace(t1.east_incid_rela_type_cd,chr(13),''),chr(10),'') as east_incid_rela_type_cd
...
,replace(replace(t1.mang_status_cd,chr(13),''),chr(10),'') as mang_status_cd
,rgst_cap
...
from ${iml_schema}.pty_rp_lp_info_h t1
where etl_dt <= to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/pty_rp_lp_info_h.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
