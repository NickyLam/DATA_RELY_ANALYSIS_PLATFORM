...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_rpt_upps_order_header_i
CreateDate: 20180529
FileName:   ${iel_data_path}/rpt_upps_order_header.i.${batch_date}.dat
IF_mark:    i
Logs:
...
,replace(replace(t1.payee_voucher_date,chr(13),''),chr(10),'') as payee_voucher_date
 from iol.upps_order_header T1
where to_char(created_tx_stamp,'yyyymmdd') = '${batch_date}';" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/rpt_upps_order_header.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes