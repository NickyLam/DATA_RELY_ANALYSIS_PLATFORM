...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_rsts_src_dw_pty_indv_party_info_f
CreateDate: 20241012
FileName:   ${iel_data_path}/rsts_src_dw_pty_indv_party_info.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
from ${iol_schema}.rsts_src_dw_pty_indv_party_info t1
where etl_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/rsts_src_dw_pty_indv_party_info.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
