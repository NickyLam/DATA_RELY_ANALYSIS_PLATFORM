...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_cqss_e_r_wrntaccundrwryrsplinf_i_w
CreateDate: 20180529
FileName:   ${iel_data_path}/cqss_e_r_wrntaccundrwryrsplinf_w.i.${batch_date}.dat
IF_mark:    i_w
Logs:
...
from ${iol_schema}.cqss_e_r_wrntaccundrwryrsplinf t
where to_char(crt_dt_tm,'yyyymmdd') <= '${batch_date}' and to_char(crt_dt_tm,'yyyymmdd') >= '${batch_date}' -6  ;" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/cqss_e_r_wrntaccundrwryrsplinf_w.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes