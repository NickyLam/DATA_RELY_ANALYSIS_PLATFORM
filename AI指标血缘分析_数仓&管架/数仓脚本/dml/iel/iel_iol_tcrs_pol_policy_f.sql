...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_tcrs_pol_policy_f
CreateDate: 20180529
FileName:   ${iel_data_path}/tcrs_pol_policy.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,replace(replace(t.policy_code,chr(13),''),chr(10),'') as policy_code
...
,replace(replace(t.decision,chr(13),''),chr(10),'') as decision
,replace(replace(t.copy_from,chr(13),''),chr(10),'') as copy_from
,t.version as version
,t.start_dt as start_dt
,t.end_dt as end_dt
,replace(replace(t.id_mark,chr(13),''),chr(10),'') as id_mark
from iol.tcrs_pol_policy t
 where start_dt <= to_date('${batch_date}','yyyymmdd')
   and end_dt > to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/tcrs_pol_policy.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes