...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_agt_bill_bus_stl_info_f_w
CreateDate: 20180529
FileName:   ${iel_data_path}/agt_bill_bus_stl_info_w.f.${batch_date}.dat
IF_mark:    f_w
Logs:
...
,replace(replace(t1.lg_pay_sys_msg_ind_no,chr(13),''),chr(10),'') as lg_pay_sys_msg_ind_no
...
,replace(replace(t1.recver_cap_acct_num,chr(13),''),chr(10),'') as recver_cap_acct_num
,replace(replace(t1.recver_cap_acct_name,chr(13),''),chr(10),'') as recver_cap_acct_name
,replace(replace(t1.payer_org_cd,chr(13),''),chr(10),'') as payer_org_cd
...
,replace(replace(t1.payer_cap_acct_num,chr(13),''),chr(10),'') as payer_cap_acct_num
,replace(replace(t1.payer_cap_acct_name,chr(13),''),chr(10),'') as payer_cap_acct_name
,replace(replace(t1.stl_status_cd,chr(13),''),chr(10),'') as stl_status_cd
,replace(replace(t1.stl_rest_code,chr(13),''),chr(10),'') as stl_rest_code
,replace(replace(t1.stl_fail_rs,chr(13),''),chr(10),'') as stl_fail_rs
...
where (start_dt <= to_date('${batch_date}', 'yyyymmdd') and
    ...
    or (end_dt <= to_date('${batch_date}', 'yyyymmdd') and
       end_dt >= to_date('${batch_date}', 'yyyymmdd') - 6) ;" \
        ...
        file="${iel_data_path}/agt_bill_bus_stl_info_w.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes