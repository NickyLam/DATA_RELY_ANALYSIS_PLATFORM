...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_icms_wyd_guarantee_cont_f
CreateDate: 20250224
FileName:   ${iel_data_path}/icms_wyd_guarantee_cont.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.localgovguartype,chr(13),''),chr(10),'') as localgovguartype
...
from ${iol_schema}.icms_wyd_guarantee_cont t1
where etl_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/icms_wyd_guarantee_cont.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
