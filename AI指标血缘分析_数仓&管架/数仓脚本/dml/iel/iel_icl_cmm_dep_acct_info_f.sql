...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_icl_cmm_dep_acct_info_f
CreateDate: 20180529
FileName:   ${iel_data_path}/cmm_dep_acct_info.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,replace(replace(t1.int_paybl_subj_id,chr(13),''),chr(10),'') as int_paybl_subj_id 
,replace(replace(t1.int_paybl_adj_subj_id,chr(13),''),chr(10),'') as int_paybl_adj_subj_id 
,replace(replace(t1.int_expns_subj_id,chr(13),''),chr(10),'') as int_expns_subj_id 
,replace(replace(t1.int_expns_adj_subj_id,chr(13),''),chr(10),'') as int_expns_adj_subj_id 
,replace(replace(t1.open_flow_num,chr(13),''),chr(10),'') as open_flow_num 
,replace(replace(t1.clos_flow_num,chr(13),''),chr(10),'') as clos_flow_num 
,t1.currt_int_paybl_adj as currt_int_paybl_adj 
,t1.td_int_expns as td_int_expns 
,t1.td_int_expns_adj as td_int_expns_adj 
from ${icl_schema}.cmm_dep_acct_info t1 
where etl_dt = to_date('${batch_date}','yyyymmdd');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/cmm_dep_acct_info.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes