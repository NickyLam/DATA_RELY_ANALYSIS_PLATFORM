...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_rpt_rcrs_mybk_supv_cust_f
CreateDate: 20220306
FileName:   ${iel_data_path}/rpt_rcrs_mybk_supv_cust.f.${batch_date}.dat
IF_mark:    f
Logs:
   sundexin
' \
 ...
 from iol.rcrs_mybk_supv_cust T
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/rpt_rcrs_mybk_supv_cust.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes