...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_hdws_dul_d_rpts_agt_ln_ac_base_info_02_f
CreateDate: 20180529
FileName:   ${iel_data_path}/hdws_dul_d_rpts_agt_ln_ac_base_info_02.f.${batch_date}.dat
IF_mark:    f
Logs:
...
' \
      ...
      ,replace(replace(bil_acct_id,chr(10),''),chr(13),'') as bil_acct_id
      ...
      ,replace(replace(sub_guar_mode_cd,chr(10),''),chr(13),'') as sub_guar_mode_cd
      ,replace(replace(loan_cate_cd,chr(10),''),chr(13),'') as loan_cate_cd
      ,replace(replace(gov_platf_loan_flg,chr(10),''),chr(13),'') as gov_platf_loan_flg
      ,replace(replace(acct_categ_cd,chr(10),''),chr(13),'') as acct_categ_cd
      ,replace(replace(loan_flg,chr(10),''),chr(13),'') as loan_flg
      ,replace(replace(acpt_flg,chr(10),''),chr(13),'') as acpt_flg
      ,replace(replace(data_src_cd,chr(10),''),chr(13),'') as data_src_cd
      ,replace(replace(del_flg,chr(10),''),chr(13),'') as del_flg
      ,replace(replace(bout_liqdt_flg,chr(10),''),chr(13),'') as bout_liqdt_flg
...
from idl.hdws_dul_d_rpts_agt_ln_ac_base_info_02 
where to_char(etl_dt,'yyyymmdd') = '${batch_date}';" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/hdws_dul_d_rpts_agt_ln_ac_base_info_02.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes