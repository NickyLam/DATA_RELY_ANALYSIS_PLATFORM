...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_irvs_ca_product_doc_f
CreateDate: 20180529
FileName:   ${iel_data_path}/irvs_ca_product_doc.f.${batch_date}.dat
IF_mark:    f
Logs:
...
from iol.irvs_ca_product_doc t
where t.start_dt <= to_date('${batch_date}','yyyymmdd') and t.end_dt > to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/irvs_ca_product_doc.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes