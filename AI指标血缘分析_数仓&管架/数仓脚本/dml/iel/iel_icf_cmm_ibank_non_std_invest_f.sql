...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_icf_cmm_ibank_non_std_invest_f
CreateDate: 20221122
FileName:   ${iel_data_path}/cmm_ibank_non_std_invest.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.uder_dir_indus_categy_cd,chr(13),''),chr(10),'') as uder_dir_indus_categy_cd
...
,replace(replace(t1.uder_bond_rating_rest_cd,chr(13),''),chr(10),'') as uder_bond_rating_rest_cd
...
,replace(replace(t1.uder_actl_finer_group,chr(13),''),chr(10),'') as uder_actl_finer_group
...
from ${icl_schema}.cmm_ibank_non_std_invest t1
where etl_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/cmm_ibank_non_std_invest.f.${batch_date}.dat" \
        charset=utf8
        safe=yes
