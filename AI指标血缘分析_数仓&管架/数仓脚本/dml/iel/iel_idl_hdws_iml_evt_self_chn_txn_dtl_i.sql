...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_hdws_iml_evt_self_chn_txn_dtl_i
CreateDate: 20180529
FileName:   ${iel_data_path}/hdws_iml_evt_self_chn_txn_dtl.i.${batch_date}.dat
IF_mark:    i
Logs:
...
,t1.etl_dt as etl_dt
,NVL2(t1.data_src_cd,'EVT_SELF_CHN_TXN_DTL'||'_'||DECODE(T1.DATA_SRC_CD,'LHWD',UPPER(SUBSTR(T1.JOB_CD,1,4)),T1.DATA_SRC_CD),'EVT_SELF_CHN_TXN_DTL') as etl_task_name
...
,replace(replace(t1.unpay_chn_cd,chr(13),''),chr(10),'') as unpay_chn_cd
,replace(replace(t1.txn_resp_desc,chr(13),''),chr(10),'') as txn_resp_desc
,replace(replace(t1.termn_id,chr(13),''),chr(10),'') as termn_id
,replace(replace(t1.acpt_org_cd,chr(13),''),chr(10),'') as acpt_org_cd
,replace(replace(t1.srv_pnt_input_mode_cd,chr(13),''),chr(10),'') as srv_pnt_input_mode_cd
,replace(replace(t1.pwd_check_flg,chr(13),''),chr(10),'') as pwd_check_flg
,replace(replace(t1.rev_seq_num,chr(13),''),chr(10),'') as rev_seq_num
from ${idl_schema}.hdws_iml_evt_self_chn_txn_dtl t1
where etl_dt = to_date('${batch_date}','yyyymmdd');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/hdws_iml_evt_self_chn_txn_dtl.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes