...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_hdws_dul_d_rpts_agt_bl_act_base_info_f
CreateDate: 20180529
FileName:   ${iel_data_path}/hdws_dul_d_rpts_agt_bl_act_base_info.f.${batch_date}.dat
IF_mark:    f
Logs:
...
' \
      ...
      ,replace(replace(small_bill_flg,chr(10),''),chr(13),'') as small_bill_flg
      ,replace(replace(rdisc_flg,chr(10),''),chr(13),'') as rdisc_flg 
from idl.hdws_dul_d_rpts_agt_bl_act_base_info 
where to_char(etl_dt,'yyyymmdd') = '${batch_date}';" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/hdws_dul_d_rpts_agt_bl_act_base_info.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes