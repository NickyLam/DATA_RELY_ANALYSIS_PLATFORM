...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_rpt_rcrs_mybk_cs_extent_info_i
CreateDate: 20220306
FileName:   ${iel_data_path}/rpt_rcrs_mybk_cs_extent_info.i.${batch_date}.dat
IF_mark:    i
Logs:
   sundexin
' \
    ...
    ,replace(replace(t.cert_valid_start_date,chr(13),''),chr(10),'') as cert_valid_start_date
    ,replace(replace(t.sex,chr(13),''),chr(10),'') as sex
    ,replace(replace(t.indiv_occ,chr(13),''),chr(10),'') as indiv_occ
    ,replace(replace(t.nationality,chr(13),''),chr(10),'') as nationality
    ,t.etl_dt as etl_dt
 from iol.rcrs_mybk_cs_extent_info t
where to_char(to_date(substr(serno,5,8),'yyyy-mm-dd'),'yyyymmdd') = '${batch_date}' and substr(serno,0,4)='MYBK';" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/rpt_rcrs_mybk_cs_extent_info.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes