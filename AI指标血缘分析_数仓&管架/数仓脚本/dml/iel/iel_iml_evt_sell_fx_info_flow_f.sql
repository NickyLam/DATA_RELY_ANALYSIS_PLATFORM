...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_evt_sell_fx_info_flow_f
CreateDate: 20221229
FileName:   ${iel_data_path}/evt_sell_fx_info_flow.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,tran_dt
,replace(replace(t1.sell_fx_tran_status_cd,chr(13),''),chr(10),'') as sell_fx_tran_status_cd
,replace(replace(t1.sell_fx_tran_type_cd,chr(13),''),chr(10),'') as sell_fx_tran_type_cd
,replace(replace(t1.bank_flow_num,chr(13),''),chr(10),'') as bank_flow_num
,replace(replace(t1.sell_fx_bus_type_cd,chr(13),''),chr(10),'') as sell_fx_bus_type_cd
...
,replace(replace(t1.sell_fx_cap_attr_cd,chr(13),''),chr(10),'') as sell_fx_cap_attr_cd
,replace(replace(t1.curr_cd,chr(13),''),chr(10),'') as curr_cd
,sell_fx_amt
,sell_fx_cash_amt
,remit_out_amt
,depot_indv_fx_acct_amt
,bk_check_amt
,replace(replace(t1.sell_fx_cny_acct_id,chr(13),''),chr(10),'') as sell_fx_cny_acct_id
...
,replace(replace(t1.fx_doc_id,chr(13),''),chr(10),'') as fx_doc_id
,bus_trast_tm
...
from ${iml_schema}.evt_sell_fx_info_flow t1
where etl_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/evt_sell_fx_info_flow.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
