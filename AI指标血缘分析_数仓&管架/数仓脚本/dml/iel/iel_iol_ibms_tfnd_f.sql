...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_ibms_tfnd_f
CreateDate: 20221013
FileName:   ${iel_data_path}/ibms_tfnd.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.carry_forword_type,chr(13),''),chr(10),'') as carry_forword_type
...
,replace(replace(t1.main_code,chr(13),''),chr(10),'') as main_code
...
from ${iol_schema}.ibms_tfnd t1
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/ibms_tfnd.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
