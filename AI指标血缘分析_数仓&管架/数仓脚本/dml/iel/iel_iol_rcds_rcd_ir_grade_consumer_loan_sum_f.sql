...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_rcds_rcd_ir_grade_consumer_loan_sum_f
CreateDate: 20180529
FileName:   ${iel_data_path}/rcds_rcd_ir_grade_consumer_loan_sum.f.${batch_date}.dat
IF_mark:    f
Logs:
...
from iol.rcds_rcd_ir_grade_consumer_loan_sum t
  where t.start_dt <= to_date('${batch_date}','yyyymmdd') and t.end_dt > to_date('${batch_date}','yyyymmdd') " \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/rcds_rcd_ir_grade_consumer_loan_sum.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes