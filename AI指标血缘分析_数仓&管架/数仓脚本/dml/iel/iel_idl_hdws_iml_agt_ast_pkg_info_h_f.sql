...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_hdws_iml_agt_ast_pkg_info_h_f
CreateDate: 20180529
FileName:   ${iel_data_path}/hdws_iml_agt_ast_pkg_info_h.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,t1.reg_dt as reg_dt
,NVL2(t1.data_src_cd,'AGT_AST_PKG_INFO_H'||'_'||DECODE(T1.DATA_SRC_CD,'LHWD',UPPER(SUBSTR(T1.JOB_CD,1,4)),T1.DATA_SRC_CD),'AGT_AST_PKG_INFO_H') as etl_task_name 
,replace(replace(t1.tfr_contr_id,chr(13),''),chr(10),'') as tfr_contr_id
,t1.pkg_tm_pnt_dbill_qtty as pkg_tm_pnt_dbill_qtty
,t1.pkg_tm_pnt_dbill_gross_bal as pkg_tm_pnt_dbill_gross_bal
,replace(replace(t1.ast_pkg_typ,chr(13),''),chr(10),'') as ast_pkg_typ
from ${idl_schema}.hdws_iml_agt_ast_pkg_info t1
where etl_dt = to_date('${batch_date}','yyyymmdd')
  and del_flg <> '1';" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/hdws_iml_agt_ast_pkg_info_h.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes