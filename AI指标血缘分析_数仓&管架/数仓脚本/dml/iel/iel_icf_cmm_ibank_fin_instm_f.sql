...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_icf_cmm_ibank_fin_instm_f
CreateDate: 20240911
FileName:   ${iel_data_path}/cmm_ibank_fin_instm.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,prod_currt_tot_asset
,fund_lever_rat
,this_ped_open_termnt_dt
,this_ped_hold_exp_dt
,replace(replace(t1.is_single_trdpty_rept_flg,chr(13),''),chr(10),'') as is_single_trdpty_rept_flg
,list_dt
,replace(replace(t1.crdt_main_cust_id,chr(13),''),chr(10),'') as crdt_main_cust_id
,replace(replace(t1.crdt_main_name,chr(13),''),chr(10),'') as crdt_main_name

from ${icl_schema}.cmm_ibank_fin_instm t1
where etl_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/cmm_ibank_fin_instm.f.${batch_date}.dat" \
        charset=utf8
        safe=yes
