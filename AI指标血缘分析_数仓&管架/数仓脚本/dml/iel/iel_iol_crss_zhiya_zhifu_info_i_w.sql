...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_crss_zhiya_zhifu_info_i_w
CreateDate: 20180529
FileName:   ${iel_data_path}/crss_zhiya_zhifu_info_w.i.${batch_date}.dat
IF_mark:    i_w
Logs:
...
,replace(replace(zhiyano,chr(10),''),chr(13),'') as zhiyano
...
,replace(replace(transq,chr(10),''),chr(13),'') as transq
...
,id_mark
,etl_timestamp
 from iol.crss_zhiya_zhifu_info 
 where start_dt between to_date('${batch_date}', 'yyyymmdd') - 6 and to_date('${batch_date}', 'yyyymmdd');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/crss_zhiya_zhifu_info_w.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes