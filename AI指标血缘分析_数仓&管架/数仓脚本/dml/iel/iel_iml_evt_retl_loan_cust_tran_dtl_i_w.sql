...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_evt_retl_loan_cust_tran_dtl_i_w
CreateDate: 20180529
FileName:   ${iel_data_path}/evt_retl_loan_cust_tran_dtl_w.i.${batch_date}.dat
IF_mark:    i_w
Logs:
...
from ${iml_schema}.evt_retl_loan_cust_tran_dtl t
where etl_dt <= to_date('${batch_date}','yyyymmdd') and etl_dt >= to_date('${batch_date}','yyyymmdd') -6 ;" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/evt_retl_loan_cust_tran_dtl_w.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes