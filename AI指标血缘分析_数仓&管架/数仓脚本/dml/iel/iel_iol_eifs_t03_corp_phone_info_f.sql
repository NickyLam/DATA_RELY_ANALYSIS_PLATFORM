...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_eifs_t03_corp_phone_info_f
CreateDate: 20240328
FileName:   ${iel_data_path}/eifs_t03_corp_phone_info.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
to_date('${batch_date}','yyyymmdd') as etl_dt
,replace(replace(t1.phone_id,chr(13),''),chr(10),'') as phone_id
,replace(replace(t1.party_id,chr(13),''),chr(10),'') as party_id
,replace(replace(t1.tel_type_cd,chr(13),''),chr(10),'') as tel_type_cd
,replace(replace(t1.long_path_zone_code,chr(13),''),chr(10),'') as long_path_zone_code
,replace(replace(t1.tel_propterty,chr(13),''),chr(10),'') as tel_propterty
...
from ${iol_schema}.eifs_t03_corp_phone_info t1
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/eifs_t03_corp_phone_info.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
