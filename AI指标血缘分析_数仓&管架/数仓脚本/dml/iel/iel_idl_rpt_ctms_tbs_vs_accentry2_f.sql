...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_rpt_ctms_tbs_vs_accentry2_f
CreateDate: 20180529
FileName:   ${iel_data_path}/rpt_ctms_tbs_vs_accentry2.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,replace(replace(t1.rev_flag,chr(13),''),chr(10),'') as rev_flag
 from iol.ctms_tbs_vs_accentry2 T1
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/rpt_ctms_tbs_vs_accentry2.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes