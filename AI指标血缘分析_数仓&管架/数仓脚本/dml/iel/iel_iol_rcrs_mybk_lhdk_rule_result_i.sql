...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_rcrs_mybk_lhdk_rule_result_i
CreateDate: 20180529
FileName:   ${iel_data_path}/rcrs_mybk_lhdk_rule_result.i.${batch_date}.dat
IF_mark:    i
Logs:
...
from ${iol_schema}.RCRS_MYBK_LHDK_RULE_RESULT t 
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd')
and substr(replace(time,'-',''),1,8)='${batch_date}';" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/rcrs_mybk_lhdk_rule_result.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes