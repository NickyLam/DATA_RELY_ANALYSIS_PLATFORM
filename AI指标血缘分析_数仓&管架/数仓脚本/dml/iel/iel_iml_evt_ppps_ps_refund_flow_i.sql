...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_evt_ppps_ps_refund_flow_i
CreateDate: 20230816
FileName:   ${iel_data_path}/evt_ppps_ps_refund_flow.i.${batch_date}.dat
IF_mark:    i
Logs:
' \
        query="select
...
,replace(replace(t1.refund_acct_belong_org_id,chr(13),''),chr(10),'') as refund_acct_belong_org_id
...
,init_indent_amt
,replace(replace(t1.init_pay_bank_tran_flow,chr(13),''),chr(10),'') as init_pay_bank_tran_flow
,replace(replace(t1.init_tran_flow_num,chr(13),''),chr(10),'') as init_tran_flow_num
,final_update_tm

from ${iml_schema}.evt_ppps_ps_refund_flow t1
where etl_dt <= to_date('${batch_date}','yyyymmdd') and etl_dt > to_date('${batch_date}','yyyymmdd')-15" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/evt_ppps_ps_refund_flow.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
