...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_ccrw_t_sys_user_f
CreateDate: 20251009
FileName:   ${iel_data_path}/ccrw_t_sys_user.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,ver
,replace(replace(t1.vir_flag,chr(13),''),chr(10),'') as vir_flag
...
,replace(replace(t1.is_teamer,chr(13),''),chr(10),'') as is_teamer
,replace(replace(t1.is_protection,chr(13),''),chr(10),'') as is_protection
,replace(replace(t1.is_show_mgr_rank,chr(13),''),chr(10),'') as is_show_mgr_rank
,protection_time_start
,protection_time_end

from ${iol_schema}.ccrw_t_sys_user t1
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/ccrw_t_sys_user.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
