...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_icf_cmm_out_remit_dtl_i
CreateDate: 20231117
FileName:   ${iel_data_path}/cmm_out_remit_dtl.i.${batch_date}.dat
IF_mark:    i
Logs:
' \
        query="select
...
,replace(replace(t1.recver_descb,chr(13),''),chr(10),'') as recver_descb
,replace(replace(t1.curr_cd,chr(13),''),chr(10),'') as curr_cd
,remit_amt
,replace(replace(t1.remit_type_cd,chr(13),''),chr(10),'') as remit_type_cd
...
,replace(replace(t1.msg_info_1,chr(13),''),chr(10),'') as msg_info_1
,replace(replace(t1.msg_info_2,chr(13),''),chr(10),'') as msg_info_2
,fee_amt
,usd_fee_amt
...
from ${icl_schema}.cmm_out_remit_dtl t1
where etl_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/cmm_out_remit_dtl.i.${batch_date}.dat" \
        charset=utf8
        safe=yes
