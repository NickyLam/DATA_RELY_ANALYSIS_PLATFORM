...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iof_ncbs_rb_int_rate_form_hist_f
CreateDate: 20230602
FileName:   ${iel_data_path}/ncbs_rb_int_rate_form_hist.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.om_hist_version_id,chr(13),''),chr(10),'') as om_hist_version_id
,last_change_date
...
from ${iol_schema}.ncbs_rb_int_rate_form_hist t1
where etl_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/ncbs_rb_int_rate_form_hist.f.${batch_date}.dat" \
        charset=utf8
        safe=yes
