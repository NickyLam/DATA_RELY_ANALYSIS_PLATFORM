...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_agt_forgn_inco_decl_info_h_f
CreateDate: 20231110
FileName:   ${iel_data_path}/agt_forgn_inco_decl_info_h.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.payer_permt_cty_rg_cd,chr(13),''),chr(10),'') as payer_permt_cty_rg_cd
,replace(replace(t1.bus_id,chr(13),''),chr(10),'') as bus_id
,replace(replace(t1.inco_type_cd,chr(13),''),chr(10),'') as inco_type_cd
,replace(replace(t1.recvbl_char_cd,chr(13),''),chr(10),'') as recvbl_char_cd
...
from ${iml_schema}.agt_forgn_inco_decl_info_h t1
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/agt_forgn_inco_decl_info_h.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
