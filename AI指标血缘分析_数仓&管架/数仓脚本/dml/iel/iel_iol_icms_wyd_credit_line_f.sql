: '
Purpose:    unload config for sqluldr2
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_icms_wyd_credit_line_f
CreateDate: 20260115
FileName:   ${iel_data_path}/icms_wyd_credit_line.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
to_date('${batch_date}','yyyymmdd') as etl_dt
,replace(replace(t1.datadt,chr(13),''),chr(10),'') as datadt
,replace(replace(t1.limitno,chr(13),''),chr(10),'') as limitno
,replace(replace(t1.custid,chr(13),''),chr(10),'') as custid
,replace(replace(t1.custidtype,chr(13),''),chr(10),'') as custidtype
,replace(replace(t1.custidno,chr(13),''),chr(10),'') as custidno
,replace(replace(t1.custname,chr(13),''),chr(10),'') as custname
,replace(replace(t1.ccycd,chr(13),''),chr(10),'') as ccycd
,replace(replace(t1.orgid,chr(13),''),chr(10),'') as orgid
,replace(replace(t1.circulflg,chr(13),''),chr(10),'') as circulflg
,replace(replace(t1.startdate,chr(13),''),chr(10),'') as startdate
,replace(replace(t1.maturitydate,chr(13),''),chr(10),'') as maturitydate
,creditline
,uncreditline
,replace(replace(t1.credittype,chr(13),''),chr(10),'') as credittype
,replace(replace(t1.begindate,chr(13),''),chr(10),'') as begindate
,replace(replace(t1.trandate,chr(13),''),chr(10),'') as trandate
,replace(replace(t1.initdate,chr(13),''),chr(10),'') as initdate
,replace(replace(t1.limitreportno,chr(13),''),chr(10),'') as limitreportno
,replace(replace(t1.status,chr(13),''),chr(10),'') as status
,replace(replace(t1.freezeflag,chr(13),''),chr(10),'') as freezeflag
,replace(replace(t1.adjustdate,chr(13),''),chr(10),'') as adjustdate
,replace(replace(t1.extenddate,chr(13),''),chr(10),'') as extenddate
,replace(replace(t1.inputuserid,chr(13),''),chr(10),'') as inputuserid
,replace(replace(t1.inputorgid,chr(13),''),chr(10),'') as inputorgid
,inputdate
,replace(replace(t1.updateuserid,chr(13),''),chr(10),'') as updateuserid
,replace(replace(t1.updateorgid,chr(13),''),chr(10),'') as updateorgid
,updatedate
,replace(replace(t1.productid,chr(13),''),chr(10),'') as productid
,replace(replace(t1.classifyresult,chr(13),''),chr(10),'') as classifyresult
,replace(replace(t1.balstatus,chr(13),''),chr(10),'') as balstatus
,replace(replace(t1.customerid,chr(13),''),chr(10),'') as customerid
,replace(replace(t1.inputid,chr(13),''),chr(10),'') as inputid
,replace(replace(t1.baserialno,chr(13),''),chr(10),'') as baserialno
,quotamod
,replace(replace(t1.hxcontractserialno,chr(13),''),chr(10),'') as hxcontractserialno
,replace(replace(t1.hxproductid,chr(13),''),chr(10),'') as hxproductid
,replace(replace(t1.hxoperateorgid,chr(13),''),chr(10),'') as hxoperateorgid
,replace(replace(t1.hxmanageorgid,chr(13),''),chr(10),'') as hxmanageorgid
,hxstartdate
,hxmaturity
,replace(replace(t1.hxstatus,chr(13),''),chr(10),'') as hxstatus
,replace(replace(t1.hxiscycle,chr(13),''),chr(10),'') as hxiscycle

from ${iol_schema}.icms_wyd_credit_line t1
where etl_dt = to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/icms_wyd_credit_line.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
