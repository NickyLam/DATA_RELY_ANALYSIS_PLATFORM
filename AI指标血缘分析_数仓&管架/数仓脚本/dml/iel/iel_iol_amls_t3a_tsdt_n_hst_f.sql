...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_amls_t3a_tsdt_n_hst_f
CreateDate: 20240802
FileName:   ${iel_data_path}/amls_t3a_tsdt_n_hst.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
from ${iol_schema}.amls_t3a_tsdt_n_hst t1
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/amls_t3a_tsdt_n_hst.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
