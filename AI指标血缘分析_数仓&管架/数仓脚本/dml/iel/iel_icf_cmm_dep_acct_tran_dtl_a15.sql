...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_icf_cmm_dep_acct_tran_dtl_a15
CreateDate: 20250221
FileName:   ${iel_data_path}/cmm_dep_acct_tran_dtl.a.${batch_date}.dat
IF_mark:    a
Logs:
' \
        query="select
...
,t1.agent_cert_start_dt
,t1.agent_cert_exp_dt
...
,t1.init_tran_timestamp
,replace(replace(t1.cross_bor_tran_flg,chr(13),''),chr(10),'') as cross_bor_tran_flg

from ${icl_schema}.cmm_dep_acct_tran_dtl t1
inner join ${icl_schema}.cmm_dep_acct_tran_dtl_bak20250221 t2
on t1.etl_dt = t2.etl_dt
and t1.tran_flow_num = t2.tran_flow_num
and t1.acct_bill_flow_num=t2.acct_bill_flow_num
and nvl(trim(t1.cross_bor_tran_flg),' ') <> nvl(trim(t2.cross_bor_tran_flg),' ')
where t1.etl_dt >= to_date('20240101','yyyymmdd') and t1.etl_dt <= to_date('${batch_date}','yyyymmdd') and t1.entry_flg='1';" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/cmm_dep_acct_tran_dtl.a.${batch_date}.dat" \
        charset=utf8
        safe=yes
