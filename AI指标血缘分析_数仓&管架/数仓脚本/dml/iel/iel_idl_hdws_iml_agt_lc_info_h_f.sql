...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_hdws_iml_agt_lc_info_h_f
CreateDate: 20180529
FileName:   ${iel_data_path}/hdws_iml_agt_lc_info_h.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,NVL2(t1.data_src_cd,'AGT_LC_INFO_H'||'_'||DECODE(T1.DATA_SRC_CD,'LHWD',UPPER(SUBSTR(T1.JOB_CD,1,4)),T1.DATA_SRC_CD),'AGT_LC_INFO_H') as etl_task_name 
from ${idl_schema}.hdws_iml_agt_lc_info t1
where etl_dt = to_date('${batch_date}','yyyymmdd')
  and del_flg <> '1';" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/hdws_iml_agt_lc_info_h.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes