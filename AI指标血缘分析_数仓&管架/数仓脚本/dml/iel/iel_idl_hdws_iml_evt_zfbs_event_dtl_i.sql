...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_hdws_iml_evt_zfbs_event_dtl_i
CreateDate: 20180529
FileName:   ${iel_data_path}/hdws_iml_evt_zfbs_event_dtl.i.${batch_date}.dat
IF_mark:    i
Logs:
...
,NVL2(t1.data_src_cd,'EVT_ZFBS_EVENT_DTL'||'_'||DECODE(T1.DATA_SRC_CD,'LHWD',UPPER(SUBSTR(T1.JOB_CD,1,4)),T1.DATA_SRC_CD),'EVT_ZFBS_EVENT_DTL') as etl_task_name
,t1.etl_dt as etl_dt
...
from ${idl_schema}.hdws_iml_evt_zfbs_event_dtl t1
where ((etl_dt = to_date('${batch_date}','yyyymmdd')-1 and data_src_cd = 'LHWD') OR (etl_dt = to_date('${batch_date}','yyyymmdd') and data_src_cd <> 'LHWD'));" \
        ...
        file="${iel_data_path}/hdws_iml_evt_zfbs_event_dtl.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes