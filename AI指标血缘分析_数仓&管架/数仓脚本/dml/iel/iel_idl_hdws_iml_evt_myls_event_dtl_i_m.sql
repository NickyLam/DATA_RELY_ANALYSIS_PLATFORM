...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_hdws_iml_evt_myls_event_dtl_i_m
CreateDate: 20180529
FileName:   ${iel_data_path}/hdws_iml_evt_myls_event_dtl_m.i.${batch_date}.dat
IF_mark:    i_m
Logs:
...
,NVL2(t1.data_src_cd,'EVT_MYLS_EVENT_DTL'||'_'||DECODE(T1.DATA_SRC_CD,'LHWD',UPPER(SUBSTR(T1.JOB_CD,1,4)),T1.DATA_SRC_CD),'EVT_MYLS_EVENT_DTL') as etl_task_name 
,t1.etl_dt as etl_dt
...
from ${idl_schema}.hdws_iml_evt_myls_event_dtl t1
where etl_dt <= to_date('${batch_date}','yyyymmdd') and etl_dt >= trunc(to_date('${batch_date}','yyyymmdd'),'mm');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/hdws_iml_evt_myls_event_dtl_m.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes