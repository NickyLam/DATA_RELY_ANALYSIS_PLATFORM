...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_pirs_bdms_cpes_redsct_details_f
CreateDate: 20180529
FileName:   ${iel_data_path}/pirs_bdms_cpes_redsct_details.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,replace(replace(t1.is_discount,chr(13),''),chr(10),'') as is_discount
,replace(replace(t1.is_allopatric,chr(13),''),chr(10),'') as is_allopatric
,replace(replace(t1.is_meet_policy,chr(13),''),chr(10),'') as is_meet_policy
,replace(replace(t1.is_refuse,chr(13),''),chr(10),'') as is_refuse
...
,replace(replace(t1.process_code,chr(13),''),chr(10),'') as process_code
,replace(replace(t1.process_msg,chr(13),''),chr(10),'') as process_msg
,'' as data_date
from ${iol_schema}.bdms_cpes_redsct_details t1
where t1.start_dt <= to_date('${batch_date}','yyyymmdd') and t1.end_dt > to_date('${batch_date}','yyyymmdd');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/pirs_bdms_cpes_redsct_details.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes