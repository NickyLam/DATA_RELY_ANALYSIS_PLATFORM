...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_hdws_iml_agt_eacct_info_f
CreateDate: 20180529
FileName:   ${iel_data_path}/hdws_iml_agt_eacct_info.f.${batch_date}.dat
IF_mark:    f
Logs:
...
,NVL2(t1.data_src_cd,'AGT_EACCT_INFO'||'_'||DECODE(T1.DATA_SRC_CD,'LHWD',UPPER(SUBSTR(T1.JOB_CD,1,4)),T1.DATA_SRC_CD),'AGT_EACCT_INFO') as etl_task_name
,replace(replace(t1.open_tm,chr(13),''),chr(10),'') as open_tm
from ${idl_schema}.hdws_iml_agt_eacct_info t1
where etl_dt = to_date('${batch_date}','yyyymmdd');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/hdws_iml_agt_eacct_info.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes