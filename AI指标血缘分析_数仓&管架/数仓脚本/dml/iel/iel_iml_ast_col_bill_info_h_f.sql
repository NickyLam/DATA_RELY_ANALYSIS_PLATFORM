...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_ast_col_bill_info_h_f
CreateDate: 20230512
FileName:   ${iel_data_path}/ast_col_bill_info_h.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.drawer_open_bank_bank_id,chr(13),''),chr(10),'') as drawer_open_bank_bank_id
...
from ${iml_schema}.ast_col_bill_info_h t1
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd')" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/ast_col_bill_info_h.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
