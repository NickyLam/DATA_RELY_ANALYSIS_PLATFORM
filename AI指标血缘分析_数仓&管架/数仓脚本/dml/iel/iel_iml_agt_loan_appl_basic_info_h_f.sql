...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iml_agt_loan_appl_basic_info_h_f
CreateDate: 20250314
FileName:   ${iel_data_path}/agt_loan_appl_basic_info_h.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.data_input_integy_flg,chr(13),''),chr(10),'') as data_input_integy_flg
...
,appl_amt
,replace(replace(t1.lmt_base_prod_id,chr(13),''),chr(10),'') as lmt_base_prod_id
,replace(replace(t1.prod_id,chr(13),''),chr(10),'') as prod_id
,replace(replace(t1.prod_policy_id,chr(13),''),chr(10),'') as prod_policy_id
,replace(replace(t1.prod_policy_edit_id,chr(13),''),chr(10),'') as prod_policy_edit_id
...
,replace(replace(t1.rela_regroup_prop_id,chr(13),''),chr(10),'') as rela_regroup_prop_id
...
,file_dt
,replace(replace(t1.rgst_reply_flg,chr(13),''),chr(10),'') as rgst_reply_flg
,b_renew_amt
,lmt_open_amt
,replace(replace(t1.int_rat_adj_ped_cd,chr(13),''),chr(10),'') as int_rat_adj_ped_cd
,b_renew_exec_year_int_rat
,replace(replace(t1.core_out_acct_org_id,chr(13),''),chr(10),'') as core_out_acct_org_id
,replace(replace(t1.crdt_org_way_cd,chr(13),''),chr(10),'') as crdt_org_way_cd
,replace(replace(t1.intd_bd_flg,chr(13),''),chr(10),'') as intd_bd_flg
,b_renew_exp_dt
...
where start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd')" \
        ...
        file="${iel_data_path}/agt_loan_appl_basic_info_h.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes
