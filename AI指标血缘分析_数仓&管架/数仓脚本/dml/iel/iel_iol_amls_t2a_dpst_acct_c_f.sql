...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_amls_t2a_dpst_acct_c_f
CreateDate: 20180529
FileName:   ${iel_data_path}/amls_t2a_dpst_acct_c.f.${batch_date}.dat
IF_mark:    f
Logs:
...
' \
    ...
    ,replace(replace(t.base_acct_type,chr(13),''),chr(10),'') as base_acct_type
    ,replace(replace(t.ib_property_cd,chr(13),''),chr(10),'') as ib_property_cd
...
from iol.amls_t2a_dpst_acct_c t
  where t.start_dt <= to_date('${batch_date}','yyyymmdd') and t.end_dt > to_date('${batch_date}','yyyymmdd') " \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/amls_t2a_dpst_acct_c.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes