...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_iol_rcds_ir_tzbl_zl_f
CreateDate: 20180529
FileName:   ${iel_data_path}/rcds_ir_tzbl_zl.f.${batch_date}.dat
IF_mark:    f
Logs:
...
' \
    ...
    ,t.var0000 as var0000
    ,replace(replace(t.var0001,chr(13),''),chr(10),'') as var0001
    ,t.start_dt as start_dt
    ,t.end_dt as end_dt
    ,replace(replace(t.id_mark,chr(13),''),chr(10),'') as id_mark
from iol.rcds_ir_tzbl_zl t
  where t.start_dt <= to_date('${batch_date}','yyyymmdd') and t.end_dt > to_date('${batch_date}','yyyymmdd') " \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/rcds_ir_tzbl_zl.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes