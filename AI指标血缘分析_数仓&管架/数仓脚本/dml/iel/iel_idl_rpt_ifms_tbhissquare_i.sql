: '
Purpose:    unload config for sqluldr2
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_rpt_ifms_tbhissquare_i
CreateDate: 20180529
FileName:   ${iel_data_path}/rpt_ifms_tbhissquare.i.${batch_date}.dat
IF_mark:    i
Logs:
   zjj 2018-07-27 create template
' \
        query="select
 to_date('${batch_date}','yyyymmdd') as etl_dt
,replace(replace(t1.square_no,chr(13),''),chr(10),'') as square_no
,t1.seq_no as seq_no
,t1.trans_date as trans_date
,t1.clear_date as clear_date
,t1.square_date as square_date
,t1.old_square_date as old_square_date
,replace(replace(t1.serial_no,chr(13),''),chr(10),'') as serial_no
,replace(replace(t1.asso_serial,chr(13),''),chr(10),'') as asso_serial
,replace(replace(t1.from_flag,chr(13),''),chr(10),'') as from_flag
,replace(replace(t1.trans_code,chr(13),''),chr(10),'') as trans_code
,replace(replace(t1.busin_code,chr(13),''),chr(10),'') as busin_code
,replace(replace(t1.client_type,chr(13),''),chr(10),'') as client_type
,replace(replace(t1.in_client_no,chr(13),''),chr(10),'') as in_client_no
,replace(replace(t1.bank_no,chr(13),''),chr(10),'') as bank_no
,replace(replace(t1.client_no,chr(13),''),chr(10),'') as client_no
,replace(replace(t1.bank_acc,chr(13),''),chr(10),'') as bank_acc
,replace(replace(t1.bank_acc_kind,chr(13),''),chr(10),'') as bank_acc_kind
,replace(replace(t1.channel,chr(13),''),chr(10),'') as channel
,replace(replace(t1.oper_no,chr(13),''),chr(10),'') as oper_no
,replace(replace(t1.term_no,chr(13),''),chr(10),'') as term_no
,replace(replace(t1.branch_no,chr(13),''),chr(10),'') as branch_no
,replace(replace(t1.open_branch,chr(13),''),chr(10),'') as open_branch
,replace(replace(t1.ta_code,chr(13),''),chr(10),'') as ta_code
,replace(replace(t1.prd_code,chr(13),''),chr(10),'') as prd_code
,replace(replace(t1.liqu_dir,chr(13),''),chr(10),'') as liqu_dir
,t1.amt as amt
,replace(replace(t1.curr_type,chr(13),''),chr(10),'') as curr_type
,replace(replace(t1.cash_flag,chr(13),''),chr(10),'') as cash_flag
,t1.unfrozen_amt as unfrozen_amt
,replace(replace(t1.host_trans_code,chr(13),''),chr(10),'') as host_trans_code
,t1.host_date as host_date
,replace(replace(t1.host_serial,chr(13),''),chr(10),'') as host_serial
,t1.frozen_amt as frozen_amt
,replace(replace(t1.check_status,chr(13),''),chr(10),'') as check_status
,replace(replace(t1.distrib_flag,chr(13),''),chr(10),'') as distrib_flag
,replace(replace(t1.amt_flag,chr(13),''),chr(10),'') as amt_flag
,replace(replace(t1.cost_income_flag,chr(13),''),chr(10),'') as cost_income_flag
,t1.cfm_vol as cfm_vol
,t1.cost as cost
,t1.cfm_income as cfm_income
,t1.vol_cumulate as vol_cumulate
,replace(replace(t1.prd_account,chr(13),''),chr(10),'') as prd_account
,replace(replace(t1.prd_account_kind,chr(13),''),chr(10),'') as prd_account_kind
,replace(replace(t1.summary,chr(13),''),chr(10),'') as summary
,replace(replace(t1.status,chr(13),''),chr(10),'') as status
,replace(replace(t1.old_square_no,chr(13),''),chr(10),'') as old_square_no
,replace(replace(t1.err_code,chr(13),''),chr(10),'') as err_code
,replace(replace(t1.err_msg,chr(13),''),chr(10),'') as err_msg
,replace(replace(t1.deal_status,chr(13),''),chr(10),'') as deal_status
,t1.amt1 as amt1
,t1.amt2 as amt2
,t1.amt3 as amt3
,replace(replace(t1.reserve1,chr(13),''),chr(10),'') as reserve1
,replace(replace(t1.reserve2,chr(13),''),chr(10),'') as reserve2
,replace(replace(t1.reserve3,chr(13),''),chr(10),'') as reserve3
,replace(replace(t1.reserve4,chr(13),''),chr(10),'') as reserve4
,replace(replace(t1.reserve5,chr(13),''),chr(10),'') as reserve5
 from iol.ifms_tbhissquare T1
where square_date=${batch_date} and start_dt <= to_date('${batch_date}','yyyymmdd') and end_dt > to_date('${batch_date}','yyyymmdd');" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/rpt_ifms_tbhissquare.i.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes