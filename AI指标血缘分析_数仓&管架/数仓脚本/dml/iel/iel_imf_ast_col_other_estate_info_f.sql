...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_imf_ast_col_other_estate_info_f
CreateDate: 20251010
FileName:   ${iel_data_path}/ast_col_other_estate_info.f.${batch_date}.dat
IF_mark:    f
Logs:
' \
        query="select
...
,replace(replace(t1.ready_house_flg,chr(13),''),chr(10),'') as ready_house_flg
...
from ${iml_schema}.ast_col_other_estate_info t1
where create_dt <= to_date('${batch_date}','yyyymmdd') and id_mark<>'D'" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/ast_col_other_estate_info.f.${batch_date}.dat" \
        charset=utf8
        safe=yes
