...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iel_idl_hdws_dul_d_gzfh_agt_bil_buy_sell_info_f
CreateDate: 20180529
FileName:   ${iel_data_path}/hdws_dul_d_gzfh_agt_bil_buy_sell_info.f.${batch_date}.dat
IF_mark:    f
Logs:
...
' \
      ...
      ,replace(replace(bil_biz_typ_cd,chr(10),''),chr(13),'') as bil_biz_typ_cd
      ,replace(replace(bil_forehand_flg,chr(10),''),chr(13),'') as bil_forehand_flg
      ,replace(replace(bil_bef_hand_name,chr(10),''),chr(13),'') as bil_bef_hand_name
      ,replace(replace(bil_forehand_typ_cd,chr(10),''),chr(13),'') as bil_forehand_typ_cd
      ,replace(replace(bil_contr_id,chr(10),''),chr(13),'') as bil_contr_id
      ,contr_due_dt
      ,replace(replace(txn_cnte_pty_ibank_num,chr(10),''),chr(13),'') as txn_cnte_pty_ibank_num
      ,replace(replace(cntrpty_name,chr(10),''),chr(13),'') as cntrpty_name
      ,replace(replace(acct_seq_num,chr(10),''),chr(13),'') as acct_seq_num 
from idl.hdws_dul_d_gzfh_agt_bil_buy_sell_info 
where to_char(etl_dt,'yyyymmdd') = '${batch_date}';" \
        field="0x1b" record="0x0a"  \
        file="${iel_data_path}/hdws_dul_d_gzfh_agt_bil_buy_sell_info.f.${batch_date}.dat" \
        charset=zhs16gbk
        safe=yes