...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_ast_col_cnstring_proj_info_icmsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
drop table ${iml_schema}.ast_col_cnstring_proj_info_icmsf1_tm purge;
drop table ${iml_schema}.ast_col_cnstring_proj_info_icmsf1_ex purge;

-- 2.2 add partition for target table
alter table ${iml_schema}.ast_col_cnstring_proj_info add partition p_icmsf1 values ('icmsf1')(
        subpartition p_icmsf1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
    )
;
alter table ${iml_schema}.ast_col_cnstring_proj_info modify partition p_icmsf1
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.ast_col_cnstring_proj_info_icmsf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.ast_col_cnstring_proj_info partition for ('icmsf1')
where create_dt < to_date('${batch_date}','yyyymmdd')
;

-- 2.4 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.ast_col_cnstring_proj_info_icmsf1_tm
compress ${option_switch} for query high
as
select
    asset_id -- 资产编号
...
create table ${iml_schema}.ast_col_cnstring_proj_info_icmsf1_ex nologging
compress ${option_switch} for query high
as select * from ${iml_schema}.ast_col_cnstring_proj_info partition for ('icmsf1') where 0=1;

-- 2.1 insert data to tm table
-- icms_clr_asset_property_construction-
insert into ${iml_schema}.ast_col_cnstring_proj_info_icmsf1_tm(
    asset_id -- 资产编号
    ,lp_id -- 法人编号
    ,rel_esat_wat_id -- 不动产证号
    ,land_char_cd -- 土地所有权性质代码
...
select
    ...
    ,P1.STARTWORKDATE -- 开工日期
    ,P1.PRESTARTDATE -- 预计竣工日期
    ...
    ,'icms_clr_asset_property_construction' -- 源表名称
    ,'icmsf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.icms_clr_asset_property_construction p1
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.ast_col_cnstring_proj_info_icmsf1_tm 
  	                                group by 
  	                                        asset_id
  	                                        ,lp_id
  	                               having count(1) > 1);
...
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${iml_schema}.ast_col_cnstring_proj_info_icmsf1_ex(
    asset_id -- 资产编号
    ,lp_id -- 法人编号
    ,rel_esat_wat_id -- 不动产证号
    ,land_char_cd -- 土地所有权性质代码
...
select
    ...
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iml_schema}.ast_col_cnstring_proj_info_icmsf1_tm n
    full join ${iml_schema}.ast_col_cnstring_proj_info_icmsf1_bk o
        on
...
whenever sqlerror exit sql.sqlcode;
alter table ${iml_schema}.ast_col_cnstring_proj_info truncate partition for ('icmsf1') reuse storage;

-- 3.2 exchage tm table and target table
alter table ${iml_schema}.ast_col_cnstring_proj_info exchange subpartition p_icmsf1_${batch_date} with table ${iml_schema}.ast_col_cnstring_proj_info_icmsf1_ex;

-- 3.3 drop subpartition last_date partition
whenever sqlerror continue none ;
alter table ${iml_schema}.ast_col_cnstring_proj_info drop subpartition p_icmsf1_${last_date} update global indexes;
...
drop table ${iml_schema}.ast_col_cnstring_proj_info_icmsf1_tm purge;
drop table ${iml_schema}.ast_col_cnstring_proj_info_icmsf1_ex purge;
drop table ${iml_schema}.ast_col_cnstring_proj_info_icmsf1_bk purge;

-- 5 gater table status
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'ast_col_cnstring_proj_info', partname => 'p_icmsf1_${batch_date}', ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);