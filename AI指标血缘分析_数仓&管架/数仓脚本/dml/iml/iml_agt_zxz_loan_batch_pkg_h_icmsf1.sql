...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_agt_zxz_loan_batch_pkg_h_icmsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.agt_zxz_loan_batch_pkg_h add partition p_icmsf1 values ('icmsf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.agt_zxz_loan_batch_pkg_h_icmsf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_zxz_loan_batch_pkg_h partition for ('icmsf1')
...
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.agt_zxz_loan_batch_pkg_h_icmsf1_tm purge;
drop table ${iml_schema}.agt_zxz_loan_batch_pkg_h_icmsf1_op purge;
drop table ${iml_schema}.agt_zxz_loan_batch_pkg_h_icmsf1_cl purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.agt_zxz_loan_batch_pkg_h_icmsf1_tm nologging
compress ${option_switch} for query high
as select
    agt_id -- 协议编号
    ,lp_id -- 法人编号
...
from ${iml_schema}.agt_zxz_loan_batch_pkg_h partition for ('icmsf1')
where 0=1
;

create table ${iml_schema}.agt_zxz_loan_batch_pkg_h_icmsf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_zxz_loan_batch_pkg_h partition for ('icmsf1') where 0=1;

create table ${iml_schema}.agt_zxz_loan_batch_pkg_h_icmsf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_zxz_loan_batch_pkg_h partition for ('icmsf1') where 0=1;
...
insert into ${iml_schema}.agt_zxz_loan_batch_pkg_h_icmsf1_tm(
    agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,batch_pkg_id -- 批次包编号
    ,batch_pkg_name -- 批次包名称
...
select
    ...
    ,nvl(trim(P1.PACKAGEFLAG),'_') -- 批次包标识代码
    ,P1.RELPACKAGENAME -- 关联批次包名称
    ,P1.RELPACKAGENO -- 关联批次包编号
    ,P1.USEDESC -- 使用要求描述
    ,DECODE(P1.PACKAGESTATUS,'1','1','2','0') -- 有效标志
...
from ${iol_schema}.icms_zxz_package_info p1
    left join ${iml_schema}.ref_pub_cd_map r1 on P1.ZXZLOANMODE = R1.SRC_CODE_VAL
        ...
        AND R1.TARGET_TAB_EN_NAME= 'AGT_ZXZ_LOAN_BATCH_PKG_H'
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.agt_zxz_loan_batch_pkg_h_icmsf1_tm 
  	                                group by 
  	                                        agt_id
  	                                        ,lp_id
  	                                        ,batch_pkg_id
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.agt_zxz_loan_batch_pkg_h_icmsf1_cl(
            agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,batch_pkg_id -- 批次包编号
    ,batch_pkg_name -- 批次包名称
    ...
    ,rgst_org_id -- 登记机构编号
        ...
        into ${iml_schema}.agt_zxz_loan_batch_pkg_h_icmsf1_op(
            agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,batch_pkg_id -- 批次包编号
    ,batch_pkg_name -- 批次包名称
...
select
    ...
    ,nvl(n.refac_kind_descb, o.refac_kind_descb) as refac_kind_descb -- 再贷款种类描述
...
from ${iml_schema}.agt_zxz_loan_batch_pkg_h_icmsf1_tm n
    full join (select * from ${iml_schema}.agt_zxz_loan_batch_pkg_h_icmsf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
            o.agt_id = n.agt_id
            and o.lp_id = n.lp_id
...
from ${iml_schema}.agt_zxz_loan_batch_pkg_h_icmsf1_bk o
    left join ${iml_schema}.agt_zxz_loan_batch_pkg_h_icmsf1_op n
        on
            o.agt_id = n.agt_id
            and o.lp_id = n.lp_id
            and o.batch_pkg_id = n.batch_pkg_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iml_schema}.agt_zxz_loan_batch_pkg_h_icmsf1_cl d
        on
            o.agt_id = d.agt_id
            and o.lp_id = d.lp_id
...
--truncate table ${iml_schema}.agt_zxz_loan_batch_pkg_h;
--alter table ${iml_schema}.agt_zxz_loan_batch_pkg_h truncate partition for ('icmsf1') reuse storage;
...
for cur in (select subpartition_name 
             ...
             where table_owner=upper('${iml_schema}') 
               and table_name=upper('agt_zxz_loan_batch_pkg_h') 
               and substr(subpartition_name,1,8)=upper('p_icmsf1')
               and substr(subpartition_name,10) >= '${batch_date}' 
               and substr(subpartition_name,10) < '20991231') loop
    v_sql := 'alter table ${iml_schema}.agt_zxz_loan_batch_pkg_h drop subpartition ' || cur.subpartition_name;
...
alter table ${iml_schema}.agt_zxz_loan_batch_pkg_h modify partition p_icmsf1 
add subpartition p_icmsf1_${batch_date} values(to_date('${batch_date}','YYYYMMDD')) ;

-- 4.3 exchange partition
alter table ${iml_schema}.agt_zxz_loan_batch_pkg_h exchange subpartition p_icmsf1_${batch_date} with table ${iml_schema}.agt_zxz_loan_batch_pkg_h_icmsf1_cl;
alter table ${iml_schema}.agt_zxz_loan_batch_pkg_h exchange subpartition p_icmsf1_20991231 with table ${iml_schema}.agt_zxz_loan_batch_pkg_h_icmsf1_op;

-- 5.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.agt_zxz_loan_batch_pkg_h to ${iml_schema};
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.agt_zxz_loan_batch_pkg_h_icmsf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'agt_zxz_loan_batch_pkg_h', partname => 'p_icmsf1_20991231',ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
