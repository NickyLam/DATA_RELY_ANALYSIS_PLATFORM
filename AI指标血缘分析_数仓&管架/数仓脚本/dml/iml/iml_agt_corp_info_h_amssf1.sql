...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_agt_corp_info_h_amssf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.agt_corp_info_h add partition p_amssf1 values ('amssf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.agt_corp_info_h_amssf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_corp_info_h partition for ('amssf1')
...
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.agt_corp_info_h_amssf1_tm purge;
drop table ${iml_schema}.agt_corp_info_h_amssf1_op purge;
drop table ${iml_schema}.agt_corp_info_h_amssf1_cl purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.agt_corp_info_h_amssf1_tm nologging
compress ${option_switch} for query high
as select
    agt_id -- 协议编号
    ,lp_id -- 法人编号
...
from ${iml_schema}.agt_corp_info_h partition for ('amssf1')
where 0=1
;

create table ${iml_schema}.agt_corp_info_h_amssf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_corp_info_h partition for ('amssf1') where 0=1;

create table ${iml_schema}.agt_corp_info_h_amssf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_corp_info_h partition for ('amssf1') where 0=1;
...
-- amss_tbl_fund_info-
insert into ${iml_schema}.agt_corp_info_h_amssf1_tm(
    agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,agt_corp_id -- 协议单位编号
    ,agt_corp_name -- 协议单位名称
...
from ${iol_schema}.amss_tbl_fund_info p1
    ...
    left join ${iml_schema}.ref_pub_cd_map r3 on P1.FUND_STATUS = R3.SRC_CODE_VAL
        ...
        AND R3.SRC_TAB_EN_NAME= 'AMSS_TBL_FUND_INFO'
    ...
    left join ${iml_schema}.ref_pub_cd_map r4 on P1.RCV_ACCT_TYPE = R4.SRC_CODE_VAL
        ...
        AND R4.SRC_TAB_EN_NAME= 'AMSS_TBL_FUND_INFO'
    ...
    left join ${iml_schema}.ref_pub_cd_map r5 on P1.CUSHION_ACCT_TYPE = R5.SRC_CODE_VAL
        AND R5.SORC_SYS_CD= 'AMSS'
        AND R5.SRC_TAB_EN_NAME= 'AMSS_TBL_FUND_INFO'
...
select
    ...
    ,P1.FUND_ID -- 协议单位编号
    ,P1.FUND_NAME -- 协议单位名称
    ,P1.FUND_SNAME -- 协议单位简称
    ,P1.MGMT_PLATF_CHN -- 代付渠道编号
    ,' ' -- 代理商编号
    ,'1' -- 协议单位类型代码
    ,P1.BANK_ORG_CODE -- 所属机构编号
    ,P1.OPER_LICENCE_NUM -- 营业执照编号
    ,${iml_schema}.timeformat_max2(P1.OPER_LICENCE_VALID_DT) -- 营业执照截止有效日期
    ,P1.LP_NAME -- 法人姓名
    ,'0000' -- 证件类型代码
    ,' ' -- 证件号码
    ,P1.LP_CEPH_NUM -- 手机号码
    ,' ' -- 电子邮箱
    ,P1.LNKM_NAME -- 联系人名称
    ,nvl(trim(P1.LNKM_IDEN_TYPE),'0000') -- 联系人证件类型代码
    ,P1.LNKM_IDEN_NUM -- 联系人证件号码
    ,P1.LNKM_CEPH_NUM -- 联系人电话号码
    ,P1.EMAIL -- 邮政编码
    ,P1.LNKM_ADDRESS -- 联系人地址
    ,CASE WHEN R1.TARGET_CD_VAL IS NOT NULL THEN R1.TARGET_CD_VAL ELSE '@'||P1.ACCT_TYPE END -- 结算账户类型代码
    ,P1.ACCT_NUM -- 结算账户编号
    ,P1.ACCT_NAME -- 结算账户名称
    ,'-' -- 本行入账标志
    ,P1.OPEN_BK_NUM -- 开户行行号
    ,P1.OPEN_BK_NAME -- 开户行行名称
    ,P1.OPEN_BK_ADDRESS -- 开户地址
    ,CASE WHEN R3.TARGET_CD_VAL IS NOT NULL THEN R3.TARGET_CD_VAL ELSE '@'||P1.EXAMINE_STATUS END -- 协议单位状态代码
    ,'-' -- 收款账户类型代码
    ,' ' -- 收款账户编号
    ,' ' -- 收款账户名称
    ,${iml_schema}.timeformat_min(null) -- 新增日期
    ,${iml_schema}.timeformat_max2(null) -- 最后修改日期
    ,' ' -- 操作柜员编号
    ,' ' -- API系统标识
    ,'-' -- 垫资账户类型代码
    ,' ' -- 垫资账户编号
    ...
    ,0 -- 垫资金额
    ,0 -- 上次使用额度
    ,P1.SMS_CEPH_NUM -- 短信通知手机号码
    ,P1.SMS_NAME -- 短信通知姓名
    ,case when P1.PHYSICS_FLAG = 1 then '1' else '0' end -- 有效标志
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,'amss_union_pay_fund' -- 源表名称
    ,'amssf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.amss_union_pay_fund p1
    left join ${iml_schema}.ref_pub_cd_map r1 on P1.ACCT_TYPE = R1.SRC_CODE_VAL
        AND R1.SORC_SYS_CD= 'AMSS'
        AND R1.SRC_TAB_EN_NAME= 'AMSS_UNION_PAY_FUND'
        AND R1.SRC_FIELD_EN_NAME= 'ACCT_TYPE'
        AND R1.TARGET_TAB_EN_NAME= 'AGT_CORP_INFO_H'
        AND R1.TARGET_TAB_FIELD_EN_NAME= 'STL_ACCT_TYPE_CD'
    left join ${iml_schema}.ref_pub_cd_map r3 on P1.EXAMINE_STATUS = R3.SRC_CODE_VAL
        AND R3.SORC_SYS_CD= 'AMSS'
        AND R3.SRC_TAB_EN_NAME= 'AMSS_UNION_PAY_FUND'
        AND R3.SRC_FIELD_EN_NAME= 'EXAMINE_STATUS'
        AND R3.TARGET_TAB_EN_NAME= 'AGT_CORP_INFO_H'
        AND R3.TARGET_TAB_FIELD_EN_NAME= 'AGT_CORP_STATUS_CD'
where p1.start_dt <= to_date('${batch_date}','yyyymmdd') and p1.end_dt > to_date('${batch_date}','yyyymmdd')
;
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.agt_corp_info_h_amssf1_tm 
  	                                group by 
  	                                        agt_id
  	                                        ,lp_id
  	                               having count(1) > 1);
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.agt_corp_info_h_amssf1_cl(
            agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,agt_corp_id -- 协议单位编号
    ,agt_corp_name -- 协议单位名称
    ...
    ,valid_flg -- 有效标志
        ...
        into ${iml_schema}.agt_corp_info_h_amssf1_op(
            agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,agt_corp_id -- 协议单位编号
    ,agt_corp_name -- 协议单位名称
...
from ${iml_schema}.agt_corp_info_h_amssf1_tm n
    full join (select * from ${iml_schema}.agt_corp_info_h_amssf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
            o.agt_id = n.agt_id
            and o.lp_id = n.lp_id
where (
...
from ${iml_schema}.agt_corp_info_h_amssf1_bk o
    left join ${iml_schema}.agt_corp_info_h_amssf1_op n
        on
            o.agt_id = n.agt_id
            and o.lp_id = n.lp_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iml_schema}.agt_corp_info_h_amssf1_cl d
        on
            o.agt_id = d.agt_id
            and o.lp_id = d.lp_id
...
--alter table ${iml_schema}.agt_corp_info_h truncate partition for ('amssf1') reuse storage;
...
whenever sqlerror exit sql.sqlcode;
alter table ${iml_schema}.agt_corp_info_h modify partition p_amssf1 
add subpartition p_amssf1_${batch_date} values(to_date('${batch_date}','YYYYMMDD')) ;

-- 4.3 exchange partition
alter table ${iml_schema}.agt_corp_info_h exchange subpartition p_amssf1_${batch_date} with table ${iml_schema}.agt_corp_info_h_amssf1_cl;
alter table ${iml_schema}.agt_corp_info_h exchange subpartition p_amssf1_20991231 with table ${iml_schema}.agt_corp_info_h_amssf1_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.agt_corp_info_h_amssf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'agt_corp_info_h', partname => 'p_amssf1_20991231',ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
