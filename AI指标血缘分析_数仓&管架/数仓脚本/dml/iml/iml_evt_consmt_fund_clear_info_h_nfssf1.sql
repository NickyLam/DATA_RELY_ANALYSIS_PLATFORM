...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_evt_consmt_fund_clear_info_h_nfssf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.evt_consmt_fund_clear_info_h add partition p_nfssf1 values ('nfssf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.evt_consmt_fund_clear_info_h_nfssf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.evt_consmt_fund_clear_info_h partition for ('nfssf1')
...
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.evt_consmt_fund_clear_info_h_nfssf1_tm purge;
drop table ${iml_schema}.evt_consmt_fund_clear_info_h_nfssf1_op purge;
drop table ${iml_schema}.evt_consmt_fund_clear_info_h_nfssf1_cl purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.evt_consmt_fund_clear_info_h_nfssf1_tm nologging
compress ${option_switch} for query high
as select
    evt_id -- 事件编号
    ,lp_id -- 法人编号
...
from ${iml_schema}.evt_consmt_fund_clear_info_h partition for ('nfssf1')
where 0=1
;

create table ${iml_schema}.evt_consmt_fund_clear_info_h_nfssf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.evt_consmt_fund_clear_info_h partition for ('nfssf1') where 0=1;

create table ${iml_schema}.evt_consmt_fund_clear_info_h_nfssf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.evt_consmt_fund_clear_info_h partition for ('nfssf1') where 0=1;
...
-- nfss_tbsquare-
insert into ${iml_schema}.evt_consmt_fund_clear_info_h_nfssf1_tm(
    evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,clear_flow_num -- 清算流水号
    ,tran_dt -- 交易日期
...
select
    ...
    ,CASE WHEN R4.TARGET_CD_VAL IS NOT NULL THEN R4.TARGET_CD_VAL ELSE '@'||P1.CHANNEL END -- 交易渠道编号
    ,P1.OPER_NO -- 交易柜员编号
    ,P1.TERM_NO -- 交易终端编号
    ,P1.BRANCH_NO -- 交易机构编号
...
from ${iol_schema}.nfss_tbsquare p1
    ...
    left join ${iml_schema}.ref_pub_cd_map r2 on P1.LIQU_DIR= R2.SRC_CODE_VAL
        ...
        AND R2.SRC_TAB_EN_NAME= 'NFSS_TBSQUARE'
        AND R2.SRC_FIELD_EN_NAME= 'LIQU_DIR'
        AND R2.TARGET_TAB_EN_NAME= 'EVT_CONSMT_FUND_CLEAR_INFO_H'
    ...
    left join ${iml_schema}.ref_pub_cd_map r3 on P1.CURR_TYPE= R3.SRC_CODE_VAL
        ...
        AND R3.SRC_TAB_EN_NAME= 'NFSS_TBSQUARE'
        AND R3.SRC_FIELD_EN_NAME= 'CURR_TYPE'
        AND R3.TARGET_TAB_EN_NAME= 'EVT_CONSMT_FUND_CLEAR_INFO_H'
   ...
   left join ${iml_schema}.ref_pub_cd_map r4 on P1.CHANNEL= R4.SRC_CODE_VAL
        ...
        AND R4.SRC_TAB_EN_NAME= 'NFSS_TBSQUARE'
        AND R4.SRC_FIELD_EN_NAME= 'CHANNEL'
        AND R4.TARGET_TAB_EN_NAME= 'EVT_CONSMT_FUND_CLEAR_INFO_H'
...
select
    ...
    ,NVL(TRIM(P1.DEAL_STATUS),'-') -- 接口处理标志代码
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,'nfss_tbhissquare' -- 源表名称
    ,'nfssf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.nfss_tbhissquare p1
    left join ${iml_schema}.ref_pub_cd_map r1 on P1.CLIENT_TYPE= R1.SRC_CODE_VAL
        AND R1.SORC_SYS_CD= 'NFSS'
        AND R1.SRC_TAB_EN_NAME= 'NFSS_TBHISSQUARE'
        AND R1.SRC_FIELD_EN_NAME= 'CLIENT_TYPE'
        AND R1.TARGET_TAB_EN_NAME= 'EVT_CONSMT_FUND_CLEAR_INFO_H'
        AND R1.TARGET_TAB_FIELD_EN_NAME= 'CUST_TYPE_CD'
    left join ${iml_schema}.ref_pub_cd_map r2 on P1.LIQU_DIR= R2.SRC_CODE_VAL
        AND R2.SORC_SYS_CD= 'NFSS'
        AND R2.SRC_TAB_EN_NAME= 'NFSS_TBHISSQUARE'
        AND R2.SRC_FIELD_EN_NAME= 'LIQU_DIR'
        AND R2.TARGET_TAB_EN_NAME= 'EVT_CONSMT_FUND_CLEAR_INFO_H'
        AND R2.TARGET_TAB_FIELD_EN_NAME= 'ACCT_DIR_CD'
    left join ${iml_schema}.ref_pub_cd_map r3 on P1.CURR_TYPE= R3.SRC_CODE_VAL
        AND R3.SORC_SYS_CD= 'NFSS'
        AND R3.SRC_TAB_EN_NAME= 'NFSS_TBHISSQUARE'
        AND R3.SRC_FIELD_EN_NAME= 'CURR_TYPE'
        AND R3.TARGET_TAB_EN_NAME= 'EVT_CONSMT_FUND_CLEAR_INFO_H'
        AND R3.TARGET_TAB_FIELD_EN_NAME= 'CURR_CD'
   left join ${iml_schema}.ref_pub_cd_map r4 on P1.CHANNEL= R4.SRC_CODE_VAL
        AND R4.SORC_SYS_CD= 'NFSS'
        AND R4.SRC_TAB_EN_NAME= 'NFSS_TBHISSQUARE'
        AND R4.SRC_FIELD_EN_NAME= 'CHANNEL'
        AND R4.TARGET_TAB_EN_NAME= 'EVT_CONSMT_FUND_CLEAR_INFO_H'
        AND R4.TARGET_TAB_FIELD_EN_NAME= 'TRAN_CHN_CD' 
where p1.start_dt <= to_date('${batch_date}','yyyymmdd') and p1.end_dt > to_date('${batch_date}','yyyymmdd')
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.evt_consmt_fund_clear_info_h_nfssf1_tm 
  	                                group by 
  	                                        evt_id
  	                                        ,lp_id
  	                               having count(1) > 1);
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.evt_consmt_fund_clear_info_h_nfssf1_cl(
            evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,clear_flow_num -- 清算流水号
    ,tran_dt -- 交易日期
    ...
    ,intfc_proc_flg_cd -- 接口处理标志代码
        ...
        into ${iml_schema}.evt_consmt_fund_clear_info_h_nfssf1_op(
            evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,clear_flow_num -- 清算流水号
    ,tran_dt -- 交易日期
...
from ${iml_schema}.evt_consmt_fund_clear_info_h_nfssf1_tm n
    full join (select * from ${iml_schema}.evt_consmt_fund_clear_info_h_nfssf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
            o.evt_id = n.evt_id
            and o.lp_id = n.lp_id
where (
...
from ${iml_schema}.evt_consmt_fund_clear_info_h_nfssf1_bk o
    left join ${iml_schema}.evt_consmt_fund_clear_info_h_nfssf1_op n
        on
            o.evt_id = n.evt_id
            and o.lp_id = n.lp_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iml_schema}.evt_consmt_fund_clear_info_h_nfssf1_cl d
        on
            o.evt_id = d.evt_id
            and o.lp_id = d.lp_id
...
--truncate table ${iml_schema}.evt_consmt_fund_clear_info_h;
--alter table ${iml_schema}.evt_consmt_fund_clear_info_h truncate partition for ('nfssf1') reuse storage;
...
for cur in (select subpartition_name 
             ...
             where table_owner=upper('${iml_schema}') 
               and table_name=upper('evt_consmt_fund_clear_info_h') 
               and substr(subpartition_name,1,8)=upper('p_nfssf1')
               and substr(subpartition_name,10) >= '${batch_date}' 
               and substr(subpartition_name,10) < '20991231') loop
    v_sql := 'alter table ${iml_schema}.evt_consmt_fund_clear_info_h drop subpartition ' || cur.subpartition_name;
...
alter table ${iml_schema}.evt_consmt_fund_clear_info_h modify partition p_nfssf1 
add subpartition p_nfssf1_${batch_date} values(to_date('${batch_date}','YYYYMMDD')) ;

-- 4.3 exchange partition
alter table ${iml_schema}.evt_consmt_fund_clear_info_h exchange subpartition p_nfssf1_${batch_date} with table ${iml_schema}.evt_consmt_fund_clear_info_h_nfssf1_cl;
alter table ${iml_schema}.evt_consmt_fund_clear_info_h exchange subpartition p_nfssf1_20991231 with table ${iml_schema}.evt_consmt_fund_clear_info_h_nfssf1_op;

-- 5.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.evt_consmt_fund_clear_info_h to ${iml_schema};
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.evt_consmt_fund_clear_info_h_nfssf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'evt_consmt_fund_clear_info_h', partname => 'p_nfssf1_20991231',ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
