/*
Purpose:    整合模型层-全量拉链脚本。切换后同业正常跑完一次批后，会再调账到当前表ibms_ttrd_accounting_secu_obj，
                       增量条件改为ETL_DT+1 (O层重抽到T+1的分区), 再重抽跑数
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_agt_secu_acct_accti_bal_h_ibmsf1_day0
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
select
 ...
 left join ${iol_schema}.ibms_vtrd_instrument_fund_mapping p2 
 ...
 where  1 = 1 
    AND P1.ETL_DT = TO_DATE('${batch_date}','yyyymmdd') + 1
;
commit;
