...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_evt_ppps_ps_pay_flow_pppsi1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.evt_ppps_ps_pay_flow add partition p_pppsi1 values ('pppsi1')(
        subpartition p_pppsi1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
    )
;
alter table ${iml_schema}.evt_ppps_ps_pay_flow modify partition p_pppsi1
...
for i in 0 .. 14 loop
    ...
    v_sql := 'select count(0) from user_tab_subpartitions where table_name = upper(''evt_ppps_ps_pay_flow'') and subpartition_name = ''P_PPPSI1_'||bat_dt||''' ';
    dbms_output.put_line(v_sql);
    execute immediate v_sql into v_p_exists;
    dbms_output.put_line(v_p_exists);
    -- exists patitions
    if v_p_exists = 1 then 
        v_sql := 'alter table iml.evt_ppps_ps_pay_flow truncate subpartition P_PPPSI1_'||bat_dt ;
        dbms_output.put_line(v_sql);
        execute immediate v_sql;
    dbms_output.put_line('BBB');  
    --no exists partitions  
    else 
        v_sql := 'alter table iml.evt_ppps_ps_pay_flow modify partition p_pppsi1 add subpartition P_PPPSI1_'||bat_dt||' values (to_date('''||bat_dt||''',''yyyymmdd'')) ';
        dbms_output.put_line(v_sql);
        execute immediate v_sql;
...
-- ppps_e_txn_payment-1
insert into ${iml_schema}.evt_ppps_ps_pay_flow(
    evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,tran_flow_num -- 交易流水号
    ...
    ,pay_mercht_abbr -- 付款商户简称
    ,pay_mercht_id -- 付款商户编号
    ,pay_mercht_name -- 付款商户名称
...
select
    ...
    ,nvl(trim(P1.HOST_STATUS),'UNKNOWN') -- 核心交易状态代码
    ,P1.CREATE_TIME -- 创建时间
    ,P1.UPDATE_TIME -- 最后更新时间
    ,P1.PYER_MRCHNT_SHRT_NM -- 付款商户简称
    ,P1.PYER_MRCHNT_NO -- 付款商户编号
    ,P1.PYER_MRCHNT_NM -- 付款商户名称
    ,P1.PYER_ACCT_NO_DE -- 付款方账户编号
    ,P1.PYER_ACCT_NM_DE -- 付款方账户名称
    ...
    ,P1.PYEE_BK_ID -- 收款方账户所属机构编号
    ,P1.PYEE_BK_NO_DE -- 收款方账户编号
    ,P1.PYEE_BK_NM_DE -- 收款方账户名称
    ,nvl(trim(P1.PYEE_ACCT_TP),'-') -- 收款方账户类型代码
    ,P1.PYEE_SGN_NO -- 收款方协议编号
    ,P1.CDTR_BANK_ID -- 收款方交易流水号
    ,P1.SYS_ID -- 系统流水号
    ,P1.SYS_RTN_CD -- 系统返回码
    ,P1.SYS_RTN_DESC -- 系统返回说明
    ,nvl(trim(P1.SYS_TYPE),'-') -- 系统类型代码
    ,to_date(P1.sys_date,'yyyy-mm-dd') as etl_dt -- ETL处理日期
    ,'ppps_e_txn_payment' -- 源表名称
    ,'pppsi1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.ppps_e_txn_payment p1
where  1 = 1 
...
-- 4.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.evt_ppps_ps_pay_flow to ${iml_schema};

-- 5 gater table status
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'evt_ppps_ps_pay_flow', partname => 'p_pppsi1_${batch_date}', ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);