...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_agt_hqd_pre_loan_appl_info_icmsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.agt_hqd_pre_loan_appl_info add partition p_icmsf1 values ('icmsf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.agt_hqd_pre_loan_appl_info_icmsf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_hqd_pre_loan_appl_info partition for ('icmsf1')
...
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.agt_hqd_pre_loan_appl_info_icmsf1_tm purge;
drop table ${iml_schema}.agt_hqd_pre_loan_appl_info_icmsf1_op purge;
drop table ${iml_schema}.agt_hqd_pre_loan_appl_info_icmsf1_cl purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.agt_hqd_pre_loan_appl_info_icmsf1_tm nologging
compress ${option_switch} for query high
as select
    appl_id -- 申请编号
    ,lp_id -- 法人编号
...
from ${iml_schema}.agt_hqd_pre_loan_appl_info partition for ('icmsf1')
where 0=1
;

create table ${iml_schema}.agt_hqd_pre_loan_appl_info_icmsf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_hqd_pre_loan_appl_info partition for ('icmsf1') where 0=1;

create table ${iml_schema}.agt_hqd_pre_loan_appl_info_icmsf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_hqd_pre_loan_appl_info partition for ('icmsf1') where 0=1;

-- 3.1 get new data into table
-- icms_hqd_iqp_loan_prior-1
insert into ${iml_schema}.agt_hqd_pre_loan_appl_info_icmsf1_tm(
    appl_id -- 申请编号
    ,lp_id -- 法人编号
    ,appl_flow_num -- 申请流水号
    ,crdt_appl_flow_num -- 信贷申请流水号
...
select
    ...
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,'icms_hqd_iqp_loan_prior' -- 源表名称
    ,'icmsf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.icms_hqd_iqp_loan_prior p1
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.agt_hqd_pre_loan_appl_info_icmsf1_tm 
  	                                group by 
  	                                        appl_id
  	                                        ,lp_id
  	                               having count(1) > 1);
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.agt_hqd_pre_loan_appl_info_icmsf1_cl(
            appl_id -- 申请编号
    ,lp_id -- 法人编号
    ,appl_flow_num -- 申请流水号
    ,crdt_appl_flow_num -- 信贷申请流水号
    ...
    ,advise_flg -- 通知展业标志
        ...
        into ${iml_schema}.agt_hqd_pre_loan_appl_info_icmsf1_op(
            appl_id -- 申请编号
    ,lp_id -- 法人编号
    ,appl_flow_num -- 申请流水号
    ,crdt_appl_flow_num -- 信贷申请流水号
...
select
    ...
    ,nvl(n.resd_dtl_addr, o.resd_dtl_addr) as resd_dtl_addr -- 居住详细地址
...
from ${iml_schema}.agt_hqd_pre_loan_appl_info_icmsf1_tm n
    full join (select * from ${iml_schema}.agt_hqd_pre_loan_appl_info_icmsf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
            o.appl_id = n.appl_id
            and o.lp_id = n.lp_id
where (
    ...
    or (
        ...
        or o.resd_dtl_addr <> n.resd_dtl_addr
...
from ${iml_schema}.agt_hqd_pre_loan_appl_info_icmsf1_bk o
    left join ${iml_schema}.agt_hqd_pre_loan_appl_info_icmsf1_op n
        on
            o.appl_id = n.appl_id
            and o.lp_id = n.lp_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iml_schema}.agt_hqd_pre_loan_appl_info_icmsf1_cl d
        on
            o.appl_id = d.appl_id
            and o.lp_id = d.lp_id
...
--truncate table ${iml_schema}.agt_hqd_pre_loan_appl_info;
--alter table ${iml_schema}.agt_hqd_pre_loan_appl_info truncate partition for ('icmsf1') reuse storage;
...
for cur in (select subpartition_name 
             ...
             where table_owner=upper('${iml_schema}') 
               and table_name=upper('agt_hqd_pre_loan_appl_info') 
               and substr(subpartition_name,1,8)=upper('p_icmsf1')
               and substr(subpartition_name,10) >= '${batch_date}' 
               and substr(subpartition_name,10) < '20991231') loop
    v_sql := 'alter table ${iml_schema}.agt_hqd_pre_loan_appl_info drop subpartition ' || cur.subpartition_name;
...
alter table ${iml_schema}.agt_hqd_pre_loan_appl_info modify partition p_icmsf1 
add subpartition p_icmsf1_${batch_date} values(to_date('${batch_date}','YYYYMMDD')) ;

-- 4.3 exchange partition
alter table ${iml_schema}.agt_hqd_pre_loan_appl_info exchange subpartition p_icmsf1_${batch_date} with table ${iml_schema}.agt_hqd_pre_loan_appl_info_icmsf1_cl;
alter table ${iml_schema}.agt_hqd_pre_loan_appl_info exchange subpartition p_icmsf1_20991231 with table ${iml_schema}.agt_hqd_pre_loan_appl_info_icmsf1_op;

-- 5.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.agt_hqd_pre_loan_appl_info to ${iml_schema};
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.agt_hqd_pre_loan_appl_info_icmsf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'agt_hqd_pre_loan_appl_info', partname => 'p_icmsf1_20991231',ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
