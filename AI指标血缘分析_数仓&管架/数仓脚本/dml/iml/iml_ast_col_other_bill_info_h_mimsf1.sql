...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_ast_col_other_bill_info_h_mimsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.ast_col_other_bill_info_h add partition p_mimsf1 values ('mimsf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.ast_col_other_bill_info_h_mimsf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.ast_col_other_bill_info_h partition for ('mimsf1')
...
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.ast_col_other_bill_info_h_mimsf1_tm purge;
drop table ${iml_schema}.ast_col_other_bill_info_h_mimsf1_op purge;
drop table ${iml_schema}.ast_col_other_bill_info_h_mimsf1_cl purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.ast_col_other_bill_info_h_mimsf1_tm nologging
compress ${option_switch} for query high
as select
    asset_id -- 资产编号
    ,lp_id -- 法人编号
    ...
    ,accptor_local_cty_or_rg_cd -- 承兑人所在国家或地区代码
    ,accptor_ext_rating_rest_cd -- 承兑人外部评级结果代码
...
from ${iml_schema}.ast_col_other_bill_info_h partition for ('mimsf1')
where 0=1
;

create table ${iml_schema}.ast_col_other_bill_info_h_mimsf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.ast_col_other_bill_info_h partition for ('mimsf1') where 0=1;

create table ${iml_schema}.ast_col_other_bill_info_h_mimsf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.ast_col_other_bill_info_h partition for ('mimsf1') where 0=1;
...
-- mims_si_ohterbill-
insert into ${iml_schema}.ast_col_other_bill_info_h_mimsf1_tm(
    asset_id -- 资产编号
    ,lp_id -- 法人编号
    ,col_id -- 押品编号
    ,bill_num -- 票据号码
...
select
    ...
    ,nvl(trim(p1.ACCEPTORCOUNTRY),'XXX') -- 承兑人所在国家或地区代码
    ,nvl(trim(p1.ACCEPTORRATING),'-') -- 承兑人外部评级结果代码
    ,nvl(trim(p1.ISBANKPASTE),'-') -- 银行保贴标志
    ,p1.BANKPASTENAME -- 银行保贴名称
    ,nvl(trim(p1.TDCURRENCY),'-') -- 币种代码
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,'mims_si_ohterbill' -- 源表名称
    ,'mimsf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.mims_si_ohterbill p1
    left join ${iml_schema}.ref_pub_cd_map r1 on nvl(P1.NOTETYPE,' ')= R1.SRC_CODE_VAL                                
        AND R1.SORC_SYS_CD= 'MIMS'                          
        AND R1.SRC_TAB_EN_NAME= 'MIMS_SI_OHTERBILL'               
        AND R1.SRC_FIELD_EN_NAME= 'NOTETYPE'                 
        AND R1.TARGET_TAB_EN_NAME= 'AST_COL_OTHER_BILL_INFO_H'       
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.ast_col_other_bill_info_h_mimsf1_tm 
  	                                group by 
  	                                        asset_id
  	                                        ,lp_id
  	                               having count(1) > 1);
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.ast_col_other_bill_info_h_mimsf1_cl(
            asset_id -- 资产编号
    ,lp_id -- 法人编号
    ,col_id -- 押品编号
    ,bill_num -- 票据号码
    ...
    ,curr_cd -- 币种代码
        ...
        into ${iml_schema}.ast_col_other_bill_info_h_mimsf1_op(
            asset_id -- 资产编号
    ,lp_id -- 法人编号
    ,col_id -- 押品编号
    ,bill_num -- 票据号码
...
select
    ...
    ,nvl(n.accptor_local_cty_or_rg_cd, o.accptor_local_cty_or_rg_cd) as accptor_local_cty_or_rg_cd -- 承兑人所在国家或地区代码
    ,nvl(n.accptor_ext_rating_rest_cd, o.accptor_ext_rating_rest_cd) as accptor_ext_rating_rest_cd -- 承兑人外部评级结果代码
...
from ${iml_schema}.ast_col_other_bill_info_h_mimsf1_tm n
    full join (select * from ${iml_schema}.ast_col_other_bill_info_h_mimsf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
...
where (
    ...
    or (
        ...
        or o.accptor_local_cty_or_rg_cd <> n.accptor_local_cty_or_rg_cd
        or o.accptor_ext_rating_rest_cd <> n.accptor_ext_rating_rest_cd
...
select
    ...
    ,o.accptor_local_cty_or_rg_cd -- 承兑人所在国家或地区代码
    ,o.accptor_ext_rating_rest_cd -- 承兑人外部评级结果代码
...
from ${iml_schema}.ast_col_other_bill_info_h_mimsf1_bk o
    left join ${iml_schema}.ast_col_other_bill_info_h_mimsf1_op n
        on
            o.asset_id = n.asset_id
            and o.lp_id = n.lp_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iml_schema}.ast_col_other_bill_info_h_mimsf1_cl d
        on
...
--truncate table ${iml_schema}.ast_col_other_bill_info_h;
--alter table ${iml_schema}.ast_col_other_bill_info_h truncate partition for ('mimsf1') reuse storage;
...
for cur in (select subpartition_name 
             ...
             where table_owner=upper('${iml_schema}') 
               and table_name=upper('ast_col_other_bill_info_h') 
               and substr(subpartition_name,1,8)=upper('p_mimsf1')
               and substr(subpartition_name,10) >= '${batch_date}' 
               and substr(subpartition_name,10) < '20991231') loop
    v_sql := 'alter table ${iml_schema}.ast_col_other_bill_info_h drop subpartition ' || cur.subpartition_name;
...
alter table ${iml_schema}.ast_col_other_bill_info_h modify partition p_mimsf1 
add subpartition p_mimsf1_${batch_date} values(to_date('${batch_date}','YYYYMMDD')) ;

-- 4.3 exchange partition
alter table ${iml_schema}.ast_col_other_bill_info_h exchange subpartition p_mimsf1_${batch_date} with table ${iml_schema}.ast_col_other_bill_info_h_mimsf1_cl;
alter table ${iml_schema}.ast_col_other_bill_info_h exchange subpartition p_mimsf1_20991231 with table ${iml_schema}.ast_col_other_bill_info_h_mimsf1_op;

-- 5.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.ast_col_other_bill_info_h to ${iml_schema};
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.ast_col_other_bill_info_h_mimsf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'ast_col_other_bill_info_h', partname => 'p_mimsf1_20991231',ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
