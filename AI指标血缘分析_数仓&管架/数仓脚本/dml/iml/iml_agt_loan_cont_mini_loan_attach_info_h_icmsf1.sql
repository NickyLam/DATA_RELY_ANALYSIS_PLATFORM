...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_agt_loan_cont_mini_loan_attach_info_h_icmsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h add partition p_icmsf1 values ('icmsf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h_icmsf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h partition for ('icmsf1')
...
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h_icmsf1_tm purge;
drop table ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h_icmsf1_op purge;
drop table ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h_icmsf1_cl purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h_icmsf1_tm nologging
compress ${option_switch} for query high
as select
    agt_id -- 协议编号
    ,cont_id -- 合同编号
...
from ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h partition for ('icmsf1')
where 0=1
;

create table ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h_icmsf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h partition for ('icmsf1') where 0=1;

create table ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h_icmsf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h partition for ('icmsf1') where 0=1;
...
-- icms_bc_upl_loan-1
insert into ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h_icmsf1_tm(
    agt_id -- 协议编号
    ,cont_id -- 合同编号
    ,stud_loan_deflt_prod_id -- 助贷默认产品编号
    ,loan_usage_tran_amt -- 贷款用途交易金额
...
select
    ...
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,'icms_bc_upl_loan' -- 源表名称
    ,'icmsf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.icms_bc_upl_loan p1
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h_icmsf1_tm 
  	                                group by 
  	                                        agt_id
  	                                        ,cont_id
  	                               having count(1) > 1);
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h_icmsf1_cl(
            agt_id -- 协议编号
    ,cont_id -- 合同编号
    ,stud_loan_deflt_prod_id -- 助贷默认产品编号
    ,loan_usage_tran_amt -- 贷款用途交易金额
    ...
    ,prtcpt_deduct_flg -- 参与批扣标志
        ...
        into ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h_icmsf1_op(
            agt_id -- 协议编号
    ,cont_id -- 合同编号
    ,stud_loan_deflt_prod_id -- 助贷默认产品编号
    ,loan_usage_tran_amt -- 贷款用途交易金额
...
from ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h_icmsf1_tm n
    full join (select * from ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h_icmsf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
            o.agt_id = n.agt_id
...
where (
    ...
    or (
        o.stud_loan_deflt_prod_id <> n.stud_loan_deflt_prod_id
...
from ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h_icmsf1_bk o
    left join ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h_icmsf1_op n
        on
            o.agt_id = n.agt_id
            and o.cont_id = n.cont_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h_icmsf1_cl d
        on
            o.agt_id = d.agt_id
...
--truncate table ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h;
--alter table ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h truncate partition for ('icmsf1') reuse storage;
...
for cur in (select subpartition_name 
             ...
             where table_owner=upper('${iml_schema}') 
               and table_name=upper('agt_loan_cont_mini_loan_attach_info_h') 
               and substr(subpartition_name,1,8)=upper('p_icmsf1')
               and substr(subpartition_name,10) >= '${batch_date}' 
               and substr(subpartition_name,10) < '20991231') loop
    v_sql := 'alter table ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h drop subpartition ' || cur.subpartition_name;
...
whenever sqlerror exit sql.sqlcode;
alter table ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h modify partition p_icmsf1 
add subpartition p_icmsf1_${batch_date} values(to_date('${batch_date}','YYYYMMDD')) ;

-- 4.3 exchange partition
alter table ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h exchange subpartition p_icmsf1_${batch_date} with table ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h_icmsf1_cl;
alter table ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h exchange subpartition p_icmsf1_20991231 with table ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h_icmsf1_op;

-- 5.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h to ${iml_schema};
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.agt_loan_cont_mini_loan_attach_info_h_icmsf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'agt_loan_cont_mini_loan_attach_info_h', partname => 'p_icmsf1_20991231',ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
