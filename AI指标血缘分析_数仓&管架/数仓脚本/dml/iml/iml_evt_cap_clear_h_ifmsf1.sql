...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_evt_cap_clear_h_ifmsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.evt_cap_clear_h add partition p_ifmsf1 values ('ifmsf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.evt_cap_clear_h_ifmsf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.evt_cap_clear_h partition for ('ifmsf1')
;

-- 2.2 drop temp table
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.evt_cap_clear_h_ifmsf1_tm purge;
drop table ${iml_schema}.evt_cap_clear_h_ifmsf1_op purge;
drop table ${iml_schema}.evt_cap_clear_h_ifmsf1_cl purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.evt_cap_clear_h_ifmsf1_tm nologging
compress ${option_switch} for query high
as select
    evt_id -- 事件编号
    ,lp_id -- 法人编号
...
from ${iml_schema}.evt_cap_clear_h partition for ('ifmsf1')
where 0=1
;

create table ${iml_schema}.evt_cap_clear_h_ifmsf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.evt_cap_clear_h partition for ('ifmsf1') where 0=1;

create table ${iml_schema}.evt_cap_clear_h_ifmsf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.evt_cap_clear_h partition for ('ifmsf1') where 0=1;
...
-- ifms_tbhissquare-1
insert into ${iml_schema}.evt_cap_clear_h_ifmsf1_tm(
    evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,clear_flow_num -- 清算流水号
    ,clear_seq_num -- 清算顺序号
...
select
    ...
    ,P1.FROZEN_AMT -- 冻结金额
    ,P1.CHECK_STATUS -- 勾对确认代码
    ,P1.DISTRIB_FLAG -- 上帐金额来源类型代码
    ,P1.AMT_FLAG -- 资金类别代码
    ,P1.COST_INCOME_FLAG -- 本金收益代码
    ,P1.CFM_VOL -- 确认份额
    ,P1.COST -- 本金金额
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.evt_cap_clear_h_ifmsf1_cl(
            evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,clear_flow_num -- 清算流水号
    ,clear_seq_num -- 清算顺序号
    ...
    ,remark_info_5 -- 备注信息5
        ...
        into ${iml_schema}.evt_cap_clear_h_ifmsf1_op(
            evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,clear_flow_num -- 清算流水号
    ,clear_seq_num -- 清算顺序号
...
from ${iml_schema}.evt_cap_clear_h_ifmsf1_tm n
    full join (select * from ${iml_schema}.evt_cap_clear_h_ifmsf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
            o.evt_id = n.evt_id
            and o.lp_id = n.lp_id
where (
...
from ${iml_schema}.evt_cap_clear_h_ifmsf1_bk o
    left join ${iml_schema}.evt_cap_clear_h_ifmsf1_op n
        on
            o.evt_id = n.evt_id
            and o.lp_id = n.lp_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iml_schema}.evt_cap_clear_h_ifmsf1_cl d
        on
            o.evt_id = d.evt_id
            and o.lp_id = d.lp_id
...
--truncate table ${iml_schema}.evt_cap_clear_h;
alter table ${iml_schema}.evt_cap_clear_h truncate partition for ('ifmsf1') reuse storage;

-- 4.2 exchange partition
alter table ${iml_schema}.evt_cap_clear_h exchange subpartition p_ifmsf1_19000101 with table ${iml_schema}.evt_cap_clear_h_ifmsf1_cl;
alter table ${iml_schema}.evt_cap_clear_h exchange subpartition p_ifmsf1_20991231 with table ${iml_schema}.evt_cap_clear_h_ifmsf1_op;

-- 5.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.evt_cap_clear_h to ${iml_schema};
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.evt_cap_clear_h_ifmsf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'evt_cap_clear_h', partname => 'p_ifmsf1_20991231', granularity => 'SUBPARTITION', degree => 8, cascade => true);
