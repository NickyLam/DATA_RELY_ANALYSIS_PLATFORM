...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_evt_trust_tran_entr_h_trusf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.evt_trust_tran_entr_h add partition p_trusf1 values ('trusf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.evt_trust_tran_entr_h_trusf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.evt_trust_tran_entr_h partition for ('trusf1')
...
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.evt_trust_tran_entr_h_trusf1_tm purge;
drop table ${iml_schema}.evt_trust_tran_entr_h_trusf1_op purge;
drop table ${iml_schema}.evt_trust_tran_entr_h_trusf1_cl purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.evt_trust_tran_entr_h_trusf1_tm nologging
compress ${option_switch} for query high
as select
    evt_id -- 事件编号
    ,lp_id -- 法人编号
...
from ${iml_schema}.evt_trust_tran_entr_h partition for ('trusf1')
where 0=1
;

create table ${iml_schema}.evt_trust_tran_entr_h_trusf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.evt_trust_tran_entr_h partition for ('trusf1') where 0=1;

create table ${iml_schema}.evt_trust_tran_entr_h_trusf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.evt_trust_tran_entr_h partition for ('trusf1') where 0=1;

-- 3.1 get new data into table
-- nfss_tcs_tbhistransreq-1
insert into ${iml_schema}.evt_trust_tran_entr_h_trusf1_tm(
    evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,entr_flow_num -- 委托流水号
    ,intior_flow_num -- 发起方流水号
...
select
    '102057'||P1.SERIAL_NO -- 事件编号
    ,'9999' -- 法人编号
    ,P1.SERIAL_NO -- 委托流水号
    ,P1.EX_SERIAL -- 发起方流水号
    ,P1.CONTRACT_NO -- 合约编号
    ,${iml_schema}.dateformat_min(TO_CHAR(P1.TRANS_DATE)) -- 申请日期
    ,${iml_schema}.dateformat_min(TO_CHAR(P1.TRANS_DATE)||lpad(to_char(P1.TRANS_TIME),6,'0')) -- 申请时间
    ,${iml_schema}.dateformat_min(TO_CHAR(P1.OCCUR_INIT_DATE)) -- 系统日期
    ...
    ,P1.CLIENT_NO -- 交易客户编号
    ,P1.BANK_ACC -- 银行账户编号
    ,P1.DEBIT_ACCOUNT -- 内部账户编号
    ,P1.CASH_FLAG -- 钞汇标识代码
    ,P1.TRANS_ACCOUNT_TYPE -- 交易介质类型代码
    ,P1.TRANS_ACCOUNT -- 交易介质编号
    ,CASE WHEN R6.TARGET_CD_VAL IS NOT NULL THEN R6.TARGET_CD_VAL ELSE '@'||P1.CHANNEL END -- 交易渠道编号
    ,P1.OPER_NO -- 交易柜员编号
    ,P1.AUTH_OPER -- 授权柜员编号
    ,P1.PRD_CODE -- 理财产品编号
    ,CASE WHEN R3.TARGET_CD_VAL IS NOT NULL THEN R3.TARGET_CD_VAL ELSE '@'||P1.CURR_TYPE END -- 产品币种代码
    ,P1.PRD_TYPE -- 产品类别代码
    ,P1.SHARE_CLASS -- 收费方式代码
    ,${iml_schema}.dateformat_min(TO_CHAR(P1.ASSO_DATE)) -- 关联交易日期
    ,P1.ASSO_SERIAL -- 关联交易流水号
    ,P1.AMT -- 交易金额
    ,CASE WHEN R4.TARGET_CD_VAL IS NOT NULL THEN R4.TARGET_CD_VAL ELSE '@'||P1.CLIENT_GROUP END -- 客户分组代码
    ,decode(trim(P1.LIQU_STATUS),'8','11',nvl(trim(P1.LIQU_STATUS),'-')) -- 账务状态代码
    ,CASE WHEN R5.TARGET_CD_VAL IS NOT NULL THEN R5.TARGET_CD_VAL ELSE '@'||P1.ORI_CHANNEL END -- 原交易渠道代码
    ,P1.ORI_BRANCH_NO -- 原交易机构编号
    ,P1.VOL -- 交易份额
    ,P1.LARG_RED_FLAG -- 巨额赎回处理代码
    ,P1.RED_MODE -- 赎回模式代码
    ,P1.DIV_MODE -- 分红方式代码
    ,P1.FROZEN_CAUSE -- 冻结原因代码
    ,P1.TARG_BANK_ACC -- 目标银行账户编号
    ,P1.CLIENT_RISK -- 客户风险等级代码
    ,CASE WHEN R2.TARGET_CD_VAL IS NOT NULL THEN R2.TARGET_CD_VAL ELSE '@'||P1.PRODUCT_RISK END -- 产品风险等级代码
    ,${iml_schema}.dateformat_min(TO_CHAR(P1.CFM_DATE)) -- 确认日期
    ,P1.CFM_NO -- TA确认流水号
    ,P1.CFM_VOL -- 确认份额
    ,P1.TO_HOST_SERIAL -- 发送主机流水号
    ,${iml_schema}.dateformat_min(TO_CHAR(P1.HOST_CHECK_DATE)) -- 主机对账日期
    ,${iml_schema}.dateformat_min(TO_CHAR(P1.ORI_HOST_CHK_DATE)) -- 原交易主机对账日期
    ,P1.HOST_TRANS_CODE -- 主机交易码
    ,${iml_schema}.dateformat_min(TO_CHAR(P1.HOST_DATE)) -- 主机日期
    ,P1.HOST_SERIAL -- 主机流水号
    ,case when P1.MONITOR_FLAG='0' then '1'
     when P1.MONITOR_FLAG='1' then '0'
     else '-' 
end -- 监管正常标志
    ...
    ,P1.DEAL_MODE -- 受理方式代码
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,'nfss_tcs_tbhistransreq' -- 源表名称
    ,'trusf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.nfss_tcs_tbhistransreq p1
    left join ${iml_schema}.ref_pub_cd_map r1 on P1.CLIENT_TYPE = R1.SRC_CODE_VAL
        AND R1.SORC_SYS_CD= 'NFSS'
        AND R1.SRC_TAB_EN_NAME= 'NFSS_TCS_TBHISTRANSREQ'
        AND R1.SRC_FIELD_EN_NAME= 'CLIENT_TYPE'
        AND R1.TARGET_TAB_EN_NAME= 'EVT_TRUST_TRAN_ENTR_H'
        AND R1.TARGET_TAB_FIELD_EN_NAME= 'CUST_TYPE_CD'
    left join ${iml_schema}.ref_pub_cd_map r6 on P1.CHANNEL = R6.SRC_CODE_VAL
        AND R6.SORC_SYS_CD= 'NFSS'
        AND R6.SRC_TAB_EN_NAME= 'NFSS_TCS_TBHISTRANSREQ'
        AND R6.SRC_FIELD_EN_NAME= 'CHANNEL'
        AND R6.TARGET_TAB_EN_NAME= 'EVT_TRUST_TRAN_ENTR_H'
        AND R6.TARGET_TAB_FIELD_EN_NAME= 'TRAN_CHN_CD'
    left join ${iml_schema}.ref_pub_cd_map r3 on P1.CURR_TYPE = R3.SRC_CODE_VAL
        AND R3.SORC_SYS_CD= 'NFSS'
        AND R3.SRC_TAB_EN_NAME= 'NFSS_TCS_TBHISTRANSREQ'
        AND R3.SRC_FIELD_EN_NAME= 'CURR_TYPE'
        AND R3.TARGET_TAB_EN_NAME= 'EVT_TRUST_TRAN_ENTR_H'
        AND R3.TARGET_TAB_FIELD_EN_NAME= 'PROD_CURR_CD'
    left join ${iml_schema}.ref_pub_cd_map r4 on P1.CLIENT_GROUP = R4.SRC_CODE_VAL
        AND R4.SORC_SYS_CD= 'NFSS'
        AND R4.SRC_TAB_EN_NAME= 'NFSS_TCS_TBHISTRANSREQ'
        AND R4.SRC_FIELD_EN_NAME= 'CLIENT_GROUP'
        AND R4.TARGET_TAB_EN_NAME= 'EVT_TRUST_TRAN_ENTR_H'
        AND R4.TARGET_TAB_FIELD_EN_NAME= 'CUST_GROUPING_CD'
    left join ${iml_schema}.ref_pub_cd_map r5 on P1.ORI_CHANNEL = R5.SRC_CODE_VAL
        AND R5.SORC_SYS_CD= 'NFSS'
        AND R5.SRC_TAB_EN_NAME= 'NFSS_TCS_TBHISTRANSREQ'
        AND R5.SRC_FIELD_EN_NAME= 'ORI_CHANNEL'
        AND R5.TARGET_TAB_EN_NAME= 'EVT_TRUST_TRAN_ENTR_H'
        AND R5.TARGET_TAB_FIELD_EN_NAME= 'INIT_TRAN_CHN_CD'
    left join ${iml_schema}.ref_pub_cd_map r2 on to_char(P1.PRODUCT_RISK) = R2.SRC_CODE_VAL
        AND R2.SORC_SYS_CD= 'NFSS'
        AND R2.SRC_TAB_EN_NAME= 'NFSS_TCS_TBHISTRANSREQ'
        AND R2.SRC_FIELD_EN_NAME= 'PRODUCT_RISK'
        AND R2.TARGET_TAB_EN_NAME= 'EVT_TRUST_TRAN_ENTR_H'
        AND R2.TARGET_TAB_FIELD_EN_NAME= 'PROD_RISK_LEVEL_CD'
where p1.start_dt <= to_date('${batch_date}','yyyymmdd') and p1.end_dt > to_date('${batch_date}','yyyymmdd')
;
commit;

-- nfss_tcs_tbtransreq-1
insert into ${iml_schema}.evt_trust_tran_entr_h_trusf1_tm(
    evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,entr_flow_num -- 委托流水号
...
select
    ...
    ,P1.DEAL_MODE -- 受理方式代码
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,'nfss_tcs_tbtransreq' -- 源表名称
    ,'trusf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.nfss_tcs_tbtransreq p1
    left join ${iol_schema}.nfss_tcs_tbhistransreq p2 on p1.serial_no=p2.serial_no
        and p2.start_dt <= to_date('${batch_date}','yyyymmdd') and p2.end_dt > to_date('${batch_date}','yyyymmdd')
    left join ${iml_schema}.ref_pub_cd_map r1 on P1.CLIENT_TYPE = R1.SRC_CODE_VAL
        AND R1.SORC_SYS_CD= 'NFSS'
        AND R1.SRC_TAB_EN_NAME= 'NFSS_TCS_TBTRANSREQ'
        AND R1.SRC_FIELD_EN_NAME= 'CLIENT_TYPE'
        AND R1.TARGET_TAB_EN_NAME= 'EVT_TRUST_TRAN_ENTR_H'
        AND R1.TARGET_TAB_FIELD_EN_NAME= 'CUST_TYPE_CD'
    left join ${iml_schema}.ref_pub_cd_map r6 on P1.CHANNEL = R6.SRC_CODE_VAL
        AND R6.SORC_SYS_CD= 'NFSS'
        AND R6.SRC_TAB_EN_NAME= 'NFSS_TCS_TBTRANSREQ'
        AND R6.SRC_FIELD_EN_NAME= 'CHANNEL'
        AND R6.TARGET_TAB_EN_NAME= 'EVT_TRUST_TRAN_ENTR_H'
        AND R6.TARGET_TAB_FIELD_EN_NAME= 'TRAN_CHN_CD'
    left join ${iml_schema}.ref_pub_cd_map r3 on P1.CURR_TYPE = R3.SRC_CODE_VAL
        AND R3.SORC_SYS_CD= 'NFSS'
        AND R3.SRC_TAB_EN_NAME= 'NFSS_TCS_TBTRANSREQ'
        AND R3.SRC_FIELD_EN_NAME= 'CURR_TYPE'
        AND R3.TARGET_TAB_EN_NAME= 'EVT_TRUST_TRAN_ENTR_H'
        AND R3.TARGET_TAB_FIELD_EN_NAME= 'PROD_CURR_CD'
    left join ${iml_schema}.ref_pub_cd_map r4 on P1.CLIENT_GROUP = R4.SRC_CODE_VAL
        AND R4.SORC_SYS_CD= 'NFSS'
        AND R4.SRC_TAB_EN_NAME= 'NFSS_TCS_TBTRANSREQ'
        AND R4.SRC_FIELD_EN_NAME= 'CLIENT_GROUP'
        AND R4.TARGET_TAB_EN_NAME= 'EVT_TRUST_TRAN_ENTR_H'
        AND R4.TARGET_TAB_FIELD_EN_NAME= 'CUST_GROUPING_CD'
    left join ${iml_schema}.ref_pub_cd_map r5 on P1.ORI_CHANNEL = R5.SRC_CODE_VAL
        AND R5.SORC_SYS_CD= 'NFSS'
        AND R5.SRC_TAB_EN_NAME= 'NFSS_TCS_TBTRANSREQ'
        AND R5.SRC_FIELD_EN_NAME= 'ORI_CHANNEL'
        AND R5.TARGET_TAB_EN_NAME= 'EVT_TRUST_TRAN_ENTR_H'
        AND R5.TARGET_TAB_FIELD_EN_NAME= 'INIT_TRAN_CHN_CD'
    left join ${iml_schema}.ref_pub_cd_map r2 on to_char(P1.PRODUCT_RISK) = R2.SRC_CODE_VAL
        AND R2.SORC_SYS_CD= 'NFSS'
        AND R2.SRC_TAB_EN_NAME= 'NFSS_TCS_TBTRANSREQ'
        AND R2.SRC_FIELD_EN_NAME= 'PRODUCT_RISK'
        AND R2.TARGET_TAB_EN_NAME= 'EVT_TRUST_TRAN_ENTR_H'
        AND R2.TARGET_TAB_FIELD_EN_NAME= 'PROD_RISK_LEVEL_CD'
where p1.start_dt <= to_date('${batch_date}','yyyymmdd') and p1.end_dt > to_date('${batch_date}','yyyymmdd')
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.evt_trust_tran_entr_h_trusf1_tm 
  	                                group by 
  	                                        evt_id
  	                                        ,lp_id
  	                               having count(1) > 1);
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.evt_trust_tran_entr_h_trusf1_cl(
            evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,entr_flow_num -- 委托流水号
    ,intior_flow_num -- 发起方流水号
    ...
    ,accpt_way_cd -- 受理方式代码
        ...
        into ${iml_schema}.evt_trust_tran_entr_h_trusf1_op(
            evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,entr_flow_num -- 委托流水号
    ,intior_flow_num -- 发起方流水号
...
select
    ...
    ,nvl(n.rela_tran_dt, o.rela_tran_dt) as rela_tran_dt -- 关联交易日期
...
from ${iml_schema}.evt_trust_tran_entr_h_trusf1_tm n
    full join (select * from ${iml_schema}.evt_trust_tran_entr_h_trusf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
            o.evt_id = n.evt_id
            and o.lp_id = n.lp_id
where (
    ...
    or (
        ...
        or o.rela_tran_dt <> n.rela_tran_dt
...
from ${iml_schema}.evt_trust_tran_entr_h_trusf1_bk o
    left join ${iml_schema}.evt_trust_tran_entr_h_trusf1_op n
        on
            o.evt_id = n.evt_id
            and o.lp_id = n.lp_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iml_schema}.evt_trust_tran_entr_h_trusf1_cl d
        on
            o.evt_id = d.evt_id
            and o.lp_id = d.lp_id
...
--truncate table ${iml_schema}.evt_trust_tran_entr_h;
--alter table ${iml_schema}.evt_trust_tran_entr_h truncate partition for ('trusf1') reuse storage;
...
for cur in (select subpartition_name 
             ...
             where table_owner=upper('${iml_schema}') 
               and table_name=upper('evt_trust_tran_entr_h') 
               and substr(subpartition_name,1,8)=upper('p_trusf1')
               and substr(subpartition_name,10) >= '${batch_date}' 
               and substr(subpartition_name,10) < '20991231') loop
    v_sql := 'alter table ${iml_schema}.evt_trust_tran_entr_h drop subpartition ' || cur.subpartition_name;
...
whenever sqlerror exit sql.sqlcode;
alter table ${iml_schema}.evt_trust_tran_entr_h modify partition p_trusf1 
add subpartition p_trusf1_${batch_date} values(to_date('${batch_date}','YYYYMMDD')) ;

-- 4.3 exchange partition
alter table ${iml_schema}.evt_trust_tran_entr_h exchange subpartition p_trusf1_${batch_date} with table ${iml_schema}.evt_trust_tran_entr_h_trusf1_cl;
alter table ${iml_schema}.evt_trust_tran_entr_h exchange subpartition p_trusf1_20991231 with table ${iml_schema}.evt_trust_tran_entr_h_trusf1_op;

-- 5.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.evt_trust_tran_entr_h to ${iml_schema};
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.evt_trust_tran_entr_h_trusf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'evt_trust_tran_entr_h', partname => 'p_trusf1_20991231',ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
