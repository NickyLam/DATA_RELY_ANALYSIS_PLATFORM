...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_evt_sec_clear_info_ctmsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
drop table ${iml_schema}.evt_sec_clear_info_ctmsf1_tm purge;
alter table ${iml_schema}.evt_sec_clear_info add partition p_ctmsf1 values ('ctmsf1')(
        subpartition p_ctmsf1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
    )
;
alter table ${iml_schema}.evt_sec_clear_info modify partition p_ctmsf1
    add subpartition p_ctmsf1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
;

whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.evt_sec_clear_info_ctmsf1_tm
...
from ${iml_schema}.evt_sec_clear_info
where 0=1;

-- it is no need to check when this segment SQL was return faied
-- 3.1 insert data to tm table
whenever sqlerror exit sql.sqlcode;
-- ctms_vs_payment_trsi_bondsdeals-1
insert into ${iml_schema}.evt_sec_clear_info_ctmsf1_tm(
    evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,pay_id -- 支付编号
    ,dept_id -- 部门编号
...
select
    ...
    ,'ctms_vs_payment_trsi_bondsdeals' -- 源表名称
    ,'ctmsf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.ctms_vs_payment_trsi_bondsdeals p1
...
alter table ${iml_schema}.evt_sec_clear_info truncate partition p_ctmsf1;

-- 3.3 exchage tm table and target table
alter table ${iml_schema}.evt_sec_clear_info exchange subpartition p_ctmsf1_${batch_date} with table ${iml_schema}.evt_sec_clear_info_ctmsf1_tm;

-- 4.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.evt_sec_clear_info to ${iml_schema};

-- 4.2 drop tm table
drop table ${iml_schema}.evt_sec_clear_info_ctmsf1_tm purge;

-- 5 gater table status
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'evt_sec_clear_info', partname => 'p_ctmsf1_${batch_date}', ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);