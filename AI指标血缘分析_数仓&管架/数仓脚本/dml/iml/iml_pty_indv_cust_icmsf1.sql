...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_pty_indv_cust_icmsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
drop table ${iml_schema}.pty_indv_cust_icmsf1_tm purge;
drop table ${iml_schema}.pty_indv_cust_icmsf1_ex purge;

-- 2.2 add partition for target table
alter table ${iml_schema}.pty_indv_cust add partition p_icmsf1 values ('icmsf1')(
        subpartition p_icmsf1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
    )
;
alter table ${iml_schema}.pty_indv_cust modify partition p_icmsf1
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.pty_indv_cust_icmsf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.pty_indv_cust partition for ('icmsf1')
where create_dt < to_date('${batch_date}','yyyymmdd')
;

-- 2.4 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.pty_indv_cust_icmsf1_tm
compress ${option_switch} for query high
as
select
    cust_id -- 客户编号
...
create table ${iml_schema}.pty_indv_cust_icmsf1_ex nologging
compress ${option_switch} for query high
as select * from ${iml_schema}.pty_indv_cust partition for ('icmsf1') where 0=1;

-- 2.1 insert data to tm table
-- icms_ind_info-
insert into ${iml_schema}.pty_indv_cust_icmsf1_tm(
    cust_id -- 客户编号
    ,lp_id -- 法人编号
    ,sorc_sys_cd -- 源系统代码
    ,gender_cd -- 性别代码
...
select
    ...
    ,'9999' -- 法人编号
    ,'ICMS' -- 源系统代码
    ,NVL(TRIM(P1.SEX),'9') -- 性别代码
    ,P1.BIRTHDAY -- 出生日期
    ,NVL(TRIM(P1.NATIONALITY),'00') -- 民族代码
    ,P1.NATIVEPLACE -- 籍贯
    ,NVL(TRIM(P1.POLITICALFACE),'-') -- 政治面貌代码
    ,DECODE(TRIM(P1.MARRIAGE),NULL,'90','99','90',P1.MARRIAGE) -- 婚姻状况代码
    ,DECODE(TRIM(P1.ISBANKSTAFF),NULL,'-','2','-',P1.ISBANKSTAFF) -- 行员标志
    ,0 -- 年龄
    ,' ' -- 居民标志
    ,NVL(TRIM(P1.NATION),'XXX') -- 国籍代码
    ,NVL(TRIM(P1.RGSTAD),'000000') -- 行政区域代码
    ,'-' -- 我行股东标志
    ,NVL(TRIM(P1.INDTYPE),'-') -- 客户性质代码
    ,NVL(TRIM(P1.ISFARMER),'-') -- 农村户口标志
    ,NVL(TRIM(P1.ISRELATIVE),'-') -- 我行关联方标志
    ,'-' -- 在我行办理过中间业务标志
    ,'-' -- 我行代发工资户标志
    ,'-' -- 我行定期客户标志
    ,'-' -- 我行理财客户标志
    ...
    ,'-' -- 配偶及子女移民标志
    ,'-' -- 享受国家优惠政策标志
    ,P1.GRADUATESCHOOL -- 毕业学校
    ,P1.GRADUATEYEAR -- 毕业年份
    ,P1.email -- 电子邮箱
    ,P1.INPUTUSERID -- 开户柜员编号
    ,P1.INPUTORGID -- 开户机构编号
    ,P1.INPUTDATE -- 开户日期
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.pty_indv_cust_icmsf1_tm 
  	                                group by 
  	                                        cust_id
  	                                        ,lp_id
  	                                        ,sorc_sys_cd
...
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${iml_schema}.pty_indv_cust_icmsf1_ex(
    cust_id -- 客户编号
    ,lp_id -- 法人编号
    ,sorc_sys_cd -- 源系统代码
    ,gender_cd -- 性别代码
...
select
    ...
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iml_schema}.pty_indv_cust_icmsf1_tm n
    full join ${iml_schema}.pty_indv_cust_icmsf1_bk o
        on
            o.cust_id = n.cust_id
            and o.lp_id = n.lp_id
...
whenever sqlerror exit sql.sqlcode;
alter table ${iml_schema}.pty_indv_cust truncate partition for ('icmsf1') reuse storage;

-- 3.2 exchage tm table and target table
alter table ${iml_schema}.pty_indv_cust exchange subpartition p_icmsf1_${batch_date} with table ${iml_schema}.pty_indv_cust_icmsf1_ex;

-- 3.3 drop subpartition last_date partition
whenever sqlerror continue none ;
alter table ${iml_schema}.pty_indv_cust drop subpartition p_icmsf1_${last_date} update global indexes;
...
drop table ${iml_schema}.pty_indv_cust_icmsf1_tm purge;
drop table ${iml_schema}.pty_indv_cust_icmsf1_ex purge;
drop table ${iml_schema}.pty_indv_cust_icmsf1_bk purge;

-- 5 gater table status
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'pty_indv_cust', partname => 'p_icmsf1_${batch_date}', ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);