...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_agt_loan_acct_pnlt_int_accr_h_ncbsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.agt_loan_acct_pnlt_int_accr_h add partition p_ncbsf1 values ('ncbsf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.agt_loan_acct_pnlt_int_accr_h_ncbsf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_loan_acct_pnlt_int_accr_h partition for ('ncbsf1')
...
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.agt_loan_acct_pnlt_int_accr_h_ncbsf1_tm purge;
drop table ${iml_schema}.agt_loan_acct_pnlt_int_accr_h_ncbsf1_op purge;
drop table ${iml_schema}.agt_loan_acct_pnlt_int_accr_h_ncbsf1_cl purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.agt_loan_acct_pnlt_int_accr_h_ncbsf1_tm nologging
compress ${option_switch} for query high
as select
    agt_id -- 协议编号
    ,lp_id -- 法人编号
...
from ${iml_schema}.agt_loan_acct_pnlt_int_accr_h partition for ('ncbsf1')
where 0=1
;

create table ${iml_schema}.agt_loan_acct_pnlt_int_accr_h_ncbsf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_loan_acct_pnlt_int_accr_h partition for ('ncbsf1') where 0=1;

create table ${iml_schema}.agt_loan_acct_pnlt_int_accr_h_ncbsf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_loan_acct_pnlt_int_accr_h partition for ('ncbsf1') where 0=1;

-- 3.1 get new data into table
-- ncbs_cl_acct_odp_detail-1
insert into ${iml_schema}.agt_loan_acct_pnlt_int_accr_h_ncbsf1_tm(
    agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,acct_id -- 账户编号
    ,int_cls_cd -- 利息分类代码
...
select
    ...
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,'ncbs_cl_acct_odp_detail' -- 源表名称
    ,'ncbsf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.ncbs_cl_acct_odp_detail p1
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.agt_loan_acct_pnlt_int_accr_h_ncbsf1_tm 
  	                                group by 
  	                                        agt_id
  	                                        ,lp_id
  	                                        ,acct_id
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.agt_loan_acct_pnlt_int_accr_h_ncbsf1_cl(
            agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,acct_id -- 账户编号
    ,int_cls_cd -- 利息分类代码
    ...
    ,up_ld_int_acm_provi_adj -- 上上日利息累计计提调整
        ...
        into ${iml_schema}.agt_loan_acct_pnlt_int_accr_h_ncbsf1_op(
            agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,acct_id -- 账户编号
    ,int_cls_cd -- 利息分类代码
...
from ${iml_schema}.agt_loan_acct_pnlt_int_accr_h_ncbsf1_tm n
    full join (select * from ${iml_schema}.agt_loan_acct_pnlt_int_accr_h_ncbsf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
            o.agt_id = n.agt_id
            and o.lp_id = n.lp_id
...
from ${iml_schema}.agt_loan_acct_pnlt_int_accr_h_ncbsf1_bk o
    left join ${iml_schema}.agt_loan_acct_pnlt_int_accr_h_ncbsf1_op n
        on
            o.agt_id = n.agt_id
            and o.lp_id = n.lp_id
            and o.acct_id = n.acct_id
    ...
    left join ${iml_schema}.agt_loan_acct_pnlt_int_accr_h_ncbsf1_cl d
        on
            o.agt_id = d.agt_id
            and o.lp_id = d.lp_id
...
--truncate table ${iml_schema}.agt_loan_acct_pnlt_int_accr_h;
--alter table ${iml_schema}.agt_loan_acct_pnlt_int_accr_h truncate partition for ('ncbsf1') reuse storage;
...
for cur in (select subpartition_name 
             ...
             where table_owner=upper('${iml_schema}') 
               and table_name=upper('agt_loan_acct_pnlt_int_accr_h') 
               and substr(subpartition_name,1,8)=upper('p_ncbsf1')
               and substr(subpartition_name,10) >= '${batch_date}' 
               and substr(subpartition_name,10) < '20991231') loop
    v_sql := 'alter table ${iml_schema}.agt_loan_acct_pnlt_int_accr_h drop subpartition ' || cur.subpartition_name;
...
whenever sqlerror exit sql.sqlcode;
alter table ${iml_schema}.agt_loan_acct_pnlt_int_accr_h modify partition p_ncbsf1 
add subpartition p_ncbsf1_${batch_date} values(to_date('${batch_date}','YYYYMMDD')) ;

-- 4.3 exchange partition
alter table ${iml_schema}.agt_loan_acct_pnlt_int_accr_h exchange subpartition p_ncbsf1_${batch_date} with table ${iml_schema}.agt_loan_acct_pnlt_int_accr_h_ncbsf1_cl;
alter table ${iml_schema}.agt_loan_acct_pnlt_int_accr_h exchange subpartition p_ncbsf1_20991231 with table ${iml_schema}.agt_loan_acct_pnlt_int_accr_h_ncbsf1_op;

-- 5.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.agt_loan_acct_pnlt_int_accr_h to ${iml_schema};
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.agt_loan_acct_pnlt_int_accr_h_ncbsf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'agt_loan_acct_pnlt_int_accr_h', partname => 'p_ncbsf1_20991231',ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
