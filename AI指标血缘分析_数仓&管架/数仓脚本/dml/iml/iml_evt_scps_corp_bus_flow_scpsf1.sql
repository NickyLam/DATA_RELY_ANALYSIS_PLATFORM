...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_evt_scps_corp_bus_flow_scpsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
drop table ${iml_schema}.evt_scps_corp_bus_flow_scpsf1_tm purge;
alter table ${iml_schema}.evt_scps_corp_bus_flow add partition p_scpsf1 values ('scpsf1')(
        subpartition p_scpsf1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
    )
;
alter table ${iml_schema}.evt_scps_corp_bus_flow modify partition p_scpsf1
    add subpartition p_scpsf1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
;

whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.evt_scps_corp_bus_flow_scpsf1_tm
...
insert into ${iml_schema}.evt_scps_corp_bus_flow_scpsf1_tm(
    evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,task_no -- 任务号
    ,ova_flow_num -- 全局流水号
...
select
    ...
    ,P1.ACCT_OPEN_DATE -- 客户开户日期
    ,P1.ORGAN_NO -- 机构编号
    ,P1.USER_NO -- 柜员编号
    ,P1.USER_ENABLE -- 开户状态代码
    ,decode(P1.ea_tempacctdt_end,to_date('00010101','yyyymmdd'),to_date('29991231','yyyymmdd'),P1.ea_tempacctdt_end) -- 临时户有效日期
    ,P1.EA_UPCRNA -- 上级单位名称
    ,nvl(trim(P1.EA_F_COMPANY_FIN_PAPTYPE),'0000')  -- 上级主管证件类型代码
    ...
    ,P1.ea_perpers_inval_date1 -- 第一证明文件到期日期
    ,nvl(trim(P1.EA_PAPERS_TYPE),'0000')  -- 第一证件类型代码
    ...
    ,nvl(regexp_substr(P1.EI_YEAR_LIMIT, '[0-9.]+'),0) -- 年累计限额
    ,nvl(regexp_substr(P1.EI_DAY_LIMIT, '[0-9.]+'),0) -- 日累计限额
    ,nvl(regexp_substr(P1.EI_DAY_TIMES, '[0-9.]+'),0) -- 日累计笔数
    ...
    ,nvl(trim(P1.FROZEN_FLAG),'-')  -- 对公止付状态代码
    ,P1.ACCT_NO -- 账户编号
    ,P1.CUST_NO -- 客户编号
    ,P1.CUST_NAME -- 客户名称
    ,nvl(trim(P1.EA_ACCT_CHAR),'-') -- 对公账户性质代码
    ,nvl(trim(P1.VISIT_SERVICE),0) -- 上门服务标志
    ,nvl(trim(P1.CHECK_TYPE),'-')  -- 核准方式代码
    ...
    ,P1.IS_PROXY -- 代理标志
    ,P1.PROXY_NAME -- 代理人姓名
    ,nvl(trim(P1.PROXY_PAPERS_TYPE),'0000')  -- 代理人证件类型代码
    ,P1.PROXY_PAPERS_NUMBER -- 代理人证件号码
    ,P1.PROXY_PHONE -- 代理人电话号码
    ,${iml_schema}.dateformat_max2(P1.PROXY_INVALDT) -- 代理人证件有效期
    ,P1.TRANS_STATE -- 对公处理状态代码
    ,P1.ACCT_CANCEL_DATE -- 客户销户日期
...
alter table ${iml_schema}.evt_scps_corp_bus_flow truncate partition p_scpsf1;

-- 3.3 exchage tm table and target table
alter table ${iml_schema}.evt_scps_corp_bus_flow exchange subpartition p_scpsf1_${batch_date} with table ${iml_schema}.evt_scps_corp_bus_flow_scpsf1_tm;
...
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'evt_scps_corp_bus_flow', partname => 'p_scpsf1_${batch_date}', ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);