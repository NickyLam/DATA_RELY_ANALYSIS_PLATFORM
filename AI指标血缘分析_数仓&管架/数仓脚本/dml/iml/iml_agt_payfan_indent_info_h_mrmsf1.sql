...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_agt_payfan_indent_info_h_mrmsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.agt_payfan_indent_info_h add partition p_mrmsf1 values ('mrmsf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.agt_payfan_indent_info_h_mrmsf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_payfan_indent_info_h partition for ('mrmsf1')
...
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.agt_payfan_indent_info_h_mrmsf1_tm purge;
drop table ${iml_schema}.agt_payfan_indent_info_h_mrmsf1_op purge;
drop table ${iml_schema}.agt_payfan_indent_info_h_mrmsf1_cl purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.agt_payfan_indent_info_h_mrmsf1_tm nologging
compress ${option_switch} for query high
as select
    agt_id -- 协议编号
    ,lp_id -- 法人编号
...
from ${iml_schema}.agt_payfan_indent_info_h partition for ('mrmsf1')
where 0=1
;

create table ${iml_schema}.agt_payfan_indent_info_h_mrmsf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_payfan_indent_info_h partition for ('mrmsf1') where 0=1;

create table ${iml_schema}.agt_payfan_indent_info_h_mrmsf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_payfan_indent_info_h partition for ('mrmsf1') where 0=1;

-- 3.1 get new data into table
-- mrms_tbl_df_order_txn-1
insert into ${iml_schema}.agt_payfan_indent_info_h_mrmsf1_tm(
    agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,tran_flow_num -- 交易流水号
    ,tran_dt -- 交易日期
...
select
    ...
    ,CASE WHEN R5.TARGET_CD_VAL IS NOT NULL THEN R5.TARGET_CD_VAL ELSE '@'||P1.PRODUCT_CATEGORIES END -- 协议单位类型代码
    ,P1.AGENT_NO -- 代理商编号
...
from ${iol_schema}.mrms_tbl_df_order_txn p1
    ...
    left join ${iml_schema}.ref_pub_cd_map r2 on P1.TXN_STA = R2.SRC_CODE_VAL
        ...
        AND R2.SRC_TAB_EN_NAME= 'MRMS_TBL_DF_ORDER_TXN'
    ...
    left join ${iml_schema}.ref_pub_cd_map r3 on P1.ACCOUNTTYPE = R3.SRC_CODE_VAL
        ...
        AND R3.SRC_TAB_EN_NAME= 'MRMS_TBL_DF_ORDER_TXN'
    ...
    left join ${iml_schema}.ref_pub_cd_map r4 on P1.BANK_ACCOUNT_TYPE = R4.SRC_CODE_VAL
        ...
        AND R4.SRC_TAB_EN_NAME= 'MRMS_TBL_DF_ORDER_TXN'
     ...
     left join ${iml_schema}.ref_pub_cd_map r5 on P1.PRODUCT_CATEGORIES = R5.SRC_CODE_VAL
        AND R5.SORC_SYS_CD= 'MRMS'
        AND R5.SRC_TAB_EN_NAME= 'MRMS_TBL_DF_ORDER_TXN'
        AND R5.SRC_FIELD_EN_NAME= 'PRODUCT_CATEGORIES'
        AND R5.TARGET_TAB_EN_NAME= 'AGT_PAYFAN_INDENT_INFO_H'
        AND R5.TARGET_TAB_FIELD_EN_NAME= 'AGT_CORP_TYPE_CD'
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.agt_payfan_indent_info_h_mrmsf1_tm 
  	                                group by 
  	                                        agt_id
  	                                        ,lp_id
  	                               having count(1) > 1);
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.agt_payfan_indent_info_h_mrmsf1_cl(
            agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,tran_flow_num -- 交易流水号
    ,tran_dt -- 交易日期
    ...
    ,teller_id -- 柜员编号
        ...
        into ${iml_schema}.agt_payfan_indent_info_h_mrmsf1_op(
            agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,tran_flow_num -- 交易流水号
    ,tran_dt -- 交易日期
...
from ${iml_schema}.agt_payfan_indent_info_h_mrmsf1_tm n
    full join (select * from ${iml_schema}.agt_payfan_indent_info_h_mrmsf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
            o.agt_id = n.agt_id
            and o.lp_id = n.lp_id
where (
...
from ${iml_schema}.agt_payfan_indent_info_h_mrmsf1_bk o
    left join ${iml_schema}.agt_payfan_indent_info_h_mrmsf1_op n
        on
            o.agt_id = n.agt_id
            and o.lp_id = n.lp_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iml_schema}.agt_payfan_indent_info_h_mrmsf1_cl d
        on
            o.agt_id = d.agt_id
            and o.lp_id = d.lp_id
...
--alter table ${iml_schema}.agt_payfan_indent_info_h truncate partition for ('mrmsf1') reuse storage;
...
alter table ${iml_schema}.agt_payfan_indent_info_h modify partition p_mrmsf1 
add subpartition p_mrmsf1_${batch_date} values(to_date('${batch_date}','YYYYMMDD')) ;

-- 4.3 exchange partition
alter table ${iml_schema}.agt_payfan_indent_info_h exchange subpartition p_mrmsf1_${batch_date} with table ${iml_schema}.agt_payfan_indent_info_h_mrmsf1_cl;
alter table ${iml_schema}.agt_payfan_indent_info_h exchange subpartition p_mrmsf1_20991231 with table ${iml_schema}.agt_payfan_indent_info_h_mrmsf1_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.agt_payfan_indent_info_h_mrmsf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'agt_payfan_indent_info_h', partname => 'p_mrmsf1_20991231',ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
