...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_agt_jd_appl_jdjrf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
drop table ${iml_schema}.agt_jd_appl_jdjrf1_tm purge;
drop table ${iml_schema}.agt_jd_appl_jdjrf1_ex purge;

-- 2.2 add partition for target table
alter table ${iml_schema}.agt_jd_appl add partition p_jdjrf1 values ('jdjrf1')(
        subpartition p_jdjrf1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
    )
;
alter table ${iml_schema}.agt_jd_appl modify partition p_jdjrf1
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.agt_jd_appl_jdjrf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_jd_appl partition for ('jdjrf1')
where create_dt < to_date('${batch_date}','yyyymmdd')
;

-- 2.4 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.agt_jd_appl_jdjrf1_tm
compress ${option_switch} for query high
as
select
    appl_id -- 申请编号
...
from ${iml_schema}.agt_jd_appl
where 0=1;

-- 2.5 create temp table
create table ${iml_schema}.agt_jd_appl_jdjrf1_ex nologging
compress ${option_switch} for query high
as select * from ${iml_schema}.agt_jd_appl partition for ('jdjrf1') where 0=1;

-- 2.1 insert data to tm table
-- rcrs_jdjr_iqp_loan_app-
insert into ${iml_schema}.agt_jd_appl_jdjrf1_tm(
    appl_id -- 申请编号
    ,lp_id -- 法人编号
    ,flow_num -- 流水号
    ,jd_appl_id -- 京东申请编号
...
select
    ...
    ,nvl(trim(P1.INTEREST_RATE),'0')*100 -- 日利率
    ...
    ,TO_NUMBER(nvl(trim(regexp_substr(P1.AUTO_SCORE, '[0-9.]+')),0)) -- 评分分值
...
from ${iol_schema}.rcrs_jdjr_iqp_loan_app p1
    left join ${iml_schema}.ref_pub_cd_map r1 on P1.CERT_TYPE = R1.SRC_CODE_VAL
        ...
        AND R1.TARGET_TAB_EN_NAME= 'AGT_JD_APPL'
    ...
    left join ${iml_schema}.ref_pub_cd_map r2 on P1.RATE_FLOAT_MODE = R2.SRC_CODE_VAL
        ...
        AND R2.TARGET_TAB_EN_NAME= 'AGT_JD_APPL'
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.agt_jd_appl_jdjrf1_tm 
  	                                group by 
  	                                        appl_id
  	                                        ,lp_id
  	                               having count(1) > 1);
...
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${iml_schema}.agt_jd_appl_jdjrf1_ex(
    appl_id -- 申请编号
    ,lp_id -- 法人编号
    ,flow_num -- 流水号
    ,jd_appl_id -- 京东申请编号
...
select
    ...
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iml_schema}.agt_jd_appl_jdjrf1_tm n
    full join ${iml_schema}.agt_jd_appl_jdjrf1_bk o
        on
            o.appl_id = n.appl_id
            and o.lp_id = n.lp_id
;
...
whenever sqlerror exit sql.sqlcode;
alter table ${iml_schema}.agt_jd_appl truncate partition for ('jdjrf1') reuse storage;

-- 3.2 exchage tm table and target table
alter table ${iml_schema}.agt_jd_appl exchange subpartition p_jdjrf1_${batch_date} with table ${iml_schema}.agt_jd_appl_jdjrf1_ex;

-- 3.3 drop subpartition last_date partition
whenever sqlerror continue none ;
alter table ${iml_schema}.agt_jd_appl drop subpartition p_jdjrf1_${last_date} update global indexes;

-- 4.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.agt_jd_appl to ${iml_schema};

-- 4.2 drop tm table
drop table ${iml_schema}.agt_jd_appl_jdjrf1_tm purge;
drop table ${iml_schema}.agt_jd_appl_jdjrf1_ex purge;
drop table ${iml_schema}.agt_jd_appl_jdjrf1_bk purge;

-- 5 gater table status
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'agt_jd_appl', partname => 'p_jdjrf1_${batch_date}', ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);