...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_evt_family_trust_prod_order_h_nfssf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.evt_family_trust_prod_order_h add partition p_nfssf1 values ('nfssf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.evt_family_trust_prod_order_h_nfssf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.evt_family_trust_prod_order_h partition for ('nfssf1')
...
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.evt_family_trust_prod_order_h_nfssf1_tm purge;
drop table ${iml_schema}.evt_family_trust_prod_order_h_nfssf1_op purge;
drop table ${iml_schema}.evt_family_trust_prod_order_h_nfssf1_cl purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.evt_family_trust_prod_order_h_nfssf1_tm nologging
compress ${option_switch} for query high
as select
    evt_id -- 事件编号
    ,lp_id -- 法人编号
...
from ${iml_schema}.evt_family_trust_prod_order_h partition for ('nfssf1')
where 0=1
;

create table ${iml_schema}.evt_family_trust_prod_order_h_nfssf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.evt_family_trust_prod_order_h partition for ('nfssf1') where 0=1;

create table ${iml_schema}.evt_family_trust_prod_order_h_nfssf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.evt_family_trust_prod_order_h partition for ('nfssf1') where 0=1;
...
-- nfss_ft_orderform-
insert into ${iml_schema}.evt_family_trust_prod_order_h_nfssf1_tm(
    evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,indent_flow_num -- 订单流水号
    ,prod_id -- 产品编号
...
select
    ...
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,'nfss_ft_orderform' -- 源表名称
    ,'nfssf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
  from ${iol_schema}.nfss_ft_orderform p1
  ...
  left join ${iml_schema}.ref_pub_cd_map r1 on p1.pay_status = r1.src_code_val
   ...
   and r1.src_tab_en_name = 'NFSS_FT_ORDERFORM'
  ...
  left join ${iml_schema}.ref_pub_cd_map r2 on p1.channel = r2.src_code_val
   ...
   and r2.src_tab_en_name = 'NFSS_FT_ORDERFORM'
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.evt_family_trust_prod_order_h_nfssf1_tm 
  	                                group by 
  	                                        evt_id
  	                                        ,lp_id
  	                               having count(1) > 1);
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.evt_family_trust_prod_order_h_nfssf1_cl(
            evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,indent_flow_num -- 订单流水号
    ,prod_id -- 产品编号
    ...
    ,cust_mgr_id -- 客户经理编号
        ...
        into ${iml_schema}.evt_family_trust_prod_order_h_nfssf1_op(
            evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,indent_flow_num -- 订单流水号
    ,prod_id -- 产品编号
...
from ${iml_schema}.evt_family_trust_prod_order_h_nfssf1_tm n
    full join (select * from ${iml_schema}.evt_family_trust_prod_order_h_nfssf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
            o.evt_id = n.evt_id
            and o.lp_id = n.lp_id
where (
...
from ${iml_schema}.evt_family_trust_prod_order_h_nfssf1_bk o
    left join ${iml_schema}.evt_family_trust_prod_order_h_nfssf1_op n
        on
            o.evt_id = n.evt_id
            and o.lp_id = n.lp_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iml_schema}.evt_family_trust_prod_order_h_nfssf1_cl d
        on
            o.evt_id = d.evt_id
            and o.lp_id = d.lp_id
...
--alter table ${iml_schema}.evt_family_trust_prod_order_h truncate partition for ('nfssf1') reuse storage;
...
alter table ${iml_schema}.evt_family_trust_prod_order_h modify partition p_nfssf1 
add subpartition p_nfssf1_${batch_date} values(to_date('${batch_date}','YYYYMMDD')) ;

-- 4.3 exchange partition
alter table ${iml_schema}.evt_family_trust_prod_order_h exchange subpartition p_nfssf1_${batch_date} with table ${iml_schema}.evt_family_trust_prod_order_h_nfssf1_cl;
alter table ${iml_schema}.evt_family_trust_prod_order_h exchange subpartition p_nfssf1_20991231 with table ${iml_schema}.evt_family_trust_prod_order_h_nfssf1_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.evt_family_trust_prod_order_h_nfssf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'evt_family_trust_prod_order_h', partname => 'p_nfssf1_20991231',ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
