...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_agt_acp_dubil_myhbf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
drop table ${iml_schema}.agt_acp_dubil_myhbf1_tm purge;
drop table ${iml_schema}.agt_acp_dubil_myhbf1_ex purge;

-- 2.2 add partition for target table
alter table ${iml_schema}.agt_acp_dubil add partition p_myhbf1 values ('myhbf1')(
        subpartition p_myhbf1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
    )
;
alter table ${iml_schema}.agt_acp_dubil modify partition p_myhbf1
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.agt_acp_dubil_myhbf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_acp_dubil partition for ('myhbf1')
where create_dt < to_date('${batch_date}','yyyymmdd')
;

-- 2.4 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.agt_acp_dubil_myhbf1_tm
compress ${option_switch} for query high
as
select
    agt_id -- 协议编号
...
create table ${iml_schema}.agt_acp_dubil_myhbf1_ex nologging
compress ${option_switch} for query high
as select * from ${iml_schema}.agt_acp_dubil partition for ('myhbf1') where 0=1;

-- 2.1 insert data to tm table
-- icms_myhb_acc_loan-
insert into ${iml_schema}.agt_acp_dubil_myhbf1_tm(
    agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,dubil_id -- 借据编号
    ,prod_id -- 产品编号
...
select
    ...
    ,NVL(TRIM(P1.CONTRACTTYPE),'00') -- 借据类型代码
    ...
    ,P1.CUSID -- 客户编号
    ,P1.CUSMGR -- 客户经理编号
    ,${iml_schema}.DATEFORMAT_MAX(P1.SETTLEDATE) -- 会计日期
    ...
    ,CASE WHEN R8.TARGET_CD_VAL IS NOT NULL THEN R8.TARGET_CD_VAL ELSE '@'||P1.ASSETCLASS END -- 贷款五级分类代码
    ,${iml_schema}.DATEFORMAT_MAX(P1.NEXTREPAYDATE) -- 下一还款日期
    ...
    ,P1.ENCASHACCTNO -- 收款帐号
    ,P1.REPAYACCTNO -- 还款帐号
    ,P1.ENCASHACCTNOBAK -- 收款账户编号
    ,P1.REPAYACCTNOBAK -- 还款账户编号
    ...
    ,NVL(TRIM(P1.REGIONCODE),'000000') -- 行政区划代码
    ,CASE WHEN R11.TARGET_CD_VAL IS NOT NULL THEN R11.TARGET_CD_VAL ELSE '@'||P1.CREDITCODE END -- 额度用途代码
...
from ${iol_schema}.icms_myhb_acc_loan p1
    left join ${iol_schema}.rcrs_myhb_acc_loan_clear p2 on p2.BILL_NO=p1.BILLNO and p2.start_dt <= to_date('${batch_date}','yyyymmdd') and p2.end_dt > to_date('${batch_date}','yyyymmdd')
    left join ${iml_schema}.ref_pub_cd_map r1 on P1.USEAREA = R1.SRC_CODE_VAL
        AND R1.SORC_SYS_CD= 'ICMS'
        AND R1.SRC_TAB_EN_NAME= 'ICMS_MYHB_ACC_LOAN'
    ...
    left join ${iml_schema}.ref_pub_cd_map r2 on P1.REPAYMODE = R2.SRC_CODE_VAL
        ...
        AND R2.SRC_TAB_EN_NAME= 'ICMS_MYHB_ACC_LOAN'
    ...
    left join ${iml_schema}.ref_pub_cd_map r4 on SUBSTR(P1.RATETYPE,1,1) = R4.SRC_CODE_VAL
        ...
        AND R4.SRC_TAB_EN_NAME= 'ICMS_MYHB_ACC_LOAN'
    ...
    left join ${iml_schema}.ref_pub_cd_map r5 on NVL(SUBSTR(P1.RATETYPE,2),' ') = R5.SRC_CODE_VAL
        ...
        AND R5.SRC_TAB_EN_NAME= 'ICMS_MYHB_ACC_LOAN'
    ...
    left join ${iml_schema}.ref_pub_cd_map r10 on P1.STATUS =R10.SRC_CODE_VAL
        ...
        AND R10.SRC_TAB_EN_NAME= 'ICMS_MYHB_ACC_LOAN'
        AND R10.SRC_FIELD_EN_NAME= 'STATUS'
        AND R10.TARGET_TAB_EN_NAME= 'AGT_ACP_DUBIL'
        AND R10.TARGET_TAB_FIELD_EN_NAME= 'CONT_STATUS_CD'
    left join ${iml_schema}.ref_pub_cd_map r8 on P1.ASSETCLASS = R8.SRC_CODE_VAL
        AND R8.SORC_SYS_CD= 'ICMS'
        AND R8.SRC_TAB_EN_NAME= 'ICMS_MYHB_ACC_LOAN'
        AND R8.SRC_FIELD_EN_NAME= 'ASSETCLASS'
    ...
    left join ${iml_schema}.ref_pub_cd_map r9 on P1.LOANUSE = R9.SRC_CODE_VAL
        ...
        AND R9.SRC_TAB_EN_NAME= 'ICMS_MYHB_ACC_LOAN'
    ...
    left join ${iml_schema}.ref_pub_cd_map r6 on P1.RATEFLOATMODE = R6.SRC_CODE_VAL
        ...
        AND R6.SRC_TAB_EN_NAME= 'ICMS_MYHB_ACC_LOAN'
    ...
    left join ${iml_schema}.ref_pub_cd_map r11 on P1.CREDITCODE = R11.SRC_CODE_VAL
        AND R11.SORC_SYS_CD= 'ICMS'
        AND R11.SRC_TAB_EN_NAME= 'ICMS_MYHB_ACC_LOAN'
        AND R11.SRC_FIELD_EN_NAME= 'CREDITCODE'
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.agt_acp_dubil_myhbf1_tm 
  	                                group by 
  	                                        agt_id
  	                                        ,lp_id
  	                               having count(1) > 1);
...
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${iml_schema}.agt_acp_dubil_myhbf1_ex(
    agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,dubil_id -- 借据编号
    ,prod_id -- 产品编号
...
select
    ...
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iml_schema}.agt_acp_dubil_myhbf1_tm n
    full join ${iml_schema}.agt_acp_dubil_myhbf1_bk o
        on
            o.agt_id = n.agt_id
            and o.lp_id = n.lp_id
;
...
whenever sqlerror exit sql.sqlcode;
alter table ${iml_schema}.agt_acp_dubil truncate partition for ('myhbf1') reuse storage;

-- 3.2 exchage tm table and target table
alter table ${iml_schema}.agt_acp_dubil exchange subpartition p_myhbf1_${batch_date} with table ${iml_schema}.agt_acp_dubil_myhbf1_ex;

-- 3.3 drop subpartition last_date partition
whenever sqlerror continue none ;
alter table ${iml_schema}.agt_acp_dubil drop subpartition p_myhbf1_${last_date} update global indexes;
...
drop table ${iml_schema}.agt_acp_dubil_myhbf1_tm purge;
drop table ${iml_schema}.agt_acp_dubil_myhbf1_ex purge;
drop table ${iml_schema}.agt_acp_dubil_myhbf1_bk purge;

-- 5 gater table status
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'agt_acp_dubil', partname => 'p_myhbf1_${batch_date}', ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);