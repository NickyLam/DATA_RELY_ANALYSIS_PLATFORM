...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_evt_ups_pay_flow_pppsi1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
drop table ${iml_schema}.evt_ups_pay_flow_pppsi1_tm purge;
alter table ${iml_schema}.evt_ups_pay_flow add partition p_pppsi1 values ('pppsi1')(
        subpartition p_pppsi1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
    )
;
alter table ${iml_schema}.evt_ups_pay_flow modify partition p_pppsi1
...
for i in 0 .. 14 loop
    ...
    v_sql := 'select count(0) from user_tab_subpartitions where table_name = upper(''evt_ups_pay_flow'') and subpartition_name = ''P_PPPSI1_'||bat_dt||''' ';
    dbms_output.put_line(v_sql);
    execute immediate v_sql into v_p_exists;
    dbms_output.put_line(v_p_exists);
    -- exists patitions
    if v_p_exists = 1 then 
        v_sql := 'alter table iml.evt_ups_pay_flow truncate subpartition P_PPPSI1_'||bat_dt ;
        dbms_output.put_line(v_sql);
        execute immediate v_sql;
    dbms_output.put_line('BBB');  
    --no exists partitions  
    else 
        v_sql := 'alter table iml.evt_ups_pay_flow modify partition p_PPPSi1 add subpartition P_PPPSI1_'||bat_dt||' values (to_date('''||bat_dt||''',''yyyymmdd'')) ';
        dbms_output.put_line(v_sql);
        execute immediate v_sql;
...
-- ppps_u_txn_payment-1
insert into ${iml_schema}.evt_ups_pay_flow(
    evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,batch_no -- 批次号
    ,tran_flow_num -- 交易流水号
...
select
    ...
    ,'ppps_u_txn_payment' -- 源表名称
    ,'pppsi1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.ppps_u_txn_payment p1
    left join ${iml_schema}.ref_pub_cd_map r1 on P1.R_P_FLG = R1.SRC_CODE_VAL
        AND R1.SORC_SYS_CD= 'PPPS'
        AND R1.SRC_TAB_EN_NAME= 'PPPS_U_TXN_PAYMENT'
        AND R1.SRC_FIELD_EN_NAME= 'R_P_FLG'
        AND R1.TARGET_TAB_EN_NAME= 'EVT_UPS_PAY_FLOW'
    ...
    left join ${iml_schema}.ref_pub_cd_map r2 on P1.PYEE_ACCT_TP = R2.SRC_CODE_VAL
        ...
        AND R2.SRC_TAB_EN_NAME= 'PPPS_U_TXN_PAYMENT'
        AND R2.SRC_FIELD_EN_NAME= 'PYEE_ACCT_TP'
        AND R2.TARGET_TAB_EN_NAME= 'EVT_UPS_PAY_FLOW'
...
-- 4.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.evt_ups_pay_flow to ${iml_schema};


-- 5 gater table status
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'evt_ups_pay_flow', partname => 'p_pppsi1_${batch_date}', ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);