...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_ast_col_org_guar_mimsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
drop table ${iml_schema}.ast_col_org_guar_mimsf1_tm purge;
drop table ${iml_schema}.ast_col_org_guar_mimsf1_ex purge;

-- 2.2 add partition for target table
alter table ${iml_schema}.ast_col_org_guar add partition p_mimsf1 values ('mimsf1')(
        subpartition p_mimsf1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
    )
;
alter table ${iml_schema}.ast_col_org_guar modify partition p_mimsf1
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.ast_col_org_guar_mimsf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.ast_col_org_guar partition for ('mimsf1')
where create_dt < to_date('${batch_date}','yyyymmdd')
;

-- 2.4 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.ast_col_org_guar_mimsf1_tm
compress ${option_switch} for query high
as
select
    asset_id -- 资产编号
...
from ${iml_schema}.ast_col_org_guar
where 0=1;

-- 2.5 create temp table
create table ${iml_schema}.ast_col_org_guar_mimsf1_ex nologging
compress ${option_switch} for query high
as select * from ${iml_schema}.ast_col_org_guar partition for ('mimsf1') where 0=1;

-- 2.1 insert data to tm table
-- mims_si_orgguarant-1
insert into ${iml_schema}.ast_col_org_guar_mimsf1_tm(
    asset_id -- 资产编号
    ,lp_id -- 法人编号
    ,guartor_type_cd -- 保证人类型代码
    ,guartor_id -- 保证人编号
...
select
    ...
    ,'mims_si_orgguarant' -- 源表名称
    ,'mimsf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.mims_si_orgguarant p1
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.ast_col_org_guar_mimsf1_tm 
  	                                group by 
  	                                        asset_id
  	                                        ,lp_id
  	                               having count(1) > 1);
...
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${iml_schema}.ast_col_org_guar_mimsf1_ex(
    asset_id -- 资产编号
    ,lp_id -- 法人编号
    ,guartor_type_cd -- 保证人类型代码
    ,guartor_id -- 保证人编号
...
select
    ...
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iml_schema}.ast_col_org_guar_mimsf1_tm n
    full join ${iml_schema}.ast_col_org_guar_mimsf1_bk o
        on
...
whenever sqlerror exit sql.sqlcode;
alter table ${iml_schema}.ast_col_org_guar truncate partition for ('mimsf1') reuse storage;

-- 3.2 exchage tm table and target table
alter table ${iml_schema}.ast_col_org_guar exchange subpartition p_mimsf1_${batch_date} with table ${iml_schema}.ast_col_org_guar_mimsf1_ex;

-- 3.3 drop subpartition last_date partition
whenever sqlerror continue none ;
alter table ${iml_schema}.ast_col_org_guar drop subpartition p_mimsf1_${last_date} update global indexes;

-- 4.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.ast_col_org_guar to ${iml_schema};

-- 4.2 drop tm table
drop table ${iml_schema}.ast_col_org_guar_mimsf1_tm purge;
drop table ${iml_schema}.ast_col_org_guar_mimsf1_ex purge;
drop table ${iml_schema}.ast_col_org_guar_mimsf1_bk purge;

-- 5 gater table status
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'ast_col_org_guar', partname => 'p_mimsf1_${batch_date}', ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);