...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_evt_ajb_repay_dtl_flow_myjbi2
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
drop table ${iml_schema}.evt_ajb_repay_dtl_flow_myjbi2_tm purge;
alter table ${iml_schema}.evt_ajb_repay_dtl_flow add partition p_myjbi2 values ('myjbi2')(
        subpartition p_myjbi2_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
    )
;
alter table ${iml_schema}.evt_ajb_repay_dtl_flow modify partition p_myjbi2
    add subpartition p_myjbi2_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
;

whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.evt_ajb_repay_dtl_flow_myjbi2_tm
...
select
    ...
    ,tran_flow_num -- 交易流水号
    ,dubil_id -- 借据编号
    ,plat_serv_fee_odd_no -- 平台服务费收费单号
...
-- icms_myjb_repay_cont_tail-
insert into ${iml_schema}.evt_ajb_repay_dtl_flow_myjbi2_tm(
    evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,repay_dt -- 还款日期
    ,tran_flow_num -- 交易流水号
...
select
    ...
    ,'icms_myjb_repay_cont_tail' -- 源表名称
    ,'myjbi2' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.icms_myjb_repay_cont_tail p1
left join ${iml_schema}.ref_pub_cd_map r1 on P1.REPAYTYPE = R1.SRC_CODE_VAL
        AND R1.SORC_SYS_CD= 'ICMS'
        AND R1.SRC_TAB_EN_NAME= 'ICMS_MYJB_REPAY_CONT_TAIL'
        AND R1.SRC_FIELD_EN_NAME= 'REPAYTYPE'
        AND R1.TARGET_TAB_EN_NAME= 'EVT_AJB_REPAY_DTL_FLOW'
...
alter table ${iml_schema}.evt_ajb_repay_dtl_flow truncate subpartition p_myjbi2_${batch_date} reuse storage;


-- 3.3 exchage tm table and target table
alter table ${iml_schema}.evt_ajb_repay_dtl_flow exchange subpartition p_myjbi2_${batch_date} with table ${iml_schema}.evt_ajb_repay_dtl_flow_myjbi2_tm;

-- 4.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.evt_ajb_repay_dtl_flow to ${iml_schema};

-- 4.2 drop tm table
drop table ${iml_schema}.evt_ajb_repay_dtl_flow_myjbi2_tm purge;

-- 5 gater table status
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'evt_ajb_repay_dtl_flow', partname => 'p_myjbi2_${batch_date}', ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);