...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_evt_consmt_fund_sign_dtl_ifmsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
drop table ${iml_schema}.evt_consmt_fund_sign_dtl_ifmsf1_tm purge;
alter table ${iml_schema}.evt_consmt_fund_sign_dtl add partition p_ifmsf1 values ('ifmsf1')(
        subpartition p_ifmsf1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
    )
;
alter table ${iml_schema}.evt_consmt_fund_sign_dtl modify partition p_ifmsf1
    add subpartition p_ifmsf1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
;

whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.evt_consmt_fund_sign_dtl_ifmsf1_tm
...
whenever sqlerror exit sql.sqlcode;
-- ifms_tbhisaccreq-1
insert into ${iml_schema}.evt_consmt_fund_sign_dtl_ifmsf1_tm(
    evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,appl_flow_num -- 申请流水号
    ,intior_flow_num -- 发起方流水号
...
select
    ...
    ,'ifms_tbhisaccreq' -- 源表名称
    ,'ifmsf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.ifms_tbhisaccreq p1
...
alter table ${iml_schema}.evt_consmt_fund_sign_dtl truncate partition p_ifmsf1;

-- 3.3 exchage tm table and target table
alter table ${iml_schema}.evt_consmt_fund_sign_dtl exchange subpartition p_ifmsf1_${batch_date} with table ${iml_schema}.evt_consmt_fund_sign_dtl_ifmsf1_tm;
...
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'evt_consmt_fund_sign_dtl', partname => 'p_ifmsf1_${batch_date}', ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);