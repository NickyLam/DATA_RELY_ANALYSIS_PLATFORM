...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_evt_ponl_bk_authen_flow_osbsi1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
drop table ${iml_schema}.evt_ponl_bk_authen_flow_osbsi1_tm purge;
alter table ${iml_schema}.evt_ponl_bk_authen_flow add partition p_osbsi1 values ('osbsi1')(
        subpartition p_osbsi1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
    )
;
alter table ${iml_schema}.evt_ponl_bk_authen_flow modify partition p_osbsi1
    add subpartition p_osbsi1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
;

whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.evt_ponl_bk_authen_flow_osbsi1_tm
...
from ${iml_schema}.evt_ponl_bk_authen_flow
where 0=1;
...
insert into ${iml_schema}.evt_ponl_bk_authen_flow_osbsi1_tm(
    evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,flow_num -- 流水号
    ,tran_tm -- 交易时间
...
select
    ...
    ,${iml_schema}.timeformat_min(P1.PTF_TRANSTIME) -- 交易时间
...
from ${iol_schema}.osbs_ats_pub_trade_flow p1
    left join ${iml_schema}.ref_pub_cd_map r1 on P1.PTF_CHANNEL = R1.SRC_CODE_VAL
        ...
        AND R1.SRC_TAB_EN_NAME= 'OSBS_ATS_PUB_TRADE_FLOW'
        AND R1.SRC_FIELD_EN_NAME= 'PTF_CHANNEL'
        AND R1.TARGET_TAB_EN_NAME= 'EVT_PONL_BK_AUTHEN_FLOW'
...
alter table ${iml_schema}.evt_ponl_bk_authen_flow truncate subpartition p_osbsi1_${batch_date} reuse storage;


-- 3.3 exchage tm table and target table
alter table ${iml_schema}.evt_ponl_bk_authen_flow exchange subpartition p_osbsi1_${batch_date} with table ${iml_schema}.evt_ponl_bk_authen_flow_osbsi1_tm;

-- 4.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.evt_ponl_bk_authen_flow to ${iml_schema};

-- 4.2 drop tm table
drop table ${iml_schema}.evt_ponl_bk_authen_flow_osbsi1_tm purge;

-- 5 gater table status
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'evt_ponl_bk_authen_flow', partname => 'p_osbsi1_${batch_date}', granularity => 'SUBPARTITION', degree => 8, cascade => true);