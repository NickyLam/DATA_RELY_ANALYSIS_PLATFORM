...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_ref_ibank_info_mpcsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
drop table ${iml_schema}.ref_ibank_info_mpcsf1_tm purge;
drop table ${iml_schema}.ref_ibank_info_mpcsf1_ex purge;

-- 2.2 add partition for target table
alter table ${iml_schema}.ref_ibank_info add partition p_mpcsf1 values ('mpcsf1')(
        subpartition p_mpcsf1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
    )
;
alter table ${iml_schema}.ref_ibank_info modify partition p_mpcsf1
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.ref_ibank_info_mpcsf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.ref_ibank_info partition for ('mpcsf1')
where create_dt < to_date('${batch_date}','yyyymmdd')
;

-- 2.4 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.ref_ibank_info_mpcsf1_tm
compress ${option_switch} for query high
as
select
    ibank_no -- 联行号
...
create table ${iml_schema}.ref_ibank_info_mpcsf1_ex nologging
compress ${option_switch} for query high
as select * from ${iml_schema}.ref_ibank_info partition for ('mpcsf1') where 0=1;

-- 2.1 insert data to tm table
-- mpcs_a49tefbrnnbr-
insert into ${iml_schema}.ref_ibank_info_mpcsf1_tm(
    ibank_no -- 联行号
    ,lp_id -- 法人编号
    ,bank_cls_id -- 行分类编号
    ,super_prtcpt_bank_no -- 上级参与行号
...
select
    ...
    ,P1.PHONE -- 电话号码
    ,P1.EMAILADDR -- 电子地址
    ,${iml_schema}.DATEFORMAT_MIN(P1.EFFDATE ) -- 生效日期
    ,${iml_schema}.DATEFORMAT_MIN(P1.INVDATE) -- 失效日期
    ,' ' -- 证书绑定CN域
    ,' ' -- 证书绑定SN域
    ,'99' -- 证书绑定状态
    ,to_date('00010101','yyyymmdd') -- 证书绑定生效日期
    ,to_date('20991231','yyyymmdd') -- 证书绑定失效日期
    ,' ' -- 最后修改操作员编号
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') -- 最后更新时间
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,'mpcs_a49tefbrnnbr' -- 源表名称
    ,'mpcsf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.mpcs_a49tefbrnnbr p1
    left join ${iol_schema}.BDMS_BMS_ECDS_BANK_DATA p2 on P1.brnnbr=P2.BANK_NO and P2.START_DT<=TO_DATE('${batch_date}','YYYYMMDD') AND P2.END_DT>TO_DATE('${batch_date}','YYYYMMDD')
where p1.start_dt <= to_date('${batch_date}','yyyymmdd') and p1.end_dt > to_date('${batch_date}','yyyymmdd')
    and P2.BANK_NO is null
;
commit;
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.ref_ibank_info_mpcsf1_tm 
  	                                group by 
  	                                        ibank_no
  	                                        ,lp_id
  	                               having count(1) > 1);
...
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${iml_schema}.ref_ibank_info_mpcsf1_ex(
    ibank_no -- 联行号
    ,lp_id -- 法人编号
    ,bank_cls_id -- 行分类编号
    ,super_prtcpt_bank_no -- 上级参与行号
...
select
    ...
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iml_schema}.ref_ibank_info_mpcsf1_tm n
    full join ${iml_schema}.ref_ibank_info_mpcsf1_bk o
        on
...
whenever sqlerror exit sql.sqlcode;
alter table ${iml_schema}.ref_ibank_info truncate partition for ('mpcsf1') reuse storage;

-- 3.2 exchage tm table and target table
alter table ${iml_schema}.ref_ibank_info exchange subpartition p_mpcsf1_${batch_date} with table ${iml_schema}.ref_ibank_info_mpcsf1_ex;

-- 3.3 drop subpartition last_date partition
whenever sqlerror continue none ;
alter table ${iml_schema}.ref_ibank_info drop subpartition p_mpcsf1_${last_date} update global indexes;
...
drop table ${iml_schema}.ref_ibank_info_mpcsf1_tm purge;
drop table ${iml_schema}.ref_ibank_info_mpcsf1_ex purge;
drop table ${iml_schema}.ref_ibank_info_mpcsf1_bk purge;

-- 5 gater table status
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'ref_ibank_info', partname => 'p_mpcsf1_${batch_date}', ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);