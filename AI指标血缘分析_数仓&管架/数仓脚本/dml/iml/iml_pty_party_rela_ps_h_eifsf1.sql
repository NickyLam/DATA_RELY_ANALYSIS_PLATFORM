...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_pty_party_rela_ps_h_eifsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.pty_party_rela_ps_h add partition p_eifsf1 values ('eifsf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.pty_party_rela_ps_h_eifsf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.pty_party_rela_ps_h partition for ('eifsf1')
...
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.pty_party_rela_ps_h_eifsf1_tm purge;
drop table ${iml_schema}.pty_party_rela_ps_h_eifsf1_op purge;
drop table ${iml_schema}.pty_party_rela_ps_h_eifsf1_cl purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.pty_party_rela_ps_h_eifsf1_tm nologging
compress ${option_switch} for query high
as select
    party_id -- 当事人编号
    ,lp_id -- 法人编号
...
from ${iml_schema}.pty_party_rela_ps_h partition for ('eifsf1')
where 0=1
;

create table ${iml_schema}.pty_party_rela_ps_h_eifsf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.pty_party_rela_ps_h partition for ('eifsf1') where 0=1;

create table ${iml_schema}.pty_party_rela_ps_h_eifsf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.pty_party_rela_ps_h partition for ('eifsf1') where 0=1;
...
insert into ${iml_schema}.pty_party_rela_ps_h_eifsf1_tm(
    party_id -- 当事人编号
    ,lp_id -- 法人编号
    ,sorc_sys_cd -- 源系统代码
    ,rela_ps_rela_type_cd -- 关联人关系类型代码
...
select
    ...
    ,'EIFS' -- 源系统代码
    ,'10303' -- 关联人关系类型代码
    ,to_date('00010101','yyyymmdd') -- 关联人参加工作时间
    ,' ' -- 关联人单位联系电话
    ,' ' -- 关联人电话号码
    ,' ' -- 关联人单位名称
    ,P1.RELA_NAME -- 关联人名称
    ...
    ,'9' -- 关联人性别代码
    ,0.0 -- 关联人月收入
    ,P1.RELA_CERT_NUM -- 关联人证件号码
    ,nvl(trim(P1.RELA_CERT_TYPE_CD),'0000') -- 关联人证件类型代码
    ,'99' -- 关联人职称代码
    ,'9' -- 关联人职务代码
    ,'-' -- 关联人职业代码
    ,'XXX' -- 国家和地区代码
...
where p1.start_dt <= to_date('${batch_date}','yyyymmdd') and p1.end_dt > to_date('${batch_date}','yyyymmdd')
    and P1.Rela_Cust_Rela_Cd='10303'  AND p1.Updated_Ts=to_timestamp('9999-12-31 00:00:00','YYYY-MM-DD HH24:MI:SS')
;
commit;
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.pty_party_rela_ps_h_eifsf1_tm 
  	                                group by 
  	                                        party_id
  	                                        ,lp_id
  	                                        ,sorc_sys_cd
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.pty_party_rela_ps_h_eifsf1_cl(
            party_id -- 当事人编号
    ,lp_id -- 法人编号
    ,sorc_sys_cd -- 源系统代码
    ,rela_ps_rela_type_cd -- 关联人关系类型代码
    ...
    ,rela_ps_phys_addr -- 关联人物理地址
        ...
        into ${iml_schema}.pty_party_rela_ps_h_eifsf1_op(
            party_id -- 当事人编号
    ,lp_id -- 法人编号
    ,sorc_sys_cd -- 源系统代码
    ,rela_ps_rela_type_cd -- 关联人关系类型代码
...
from ${iml_schema}.pty_party_rela_ps_h_eifsf1_tm n
    full join (select * from ${iml_schema}.pty_party_rela_ps_h_eifsf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
...
from ${iml_schema}.pty_party_rela_ps_h_eifsf1_bk o
    left join ${iml_schema}.pty_party_rela_ps_h_eifsf1_op n
        on
            o.party_id = n.party_id
            and o.lp_id = n.lp_id
            and o.sorc_sys_cd = n.sorc_sys_cd
    ...
    left join ${iml_schema}.pty_party_rela_ps_h_eifsf1_cl d
        on
...
--alter table ${iml_schema}.pty_party_rela_ps_h truncate partition for ('eifsf1') reuse storage;
...
alter table ${iml_schema}.pty_party_rela_ps_h modify partition p_eifsf1 
add subpartition p_eifsf1_${batch_date} values(to_date('${batch_date}','YYYYMMDD')) ;

-- 4.3 exchange partition
alter table ${iml_schema}.pty_party_rela_ps_h exchange subpartition p_eifsf1_${batch_date} with table ${iml_schema}.pty_party_rela_ps_h_eifsf1_cl;
alter table ${iml_schema}.pty_party_rela_ps_h exchange subpartition p_eifsf1_20991231 with table ${iml_schema}.pty_party_rela_ps_h_eifsf1_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.pty_party_rela_ps_h_eifsf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'pty_party_rela_ps_h', partname => 'p_eifsf1_20991231',ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
