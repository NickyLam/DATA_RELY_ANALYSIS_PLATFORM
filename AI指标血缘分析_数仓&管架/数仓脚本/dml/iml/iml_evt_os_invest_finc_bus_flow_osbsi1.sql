...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_evt_os_invest_finc_bus_flow_osbsi1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
drop table ${iml_schema}.evt_os_invest_finc_bus_flow_osbsi1_tm purge;
alter table ${iml_schema}.evt_os_invest_finc_bus_flow add partition p_osbsi1 values ('osbsi1')(
        subpartition p_osbsi1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
    )
;
alter table ${iml_schema}.evt_os_invest_finc_bus_flow modify partition p_osbsi1
    add subpartition p_osbsi1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
;

whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.evt_os_invest_finc_bus_flow_osbsi1_tm
...
-- osbs_fls_pub_trade_flow-1
insert into ${iml_schema}.evt_os_invest_finc_bus_flow_osbsi1_tm(
    evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,flow_num -- 流水号
    ,tran_dt -- 交易日期
...
select
    '201002'||p1.PTF_TRADE_FLOWNO -- 事件编号
    ,'9999' -- 法人编号
    ...
    ,'osbs_fls_pub_trade_flow' -- 源表名称
    ,'osbsi1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.osbs_fls_pub_trade_flow p1
    left join ${iml_schema}.ref_pub_cd_map r1 on P1.PTF_CURRENCY = R1.SRC_CODE_VAL
        AND R1.SORC_SYS_CD= 'OSBS'
        AND R1.SRC_TAB_EN_NAME= 'OSBS_FLS_PUB_TRADE_FLOW'
        AND R1.SRC_FIELD_EN_NAME= 'PTF_CURRENCY'
        AND R1.TARGET_TAB_EN_NAME= 'EVT_OS_INVEST_FINC_BUS_FLOW'
...
alter table ${iml_schema}.evt_os_invest_finc_bus_flow truncate subpartition p_osbsi1_${batch_date} reuse storage;


-- 3.3 exchage tm table and target table
alter table ${iml_schema}.evt_os_invest_finc_bus_flow exchange subpartition p_osbsi1_${batch_date} with table ${iml_schema}.evt_os_invest_finc_bus_flow_osbsi1_tm;

-- 4.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.evt_os_invest_finc_bus_flow to ${iml_schema};

-- 4.2 drop tm table
drop table ${iml_schema}.evt_os_invest_finc_bus_flow_osbsi1_tm purge;

-- 5 gater table status
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'evt_os_invest_finc_bus_flow', partname => 'p_osbsi1_${batch_date}', ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);