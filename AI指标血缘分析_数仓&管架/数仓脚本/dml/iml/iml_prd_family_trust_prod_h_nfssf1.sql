...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_prd_family_trust_prod_h_nfssf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.prd_family_trust_prod_h add partition p_nfssf1 values ('nfssf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.prd_family_trust_prod_h_nfssf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.prd_family_trust_prod_h partition for ('nfssf1')
...
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.prd_family_trust_prod_h_nfssf1_tm purge;
drop table ${iml_schema}.prd_family_trust_prod_h_nfssf1_op purge;
drop table ${iml_schema}.prd_family_trust_prod_h_nfssf1_cl purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.prd_family_trust_prod_h_nfssf1_tm nologging
compress ${option_switch} for query high
as select
    prod_id -- 产品编号
    ,lp_id -- 法人编号
...
from ${iml_schema}.prd_family_trust_prod_h partition for ('nfssf1')
where 0=1
;

create table ${iml_schema}.prd_family_trust_prod_h_nfssf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.prd_family_trust_prod_h partition for ('nfssf1') where 0=1;

create table ${iml_schema}.prd_family_trust_prod_h_nfssf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.prd_family_trust_prod_h partition for ('nfssf1') where 0=1;
...
-- nfss_ft_product-
insert into ${iml_schema}.prd_family_trust_prod_h_nfssf1_tm(
    prod_id -- 产品编号
    ,lp_id -- 法人编号
    ,init_prod_id -- 原产品编号
    ,prod_name -- 产品名称
...
select
  ...
  left join ${iml_schema}.ref_pub_cd_map r2 on p1.product_status = r2.src_code_val
   ...
   and r2.src_tab_en_name = 'NFSS_FT_PRODUCT'
  ...
  left join (select product_id,
               ...
               from ${iol_schema}.nfss_ft_networth
              where start_dt <= to_date('${batch_date}', 'yyyymmdd') and end_dt > to_date('${batch_date}', 'yyyymmdd')) p2
    on p1.product_id = p2.product_id
   and p2.rn = 1
  left join ${iol_schema}.nfss_ft_product_history p3
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.prd_family_trust_prod_h_nfssf1_tm 
  	                                group by 
  	                                        prod_id
  	                                        ,lp_id
  	                               having count(1) > 1);
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.prd_family_trust_prod_h_nfssf1_cl(
            prod_id -- 产品编号
    ,lp_id -- 法人编号
    ,init_prod_id -- 原产品编号
    ,prod_name -- 产品名称
    ...
    ,update_remark -- 更新备注
        ...
        into ${iml_schema}.prd_family_trust_prod_h_nfssf1_op(
            prod_id -- 产品编号
    ,lp_id -- 法人编号
    ,init_prod_id -- 原产品编号
    ,prod_name -- 产品名称
...
select
    ...
    ,nvl(n.acvmnt_base, o.acvmnt_base) as acvmnt_base -- 业绩比较基准
...
from ${iml_schema}.prd_family_trust_prod_h_nfssf1_tm n
    full join (select * from ${iml_schema}.prd_family_trust_prod_h_nfssf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
            o.prod_id = n.prod_id
            and o.lp_id = n.lp_id
where (
...
from ${iml_schema}.prd_family_trust_prod_h_nfssf1_bk o
    left join ${iml_schema}.prd_family_trust_prod_h_nfssf1_op n
        on
            o.prod_id = n.prod_id
            and o.lp_id = n.lp_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iml_schema}.prd_family_trust_prod_h_nfssf1_cl d
        on
            o.prod_id = d.prod_id
            and o.lp_id = d.lp_id
...
--alter table ${iml_schema}.prd_family_trust_prod_h truncate partition for ('nfssf1') reuse storage;
...
alter table ${iml_schema}.prd_family_trust_prod_h modify partition p_nfssf1 
add subpartition p_nfssf1_${batch_date} values(to_date('${batch_date}','YYYYMMDD')) ;

-- 4.3 exchange partition
alter table ${iml_schema}.prd_family_trust_prod_h exchange subpartition p_nfssf1_${batch_date} with table ${iml_schema}.prd_family_trust_prod_h_nfssf1_cl;
alter table ${iml_schema}.prd_family_trust_prod_h exchange subpartition p_nfssf1_20991231 with table ${iml_schema}.prd_family_trust_prod_h_nfssf1_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.prd_family_trust_prod_h_nfssf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'prd_family_trust_prod_h', partname => 'p_nfssf1_20991231',ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
