...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_agt_bond_outrht_repo_ctmsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
drop table ${iml_schema}.agt_bond_outrht_repo_ctmsf1_tm purge;
drop table ${iml_schema}.agt_bond_outrht_repo_ctmsf1_ex purge;

-- 2.2 add partition for target table
alter table ${iml_schema}.agt_bond_outrht_repo add partition p_ctmsf1 values ('ctmsf1')(
        subpartition p_ctmsf1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
    )
;
alter table ${iml_schema}.agt_bond_outrht_repo modify partition p_ctmsf1
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.agt_bond_outrht_repo_ctmsf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_bond_outrht_repo partition for ('ctmsf1')
where create_dt < to_date('${batch_date}','yyyymmdd')
;

-- 2.4 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.agt_bond_outrht_repo_ctmsf1_tm
compress ${option_switch} for query high
as
select
    agt_id -- 协议编号
...
from ${iml_schema}.agt_bond_outrht_repo
where 0=1;

-- 2.5 create temp table
create table ${iml_schema}.agt_bond_outrht_repo_ctmsf1_ex nologging
compress ${option_switch} for query high
as select * from ${iml_schema}.agt_bond_outrht_repo partition for ('ctmsf1') where 0=1;

-- 2.1 insert data to tm table
-- ctms_tbs_vs_payment_repodeals-
insert into ${iml_schema}.agt_bond_outrht_repo_ctmsf1_tm(
    agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,bus_id -- 业务编号
    ,bus_table_name -- 业务表名称
...
select
    ...
    ,P1.REPO_DAYS -- 回购天数
    ,TO_CHAR(P1.REPODEALS_ID_GRAND) -- 原业务编号
    ,P1.REPO_ID -- 回购编号
    ,CASE WHEN R2.TARGET_CD_VAL IS NOT NULL THEN R2.TARGET_CD_VAL ELSE '@'||P1.CLEARING_TYPE END -- 清算类型代码
...
from ${iol_schema}.ctms_tbs_vs_payment_repodeals p1
    left join ${iml_schema}.ref_pub_cd_map r1 on P1.BUYORSELL = R1.SRC_CODE_VAL
        ...
        AND R1.SRC_TAB_EN_NAME= 'CTMS_TBS_VS_PAYMENT_REPODEALS'
        AND R1.SRC_FIELD_EN_NAME= 'BUYORSELL'
        AND R1.TARGET_TAB_EN_NAME= 'AGT_BOND_OUTRHT_REPO'
    ...
    left join ${iml_schema}.ref_pub_cd_map r3 on P1.SETTLE_TYPE = R3.SRC_CODE_VAL
        ...
        AND R3.SRC_TAB_EN_NAME= 'CTMS_TBS_VS_PAYMENT_REPODEALS'
        AND R3.SRC_FIELD_EN_NAME= 'SETTLE_TYPE'
        AND R3.TARGET_TAB_EN_NAME= 'AGT_BOND_OUTRHT_REPO'
    ...
    left join ${iml_schema}.ref_pub_cd_map r4 on P1.SETTLE_TYPE2 = R4.SRC_CODE_VAL
        ...
        AND R4.SRC_TAB_EN_NAME= 'CTMS_TBS_VS_PAYMENT_REPODEALS'
        AND R4.SRC_FIELD_EN_NAME= 'SETTLE_TYPE2'
        AND R4.TARGET_TAB_EN_NAME= 'AGT_BOND_OUTRHT_REPO'
        AND R4.TARGET_TAB_FIELD_EN_NAME= 'EXP_STL_WAY_CD'
    left join ${iml_schema}.ref_pub_cd_map r2 on P1.CLEARING_TYPE = R2.SRC_CODE_VAL
        AND R2.SORC_SYS_CD= 'CTMS'
        AND R2.SRC_TAB_EN_NAME= 'CTMS_TBS_VS_PAYMENT_REPODEALS'
        AND R2.SRC_FIELD_EN_NAME= 'CLEARING_TYPE'
        AND R2.TARGET_TAB_EN_NAME= 'AGT_BOND_OUTRHT_REPO'
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.agt_bond_outrht_repo_ctmsf1_tm 
  	                                group by 
  	                                        agt_id
  	                                        ,lp_id
  	                               having count(1) > 1);
...
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${iml_schema}.agt_bond_outrht_repo_ctmsf1_ex(
    agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,bus_id -- 业务编号
    ,bus_table_name -- 业务表名称
...
select
    ...
    ,nvl(n.fir_net_price, o.fir_net_price) as fir_net_price -- 首次净价
    ...
    ,case when (
                ...
                or o.fir_net_price <> n.fir_net_price
    ...
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iml_schema}.agt_bond_outrht_repo_ctmsf1_tm n
    full join ${iml_schema}.agt_bond_outrht_repo_ctmsf1_bk o
        on
            o.agt_id = n.agt_id
            and o.lp_id = n.lp_id
;
...
whenever sqlerror exit sql.sqlcode;
alter table ${iml_schema}.agt_bond_outrht_repo truncate partition for ('ctmsf1') reuse storage;

-- 3.2 exchage tm table and target table
alter table ${iml_schema}.agt_bond_outrht_repo exchange subpartition p_ctmsf1_${batch_date} with table ${iml_schema}.agt_bond_outrht_repo_ctmsf1_ex;

-- 3.3 drop subpartition last_date partition
whenever sqlerror continue none ;
alter table ${iml_schema}.agt_bond_outrht_repo drop subpartition p_ctmsf1_${last_date} update global indexes;

-- 4.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.agt_bond_outrht_repo to ${iml_schema};

-- 4.2 drop tm table
drop table ${iml_schema}.agt_bond_outrht_repo_ctmsf1_tm purge;
drop table ${iml_schema}.agt_bond_outrht_repo_ctmsf1_ex purge;
drop table ${iml_schema}.agt_bond_outrht_repo_ctmsf1_bk purge;

-- 5 gater table status
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'agt_bond_outrht_repo', partname => 'p_ctmsf1_${batch_date}', ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);