...
select
    ...
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,'ibms_ttrd_accounting_secu_obj_his' -- 源表名称
    ,'ibmsf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.ibms_ttrd_accounting_secu_obj_his p1
  left join ${iol_schema}.ibms_ttrd_acc_secu t5
    on p1.secu_acct_id = t5.accid
AND T5.START_DT <= to_date('${batch_date}','yyyymmdd') 
AND T5.END_DT > to_date('${batch_date}','yyyymmdd')
  left join ${iol_schema}.ibms_ttrd_acctg_account_type t6
    on t5.acting_type = t6.typeid
AND T6.START_DT <= to_date('${batch_date}','yyyymmdd') 
AND T6.END_DT > to_date('${batch_date}','yyyymmdd')
  left join ${iml_schema}.ref_pub_cd_map r11
    on to_char(t6.i9_class) = r11.src_code_val
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.agt_secu_acct_accti_bal_h_ibmsf1_tm 
  	                                group by 
  	                                        agt_id
  	                                        ,lp_id
  	                                        ,obj_id
...
for cur in (select subpartition_name 
             ...
             where table_owner=upper('${iml_schema}') 
               and table_name=upper('agt_secu_acct_accti_bal_h') 
               and substr(subpartition_name,1,8)=upper('p_ibmsf1')
               and substr(subpartition_name,10) >= '${batch_date}' 
               and substr(subpartition_name,10) < '20991231') loop
    v_sql := 'alter table ${iml_schema}.agt_secu_acct_accti_bal_h drop subpartition ' || cur.subpartition_name;
...
whenever sqlerror exit sql.sqlcode;
alter table ${iml_schema}.agt_secu_acct_accti_bal_h modify partition p_ibmsf1 
add subpartition p_ibmsf1_${batch_date} values(to_date('${batch_date}','YYYYMMDD')) ;

-- 4.3 exchange partition
alter table ${iml_schema}.agt_secu_acct_accti_bal_h exchange subpartition p_ibmsf1_${batch_date} with table ${iml_schema}.agt_secu_acct_accti_bal_h_ibmsf1_cl;
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'agt_secu_acct_accti_bal_h', partname => 'p_ibmsf1_20991231',ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
