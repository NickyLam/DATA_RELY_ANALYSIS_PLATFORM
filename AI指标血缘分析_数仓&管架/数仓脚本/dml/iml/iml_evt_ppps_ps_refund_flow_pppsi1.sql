...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_evt_ppps_ps_refund_flow_pppsi1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.evt_ppps_ps_refund_flow add partition p_pppsi1 values ('pppsi1')(
        subpartition p_pppsi1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
    )
;
alter table ${iml_schema}.evt_ppps_ps_refund_flow modify partition p_pppsi1
...
for i in 0 .. 14 loop
    ...
    v_sql := 'select count(0) from user_tab_subpartitions where table_name = upper(''evt_ppps_ps_refund_flow'') and subpartition_name = ''P_PPPSI1_'||bat_dt||''' ';
    dbms_output.put_line(v_sql);
    execute immediate v_sql into v_p_exists;
    dbms_output.put_line(v_p_exists);
    -- exists patitions
    if v_p_exists = 1 then 
        v_sql := 'alter table iml.evt_ppps_ps_refund_flow truncate subpartition P_PPPSI1_'||bat_dt ;
        dbms_output.put_line(v_sql);
        execute immediate v_sql;
    dbms_output.put_line('BBB');  
    --no exists partitions  
    else 
        v_sql := 'alter table iml.evt_ppps_ps_refund_flow modify partition p_pppsi1 add subpartition P_PPPSI1_'||bat_dt||' values (to_date('''||bat_dt||''',''yyyymmdd'')) ';
        dbms_output.put_line(v_sql);
        execute immediate v_sql;
...
-- ppps_e_txn_refund-1
insert into ${iml_schema}.evt_ppps_ps_refund_flow(
    evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,tran_flow_num -- 交易流水号
...
select
    ...
    ,P1.ORI_TRX_AMT_D -- 原订单金额
    ,P1.ORI_DBTR_BANK_ID -- 原付款行交易流水号
    ,P1.ORI_TRX_ID -- 原交易流水号
    ,P1.UPDATE_TIME -- 最后更新时间
    ,to_date(P1.sys_date,'yyyy-mm-dd') as etl_dt -- ETL处理日期
    ,'ppps_e_txn_refund' -- 源表名称
    ,'pppsi1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.ppps_e_txn_refund p1
where  1 = 1 
...
-- 4.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.evt_ppps_ps_refund_flow to ${iml_schema};

-- 5 gater table status
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'evt_ppps_ps_refund_flow', partname => 'p_pppsi1_${batch_date}', ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);