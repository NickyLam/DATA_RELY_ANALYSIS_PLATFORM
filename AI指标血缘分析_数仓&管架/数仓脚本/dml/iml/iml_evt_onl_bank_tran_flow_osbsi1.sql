...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_evt_onl_bank_tran_flow_osbsi1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
drop table ${iml_schema}.evt_onl_bank_tran_flow_osbsi1_tm purge;
alter table ${iml_schema}.evt_onl_bank_tran_flow add partition p_osbsi1 values ('osbsi1')(
        subpartition p_osbsi1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
    )
;
alter table ${iml_schema}.evt_onl_bank_tran_flow modify partition p_osbsi1
    add subpartition p_osbsi1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
;

whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.evt_onl_bank_tran_flow_osbsi1_tm
...
-- osbs_pub_trade_flow-
insert into ${iml_schema}.evt_onl_bank_tran_flow_osbsi1_tm(
    evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,flow_num -- 流水号
    ,tran_dt -- 交易日期
...
alter table ${iml_schema}.evt_onl_bank_tran_flow truncate subpartition p_osbsi1_${batch_date} reuse storage;


-- 3.3 exchage tm table and target table
alter table ${iml_schema}.evt_onl_bank_tran_flow exchange subpartition p_osbsi1_${batch_date} with table ${iml_schema}.evt_onl_bank_tran_flow_osbsi1_tm;
...
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'evt_onl_bank_tran_flow', partname => 'p_osbsi1_${batch_date}', granularity => 'SUBPARTITION', degree => 8, cascade => true);