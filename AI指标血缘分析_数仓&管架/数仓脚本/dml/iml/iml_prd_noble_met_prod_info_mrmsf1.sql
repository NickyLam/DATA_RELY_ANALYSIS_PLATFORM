...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_prd_noble_met_prod_info_mrmsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.prd_noble_met_prod_info add partition p_mrmsf1 values ('mrmsf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.prd_noble_met_prod_info_mrmsf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.prd_noble_met_prod_info partition for ('mrmsf1')
;

-- 2.2 drop temp table
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.prd_noble_met_prod_info_mrmsf1_tm purge;
drop table ${iml_schema}.prd_noble_met_prod_info_mrmsf1_op purge;
drop table ${iml_schema}.prd_noble_met_prod_info_mrmsf1_cl purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.prd_noble_met_prod_info_mrmsf1_tm nologging
compress ${option_switch} for query high
as select
    prod_id -- 产品编号
    ,lp_id -- 法人编号
...
from ${iml_schema}.prd_noble_met_prod_info partition for ('mrmsf1')
where 0=1
;

create table ${iml_schema}.prd_noble_met_prod_info_mrmsf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.prd_noble_met_prod_info partition for ('mrmsf1') where 0=1;

create table ${iml_schema}.prd_noble_met_prod_info_mrmsf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.prd_noble_met_prod_info partition for ('mrmsf1') where 0=1;

-- 3.1 get new data into table
-- mrms_tbl_product_info_detail-
insert into ${iml_schema}.prd_noble_met_prod_info_mrmsf1_tm(
    prod_id -- 产品编号
    ,lp_id -- 法人编号
    ,ser_num -- 序列号
    ,merchd_id -- 商品编号
...
select
    ...
    ,P1.ADDDATA2 -- 附加数据2
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,'mrms_tbl_product_info_detail' -- 源表名称
    ,'mrmsf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.mrms_tbl_product_info_detail p1
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.prd_noble_met_prod_info_mrmsf1_cl(
            prod_id -- 产品编号
    ,lp_id -- 法人编号
    ,ser_num -- 序列号
    ,merchd_id -- 商品编号
    ...
    ,addit_data_2 -- 附加数据2
        ...
        into ${iml_schema}.prd_noble_met_prod_info_mrmsf1_op(
            prod_id -- 产品编号
    ,lp_id -- 法人编号
    ,ser_num -- 序列号
    ,merchd_id -- 商品编号
...
from ${iml_schema}.prd_noble_met_prod_info_mrmsf1_tm n
    full join (select * from ${iml_schema}.prd_noble_met_prod_info_mrmsf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
            o.prod_id = n.prod_id
            and o.lp_id = n.lp_id
where (
...
from ${iml_schema}.prd_noble_met_prod_info_mrmsf1_bk o
    left join ${iml_schema}.prd_noble_met_prod_info_mrmsf1_op n
        on
            o.prod_id = n.prod_id
            and o.lp_id = n.lp_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iml_schema}.prd_noble_met_prod_info_mrmsf1_cl d
        on
            o.prod_id = d.prod_id
            and o.lp_id = d.lp_id
...
alter table ${iml_schema}.prd_noble_met_prod_info truncate partition for ('mrmsf1') reuse storage;

-- 4.2 exchange partition
alter table ${iml_schema}.prd_noble_met_prod_info exchange subpartition p_mrmsf1_19000101 with table ${iml_schema}.prd_noble_met_prod_info_mrmsf1_cl;
alter table ${iml_schema}.prd_noble_met_prod_info exchange subpartition p_mrmsf1_20991231 with table ${iml_schema}.prd_noble_met_prod_info_mrmsf1_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.prd_noble_met_prod_info_mrmsf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'prd_noble_met_prod_info', partname => 'p_mrmsf1_20991231', granularity => 'SUBPARTITION', degree => 8, cascade => true);
