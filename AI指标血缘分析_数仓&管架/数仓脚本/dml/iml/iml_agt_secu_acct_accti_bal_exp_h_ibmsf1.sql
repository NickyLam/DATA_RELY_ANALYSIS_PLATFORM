...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_agt_secu_acct_accti_bal_exp_h_ibmsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.agt_secu_acct_accti_bal_exp_h add partition p_ibmsf1 values ('ibmsf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.agt_secu_acct_accti_bal_exp_h_ibmsf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_secu_acct_accti_bal_exp_h partition for ('ibmsf1')
...
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.agt_secu_acct_accti_bal_exp_h_ibmsf1_tm purge;
drop table ${iml_schema}.agt_secu_acct_accti_bal_exp_h_ibmsf1_op purge;
drop table ${iml_schema}.agt_secu_acct_accti_bal_exp_h_ibmsf1_cl purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.agt_secu_acct_accti_bal_exp_h_ibmsf1_tm nologging
compress ${option_switch} for query high
as select
    agt_id -- 协议编号
    ,lp_id -- 法人编号
...
from ${iml_schema}.agt_secu_acct_accti_bal_exp_h partition for ('ibmsf1')
where 0=1
;

create table ${iml_schema}.agt_secu_acct_accti_bal_exp_h_ibmsf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_secu_acct_accti_bal_exp_h partition for ('ibmsf1') where 0=1;

create table ${iml_schema}.agt_secu_acct_accti_bal_exp_h_ibmsf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_secu_acct_accti_bal_exp_h partition for ('ibmsf1') where 0=1;

-- 3.1 get new data into table
-- ibms_ttrd_accounting_secu_obj_comp
insert into ${iml_schema}.agt_secu_acct_accti_bal_exp_h_ibmsf1_tm(
    agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,obj_id -- 对象编号
    ,task_id -- 任务编号
...
select
    ...
    ,case when t5.acting_type ='596' then '403010100001' else nvl(p2.prod_code,' ') end -- 标准产品编号
    ,CASE WHEN R11.TARGET_CD_VAL IS NOT NULL THEN R11.TARGET_CD_VAL   --- 0日调账及以后，三分类从 ibms_ttrd_acctg_account_type 取
    ...
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,'ibms_ttrd_accounting_secu_obj_comp' -- 源表名称
    ,'ibmsf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.ibms_ttrd_accounting_secu_obj_comp p1
...
AND T6.END_DT > to_date('${batch_date}','yyyymmdd')
    left join ${iol_schema}.ibms_ttrd_instrument p2 on P1.i_code = p2.i_code
...
where  1 = 1 
    AND TO_DATE(P1.BEG_DATE,'YYYY-MM-DD') <= TO_DATE('${batch_date}','yyyymmdd') and TO_DATE(P1.END_DATE,'YYYY-MM-DD')>TO_DATE('${batch_date}','yyyymmdd')
;
commit;
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.agt_secu_acct_accti_bal_exp_h_ibmsf1_tm 
  	                                group by 
  	                                        agt_id
  	                                        ,lp_id
  	                                        ,obj_id
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.agt_secu_acct_accti_bal_exp_h_ibmsf1_cl(
            agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,obj_id -- 对象编号
    ,task_id -- 任务编号
    ...
    ,comb_tran_id -- 组合交易编号
        ...
        into ${iml_schema}.agt_secu_acct_accti_bal_exp_h_ibmsf1_op(
            agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,obj_id -- 对象编号
    ,task_id -- 任务编号
...
select
    ...
    ,nvl(n.pre_recv_int_income, o.pre_recv_int_income) as pre_recv_int_income -- 税前预收利息收入
...
from ${iml_schema}.agt_secu_acct_accti_bal_exp_h_ibmsf1_tm n
    full join (select * from ${iml_schema}.agt_secu_acct_accti_bal_exp_h_ibmsf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
            o.agt_id = n.agt_id
            and o.lp_id = n.lp_id
...
select
    ...
    ,o.pre_recv_int_income -- 税前预收利息收入
...
from ${iml_schema}.agt_secu_acct_accti_bal_exp_h_ibmsf1_bk o
    left join ${iml_schema}.agt_secu_acct_accti_bal_exp_h_ibmsf1_op n
        on
            o.agt_id = n.agt_id
            and o.lp_id = n.lp_id
            and o.obj_id = n.obj_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iml_schema}.agt_secu_acct_accti_bal_exp_h_ibmsf1_cl d
        on
            o.agt_id = d.agt_id
            and o.lp_id = d.lp_id
...
--truncate table ${iml_schema}.agt_secu_acct_accti_bal_exp_h;
--alter table ${iml_schema}.agt_secu_acct_accti_bal_exp_h truncate partition for ('ibmsf1') reuse storage;
...
for cur in (select subpartition_name 
             ...
             where table_owner=upper('${iml_schema}') 
               and table_name=upper('agt_secu_acct_accti_bal_exp_h') 
               and substr(subpartition_name,1,8)=upper('p_ibmsf1')
               and substr(subpartition_name,10) >= '${batch_date}' 
               and substr(subpartition_name,10) < '20991231') loop
    v_sql := 'alter table ${iml_schema}.agt_secu_acct_accti_bal_exp_h drop subpartition ' || cur.subpartition_name;
...
whenever sqlerror exit sql.sqlcode;
alter table ${iml_schema}.agt_secu_acct_accti_bal_exp_h modify partition p_ibmsf1 
add subpartition p_ibmsf1_${batch_date} values(to_date('${batch_date}','YYYYMMDD')) ;

-- 4.3 exchange partition
alter table ${iml_schema}.agt_secu_acct_accti_bal_exp_h exchange subpartition p_ibmsf1_${batch_date} with table ${iml_schema}.agt_secu_acct_accti_bal_exp_h_ibmsf1_cl;
alter table ${iml_schema}.agt_secu_acct_accti_bal_exp_h exchange subpartition p_ibmsf1_20991231 with table ${iml_schema}.agt_secu_acct_accti_bal_exp_h_ibmsf1_op;

-- 5.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.agt_secu_acct_accti_bal_exp_h to ${iml_schema};
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.agt_secu_acct_accti_bal_exp_h_ibmsf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'agt_secu_acct_accti_bal_exp_h', partname => 'p_ibmsf1_20991231',ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
