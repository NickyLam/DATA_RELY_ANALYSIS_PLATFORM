...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_pty_party_rating_h_eifsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.pty_party_rating_h add partition p_eifsf1 values ('eifsf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.pty_party_rating_h_eifsf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.pty_party_rating_h partition for ('eifsf1')
...
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.pty_party_rating_h_eifsf1_tm purge;
drop table ${iml_schema}.pty_party_rating_h_eifsf1_op purge;
drop table ${iml_schema}.pty_party_rating_h_eifsf1_cl purge;
drop table ${iml_schema}.pty_party_rating_h_tmp purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.pty_party_rating_h_eifsf1_tm nologging
compress ${option_switch} for query high
as select
    party_id -- 当事人编号
    ,lp_id -- 法人编号
...
from ${iml_schema}.pty_party_rating_h partition for ('eifsf1')
where 0=1
;

create table ${iml_schema}.pty_party_rating_h_eifsf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.pty_party_rating_h partition for ('eifsf1') where 0=1;

create table ${iml_schema}.pty_party_rating_h_eifsf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.pty_party_rating_h partition for ('eifsf1') where 0=1;

create table ${iml_schema}.pty_party_rating_h_tmp nologging
compress ${option_switch} for query high
as
(select t.*,row_number() over(partition by party_id order by updated_ts desc) rn
...
-- eifs_t01_corp_cust_ext_info-3
insert into ${iml_schema}.pty_party_rating_h_eifsf1_tm(
    party_id -- 当事人编号
    ,lp_id -- 法人编号
    ,sorc_sys_cd -- 源系统代码
    ,party_rating_type_cd -- 当事人评级类型代码
...
select
    ...
    ,' ' -- 评级结果代码
    ,' ' -- 内评系统任务流水号
    ,nvl(trim(P1.CUST_RISK_LEVEL_CD),'0')  -- 评级等级代码
    ,0 -- 限额
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
...
-- and p1.start_dt <= to_date('${batch_date}','yyyymmdd') and p1.end_dt > to_date('${batch_date}','yyyymmdd')
    and trim( P1.CUST_RISK_LEVEL_CD ) is  not null
;
commit;
...
select
    ...
    ,'eifsf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iml_schema}.pty_party_rating_h_tmp p1
...
-- eifs_t01_per_cust_ext_info-security_grade
insert into ${iml_schema}.pty_party_rating_h_eifsf1_tm(
    party_id -- 当事人编号
    ,lp_id -- 法人编号
    ,sorc_sys_cd -- 源系统代码
...
select
    ...
    ,' ' -- 评级结果代码
    ,' ' -- 内评系统任务流水号
    ,NVL(TRIM(P1.SECURITY_GRADE),'X') -- 评级等级代码
    ,'0' -- 限额
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,'eifs_t01_per_cust_ext_info' -- 源表名称
    ,'eifsf1' -- 任务编码
...
where p1.rn=1
-- and p1.start_dt <= to_date('${batch_date}','yyyymmdd') and p1.end_dt > to_date('${batch_date}','yyyymmdd')
    and trim( P1.SECURITY_GRADE ) is  not null
;
commit;

-- eifs_t06_corp_rating_info-
insert into ${iml_schema}.pty_party_rating_h_eifsf1_tm(
    party_id -- 当事人编号
    ,lp_id -- 法人编号
    ,sorc_sys_cd -- 源系统代码
...
select
    ...
    ,P1.GRADE_ID -- 序号
    ,' ' -- 评级机构编号
    ,P1.RATING_ORG_NAME -- 评级机构名称
    ,${iml_schema}.dateformat_min(P1.EXT_CRDT_RATING_DT)-- 评级日期
    ,'0' -- 评级分值
    ,${iml_schema}.dateformat_min(P1.EXT_CRDT_EFFECT_DT) -- 评级生效日期
    ,${iml_schema}.dateformat_max2(P1.EXT_CRDT_INVALID_DT) -- 评级失效日期
    ,' ' -- 评级结果代码
    ,' ' -- 内评系统任务流水号
    ,NVL(TRIM(P1.EXT_CRDT_RATING_CD),'999') -- 评级等级代码
    ,'0' -- 限额
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,'eifs_t06_corp_rating_info' -- 源表名称
    ,'eifsf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.eifs_t06_corp_rating_info p1
    left join ${iol_schema}.eifs_t00_corp_cust_no_ref p2 on P1.PARTY_ID=P2.PARTY_ID AND p2.start_dt<= to_date('${batch_date}','yyyymmdd') 
and p2.end_dt > to_date('${batch_date}','yyyymmdd')
where p1.start_dt <= to_date('${batch_date}','yyyymmdd') and p1.end_dt > to_date('${batch_date}','yyyymmdd')
    and trim( P1.EXT_CRDT_RATING_CD ) is  not null
;
commit;

-- eifs_t06_per_rating_info-
insert into ${iml_schema}.pty_party_rating_h_eifsf1_tm(
    party_id -- 当事人编号
    ,lp_id -- 法人编号
    ,sorc_sys_cd -- 源系统代码
...
select
    ...
    ,'0' -- 限额
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,'eifs_t06_per_rating_info' -- 源表名称
    ,'eifsf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.eifs_t06_per_rating_info p1
    left join ${iol_schema}.eifs_t00_per_cust_no_ref p2 on P1.PARTY_ID=P2.PARTY_ID AND p2.start_dt<= to_date('${batch_date}','yyyymmdd') 
and p2.end_dt > to_date('${batch_date}','yyyymmdd')
where p1.start_dt <= to_date('${batch_date}','yyyymmdd') and p1.end_dt > to_date('${batch_date}','yyyymmdd')
    and trim( P1.EXT_CRDT_RATING_CD ) is  not null
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.pty_party_rating_h_eifsf1_tm 
  	                                group by 
  	                                        party_id
  	                                        ,lp_id
  	                                        ,sorc_sys_cd
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.pty_party_rating_h_eifsf1_cl(
            party_id -- 当事人编号
    ,lp_id -- 法人编号
    ,sorc_sys_cd -- 源系统代码
    ,party_rating_type_cd -- 当事人评级类型代码
    ...
    ,lmt -- 限额
        ...
        into ${iml_schema}.pty_party_rating_h_eifsf1_op(
            party_id -- 当事人编号
    ,lp_id -- 法人编号
    ,sorc_sys_cd -- 源系统代码
    ,party_rating_type_cd -- 当事人评级类型代码
...
from ${iml_schema}.pty_party_rating_h_eifsf1_tm n
    full join (select * from ${iml_schema}.pty_party_rating_h_eifsf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
...
from ${iml_schema}.pty_party_rating_h_eifsf1_bk o
    left join ${iml_schema}.pty_party_rating_h_eifsf1_op n
        on
            o.party_id = n.party_id
            and o.lp_id = n.lp_id
            and o.sorc_sys_cd = n.sorc_sys_cd
    ...
    left join ${iml_schema}.pty_party_rating_h_eifsf1_cl d
        on
...
--alter table ${iml_schema}.pty_party_rating_h truncate partition for ('eifsf1') reuse storage;
...
alter table ${iml_schema}.pty_party_rating_h modify partition p_eifsf1 
add subpartition p_eifsf1_${batch_date} values(to_date('${batch_date}','YYYYMMDD')) ;

-- 4.3 exchange partition
alter table ${iml_schema}.pty_party_rating_h exchange subpartition p_eifsf1_${batch_date} with table ${iml_schema}.pty_party_rating_h_eifsf1_cl;
alter table ${iml_schema}.pty_party_rating_h exchange subpartition p_eifsf1_20991231 with table ${iml_schema}.pty_party_rating_h_eifsf1_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.pty_party_rating_h_eifsf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'pty_party_rating_h', partname => 'p_eifsf1_20991231',ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
