...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_pty_finc_cust_risk_estim_h_nfssf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.pty_finc_cust_risk_estim_h add partition p_nfssf1 values ('nfssf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.pty_finc_cust_risk_estim_h_nfssf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.pty_finc_cust_risk_estim_h partition for ('nfssf1')
...
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.pty_finc_cust_risk_estim_h_nfssf1_tm purge;
drop table ${iml_schema}.pty_finc_cust_risk_estim_h_nfssf1_op purge;
drop table ${iml_schema}.pty_finc_cust_risk_estim_h_nfssf1_cl purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.pty_finc_cust_risk_estim_h_nfssf1_tm nologging
compress ${option_switch} for query high
as select
    party_id -- 当事人编号
    ,lp_id -- 法人编号
...
from ${iml_schema}.pty_finc_cust_risk_estim_h partition for ('nfssf1')
where 0=1
;

create table ${iml_schema}.pty_finc_cust_risk_estim_h_nfssf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.pty_finc_cust_risk_estim_h partition for ('nfssf1') where 0=1;

create table ${iml_schema}.pty_finc_cust_risk_estim_h_nfssf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.pty_finc_cust_risk_estim_h partition for ('nfssf1') where 0=1;

-- 3.1 get new data into table
-- nfss_tbclientriskinfo-
insert into ${iml_schema}.pty_finc_cust_risk_estim_h_nfssf1_tm(
    party_id -- 当事人编号
    ,lp_id -- 法人编号
    ,sorc_sys_cd -- 源系统代码
    ,seq_num -- 序号
...
select
    ...
    ,'-' -- 非柜面渠道购买高风险产品标志
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,'nfss_tbclientriskinfo' -- 源表名称
    ,'nfssf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.nfss_tbclientriskinfo p1
    inner join ${iol_schema}.nfss_tbclientseller p2 on p1.IN_CLIENT_NO=p2.IN_CLIENT_NO 
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.pty_finc_cust_risk_estim_h_nfssf1_tm 
  	                                group by 
  	                                        party_id
  	                                        ,lp_id
  	                                        ,sorc_sys_cd
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.pty_finc_cust_risk_estim_h_nfssf1_cl(
            party_id -- 当事人编号
    ,lp_id -- 法人编号
    ,sorc_sys_cd -- 源系统代码
    ,seq_num -- 序号
    ...
    ,non_cnter_chn_buy_high_risk_prod_flg -- 非柜面渠道购买高风险产品标志
        ...
        into ${iml_schema}.pty_finc_cust_risk_estim_h_nfssf1_op(
            party_id -- 当事人编号
    ,lp_id -- 法人编号
    ,sorc_sys_cd -- 源系统代码
    ,seq_num -- 序号
...
from ${iml_schema}.pty_finc_cust_risk_estim_h_nfssf1_tm n
    full join (select * from ${iml_schema}.pty_finc_cust_risk_estim_h_nfssf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
...
from ${iml_schema}.pty_finc_cust_risk_estim_h_nfssf1_bk o
    left join ${iml_schema}.pty_finc_cust_risk_estim_h_nfssf1_op n
        on
            o.party_id = n.party_id
            and o.lp_id = n.lp_id
            and o.sorc_sys_cd = n.sorc_sys_cd
    ...
    left join ${iml_schema}.pty_finc_cust_risk_estim_h_nfssf1_cl d
        on
...
--alter table ${iml_schema}.pty_finc_cust_risk_estim_h truncate partition for ('nfssf1') reuse storage;
...
alter table ${iml_schema}.pty_finc_cust_risk_estim_h modify partition p_nfssf1 
add subpartition p_nfssf1_${batch_date} values(to_date('${batch_date}','YYYYMMDD')) ;

-- 4.3 exchange partition
alter table ${iml_schema}.pty_finc_cust_risk_estim_h exchange subpartition p_nfssf1_${batch_date} with table ${iml_schema}.pty_finc_cust_risk_estim_h_nfssf1_cl;
alter table ${iml_schema}.pty_finc_cust_risk_estim_h exchange subpartition p_nfssf1_20991231 with table ${iml_schema}.pty_finc_cust_risk_estim_h_nfssf1_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.pty_finc_cust_risk_estim_h_nfssf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'pty_finc_cust_risk_estim_h', partname => 'p_nfssf1_20991231',ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
