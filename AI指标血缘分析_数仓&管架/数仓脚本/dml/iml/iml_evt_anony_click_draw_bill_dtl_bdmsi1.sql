...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_evt_anony_click_draw_bill_dtl_bdmsi1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.evt_anony_click_draw_bill_dtl add partition p_bdmsi1 values ('bdmsi1')(
...
create table ${iml_schema}.evt_anony_click_draw_bill_dtl_bdmsi1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.evt_anony_click_draw_bill_dtl partition for ('bdmsi1') 
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
drop table ${iml_schema}.evt_anony_click_draw_bill_dtl_bdmsi1_tm purge;
drop table ${iml_schema}.evt_anony_click_draw_bill_dtl_bdmsi1_op purge;
drop table ${iml_schema}.evt_anony_click_draw_bill_dtl_bdmsi1_cl purge;

-- 2.3 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.evt_anony_click_draw_bill_dtl_bdmsi1_tm nologging
compress ${option_switch} for query high
as select
    evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,match_dtl_ser_num -- 匹配明细序列号
...
from ${iml_schema}.evt_anony_click_draw_bill_dtl partition for ('bdmsi1')
where 0=1
;

create table ${iml_schema}.evt_anony_click_draw_bill_dtl_bdmsi1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.evt_anony_click_draw_bill_dtl partition for ('bdmsi1') where 0=1;

create table ${iml_schema}.evt_anony_click_draw_bill_dtl_bdmsi1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.evt_anony_click_draw_bill_dtl partition for ('bdmsi1') where 0=1;

-- 3.1 get new data into table
-- bdms_cpes_anoclick_details
insert into ${iml_schema}.evt_anony_click_draw_bill_dtl_bdmsi1_tm(
    evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,match_dtl_ser_num -- 匹配明细序列号
    ,match_batch_ser_num -- 匹配批次序列号
...
select
    ...
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,'bdms_cpes_anoclick_details' -- 源表名称
    ,'bdmsi1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.bdms_cpes_anoclick_details p1
    ...
    left join ${iml_schema}.ref_pub_cd_map r1 on trim(P1.ACCOUNT_STATUS) = R1.SRC_CODE_VAL
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.evt_anony_click_draw_bill_dtl_bdmsi1_tm 
  	                                group by 
  	                                        evt_id
  	                                        ,lp_id
  	                               having count(1) > 1);
...
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${iml_schema}.evt_anony_click_draw_bill_dtl_bdmsi1_op(
        evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,match_dtl_ser_num -- 匹配明细序列号
    ,match_batch_ser_num -- 匹配批次序列号
...
from ${iml_schema}.evt_anony_click_draw_bill_dtl_bdmsi1_tm n
    left join ${iml_schema}.evt_anony_click_draw_bill_dtl_bdmsi1_bk o
        on
            o.evt_id = n.evt_id
            and o.lp_id = n.lp_id
where (
    ...
    or (
        o.match_dtl_ser_num <> n.match_dtl_ser_num
        or o.match_batch_ser_num <> n.match_batch_ser_num
...
insert /*+ append */ all
    when end_dt <= to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.evt_anony_click_draw_bill_dtl_bdmsi1_cl(
            evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,match_dtl_ser_num -- 匹配明细序列号
    ,match_batch_ser_num -- 匹配批次序列号
    ...
    ,fir_cntpty_ibank_no -- 首次交易对手联行号
        ...
        into ${iml_schema}.evt_anony_click_draw_bill_dtl_bdmsi1_op(
            evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,match_dtl_ser_num -- 匹配明细序列号
    ,match_batch_ser_num -- 匹配批次序列号
...
from ${iml_schema}.evt_anony_click_draw_bill_dtl_bdmsi1_bk o
    left join ${iml_schema}.evt_anony_click_draw_bill_dtl_bdmsi1_op n
        on
            o.evt_id = n.evt_id
            and o.lp_id = n.lp_id
...
for cur in (select subpartition_name 
             ...
             where table_owner=upper('${iml_schema}') 
               and table_name=upper('evt_anony_click_draw_bill_dtl') 
               and substr(subpartition_name,1,8)=upper('p_bdmsi1')
               and substr(subpartition_name,10) >= '${batch_date}' 
               and substr(subpartition_name,10) < '20991231') loop
    v_sql := 'alter table ${iml_schema}.evt_anony_click_draw_bill_dtl drop subpartition ' || cur.subpartition_name;
...
alter table ${iml_schema}.evt_anony_click_draw_bill_dtl modify partition p_bdmsi1 
add subpartition p_bdmsi1_${batch_date} values (to_date('${batch_date}','YYYYMMDD'));
  
-- 4.2 exchange partition
alter table ${iml_schema}.evt_anony_click_draw_bill_dtl exchange subpartition p_bdmsi1_${batch_date} with table ${iml_schema}.evt_anony_click_draw_bill_dtl_bdmsi1_cl;
alter table ${iml_schema}.evt_anony_click_draw_bill_dtl exchange subpartition p_bdmsi1_20991231 with table ${iml_schema}.evt_anony_click_draw_bill_dtl_bdmsi1_op;

-- 5.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.evt_anony_click_draw_bill_dtl to ${iml_schema};
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.evt_anony_click_draw_bill_dtl_bdmsi1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'evt_anony_click_draw_bill_dtl', partname => 'p_bdmsi1_20991231', ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1', no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
