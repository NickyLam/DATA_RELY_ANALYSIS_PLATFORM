...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_agt_consmt_fund_lot_h_nfssf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.agt_consmt_fund_lot_h add partition p_nfssf1 values ('nfssf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.agt_consmt_fund_lot_h_nfssf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_consmt_fund_lot_h partition for ('nfssf1')
...
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.agt_consmt_fund_lot_h_nfssf1_tm purge;
drop table ${iml_schema}.agt_consmt_fund_lot_h_nfssf1_op purge;
drop table ${iml_schema}.agt_consmt_fund_lot_h_nfssf1_cl purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.agt_consmt_fund_lot_h_nfssf1_tm nologging
compress ${option_switch} for query high
as select
    agt_id -- 协议编号
    ,lp_id -- 法人编号
...
from ${iml_schema}.agt_consmt_fund_lot_h partition for ('nfssf1')
where 0=1
;

create table ${iml_schema}.agt_consmt_fund_lot_h_nfssf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_consmt_fund_lot_h partition for ('nfssf1') where 0=1;

create table ${iml_schema}.agt_consmt_fund_lot_h_nfssf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_consmt_fund_lot_h partition for ('nfssf1') where 0=1;
...
-- nfss_tbshare0-
insert into ${iml_schema}.agt_consmt_fund_lot_h_nfssf1_tm(
    agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,std_prod_id -- 标准产品编号
    ,intnal_cust_id -- 内部客户编号
...
select
  ...
  from ${iol_schema}.nfss_tbshare0 p1
  left join ${iol_schema}.nfss_tbproduct p2
  ...
  left join ${iol_schema}.ifms_tbdict p3
    on p2.prd_attr = p3.val
   and p3.hs_key = 'K_HXYHZSJDXFUND'
  ...
  left join ${iml_schema}.ref_pub_cd_map r1
   ...
   and r1.src_tab_en_name = 'NFSS_TBSHARE0'
   and r1.src_field_en_name = 'CLIENT_TYPE'
   and r1.target_tab_en_name = 'AGT_CONSMT_FUND_LOT_H'
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.agt_consmt_fund_lot_h_nfssf1_tm 
  	                                group by 
  	                                        agt_id
  	                                        ,lp_id
  	                                        ,seller_id
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.agt_consmt_fund_lot_h_nfssf1_cl(
            agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,std_prod_id -- 标准产品编号
    ,intnal_cust_id -- 内部客户编号
    ...
    ,new_assign_prft -- 新分配收益
        ...
        into ${iml_schema}.agt_consmt_fund_lot_h_nfssf1_op(
            agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,std_prod_id -- 标准产品编号
    ,intnal_cust_id -- 内部客户编号
...
from ${iml_schema}.agt_consmt_fund_lot_h_nfssf1_tm n
    full join (select * from ${iml_schema}.agt_consmt_fund_lot_h_nfssf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
            o.agt_id = n.agt_id
            and o.lp_id = n.lp_id
...
from ${iml_schema}.agt_consmt_fund_lot_h_nfssf1_bk o
    left join ${iml_schema}.agt_consmt_fund_lot_h_nfssf1_op n
        on
            o.agt_id = n.agt_id
            and o.lp_id = n.lp_id
            and o.seller_id = n.seller_id
    ...
    left join ${iml_schema}.agt_consmt_fund_lot_h_nfssf1_cl d
        on
            o.agt_id = d.agt_id
            and o.lp_id = d.lp_id
...
--truncate table ${iml_schema}.agt_consmt_fund_lot_h;
--alter table ${iml_schema}.agt_consmt_fund_lot_h truncate partition for ('nfssf1') reuse storage;
...
for cur in (select subpartition_name 
             ...
             where table_owner=upper('${iml_schema}') 
               and table_name=upper('agt_consmt_fund_lot_h') 
               and substr(subpartition_name,1,8)=upper('p_nfssf1')
               and substr(subpartition_name,10) >= '${batch_date}' 
               and substr(subpartition_name,10) < '20991231') loop
    v_sql := 'alter table ${iml_schema}.agt_consmt_fund_lot_h drop subpartition ' || cur.subpartition_name;
...
alter table ${iml_schema}.agt_consmt_fund_lot_h modify partition p_nfssf1 
add subpartition p_nfssf1_${batch_date} values(to_date('${batch_date}','YYYYMMDD')) ;

-- 4.3 exchange partition
alter table ${iml_schema}.agt_consmt_fund_lot_h exchange subpartition p_nfssf1_${batch_date} with table ${iml_schema}.agt_consmt_fund_lot_h_nfssf1_cl;
alter table ${iml_schema}.agt_consmt_fund_lot_h exchange subpartition p_nfssf1_20991231 with table ${iml_schema}.agt_consmt_fund_lot_h_nfssf1_op;

-- 5.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.agt_consmt_fund_lot_h to ${iml_schema};
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.agt_consmt_fund_lot_h_nfssf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'agt_consmt_fund_lot_h', partname => 'p_nfssf1_20991231',ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
