...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_evt_event_bdmsi1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
drop table ${iml_schema}.evt_event_bdmsi1_tm purge;
alter table ${iml_schema}.evt_event add partition p_bdmsi1 values ('bdmsi1')(
        subpartition p_bdmsi1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
    )
;
alter table ${iml_schema}.evt_event modify partition p_bdmsi1
    add subpartition p_bdmsi1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
;

whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.evt_event_bdmsi1_tm
...
-- bdms_account_log-
insert into ${iml_schema}.evt_event_bdmsi1_tm(
    evt_id -- 事件编号
    ,lp_id -- 法人编号
    ,src_event_id -- 源事件编号
    ,event_type_cd -- 事件类型代码
...
select
    ...
    ,TO_CHAR(P1.ID) -- 源事件编号
    ,'101003' -- 事件类型代码
    ,${iml_schema}.DATEFORMAT_MAX(P1.TXDATE) -- 事件日期
    ,' ' -- 事件时间
    ,' ' -- 事件交易码
    ,' ' -- 事件渠道编号
    ,NVL(TO_CHAR(P2.OPR_NO),' ') -- 事件柜员编号
...
select
    ...
    ,P1.PROV_DE_ID -- 源事件编号
    ,'104008' -- 事件类型代码
    ,${iml_schema}.DATEFORMAT_MIN(P1.ACCT_DT) -- 事件日期
    ,' ' -- 事件时间
    ,' ' -- 事件交易码
    ,' ' -- 事件渠道编号
    ,' ' -- 事件柜员编号
...
select
    '101007'||TO_CHAR(CREATE_TIME,'YYYYMMDD')||P1.TRADE_DETAIL_ID -- 事件编号
    ,'9999' -- 法人编号
    ,P1.TRADE_DETAIL_ID -- 源事件编号
    ,'101007' -- 事件类型代码
    ,TO_DATE(TO_CHAR(P1.CREATE_TIME,'YYYY/MM/DD'),'YYYY/MM/DD') -- 事件日期
    ,TO_CHAR(P1.CREATE_TIME,'HH24:MI:SS') -- 事件时间
    ,' ' -- 事件交易码
    ,' ' -- 事件渠道编号
    ,' ' -- 事件柜员编号
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
...
select
    ...
    ,'9999' -- 法人编号
    ,P1.TRADE_DRAFT_ID -- 源事件编号
    ,'101006' -- 事件类型代码
    ,TO_DATE(TO_CHAR(CREATE_TIME,'YYYY/MM/DD'),'YYYY/MM/DD') -- 事件日期
    ,TO_CHAR(CREATE_TIME,'hh24:mi:ss') -- 事件时间
    ,P1.RECODE -- 事件交易码
    ,' ' -- 事件渠道编号
    ,P1.LAST_UPD_OPER_NO -- 事件柜员编号
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
...
select
    ...
    ,P1.ID -- 源事件编号
    ,'104011' -- 事件类型代码
    ,${iml_schema}.DATEFORMAT_MIN(P1.TXN_DATE) -- 事件日期
    ,' ' -- 事件时间
    ,' ' -- 事件交易码
    ,' ' -- 事件渠道编号
    ,' ' -- 事件柜员编号
...
select
    ...
    ,P1.ID -- 源事件编号
    ,'201001' -- 事件类型代码
    ,${iml_schema}.DATEFORMAT_MIN(P1.MSG_DT) -- 事件日期
    ,TO_CHAR(TO_DATE(P1.MSG_TM,'YYYY/MM/DD HH24:MI:SS'),'HH24:MI:SS') -- 事件时间
    ,' ' -- 事件交易码
    ,' ' -- 事件渠道编号
    ,' ' -- 事件柜员编号
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
...
alter table ${iml_schema}.evt_event truncate subpartition p_bdmsi1_${batch_date} reuse storage;


-- 3.3 exchage tm table and target table
alter table ${iml_schema}.evt_event exchange subpartition p_bdmsi1_${batch_date} with table ${iml_schema}.evt_event_bdmsi1_tm;
...
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'evt_event', partname => 'p_bdmsi1_${batch_date}', granularity => 'SUBPARTITION', degree => 8, cascade => true);