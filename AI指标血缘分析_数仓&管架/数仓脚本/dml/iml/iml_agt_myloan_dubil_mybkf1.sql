...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_agt_myloan_dubil_mybkf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
drop table ${iml_schema}.agt_myloan_dubil_mybkf1_tm purge;
drop table ${iml_schema}.agt_myloan_dubil_mybkf1_ex purge;

-- 2.2 add partition for target table
alter table ${iml_schema}.agt_myloan_dubil add partition p_mybkf1 values ('mybkf1')(
        subpartition p_mybkf1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
    )
;
alter table ${iml_schema}.agt_myloan_dubil modify partition p_mybkf1
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.agt_myloan_dubil_mybkf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_myloan_dubil partition for ('mybkf1')
where create_dt < to_date('${batch_date}','yyyymmdd')
;

-- 2.4 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.agt_myloan_dubil_mybkf1_tm
compress ${option_switch} for query high
as
select
    agt_id -- 协议编号
    ...
    ,myloan_dubil_type_cd -- 借据类型代码
    ,hxb_loan_amt -- 我行贷款金额
    ,hxb_loan_tot_perds -- 我行贷款总期数
...
from ${iml_schema}.agt_myloan_dubil
where 0=1;

-- 2.5 create temp table
create table ${iml_schema}.agt_myloan_dubil_mybkf1_ex nologging
compress ${option_switch} for query high
as select * from ${iml_schema}.agt_myloan_dubil partition for ('mybkf1') where 0=1;

-- 2.1 insert data to tm table
-- icms_mybk_acc_loan-
insert into ${iml_schema}.agt_myloan_dubil_mybkf1_tm(
    agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,dubil_id -- 借据编号
    ,ext_prod_id -- 外部产品编号
...
select
    ...
    ,CASE WHEN R9.TARGET_CD_VAL IS NOT NULL THEN R9.TARGET_CD_VAL ELSE '@'||P1.LOANUSE END -- 贷款用途代码
    ,CASE WHEN R1.TARGET_CD_VAL IS NOT NULL THEN R1.TARGET_CD_VAL ELSE '@'||P1.USEAREA END -- 贷款资金使用位置代码
    ,${iml_schema}.DATEFORMAT_MIN(P1.ENCASHDATE) -- 放款日期
    ,NVL(TRIM(P1.CURRENCY),'CNY') -- 币种代码
    ,P1.ENCASHAMT -- 放款金额
    ,${iml_schema}.DATEFORMAT_MIN(P1.STARTDATE) -- 贷款起息日期
    ,${iml_schema}.dateformat_max(P1.ENDDATE) -- 贷款到期日期
    ,P1.TOTALTERMS -- 贷款合同期限
    ,CASE WHEN R3.TARGET_CD_VAL IS NOT NULL THEN R3.TARGET_CD_VAL ELSE '@'||P1.REPAYMODE END -- 还款方式代码
    ,P1.GRACEDAY -- 宽限期天数
    ,nvl(trim(P1.RATETYPE),'-') -- 源利率类型代码
    ,CASE WHEN R4.TARGET_CD_VAL IS NOT NULL THEN R4.TARGET_CD_VAL ELSE '@'||SUBSTR(P1.RATETYPE,1,1) END -- 利率调整方式代码
    ,CASE WHEN R5.TARGET_CD_VAL IS NOT NULL THEN R5.TARGET_CD_VAL ELSE '@'||NVL(SUBSTR(P1.RATETYPE,2),' ') END -- 利率调整周期单位代码
    ,CASE WHEN P1.RATETYPE='F' THEN '0'
     WHEN P1.RATETYPE ='L4' THEN '4' 
     ELSE '1' END -- 利率调整周期频率
    ,P1.DAYRATE*100 -- 贷款实际日利率
    ,DECODE(TRIM(P1.PRINREPAYFREQUENCY),'99','00',NVL(TRIM(P1.PRINREPAYFREQUENCY),'00')) -- 本金还款频率代码
    ,NVL(TRIM(P1.INTREPAYFREQUENCY),'00') -- 利息还款频率代码
    ,nvl(trim(P1.GUARANTEETYPE),'-') -- 担保类型代码
    ,P1.CREDITNO -- 授信申请编号
    ,nvl(trim(P1.ENCASHACCTTYPE),'-') -- 收款账号类型代码
    ,P1.ENCASHACCTNAME -- 收款账户名称
    ,P1.ENCASHACCTNO -- 收款账户编号
    ,P1.ENCASHBANKNAME -- 收款银行名称
    ,nvl(trim(P1.REPAYACCTTYPE),'-') -- 还款账号类型代码
    ,P1.REPAYACCTNAME -- 还款账户名称
    ,P1.REPAYACCTNO -- 还款账户编号
    ,P1.REPAYBANKNAME -- 还款银行名称
    ,nvl(trim(P1.BSNTYPE),'-') -- 产品类型代码
    ,${iml_schema}.dateformat_max2(P1.SETTLEDATE) -- 会计日期
    ,NVL(TRIM(P1.STATUS),'-') -- 合约状态代码
    ,${iml_schema}.dateformat_max2(P1.CLEARDATE) -- 结清日期
    ,nvl(P1.CLASSIFYRESULT,'99') -- 贷款五级分类代码
    ...
    ,NVL(TRIM(P1.INDUSTRYTYPE),'-') -- 贷款投向行业代码
    ,P1.CUSID -- 客户编号
    ,P1.CUSMGRID -- 客户经理编号
    ,P1.INTBAL -- 正常利息余额
    ,P1.OVDINTBAL -- 逾期利息余额
    ...
    ,P1.BIZTYPE -- 产品编号
    ,CASE WHEN TRIM(P1.TYPECONTRIBUTIONRATION) IS NULL THEN '-'
     WHEN R7.TARGET_CD_VAL IS NOT NULL THEN R7.TARGET_CD_VAL 
     ELSE '@'||P1.TYPECONTRIBUTIONRATION
END -- 出资类型代码
    ,nvl(trim(p1.CONTRIBUTIONRATION),0) -- 出资比例
    ,NVL(TRIM(P1.ISWHITE),'-') -- 白户标志
    ,nvl(trim(p1.BUSINESSESFLAG),'0') -- 客户性质代码
    ,case when trim(P1.AGRIFLG)='2' then '0' else nvl(trim(P1.AGRIFLG),'-') end -- 农户标志
    ,nvl(trim(P1.ISDEBTTRANSFER),'-') -- 债权直转标志
    ,nvl(trim(P1.CONTRACTTYPE),'-') -- 借据类型代码
    ,P1.SELFENCASHAMT -- 我行贷款金额
    ,P1.SELFTERMS -- 我行贷款总期数
    ,${iml_schema}.DATEFORMAT_MIN(P1.SELFSTARTDATE) -- 我行贷款起始日期
    ,nvl(trim(P1.OCCURTYPE),'0100') -- 发生方式代码
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,'icms_mybk_acc_loan' -- 源表名称
    ,'mybkf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.icms_mybk_acc_loan p1
    left join ${iol_schema}.rcrs_mybk_acc_loan_clear p2 on p2.CONTRACT_NO=p1.CONTRACTNO and p2.start_dt <= to_date('${batch_date}','yyyymmdd') and p2.end_dt > to_date('${batch_date}','yyyymmdd')
    left join ${iml_schema}.ref_pub_cd_map r9 on P1.LOANUSE = R9.SRC_CODE_VAL
        AND R9.SORC_SYS_CD= 'ICMS'
        AND R9.SRC_TAB_EN_NAME= 'ICMS_MYBK_ACC_LOAN'
        AND R9.SRC_FIELD_EN_NAME= 'LOANUSE'
        AND R9.TARGET_TAB_EN_NAME= 'AGT_MYLOAN_DUBIL'
        AND R9.TARGET_TAB_FIELD_EN_NAME= 'LOAN_USAGE_CD'
    left join ${iml_schema}.ref_pub_cd_map r1 on P1.USEAREA = R1.SRC_CODE_VAL
        AND R1.SORC_SYS_CD= 'ICMS'
        AND R1.SRC_TAB_EN_NAME= 'ICMS_MYBK_ACC_LOAN'
        AND R1.SRC_FIELD_EN_NAME= 'USEAREA'
        AND R1.TARGET_TAB_EN_NAME= 'AGT_MYLOAN_DUBIL'
        AND R1.TARGET_TAB_FIELD_EN_NAME= 'LOAN_CAP_USE_POSITION_CD'
    left join ${iml_schema}.ref_pub_cd_map r3 on   P1.REPAYMODE = R3.SRC_CODE_VAL
        AND R3.SORC_SYS_CD= 'ICMS'
        AND R3.SRC_TAB_EN_NAME= 'ICMS_MYBK_ACC_LOAN'
        AND R3.SRC_FIELD_EN_NAME= 'REPAYMODE'
        AND R3.TARGET_TAB_EN_NAME= 'AGT_MYLOAN_DUBIL'
        AND R3.TARGET_TAB_FIELD_EN_NAME= 'REPAY_WAY_CD'
    left join ${iml_schema}.ref_pub_cd_map r4 on SUBSTR(P1.RATETYPE,1,1) = R4.SRC_CODE_VAL
        AND R4.SORC_SYS_CD= 'ICMS'
        AND R4.SRC_TAB_EN_NAME= 'ICMS_MYBK_ACC_LOAN'
        AND R4.SRC_FIELD_EN_NAME= 'RATETYPE'
        AND R4.TARGET_TAB_EN_NAME= 'AGT_MYLOAN_DUBIL'
        AND R4.TARGET_TAB_FIELD_EN_NAME= 'INT_RAT_ADJ_WAY_CD'
    left join ${iml_schema}.ref_pub_cd_map r5 on NVL(SUBSTR(P1.RATETYPE,2),' ') = R5.SRC_CODE_VAL
        AND R5.SORC_SYS_CD= 'ICMS'
        AND R5.SRC_TAB_EN_NAME= 'ICMS_MYBK_ACC_LOAN'
        AND R5.SRC_FIELD_EN_NAME= 'RATETYPE'
        AND R5.TARGET_TAB_EN_NAME= 'AGT_MYLOAN_DUBIL'
    ...
    left join ${iml_schema}.ref_pub_cd_map r6 on P1.RATEFLOATMODE = R6.SRC_CODE_VAL
        ...
        AND R6.TARGET_TAB_EN_NAME= 'AGT_MYLOAN_DUBIL'
        AND R6.TARGET_TAB_FIELD_EN_NAME= 'INT_RAT_FLOAT_DIR_CD'
    left join ${iml_schema}.ref_pub_cd_map r7 on P1.TYPECONTRIBUTIONRATION = R7.SRC_CODE_VAL
        AND R7.SORC_SYS_CD= 'ICMS'
        AND R7.SRC_TAB_EN_NAME= 'ICMS_MYBK_ACC_LOAN'
        AND R7.SRC_FIELD_EN_NAME= 'TYPECONTRIBUTIONRATION'
        AND R7.TARGET_TAB_EN_NAME= 'AGT_MYLOAN_DUBIL'
        AND R7.TARGET_TAB_FIELD_EN_NAME= 'CONTRI_TYPE_CD'
...
select
    ...
    ,NVL(TRIM(P1.INDUSTRY_TYPE),'-') -- 贷款投向行业代码
    ,P1.CUS_ID -- 客户编号
    ,P1.CUS_MGR_ID -- 客户经理编号
    ,P1.INT_BAL -- 正常利息余额
...
from ${iol_schema}.rcrs_mybk_acc_loan_clear p1
    left join ${iml_schema}.ref_pub_cd_map r9 on P1.LOAN_USE = R9.SRC_CODE_VAL
        AND R9.SORC_SYS_CD= 'ICMS'
        AND R9.SRC_TAB_EN_NAME= 'RCRS_MYBK_ACC_LOAN_CLEAR'
    ...
    left join ${iml_schema}.ref_pub_cd_map r8 on P1.ASSET_CLASS = R8.SRC_CODE_VAL
        ...
        AND R8.SRC_TAB_EN_NAME= 'RCRS_MYBK_ACC_LOAN_CLEAR'
        AND R8.SRC_FIELD_EN_NAME= 'ASSET_CLASS'
        AND R8.TARGET_TAB_EN_NAME= 'AGT_MYLOAN_DUBIL'
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.agt_myloan_dubil_mybkf1_tm 
  	                                group by 
  	                                        agt_id
  	                                        ,lp_id
  	                               having count(1) > 1);
...
/
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${iml_schema}.agt_myloan_dubil_mybkf1_ex(
    agt_id -- 协议编号
    ,lp_id -- 法人编号
    ,dubil_id -- 借据编号
    ,ext_prod_id -- 外部产品编号
...
select
    ...
    ,nvl(n.contri_type_cd, o.contri_type_cd) as contri_type_cd -- 出资类型代码
    ...
    ,nvl(n.cred_rht_turn_flg, o.cred_rht_turn_flg) as cred_rht_turn_flg -- 债权直转标志
    ,nvl(n.myloan_dubil_type_cd, o.myloan_dubil_type_cd) as myloan_dubil_type_cd -- 借据类型代码
    ,nvl(n.hxb_loan_amt, o.hxb_loan_amt) as hxb_loan_amt -- 我行贷款金额
    ,nvl(n.hxb_loan_tot_perds, o.hxb_loan_tot_perds) as hxb_loan_tot_perds -- 我行贷款总期数
    ...
    ,case when (
                ...
                or o.contri_type_cd <> n.contri_type_cd
                ...
                or o.cred_rht_turn_flg <> n.cred_rht_turn_flg
                or o.myloan_dubil_type_cd <> n.myloan_dubil_type_cd
                or o.hxb_loan_amt <> n.hxb_loan_amt
                or o.hxb_loan_tot_perds <> n.hxb_loan_tot_perds
    ...
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iml_schema}.agt_myloan_dubil_mybkf1_tm n
    full join ${iml_schema}.agt_myloan_dubil_mybkf1_bk o
        on
            o.agt_id = n.agt_id
            and o.lp_id = n.lp_id
;
...
whenever sqlerror exit sql.sqlcode;
alter table ${iml_schema}.agt_myloan_dubil truncate partition for ('mybkf1') reuse storage;

-- 3.2 exchage tm table and target table
alter table ${iml_schema}.agt_myloan_dubil exchange subpartition p_mybkf1_${batch_date} with table ${iml_schema}.agt_myloan_dubil_mybkf1_ex;

-- 3.3 drop subpartition last_date partition
whenever sqlerror continue none ;
alter table ${iml_schema}.agt_myloan_dubil drop subpartition p_mybkf1_${last_date} update global indexes;

-- 4.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.agt_myloan_dubil to ${iml_schema};

-- 4.2 drop tm table
drop table ${iml_schema}.agt_myloan_dubil_mybkf1_tm purge;
drop table ${iml_schema}.agt_myloan_dubil_mybkf1_ex purge;
drop table ${iml_schema}.agt_myloan_dubil_mybkf1_bk purge;

-- 5 gater table status
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'agt_myloan_dubil', partname => 'p_mybkf1_${batch_date}', ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);