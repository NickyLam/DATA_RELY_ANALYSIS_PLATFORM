...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_pty_party_rating_h_mpcsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.pty_party_rating_h add partition p_mpcsf1 values ('mpcsf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.pty_party_rating_h_mpcsf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.pty_party_rating_h partition for ('mpcsf1')
...
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.pty_party_rating_h_mpcsf1_tm purge;
drop table ${iml_schema}.pty_party_rating_h_mpcsf1_op purge;
drop table ${iml_schema}.pty_party_rating_h_mpcsf1_cl purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.pty_party_rating_h_mpcsf1_tm nologging
compress ${option_switch} for query high
as select
    party_id -- 当事人编号
    ,lp_id -- 法人编号
...
from ${iml_schema}.pty_party_rating_h partition for ('mpcsf1')
where 0=1
;

create table ${iml_schema}.pty_party_rating_h_mpcsf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.pty_party_rating_h partition for ('mpcsf1') where 0=1;

create table ${iml_schema}.pty_party_rating_h_mpcsf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.pty_party_rating_h partition for ('mpcsf1') where 0=1;
...
-- mpcs_a92accinfo-
insert into ${iml_schema}.pty_party_rating_h_mpcsf1_tm(
    party_id -- 当事人编号
    ,lp_id -- 法人编号
    ,sorc_sys_cd -- 源系统代码
    ,party_rating_type_cd -- 当事人评级类型代码
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.pty_party_rating_h_mpcsf1_tm 
  	                                group by 
  	                                        party_id
  	                                        ,lp_id
  	                                        ,sorc_sys_cd
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.pty_party_rating_h_mpcsf1_cl(
            party_id -- 当事人编号
    ,lp_id -- 法人编号
    ,sorc_sys_cd -- 源系统代码
    ,party_rating_type_cd -- 当事人评级类型代码
    ...
    ,lmt -- 限额
        ...
        into ${iml_schema}.pty_party_rating_h_mpcsf1_op(
            party_id -- 当事人编号
    ,lp_id -- 法人编号
    ,sorc_sys_cd -- 源系统代码
    ,party_rating_type_cd -- 当事人评级类型代码
...
from ${iml_schema}.pty_party_rating_h_mpcsf1_tm n
    full join (select * from ${iml_schema}.pty_party_rating_h_mpcsf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
...
from ${iml_schema}.pty_party_rating_h_mpcsf1_bk o
    left join ${iml_schema}.pty_party_rating_h_mpcsf1_op n
        on
            o.party_id = n.party_id
            and o.lp_id = n.lp_id
            and o.sorc_sys_cd = n.sorc_sys_cd
    ...
    left join ${iml_schema}.pty_party_rating_h_mpcsf1_cl d
        on
...
--alter table ${iml_schema}.pty_party_rating_h truncate partition for ('mpcsf1') reuse storage;
...
alter table ${iml_schema}.pty_party_rating_h modify partition p_mpcsf1 
add subpartition p_mpcsf1_${batch_date} values(to_date('${batch_date}','YYYYMMDD')) ;

-- 4.3 exchange partition
alter table ${iml_schema}.pty_party_rating_h exchange subpartition p_mpcsf1_${batch_date} with table ${iml_schema}.pty_party_rating_h_mpcsf1_cl;
alter table ${iml_schema}.pty_party_rating_h exchange subpartition p_mpcsf1_20991231 with table ${iml_schema}.pty_party_rating_h_mpcsf1_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.pty_party_rating_h_mpcsf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'pty_party_rating_h', partname => 'p_mpcsf1_20991231',ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
