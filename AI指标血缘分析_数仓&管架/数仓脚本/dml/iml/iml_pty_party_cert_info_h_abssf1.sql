...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_pty_party_cert_info_h_abssf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.pty_party_cert_info_h add partition p_abssf1 values ('abssf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.pty_party_cert_info_h_abssf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.pty_party_cert_info_h partition for ('abssf1')
...
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.pty_party_cert_info_h_abssf1_tm purge;
drop table ${iml_schema}.pty_party_cert_info_h_abssf1_op purge;
drop table ${iml_schema}.pty_party_cert_info_h_abssf1_cl purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.pty_party_cert_info_h_abssf1_tm nologging
compress ${option_switch} for query high
as select
    party_id -- 当事人编号
    ,lp_id -- 法人编号
...
from ${iml_schema}.pty_party_cert_info_h partition for ('abssf1')
where 0=1
;

create table ${iml_schema}.pty_party_cert_info_h_abssf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.pty_party_cert_info_h partition for ('abssf1') where 0=1;

create table ${iml_schema}.pty_party_cert_info_h_abssf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.pty_party_cert_info_h partition for ('abssf1') where 0=1;
...
insert into ${iml_schema}.pty_party_cert_info_h_abssf1_tm(
    party_id -- 当事人编号
    ,lp_id -- 法人编号
    ,sorc_sys_cd -- 源系统代码
    ,cert_type_cd -- 证件类型代码
...
select
    ...
    ,' ' -- 证件地址
    ,' ' -- 发证机关
    ,NVL(TRIM(P1.ISSUECOUNTRY),'XXX') -- 发证机关国家代码
    ,to_date('00010101','yyyymmdd') -- 证件生效日期
    ,to_date('20991231','yyyymmdd') -- 证件失效日期
    ,'000000' -- 发证机关地区代码
    ,' ' -- 信用代码证编号
    ,' ' -- 证件有效标志
    ,' ' -- 证件状态代码
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.pty_party_cert_info_h_abssf1_tm 
  	                                group by 
  	                                        party_id
  	                                        ,lp_id
  	                                        ,sorc_sys_cd
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.pty_party_cert_info_h_abssf1_cl(
            party_id -- 当事人编号
    ,lp_id -- 法人编号
    ,sorc_sys_cd -- 源系统代码
    ,cert_type_cd -- 证件类型代码
    ...
    ,netw_vrfction_rest_cd -- 联网核查结果代码
        ...
        into ${iml_schema}.pty_party_cert_info_h_abssf1_op(
            party_id -- 当事人编号
    ,lp_id -- 法人编号
    ,sorc_sys_cd -- 源系统代码
    ,cert_type_cd -- 证件类型代码
...
from ${iml_schema}.pty_party_cert_info_h_abssf1_tm n
    full join (select * from ${iml_schema}.pty_party_cert_info_h_abssf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
...
from ${iml_schema}.pty_party_cert_info_h_abssf1_bk o
    left join ${iml_schema}.pty_party_cert_info_h_abssf1_op n
        on
            o.party_id = n.party_id
            and o.lp_id = n.lp_id
            and o.sorc_sys_cd = n.sorc_sys_cd
    ...
    left join ${iml_schema}.pty_party_cert_info_h_abssf1_cl d
        on
...
--alter table ${iml_schema}.pty_party_cert_info_h truncate partition for ('abssf1') reuse storage;
...
alter table ${iml_schema}.pty_party_cert_info_h modify partition p_abssf1 
add subpartition p_abssf1_${batch_date} values(to_date('${batch_date}','YYYYMMDD')) ;

whenever sqlerror exit sql.sqlcode;
-- 4.3 exchange partition
alter table ${iml_schema}.pty_party_cert_info_h exchange subpartition p_abssf1_${batch_date} with table ${iml_schema}.pty_party_cert_info_h_abssf1_cl;
alter table ${iml_schema}.pty_party_cert_info_h exchange subpartition p_abssf1_20991231 with table ${iml_schema}.pty_party_cert_info_h_abssf1_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.pty_party_cert_info_h_abssf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'pty_party_cert_info_h', partname => 'p_abssf1_20991231',ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
