...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_agt_dep_acct_stl_info_h_ncbsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.agt_dep_acct_stl_info_h add partition p_ncbsf1 values ('ncbsf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.agt_dep_acct_stl_info_h_ncbsf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_dep_acct_stl_info_h partition for ('ncbsf1')
...
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.agt_dep_acct_stl_info_h_ncbsf1_tm purge;
drop table ${iml_schema}.agt_dep_acct_stl_info_h_ncbsf1_op purge;
drop table ${iml_schema}.agt_dep_acct_stl_info_h_ncbsf1_cl purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.agt_dep_acct_stl_info_h_ncbsf1_tm nologging
compress ${option_switch} for query high
as select
    agt_id -- 协议编号
    ,acct_id -- 账户编号
...
from ${iml_schema}.agt_dep_acct_stl_info_h partition for ('ncbsf1')
where 0=1
;

create table ${iml_schema}.agt_dep_acct_stl_info_h_ncbsf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_dep_acct_stl_info_h partition for ('ncbsf1') where 0=1;

create table ${iml_schema}.agt_dep_acct_stl_info_h_ncbsf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.agt_dep_acct_stl_info_h partition for ('ncbsf1') where 0=1;

-- 3.1 get new data into table
-- ncbs_rb_acct_settle-1
insert into ${iml_schema}.agt_dep_acct_stl_info_h_ncbsf1_tm(
    agt_id -- 协议编号
    ,acct_id -- 账户编号
    ,stl_id -- 结算编号
    ,lp_id -- 法人编号
...
select
    ...
    ,nvl(trim(P1.SETTLE_METHOD),'-') -- 结算方法代码
    ,P1.TRAN_TYPE -- 交易码
    ,CASE WHEN TRIM(R1.TARGET_CD_VAL) IS NOT NULL THEN R1.TARGET_CD_VAL ELSE '@'||P1.PAY_REC_IND END -- 收付标识代码
    ,nvl(trim(P1.AMT_TYPE),'-') -- 金额类型代码
    ,P1.SETTLE_CLIENT -- 结算客户编号
    ,decode(trim(p1.BANK_IN_OUT),'','-','I','1','O','0',p1.BANK_IN_OUT) -- 行外标志
    ,decode(trim(p1.SETTLE_BANK_FLAG),'','-','I','1','O','0',p1.SETTLE_BANK_FLAG) -- 我行结算标志
    ,P1.PAYEE_BANK_CODE -- 收款行行号
    ,P1.PAYEE_BANK_NAME -- 收款行名称
    ,P1.SETTLE_BANK_NAME -- 结算行行号
    ,P1.SETTLE_ACCT_INTERNAL_KEY -- 结算账户编号
    ,nvl(trim(p8.card_no),p1.SETTLE_BASE_ACCT_NO) -- 结算客户账号
    ...
    ,decode(trim(p1.AUTO_BLOCKING),'','-','Y','1','N','0',p1.AUTO_BLOCKING) -- 自动锁定标志
    ,P1.TRUSTED_PAY_NO -- 受托支付编号
    ,P1.SETTLE_WEIGHT -- 结算权重
    ,nvl(trim(P1.SETTLE_CCY),'-') -- 结算币种代码
    ,P1.SETTLE_AMT -- 结算金额
    ,P1.SETTLE_XRATE -- 结算汇率
    ,nvl(trim(P1.FREEZE_TYPE),'-') -- 受托人账户冻结方式代码
    ,decode(trim(p1.SELF_SUPPORT_FLAG),'','-','Y','1','N','0',p1.SELF_SUPPORT_FLAG) -- 自营标志
    ,P1.PRIORITY -- 优先等级
    ...
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,'ncbs_rb_acct_settle' -- 源表名称
    ,'ncbsf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.ncbs_rb_acct_settle p1
    left join ${iml_schema}.ref_pub_cd_map r1 on P1.PAY_REC_IND=R1.SRC_CODE_VAL
   AND R1.SORC_SYS_CD= 'NCBS'
   AND R1.SRC_TAB_EN_NAME ='NCBS_RB_ACCT_SETTLE'
   AND R1.SRC_FIELD_EN_NAME ='PAY_REC_IND'
   AND R1.TARGET_TAB_EN_NAME='AGT_DEP_ACCT_STL_INFO_H'
   AND R1.TARGET_TAB_FIELD_EN_NAME='ACPT_PAY_IDF_CD'
    left join (select distinct base_acct_no,card_no from ${iol_schema}.ncbs_new_old_seq_no) p8 on p1.SETTLE_BASE_ACCT_NO=p8.BASE_ACCT_NO and p8.BASE_ACCT_NO LIKE '0%'
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.agt_dep_acct_stl_info_h_ncbsf1_tm 
  	                                group by 
  	                                        agt_id
  	                                        ,acct_id
  	                                        ,stl_id
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.agt_dep_acct_stl_info_h_ncbsf1_cl(
            agt_id -- 协议编号
    ,acct_id -- 账户编号
    ,stl_id -- 结算编号
    ,lp_id -- 法人编号
    ...
    ,open_acct_bank_fin_inst_id -- 开户银行金融机构编号
        ...
        into ${iml_schema}.agt_dep_acct_stl_info_h_ncbsf1_op(
            agt_id -- 协议编号
    ,acct_id -- 账户编号
    ,stl_id -- 结算编号
    ,lp_id -- 法人编号
...
select
    ...
    ,nvl(n.stl_id, o.stl_id) as stl_id -- 结算编号
    ...
    ,nvl(n.out_line_flg, o.out_line_flg) as out_line_flg -- 行外标志
    ...
    ,nvl(n.open_acct_bank_fin_inst_id, o.open_acct_bank_fin_inst_id) as open_acct_bank_fin_inst_id -- 开户银行金融机构编号
    ,case when
            n.agt_id is null
            and n.acct_id is null
            and n.stl_id is null
            and n.lp_id is null
            and n.stl_acct_cls_cd is null
            and n.stl_method_cd is null
            and n.acpt_pay_idf_cd is null
...
from ${iml_schema}.agt_dep_acct_stl_info_h_ncbsf1_tm n
    full join (select * from ${iml_schema}.agt_dep_acct_stl_info_h_ncbsf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
            o.agt_id = n.agt_id
            and o.acct_id = n.acct_id
            and o.stl_id = n.stl_id
            and o.lp_id = n.lp_id
            and o.stl_acct_cls_cd = n.stl_acct_cls_cd
            and o.stl_method_cd = n.stl_method_cd
            and o.acpt_pay_idf_cd = n.acpt_pay_idf_cd
            and o.amt_type_cd = n.amt_type_cd
where (
        o.agt_id is null
        and o.acct_id is null
        and o.stl_id is null
        and o.lp_id is null
        and o.stl_acct_cls_cd is null
        and o.stl_method_cd is null
        and o.acpt_pay_idf_cd is null
    ...
    or (
        ...
        or o.open_acct_bank_fin_inst_id <> n.open_acct_bank_fin_inst_id
    )
;
commit;
...
select
    ...
    ,o.tran_tm -- 交易时间
    ,o.open_acct_bank_fin_inst_id -- 开户银行金融机构编号
    ,o.start_dt -- 开始时间
...
from ${iml_schema}.agt_dep_acct_stl_info_h_ncbsf1_bk o
    left join ${iml_schema}.agt_dep_acct_stl_info_h_ncbsf1_op n
        on
            o.agt_id = n.agt_id
            and o.acct_id = n.acct_id
            and o.stl_id = n.stl_id
    ...
    left join ${iml_schema}.agt_dep_acct_stl_info_h_ncbsf1_cl d
        on
            o.agt_id = d.agt_id
            and o.acct_id = d.acct_id
            and o.stl_id = d.stl_id
            and o.lp_id = d.lp_id
            and o.stl_acct_cls_cd = d.stl_acct_cls_cd
            and o.stl_method_cd = d.stl_method_cd
            and o.acpt_pay_idf_cd = d.acpt_pay_idf_cd
...
--truncate table ${iml_schema}.agt_dep_acct_stl_info_h;
--alter table ${iml_schema}.agt_dep_acct_stl_info_h truncate partition for ('ncbsf1') reuse storage;
...
for cur in (select subpartition_name 
             ...
             where table_owner=upper('${iml_schema}') 
               and table_name=upper('agt_dep_acct_stl_info_h') 
               and substr(subpartition_name,1,8)=upper('p_ncbsf1')
               and substr(subpartition_name,10) >= '${batch_date}' 
               and substr(subpartition_name,10) < '20991231') loop
    v_sql := 'alter table ${iml_schema}.agt_dep_acct_stl_info_h drop subpartition ' || cur.subpartition_name;
...
alter table ${iml_schema}.agt_dep_acct_stl_info_h modify partition p_ncbsf1 
add subpartition p_ncbsf1_${batch_date} values(to_date('${batch_date}','YYYYMMDD')) ;

-- 4.3 exchange partition
alter table ${iml_schema}.agt_dep_acct_stl_info_h exchange subpartition p_ncbsf1_${batch_date} with table ${iml_schema}.agt_dep_acct_stl_info_h_ncbsf1_cl;
alter table ${iml_schema}.agt_dep_acct_stl_info_h exchange subpartition p_ncbsf1_20991231 with table ${iml_schema}.agt_dep_acct_stl_info_h_ncbsf1_op;

-- 5.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.agt_dep_acct_stl_info_h to ${iml_schema};
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.agt_dep_acct_stl_info_h_ncbsf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'agt_dep_acct_stl_info_h', partname => 'p_ncbsf1_20991231',ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
