...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_prd_trust_prod_trusf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
drop table ${iml_schema}.prd_trust_prod_trusf1_tm purge;
drop table ${iml_schema}.prd_trust_prod_trusf1_ex purge;

-- 2.2 add partition for target table
alter table ${iml_schema}.prd_trust_prod add partition p_trusf1 values ('trusf1')(
        subpartition p_trusf1_${batch_date} values (to_date('${batch_date}','yyyymmdd'))
    )
;
alter table ${iml_schema}.prd_trust_prod modify partition p_trusf1
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.prd_trust_prod_trusf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.prd_trust_prod partition for ('trusf1')
where create_dt < to_date('${batch_date}','yyyymmdd')
;

-- 2.4 create temp table
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.prd_trust_prod_trusf1_tm
compress ${option_switch} for query high
as
select
    prod_id -- 产品编号
...
from ${iml_schema}.prd_trust_prod
where 0=1;

-- 2.5 create temp table
create table ${iml_schema}.prd_trust_prod_trusf1_ex nologging
compress ${option_switch} for query high
as select * from ${iml_schema}.prd_trust_prod partition for ('trusf1') where 0=1;

-- 2.1 insert data to tm table
-- nfss_tcs_tbproduct-
insert into ${iml_schema}.prd_trust_prod_trusf1_tm(
    prod_id -- 产品编号
    ,lp_id -- 法人编号
    ,belong_cate_cd -- 归属类别代码
    ,prod_tepla -- 产品模板
...
select
    ...
    ,CASE WHEN R8.TARGET_CD_VAL IS NOT NULL THEN R8.TARGET_CD_VAL ELSE '@'||to_char(P1.RISK_LEVEL) END -- 风险等级代码
    ...
    ,'nfss_tcs_tbproduct' -- 源表名称
    ,'trusf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.nfss_tcs_tbproduct p1 
    ...
    left join  ${iol_schema}.nfss_tcs_tbdict p3 on p3.val = p2.field_value
    ...
    left join ${iml_schema}.ref_pub_cd_map r2 on P1.DIV_MODE= R2.SRC_CODE_VAL
        ...
        AND R2.SRC_TAB_EN_NAME= 'NFSS_TCS_TBPRODUCT'
        AND R2.SRC_FIELD_EN_NAME= 'DIV_MODE'
        AND R2.TARGET_TAB_EN_NAME= 'PRD_TRUST_PROD'
    ...
    left join ${iml_schema}.ref_pub_cd_map r3 on P1.PRD_ATTR= R3.SRC_CODE_VAL
        ...
        AND R3.SRC_TAB_EN_NAME= 'NFSS_TCS_TBPRODUCT'
        AND R3.SRC_FIELD_EN_NAME= 'PRD_ATTR'
        AND R3.TARGET_TAB_EN_NAME= 'PRD_TRUST_PROD'
    ...
    left join ${iml_schema}.ref_pub_cd_map r8 on to_char(P1.RISK_LEVEL)= R8.SRC_CODE_VAL
        ...
        AND R8.SRC_TAB_EN_NAME= 'NFSS_TCS_TBPRODUCT'
        AND R8.SRC_FIELD_EN_NAME= 'RISK_LEVEL'
        AND R8.TARGET_TAB_EN_NAME= 'PRD_TRUST_PROD'
    ...
    left join ${iml_schema}.ref_pub_cd_map r4 on P1.STATUS= R4.SRC_CODE_VAL
        ...
        AND R4.SRC_TAB_EN_NAME= 'NFSS_TCS_TBPRODUCT'
        AND R4.SRC_FIELD_EN_NAME= 'STATUS'
        AND R4.TARGET_TAB_EN_NAME= 'PRD_TRUST_PROD'
    ...
    left join ${iml_schema}.ref_pub_cd_map r5 on P1.CURR_TYPE= R5.SRC_CODE_VAL
        ...
        AND R5.SRC_TAB_EN_NAME= 'NFSS_TCS_TBPRODUCT'
        AND R5.SRC_FIELD_EN_NAME= 'CURR_TYPE'
        AND R5.TARGET_TAB_EN_NAME= 'PRD_TRUST_PROD'
    ...
    left join ${iml_schema}.ref_pub_cd_map r6 on P1.INCOME_CURR_TYPE= R6.SRC_CODE_VAL
        ...
        AND R6.SRC_TAB_EN_NAME= 'NFSS_TCS_TBPRODUCT'
        AND R6.SRC_FIELD_EN_NAME= 'INCOME_CURR_TYPE'
        AND R6.TARGET_TAB_EN_NAME= 'PRD_TRUST_PROD'
    ...
    left join ${iml_schema}.ref_pub_cd_map r7 on P1.PRD_TYPE= R7.SRC_CODE_VAL
        ...
        AND R7.SRC_TAB_EN_NAME= 'NFSS_TCS_TBPRODUCT'
        AND R7.SRC_FIELD_EN_NAME= 'PRD_TYPE'
        AND R7.TARGET_TAB_EN_NAME= 'PRD_TRUST_PROD'
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.prd_trust_prod_trusf1_tm 
  	                                group by 
  	                                        prod_id
  	                                        ,lp_id
  	                               having count(1) > 1);
...
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${iml_schema}.prd_trust_prod_trusf1_ex(
    prod_id -- 产品编号
    ,lp_id -- 法人编号
    ,belong_cate_cd -- 归属类别代码
    ,prod_tepla -- 产品模板
...
select
    ...
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iml_schema}.prd_trust_prod_trusf1_tm n
    full join ${iml_schema}.prd_trust_prod_trusf1_bk o
        on
            o.prod_id = n.prod_id
            and o.lp_id = n.lp_id
;
...
whenever sqlerror exit sql.sqlcode;
alter table ${iml_schema}.prd_trust_prod truncate partition for ('trusf1') reuse storage;

-- 3.2 exchage tm table and target table
alter table ${iml_schema}.prd_trust_prod exchange subpartition p_trusf1_${batch_date} with table ${iml_schema}.prd_trust_prod_trusf1_ex;

-- 3.3 drop subpartition last_date partition
whenever sqlerror continue none ;
alter table ${iml_schema}.prd_trust_prod drop subpartition p_trusf1_${last_date} update global indexes;

-- 4.1 table grant
whenever sqlerror exit sql.sqlcode;
-- grant select on ${iml_schema}.prd_trust_prod to ${iml_schema};

-- 4.2 drop tm table
drop table ${iml_schema}.prd_trust_prod_trusf1_tm purge;
drop table ${iml_schema}.prd_trust_prod_trusf1_ex purge;
drop table ${iml_schema}.prd_trust_prod_trusf1_bk purge;

-- 5 gater table status
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'prd_trust_prod', partname => 'p_trusf1_${batch_date}', ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);