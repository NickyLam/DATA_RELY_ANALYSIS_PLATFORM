...
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py yyyymmdd iml_ast_col_list_stock_inpwn_info_icmsf1
CreateDate: 20180515
Logs:
    zjj 2018-05-15 新建表本
*/
...
alter table ${iml_schema}.ast_col_list_stock_inpwn_info add partition p_icmsf1 values ('icmsf1')(
...
-- it is no need to check when this segment SQL was return faied
whenever sqlerror continue none ;
create table ${iml_schema}.ast_col_list_stock_inpwn_info_icmsf1_bk nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.ast_col_list_stock_inpwn_info partition for ('icmsf1')
...
whenever sqlerror continue none ;
-- it is no need to check when this segment SQL was return faied
drop table ${iml_schema}.ast_col_list_stock_inpwn_info_icmsf1_tm purge;
drop table ${iml_schema}.ast_col_list_stock_inpwn_info_icmsf1_op purge;
drop table ${iml_schema}.ast_col_list_stock_inpwn_info_icmsf1_cl purge;

-- 2.3 create temp table
-- it is no need to check when this segment SQL was return faied
whenever sqlerror exit sql.sqlcode;
create table ${iml_schema}.ast_col_list_stock_inpwn_info_icmsf1_tm nologging
compress ${option_switch} for query high
as select
    asset_id -- 资产编号
    ,lp_id -- 法人编号
    ...
    ,close_pos_line -- 平仓线
    ,etd_inpwn_flg -- 场内质押标志
    ,inpwn_rgst_id -- 质押登记编号
    ,inpwn_tot_val -- 质押总价值
    ,restr_exp_dt -- 限售到期日期
    ,other_comnt -- 其他说明
    ,curr_cd -- 币种代码
...
from ${iml_schema}.ast_col_list_stock_inpwn_info partition for ('icmsf1')
where 0=1
;

create table ${iml_schema}.ast_col_list_stock_inpwn_info_icmsf1_op nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.ast_col_list_stock_inpwn_info partition for ('icmsf1') where 0=1;

create table ${iml_schema}.ast_col_list_stock_inpwn_info_icmsf1_cl nologging
compress ${option_switch} for query high
as
select * from ${iml_schema}.ast_col_list_stock_inpwn_info partition for ('icmsf1') where 0=1;

-- 3.1 get new data into table
-- icms_clr_asset_finance_listedstock-
insert into ${iml_schema}.ast_col_list_stock_inpwn_info_icmsf1_tm(
    asset_id -- 资产编号
    ,lp_id -- 法人编号
    ,stock_cd -- 股票代码
    ,stock_name -- 股票名称
...
select
    ...
    ,decode(P1.PLEDGETAG,'01','1','02','0',' ','-',P1.PLEDGETAG) -- 场内质押标志
    ,P1.PLEDGEREGISTRATION -- 质押登记编号
    ,p1.TOTALVALUE -- 质押总价值
    ,P1.DUEDATE -- 限售到期日期
    ,p1.REMARK -- 其他说明
    ...
    ,p1.STOCKIDC -- 托管券商名称
    ,to_date('${batch_date}','yyyymmdd') as etl_dt -- ETL处理日期
    ,'icms_clr_asset_finance_listedstock' -- 源表名称
    ,'icmsf1' -- 任务编码
    ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp -- ETL处理时间戳
from ${iol_schema}.icms_clr_asset_finance_listedstock p1
    left join ${iml_schema}.ref_pub_cd_map r1 on P1.BOURSE = R1.SRC_CODE_VAL
        AND R1.SORC_SYS_CD= 'ICMS'
        AND R1.SRC_TAB_EN_NAME= 'ICMS_CLR_ASSET_FINANCE_LISTEDSTOCK'
...
begin
  for cur in (select 1 from dual) loop
  	select count(1) into cnt from (select 1 from ${iml_schema}.ast_col_list_stock_inpwn_info_icmsf1_tm 
  	                                group by 
  	                                        asset_id
  	                                        ,lp_id
  	                               having count(1) > 1);
...
insert /*+ append */ all
    when end_dt = to_date('${batch_date}', 'yyyymmdd') then
        into ${iml_schema}.ast_col_list_stock_inpwn_info_icmsf1_cl(
            asset_id -- 资产编号
    ,lp_id -- 法人编号
    ,stock_cd -- 股票代码
    ,stock_name -- 股票名称
    ...
    ,trust_broker_name -- 托管券商名称
        ...
        into ${iml_schema}.ast_col_list_stock_inpwn_info_icmsf1_op(
            asset_id -- 资产编号
    ,lp_id -- 法人编号
    ,stock_cd -- 股票代码
    ,stock_name -- 股票名称
...
select
    ...
    ,nvl(n.etd_inpwn_flg, o.etd_inpwn_flg) as etd_inpwn_flg -- 场内质押标志
    ,nvl(n.inpwn_rgst_id, o.inpwn_rgst_id) as inpwn_rgst_id -- 质押登记编号
...
from ${iml_schema}.ast_col_list_stock_inpwn_info_icmsf1_tm n
    full join (select * from ${iml_schema}.ast_col_list_stock_inpwn_info_icmsf1_bk where start_dt < to_date('${batch_date}','yyyymmdd') and end_dt >= to_date('${batch_date}','yyyymmdd')) o
        on
...
where (
    ...
    or (
        ...
        or o.etd_inpwn_flg <> n.etd_inpwn_flg
        or o.inpwn_rgst_id <> n.inpwn_rgst_id
...
select
    ...
    ,o.etd_inpwn_flg -- 场内质押标志
    ,o.inpwn_rgst_id -- 质押登记编号
...
from ${iml_schema}.ast_col_list_stock_inpwn_info_icmsf1_bk o
    left join ${iml_schema}.ast_col_list_stock_inpwn_info_icmsf1_op n
        on
            o.asset_id = n.asset_id
            and o.lp_id = n.lp_id
            and o.end_dt >= to_date('${batch_date}','yyyymmdd')
    left join ${iml_schema}.ast_col_list_stock_inpwn_info_icmsf1_cl d
        on
...
--alter table ${iml_schema}.ast_col_list_stock_inpwn_info truncate partition for ('icmsf1') reuse storage;
...
for cur in (select subpartition_name 
             ...
             where table_owner=upper('${iml_schema}') 
               and table_name=upper('ast_col_list_stock_inpwn_info') 
               and substr(subpartition_name,1,8)=upper('p_icmsf1')
               and substr(subpartition_name,10) >= '${batch_date}' 
               and substr(subpartition_name,10) < '20991231') loop
    v_sql := 'alter table ${iml_schema}.ast_col_list_stock_inpwn_info drop subpartition ' || cur.subpartition_name;
...
whenever sqlerror exit sql.sqlcode;
alter table ${iml_schema}.ast_col_list_stock_inpwn_info modify partition p_icmsf1 
add subpartition p_icmsf1_${batch_date} values(to_date('${batch_date}','YYYYMMDD')) ;

-- 4.3 exchange partition
alter table ${iml_schema}.ast_col_list_stock_inpwn_info exchange subpartition p_icmsf1_${batch_date} with table ${iml_schema}.ast_col_list_stock_inpwn_info_icmsf1_cl;
alter table ${iml_schema}.ast_col_list_stock_inpwn_info exchange subpartition p_icmsf1_20991231 with table ${iml_schema}.ast_col_list_stock_inpwn_info_icmsf1_op;
...
whenever sqlerror exit sql.sqlcode;
drop table ${iml_schema}.ast_col_list_stock_inpwn_info_icmsf1_bk purge;

-- 6 gater table status
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${iml_schema}',tabname => 'ast_col_list_stock_inpwn_info', partname => 'p_icmsf1_20991231',ESTIMATE_PERCENT=>10, method_opt=>'for all columns size 1',no_invalidate=>false, force=>true, granularity => 'SUBPARTITION', degree => 8, cascade => true);
