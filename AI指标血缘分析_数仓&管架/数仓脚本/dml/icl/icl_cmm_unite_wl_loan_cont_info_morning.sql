...
lter session force parallel dml parallel 4;
...
insert /*+ append */ into ${icl_schema}.cmm_unite_wl_loan_cont_info_ex(
 ...
 left join ( select t1.objectno as cont_id
                    ...
                    ,max(case when t1.tagid ='2025041800000023' then to_char(t2.itemno)   else '' end)  as dir_indus_cd		             -- 行业投向代码
                    ...
                    ,max(case when t1.tagid ='2025033100000062' then to_char(t1.tagvalue) else '' end)  as agclt_flg		                   -- 涉农标志
...
delete from ${icl_schema}.cmm_icl_batch_jnl  where etl_dt = to_date('${batch_date}', 'yyyymmdd') and tab_name = 'cmm_unite_wl_loan_cont_info_morning';
commit;
whenever sqlerror exit sql.sqlcode;
...
select
   ...
   ,'cmm_unite_wl_loan_cont_info_morning'
   ,1                                                                 -- 跑批状态
