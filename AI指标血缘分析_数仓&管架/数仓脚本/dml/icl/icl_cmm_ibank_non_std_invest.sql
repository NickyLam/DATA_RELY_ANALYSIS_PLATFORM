...
Logs:
            ...
            20200424 周沁晖 1、调整资产四分类代码 acctnt_cls_cd ——> asset_four_cls_cd
			      ...
			      20210113 谢  宁 新增字段【底层资产投向行业门类代码】
			      20210122 周沁晖 增加字段【当期余额】
            20210322 陈伟峰 过滤余额为0的数据（逾期数据）
            ...
            20210619 陈伟峰 增加字段【底层债券评级结果代码】
            20210621 陈伟峰 增加字段【最终投向类型代码、最终投向行业_大类、最终投向行业_细类、投向产业基金的部分、投向债转股的部分、投向私募股权投资基金的部分、投向私募资产管理产品的部分】
            20210705 何桐金 增加字段【底层资产类型编号】
                            增加字段【交易对手客户编号】
            20210705 陈伟峰 调整科目15130701逻辑置空
            20210708 何桐金 调整第一组和第二组T12表过滤条件，增加stream_id = (select t.cash_accid from ttrd_acc_cash_ext_map t where t.cash_ext_accid = '3080');
            20210813 陈伟峰 将第二组拆分成【同业净值型产品投资】模型，投产日净值型产品入到新模型中，投产日前仍进到非标模型中
            20210830 何桐金 调整PRD_IBANK_BOND_EVLTION算法，从全量快照，改成全量流水，C层同步更改
            ...
            20211227 陈伟峰 新增字段【表内外标志代码、本金逾期标志、利息逾期标志】
                            ...
                            调整字段【利息调整科目编号、应计利息、应收利息、公允价值变动、利息调整金额】加工逻辑
            20211227 陈伟峰 修复利息调整逻辑大小写问题
            20221021 温旺清 1、增加字段【应收未收利息科目编号、授信金融工具编号、资产唯一标识编号】
                            2、调整字段【应计利息、应收利息、利息科目编号、公允价值变动】的加工口径
            20221124 温旺清 增加字段【最终投向行业_中类】
...
drop table ${icl_schema}.tmp_cmm_ibank_non_std_invest_04 purge;
drop table ${icl_schema}.tmp_cmm_ibank_non_std_invest_05 purge;
drop table ${icl_schema}.tmp_cmm_ibank_non_std_invest_06 purge;
...
--本金科目信息
whenever sqlerror exit sql.sqlcode;
create table ${icl_schema}.tmp_cmm_ibank_non_std_invest_01 as 			
...
whenever sqlerror exit sql.sqlcode;
create table ${icl_schema}.tmp_cmm_ibank_non_std_invest_04
nologging
compress ${option_switch} for query high
as
select *
...
select t9.i_code,
  ...
  left join ${icl_schema}.tmp_cmm_ibank_non_std_invest_04 t10
...
--手工记账利息
whenever sqlerror exit sql.sqlcode;
create table ${icl_schema}.tmp_cmm_ibank_non_std_invest_05 as 
...
--公允价值变动核算差额
whenever sqlerror exit sql.sqlcode;
create table ${icl_schema}.tmp_cmm_ibank_non_std_invest_06 as
...
insert /*+ append */ into ${icl_schema}.cmm_ibank_non_std_invest_ex(
    ...
    ,final_dir_indus_gen	                 --最终投向行业_大类
   	,final_dir_indus_middle_class           --最终投向行业_中类
...
select to_date('${batch_date}','yyyymmdd')    as etl_dt                    --数据日期
      ...
      ,case when t4.prod_type_cd in ('0170') and t4.asset_type_name = '票据资管计划（非保本）' and '${batch_date}' <= '20201231' then '15032201'
            ...
            else t22.subj_id end            as subj_id             		     --科目编号
      ,coalesce(trim(t14.int_subj_id), t14.int_subj_id2, t22.int_subj_id)         as int_subj_id   --利息科目编号
      ,nvl(trim(t14.un_int_subj_id),t22.int_subj_id)      as recvbl_uncol_int_subj_id	  --应收未收利息科目编号
      ,coalesce(t14.int_subj_id, decode(t25.gzb_type,'2',t25.subj_code,''),t22.int_adj_subj_id)  as int_adj_subj_id  			--利息调整科目编号
      ,t3.tran_market_id                      as tran_market_id             --交易市场编号
      ,t3.exchg_acct_id                       as exchg_acct_id              --交易所账户编号
      ,t21.cust_id                            as cntpty_cust_id             --交易对手客户编号	
      ...
      ,t1.net_price_cost                      as currt_bal                  --当期余额
       ...
       ,case when t4.prod_type_cd in ('0702','0703','0704','0705') then nvl(t38.receive_ai,0)
      ...
      ,case when t25.gzb_type='2' then t1.int_adj_amt+t25.value
            else t1.int_adj_amt end				    as int_adj_amt				        --利息调整金额
      ,case when t25.gzb_type='4.1' then t1.evha_val_chag+t25.value+nvl(t37.chg_fv,0)
            else t1.evha_val_chag+nvl(t37.chg_fv,0) end    as evha_val_chag				      --公允价值变动
      ...
      ,case when t13.business_category = '0' then '21'
       ...
       else '-' end                           as uder_dir_indus_categy_cd   --底层资产投向行业门类代码
      ...
      ,t9.grade                               as uder_bond_rating_rest_cd   --底层债券评级结果代码   
	  ...
	  ,t13.business_category_mid              as final_dir_indus_middle_class  --最终投向行业_中类
      ...
      ,case when t23.is_cp_overdue = '1' then  decode(${iml_schema}.dateformat_max2(t23.beg_date_cp_overdue),to_date('29991231', 'yyyymmdd'),'0',
        to_date('${batch_date}', 'yyyymmdd') - ${iml_schema}.dateformat_max(t23.beg_date_cp_overdue))  else '0' end             as pric_ovdue_days  -- 本金逾期天数
      ...
      ,case when t23.is_ai_overdue = '1' then decode(${iml_schema}.dateformat_max2(t23.beg_date_ai_overdue),to_date('29991231', 'yyyymmdd'),'0',
        to_date('${batch_date}', 'yyyymmdd') - ${iml_schema}.dateformat_max(t23.beg_date_ai_overdue)) else '0' end              as int_ovdue_days   -- 利息逾期天数
      ,t1.job_cd
 ...
 inner join ${iml_schema}.prd_ibank_cap_ld_fin_instm t4 --同业资金借贷金融工具
  ...
  left join (select a.i_code,a.a_type,a.m_type,a.ad_fixingrate,a.ad_spread,a.ad_multiplier 	      
              ...
              where t.cash_ext_accid = '3080'
                and to_char(to_date(trim(a.ad_startdate),'yyyy-mm-dd'),'yyyymmdd') <= '${batch_date}'
                and to_char(to_date(trim(a.ad_enddate),'yyyy-mm-dd'),'yyyymmdd') > '${batch_date}'
   ...
   and t18.rn = 1
/*  left join (select o1.acctg_obj_id as obj_id,
                    sum(case when to_date('${batch_date}', 'yyyymmdd') - to_date(o1.cal_end_date, 'yyyy-mm-dd') < 0 
                             then 0
                             else to_date('${batch_date}', 'yyyymmdd') -to_date(o1.cal_end_date, 'yyyy-mm-dd')
                              end) as duedays,
                    sum(due_ai) as due_ai,
                    sum(due_cp) as due_cp,
                    min(cal_end_date) as cal_end_date
               from ${iol_schema}.ibms_ttrd_accounting_dtldue_obj_his o1
              where to_date(o1.beg_date, 'yyyy-mm-dd') = to_date('${batch_date}', 'yyyymmdd')
                and due_cp <> 0
  ...
  left join  (select o1.acctg_obj_id as obj_id,
              ...
              where to_date(o1.beg_date, 'yyyy-mm-dd') = to_date('${batch_date}', 'yyyymmdd')
                and due_ai <> 0
              group by acctg_obj_id) t20
   on  t1.obj_id = t20.obj_id */
  left join ${iml_schema}.pty_ibank_cntpty_info t21
    on t6.cntpty_id= t21.src_party_id	
   and t21.create_dt <= to_date('${batch_date}', 'yyyymmdd')
   and t21.job_cd = 'ibmsf1'
   and t21.id_mark<>'D'
  left join ${icl_schema}.cmm_ibank_non_std_invest t22
    on t1.obj_id = t22.obj_id
   and t22.etl_dt = to_date('${batch_date}', 'yyyymmdd') - 1
  ...
  left join ${iml_schema}.pty_ibank_cntpty_info t24  --${iol_schema}.ibms_ttrd_institution t24
   ...
   and t24.id_mark<>'D'
  left join ${icl_schema}.tmp_cmm_ibank_non_std_invest_05 t25
  ...
  left join ${icl_schema}.tmp_cmm_ibank_non_std_invest_06 t37
    on t1.obj_id =t37.accti_obj_id
  left join (select inst_i_code,inst_a_type,inst_m_type,receive_ai
    ...
    on t1.fin_instm_id = t38.inst_i_code  
   and t1.asset_type_id = t38.inst_a_type 
   and t1.market_type_id = t38.inst_m_type
 where t1.start_dt <= to_date('${batch_date}', 'yyyymmdd')
   and t1.end_dt > to_date('${batch_date}', 'yyyymmdd')
   and t1.job_cd = 'ibmsf1'
   and t1.obj_id <> '20219'
 /*  and not exists (select 1 
                     from ${icl_schema}.tmp_cmm_ibank_non_std_invest_04 tt
...
select to_date('${batch_date}','yyyymmdd')    as etl_dt                    --数据日期
      ...
      ,nvl(trim(t14.int_subj_id),t21.int_subj_id)         as int_subj_id    		    --利息科目编号
      ,nvl(trim(t14.un_int_subj_id),t21.int_subj_id)      as recvbl_uncol_int_subj_id	  --应收未收利息科目编号
      ,coalesce(t14.int_subj_id, decode(t25.gzb_type,'2',t25.subj_code,''),t21.int_adj_subj_id)  as int_adj_subj_id  			--利息调整科目编号	  	  
      ,t3.tran_market_id                      as tran_market_id             --交易市场编号
      ,t3.exchg_acct_id                       as exchg_acct_id              --交易所账户编号
      ,t20.cust_id                            as cntpty_cust_id                         --交易对手客户编号
      ,t6.cntpty_id                           as cntpty_id                  --交易对手编号
      ,t6.cntpty_name                         as cntpty_name                --交易对手名称
      ...
      ,trim(t6.apv_odd_no)                    as apv_odd_no                 --审批单号
	  ,nvl(trim(t36.approve_i_code), t1.fin_instm_id)     as crdt_fin_instm_id          --授信金融工具编号
	  ,nvl(trim(t36.approve_i_code), t1.fin_instm_id) || '_' || t1.asset_thd_cls_cd || '_' || t1.ext_vch_acct_id    as asset_uniq_idf_id          --资产唯一标识编号  	  
      ,t4.curr_cd                             as curr_cd                    --币种代码
      ...
      ,t1.net_price_cost                      as currt_bal                  --当期余额	  
       ...
       ,case when t4.prod_type_cd in ('0000', '0005', '1100', '1200', '0170') then
             ...
             else t1.acru_int + t1.recvbl_uncol_acru_int + nvl(t25.value, 0)
         end as int_recvbl                    -- 应收利息	 
      ,t1.recvbl_uncol_net_price_cost         as recvbl_uncol_pric          --应收未收净价成本
      ,t1.recvbl_uncol_acru_int               as recvbl_uncol_int           --应收未收应计利息
      ,t1.int_adj_amt						              as int_adj_amt				        --利息调整金额
      ,case when t25.gzb_type='4.1' then t1.evha_val_chag+t25.value
            else t1.evha_val_chag end         as evha_val_chag 			        --公允价值变动
      ...
      ,t1.ext_dimen_info	                    as ovdue_status               --逾期状态
      ,''                                                                   --表内外标志代码
      ,(case when t1.ext_dimen_info like '%OverDue90%' then '1' else '0' end) as  ovdue_flg --逾期标志
      ,''                                                                   --本金逾期标志   
      ,t18.cal_end_date	                      as pric_ovdue_dt              --本金逾期日期
      ,nvl(trim(t18.duedays),0)               as pric_ovdue_days            --本金逾期天数
      ,''                                                                   --利息逾期标志   
      ,t19.cal_end_date	                      as int_ovdue_dt               --利息逾期日期
      ,nvl(trim(t19.duedays),0)	              as int_ovdue_days             --利息逾期天数
      ,t1.job_cd
      ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6') as etl_timestamp   --数据处理时间
  from ${iml_schema}.agt_secu_acct_accti_bal_h t1  --证券账户核算余额历史
  left join ${iml_schema}.agt_intnal_secu_acct t2 --内部证券账户
 ...
 inner join ${iml_schema}.prd_fin_instm t4 --金融工具表
  ...
  left join (select o1.acctg_obj_id as obj_id,
              ...
              where to_date(o1.beg_date, 'yyyy-mm-dd') = to_date('${batch_date}', 'yyyymmdd')
                and due_cp <> 0
              group by acctg_obj_id) t18
    on  t1.obj_id = t18.obj_id
  left join  (select o1.acctg_obj_id as obj_id,
                     sum(case when to_date('${batch_date}', 'yyyymmdd') - to_date(o1.cal_end_date, 'yyyy-mm-dd') < 0 
                              then 0
                              else to_date('${batch_date}', 'yyyymmdd') -to_date(o1.cal_end_date, 'yyyy-mm-dd')
              ...
              group by acctg_obj_id) t19
   on  t1.obj_id = t19.obj_id
 left join ${iml_schema}.pty_ibank_cntpty_info t20
   on t6.cntpty_id= t20.src_party_id	
  and t20.create_dt <= to_date('${batch_date}', 'yyyymmdd')
  and t20.job_cd = 'ibmsf1'
  and t20.id_mark<>'D'
 left join ${icl_schema}.cmm_ibank_non_std_invest t21
