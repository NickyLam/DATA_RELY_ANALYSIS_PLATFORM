...
Logs: 20250219 谢  宁 新增组别【微业贷】
      ...
      20260306 陈  凭 调整第二组微业贷加工逻辑，替换T1表为agt_wyd_loan_cont_h，更新【贷款合同编号、担保合同编号、标准产品编号】取值逻辑
...
insert /*+ append */ into ${icl_schema}.cmm_unite_wl_loan_guar_cont_rela_ex(
 ...
 select
       ...
       ,t2.curr_cd                          -- 担保币种代码    
       ,t2.guar_cont_status_cd              -- 担保合同状态代码
       ,t2.guartor_cust_id                  -- 担保人客户号    
       ,t2.guartor_name                     -- 担保人名称      
       ,t2.effect_dt                        -- 担保生效日期    
       ,t2.invalid_dt                       -- 担保到期日期    
       ,t1.job_cd                           -- 任务代码
       ,to_timestamp('${batch_timestamp}','yyyy-mm-dd hh24:mi:ss.ff6')  -- etl处理时间戳
   from ${iml_schema}.agt_wsd_guar_contasguar_rela_h t1
   left join ${iml_schema}.agt_wsd_guar_cont_info_h t2 
     on t1.agt_id = t2.agt_id
    and t2.start_dt <= to_date('${batch_date}','yyyymmdd')
    and t2.end_dt > to_date('${batch_date}','yyyymmdd')
    and t2.job_cd = 'mybkf1'
   left join ${iol_schema}.icms_mybk_business_contract t3
     on t1.obj_id = t3.serialno
...
insert /*+ append */ into ${icl_schema}.cmm_unite_wl_loan_guar_cont_rela_ex(
 ...
 select
       ...
       ,'C'                                 -- 担保方式代码    
       ,t3.curr_cd                          -- 担保币种代码    
       ,case when t1.status_cd in ('101','112') then '2' 
	         when t1.status_cd in ('111','103') then '4'
		else '-' end                        -- 担保合同状态代码
       ,t1.guartor_id                       -- 担保人客户号    
       ,t1.guartor_name                     -- 担保人名称      
       ,t1.effect_dt                        -- 担保生效日期    
       ,t1.exp_dt                           -- 担保到期日期    
       ,t1.job_cd                           -- 任务代码
       ,to_timestamp('${batch_timestamp}','yyyy-mm-dd hh24:mi:ss.ff6')  -- etl处理时间戳
   from ${iml_schema}.agt_wph_guar_cont_info_h t1
  left join ${iml_schema}.agt_wph_loan_guar_cont_rela t2
     on t1.guar_cont_id = t2.obj_id
	and t2.start_dt <= to_date('${batch_date}', 'yyyymmdd')
	and t2.end_dt > to_date('${batch_date}', 'yyyymmdd')
    and t2.job_cd ='icmsf1'
  left join  ${iml_schema}.agt_wph_guar_cont_guar_rela t3
     on t2.OBJ_ID = t3.guar_cont_id
	and t3.start_dt <= to_date('${batch_date}', 'yyyymmdd')
	and t3.end_dt > to_date('${batch_date}', 'yyyymmdd')
...
insert /*+ append */ into ${icl_schema}.cmm_unite_wl_loan_guar_cont_rela_ex(
 ...
 select
       ...
       ,t3.curr_cd                             --担保币种代码
       ,decode(t3.guar_cont_status_cd,'112','2','110','1','111','4',t3.guar_cont_status_cd)   --担保合同状态代码
       ,t3.guartor_id                          --担保人客户号
       ,t3.guartor_name                        --担保人名称
       ,t3.effect_dt                           --担保生效日期
       ,t3.exp_dt                              --担保到期日期  
       ,'icms_lh'                              --任务代码
       ,to_timestamp('${batch_timestamp}','yyyy-mm-dd hh24:mi:ss.ff6')  -- etl处理时间戳
   from ${iml_schema}.agt_lhwd_guar_cont_rela_h t1
  left join ${iml_schema}.agt_lhwd_guar_cont_guar_rela_h t2
	 on t1.guar_cont_id=t2.guar_cont_id
	and t2.start_dt <= to_date('${batch_date}', 'yyyymmdd')
	and t2.end_dt > to_date('${batch_date}', 'yyyymmdd')
    and t2.job_cd ='icmsf1'
   left join ${iml_schema}.agt_lhwd_guar_cont_info_h t3
     on t1.guar_cont_id=t3.guar_cont_id
	and t3.start_dt <= to_date('${batch_date}', 'yyyymmdd')
	and t3.end_dt > to_date('${batch_date}', 'yyyymmdd')
    and t3.job_cd ='icmsf1'
   left join ${iml_schema}.agt_lhwd_loan_cont_info_h t4
     on t1.obj_id=t4.cont_id
...
delete from ${icl_schema}.cmm_icl_batch_jnl  where etl_dt = to_date('${batch_date}', 'yyyymmdd') and tab_name = 'cmm_unite_wl_loan_guar_cont_rela';
commit;
whenever sqlerror exit sql.sqlcode;
...
select
   ...
   ,'cmm_unite_wl_loan_guar_cont_rela'
   ,1                                                                 -- 跑批状态
