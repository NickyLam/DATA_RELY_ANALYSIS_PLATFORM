/*
Purpose:    共性加工层-对公贷款余额信息表：包括所有的对公贷款余额，其中包含传统的对公贷款余额、委托贷款余额、垫款余额等。数据全部来源于新核心系统NCBS。
Author:     Sunline
Usage:      python $ETL_HOME/script/main.py 20220930 icl_cmm_corp_loan_acct_bal_info
CreateDate: 20220329
...
--alter table ${icl_schema}.cmm_corp_loan_acct_bal_info drop partition p_${retain_day};
alter table ${icl_schema}.cmm_corp_loan_acct_bal_info add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 1.2 create table for exchage and add partition
whenever sqlerror continue none;
drop table ${icl_schema}.tmp_cmm_corp_loan_acct_bal_info_01 purge;

-- 1.3 insert data to tmp table
-- 获取账户结算信息
whenever sqlerror exit sql.sqlcode;
create table ${icl_schema}.tmp_cmm_corp_loan_acct_bal_info_01
nologging
...
select  t1.lp_id                                                  -- 法人编号
       ...
       ,nvl(t3.ld_nomal_pric,0) as nomal_pric                     -- 正常本金
       ,case when nvl(trim(t10.init_loan_num),t1.accti_status_cd) in ('WRN') then 0
             else nvl(t3.ld_ovdue_pric,0)
        end as ovdue_pric                                         -- 逾期本金
       ,nvl(t4.ld_wrt_off_pric,0) as wrt_off_pric                 -- 核销本金
       ,nvl(t4.ld_wrt_off_nomal_int,0) + nvl(t4.ld_wrt_off_comp_int_int,0) + nvl(t4.ld_wrt_off_pnlt_int,0) as wrt_off_int                                       -- 核销利息
       ,case when t1.prod_id like '6020301%' then 0
             when nvl(t8.discnt_int,0) <> 0 and nvl(t3.ld_nomal_pric,0) + nvl(t3.ld_ovdue_pric,0) - nvl(t4.ld_wrt_off_pric, 0) > 0 then nvl(t8.discnt_int,0) - nvl(t8.ld_acm_provi_int, 0)
             else case when nvl(trim(t10.init_loan_num),t1.accti_status_cd) in ('ZHC', 'YUQ') and nvl(t8.discnt_int,0) = 0 then nvl(t8.ld_acm_provi_int, 0) + nvl(t8.ld_acm_int_adj_amt,0) + nvl(t3.ld_ovdue_int, 0) + nvl(t3.ld_ovdue_pnlt, 0) + nvl(t9.int_accrued_prev,0) + nvl(t9.int_adj_prev,0)
                       when nvl(trim(t10.init_loan_num),t1.accti_status_cd) = 'WRN' then nvl(t9.int_accrued_prev,0) + nvl(t9.int_adj_prev,0)
                      else 0
                  end
        end as in_bs_int                                         -- 表内利息
       ,case when t1.prod_id like '6020301%' then 0
             else case when nvl(trim(t10.init_loan_num),t1.accti_status_cd) in ('FYJ', 'FY') then /*coalesce(t6.ld_acm_provi_int, 0) + coalesce(t6.ld_acm_int_adj_amt,0) + */nvl(t3.ld_ovdue_int, 0) + nvl(t3.ld_ovdue_pnlt, 0) + nvl(t3.ld_ovdue_comp_int, 0)
                       else 0
                   end
        end as off_bs_int                                         -- 表外利息
       ,nvl(t3.ld_nomal_pric,0) + nvl(t3.ld_ovdue_pric,0) as pric_bal                                                                                          -- 本金余额
       ,nvl(t3.ld_nomal_pric,0) + nvl(t3.ld_ovdue_pric,0) - nvl(t4.ld_wrt_off_pric,0) as currt_bal                                                             -- 当期余额
       ,(nvl(t3.ld_nomal_pric,0) + nvl(t3.ld_ovdue_pric,0)- nvl(t4.ld_wrt_off_pric,0)) * nvl(t5.convt_cny_exch_rat, 1) as cl_curr_currt_bal                                               -- 折本币当期余额                                                                               -- 表外利息折本币月日均余额
       ,t1.job_cd                             -- 任务代码
...
--   and t7.crdt_prod_cate_cd not in ('2','3','4')   --零售贷款,联合网贷,个人委托贷款
  ...
  left join ${iml_schema}.agt_loan_acct_int_dtl_h t8
...
whenever sqlerror continue none;
drop table ${icl_schema}.cmm_corp_loan_acct_bal_info_ex purge;
alter table ${icl_schema}.cmm_corp_loan_acct_bal_info add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 2.1 insert into ex table
whenever sqlerror exit sql.sqlcode;
create table ${icl_schema}.cmm_corp_loan_acct_bal_info_ex
nologging
compress ${option_switch} for query high
as select * from ${icl_schema}.cmm_corp_loan_acct_bal_info where 0=1;

whenever sqlerror exit sql.sqlcode;
insert into ${icl_schema}.cmm_corp_loan_acct_bal_info_ex(
       etl_dt                         -- 数据日期
       ,lp_id                         -- 法人编号
       ,acct_id                       -- 账户编号
       ,dubil_num                     -- 借据号
...
select to_date('${batch_date}','yyyymmdd')                        -- 数据日期
       ...
       ,nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0) + nvl(t10.wrtn_off_pric, 0)  -- 本金余额
       ,nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0)             -- 当期余额
       ,(nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0)) * nvl(t5.convt_cny_exch_rat, 1)         -- 折本币当期余额	   	   	   
       ,coalesce(t6.currt_bal,t6_1.currt_bal, 0)                                      -- 日初余额
       ,case when substr('${batch_date}',7,2) = '01' then nvl((nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0)),0) else coalesce(t6.ear_m_bal,t6_1.ear_m_bal,0.0) end       --月初余额
       ,case when substr('${batch_date}',5,4) in ('0101','0401','0701','1001') then nvl((nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0)),0)  else coalesce(t6.ear_s_bal,t6_1.ear_s_bal,0.0) end   --季初余额
       ,case when substr('${batch_date}',5,4) = '0101' then nvl((nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0)),0) else coalesce(t6.ear_y_bal,t6_1.ear_y_bal,0.0) end     --年初余额
       ,case when substr('${batch_date}',5,4) = '0101' then nvl((nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0)),0) else coalesce(t6.y_acm_bal,t6_1.y_acm_bal,0.0)+nvl((nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0)),0) end          --年累计余额
       ,case when substr('${batch_date}',5,4) in ('0101','0401','0701','1001') then nvl((nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0)),0) else coalesce(t6.s_acm_bal,t6_1.s_acm_bal,0.0) + nvl((nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0)),0) end       --季累计余额
       ,case when substr('${batch_date}',7,2) = '01' then nvl((nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0)),0) else coalesce(t6.m_acm_bal,t6_1.m_acm_bal,0.0) + nvl((nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0)),0) end               --月累计余额
       ...
       ,case when substr('${batch_date}',5,4) = '0101' then coalesce(t6.cl_curr_currt_bal,t6_1.cl_curr_currt_bal,0.0) else coalesce(t6.cl_curr_ear_y_bal,t6_1.cl_curr_ear_y_bal,0.0) end      --折本币年初余额	   	   
       ,case when substr('${batch_date}',5,4) = '0101' then (nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0))*nvl(trim(t5.convt_cny_exch_rat),1) else coalesce(t6.cl_curr_y_acm_bal,t6_1.cl_curr_y_acm_bal,0.0) + (nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0))*nvl(trim(t5.convt_cny_exch_rat),1) end        --折本币年累计余额		
       ...
       ,case when substr('${batch_date}',5,4) in ('0101','0401','0701','1001') then (nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0))*nvl(trim(t5.convt_cny_exch_rat),1) else coalesce(t6.cl_curr_s_acm_bal,t6_1.cl_curr_s_acm_bal, 0.0) + (nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0))*nvl(trim(t5.convt_cny_exch_rat),1) end   -- 折本币季累计余额
       ...
       ,case when substr('${batch_date}',7,2) = '01' then (nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0))*nvl(trim(t5.convt_cny_exch_rat),1) else coalesce(t6.cl_curr_m_acm_bal,t6_1.cl_curr_m_acm_bal, 0.0) + (nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0))*nvl(trim(t5.convt_cny_exch_rat),1) end    -- 折本币月累计余额
       ...
       ,(case when substr('${batch_date}',5,4) = '0101' then (nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0)) else coalesce(t6.y_acm_bal,t6_1.y_acm_bal, 0.0) + (nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0)) end) / (to_date('${batch_date}', 'yyyymmdd') - trunc(to_date('${batch_date}', 'yyyymmdd'), 'y') + 1) -- 年日均余额
       ,(case when substr('${batch_date}',5,4) in ('0101','0401','0701','1001') then (nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0)) else coalesce(t6.s_acm_bal,t6_1.s_acm_bal, 0.0) + (nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0)) end) / (to_date('${batch_date}', 'yyyymmdd') - trunc(to_date('${batch_date}', 'yyyymmdd'), 'q') + 1) -- 季日均余额
       ,(case when substr('${batch_date}',7,2) = '01' then (nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0)) else coalesce(t6.m_acm_bal,t6_1.m_acm_bal, 0.0) + (nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0)) end) / to_number(substr('${batch_date}', 7, 2)) -- 月日均余额
       ,(case when substr('${batch_date}',5,4) = '0101' then (nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0))*nvl(trim(t5.convt_cny_exch_rat),1) else coalesce(t6.cl_curr_y_acm_bal,t6_1.cl_curr_y_acm_bal, 0.0) + (nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0))*nvl(trim(t5.convt_cny_exch_rat),1) end) / (to_date('${batch_date}', 'yyyymmdd') - trunc(to_date('${batch_date}', 'yyyymmdd'), 'y') + 1) -- 折本币年日均余额
       ,(case when substr('${batch_date}',5,4) in ('0101','0401','0701','1001') then (nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0))*nvl(trim(t5.convt_cny_exch_rat),1) else coalesce(t6.cl_curr_s_acm_bal,t6_1.cl_curr_s_acm_bal, 0.0) + (nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0))*nvl(trim(t5.convt_cny_exch_rat),1) end) / (to_date('${batch_date}', 'yyyymmdd') - trunc(to_date('${batch_date}', 'yyyymmdd'), 'q') + 1) -- 折本币季日均余额
       ,(case when substr('${batch_date}',7,2) = '01' then (nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0))*nvl(trim(t5.convt_cny_exch_rat),1) else coalesce(t6.cl_curr_m_acm_bal,t6_1.cl_curr_m_acm_bal, 0.0) + (nvl(t10.nomal_pric, 0) + nvl(t10.log_pric, 0))*nvl(trim(t5.convt_cny_exch_rat),1) end) / to_number(substr('${batch_date}', 7, 2)) -- 折本币月日均余额	   	   	   	   	             	   
       ...
       ,(case when substr('${batch_date}',5,4) = '0101' then nvl(t1.nomal_pric,0)  else coalesce(t6.nomal_pric_y_acm_bal,t6_1.nomal_pric_y_acm_bal,0.0) + nvl(t1.nomal_pric,0)  end ) / (to_date('${batch_date}', 'yyyymmdd') - trunc(to_date('${batch_date}', 'yyyymmdd'), 'y') + 1)                                   -- 正常本金年日均余额
       ,(case when substr('${batch_date}',5,4) in ('0101','0401','0701','1001') then nvl(t1.nomal_pric,0)  else coalesce(t6.nomal_pric_s_acm_bal,t6_1.nomal_pric_s_acm_bal,0.0) + nvl(t1.nomal_pric,0)  end) / (to_date('${batch_date}', 'yyyymmdd') - trunc(to_date('${batch_date}', 'yyyymmdd'), 'q') + 1)            -- 正常本金季日均余额
       ,(case when substr('${batch_date}',7,2) = '01' then nvl(t1.nomal_pric,0)  else coalesce(t6.nomal_pric_m_acm_bal,t6_1.nomal_pric_m_acm_bal,0.0) + nvl(t1.nomal_pric,0)  end) / to_number(substr('${batch_date}', 7, 2))                                                                                           -- 正常本金月日均余额
       ...
       ,case when substr('${batch_date}',5,4) in ('0101','0401','0701','1001') then nvl(t1.ovdue_pric,0)  else coalesce(t6.ovdue_pric_cl_curr_s_acm_bal,t6_1.ovdue_pric_cl_curr_s_acm_bal,0.0) + nvl(t1.ovdue_pric,0)*nvl(t5.convt_cny_exch_rat, 1)  end                                                                        -- 逾期本金折本币季累计余额
       ...
       ,0                               -- 呆账本金本币季日均余额
       ,0                               -- 呆账本金本币月日均余额
       ,case when substr('${batch_date}',5,4) = '0101' then nvl(t1.in_bs_int,0)  else coalesce(t6.in_bs_int_y_acm_bal,t6_1.in_bs_int_y_acm_bal,0.0) + nvl(t1.in_bs_int,0) end                             -- 表内利息年累计余额
       ,case when substr('${batch_date}',5,4) in ('0101','0401','0701','1001') then nvl(t1.in_bs_int,0) else coalesce(t6.in_bs_int_s_acm_bal,t6_1.in_bs_int_s_acm_bal,0.0) + nvl(t1.in_bs_int,0) end      -- 表内利息季累计余额
       ,case when substr('${batch_date}',7,2) = '01' then nvl(t1.in_bs_int,0)  else coalesce(t6.in_bs_int_m_acm_bal,t6_1.in_bs_int_m_acm_bal,0.0) + nvl(t1.in_bs_int,0) end                               -- 表内利息月累计余额
       ,case when substr('${batch_date}',5,4) = '0101' then nvl(t1.in_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1)  else nvl(t1.in_bs_int,0) + nvl(t1.in_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1) end                                    -- 表内利息折本币年累计余额
       ,case when substr('${batch_date}',5,4) in ('0101','0401','0701','1001') then nvl(t1.in_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1) else coalesce(t6.in_bs_int_cl_curr_s_acm_bal,t6_1.in_bs_int_cl_curr_s_acm_bal,0.0) + nvl(t1.in_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1) end             -- 表内利息折本币季累计余额
       ,case when substr('${batch_date}',7,2) = '01' then nvl(t1.in_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1)  else coalesce(t6.in_bs_int_cl_curr_m_acm_bal,t6_1.in_bs_int_cl_curr_m_acm_bal,0.0) + nvl(t1.in_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1) end                                      -- 表内利息折本币月累计余额
       ,(case when substr('${batch_date}',5,4) = '0101' then nvl(t1.in_bs_int,0) else coalesce(t6.in_bs_int_y_avg_bal,t6_1.in_bs_int_y_avg_bal,0.0) + nvl(t1.in_bs_int,0) end ) / (to_date('${batch_date}', 'yyyymmdd') - trunc(to_date('${batch_date}', 'yyyymmdd'), 'y') + 1)                          -- 表内利息年日均余额
       ,(case when substr('${batch_date}',5,4) in ('0101','0401','0701','1001') then nvl(t1.in_bs_int,0)  else coalesce(t6.in_bs_int_q_avg_bal,t6_1.in_bs_int_q_avg_bal,0.0) + nvl(t1.in_bs_int,0) end) / (to_date('${batch_date}', 'yyyymmdd') - trunc(to_date('${batch_date}', 'yyyymmdd'), 'q') + 1)  -- 表内利息季日均余额
       ,(case when substr('${batch_date}',7,2) = '01' then nvl(t1.in_bs_int,0) else coalesce(t6.in_bs_int_m_avg_bal,t6_1.in_bs_int_m_avg_bal,0.0) + nvl(t1.in_bs_int,0) end) / to_number(substr('${batch_date}', 7, 2))                                                                                  -- 表内利息月日均余额
       ,(case when substr('${batch_date}',5,4) = '0101' then nvl(t1.in_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1)  else coalesce(t6.in_bs_int_dc_y_avg_bal,t6_1.in_bs_int_dc_y_avg_bal,0.0) + nvl(t1.in_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1) end ) / (to_date('${batch_date}', 'yyyymmdd') - trunc(to_date('${batch_date}', 'yyyymmdd'), 'y') + 1)                         -- 表内利息折本币年日均余额
       ,(case when substr('${batch_date}',5,4) in ('0101','0401','0701','1001') then nvl(t1.in_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1) else coalesce(t6.in_bs_int_dc_q_avg_bal,t6_1.in_bs_int_dc_q_avg_bal,0.0) + nvl(t1.in_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1)  end) / (to_date('${batch_date}', 'yyyymmdd') - trunc(to_date('${batch_date}', 'yyyymmdd'), 'q') + 1)  -- 表内利息折本币季日均余额
       ,(case when substr('${batch_date}',7,2) = '01' then nvl(t1.in_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1)  else coalesce(t6.in_bs_int_dc_m_avg_bal,t6_1.in_bs_int_dc_m_avg_bal,0.0) + nvl(t1.in_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1) end) / to_number(substr('${batch_date}', 7, 2))                                                                                 -- 表内利息折本币月日均余额
       ,case when substr('${batch_date}',5,4) = '0101' then nvl(t1.off_bs_int,0) else coalesce(t6.off_bs_int_y_acm_bal,t6_1.off_bs_int_y_acm_bal,0.0) + nvl(t1.off_bs_int,0) end                             -- 表外利息年累计余额
       ,case when substr('${batch_date}',5,4) in ('0101','0401','0701','1001') then nvl(t1.off_bs_int,0) else coalesce(t6.off_bs_int_s_acm_bal,t6_1.off_bs_int_s_acm_bal,0.0) + nvl(t1.off_bs_int,0) end     -- 表外利息季累计余额
       ,case when substr('${batch_date}',7,2) = '01' then nvl(t1.off_bs_int,0) else coalesce(t6.off_bs_int_m_acm_bal,t6_1.off_bs_int_m_acm_bal,0.0) + nvl(t1.off_bs_int,0) end                               -- 表外利息月累计余额
       ,case when substr('${batch_date}',5,4) = '0101' then nvl(t1.off_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1)  else coalesce(t6.off_bs_int_cl_curr_y_acm_bal,t6_1.off_bs_int_cl_curr_y_acm_bal,0.0) + nvl(t1.off_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1) end                               -- 表外利息折本币年累计余额
	     ,case when substr('${batch_date}',5,4) in ('0101','0401','0701','1001') then nvl(t1.off_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1) else coalesce(t6.off_bs_int_cl_curr_s_acm_bal,t6_1.off_bs_int_cl_curr_s_acm_bal,0.0) + nvl(t1.off_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1) end        -- 表外利息折本币季累计余额
       ,case when substr('${batch_date}',7,2) = '01' then nvl(t1.off_bs_int,0)  else coalesce(t6.off_bs_int_cl_curr_m_acm_bal,t6_1.off_bs_int_cl_curr_m_acm_bal,0.0) + nvl(t1.off_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1) end                                 -- 表外利息折本币月累计余额
       ,(case when substr('${batch_date}',5,4) = '0101' then nvl(t1.off_bs_int,0) else coalesce(t6.off_bs_int_y_avg_bal,t6_1.off_bs_int_y_avg_bal,0.0) + nvl(t1.off_bs_int,0) end ) / (to_date('${batch_date}', 'yyyymmdd') - trunc(to_date('${batch_date}', 'yyyymmdd'), 'y') + 1)                        -- 表外利息年日均余额
       ,(case when substr('${batch_date}',5,4) in ('0101','0401','0701','1001') then nvl(t1.off_bs_int,0) else coalesce(t6.off_bs_int_q_avg_bal,t6_1.off_bs_int_q_avg_bal,0.0) + nvl(t1.off_bs_int,0) end) / (to_date('${batch_date}', 'yyyymmdd') - trunc(to_date('${batch_date}', 'yyyymmdd'), 'q') + 1)  -- 表外利息季日均余额
       ,(case when substr('${batch_date}',7,2) = '01' then nvl(t1.off_bs_int,0) else coalesce(t6.off_bs_int_m_avg_bal,t6_1.off_bs_int_m_avg_bal,0.0) + nvl(t1.off_bs_int,0) end) / to_number(substr('${batch_date}', 7, 2))                                                                                 -- 表外利息月日均余额
       ,(case when substr('${batch_date}',5,4) = '0101' then nvl(t1.off_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1)  else coalesce(t6.off_bs_int_dc_y_avg_bal,t6_1.off_bs_int_dc_y_avg_bal,0.0) + nvl(t1.off_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1)  end ) / (to_date('${batch_date}', 'yyyymmdd') - trunc(to_date('${batch_date}', 'yyyymmdd'), 'y') + 1)                         -- 表外利息折本币年日均余额
       ,(case when substr('${batch_date}',5,4) in ('0101','0401','0701','1001') then nvl(t1.off_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1)  else coalesce(t6.off_bs_int_dc_q_avg_bal,t6_1.off_bs_int_dc_q_avg_bal,0.0) + nvl(t1.off_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1)  end) / (to_date('${batch_date}', 'yyyymmdd') - trunc(to_date('${batch_date}', 'yyyymmdd'), 'q') + 1)  -- 表外利息折本币季日均余额
       ,(case when substr('${batch_date}',7,2) = '01' then nvl(t1.off_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1)  else coalesce(t6.off_bs_int_dc_m_avg_bal,t6_1.off_bs_int_dc_m_avg_bal,0.0) + nvl(t1.off_bs_int,0)*nvl(t5.convt_cny_exch_rat, 1)  end) / to_number(substr('${batch_date}', 7, 2))                                                                                 -- 表外利息折本币月日均余额
       ,t1.job_cd                             -- 任务代码
       ,to_timestamp('${batch_timestamp}', 'yyyy-mm-dd hh24:mi:ss.ff6')  -- 数据处理时间
  from ${icl_schema}.tmp_cmm_corp_loan_acct_bal_info_01 t1
  left join ${iml_schema}.ref_cny_fori_exch_mdl_p_h t5
    on t1.curr_cd = t5.curr_cd
   and t5.start_dt <= to_date('${batch_date}','yyyymmdd')
   and t5.end_dt > to_date('${batch_date}','yyyymmdd')
   and t5.job_cd = 'ncbsf1'
  left join ${icl_schema}.cmm_corp_loan_acct_bal_info t6
  ...
  left join ${icl_schema}.cmm_corp_loan_acct_bal_info t6_1
...
alter table ${icl_schema}.cmm_corp_loan_acct_bal_info exchange partition p_${batch_date} with table ${icl_schema}.cmm_corp_loan_acct_bal_info_ex;
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${icl_schema}',tabname => 'cmm_corp_loan_acct_bal_info', partname => 'p_${batch_date}', granularity => 'PARTITION', degree => 8, cascade => true);
