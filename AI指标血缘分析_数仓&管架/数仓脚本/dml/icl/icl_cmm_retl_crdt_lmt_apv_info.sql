/*
purpose:    共性加工层-零售授信额度审批信息：包括所有零售信贷类业务的业务申请和额度审批信息，包含零售表内贷款业务、表外业务、同业表内业务、同业表外业务等业务。数据来源于综合信贷管理系统ICMS。
author:     sunline
usage:      python $ETL_HOME/script/main.py yyyymmdd icl_cmm_retl_crdt_lmt_apv_info
createdate: 20210121
logs:       20221128 温旺清 新增模型
            20221202 温旺清 主表增加过滤条件t1.loan_distr_type_cd<>'0201'
*/
set timing on
-- 0.1 alter parallel
...
--alter table ${icl_schema}.cmm_retl_crdt_lmt_apv_info drop partition p_${retain_day};
alter table ${icl_schema}.cmm_retl_crdt_lmt_apv_info add partition p_${batch_date} values (to_date('${batch_date}','yyyymmdd'));

-- 1.2 drop temporary table cmm_retl_crdt_lmt_apv_info_ex
whenever sqlerror continue none ;
drop table ${icl_schema}.cmm_retl_crdt_lmt_apv_info_ex purge;

-- 2.1 create temporary table cmm_retl_crdt_lmt_apv_info_ex
whenever sqlerror exit sql.sqlcode;
create table ${icl_schema}.cmm_retl_crdt_lmt_apv_info_ex
nologging
compress ${option_switch} for query high
as
select * from ${icl_schema}.cmm_retl_crdt_lmt_apv_info where 0=1;

-- 2.2 insert into data to temporary table cmm_retl_crdt_lmt_apv_info_ex

--第一组（共一组）零售申请信息
whenever sqlerror exit sql.sqlcode;
insert /*+ append */ into ${icl_schema}.cmm_retl_crdt_lmt_apv_info_ex(
       etl_dt                        -- 数据日期
       ,lp_id                        -- 法人编号
...
(select distinct apv_flow_num 
 ...
 where t1.start_dt <= to_date('${batch_date}','yyyymmdd')
   ...
   and t1.loan_distr_type_cd<>'0201';
commit;

-- 2.3 exchage ex table and target table
alter table ${icl_schema}.cmm_retl_crdt_lmt_apv_info exchange partition p_${batch_date} with table ${icl_schema}.cmm_retl_crdt_lmt_apv_info_ex;
...
whenever sqlerror exit sql.sqlcode;
exec dbms_stats.gather_table_stats(ownname => '${icl_schema}',tabname => 'cmm_retl_crdt_lmt_apv_info', partname => 'p_${batch_date}', estimate_percent => 10, method_opt=>'for all columns size 1',no_invalidate=>false,granularity=>'partition',cascade => true,force=>true,degree => 8);