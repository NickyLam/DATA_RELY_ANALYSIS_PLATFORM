...
Logs:       20241118 谢  宁 增加完成状态表
            ...
            20260306 陈  凭 调整第二组微业贷【客户编号】取值逻辑
...
insert /*+ append */ into ${icl_schema}.cmm_unite_wl_guar_cont_info_ex(
 ...
 select
       ...
       ,t1.guar_guar_form_cd                    -- 保证种类代码
       ,t1.guar_cont_status_cd                  -- 状态代码
       ,t1.curr_cd                              -- 币种代码
       ,t1.agt_sign_dt                          -- 签订日期
       ,t1.effect_dt                            -- 生效日期
       ,t1.invalid_dt                           -- 到期日期
       ,t1.vouchee_cust_id                      -- 客户编号
       ,t1.guartor_cust_id                      -- 担保人客户编号
       ...
       ,t1.guar_org_name                        -- 担保机构名称
       ,t1.guar_item_promis_id                  -- 担保事项承诺书编号
       ,t1.rgst_org_id                          -- 登记机构编号
       ,t1.rgst_teller_id                       -- 登记人编号
       ,t1.rgst_dt                              -- 登记日期
       ,t1.job_cd                               -- 任务代码
       ,to_timestamp('${batch_timestamp}','yyyy-mm-dd hh24:mi:ss.ff6')  -- etl处理时间戳
 from ${iml_schema}.agt_wsd_guar_cont_info_h t1
...
insert /*+ append */ into ${icl_schema}.cmm_unite_wl_guar_cont_info_ex(
 ...
 select
       ...
       ,t1.guar_cont_type_cd                          --担保合同类型代码
       ,t1.guar_way_cd                                --担保方式代码
       ,decode (t1.guar_way_cd,'A','40','B','30','C','20','D','10','-')       --保证种类代码
...
insert /*+ append */ into ${icl_schema}.cmm_unite_wl_guar_cont_info_ex(
 ...
 select
       ...
       ,t1.guar_way_cd                                                                --担保方式代码
       ,decode (t1.guar_way_cd,'A','40','B','30','C','20','D','10','-')           --保证种类代码
       ,decode (t1.guar_cont_status_cd,'110','2','1')                              --状态代码
       ,t1.curr_cd                                                                    --币种代码
       ,t1.sign_dt                                                                    --签订日期
       ,t1.effect_dt                                                                  --生效日期
       ,t1.exp_dt                                                                     --到期日期
       ,t1.guar_cust_id                                                               --客户编号
       ,t1.guartor_id                                                                 --担保人客户编号
       ,t1.guartor_name                                                               --担保人名称
       ,t1.guartor_cert_type_cd                                                       --担保人证件类型代码
       ,t1.guartor_cert_no                                                            --担保人证件号码
       ...
       ,''                                                                            --担保机构名称
       ,''                                                                            --担保事项承诺书编号
       ,t1.rgst_teller_id                                                             --登记机构编号
       ,t1.rgst_org_id                                                                --登记人编号
       ,t1.rgst_dt                                                                    --登记日期
       ,'icms_lh'                                                                     --任务代码
       ,to_timestamp('${batch_timestamp}','yyyy-mm-dd hh24:mi:ss.ff6')                --etl处理时间戳
 from ${iml_schema}.agt_lhwd_guar_cont_info_h t1
 where t1.start_dt <= to_date('${batch_date}', 'yyyymmdd')
   and t1.end_dt > to_date('${batch_date}', 'yyyymmdd')
   and t1.job_cd ='icmsf1'
;
...
delete from ${icl_schema}.cmm_icl_batch_jnl  where etl_dt = to_date('${batch_date}', 'yyyymmdd') and tab_name = 'cmm_unite_wl_guar_cont_info';
commit;
whenever sqlerror exit sql.sqlcode;
