CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_ICL_CMM_COL_INFO(I_P_DATE IN INTEGER,
                                                   O_ERRCODE OUT VARCHAR2
                                                   )
  /**************************************************************************
  *  程序名称：ETL_O_ICL_CMM_COL_INFO
  *  功能描述：押品信息
  *  创建日期：20220525
  *  开发人员：梅炜
...
AS
  ...
  V_PROC_NAME VARCHAR2(100) := 'ETL_O_ICL_CMM_COL_INFO'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  DELETE FROM RRP_MDL.O_ICL_CMM_COL_INFO T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
