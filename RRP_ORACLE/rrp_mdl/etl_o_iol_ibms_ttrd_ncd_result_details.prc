CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IOL_IBMS_TTRD_NCD_RESULT_DETAILS(I_P_DATE IN INTEGER, --跑批日期
                                                 O_ERRCODE  OUT VARCHAR2 --错误代码
                                                 )
 /*******************************************************************
  **存储过程详细说明： 同业存单发行结果明细表
  **存储过程名称：    ETL_O_IOL_IBMS_TTRD_NCD_RESULT_DETAILS
  **存储过程创建日期：20220707
  **存储过程创建人：  赖海强
  **输入参数：        I_P_DATE
  **输出参数：        O_ERRCODE
 ...
 AS
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IOL_IBMS_TTRD_NCD_RESULT_DETAILS'; -- 程序名称
 ...
 -- 支持重跑 --
  ...
  EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_IOL_IBMS_TTRD_NCD_RESULT_DETAILS';
  ...
  V_STARTTIME := SYSDATE;
  INSERT /*+APPEND*/ INTO RRP_MDL.O_IOL_IBMS_TTRD_NCD_RESULT_DETAILS NOLOGGING
    (  SEQ_ID	                --序列号
      ,SYSORDID	              --交易单号
      ,REF_SYSORDID	          --子交易单号
      ,I_CODE	                --存单代码
    ...
    SELECT /*+PARALLEL*/
      ...
      ,END_DT	                --结束时间
      ,ID_MARK	              --增删标志
   FROM IOL.V_IBMS_TTRD_NCD_RESULT_DETAILS   --同业存单发行结果明细表_视图
...
END ETL_O_IOL_IBMS_TTRD_NCD_RESULT_DETAILS;
/
