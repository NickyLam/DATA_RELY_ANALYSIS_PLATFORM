CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_GUA_COLL_VAL_SPLT(I_P_DATE IN INTEGER,
                                                    O_ERRCODE OUT VARCHAR2
                                                   )
/**************************************************************************
 *  程序名称：ETL_M_GUA_COLL_VAL_SPLT
 *  功能描述：抵质押物价值拆分表
 *  创建日期：20220830
 *  开发人员：梅炜
 *  来源表：
...
AS
  ...
  V_TAB_NAME  VARCHAR2(100) := 'M_GUA_COLL_VAL_SPLT'; --表名
  V_PROC_NAME VARCHAR2(100) := 'ETL_M_GUA_COLL_VAL_SPLT'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM RRP_MDL.M_GUA_COLL_VAL_SPLT T WHERE T.DATA_DT = V_P_DATE;--普通表的重跑处理
  /*EXECUTE IMMEDIATE ('ALTER TABLE '||'M_GUA_COLL_VAL_SPLT'||' TRUNCATE PARTITION '||'写上分区名');--分区表的重跑处理*/
  ...
  WITH TMP1 AS (
    SELECT DATA_DT,COLL_ID,RCPT_ID,COUNT(1)
      FROM RRP_MDL.M_GUA_COLL_VAL_SPLT T
