CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IOL_CTMS_TBS_V_IAMDEALS(I_P_DATE IN INTEGER,
                              O_ERRCODE OUT VARCHAR2
                               )
  /**************************************************************************
  *  程序名称：ETL_O_IOL_CTMS_TBS_V_IAMDEALS
  *  功能描述：拆借交易
  *  创建日期：20220525
  *  开发人员：梅炜
  *  来源表：
  ...
  *************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IOL_CTMS_TBS_V_IAMDEALS'; -- 程序名称
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_IOL_CTMS_TBS_V_IAMDEALS';
  ...
  INSERT /*+APPEND*/ INTO RRP_MDL.O_IOL_CTMS_TBS_V_IAMDEALS NOLOGGING
    (  DEAL_ID                  --引用表ID
      ...
      ,END_DT                   --结束时间
      ,ID_MARK                  --增删标志
      ,TRADE_TIME	              --交易时间	--ADD BY YJY 20250919
     )
  SELECT /*+PARALLEL*/
       DEAL_ID                  --引用表ID
      ,DEAL_TABLENAME           --引用表名
  ...
  END ETL_O_IOL_CTMS_TBS_V_IAMDEALS;
/
