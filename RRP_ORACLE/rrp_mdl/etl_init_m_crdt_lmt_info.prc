CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_M_CRDT_LMT_INFO(I_P_DATE IN INTEGER,
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM RRP_MDL.M_CRDT_LMT_INFO T WHERE T.DATA_DT = V_P_DATE;--普通表的重跑处理
  --DELETE FROM RRP_MDL.M_CRDT_LMT_SUB T WHERE T.DATA_DT = V_P_DATE;
  --用到的临时表
  ...
  SELECT /*+USE_HASH(B1,A,C,D,O)*/
         ...
         CASE WHEN B.WRT_OFF_FLG = '1' THEN 0
              WHEN B.WRT_OFF_FLG <> '1' THEN
              CASE WHEN B.SUBJ_ID LIKE '1313%' THEN 0
   ...
   INNER JOIN ICL.V_CMM_CORP_LOAN_ACCT_INFO B     --对公贷款账户信息表
     ...
     AND B.ETL_DT = V_DATE
    LEFT JOIN ICL.V_CMM_CORP_LOAN_CONT_INFO C  --对公贷款合同信息表
      ON C.CONT_ID = A.CONT_ID
     AND C.ETL_DT = V_DATE
    LEFT JOIN ICL.V_CMM_CORP_LOAN_CONT_INFO D  --对公贷款合同信息表 取额度合同
      ON D.CONT_ID = C.LMT_CONT_ID
     AND D.ETL_DT = V_DATE
    LEFT JOIN IOL.V_IFRS_VAL_RPT_TRADE AA    --估值报告表
      ON A.DUBIL_ID = AA.V_TRADE_NO
     AND AA.ETL_DT = CASE WHEN SUBSTR(TO_CHAR(D_DATA_DT,'YYYYMMDD'),5) = '1231' AND SUBSTR(V_P_DATE,5) = '1231'
                          THEN V_DATE
                          ELSE V_DATE - 1 END
  ...
  SELECT /*+USE_HASH(B1,A,C,D,O)*/
   ...
   INNER JOIN ICL.V_CMM_CORP_LOAN_DUBIL_INFO B --对公贷款借据信息
    ...
    LEFT JOIN ICL.V_CMM_CORP_LOAN_CONT_INFO D  --对公贷款合同信息表 取额度合同
      ON D.CONT_ID = C.LMT_CONT_ID
     AND D.ETL_DT = V_DATE
    LEFT JOIN IOL.V_IFRS_VAL_RPT_TRADE O --估值报告表
      ON O.V_TRADE_NO = A.BILL_NUM
     AND O.V_BUSINESSTYPE = B.STD_PROD_ID
     AND O.ETL_DT = CASE WHEN SUBSTR(TO_CHAR(D_DATA_DT,'YYYYMMDD'),5) = '1231' AND SUBSTR(V_P_DATE,5) = '1231'
                          THEN V_DATE
                          ELSE V_DATE - 1 END/*TO_DATE(V_P_DATE,'YYYYMMDD')*/
  ...
  SELECT /*+USE_HASH(A,C,F2,D,DD,O)*/
   ...
   INNER JOIN ICL.V_CMM_CORP_LOAN_DUBIL_INFO B --对公贷款借据信息
     ...
     AND B.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN ICL.V_CMM_CORP_LOAN_CONT_INFO D  --对公贷款合同信息表
      ON D.CONT_ID = B.CONT_ID
     AND NVL(TRIM(D.CRDT_TYPE_CD),'02') = '02'
     AND D.ETL_DT = V_DATE
    LEFT JOIN ICL.V_CMM_BILL_CENTER_INFO J --票据中心信息
      ON J.BILL_ID = A.BILL_ID
     AND J.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN ICL.V_CMM_CORP_LOAN_CONT_INFO E  --对公贷款合同信息表
      ON E.CONT_ID = D.LMT_CONT_ID
     AND E.ETL_DT = V_DATE
    LEFT JOIN IOL.V_IFRS_VAL_RPT_TRADE O --估值报告表 关联估值表取 转 公允价值变动
      ON O.V_TRADE_NO = B.BILL_NUM
     AND O.ETL_DT = CASE WHEN SUBSTR(TO_CHAR(D_DATA_DT,'YYYYMMDD'),5) = '1231' AND SUBSTR(V_P_DATE,5) = '1231'
                         THEN V_DATE
                         ELSE V_DATE - 1 END/*V_DATE*/
  ...
  SELECT /*+USE_HASH(A,C,D)*/
    ...
    FROM ICL.V_CMM_CORP_LOAN_CONT_INFO E --对公贷款合同信息
   INNER JOIN ICL.V_CMM_CORP_LOAN_DUBIL_INFO B --对公贷款借据信息
      ON B.CONT_ID = E.CONT_ID
     AND B.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   INNER JOIN ICL.V_CMM_LC_ACCT_INFO A --信用证账户信息
  ...
  SELECT /*+USE_HASH(A,B,C,D)*/
   ...
   INNER JOIN ICL.V_CMM_CORP_LOAN_DUBIL_INFO B --对公贷款借据信息
      ON B.CONT_ID = E.CONT_ID
     AND B.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   INNER JOIN ICL.V_CMM_LOG_ACCT_INFO A --保函账户信息
  ...
  SELECT /*+USE_HASH(A,B,C,D)*/
     ...
     FROM ICL.V_CMM_CORP_LOAN_DUBIL_INFO A --对公贷款借据信息
    INNER JOIN ICL.V_CMM_BA_ACCT_INFO B --银承账户信息
       ON B.BILL_NUM = A.BILL_NUM
      AND B.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
     LEFT JOIN ICL.V_CMM_DEP_CUST_ACCT_INFO C  --存款客户账户信息表
  ...
  SELECT /*+USE_HASH(A,B,C,D)*/
    ...
    LEFT JOIN ICL.V_CMM_CORP_LOAN_CONT_INFO D  --对公贷款合同信息表
      ON D.CONT_ID = A.CONT_ID
     AND D.ETL_DT = V_DATE
   WHERE NVL(A.DISTR_DT,DATE'0001-01-01') = TO_DATE(I_P_DATE,'YYYYMMDD') --发放日=当天
  ...
  SELECT /*+USE_HASH(A,B,C,D)*/
    ...
    FROM ICL.V_CMM_RETL_LOAN_DUBIL_INFO B --零售贷款借据信息
   INNER JOIN ICL.V_CMM_RETL_LOAN_ACCT_INFO A  --零售贷款账户信息
      ON A.DUBIL_NUM = B.DUBIL_ID
     AND A.ETL_DT = V_DATE
    LEFT JOIN ICL.V_CMM_RETL_LOAN_CONT_INFO C --零售合同表
      ON C.CONT_ID = B.CONT_ID
     AND C.ETL_DT  = V_DATE
  ...
  SELECT V_P_DATE                        AS DATA_DT,                    --数据日期
    ...
    --FROM RRP_MDL. M_CRDT_LMT_INFO_TEMP06 T --余额之和
    FROM (--modify by tangan at 20230214 对公已用额度取借据余额之和
      SELECT A.DUBIL_ID                        AS RCPT_ID,             --借据号
             A.CONT_ID                         AS CONT_ID,              --合同号
             TRIM(C.LMT_CONT_ID)               AS LMT_CONT_ID,          --授信合同号
             A.STD_PROD_ID                     AS LOAN_PROD_ID,         --产品号
             /*A.DUBIL_AMT                       AS LOAN_AMT,             --借据金额
             A.DUBIL_BAL                       AS LOAN_BAL              --借据余额*/
  ...
  SELECT  /*+USE_HASH(A,C,D,E,F,TA,B)*/
         ...
         /*,CASE WHEN C.CRDT_CUST_TYPE_CD = '2' THEN '05' --同业客户
               WHEN C.CRDT_CUST_TYPE_CD = '5' THEN '02' --集团客户
               WHEN C.CRDT_CUST_TYPE_CD = '3' OR C.CUST_TYPE_CD='2' THEN '01' --对公客户
               ELSE '99' --其他
           END                                            AS CRDT_SUBJ_TYP               --15授信主体类型 mod by hulj 20230220*/
    ...
    FROM ICL.V_CMM_CORP_LOAN_CONT_INFO A --对公贷款合同信息
   ...
   INNER JOIN ICL.V_CMM_CORP_CUST_BASIC_INFO C--对公客户基本信息
      /*迁移数据会有部分客户号不在ECIF系统中，直接剔除*/
      ON C.CUST_ID = A.CUST_ID
     AND C.ETL_DT = V_DATE
    LEFT JOIN ICL.V_CMM_CORP_CRDT_LMT_APV_INFO D --对公授信额度合同审批信息
      ON D.CRDT_LMT_APV_FLOW_NUM = A.APV_FLOW_NUM
     AND D.ETL_DT = V_DATE
    LEFT JOIN ICL.V_CMM_CLERK_INFO E --行员信息表
      ON E.TELLER_ID = TRIM(A.MGMT_TELLER_ID)
     AND E.ETL_DT = V_DATE
    LEFT JOIN LMT_INTO_TEMP07 G --当天授信当天结清的
      ON G.LMT_CONT_ID = A.CONT_ID
    LEFT JOIN ICL.V_CMM_CORP_LOAN_LMT_CONT_ATTACH_INFO H  --对公贷款额度合同补充信息
      ON H.CONT_ID = A.CONT_ID
     AND H.ETL_DT = V_DATE
    LEFT JOIN ICL.V_CMM_CORP_LOAN_APPL_INFO I --对公贷款申请表
      ON I.LOAN_APPL_FLOW_NUM = D.RELA_CRDT_LMT_APV_FLOW_NUM
     AND I.ETL_DT = V_DATE
    LEFT JOIN IML.V_AGT_PASCT_TURN_CRDT_INFO_H K
  ...
  SELECT  /*+USE_HASH(A,B,C,D,E,F,TA)*/
         ...
         ,CASE WHEN C.CRDT_CUST_TYPE_CD = '3' THEN '05' --同业客户
               ...
               WHEN C.CRDT_CUST_TYPE_CD = '2' OR C.CUST_TYPE_CD LIKE '2%' THEN '01' --对公客户
               ELSE '99' --其他
           END                                               CRDT_SUBJ_TYP               --15授信主体类型 mod by hulj 20230818
         ...
         ,B.STD_PROD_ID                                      CRDT_BIZ_TYP                --21授信业务类型
         --由于行内额度管理无法区分授信业务类型该字段取额度合同标准产品号
         ...
         ,NULL                                               GROUP_CRDT_FLG              --36集团授信标志
         ,B.STD_PROD_ID                                      STD_PROD_ID                 --37标准产品编号
         ,E.APPL_WAY_CD                                      APPL_WAY_CD                 --38申请方式代码
         ,K.APPROVE_SERIAL_FLOW_NUM                          APPROVE_SERIAL_FLOW_NUM     --39被动转授信集团批复流水号
         ,B.APV_FLOW_NUM                                     APV_FLOW_NUM                --40批复流水号
         ,E.LMT_UNDER_SELLBL_PROD_ID                         LMT_UNDER_SELLBL_PROD_ID    --41可售产品
         ,F.CRDT_APV_AMT                                     APV_AMT                     --42批复金额
    FROM RRP_MDL.M_CRDT_LMT_INFO_TEMP06 A
    LEFT JOIN ICL.V_CMM_CORP_LOAN_CONT_INFO B --对公贷款合同信息
      ON B.CONT_ID = A.CONT_ID
     AND B.ETL_DT = V_DATE
    LEFT JOIN ICL.V_CMM_CORP_CUST_BASIC_INFO C--对公客户基本信息
      ON C.CUST_ID = B.CUST_ID
     AND C.ETL_DT = V_DATE
    LEFT JOIN ICL.V_CMM_CORP_LOAN_CONT_INFO D --对公贷款合同信息
      ON D.CONT_ID = A.LMT_CONT_ID
     AND D.ETL_DT = V_DATE
    LEFT JOIN ICL.V_CMM_CORP_CRDT_LMT_APV_INFO F --对公授信额度合同审批信息
      ON F.CRDT_LMT_APV_FLOW_NUM = D.APV_FLOW_NUM
     AND F.ETL_DT = V_DATE
    LEFT JOIN ICL.V_CMM_CORP_LOAN_LMT_CONT_ATTACH_INFO G  --对公贷款额度合同补充信息
      ON G.CONT_ID  = A.CONT_ID
     AND G.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN ICL.V_CMM_CORP_LOAN_APPL_INFO E --对公贷款申请信息
      ON F.RELA_CRDT_LMT_APV_FLOW_NUM = E.LOAN_APPL_FLOW_NUM
     AND E.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN IML.V_AGT_PASCT_TURN_CRDT_INFO_H K
      ON E.LOAN_APPL_FLOW_NUM = K.OBJ_ID
  ...
  SELECT  /*+USE_HASH(A B M C D)*/
    ...
    FROM ICL.V_CMM_RETL_LOAN_CRDT_LMT_INFO T1  --零售贷款授信额度信息
    LEFT JOIN ICL.V_CMM_RETL_CRDT_LMT_APV_INFO T3 --零售授信额度审批信息
    ...
    LEFT JOIN ICL.V_CMM_RETL_LOAN_APPL_INFO TE --零售贷款申请信息 --modify by tangan at 20230129
      ON TE.LOAN_APPL_FLOW_NUM = T1.LMT_APPL_FLOW_NUM
     AND TE.ETL_DT = V_DATE
   WHERE T1.PROD_ID NOT IN ('202020200007','201010300005') --剔除新心金融小微贷    --modify by yjy 20240417 剔除201010300005-新兴贷
  ...
  SELECT  /*+USE_HASH(A B M C D)*/
    ...
    FROM ICL.V_CMM_RETL_LOAN_CONT_INFO T1  --零售贷款合同信息
   ...
   WHERE TRIM(T1.LMT_CONT_ID) IS NULL --根据‘关联合同编号’为空区分单笔单批合同
     AND T1.PROD_ID NOT IN ('202020200007','201010300005') --剔除新心金融小微贷  --modify by yjy 20240417 剔除201010300005-新兴贷
     AND T1.ETL_DT = V_DATE;
  ...
  WITH LOAN_APPL AS (
    SELECT T.*
      FROM (SELECT A.*,ROW_NUMBER() OVER(PARTITION BY A.CUST_ID ORDER BY A.APPL_AMT DESC,A.LOAN_APPL_FLOW_NUM DESC) AS RN --按大的一笔授信申请取数
              FROM ICL.V_CMM_RETL_LOAN_APPL_INFO A -- 贷款申请信息
             WHERE A.PROD_ID = '202020200007'
               AND A.ETL_DT = V_DATE) T
     WHERE T.RN = 1),
  TMP_DUBIL AS (
    SELECT A.CUST_ID
  ...
  SELECT  V_P_DATE                               AS DATA_DT                      --01数据日期
   ...
   WHERE A.CURRT_BAL > 0  -- 余额大于0
     AND A.WRT_OFF_FLG <> '1' --不含核销
     AND B.STD_PROD_ID IN ('201010300005') --个人消费信用贷（新兴贷2.0）
     AND B.ETL_DT = V_DATE;
  ...
  V_STEP := V_STEP + 1;
  V_STEP_DESC := '插入授信额度主表-联合网贷-非网商贷'; --17
  V_STARTTIME := SYSDATE;
  INSERT INTO RRP_MDL.M_CRDT_LMT_INFO_TEMP02
    (DATA_DT                         --数据日期
    ,LGL_REP_ID                      --法人编号
  ...
  WITH CMM_UNITE_WL_LMT_INFO_QC AS (
     ...
     SELECT T.CUST_ID                            AS CUST_ID --客户号
           ...
           ,CASE
                 --WHEN T.BUS_BREED_ID IN ('202020100001','202020200004','02001006135011','02001006160048') THEN '202020100001' -- 网商贷
                 WHEN T.BUS_BREED_ID IN ('02001004165051','02001004120222','202010100001','202010100002') THEN '202010100001' --借呗
                 WHEN T.BUS_BREED_ID IN ('02001004165085','202010100004','202010100005') THEN '202010100004' --京东
                 WHEN T.BUS_BREED_ID IN ('02001004135021','202010100003') THEN '202010100003' --花呗
                 WHEN T.BUS_BREED_ID IN ('0900600100001','202010100006') THEN '202010100006' --微粒贷
       ...
       FROM ICL.V_CMM_UNITE_WL_LMT_INFO T
      WHERE T.ETL_DT = V_DATE - 1
        ...
        AND ((T.BUS_BREED_ID IN ('202010100008','202020100003') AND T.STATUS_CD = '01' )
              OR T.BUS_BREED_ID NOT IN ('202020100001','202020200004','02001006135011','02001006160048','202010100008','202020100003')
             ) --微粒贷只取审批通过 剔除网商贷 --20240105
  ...
  SELECT  V_P_DATE                               AS DATA_DT                      --数据日期
         ...
         ,SUM(NVL(A.NOMAL_PRIC,0) + NVL(A.OVDUE_PRIC,0) + NVL(A.IDLE_PRIC,0) + NVL(A.BAD_DEBT_PRIC,0))
          ...
          --授信额度根据网商贷新规,改为单笔单批，授信额度、已用额度等于放款金额
         /*,CASE WHEN NVL(A.STD_PROD_ID,TB.BUS_BREED_ID1) IN ('202020100001','202020200004','02001006135011','02001006160048')
               THEN A.DUBIL_AMT
               ELSE NVL(TB.CRDT_LMT,A.DUBIL_AMT)
           END                                   AS CRDT_LMT                     --授信额度*/
         ,NVL(TB.CRDT_LMT,A.DUBIL_AMT)           AS CRDT_LMT                     --授信额度
         ,SUM(NVL(A.NOMAL_PRIC,0) + NVL(A.OVDUE_PRIC,0) + NVL(A.IDLE_PRIC,0) + NVL(A.BAD_DEBT_PRIC,0)) AS ALDY_USE_LMT --已用额度
         ...
         ,CASE --WHEN NVL(A.STD_PROD_ID,TB.BUS_BREED_ID1) IN ('202020100001','202020200004','02001006135011','02001006160048') THEN '202020100001' -- 网商贷
               WHEN NVL(A.STD_PROD_ID,TB.BUS_BREED_ID1) IN ('02001004165051','02001004120222','202010100001','202010100002') THEN '202010100001' --借呗
               ...
               WHEN NVL(A.STD_PROD_ID,TB.BUS_BREED_ID1) IN ('202010100008','202020100003')  THEN NVL(A.STD_PROD_ID,TB.BUS_BREED_ID1) ----微粒贷新增代码
          END                                    AS BUS_BREED_ID                --业务品种编号  --ADD BY LIP 20220728
         ,'LS'||'联合网贷-非网商贷'              AS DATA_SRC_DESC               --数据来源描述  --ADD BY LIP 20220728
         ,A.STD_PROD_ID                          AS STD_PROD_ID                 --标准产品编号
    FROM ICL.V_CMM_UNITE_WL_DUBIL_INFO A --联合网贷借据信息
    LEFT JOIN ICL.V_CMM_UNITE_WL_WRT_OFF_INFO B --联合网贷核销表
   ...
   WHERE CASE WHEN A.STD_PROD_ID IN ('202010100004') AND V_P_DATE > '20240601' THEN 'N' --数仓改造，与业务确认20240531之前京东金条的已经全部结清/核销
              ...
              WHEN A.WRT_OFF_FLG = '1' AND (B.FIR_WRT_OFF_DT = TO_DATE('00010101','YYYYMMDD')
                   OR NVL(B.FIR_WRT_OFF_DT,TO_DATE(V_P_DATE,'YYYYMMDD')) >= V_MONTH_START_DATE - 1)
              THEN 'Y' --核销了且核销日期大于月初
         ...
         /*(((LEAST(CASE WHEN A.PAYOFF_DT = TO_DATE('00010101','YYYYMMDD') THEN TO_DATE(V_P_DATE,'YYYYMMDD') ELSE A.PAYOFF_DT END,
                     CASE WHEN A.LAST_REPAY_DT = TO_DATE('00010101','YYYYMMDD') AND A.LAST_REPAY_DT >= A.DISTR_DT
               ...
               OR NVL(A.IN_BS_INT,0) + NVL(A.CURRT_BAL,0) > 0 OR A.WRT_OFF_FLG = '1')
               AND (NVL(B.FIR_WRT_OFF_DT,TO_DATE(V_P_DATE,'YYYYMMDD')) >= V_MONTH_START_DATE - 1 
                   OR B.FIR_WRT_OFF_DT = TO_DATE('00010101','YYYYMMDD')))   --MODIBY BY YJY 20240520*/
         /*((NVL(A.NOMAL_PRIC,0) + NVL(A.OVDUE_PRIC,0) + NVL(A.IDLE_PRIC,0) + NVL(A.BAD_DEBT_PRIC,0) > 0)--取余额大于0的借据
  ...
  --网商贷根据资本新规，调整为单笔单批
  V_STEP := V_STEP + 1;
  V_STEP_DESC := '插入授信额度主表-联合网贷-网商贷'; --17
  V_STARTTIME := SYSDATE;
  INSERT INTO RRP_MDL.M_CRDT_LMT_INFO_TEMP02
    (DATA_DT                         --数据日期
    ,LGL_REP_ID                      --法人编号
  ...
  SELECT  V_P_DATE                               AS DATA_DT                      --数据日期
         ...
         ,A.CURR_CD                              AS CUR                          --币种
         ,'04'                                   AS CRDT_SUBJ_TYP                --授信主体类型 04个人客户授信
         ,MIN(CASE WHEN TO_CHAR(A.DISTR_DT,'YYYYMMDD') NOT IN ('00010101','20991231') THEN TO_CHAR(A.DISTR_DT,'YYYYMMDD')
              END)                               AS EFF_DT                       --生效日期
         ,CASE WHEN NVL(A.NOMAL_PRIC,0) + NVL(A.OVDUE_PRIC,0) + NVL(A.IDLE_PRIC,0) + NVL(A.BAD_DEBT_PRIC,0) > 0
                    AND A.WRT_OFF_FLG <> '1' THEN 'Y'
               WHEN A.WRT_OFF_FLG = '1' AND B.FIR_WRT_OFF_DT >= V_MONTH_START_DATE-1 THEN 'N'
         ...
         ,MIN(CASE WHEN TO_CHAR(A.DISTR_DT,'YYYYMMDD') NOT IN ('00010101','20991231') THEN TO_CHAR(A.DISTR_DT,'YYYYMMDD')
              END)                               AS CRDT_START_DT                --授信开始日期
         ,MAX(CASE WHEN TO_CHAR(A.EXP_DT,'YYYYMMDD') NOT IN ('00010101','20991231') THEN TO_CHAR(A.EXP_DT,'YYYYMMDD')
              END)                               AS CRDT_EXP_DT                  --授信到期日期
         ...
         ,'800924'                               AS DEPT_LINE                    --部门条线 /*零售信贷部(普惠金融部)*/
         ,'联合网贷'                             AS DATA_SRC                     --数据来源
         ,CASE WHEN A.STD_PROD_ID IN ('202020100001','202020200004','02001006135011','02001006160048') THEN '202020100001' --网商贷
          END                                    AS BUS_BREED_ID                 --业务品种编号  --ADD BY LIP 20220728
  ...
  SELECT DATA_DT                         --数据日期
        ...
        /*COALESCE(TA.ORG_ID1,TB.ORG_ID1,'800') AS ORG_ID   --机构编号*/
        ,CRDT_LMT                        --授信额度
        ...
        ,T.GROUP_CRDT_FLG                --集团授信标志
        ,T.STD_PROD_ID                   --标准产品编号
        ,T.APPL_WAY_CD                   --申请方式代码
        ,T.APPROVE_SERIAL_FLOW_NUM       --被动转授信集团批复流水号
        ,T.APV_FLOW_NUM                  --批复流水号
        ,T.LMT_UNDER_SELLBL_PROD_ID      --可售产品
        ,T.APV_AMT                       --批复金额
        ,ROW_NUMBER() OVER(PARTITION BY CRDT_CONT_ID ORDER BY CRDT_STAT DESC,CRDT_APP_DT DESC NULLS LAST) RN --联合网贷去重
  ...
  WITH TMP1 AS (
     SELECT CUST_ID,TO_CHAR(START_DT,'YYYYMMDD') FIRST_CRDT_DT
       FROM ICL.V_CMM_CORP_LOAN_CONT_INFO  --对公贷款合同信息
     ...
     SELECT CUST_ID,TO_CHAR(BEGIN_DT,'YYYYMMDD') FIRST_CRDT_DT
       FROM ICL.V_CMM_RETL_LOAN_CRDT_LMT_INFO --零售贷款授信额度信息
      WHERE ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
     UNION ALL
     SELECT CUST_ID,TO_CHAR(START_DT,'YYYYMMDD') FIRST_CRDT_DT
       FROM ICL.V_CMM_RETL_LOAN_CONT_INFO -- 零售贷款合同信息
      WHERE ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
     UNION ALL
     SELECT CUST_ID,TO_CHAR(DUBIL_OPEN_DT,'YYYYMMDD') FIRST_CRDT_DT
       FROM ICL.V_CMM_RETL_LOAN_DUBIL_INFO -- 零售贷款借据信息
      WHERE ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
     UNION ALL
     SELECT CUST_ID,TO_CHAR(BEGIN_DT,'YYYYMMDD') FIRST_CRDT_DT
       FROM ICL.V_CMM_UNITE_WL_LMT_INFO --联合网贷额度信息
  ...
  SELECT DATA_DT                         --数据日期
        ...
        ,NVL(TRIM(CRDT_CONT_NM),CRDT_CONT_ID) CRDT_CONT_NM --授信合同名称
        ,CUST_ID                         --客户编号
        ,/*COALESCE(TA.ORG_ID1,TB.ORG_ID1,'800')*/
         NVL(T.ORG_ID,'800') ORG_ID --机构编号
        ,CRDT_LMT                        --授信额度
        ,SUM(ALDY_USE_LMT) OVER(PARTITION BY CRDT_CONT_ID) ALDY_USE_LMT --已用额度
        ,SUR_CRDT_LMT                    --剩余授信额度
        ,EXP_CRDT_LMT                    --敞口授信额度
        ...
        ,T.GROUP_CRDT_FLG                --集团授信标志  ADD BY HULJ20230208
        ,T.STD_PROD_ID                   --标准产品编号
        ,T.APPL_WAY_CD                   --申请方式代码
        ,T.APPROVE_SERIAL_FLOW_NUM       --被动转授信集团批复流水号
        ,T.APV_FLOW_NUM                  --批复流水号
  ...
  WITH TMP AS (
    ...
    LEFT JOIN ICL.V_CMM_RETL_LOAN_CONT_INFO TA --零售贷款合同信息表
      ON TA.CONT_ID = A.CRDT_CONT_ID
     AND TA.ETL_DT = V_DATE
    LEFT JOIN ICL.V_CMM_RETL_LOAN_CRDT_LMT_INFO TB --零售贷款授信额度信息
  ...
  SELECT  V_P_DATE                                       AS DATA_DT                            --数据日期
         ...
         ,'CNY'                                          AS CUR                                --币种
         ,CASE WHEN SUM(NVL(A.CRDT_LMT,0)) < SUM(NVL(A.ALDY_USE_LMT,0)) THEN SUM(NVL(A.ALDY_USE_LMT,0) * D.EXRT)
               ELSE SUM(NVL(A.CRDT_LMT,0) * D.EXRT )
          END                                            AS CRDT_TOTAL_LMT                     --授信总额度
         ,SUM(NVL(A.ALDY_USE_LMT,0) * D.EXRT)            AS ALDY_USE_LMT                       --已用额度
         ,NULL                                           AS EXP_CRDT_LMT                       --敞口授信额度
         ,NULL                                           AS EXP_ALDY_USE_LMT                   --敞口已用额度
         ,CASE WHEN SUM(CASE WHEN C.LOAN_BIZ_TYP LIKE '0102%' THEN NVL(A.CRDT_LMT,0) ELSE 0 END)
               ...
               THEN SUM(CASE WHEN C.LOAN_BIZ_TYP LIKE '0102%' THEN NVL(A.ALDY_USE_LMT,0) * D.EXRT ELSE 0 END)
               ELSE SUM(CASE WHEN C.LOAN_BIZ_TYP LIKE '0102%' THEN NVL(A.CRDT_LMT,0) * D.EXRT ELSE 0 END)
          END                                            AS OPR_CRDT_TOT_AMT                   --经营授信总额
         ,SUM(CASE WHEN C.LOAN_BIZ_TYP LIKE '0102%' THEN NVL(A.ALDY_USE_LMT,0) * D.EXRT ELSE 0 END)
         ...
         ,CASE WHEN SUM(CASE WHEN C.LOAN_BIZ_TYP LIKE '0101%' THEN NVL(A.CRDT_LMT,0) ELSE 0 END)
               ...
               THEN SUM(CASE WHEN C.LOAN_BIZ_TYP LIKE '0101%' THEN NVL(A.ALDY_USE_LMT,0) * D.EXRT ELSE 0 END)
               ELSE SUM(CASE WHEN C.LOAN_BIZ_TYP LIKE '0101%' THEN NVL(A.CRDT_LMT,0) * D.EXRT ELSE 0 END)
          END                                            AS HSE_CRDT_LMT                       --住房授信额度
         ,SUM(CASE WHEN C.LOAN_BIZ_TYP LIKE '0101%' THEN NVL(A.ALDY_USE_LMT,0) * D.EXRT ELSE 0 END)
         ...
         ,CASE WHEN SUM(CASE WHEN C.LOAN_BIZ_TYP = '010301' THEN NVL(A.CRDT_LMT,0) ELSE 0 END)
               ...
               THEN SUM(CASE WHEN C.LOAN_BIZ_TYP = '010301' THEN NVL(A.ALDY_USE_LMT,0) * D.EXRT ELSE 0 END)
               ELSE SUM(CASE WHEN C.LOAN_BIZ_TYP = '010301' THEN NVL(A.CRDT_LMT,0) * D.EXRT ELSE 0 END)
          END                                            AS CAR_LOAN_CRDT_LMT                  --车贷授信额度
         ,SUM(CASE WHEN C.LOAN_BIZ_TYP = '010301' THEN NVL(A.ALDY_USE_LMT,0) * D.EXRT ELSE 0 END)
         ...
         ,CASE WHEN SUM(CASE WHEN C.LOAN_BIZ_TYP LIKE '0104%' THEN NVL(A.CRDT_LMT,0) ELSE 0 END)
               ...
               THEN SUM(CASE WHEN C.LOAN_BIZ_TYP LIKE '0104%' THEN NVL(A.ALDY_USE_LMT,0) * D.EXRT ELSE 0 END)
               ELSE SUM(CASE WHEN C.LOAN_BIZ_TYP LIKE '0104%' THEN NVL(A.CRDT_LMT,0) * D.EXRT ELSE 0 END)
          END                                            AS SL_CRDT_LMT                        --助学授信额度
         ,SUM(CASE WHEN C.LOAN_BIZ_TYP LIKE '0104%' THEN NVL(A.ALDY_USE_LMT,0) * D.EXRT ELSE 0 END)
         ...
         ,CASE WHEN SUM(CASE WHEN C.LOAN_BIZ_TYP = '010399' THEN NVL(A.CRDT_LMT,0) ELSE 0 END)
               ...
               THEN SUM(CASE WHEN C.LOAN_BIZ_TYP = '010399' THEN NVL(A.ALDY_USE_LMT,0) * D.EXRT ELSE 0 END)
               ELSE SUM(CASE WHEN C.LOAN_BIZ_TYP = '010399' THEN NVL(A.CRDT_LMT,0) * D.EXRT ELSE 0 END)
          END                                            AS OTH_CNSMP_CRDT_LMT                 --其他消费授信额度
         ,SUM(CASE WHEN C.LOAN_BIZ_TYP = '010399' THEN NVL(A.ALDY_USE_LMT,0) * D.EXRT ELSE 0 END)
  ...
  -- 去掉表的主键，通过语句判断数据是否重复--
  V_STEP := V_STEP + 1;
  V_STEP_DESC := 'M_CRDT_LMT_SUB是否重复';--22
  WITH TMP2 AS (
  ...
  V_STEP := V_STEP + 1;
  V_STEP_DESC := 'M_CRDT_LMT_INFO是否重复'; --23
  WITH TMP1 AS (
