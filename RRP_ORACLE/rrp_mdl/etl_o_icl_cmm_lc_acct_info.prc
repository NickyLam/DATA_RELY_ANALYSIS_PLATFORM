CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_ICL_CMM_LC_ACCT_INFO(I_P_DATE IN INTEGER,
                                                       O_ERRCODE OUT VARCHAR2
                                                       )
  /**************************************************************************
  *  程序名称：ETL_O_ICL_CMM_LC_ACCT_INFO
  *  功能描述：信用证账户信息
  *  创建日期：20220525
  *  开发人员：梅炜
  *  来源表：
  *  目标表： O_ICL_CMM_LC_ACCT_INFO
  *  配置表：
  *  修改情况：序号  修改日期  修改人   修改原因
  *             1    20220525  梅炜     首次创建
  *             2    20250414  YJY      新增复核柜员编号、复核柜员名称
  *             3    20251016  YJY      新增交易完成时间 
  *             4    20251107  YJY      新增受益人开户行账号、受益人开户行名称
  ************************************************************************/
AS
  -- 定义变量 --
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_ICL_CMM_LC_ACCT_INFO'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  DELETE FROM RRP_MDL.O_ICL_CMM_LC_ACCT_INFO T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  --EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_ICL_CMM_LC_ACCT_INFO';
  ...
  -- 程序业务逻辑处理主体部分 --
  V_STEP := V_STEP + 1;
  V_STEP_DESC := '数据落地-信用证账户信息';
  V_STARTTIME := SYSDATE;
  INSERT INTO RRP_MDL.O_ICL_CMM_LC_ACCT_INFO
    (ETL_DT                         --数据日期
    ,LP_ID                          --法人编号
    ,ACCT_ID                        --账户编号
    ,LC_ID                          --信用证编号
    ...
    ,ACPTY_TOT                      --可承兑总额
    ,JOB_CD                         --任务代码
    ,CHECK_TELLER_ID                --复核柜员编号 ADD BY YJY 20250414
    ,CHECK_TELLER_NAME              --复核柜员名称 ADD BY YJY 20250414
    ,TRAN_CMPLT_TM                  --交易完成时间 ADD BY YJY 20251016
    ,BENEFC_OPEN_BANK_ACCT_NUM      --受益人开户行账号  ADD BY YJY 20251107
    ,BENEFC_OPEN_BANK_NAME          --受益人开户行名称  ADD BY YJY 20251107
    )
  SELECT 
     ETL_DT                         --数据日期
    ,LP_ID                          --法人编号
    ...
    ,BENEFC_OPEN_BANK_ACCT_NUM      --受益人开户行账号  ADD BY YJY 20251107
    ,BENEFC_OPEN_BANK_NAME          --受益人开户行名称  ADD BY YJY 20251107
    FROM ICL.V_CMM_LC_ACCT_INFO  --视图-信用证账户信息
...
END ETL_O_ICL_CMM_LC_ACCT_INFO;
/
