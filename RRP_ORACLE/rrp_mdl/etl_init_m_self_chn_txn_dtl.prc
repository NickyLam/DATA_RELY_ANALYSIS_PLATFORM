CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_M_SELF_CHN_TXN_DTL(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_INIT_M_SELF_CHN_TXN_DTL
  *  功能描述：自助渠道交易流水
  *  创建日期：20220930
  *  开发人员：MW
  *  来源表：
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(200) := 'ETL_INIT_M_SELF_CHN_TXN_DTL'; -- 程序名称
  ...
  V_TAB_NAME := 'M_SELF_CHN_TXN_DTL'; --表名
  ...
  INSERT INTO M_SELF_CHN_TXN_DTL
      ...
      ,(CASE WHEN T1.TRAN_STATUS_CD IN ('1','2','7','9') THEN '00' --成功
        ...
        ELSE '01' END)                                             AS TXN_RESP_NUM              --6交易返回码
     ...
     FROM O_IML_EVT_ATMP_UNIONPAY_TRAN_FLOW T1--ATMP银联前置交易流水
  ...
  END ETL_INIT_M_SELF_CHN_TXN_DTL;
/
