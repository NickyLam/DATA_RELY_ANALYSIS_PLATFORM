CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_O_ICL_CMM_INTNAL_ACCT(I_P_DATE IN INTEGER,
                              O_ERRCODE OUT VARCHAR2
                               )
  /**************************************************************************
  *  程序名称：ETL_INIT_O_ICL_CMM_INTNAL_ACCT
  *  功能描述：内部账户
  *  创建日期：20220525
  *  开发人员：梅炜
  *  来源表：
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(500) := 'ETL_INIT_O_ICL_CMM_INTNAL_ACCT'; -- 程序名称
...
EXECUTE IMMEDIATE 'TRUNCATE TABLE '|| V_TABLE_NAME;
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
   -- EXECUTE IMMEDIATE ' TRUNCATE TABLE  O_ICL_CMM_INTNAL_ACCT ;
  -- -- EXECUTE IMMEDIATE ' TRUNCATE TABLE O_ICL_CMM_INTNAL_ACCT';
  ...
  (
    ...
    SELECT
      ...
      WHEN  ACCT_STATUS_CD ='1' THEN 'A' ELSE ACCT_STATUS_CD END--账户状态代码
						,OPEN_ACCT_DT  --开户日期
						,CLOS_ACCT_DT  --销户日期
						,IN_OUT_TAB_FLG  --表内外标志
						,SUSPD_WRTOFF_FLG  --挂销账标志
    ...
    WHERE ETL_DT = (SELECT MAX(ETL_DT) FROM ICL.V_CMM_INTNAL_ACCT)
    ;
  ...
  END ETL_INIT_O_ICL_CMM_INTNAL_ACCT;
/
