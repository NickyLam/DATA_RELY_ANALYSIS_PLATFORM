CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_O_ICL_CMM_IBANK_CASH_DEBIT_CRDT(I_P_DATE IN INTEGER,
                              O_ERRCODE OUT VARCHAR2
                               )
  /**************************************************************************
  *  程序名称：ETL_INIT_O_ICL_CMM_IBANK_CASH_DEBIT_CRDT
  *  功能描述：同业现金借贷
  *  创建日期：20220525
  *  开发人员：梅炜
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(500) := 'ETL_INIT_O_ICL_CMM_IBANK_CASH_DEBIT_CRDT'; -- 程序名称
...
EXECUTE IMMEDIATE 'TRUNCATE TABLE '|| V_TABLE_NAME;
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
   -- EXECUTE IMMEDIATE ' TRUNCATE TABLE  O_ICL_CMM_IBANK_CASH_DEBIT_CRDT ;
  -- -- EXECUTE IMMEDIATE ' TRUNCATE TABLE O_ICL_CMM_IBANK_CASH_DEBIT_CRDT';
  ...
  (      ETL_DT  --数据日期
    ...
    SELECT
      ...
      /*ETL_DT*/TO_DATE(V_P_DATE,'YYYYMMDD')  --数据日期
      ,LP_ID  --法人编号
      ,EXT_SECU_ACCT_ID  --外部证券账户编号
      ,INTNAL_SECU_ACCT_ID  --内部证券账户编号
      ,ACCT_ID  --账户编号
      ...
      ,TRAN_SEQ_NUM  --交易序号
    ,JOB_CD --任务代码
    /*FROM ICL.V_CMM_IBANK_CASH_DEBIT_CRDT  --视图-同业现金借贷
    */
    ...
    AND TRUNC(ETL_DT,'MM') = TRUNC(TO_DATE(V_P_DATE,'YYYYMMDD'),'MM')

    /*union all --20220930数据测试用，后面需删除
    ...
    *\
    FROM(SELECT \*+ PARALLEL *\
    ROW_NUMBER() OVER(PARTITION BY OBJ_ID ORDER BY A.ETL_DT DESC) RN,A.*
            FROM ICL.V_CMM_IBANK_CASH_DEBIT_CRDT@LINK_EDW A--同业现金借贷
           WHERE TRUNC(A.ETL_DT,'MM') = TRUNC(TO_DATE(V_P_DATE,'YYYYMMDD'),'MM')
             AND A.ETL_DT <= TO_DATE(V_P_DATE,'YYYYMMDD')
             AND A.FIN_INSTM_ID IN ('469462203280125100085901',
'1302209060125100096219',
'1302209060125100096218',
'469452109070125100074529',
'1302208230125100095384',
'1302207050125100092753',
'469462203150125100085170',
'1302208230125100095380',
'1302209070125100096265',
'1302209070125100096265',
'469472203160125100085406',
'1302208240125100095430',
'469462204060125100086735',
'469462208220125100095342',
'1302209060125100096218',
'469462206070125100090755',
'1302209060125100096219',
'469442207220125100093581'
))
    WHERE RN = 1 --按照日期取最新一条，因为源系统当债券余额为0时第二天就不提供了*/
  ...
  END ETL_INIT_O_ICL_CMM_IBANK_CASH_DEBIT_CRDT;
/
