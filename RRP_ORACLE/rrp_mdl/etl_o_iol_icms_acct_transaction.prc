CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IOL_ICMS_ACCT_TRANSACTION (I_P_DATE IN INTEGER, --跑批日期
                                                 O_ERRCODE  OUT VARCHAR2 --错误代码
                                                 )
 /*******************************************************************
  **存储过程详细说明： 帐务-交易信息
  **存储过程名称：    ETL_O_IOL_ICMS_ACCT_TRANSACTION
  **存储过程创建日期：20240205
  **存储过程创建人：  HULIJUAN
  **输入参数：        I_P_DATE
  **输出参数：        O_ERRCODE
 ...
 AS
  ...
  V_PROC_NAME VARCHAR2(500) := 'ETL_O_IOL_ICMS_ACCT_TRANSACTION'; -- 程序名称
  ...
  EXECUTE IMMEDIATE 'TRUNCATE TABLE O_IOL_ICMS_ACCT_TRANSACTION';
  ...
  -- 程序业务逻辑处理主体部分 --
  V_STEP := V_STEP + 1;
  V_STEP_DESC := '数据落地-帐务-交易信息';
  V_STARTTIME := SYSDATE;
  INSERT /*+APPEND*/ INTO RRP_MDL.O_IOL_ICMS_ACCT_TRANSACTION NOLOGGING
    (
          SERIALNO              --交易流水号
  ...
  SELECT /*+PARALLEL*/
         ...
         ,END_DT                --结束时间
         ,ID_MARK               --增删标志
   FROM IOL.V_ICMS_ACCT_TRANSACTION   --帐务-交易信息
...
END ETL_O_IOL_ICMS_ACCT_TRANSACTION;
/
