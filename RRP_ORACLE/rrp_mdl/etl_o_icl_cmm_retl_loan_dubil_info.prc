CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_ICL_CMM_RETL_LOAN_DUBIL_INFO(I_P_DATE IN INTEGER,
                                                               O_ERRCODE OUT VARCHAR2
                                                               )
  /**************************************************************************
  *  程序名称：ETL_O_ICL_CMM_RETL_LOAN_DUBIL_INFO
  *  功能描述：零售贷款借据信息
  *  创建日期：20220525
  *  开发人员：梅炜
  *  来源表：
  ...
  *             3    20231206  hulj     优化O层改成分区表
  *             4    20231226  hulj     新增字段利率浮动类型代码
  *             5    20231228  hulj     新增字段重组贷款标志、重组贷款类型代码
  *             6    20240429  YJY      新增养老产业标志
  *             7    20241216  YJY      新增尽调标志、线下核身标志
  *             8    20250324  YJY      新增诉讼费余额
  *             9    20250618  YJY      新增出账流水号
  ***********************************************************************/
AS
  -- 定义变量 --
  ...
  V_TAB_NAME  VARCHAR2(50) := 'O_ICL_CMM_RETL_LOAN_DUBIL_INFO'; --表名
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_ICL_CMM_RETL_LOAN_DUBIL_INFO'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM RRP_MDL.O_ICL_CMM_RETL_LOAN_DUBIL_INFO T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  --EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_ICL_CMM_RETL_LOAN_DUBIL_INFO';
  ...
  INSERT INTO RRP_MDL.O_ICL_CMM_RETL_LOAN_DUBIL_INFO
    ...
    ,DUE_DILIGENCE_FLG           --尽调标志       add by yjy 20241216
    ,OUTLINE_VRIF_IDTI_FLG       --线下核身标志   add by yjy 20241216
    ,SUIT_FEE_BAL                --诉讼费余额     add by yjy 20250324
    ,OUT_ACCT_FLOW_NUM           --出账流水号     add by yjy 20250618
    )
  SELECT 
     ETL_DT                      --数据日期
    ,LP_ID                       --法人编号
...
END ETL_O_ICL_CMM_RETL_LOAN_DUBIL_INFO;
/
