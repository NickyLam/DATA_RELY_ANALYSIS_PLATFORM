CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IML_EVT_IBANK_TRAN(I_P_DATE IN INTEGER,
                                                     O_ERRCODE OUT VARCHAR2
                                                     )
  /**************************************************************************
  *  程序名称：ETL_O_IML_EVT_IBANK_TRAN
  *  功能描述：同业交易表
  *  创建日期：20220525
  *  开发人员：梅炜
...
AS
  ...
  V_TAB_NAME  VARCHAR2(50) := 'O_IML_EVT_IBANK_TRAN'; --表名
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IML_EVT_IBANK_TRAN'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  DELETE FROM RRP_MDL.O_IML_EVT_IBANK_TRAN T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  --EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_IML_EVT_IBANK_TRAN';
  ...
  INSERT INTO RRP_MDL.O_IML_EVT_IBANK_TRAN
    ...
    ,ACCT_B_CATE_CD --账簿类型 T-交易账簿 B-银行账簿
    )
