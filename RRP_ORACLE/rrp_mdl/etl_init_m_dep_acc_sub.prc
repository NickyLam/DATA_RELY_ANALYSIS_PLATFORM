CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_M_DEP_ACC_SUB(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_INIT_M_DEP_ACC_SUB
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(200) := 'ETL_INIT_M_DEP_ACC_SUB'; -- 程序名称
 ...
 /* V_STEP_DESC := '初始化表增加分区';
  ...
  SELECT
   ...
   ,--NVL(A.OLD_CUST_ACCT_SUB_ACCT_NUM,A.CUST_ACCT_SUB_ACCT_NUM)
   CASE WHEN A.SUBJ_ID LIKE '201101%'
                AND A.AGREE_DEP_FLG LIKE '%1%'
                AND(A.CURRT_BAL - NVL(A.AGREE_DEP_INIT_AMT, 0)) > 0
                AND NVL(A.AGREE_DEP_INIT_AMT, 0) != 0 --ADD BY XXL 20211020 排除协定金额为0或为空的情况
               THEN 'XD'||SUBSTR(NVL(A.OLD_CUST_ACCT_SUB_ACCT_NUM,A.CUST_ACCT_SUB_ACCT_NUM),3,LENGTH(NVL(A.OLD_CUST_ACCT_SUB_ACCT_NUM,A.CUST_ACCT_SUB_ACCT_NUM))) --协定存款的特殊处理
               WHEN A.SUBJ_ID LIKE '201101%'
                AND A.AGREE_DEP_FLG LIKE '%1%'
                AND (A.CURRT_BAL - NVL(A.AGREE_DEP_INIT_AMT, 0)) < 0
                AND NVL(A.AGREE_DEP_INIT_AMT, 0) != 0
               THEN 'XD002'       -- MDF BY TYJ 20220609  协定存款结算户部分特殊处理，赋默认值XD002
               ELSE NVL(A.OLD_CUST_ACCT_SUB_ACCT_NUM,A.CUST_ACCT_SUB_ACCT_NUM)
...
WHERE ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')) HT
 ...
 AND HT.RN = 1
  ...
  SELECT
   ...
   ,A.CUST_TYPE_CD                           AS CUST_TYP                          --客户类型
   ,--NVL(A.OLD_CUST_ACCT_SUB_ACCT_NUM,A.CUST_ACCT_SUB_ACCT_NUM)
   CASE WHEN A.CURRT_BAL - NVL(A.AGREE_DEP_INIT_AMT, 0) > 0 THEN 'XD002'
               WHEN A.CURRT_BAL - NVL(A.AGREE_DEP_INIT_AMT, 0) < 0 THEN 'XD001'
                END             AS SUB_ACC_ID                        --子账户编号20221102 XUXIAOBIN业务希望用回以前的
   ,A.ACCT_ID                                AS ACC_ID_EAST                       --账户编号_EAST
   ,A.EAR_M_BAL                              AS EAR_M_BAL                         --月初余额
   ,TO_CHAR(A.NEXT_INT_SET_DT,'YYYYMMDD')    AS NEXT_INT_SET_DT                   --下次结息日期
...
WHERE ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')) HT
 ...
 AND HT.RN = 1
  ...
  WHERE A.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
  AND A.AGREE_DEP_FLG = '1' --仅处理协定存款
  AND NVL(A.AGREE_DEP_INIT_AMT, 0) <> 0
  AND  (SUBSTR(A.SUBJ_ID, 1, 4) IN ('2011') )
  --科目筛选  20220901 XUXIAOBIN MODIFY
  ...
  WITH TMP1 AS (
    SELECT DATA_DT, NVL(t.ACCT_CARD_NO,t.ACC_ID)||t.sub_acc_id,COUNT(1)
      FROM M_DEP_ACC_SUB T
     WHERE DATA_DT = V_P_DATE
    GROUP BY DATA_DT, NVL(t.ACCT_CARD_NO,t.ACC_ID)||t.sub_acc_id
    HAVING COUNT(1) > 1)
