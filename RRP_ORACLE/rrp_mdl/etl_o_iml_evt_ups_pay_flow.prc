CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IML_EVT_UPS_PAY_FLOW(I_P_DATE IN INTEGER,
                                                       O_ERRCODE OUT VARCHAR2
                                                       )
  /**************************************************************************
  *  程序名称：ETL_O_IML_EVT_UPS_PAY_FLOW
  *  功能描述：银联付款流水
  *  创建日期：20231031
  *  开发人员：HULIJUAN
  *  来源表： IML.V_EVT_UPS_PAY_FLOW
  *  目标表： O_IML_EVT_UPS_PAY_FLOW
  *  配置表：
  *  修改情况：序号  修改日期     修改人      修改原因
  *             1    20231031     HULIJUAN    首次创建
  ***************************************************************************/
AS
  -- 定义变量 --
  ...
  V_TAB_NAME  VARCHAR2(50) := 'O_IML_EVT_UPS_PAY_FLOW'; --表名
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IML_EVT_UPS_PAY_FLOW'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM RRP_MDL.O_IML_EVT_UPS_PAY_FLOW T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  --EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_IML_EVT_UPS_PAY_FLOW';
  ...
  -- 程序业务逻辑处理主体部分 --
  V_STEP := V_STEP + 1;
  V_STEP_DESC := '数据落地-银联付款流水';
  V_STARTTIME := SYSDATE;
  INSERT INTO RRP_MDL.O_IML_EVT_UPS_PAY_FLOW
    (EVT_ID   --事件编号
    ,LP_ID   --法人编号
    ,BATCH_NO   --批次号
    ,TRAN_FLOW_NUM   --交易流水号
  ...
  SELECT EVT_ID   --事件编号
        ...
        ,SRC_TABLE_NAME   --源表名称
        ,JOB_CD   --任务代码
    FROM IML.V_EVT_UPS_PAY_FLOW  --视图-银联付款流水
...
END ETL_O_IML_EVT_UPS_PAY_FLOW;
/
