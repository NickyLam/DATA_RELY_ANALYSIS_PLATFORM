CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_PUM_EXRT_INFO(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_M_PUM_EXRT_INFO
  *  功能描述：监管集市各类币种折币信息
  *  创建日期：20220519
  *  开发人员：hulijuan
...
AS
  ...
  V_TAB_NAME  VARCHAR2(100) := 'M_PUM_EXRT_INFO'; --表名
  V_PROC_NAME VARCHAR2(100) := 'ETL_M_PUM_EXRT_INFO'; -- 程序名称
...
BEGIN
 ...
 -- 分区表分区处理 --
  ...
  V_STEP_DESC := '汇率表';
  V_STARTTIME := SYSDATE;
  INSERT INTO RRP_MDL.M_PUM_EXRT_INFO
    (DATA_DT          --数据日期
     ,LGL_REP_ID       --法人编号
     ,ORG_ID           --机构编号
     ,BASE_CUR         --基准币种
  ...
  SELECT  TO_CHAR(A.ETL_DT,'YYYYMMDD')                    AS DATA_DT            --数据日期
         ...
         ,'CNY'                                           AS CNV_CUR            --折算币种
         ,COALESCE(A.CNY_EXCH_RAT*100,100)                AS MDL_PRC            --中间价
         ,COALESCE(A.BASE_PRICE,100)                      AS BASE_PRC           --基准价
    ...
    FROM RRP_MDL.O_ICL_CMM_EXCH_RAT_INFO A --汇率信息
  ...
  SELECT  TO_CHAR(A.ETL_DT,'YYYYMMDD')                    AS DATA_DT            --数据日期
         ...
         ,'USD'                                           AS CNV_CUR            --折算币种
         --MOD BY LIP 20230704
         ,COALESCE(A.USD_EXCH_RAT*100,100)                AS MDL_PRC            --中间价
         ,COALESCE(A.BASE_PRICE,100)                      AS BASE_PRC           --基准价
  ...
  SELECT  TO_CHAR(A.ETL_DT,'YYYYMMDD')                    AS DATA_DT            --数据日期
         ...
         ,'EUR'                                           AS CNV_CUR            --折算币种
         --MOD BY LIP 20230704
         ,COALESCE(A.EUR_EXCH_RAT*100,100)                AS MDL_PRC            --中间价
         ,COALESCE(A.BASE_PRICE,100)                      AS BASE_PRC           --基准价
  ...
  WITH TMP1 AS (
    SELECT DATA_DT, BASE_CUR,CNV_CUR,COUNT(1)
      FROM RRP_MDL.M_PUM_EXRT_INFO T
