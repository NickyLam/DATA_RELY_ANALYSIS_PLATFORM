CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_PUM_EMP_POST(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_M_PUM_EMP_POST
...
AS
  ...
  V_TAB_NAME  VARCHAR2(100) := 'M_PUM_EMP_POST'; --表名
  V_PROC_NAME VARCHAR2(100) := 'ETL_M_PUM_EMP_POST'; --程序名称
...
BEGIN
 ...
 -- 分区表分区处理 --
  ...
  V_STEP_DESC := '岗位表-逻辑1';
  V_STARTTIME := SYSDATE;
  INSERT INTO RRP_MDL.M_PUM_EMP_POST
  (
    DATA_DT      --数据日期
   ,LGL_REP_ID   --法人编号
   ,POST_ID      --岗位编号
  ...
  SELECT V_P_DATE                               AS DATA_DT     --数据日期
        ...
        ,CASE WHEN TRIM(A.TYPE_CD) = '1' THEN '基准岗位'
              ...
              ELSE '具体岗'
          END                                   AS POST_TYP    --岗位类型
        ,A.POST_NAME                            AS POST_NM     --岗位名称
        ,A.POST_NAME                            AS POST_EXPL   --岗位说明
        ,CASE WHEN STATUS_CD = '2' THEN '未启用'
              ELSE '启用'
          END                                   AS POST_STAT   --岗位状态
        ,'800915'                               AS DEPT_LINE   --部门条线
        ,SUBSTR(A.JOB_CD,0,4)                   AS DATA_SRC    --数据来源
    FROM RRP_MDL.O_IML_REF_POSTN_PARA A  --职位参数
   INNER JOIN RRP_MDL.ORG_CONFIG B
      ON B.ORG_ID = (CASE WHEN SUBSTR(A.ORG_ID,1,3) = '896' THEN '896' ELSE NVL(TRIM(A.ORG_ID),'800') END)
   WHERE TRIM(A.POST_NAME) IS NOT NULL
  ...
  SELECT V_P_DATE                               AS DATA_DT     --数据日期
        ...
        ,SUBSTR(A.JOB_CD,0,4)                   AS DATA_SRC    --数据来源
    FROM RRP_MDL.O_IML_REF_TELLER_JOBS_INFO_H A  --柜员岗位信息历史
    LEFT JOIN RRP_MDL.ORG_CONFIG B
      ON B.ORG_ID = (CASE WHEN SUBSTR(SUBSTR(A.JOBS_NAME,1,6),1,3) = '896' THEN '896' ELSE SUBSTR(A.JOBS_NAME,1,6) END)
   ...
   WHERE A.START_DT <= TO_DATE(V_P_DATE,'YYYYMMDD')
     ...
     AND TRIM(A.JOBS_ID) IS NOT NULL
     AND TRIM(A.JOBS_NAME) IS NOT NULL;
