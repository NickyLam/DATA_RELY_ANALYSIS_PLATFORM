CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_CUST_IND_CRDL_SUB(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_M_CUST_IND_CRDL_SUB
...
AS
  ...
  V_TAB_NAME  VARCHAR2(100) := 'M_CUST_IND_CRDL_SUB'; --表名
  V_PROC_NAME VARCHAR2(100) := 'ETL_M_CUST_IND_CRDL_SUB'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM RRP_MDL.M_CUST_IND_CRDL_SUB T WHERE T.DATA_DT = V_P_DATE;--普通表的重跑处理
  ...
  V_STARTTIME := SYSDATE;
  INSERT INTO RRP_MDL.M_CUST_IND_CRDL_SUB
    (DATA_DT                --数据日期
    ,LGL_REP_ID             --法人编号
    ,CUST_ID                --客户编号
    ,CRDL_TYP               --证件类型
  ...
  SELECT V_P_DATE                         AS DATA_DT                --数据日期
        ...
        ,NULL                             AS DEPT_LINE              --部门条线 /*零售信贷部(普惠金融部)*/
        /*,SUBSTR(A.JOB_CD,0,4)             AS DATA_SRC               --数据来源*/
        ,'EIFS'                           AS DATA_SRC               --数据来源
    FROM (SELECT T.LP_ID,T.PARTY_ID,T.CERT_TYPE_CD,T.CERT_NUM,
                 ROW_NUMBER()OVER (PARTITION BY T.PARTY_ID,T.CERT_TYPE_CD,T.CERT_NUM ORDER BY T.CERT_INVALID_DT) RN
            FROM RRP_MDL.O_IML_PTY_PARTY_CERT_INFO_H T
           WHERE SUBSTR(T.SORC_SYS_CD,0,4) = 'EIFS'
             AND T.START_DT <= TO_DATE(V_P_DATE,'YYYYMMDD')
             AND T.END_DT > TO_DATE(V_P_DATE,'YYYYMMDD')
             AND T.ID_MARK <> 'D') A --当事人证件信息历史
   INNER JOIN RRP_MDL.O_ICL_CMM_INDV_CUST_BASIC_INFO B --关联个人表剔除对公客户
      ON B.CUST_ID = A.PARTY_ID
     AND B.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN (SELECT CC.PARTY_ID,CC.LICEN_ISSUE_AUTHO_DIST_CD,CC.CRDL_EFF_DT,CC.CRDL_EXP_DT,CC.CRDL_ADDR
  ...
  WITH TMP1 AS (
    SELECT DATA_DT, CUST_ID,CRDL_NO,CRDL_TYP,COUNT(1)
      FROM RRP_MDL.M_CUST_IND_CRDL_SUB T
