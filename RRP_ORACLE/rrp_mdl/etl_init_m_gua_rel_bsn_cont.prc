CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_M_GUA_REL_BSN_CONT(I_P_DATE IN INTEGER,
                                                O_ERRCODE OUT VARCHAR2
                                                )
  /**************************************************************************
  *  程序名称：ETL_INIT_M_GUA_REL_BSN_CONT
  *  功能描述：担保合同和业务合同对应关系表
  *  创建日期：20220525
  *  开发人员：梅炜
  *  来源表：
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(200) := 'ETL_INIT_M_GUA_REL_BSN_CONT'; -- 程序名称
  ...
  V_TAB_NAME := 'M_GUA_REL_BSN_CONT'; --表名
  ...
  (
     ...
     ,A.LOAN_CONT_ID                           AS BIZ_CONT_ID        --业务合同号
     ,A.GUAR_CONT_ID                           AS GUA_CONT_ID        --担保合同号
     ,B.DIRECTOR_ORG_ID                        AS ORG_ID             --机构编号
     ,CASE WHEN A.GUAR_CONT_STATUS_CD ='102' THEN '1'
            WHEN A.GUAR_CONT_STATUS_CD ='103' THEN '2'
            ELSE '0' END                    AS GUA_STAT           --担保状态
    ,TO_CHAR(A.GUAR_START_DT,'YYYYMMDD')       AS GUA_REL_START_DT   --担保关系开始日期
    ,TO_CHAR(A.GUAR_EXP_DT,'YYYYMMDD')         AS GUA_REL_RLV_DT     --担保关系解除日期
    ,'800919'                                  AS DEPT_LINE          --部门条线
    ,SUBSTR(A.JOB_CD,0,4)                     AS DATA_SRC             --数据来源
   FROM RRP_MDL.O_ICL_CMM_LOAN_GUAR_CONT_RELA A      --贷款合同与担保合同关系
   LEFT JOIN RRP_MDL.O_ICL_CMM_GUAR_CONT B                --担保合同
  ...
  END ETL_INIT_M_GUA_REL_BSN_CONT;
/
