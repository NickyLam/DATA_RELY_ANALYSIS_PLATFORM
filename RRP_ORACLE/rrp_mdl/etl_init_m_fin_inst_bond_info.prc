CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_M_FIN_INST_BOND_INFO(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_INIT_M_FIN_INST_BOND_INFO
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(200) := 'ETL_INIT_M_FIN_INST_BOND_INFO'; -- 程序名称
  ...
  V_TAB_NAME := 'M_FIN_INST_BOND_INFO'; --表名
  ...
  /*****************资金系统******************/
  INSERT INTO M_FIN_INST_BOND_INFO
  (
    DATA_DT                --数据日期
   ,LGL_REP_ID             --法人编号
   ,ORG_ID                 --机构编号
  ...
  SELECT
   ...
   ,CASE WHEN  E1.B_INFO_FULLNAME LIKE '%专项%'
         THEN 'B0201'
         WHEN E1.B_INFO_FULLNAME LIKE '%一般%'
         THEN 'B0202'
   ...
   ,TD.TAR_VALUE_CODE                   AS  ISU_SUBJ_RTG          --发行主体评级
   ...
   ,DECODE(NVL(C.RATING_REST_CD,'C00'),'AAA','C01','AA+','C02','AA','C03','AA-','C04','A+','C05','A','C06','A-','C07','BBB+','C08','BBB','C09','BBB-','C10','BB+','C11','BB',
                                   ...
                                   'C24','A-3','C25','C00','C00','无','C00','C99')                    AS  BOND_RTG              --债券评级
   ...
   ,CASE WHEN (A.FAC_VAL_INT_RAT - WIND.B_AVG_YIELD)/WIND.B_AVG_YIELD <=0.15
         THEN 'A'
         WHEN (A.FAC_VAL_INT_RAT - WIND.B_AVG_YIELD)/WIND.B_AVG_YIELD <=0.3
         THEN 'B'
         WHEN (A.FAC_VAL_INT_RAT - WIND.B_AVG_YIELD)/WIND.B_AVG_YIELD >0.3
         THEN 'C'
         WHEN A.BOND_ID IN ('1563010','1563039','1563040','1543010','1572003')
         THEN 'A'
  ...
  LEFT JOIN O_IML_PTY_BOND_ISSUER_INFO E -- 债券发行人信息 --MDF BY HAP 20201012 用表资金交易对手信息表的话有部分关联不上
      ON A.ISSUER_CD = E.ISSUER_ID
      AND E.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
      LEFT JOIN O_IML_PTY_BOND_ISSUER_INFO F--债券发行人信息
      on NVL(TRIM(E.ISSUER_ID),'-')=F.ISSUER_ID
  ...
  LEFT JOIN (SELECT S.BOND_ID,S.ENTRY_ORG_ID,S.CUSTM_BOND_ID,ROW_NUMBER() OVER(PARTITION BY BOND_ID ORDER BY S.ISSUE_DT DESC)RN
    ...
    LEFT JOIN (SELECT
     ...
     AND A.BOND_MARKET_TYPE_CD = C.BOND_MARKET_TYPE_CD
     AND C. RN = 1
     AND A.ETL_DT = C.ETL_DT
     LEFT JOIN CODE_MAP TA --产品类别转码
     ON TA.SRC_VALUE_CODE =A.BOND_TYPE_CD
     AND TA.SRC_CLASS_CODE = 'CD1486'
     AND TA.TAR_CLASS_CODE = 'T0018'
     AND TA.MOD_FLG = 'MDM'
     LEFT JOIN ADD_BOND_TYPE_CD M --债券类型代码表 add by hyf
      ON A.BOND_TYPE_CD = M.PRODUCT_TYPE_CODE
     LEFT JOIN CODE_MAP TB --发行方式转码
     ON TB.SRC_VALUE_CODE =B.ISSUE_STATUS_CD
     AND TB.SRC_CLASS_CODE = 'CD2021'
     AND TB.TAR_CLASS_CODE = 'D0068'
     AND TB.MOD_FLG = 'MDM'
   /*   ON A1.ISSUER_CD = B1.ISSUER_ID
     AND A1.ETL_DT = B1.ETL_DT*/
    LEFT JOIN (SELECT T.ISSUER_CUST_ID,T.RATING_REST_CD,ROW_NUMBER()OVER (PARTITION BY ISSUER_CUST_ID ORDER BY (CASE WHEN RATING_DT IS NULL THEN TO_DATE('19000101','YYYYMMDD') ELSE RATING_DT END)  DESC) RN
               FROM   O_ICL_CMM_ISSUER_RATING_INFO T
               WHERE T.ETL_DT = TO_DATE('20220930','YYYYMMDD')
               /*AND T.RATING_DT IS NOT NULL*/) T4
          ON T4.ISSUER_CUST_ID = A.ISSUER_CUST_ID
          AND T4.RN = 1
    LEFT JOIN CODE_MAP TD --发行人主体评级转码
          ON  TD.SRC_VALUE_CODE = T4.RATING_REST_CD
          AND TD.SRC_CLASS_CODE ='CD1850'
          AND TD.TAR_CLASS_CODE = 'D0062'
          AND TD.MOD_FLG = 'MDM'
    LEFT JOIN O_IOL_WIND_WINDCUSTOMCODE C1  --Wind兼容代码
      ON REGEXP_REPLACE(A.BOND_ID,'[^0-9]','')||'.'|| DECODE(A.BOND_MARKET_TYPE_CD,'XSHG','SH'  --债券编号去了非数字处理20201113 by hap
                                   ,'XSHE','SZ','X_CNBD','IB','DXFX','IB') = C1.S_INFO_WINDCODE
    LEFT JOIN ( SELECT T.*,ROW_NUMBER() OVER(PARTITION BY S_INFO_WINDCODE ORDER BY IS_FIN_INST DESC ) AS RN
                FROM O_IOL_WIND_CBONDISSUER T)D1 --中国债券主体表 --mdf by hap 20200928
      ON REGEXP_REPLACE(A.BOND_ID,'[^0-9]','')||'.'|| DECODE(A.BOND_MARKET_TYPE_CD,'XSHG','SH'
                                   ,'XSHE','SZ','X_CNBD','IB','DXFX','IB') = D1.S_INFO_WINDCODE
      ...
      LEFT JOIN ( SELECT T.*,ROW_NUMBER() OVER(PARTITION BY S_INFO_WINDCODE ORDER BY B_ISSUE_FIRSTISSUE) AS RN
                FROM O_IOL_WIND_CBONDDESCRIPTION T)E1 --中国债券基本资料表 --mdf by hap 2021112
      ON REGEXP_REPLACE(A.BOND_ID,'[^0-9]','')||'.'|| DECODE(A.BOND_MARKET_TYPE_CD,'XSHG','SH'
                                   ,'XSHE','SZ','X_CNBD','IB','DXFX','IB') = E1.S_INFO_WINDCODE
...
/*    LEFT JOIN (SELECT T.*,ROW_NUMBER() OVER(PARTITION BY S_INFO_WINDCODE ORDER BY ANN_DT DESC) AS RN
                FROM O_IOL_WIND_CBONDRATING T)I1 --中国债券信用评级表
     ON REGEXP_REPLACE(A.BOND_ID,'[^0-9]','')||'.'|| DECODE(A.BOND_MARKET_TYPE_CD,'XSHG','SH'
                                   ,'XSHE','SZ','X_CNBD','IB','DXFX','IB') = I1.S_INFO_WINDCODE
     AND A.ETL_DT = I1.ETL_DT
     AND I1.RN = 1*/
    LEFT JOIN (SELECT TRADE_DT, --交易日期
                 ...
                 FROM O_IOL_WIND_CBONDCURVECNBD --中债登债券收益率曲线
     ...
     AND (CASE WHEN A.TENOR LIKE '%Y%'
               THEN TO_NUMBER(REPLACE(A.TENOR,'Y',''))
               WHEN A.TENOR LIKE '%D%'
               THEN (CASE WHEN TO_NUMBER(REPLACE(A.TENOR,'D',''))<360
                          THEN CEIL(REPLACE(A.TENOR,'D','')/360)
                          WHEN TO_NUMBER(REPLACE(A.TENOR,'D',''))>=360
                          THEN TRUNC(TO_NUMBER(REPLACE(A.TENOR,'D',''))/360) END)
          END) = WIND.B_ANAL_CURVETERM       --期限
     /*LEFT JOIN  (SELECT I_CODE AS BOND_ID,S_GRADE AS RATING_REST_CD,A_TYPE AS ASSET_TYPE_ID,M_TYPE AS BOND_MARKET_TYPE_CD
                    ...
                    FROM O_IOL_IBMS_TINSTRUMENT_INST_GRADE
                    WHERE START_DT <= TO_DATE(V_P_DATE,'YYYYMMDD')
                    AND END_DT >TO_DATE(V_P_DATE,'YYYYMMDD')
                   ) B2  --债券评级信息表
      ON B2.BOND_ID||'.'||B2.BOND_MARKET_TYPE_CD = A.BOND_ID||'.'||A.BOND_MARKET_TYPE_CD
     AND B2.ASSET_TYPE_ID = A.ASSET_TYPE_ID
     AND B2.RN = 1 */
    where A.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
      AND A.BOND_TYPE_CD <> 'W'
  ...
  SELECT
   ...
   ,A.FIN_INSTM_ID /*|| '.' || A.ASSET_TYPE_ID || '.' || A.MARKET_TYPE_ID*/  AS  BOND_ID  --债券编号
   ,/*B.FIN_INSTM_NAME*/A.BOND_NAME                    AS  PROD_NM               --产品名称--20221013 XUXIAOBIN 业务孙若真 提议修改
   ,CASE WHEN A.ASSET_TYPE_NAME LIKE '%其他债券%' OR A.ASSET_TYPE_NAME LIKE '%私募债%'
                ...
                WHEN A.ASSET_TYPE_NAME LIKE '%资产支持%' THEN 'D02'
                     WHEN  E1.B_INFO_FULLNAME LIKE '%专项%'
                     THEN 'B0201'
                     WHEN E1.B_INFO_FULLNAME LIKE '%一般%'
                     THEN 'B0202'
                     ELSE TA.TAR_VALUE_CODE
               END
                                     AS  PROD_CL               --产品分类 -- XUXIAOBIN 20221013 XUXIAOBIN JS MODIFY
   ,NULL                                AS  FIN_DEBT_SUB_CL       --金融债细类
   ...
   ,DECODE(NVL(B2.RATING_REST_CD,'C00'),'AAA','C01','AA+','C02','AA','C03','AA-','C04','A+','C05','A','C06','A-','C07','BBB+','C08','BBB','C09','BBB-','C10','BB+','C11','BB',
    'C12','BB-','C13','B+','C14','B','C15','B-','C16','CCC+','C17','CCC','C18','CCC-','C19','CC','C20','C','C21','D','C22','A-1','C23','A-2',
     'C24','A-3','C25','C00','C00','无','C00','C99')AS  BOND_RTG              --债券评级
   ,CASE WHEN B.TENOR_CD LIKE '%Y%'
               THEN  replace(B.tenor_cd,'Y','') * 12
   ...
   ,CASE WHEN (A.FAC_VAL_INT_RAT - WIND.B_AVG_YIELD)/WIND.B_AVG_YIELD <=0.15
         ...
         WHEN (A.FAC_VAL_INT_RAT - WIND.B_AVG_YIELD)/WIND.B_AVG_YIELD >0.3
         THEN 'C'
         WHEN A.FIN_INSTM_ID IN ('1563010','1563039','1563040','1543010','1572003')
         THEN 'A'
          END                                  AS  FLT_RATE_TYP          --浮动利率类型
   ,NULL                                 AS  AST_SCRTZ_TYP         --资产证券化类型
   ...
   WHEN B1.TRUST_ORG_ID IN('02','03') THEN '900'
   ...
   ,/*A.ASSET_TYPE_NAME*/B1.BOND_CLS_NAME        AS ASSET_NAME             --资产名称20221121 XUXIAOBIN MODIFY 产品名称以债券基本信息比较正确
   ...
   ,A.FIN_INSTM_ID||'.'||A.MARKET_TYPE_ID             --主键
   FROM (SELECT ROW_NUMBER()OVER(PARTITION BY T.FIN_INSTM_ID,T.ASSET_TYPE_ID,T.MARKET_TYPE_ID ORDER BY T.LAST_UPDATE_DT DESC)RN
                 ,T.* FROM O_ICL_CMM_IBANK_BOND_INVEST T) A   --同业债券投资表
   LEFT JOIN O_ICL_CMM_IBANK_FIN_INSTM B  --同业金融工具表
        ON A.FIN_INSTM_ID||'.'||A.MARKET_TYPE_ID = B.FIN_INSTM_ID||'.'||B.MARKET_TYPE_ID
        AND A.ASSET_TYPE_ID = B.ASSET_TYPE_ID
        AND A.ETL_DT = B.ETL_DT
   LEFT JOIN O_ICL_CMM_BOND_BASIC_INFO B1--债券基本信息 ADD BY HAP 20210819 取托管机构
        ON A.FIN_INSTM_ID||'.'||A.MARKET_TYPE_ID = B1.BOND_ID||'.'||B1.BOND_MARKET_TYPE_CD
        AND A.ASSET_TYPE_ID = B1.ASSET_TYPE_ID
        AND A.ETL_DT = B1.ETL_DT
   LEFT JOIN O_IML_PTY_BOND_ISSUER_INFO E -- 债券发行人信息
      ON B1.ISSUER_CD = E.ISSUER_ID
      LEFT JOIN O_IML_PTY_BOND_ISSUER_INFO F--债券发行人信息
      on NVL(TRIM(E.ISSUER_ID),'-')=F.ISSUER_ID
      AND f.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN (SELECT T.ISSUER_CUST_ID,T.RATING_REST_CD,ROW_NUMBER()OVER (PARTITION BY ISSUER_CUST_ID ORDER BY (CASE WHEN RATING_DT IS NULL THEN TO_DATE('19000101','YYYYMMDD') ELSE RATING_DT END) DESC) RN
     ...
     AND TA.MOD_FLG = 'MDM'
     LEFT JOIN (SELECT T.*,ROW_NUMBER() OVER(PARTITION BY S_INFO_WINDCODE ORDER BY IS_FIN_INST DESC ) AS RN
              FROM O_IOL_WIND_CBONDISSUER T)D --中国债券主体表
     ON REGEXP_REPLACE(A.FIN_INSTM_ID,'[^0-9]','')||'.'|| DECODE(A.MARKET_TYPE_ID,'XSHG','SH'
                                 ,'XSHE','SZ','X_CNBD','IB','DXFX','IB') = D.S_INFO_WINDCODE
     AND A.ETL_DT BETWEEN D.START_DT AND D.END_DT
     AND D.RN = 1
     LEFT JOIN (SELECT T.*,ROW_NUMBER() OVER(PARTITION BY S_INFO_WINDCODE ORDER BY ANN_DT DESC) AS RN
                FROM O_IOL_WIND_CBONDRATING T)E --中国债券信用评级表
     ON REGEXP_REPLACE(A.FIN_INSTM_ID,'[^0-9]','')||'.'|| DECODE(A.MARKET_TYPE_ID,'XSHG','SH'
                                   ,'XSHE','SZ','X_CNBD','IB','DXFX','IB') = E.S_INFO_WINDCODE
     AND A.ETL_DT = E.ETL_DT
     AND E.RN = 1
     LEFT JOIN ( SELECT T.*,ROW_NUMBER() OVER(PARTITION BY S_INFO_WINDCODE ORDER BY B_ISSUE_FIRSTISSUE) AS RN
                FROM O_IOL_WIND_CBONDDESCRIPTION T)E1  --中国债券基本资料表
      ON REGEXP_REPLACE(A.FIN_INSTM_ID,'[^0-9]','')||'.'|| DECODE(A.MARKET_TYPE_ID,'XSHG','SH'
                                   ,'XSHE','SZ','X_CNBD','IB','DXFX','IB') = E1 .S_INFO_WINDCODE
    ...
    LEFT JOIN (SELECT T.*,ROW_NUMBER() OVER(PARTITION BY S_INFO_COMPCODE ORDER BY ANN_DT DESC) AS RN
                FROM O_IOL_WIND_CBONDISSUERRATING T WHERE T.ANN_DT <= I_P_DATE) G  --中国债券发行主体信用评级表
      ON D.S_INFO_COMPCODE = G.S_INFO_COMPCODE
      AND G.RN =1
    LEFT JOIN O_IML_PTY_IBANK_CUST_CHAT_INFO T2
    ON NVL(NVL(SUBSTR(TRIM(A.ISSUER_CUST_ID),1,INSTR(TRIM(A.ISSUER_CUST_ID),'、')-1),TRIM(A.ISSUER_CUST_ID)),A.ISSUER_ID)=T2.party_id
    /*LEFT JOIN M_DICT_FINA_CODE T3 ON T2.fin_inst_cate_cd=T3.ECIF_FINA_CODE*/
  ...
  LEFT JOIN (SELECT TRADE_DT, --交易日期
     ...
     ON A.VALUE_DT BETWEEN TO_DATE(WIND.TRADE_DT,'YYYYMMDD') AND TO_DATE(WIND.MAX_DAY,'YYYYMMDD')
     AND (CASE WHEN B.TENOR_CD LIKE '%Y%'
               THEN TO_NUMBER(REPLACE(B.TENOR_CD,'Y',''))
               WHEN B.TENOR_CD LIKE '%D%'
               THEN (CASE WHEN TO_NUMBER(REPLACE(B.TENOR_CD,'D',''))<360
                          THEN CEIL(REPLACE(B.TENOR_CD,'D','')/360)
                          WHEN TO_NUMBER(REPLACE(B.TENOR_CD,'D',''))>=360
                          THEN TRUNC(TO_NUMBER(REPLACE(B.TENOR_CD,'D',''))/360) END)
          END) = WIND.B_ANAL_CURVETERM  --期限
    LEFT JOIN  (SELECT I_CODE AS BOND_ID,S_GRADE AS RATING_REST_CD,A_TYPE AS ASSET_TYPE_ID,M_TYPE AS BOND_MARKET_TYPE_CD
                          ,ROW_NUMBER() OVER(PARTITION BY I_CODE,A_TYPE,M_TYPE ORDER BY S_GRADE ASC) AS RN
                    FROM O_IOL_IBMS_TINSTRUMENT_INST_GRADE
     ...
     AND B2.RN = 1
    WHERE
      /*(B.ASSET_TYPE_NAME LIKE '%公司债%' OR B.ASSET_TYPE_NAME LIKE '%企业债%' OR B.ASSET_TYPE_NAME LIKE '%资产支持%'
      OR B.ASSET_TYPE_NAME LIKE '%其他债券%' OR B.ASSET_TYPE_NAME LIKE '%私募债%' )*/--20221013 XUXIAOBIN ZHUSHI
      --AND D.S_INFO_TYPECODE = '115200000'     --数仓提供的视图无此字段，回复未与万德同步，待处理！
  ...
  END ETL_INIT_M_FIN_INST_BOND_INFO;
/
