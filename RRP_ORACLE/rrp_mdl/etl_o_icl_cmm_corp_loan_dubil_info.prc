CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_ICL_CMM_CORP_LOAN_DUBIL_INFO(I_P_DATE IN INTEGER,
                                                               O_ERRCODE OUT VARCHAR2
                                                               )
  /**************************************************************************
  *  程序名称：ETL_O_ICL_CMM_CORP_LOAN_DUBIL_INFO
  *  功能描述：对公贷款借据信息
  *  创建日期：20220525
  *  开发人员：梅炜
  ...
  *             1    20220525  梅炜     首次创建
  *             2    20221615           修改参数
  *             3    20231226  hulj     新增字段利率浮动类型代码
  *             4    20240401  YJY      新增置换旧债标志
  *             5    20240417  YJY      PAYOFF_DT截掉时间戳
  *             6    20241217  YJY      新增诉讼费余额
  *             7    20250508  YJY      新增绿色信贷客户标志、绿色信贷分类_旧版代码、绿色信贷分类_新版代码
  *             8    20250902  YJY      新增承兑汇票手续费金额
  *             9    20251021  YJY      新增信用证开户行编号
  *            10    20251030  YJY      新增问题资产标志
  *            11    20251107  YJY      新增信用证开证行名称、结清类型代码、贷款使用备注
  *******************************************************************/
AS
  -- 定义变量 --
  ...
  V_TAB_NAME  VARCHAR2(50) := 'O_ICL_CMM_CORP_LOAN_DUBIL_INFO'; --表名
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_ICL_CMM_CORP_LOAN_DUBIL_INFO'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM RRP_MDL.O_ICL_CMM_CORP_LOAN_DUBIL_INFO T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  --EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_ICL_CMM_CORP_LOAN_DUBIL_INFO';
  ...
  INSERT INTO RRP_MDL.O_ICL_CMM_CORP_LOAN_DUBIL_INFO
    ...
    ,ACCPT_BIL_COMM_FEE_AMT            --承兑汇票手续费金额   ADD BY YJY 20250902
    ,LC_OPEN_BANK_ID                   --信用证开户行编号  ADD BY YJY 20251021
    ,PROB_ASSET_FLG                    --问题资产标志   ADD BY YJY 20251030
    ,LC_OPEN_BANK_NAME                 --信用证开证行名称  ADD BY 20251107
    ,PAYOFF_TYPE_CD                    --结清类型代码  ADD BY 20251107
    ,LOAN_USE_REMARK                   --贷款使用备注  ADD BY 20251107
    )
  SELECT 
     ETL_DT                            --数据日期
    ,LP_ID                             --法人编号
    ...
    ,PAYOFF_TYPE_CD                    --结清类型代码  ADD BY 20251107
    ,LOAN_USE_REMARK                   --贷款使用备注  ADD BY 20251107
    FROM ICL.V_CMM_CORP_LOAN_DUBIL_INFO --视图-对公贷款借据信息
...
END ETL_O_ICL_CMM_CORP_LOAN_DUBIL_INFO;
/
