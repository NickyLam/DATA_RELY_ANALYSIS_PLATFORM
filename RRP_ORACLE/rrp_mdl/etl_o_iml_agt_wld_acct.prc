CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IML_AGT_WLD_ACCT(I_P_DATE IN INTEGER,
                                                   O_ERRCODE OUT VARCHAR2
                                                   )
  /**************************************************************************
  *  程序名称：ETL_O_IML_AGT_WLD_ACCT
  *  功能描述：微粒贷账户
  *  创建日期：20230202
  *  开发人员：MW
...
AS
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IML_AGT_WLD_ACCT'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM RRP_MDL.O_IML_AGT_WLD_ACCT T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_IML_AGT_WLD_ACCT';
  ...
  SELECT AGT_ID                         --协议编号
        ...
        ,START_DT                       --创建日期
        ,END_DT                         --更新日期
        ,TO_DATE(V_P_DATE,'YYYYMMDD')   --ETL处理日期
        --,ID_MARK  --增删标志
        --,SRC_TABLE_NAME  --源表名称
        --,JOB_CD  --任务编码
        --,ETL_TIMESTAMP  --ETL处理时间戳
        ,OUT_LINE_CUST_ID               --外部客户编号
    FROM IML.V_AGT_WLD_ACCT_H  T1 --视图-微粒贷账户
   WHERE START_DT <= TO_DATE(V_P_DATE,'YYYYMMDD')
     AND END_DT > TO_DATE(V_P_DATE,'YYYYMMDD') 
     AND NOT EXISTS (SELECT 1 FROM RRP_MDL.O_IML_AGT_WLD_ACCT T2
  ...
  ETL_DBMS_STATS(V_P_DATE, 'O_IML_AGT_WLD_ACCT','', O_ERRCODE);
