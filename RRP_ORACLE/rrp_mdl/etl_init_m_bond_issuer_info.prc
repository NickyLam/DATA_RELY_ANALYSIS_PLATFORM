CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_M_BOND_ISSUER_INFO(I_P_DATE IN INTEGER,
                                                O_ERRCODE OUT VARCHAR2
                                                )
  /**************************************************************************
  *  程序名称：ETL_INIT_M_BOND_ISSUER_INFO
  *  功能描述：债券发行人信息
  *  创建日期：20221031
  *  开发人员：梅炜
  *  来源表：
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(200) := 'ETL_INIT_M_BOND_ISSUER_INFO'; -- 程序名称
  ...
  V_TAB_NAME := 'M_BOND_ISSUER_INFO'; --表名
  V_PART_NAME := 'PARTITION_'||V_P_DATE;
   SELECT  TRUNC(TO_DATE(V_P_DATE,'YYYYMMDD'), 'MM')
  ...
  (
    ...
    ,A.ISSUER_ID      CUST_ID            --客户号
    ,A.CN_NAME        CUST_NM            --客户中文名称
    ,A.EN_NAME        CUST_NM_EN        --客户英文名称
    ,'Y'              BIO_FLG            --境内外标志
    ,'12'             CUST_TYP          --客户大类
    ...
    ,NULL              ID_NO            --证件号码
    ,NULL              ORG_ID          --机构编号
    ,'CHN'            NATION_CD        --国籍或注册地国家代码
    ,'1'              RESIDENT_FLG     --居民标志
    ,NVL(B.OPERATED_COUNTRY,'CHN')
                      NATION_CD1      --住所或经营所在地国家代码
    ,NULL              RELA_TYP        --关联方类型
    ,NULL              STOCK_PCT        --持股比例
    ,'111'            BUSI_RELA_TYP    --业务关系类型  '111'->无结算业务、无理财业务、无咨询业务
    ,'资金发行人'      DEPTLINE          --部门条线
    ,'债券发行人信息'              DATA_SRC          --数据来源

    FROM O_IML_PTY_BOND_ISSUER_INFO A  --债券发行人信息
    /*LEFT JOIN (SELECT T1.OPERATED_COUNTRY,T1.ASSET_NAME,B.ISSUER_CD from S_IOL_RDW_DCM_FORECURR_DEBT_SENSITIV t1
    LEFT JOIN S_ICL_CMM_BOND_BASIC_INFO B  --债券基本信息表
      ON T1.LOAN_REF_NO = B.BOND_ID
      AND B.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')) B*/ --ADD BY HAP 20210125 取财务集市补录表的经营地
    LEFT JOIN (SELECT * FROM(
             SELECT T.*,ROW_NUMBER() OVER(PARTITION BY ISSUER_CD ORDER BY OPERATED_COUNTRY ) AS RN
             FROM(SELECT T1.OPERATED_COUNTRY,T1.ASSET_NAME,B.ISSUER_CD
                         FROM ADD_DCM_FORECURR_DEBT_SENSITIV t1
                 LEFT JOIN O_ICL_CMM_BOND_BASIC_INFO B
                ON T1.LOAN_REF_NO = B.BOND_ID
                AND B.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
                )T )WHERE RN=1
                )B
      ON A.ISSUER_ID = B.ISSUER_CD
    WHERE /*A.ISSUER_ID NOT IN ( SELECT C.ISSUER_ID
                               FROM S_IML_PTY_CAP_CNTPTY_INFO C --资金交易对手信息
...
/*   -- DBMS_STATS.GATHER_TABLE_STATS(OWNNAME=>'写上数据库用户名',TABLENAME => '写上表名字',PARTNAME => 'P_'||V_P_DATE||'',DEGREE => 16,CASCATE => TRUE);
  ...
  END ETL_INIT_M_BOND_ISSUER_INFO;
/
