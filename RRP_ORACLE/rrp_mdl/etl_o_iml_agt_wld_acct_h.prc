CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IML_AGT_WLD_ACCT_H(I_P_DATE IN INTEGER,
                                                     O_ERRCODE OUT VARCHAR2
                                                     )
  /**************************************************************************
  *  程序名称：ETL_O_IML_AGT_WLD_ACCT_H
  *  功能描述：微粒贷账户信息历史
  *  创建日期：20230612
  *  开发人员：梅炜
  *  来源表：
  *  目标表： O_IML_AGT_WLD_ACCT_H
  *  配置表：
...
AS
  ...
  V_TAB_NAME	VARCHAR2(200) := 'O_IML_AGT_WLD_ACCT_H'; --表名
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IML_AGT_WLD_ACCT_H'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM RRP_MDL.O_IML_AGT_WLD_ACCT_H T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_IML_AGT_WLD_ACCT_H';
  ...
  -- 程序业务逻辑处理主体部分 --
  V_STEP := 2;
  V_STEP_DESC := '数据落地-微粒贷账户信息历史';
  V_STARTTIME := SYSDATE;
  INSERT INTO RRP_MDL.O_IML_AGT_WLD_ACCT_H
    (AGT_ID                               --协议编号
    ,LP_ID                               --法人编号
    ,ACCT_ID                             --账户编号
    ,ACCT_TYPE_CD                        --账户类型代码
  ...
  SELECT AGT_ID                               --协议编号
        ...
        ,END_DT                              --结束日期
        ,OUT_LINE_CUST_ID                    --行外客户编号
    FROM IML.V_AGT_WLD_ACCT_H;  --视图-微粒贷账户信息历史
