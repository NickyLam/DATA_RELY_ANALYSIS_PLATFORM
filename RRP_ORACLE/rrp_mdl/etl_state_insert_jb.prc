CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_STATE_INSERT_JB(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_STATE_INSERT_JB
...
AS
  ...
  V_PROC_NAME  VARCHAR2(50) := 'ETL_STATE_INSERT_JB'; --程序名称
  CURSOR C_TAB IS SELECT * FROM RRP_MDL.VIEW_LIST_FROM_HDW WHERE SUBSTR(ID,0,2) = 'JB';
BEGIN
  --处理参数及月末等判断逻辑
