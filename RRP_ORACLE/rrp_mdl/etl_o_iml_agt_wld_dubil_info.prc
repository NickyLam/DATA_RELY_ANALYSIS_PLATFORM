CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IML_AGT_WLD_DUBIL_INFO(I_P_DATE IN INTEGER,
                                                         O_ERRCODE OUT VARCHAR2
                                                         )
  /**************************************************************************
  *  程序名称：ETL_O_IML_AGT_WLD_DUBIL_INFO
  *  功能描述：微粒贷借据信息
  *  创建日期：20221222
  *  开发人员：梅炜
  *  来源表： IML.V_AGT_WLD_DUBIL_INFO
  *  目标表： O_IML_AGT_WLD_DUBIL_INFO
  *  配置表：
...
AS
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IML_AGT_WLD_DUBIL_INFO'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM RRP_MDL.O_IML_AGT_WLD_DUBIL_INFO T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_IML_AGT_WLD_DUBIL_INFO';
  ...
  -- 程序业务逻辑处理主体部分 --
  V_STEP := 2 ;
  V_STEP_DESC := '数据落地-微粒贷借据信息';
  V_STARTTIME := SYSDATE;
  INSERT INTO RRP_MDL.O_IML_AGT_WLD_DUBIL_INFO
    (AGT_ID                           --协议编号
    ,LP_ID                            --法人编号
  ...
  SELECT 
    ...
    --,JOB_CD  --任务编码
    --,ETL_TIMESTAMP  --ETL处理时间戳
    FROM IML.V_AGT_WLD_DUBIL_INFO  --视图-微粒贷借据信息
   /*WHERE CREATE_DT <= TO_DATE(V_P_DATE,'YYYYMMDD') -1*/;
  ...
  SELECT AGT_ID  --协议编号
        ...
        --,UPDATE_DT  --更新日期
        --,ETL_DT
    FROM IML.V_AGT_WLD_DUBIL_INFO_H T1
   WHERE START_DT <= TO_DATE(V_P_DATE,'YYYYMMDD')-1
     AND END_DT > TO_DATE(V_P_DATE,'YYYYMMDD')-1
     AND NOT EXISTS (SELECT 1 FROM RRP_MDL.O_IML_AGT_WLD_DUBIL_INFO T2
                      WHERE T1.DUBIL_ID = T2.DUBIL_ID);
  ...
  ETL_DBMS_STATS(V_P_DATE, 'O_IML_AGT_WLD_DUBIL_INFO', '', O_ERRCODE);
...
END ETL_O_IML_AGT_WLD_DUBIL_INFO;
/
