CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_O_IML_AGT_LOAN_RISK_CLS_H(I_P_DATE IN INTEGER,
                                                O_ERRCODE OUT VARCHAR2
                                                )
  /**************************************************************************
  *  程序名称：ETL_INIT_O_IML_AGT_LOAN_RISK_CLS_H
  *  功能描述：贷款风险分类历史
  *  创建日期：20220611
  *  开发人员：梅炜
  *  来源表：
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_INIT_O_IML_AGT_LOAN_RISK_CLS_H'; -- 程序名称
...
EXECUTE IMMEDIATE 'TRUNCATE TABLE '|| V_TABLE_NAME;
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  -- -- EXECUTE IMMEDIATE ' TRUNCATE TABLE  O_IML_AGT_LOAN_RISK_CLS_H ';
   -- EXECUTE IMMEDIATE ' TRUNCATE TABLE O_IML_AGT_LOAN_RISK_CLS_H';
  /*-- EXECUTE IMMEDIATE ('ALTER TABLE '||'O_IML_AGT_LOAN_RISK_CLS_H'||' TRUNCATE PARTITION '||'写上分区名');--分区表的重跑处理*/
 ...
 /* V_STEP := V_STEP + 1;
  V_STEP_DESC := '分区处理';
  V_STARTTIME := SYSDATE;
  ETL_PARTITION_ADD(I_P_DATE, 'O_IML_AGT_LOAN_RISK_CLS_H', '1', O_ERRCODE);
  V_SQLCOUNT := SQL%ROWCOUNT;
  ...
  */
  ...
  (
    ...
    RISK_CLS_ID  --风险分类编号
      ...
      ,SRC_TABLE_NAME  --源表名称
      ,JOB_CD  --任务编码
    FROM IML.V_AGT_LOAN_RISK_CLS_H
    ;
  ...
  END ETL_INIT_O_IML_AGT_LOAN_RISK_CLS_H;
/
