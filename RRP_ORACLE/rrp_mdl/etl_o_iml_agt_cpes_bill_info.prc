CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IML_AGT_CPES_BILL_INFO(I_P_DATE IN INTEGER,
                                                         O_ERRCODE OUT VARCHAR2
                                                         )
  /**************************************************************************
  *  程序名称：ETL_O_IML_AGT_CPES_BILL_INFO
  *  功能描述：票交所票据信息
  *  创建日期：20230215
  *  开发人员：MW
  *  来源表：
...
AS
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IML_AGT_CPES_BILL_INFO'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM RRP_MDL.O_IML_AGT_CPES_BILL_INFO T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_IML_AGT_CPES_BILL_INFO';
  ...
  ETL_DBMS_STATS(V_P_DATE, 'O_IML_AGT_CPES_BILL_INFO','', O_ERRCODE);
...
END ETL_O_IML_AGT_CPES_BILL_INFO;
/
