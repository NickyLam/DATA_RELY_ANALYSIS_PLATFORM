CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_ICL_CMM_AGENT_CONSMT_TRAN_DTL(I_P_DATE IN INTEGER,
                                                                O_ERRCODE OUT VARCHAR2
                                                                )
  /**************************************************************************
  *  程序名称：ETL_O_ICL_CMM_AGENT_CONSMT_TRAN_DTL
  *  功能描述：代理代销交易明细
  *  创建日期：20220525
  *  开发人员：梅炜
...
AS
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_ICL_CMM_AGENT_CONSMT_TRAN_DTL'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  DELETE FROM RRP_MDL.O_ICL_CMM_AGENT_CONSMT_TRAN_DTL T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  ...
  ETL_DBMS_STATS(V_P_DATE, 'O_ICL_CMM_AGENT_CONSMT_TRAN_DTL', '', O_ERRCODE);
...
END ETL_O_ICL_CMM_AGENT_CONSMT_TRAN_DTL;
/
