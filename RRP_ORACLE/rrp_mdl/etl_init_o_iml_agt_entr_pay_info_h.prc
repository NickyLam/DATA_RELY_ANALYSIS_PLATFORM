CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_O_IML_AGT_ENTR_PAY_INFO_H(I_P_DATE IN INTEGER,
                                                O_ERRCODE OUT VARCHAR2
                                                )
  /**************************************************************************
  *  程序名称：ETL_INIT_O_IML_AGT_ENTR_PAY_INFO_H
  *  功能描述：受托支付信息历史
  *  创建日期：20220611
  *  开发人员：梅炜
  *  来源表：
  *  目标表：  O_IML_AGT_ENTR_PAY_INFO_H
  *  配置表：  CODE_MAP
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_INIT_O_IML_AGT_ENTR_PAY_INFO_H'; -- 程序名称
...
EXECUTE IMMEDIATE 'TRUNCATE TABLE '|| V_TABLE_NAME;
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  -- -- EXECUTE IMMEDIATE ' TRUNCATE TABLE  O_IML_AGT_ENTR_PAY_INFO_H ';
   -- EXECUTE IMMEDIATE ' TRUNCATE TABLE O_IML_AGT_ENTR_PAY_INFO_H ';
  /*-- EXECUTE IMMEDIATE ('ALTER TABLE '||'O_IML_AGT_ENTR_PAY_INFO_H'||' TRUNCATE PARTITION '||'写上分区名');--分区表的重跑处理*/
 ...
 /* V_STEP := V_STEP + 1;
  V_STEP_DESC := '分区处理';
  V_STARTTIME := SYSDATE;
  ETL_PARTITION_ADD(I_P_DATE, 'O_IML_AGT_ENTR_PAY_INFO_H', '1', O_ERRCODE);
  V_SQLCOUNT := SQL%ROWCOUNT;
  ...
  */
  ...
  V_STEP_DESC := '数据落地-受托支付信息历史';
  V_STARTTIME := SYSDATE;
  INSERT INTO RRP_MDL.O_IML_AGT_ENTR_PAY_INFO_H
  (
      AGT_ID   --协议编号
      ,LP_ID   --法人编号
      ,PAY_FLOW_NUM   --支付流水号
     ...
     -- ,TRAN_IN_BK_BANK_NO   --转入行行号
      ...
      ,SRC_TABLE_NAME   --源表名称
      ,JOB_CD   --任务编码
    FROM IML.V_AGT_ENTR_PAY_INFO_H
    ;
  ...
  END ETL_INIT_O_IML_AGT_ENTR_PAY_INFO_H;
/
