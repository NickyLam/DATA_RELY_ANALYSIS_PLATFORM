CREATE OR REPLACE PROCEDURE RRP_MDL."ETL_INIT_O_IML_EVT_REPAY_FLOW" (I_P_DATE IN INTEGER, --跑批日期
                                                 O_ERRCODE  OUT VARCHAR2 --错误代码
                                                 )
 /*******************************************************************
  **存储过程详细说明： 还款流水表
  **存储过程名称：    ETL_INIT_O_IML_EVT_REPAY_FLOW
  **存储过程创建日期：20221129
  **存储过程创建人：  HULIJUAN
  **输入参数：        I_P_DATE
  **输出参数：        O_ERRCODE
 ...
 AS
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_INIT_O_IML_EVT_REPAY_FLOW'; -- 程序名称
...
,ETL_TIMESTAMP                                    --数据处理时间
,CALLBK_RS                                        --回收原因
    FROM IML.V_EVT_REPAY_FLOW@LINK_SIT2   --还款流水表视图
...
END ETL_INIT_O_IML_EVT_REPAY_FLOW;
/
