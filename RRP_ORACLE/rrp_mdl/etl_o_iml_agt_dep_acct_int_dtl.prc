CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IML_AGT_DEP_ACCT_INT_DTL(I_P_DATE IN INTEGER, --跑批日期
                                                           O_ERRCODE OUT VARCHAR2 --错误代码
                                                           )
 /*******************************************************************
  **存储过程详细说明： 存款账户利息明细表
  **存储过程名称：    ETL_O_IML_AGT_DEP_ACCT_INT_DTL
  **存储过程创建日期：20221130
  **存储过程创建人：  HULIJUAN
  **输入参数：        I_P_DATE
  **输出参数：        O_ERRCODE
...
AS
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IML_AGT_DEP_ACCT_INT_DTL'; --程序名称
...
BEGIN
  ...
  -- 程序业务逻辑处理主体部分 --
  V_STEP := V_STEP + 1;
  V_STEP_DESC := '数据落地-存款账户利息明细';
  V_STARTTIME := SYSDATE;
  --清理当天数据
  EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_IML_AGT_DEP_ACCT_INT_DTL';
  INSERT /*+APPEND*/ INTO RRP_MDL.O_IML_AGT_DEP_ACCT_INT_DTL NOLOGGING
    (AGT_ID                         --协议编号
    ,LP_ID                          --法人编号
    ,ACCT_ID                        --账户编号
    ,INT_CLS_CD                     --利息分类代码
  ...
  SELECT /*+PARALLEL*/
        ...
        ,JOB_CD                            --任务代码
        ,ETL_TIMESTAMP                     --数据处理时间
    FROM IML.V_AGT_DEP_ACCT_INT_DTL    --存款账户利息明细表 视图
  ...
  ETL_DBMS_STATS(V_P_DATE, 'O_IML_AGT_DEP_ACCT_INT_DTL', '', O_ERRCODE);
...
END ETL_O_IML_AGT_DEP_ACCT_INT_DTL;
/
