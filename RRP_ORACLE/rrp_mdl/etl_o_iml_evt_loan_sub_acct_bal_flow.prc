CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IML_EVT_LOAN_SUB_ACCT_BAL_FLOW(I_P_DATE IN INTEGER,
                                                                 O_ERRCODE OUT VARCHAR2
                                                                )
  /**************************************************************************
  *  程序名称：ETL_O_IML_EVT_LOAN_SUB_ACCT_BAL_FLOW
  *  功能描述：贷款分户余额变动流水
  *  创建日期：20251105
  *  开发人员：YJY
  *  来源表： IML.V_EVT_LOAN_SUB_ACCT_BAL_FLOW
  *  目标表： O_IML_EVT_LOAN_SUB_ACCT_BAL_FLOW
  *  配置表：
...
AS
  ...
  V_TAB_NAME  VARCHAR2(50) := 'O_IML_EVT_LOAN_SUB_ACCT_BAL_FLOW'; --表名
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IML_EVT_LOAN_SUB_ACCT_BAL_FLOW'; --程序名称
...
BEGIN
  ...
  -- 程序业务逻辑处理主体部分 --
  V_STEP := 2;
  V_STEP_DESC := '数据落地-贷款分户余额变动流水';
  V_STARTTIME := SYSDATE;
  INSERT INTO RRP_MDL.O_IML_EVT_LOAN_SUB_ACCT_BAL_FLOW
  (   
       EVT_ID                                --事件编号
       ,LP_ID                                --法人编号
       ,SOB_ID                               --账套编号
  ...
  SELECT 
       ...
       ,JOB_CD                               --任务编码
       ,ETL_TIMESTAMP                        --ETL处理时间戳
    FROM IML.V_EVT_LOAN_SUB_ACCT_BAL_FLOW --视图-贷款分户余额变动流水
...
END ETL_O_IML_EVT_LOAN_SUB_ACCT_BAL_FLOW;
/
