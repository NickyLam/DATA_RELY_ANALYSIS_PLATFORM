CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IML_EVT_BIGAMT_TRAN_EVT(I_P_DATE IN INTEGER, --跑批日期
                                                          O_ERRCODE OUT VARCHAR2 --错误代码
                                                          )
 /*******************************************************************
  **存储过程详细说明： 大额交易事件
  **存储过程名称：    ETL_O_IML_EVT_BIGAMT_TRAN_EVT
  **存储过程创建日期：20221201
  **存储过程创建人：  HULIJUAN
  **输入参数：        I_P_DATE
  **输出参数：        O_ERRCODE
...
AS
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IML_EVT_BIGAMT_TRAN_EVT'; --程序名称
...
BEGIN
  ...
  ETL_DBMS_STATS(V_P_DATE, 'O_IML_EVT_BIGAMT_TRAN_EVT', '', O_ERRCODE);
...
END ETL_O_IML_EVT_BIGAMT_TRAN_EVT;
/
