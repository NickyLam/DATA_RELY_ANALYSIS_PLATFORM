CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_ICL_CMM_CORP_LOAN_ACCT_INFO(I_P_DATE IN INTEGER,
                                                              O_ERRCODE OUT VARCHAR2
                                                              )
  /**************************************************************************
  *  程序名称：ETL_O_ICL_CMM_CORP_LOAN_ACCT_INFO
  *  功能描述：对公贷款账户信息
  *  创建日期：20220525
  *  开发人员：梅炜
  ...
  *  修改情况：序号  修改日期  修改人    修改原因
   *             1    20220525  梅炜     首次创建
                 2    20220615           修改参数
                 3    20230901  胡丽娟   增加资产证券化标志、资产转让标志字段
                 4    20231207  hulj     增加原始到期日期
                 5    20240506  YJY      应监管和业务要求,调整广州成达实业投资有限公司(客户号：5600006619)客户下的本金逾期金额、本金逾期天数、利息逾期金额及利息逾期天数写固定为“0”
                 6    20250415  YJY      改为分区表，并保留近一个月的历史数据
                 7    20250829  YJY      新增首次展期到期日
                 8    20251112  YJY      新增当日价差损益
  ***************************************************************************/
AS
  -- 定义变量 --
  ...
  V_TAB_NAME  VARCHAR2(100):= 'O_ICL_CMM_CORP_LOAN_ACCT_INFO'; --表名
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_ICL_CMM_CORP_LOAN_ACCT_INFO'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM RRP_MDL.O_ICL_CMM_CORP_LOAN_ACCT_INFO T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  --EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_ICL_CMM_CORP_LOAN_ACCT_INFO'; --mod BY YJY 20250415 改为分区表重跑
  --分区表的重跑处理
  ...
  (  ETL_DT                          --数据日期
    ...
    ,INIT_EXP_DT                     --原始到期日期
    ,FIR_RENEW_EXP_DT                --首次展期到期日  MOD BY YJY 20250829
    ,TD_SPD_PL                       --当日价差损益 ADD BY YJY 20251112
    )
  SELECT ETL_DT                   --数据日期
        ,LP_ID                    --法人编号
        ,ACCT_ID                  --账户编号
        ...
        ,ASSET_TRAN_DT            --资产转让日期
        ,TRAN_BF_INT_RECVBL       --转让前应收利息
        ,CASE WHEN CUST_ID = '5600006619' 
              THEN '0'
              ELSE OVDUE_FLG 
          END OVDUE_FLG           --逾期标志  MODIFY BY YJY 20240506 
        ,CASE WHEN CUST_ID = '5600006619' 
              THEN '0'
              ELSE PRIC_OVDUE_FLG  
          END PRIC_OVDUE_FLG      --本金逾期标志  MODIFY BY YJY 20240506 
        ,CASE WHEN CUST_ID = '5600006619' 
              THEN '0'
              ELSE INT_OVDUE_FLG  
          END INT_OVDUE_FLG       --利息逾期标志  MODIFY BY YJY 20240506 
        ,CASE WHEN CUST_ID = '5600006619' 
              THEN 0 
              ELSE CURR_OVDUE_PERDS  
          END CURR_OVDUE_PERDS    --当前逾期期数  MODIFY BY YJY 20240506 
        ,CASE WHEN CUST_ID = '5600006619' 
              THEN 0 
              ELSE PRIC_OVDUE_DAYS  
          END PRIC_OVDUE_DAYS      --本金逾期天数  MODIFY BY YJY 20240506 
        ,CASE WHEN CUST_ID = '5600006619' 
              THEN 0 
              ELSE INT_OVDUE_DAYS  
          END INT_OVDUE_DAYS       --利息逾期天数  MODIFY BY YJY 20240506 
        ,CASE WHEN CUST_ID = '5600006619'
              THEN 0 
              ELSE OVDUE_PRIC_BAL  
          END OVDUE_PRIC_BAL       --逾期本金余额  MODIFY BY YJY 20240506 
        ,CASE WHEN CUST_ID = '5600006619'
              THEN 0 
              ELSE OVDUE_INT_AMT 
          END OVDUE_INT_AMT         --逾期利息金额  MODIFY BY YJY 20240506 
        ,CASE WHEN CUST_ID = '5600006619' 
              THEN 0 
              ELSE OVDUE_COMP_INT_AMT
          END OVDUE_COMP_INT_AMT     --逾期复利金额  MODIFY BY YJY 20240506 
        ,CASE WHEN CUST_ID = '5600006619' 
              THEN TO_DATE('2999-12-31','YYYY-MM-DD')
              ELSE FIR_OVDUE_DT  
          END FIR_OVDUE_DT           --首次逾期日期  MODIFY BY YJY 20240506 
        ,CASE WHEN CUST_ID = '5600006619' 
              THEN TO_DATE('2999-12-31','YYYY-MM-DD') 
              ELSE PRIC_OVDUE_DT  
          END PRIC_OVDUE_DT           --本金逾期日期  MODIFY BY YJY 20240506 
        ,CASE WHEN CUST_ID = '5600006619' 
              THEN TO_DATE('2999-12-31','YYYY-MM-DD') 
              ELSE INT_OVDUE_DT  
          END INT_OVDUE_DT            --利息逾期日期  MODIFY BY YJY 20240506
        ,OVDUE_INT_RAT                --逾期利率
        ,OVDUE_INT_RAT_FLO_VAL        --逾期利率浮动值
        ,TOT_PERDS                    --总期数
        ,CURR_ISSUE_PERDS             --本期期数
        ...
        ,CASE WHEN CUST_ID = '5600006619'
              THEN 0 
              ELSE OVDUE_PRIC  
          END OVDUE_PRIC              --逾期本金   MODIFY BY YJY 20240506
        ,IDLE_PRIC                    --呆滞本金
        ,BAD_DEBT_PRIC                --呆账本金
        ,RECVBL_OVER_INT              --应收欠息
        ,RECVBL_ACRU_PNLT             --应收应计罚息
...
END ETL_O_ICL_CMM_CORP_LOAN_ACCT_INFO;
/
