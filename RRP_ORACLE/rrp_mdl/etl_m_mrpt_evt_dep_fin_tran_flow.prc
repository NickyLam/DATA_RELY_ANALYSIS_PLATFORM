CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_MRPT_EVT_DEP_FIN_TRAN_FLOW(I_P_DATE IN INTEGER,
                              O_ERRCODE OUT VARCHAR2
                               )
  /**************************************************************************
  *  程序名称：M_MRPT_EVT_DEP_FIN_TRAN_FLOW
  *  功能描述：存款金融交易流水
  *  创建日期：20221226
  *  开发人员：阳娟
  *  来源表： O_IML_EVT_DEP_FIN_TRAN_FLOW
  *  目标表： M_MRPT_EVT_DEP_FIN_TRAN_FLOW
  *  配置表：
 ...
 -- 定义变量 --
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_M_MRPT_EVT_DEP_FIN_TRAN_FLOW'; -- 程序名称
...
BEGIN
  ...
  V_TAB_NAME := 'M_MRPT_EVT_DEP_FIN_TRAN_FLOW'; --表名称
  ...
  END IF ;*/
  V_SQL :='TRUNCATE TABLE M_MRPT_EVT_DEP_FIN_TRAN_FLOW';
  EXECUTE IMMEDIATE V_SQL;
  ...
  I_STEP_DESC := '数据落地-存款金融交易流水';
  D_STARTTIME := SYSDATE;
  INSERT INTO RRP_MDL.M_MRPT_EVT_DEP_FIN_TRAN_FLOW
  (
         DATA_DT
        ,EVT_ID  --事件编号
        ,LP_ID  --法人编号
    ...
    SELECT
        ...
        ,CASH_FROM_CD
	      ...
	      ,CASH_FROM_CTY_RG_CD
	      ,CASH_TO_CTY_RG_CD
    FROM  RRP_MDL.O_IML_EVT_DEP_FIN_TRAN_FLOW  --视图-存款金融交易流水
 ...
 /* I_STEP := I_STEP + 1;
  ...
  END ETL_M_MRPT_EVT_DEP_FIN_TRAN_FLOW;
/
