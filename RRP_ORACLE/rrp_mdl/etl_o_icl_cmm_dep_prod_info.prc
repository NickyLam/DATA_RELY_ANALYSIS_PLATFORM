CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_ICL_CMM_DEP_PROD_INFO(I_P_DATE IN INTEGER,
                                                        O_ERRCODE OUT VARCHAR2
                                                        )
  /**************************************************************************
  *  程序名称：ETL_O_ICL_CMM_DEP_PROD_INFO
  *  功能描述：存款产品信息
  *  创建日期：20220611
  *  开发人员：梅炜
  *  来源表：
  ...
  *             1    20220611  梅炜      首次创建
  *             2    20240528  YJY      新增期次产品类别
  *             3    20250226  YJY      新增期次期限类型代码
  *             4    20251107  YJY      新增存款期限
  ************************************************************************/
AS
  -- 定义变量 --
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_ICL_CMM_DEP_PROD_INFO'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  DELETE FROM RRP_MDL.O_ICL_CMM_DEP_PROD_INFO T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');--普通表的重跑处理
  ...
  INSERT INTO RRP_MDL.O_ICL_CMM_DEP_PROD_INFO
    ...
    ,PD_PROD_CATE_CD      --期次产品类别代码   ADD BY YJY 20240528
    ,PD_TENOR_TYPE_CD     --期次期限类型代码   add by yjy 20250226
    ,DEP_TENOR            --存款期限  ADD BY YJY 20251107
    )
  SELECT ETL_DT               --数据日期
    ,LP_ID                --法人编号
    ,PROD_ID              --产品编号
    ...
    ,PD_TENOR_TYPE_CD     --期次期限类型代码   add by yjy 20250226
    ,DEP_TENOR            --存款期限  ADD BY YJY 20251107
    FROM ICL.V_CMM_DEP_PROD_INFO   --视图-存款产品信息
...
END ETL_O_ICL_CMM_DEP_PROD_INFO;
/
