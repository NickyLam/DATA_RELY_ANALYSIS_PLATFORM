CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_O_IOL_ICMS_FINA_REPORT(I_P_DATE IN INTEGER,
                                                O_ERRCODE OUT VARCHAR2
                                                )
  /**************************************************************************
  *  程序名称：ETL_INIT_O_IOL_ICMS_FINA_REPORT
  *  功能描述：财报簿
  *  创建日期：20220611
  *  开发人员：梅炜
  *  来源表：
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_INIT_O_IOL_ICMS_FINA_REPORT'; -- 程序名称
...
EXECUTE IMMEDIATE 'TRUNCATE TABLE '|| V_TABLE_NAME;
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  -- -- EXECUTE IMMEDIATE ' TRUNCATE TABLE  O_IOL_ICMS_FINA_REPORT ';
   -- EXECUTE IMMEDIATE ' TRUNCATE TABLE O_IOL_ICMS_FINA_REPORT';
  /*-- EXECUTE IMMEDIATE ('ALTER TABLE '||'O_IOL_ICMS_FINA_REPORT'||' TRUNCATE PARTITION '||'写上分区名');--分区表的重跑处理*/
 ...
 /* V_STEP := V_STEP + 1;
  V_STEP_DESC := '分区处理';
  V_STARTTIME := SYSDATE;
  ETL_PARTITION_ADD(I_P_DATE, 'O_IOL_ICMS_FINA_REPORT', '1', O_ERRCODE);
  V_SQLCOUNT := SQL%ROWCOUNT;
  ...
  */
  ...
  END ETL_INIT_O_IOL_ICMS_FINA_REPORT;
/
