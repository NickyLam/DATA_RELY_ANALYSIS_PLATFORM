CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_MRPT_EVT_INTELLGE_BRAC_BUS_FLOW (I_P_DATE IN INTEGER, --跑批日期
                                                 O_ERRCODE  OUT VARCHAR2 --错误代码
                                                 )
 /*******************************************************************
  **存储过程详细说明： 智能网点业务流水表
  **存储过程名称：    ETL_M_MRPT_EVT_INTELLGE_BRAC_BUS_FLOW
  **存储过程创建日期：20221213
  **存储过程创建人：  阳娟
  **输入参数：        I_P_DATE
  **输出参数：        O_ERRCODE
 ...
 AS
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_M_MRPT_EVT_INTELLGE_BRAC_BUS_FLOW'; -- 程序名称
...
BEGIN
  ...
  V_TAB_NAME := 'M_MRPT_EVT_INTELLGE_BRAC_BUS_FLOW'; --表名称

  -- 支持重跑 --
  I_STEP := 1;
  I_STEP_DESC := '-- 程序跑批开始 --';
  D_STARTTIME := SYSDATE;
  --DELETE FROM M_MRPT_EVT_INTELLGE_BRAC_BUS_FLOW T WHERE T.DATA_DT = V_P_DATE;--普通表的重跑处理
  /*SELECT COUNT(0)
    INTO V_PART_COUNT
  ...
  I_STEP_DESC := '数据落地-智能网点业务流水表';
  D_STARTTIME := SYSDATE;

 INSERT /*+APPEND*/ INTO RRP_MDL.M_MRPT_EVT_INTELLGE_BRAC_BUS_FLOW NOLOGGING
    (
      DATA_DT                                --数据日期
     ,EVT_ID                                 --事件编号
     ,BUS_FLOW_NUM                           --业务流水号
     ...
     ,JOB_CD                                 --任务代码
     ,ETL_TIMESTAMP                          --数据处理时间
    FROM RRP_MDL.O_IML_EVT_INTELLGE_BRAC_BUS_FLOW   --智能网点业务流水表
...
END ETL_M_MRPT_EVT_INTELLGE_BRAC_BUS_FLOW;
/
