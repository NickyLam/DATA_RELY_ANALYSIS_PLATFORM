CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_CPTL_REPO_AST_INFO(I_P_DATE IN INTEGER,
                                                O_ERRCODE OUT VARCHAR2
                                                )
  /**************************************************************************
  *  程序名称：ETL_M_CPTL_REPO_AST_INFO
  *  功能描述：回购业务（资产方）信息
  *  创建日期：20220608
  *  开发人员：梅炜
  *  来源表：
  ...
  *             4    20221031  XUXIAOBIN  是报送一个月，月中结清月底余额则为0，余额为0条件需在应用层添加
  *             5    20221114  hulj       增加数据重复校验
  *             6    20230816  HYF        新增SPV_NAME 用于区分债券回购中SPV部分
  ***************************************************************************/
AS
  -- 定义变量 --
  ...
  V_TAB_NAME  VARCHAR2(100) := 'M_CPTL_REPO_AST_INFO'; --表名
  V_PROC_NAME VARCHAR2(100) := 'ETL_M_CPTL_REPO_AST_INFO'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM RRP_MDL.M_CPTL_REPO_AST_INFO T WHERE T.DATA_DT = V_P_DATE;--普通表的重跑处理
  /*EXECUTE IMMEDIATE ('ALTER TABLE '||'M_CPTL_REPO_AST_INFO'||' TRUNCATE PARTITION '||'写上分区名');--分区表的重跑处理*/
  ...
  SELECT T.DATA_DT                   --数据日期
    ...
    SELECT
        ...
        ,'A'                                                AS OPR_TYP                   --经营类型
        ,A.STL_AMT                                          AS AMT                       --发生额
        ,A.CURRT_BAL                                        AS BAL                       --余额
        ,A.INT_AMT                                          AS INT                       --利息
        ,NULL                                               AS NEXT_INT_PAY_DT           --下一付息日
        ,TO_CHAR(A.BUS_DT,'YYYYMMDD')                       AS START_DT                  --起始日期 --MODIFY BY LHQ 20220617 根据源系统口径修改起始日期取值
        ...
        ,CASE WHEN A.BUS_TYPE_CD ='BT02' THEN A.DISCNT_INT_RAT
          ...
          END                                               AS APPT_RESL_OR_REPO_RATE    --约定返售或回购利率 --MODIFY 20220908 修改买断式和质押式回购利率取值 LHQ
    ...
    LEFT JOIN RRP_MDL.O_ICL_CMM_CORP_LOAN_DUBIL_INFO M --对公贷款借据信息
      ON M.BILL_ID = A.BILL_ID
     AND M.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN RRP_MDL.O_IOL_IFRS_FCT_ECL_RES_DTL D  --减值结果表
      ON D.V_ID_NUMBER = A.BILL_NUM
  ...
  SELECT TO_CHAR(A.ETL_DT,'YYYYMMDD')                    AS DATA_DT                      --数据日期
        ,A.LP_ID                                         AS LGL_REP_ID                   --法人编号
        ,NVL(SPV.PARTY_ID,A.CUST_ID)                     AS CUST_ID                      --客户编号 --MODIFY BY XUXIAOBIN 2022/07/12
        ,A.ENTRY_ORG_ID                                  AS ORG_ID                       --机构编号
        ,SUBSTR(A.BAG_ID||'.'||A.BOND_ID_COMB,1,INSTR(A.BAG_ID||'.'||A.BOND_ID_COMB,'.')-1) AS ACC_ID  --账户编号--MODIFY BY XUXIAOBIN 2022/07/12 AS ACC_ID                              --账户编号
        ,NULL                                            AS LMT_ID                       --额度编号
        ,NULL                                            AS ACC_TYP                      --账户类型
        ,A.CURR_CD                                       AS CUR                          --币种
        ...
        ,NVL(TRIM(F.N_ECL_BEFORE),B.IMPAM_PREP)          AS PRO_IMPT                     --减值准备
        ,NULL                                            AS DEPT_LINE                    --部门条线
        ,'债券回购'                                      AS DATA_SRC                     --数据来源
        ,A.TRAN_ID                                       AS TRAN_ID                      --交易编号
        ,A.BUS_ID                                        AS BUS_ID                       --业务编号
        ,A.CUST_ID                                       AS SPV_CUST_ID                  --SPV客户编号
        ,SPV.SPV_NAME                                    AS SPV_NAME                     --SPV名称
    FROM RRP_MDL.O_ICL_CMM_CAP_BOND_REPO A  --资金债券回购
    LEFT JOIN RRP_MDL.O_ICL_CMM_CAP_BUS_POST B --资金债券持仓
    ...
    LEFT JOIN RRP_MDL.O_IOL_IFRS_FCT_ECL_RES_DTL F --减值结果表
      ON F.V_ID_NUMBER = B.MAIN_ASSET_ID
     AND F.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN RRP_MDL.CODE_MAP TTA --回购类型转码
  ...
  SELECT TO_CHAR(A.ETL_DT,'YYYYMMDD')                    AS DATA_DT                      --数据日期
        ,A.LP_ID                                         AS LGL_REP_ID                   --法人编号
        ,DECODE(NVL(TRIM(D.CUST_ID),NVL(TRIM(D.SRC_PARTY_ID), '-')),'-','9620007313'
        ...
        ,CASE WHEN A.PAY_INT_PED_CD = '0M' THEN '01'--0M 按月
              ...
              ELSE '99' --其他 00 未知 0D 按日 1M 按周 4M 按4个月 --20220929 XUXIAOBIN ADD
          END                                            AS INT_CALC_MODE                --计息方式
        ,NULL                                            AS ACT_END_DT                   --实际终止日期
        ,NULL                                            AS APPT_RESL_OR_REPO_DT         --约定返售或回购日期
        ,NULL                                            AS APPT_RESL_OR_REPO_RATE       --约定返售或回购利率
        ,NULL                                            AS APPT_RESL_OR_REPO_INT        --约定返售或回购利息
        ...
        ,A.BUS_ID                                        AS BUS_ID                       --业务编号
        ,D.CUST_ID                                       AS SPV_CUST_ID                  --SPV客户号
    ...
    LEFT JOIN RRP_MDL.O_IOL_IFRS_FCT_ECL_RES_DTL F --减值结果表
     ...
     AND F.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN RRP_MDL.O_IML_EVT_IBANK_TRAN TRD --同业交易表 ADD BY XUXIAOBIN 20221107
      ON TRD.INTNAL_TRAN_NUM = A.BUS_ID
     AND TRD.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
  ...
  WITH TMP1 AS (
    SELECT DATA_DT, ACC_ID,COUNT(1)
      FROM RRP_MDL.M_CPTL_REPO_AST_INFO T
