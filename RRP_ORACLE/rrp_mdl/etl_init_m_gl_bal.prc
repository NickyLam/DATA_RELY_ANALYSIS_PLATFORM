CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_M_GL_BAL(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_INIT_M_GL_BAL
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(200) := 'ETL_INIT_M_GL_BAL'; -- 程序名称
  ...
  (SELECT A.ETL_DT                                 AS ETL_DT                        --数据日期
      ...
      /* LEFT JOIN ORG_CONFIG D
         ON A.ORG_ID = D.ORG_ID*/
       /*INNER JOIN RRP_MDL.B_GL_BAL_T B
         ON A.ETL_DT = B.ACCTING_DATE*/
    WHERE --A.CURR_CD = 'CNY'
        A.ACCT_DURAN = CASE WHEN A.ETL_DT = V_Y_END_DT THEN SUBSTR(V_P_DATE,1,4) || '-14' ELSE SUBSTR(V_DATA_DT, 0, 7) END --年末取结转后的数据，监管报送也是取的调账后的
        AND A.ORG_ID <>'892001' /*WUHB 20220418 892001 -私人银行部（财富管理部）-弃用  已经不使用固排除*/
        --AND A.DTL_SUBJ_FLG='1'/*hulj 外管局需要取汇总数据，去掉条件*/
        AND A.ETL_DT = V_DATE
  ...
  LEFT JOIN M_GL_BAL_TMP3 D
       ...
       AND D.STD_PROD_ID = T1.STD_PROD_ID
 /* LEFT JOIN ORG_CONFIG D
   ON T1.ORG_ID = D.ORG_ID*/
  ;
   V_SQLCOUNT := SQL%ROWCOUNT;
   V_SQLMSG := '返回值：['||SQLCODE||'],描述信息：'||SQLERRM;
   O_ERRCODE := '0';
  ...
  FROM (
    SELECT
     ...
     /* ,A.ORG_ID        AS ORG_ID           --02 机构号*/
      ,A.ORG_ID/*NVL(D.ORG_ID1,A.ORG_ID)*/
                       AS ORG_ID           --02 机构号
      ,A.SUBJ_ID       AS SUBJ_ID          --03 科目号
      ,A.CURR_CD       AS CUR              --04 币种
      ,A.STD_PROD_ID   AS STD_PROD_ID      --05 标准产品编号
      /*,SUM(NVL(A.END_CR_BAL,0) - NVL(A.BGN_CR_BAL,0))*/
      ,SUM(NVL(A.TD_OC_CR_BAL,0))
                       AS YEAR_DLY_AVG_CR_BAL --05 年日均贷方余额
      /*,SUM(NVL(A.END_DR_BAL,0) - NVL(A.BGN_DR_BAL,0))*/
      ,SUM(NVL(A.TD_OC_DR_BAL,0))
                       AS YEAR_DLY_AVG_DR_BAL --05 年日均借方余额
/*   FROM M_GL_BAL   A
   WHERE A.DATA_DT BETWEEN (SUBSTR(V_P_DATE,1,4) || '0101') AND V_P_DATE
   GROUP BY A.DATA_DT, A.SUBJ_ID, A.ORG_ID, A.CUR) T
  GROUP BY T.SUBJ_ID, T.ORG_ID,T.CUR;*/
      FROM  O_ICL_CMM_GL_BAL A
  /* LEFT JOIN ORG_CONFIG D
          ON A.ORG_ID = D.ORG_ID*/
...
/*  FROM (
    SELECT
       A.DATA_DT       AS DATA_DT          --01 数据日期
      ,A.ORG_ID        AS ORG_ID           --02 机构号
      ,A.SUBJ_ID       AS SUBJ_ID          --03 科目号
      ,A.CUR           AS CUR              --04 币种
      ,SUM(NVL(A.END_CR_BAL,0) - NVL(A.BGN_CR_BAL,0))
                       AS MON_DLY_AVG_CR_BAL --05 月日均贷方余额
      ,SUM(NVL(A.END_DR_BAL,0) - NVL(A.BGN_DR_BAL,0))
                       AS MON_DLY_AVG_DR_BAL --05 月日均贷方余额
   FROM M_GL_BAL   A
   WHERE A.DATA_DT BETWEEN (SUBSTR(V_P_DATE,1,6) || '01') AND V_P_DATE
   GROUP BY A.DATA_DT, A.SUBJ_ID, A.ORG_ID, A.CUR) T
  GROUP BY T.SUBJ_ID, T.ORG_ID,T.CUR;*/
  FROM (
    SELECT
      ...
      /*,SUM(NVL(A.END_DR_BAL,0) - NVL(A.BGN_DR_BAL,0))*/
      ,SUM(NVL(A.TD_OC_DR_BAL,0))
                       AS MON_DLY_AVG_DR_BAL --05 月日均借方余额
/*   FROM M_GL_BAL   A
   WHERE A.DATA_DT BETWEEN (SUBSTR(V_P_DATE,1,4) || '0101') AND V_P_DATE
   GROUP BY A.DATA_DT, A.SUBJ_ID, A.ORG_ID, A.CUR) T
  GROUP BY T.SUBJ_ID, T.ORG_ID,T.CUR;*/
...
/* LEFT JOIN ORG_CONFIG D
  ...
  WITH TMP1 AS (
    SELECT DATA_DT, FREQ,ORG_ID,SUBJ_ID,STD_PROD_ID,CUR,COUNT(1)
      FROM M_GL_BAL T
     WHERE DATA_DT = V_P_DATE
    GROUP BY DATA_DT,FREQ,ORG_ID,SUBJ_ID,STD_PROD_ID,CUR
    HAVING COUNT(1) > 1)
