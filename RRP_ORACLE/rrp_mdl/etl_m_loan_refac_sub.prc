CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_LOAN_REFAC_SUB(I_P_DATE IN INTEGER, --跑批日期
                                                 O_ERRCODE  OUT VARCHAR2 --错误代码
                                                 )
/***********************************************************************
  **  存储过程详细说明：支小再贷款子表
  **  存储过程名称:  ETL_M_LOAN_REFAC_SUB
  **  存储过程创建日期:2022-07-13
  **  存储过程创建人：MW
  **  调用方法:
       DECLARE
       ...
       BEGIN
         I_P_DATE := '20220307';
         ETL_M_LOAN_REFAC_SUB(I_P_DATE, O_ERRCODE);
       END;
  **  输入参数:   I_P_DATE
  **  输出参数:   O_ERRCODE
  **  返回值:     O_ERRCODE
...
AS
  ...
  V_TAB_NAME  VARCHAR2(100) := 'M_LOAN_REFAC_SUB'; --表名
  V_PROC_NAME VARCHAR2(100) := 'ETL_M_LOAN_REFAC_SUB'; --程序名称
...
BEGIN
  ...
  SELECT TO_CHAR(A.ETL_DT,'YYYYMMDD')          AS DATA_DT              --数据日期
         ...
         ,ROUND( (CASE WHEN A.REFAC_EXP_DT > TO_DATE(V_P_DATE,'YYYYMMDD')  
                       THEN TO_DATE(V_P_DATE,'YYYYMMDD')  
                       ELSE A.REFAC_EXP_DT 
                   END
                  - CASE WHEN A.REFAC_DISTR_DT < TRUNC(TO_DATE(V_P_DATE,'YYYYMMDD') ,'Q') - 1 
                         THEN TRUNC(TO_DATE(V_P_DATE,'YYYYMMDD') ,'Q') - 1 
                         ELSE A.REFAC_DISTR_DT 
                     END) * A.REFAC_AMT * A.USE_INT_RAT / 365, 2)                                
                                               AS PBL_INT      --再贷款应付利息
   ...
   WHERE A.REFAC_DISTR_DT <= TO_DATE(V_P_DATE,'YYYYMMDD')  --再贷款发放日期 陈明明确认的 卡发放日期小于等于季末 到期日期大于等于季末
     AND A.REFAC_EXP_DT >= TO_DATE(V_P_DATE,'YYYYMMDD')   --再贷款到期日期
  ...
  WITH TMP1 AS (
    SELECT DATA_DT,DUBIL_ID,PMO_CONT_ID,REFAC_CONT_ID,COUNT(1)
      FROM RRP_MDL.M_LOAN_REFAC_SUB T
