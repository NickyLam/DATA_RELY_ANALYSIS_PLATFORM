CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_GUA_CONT_INFO_YBT(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_M_GUA_CONT_INFO_YBT
  *  功能描述：担保合同信息-一表通
  *  创建日期：20251029
  *  开发人员：wyx
  *  来源表：
  *  目标表：  M_GUA_CONT_INFO_YBT
  *  配置表：  CODE_MAP
...
AS
  ...
  V_TAB_NAME          VARCHAR2(100) := 'M_GUA_CONT_INFO_YBT';      --表名
  V_PROC_NAME         VARCHAR2(100) := 'ETL_M_GUA_CONT_INFO_YBT';  --程序名称
  V_SYSTEM            VARCHAR2(30):= '监管报送';               --来源系统 --默认写监管报送系统，有真实来源的按实际写
BEGIN
  -- 处理参数及月末等判断逻辑 --
  V_P_DATE    := TO_CHAR(I_P_DATE); --获取跑批日期
  V_DATE      := TO_DATE(V_P_DATE,'YYYYMMDD');
  V_START_DT  := TRUNC(V_DATE,'Y');
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM M_GUA_CONT_INFO_YBT T WHERE T.DATA_DT = V_P_DATE;--普通表的重跑处理
  /*EXECUTE IMMEDIATE ('ALTER TABLE '||'M_GUA_CONT_INFO_YBT'||' TRUNCATE PARTITION '||'写上分区名');--分区表的重跑处理*/
  ...
  V_STARTTIME := SYSDATE;
  EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.M_GUA_CONT_INFO_YBT_TMP';
  COMMIT;

  --担保合同信息--被担保业务类型
  INSERT INTO RRP_MDL.M_GUA_CONT_INFO_YBT_TMP(CONT_ID,CONT_TYPE,EXP_DT,PAYOFF_DT,CONT_MGMT_ID)

  WITH TMP_LOAN_LMT_CONT AS (
  ...
  SELECT A.GUAR_CONT_ID,
         MIN(COALESCE(B.CONT_TYPE,C.CONT_TYPE,D.CONT_TYPE)) AS CONT_TYPE,
   ...
   INNER JOIN TMP_LOAN_LMT_CONT B        --业务合同
      ON B.CONT_ID = A.LOAN_CONT_ID
    LEFT JOIN TMP_LOAN_CONT_LMT_RELA D   --业务合同：与授信合同关系
      ON A.LOAN_CONT_ID = D.LMT_CONT_ID
    LEFT JOIN TMP_DUBIL_INFO C           --借据合同
  ...
  V_STARTTIME := SYSDATE;
  --担保合同信息--被担保业务类型
  INSERT INTO RRP_MDL.M_GUA_CONT_INFO_YBT_TMP
  ...
  V_STARTTIME := SYSDATE;
  EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.M_GUA_CONT_INFO_YBT_TEMP01';
  --普通贷款
  INSERT INTO RRP_MDL.M_GUA_CONT_INFO_YBT_TEMP01
    ( RCPT_ID              --借据号
     ,CONT_ID              --合同号
     ,BILL_NO              --票据号
     ,LMT_CONT_ID          --授信合同号
  ...
  INSERT INTO RRP_MDL.TMP_RATE (CONT_ID, COLLATERAL_RATIO,COLLATERAL_RATIO_ORIG)
    WITH LMT_CONT_RELA AS  --额度合同和业务合同的关系
      (SELECT /*+MATERIALIZE*/A1.CONT_ID,A1.LMT_CONT_ID
       ...
       SELECT DISTINCT CONT_ID,LMT_CONT_ID
         FROM RRP_MDL.M_GUA_CONT_INFO_YBT_TEMP01 TA
    ...
    GUAR_CONT_BALANCE_SUM AS --担保合同下所有借据的本金余额总额
      (SELECT /*+MATERIALIZE*/A.GUAR_CONT_ID
        ...
        INNER JOIN RRP_MDL.M_GUA_CONT_INFO_YBT_TEMP01 D --所有借据的余额整合
  ...
  V_STARTTIME := SYSDATE;
  INSERT INTO RRP_MDL.M_GUA_CONT_INFO_YBT
    (DATA_DT            --数据日期
    ,LGL_REP_ID         --法人编号
    ,GUA_CONT_ID        --担保合同号
    ,ORG_ID             --机构编号
  ...
  WITH TMP1 AS (
    --ADD BY 20240321 获取当年销户或是余额不为0的业务/额度合同
    SELECT CONT_ID FROM RRP_MDL.M_GUA_CONT_INFO_YBT_TEMP01
     WHERE (CLOS_ACCT_DT >= TRUNC(TO_DATE(V_P_DATE,'YYYYMMDD'),'Y') OR LOAN_BAL <> 0)
     UNION ALL
    SELECT LMT_CONT_ID FROM RRP_MDL.M_GUA_CONT_INFO_YBT_TEMP01
     WHERE (CLOS_ACCT_DT >= TRUNC(TO_DATE(V_P_DATE,'YYYYMMDD'),'Y') OR LOAN_BAL <> 0)),
  ...
  SELECT V_P_DATE                                   AS DATA_DT            --数据日期
   ...
   INNER JOIN RRP_MDL.M_GUA_CONT_INFO_YBT_TMP C --被担保业务类型   20251016
  ...
  SELECT V_P_DATE                                  AS DATA_DT             --数据日期
    ...
    FROM RRP_MDL.O_ICL_CMM_UNITE_WL_GUAR_CONT_INFO A  --联合网贷担保合同信息
   INNER JOIN RRP_MDL.M_GUA_CONT_INFO_YBT_TMP C --被担保业务类型
      ON C.CONT_ID = A.GUAR_CONT_ID
    LEFT JOIN (SELECT B.GUARTOR_ID,MAX(CURR_CD) AS CURR_CD,MAX(GUARTOR_NET_ASSET_AMT) AS GUARTOR_NET_ASSET_AMT
                 FROM RRP_MDL.O_ICL_CMM_COL_GUARTOR_RATING_INFO B --押品保证人评级信息
                WHERE B.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
  ...
  IF V_P_DATE = TO_CHAR(LAST_DAY(TO_DATE(V_P_DATE,'YYYYMMDD')),'YYYYMMDD') THEN
    --ADD BY 20240229 插入当月内被删除的担保合同数据
    V_STEP := V_STEP + 1;
    V_STEP_DESC := '插入当年内被删除的担保合同数据';
    V_STARTTIME := SYSDATE;
    INSERT INTO RRP_MDL.M_GUA_CONT_INFO_YBT
      (DATA_DT              --数据日期
      ,LGL_REP_ID           --法人编号
     ...
     SELECT T.*,ROW_NUMBER() OVER(PARTITION BY T.GUA_CONT_ID ORDER BY T.DATA_DT DESC) RN
       FROM RRP_MDL.M_GUA_CONT_INFO_YBT T
       LEFT JOIN RRP_MDL.M_GUA_CONT_INFO_YBT TB
