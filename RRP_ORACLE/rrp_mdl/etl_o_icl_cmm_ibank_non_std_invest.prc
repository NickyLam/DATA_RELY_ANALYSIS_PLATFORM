CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_ICL_CMM_IBANK_NON_STD_INVEST(I_P_DATE IN INTEGER,
                                                               O_ERRCODE OUT VARCHAR2
                                                               )
  /**************************************************************************
  *  程序名称：ETL_O_ICL_CMM_IBANK_NON_STD_INVEST
  *  功能描述：同业非标投资
  *  创建日期：20220525
  *  开发人员：梅炜
  *  来源表：
...
AS
  ...
  V_TAB_NAME  VARCHAR2(50) := 'O_ICL_CMM_IBANK_NON_STD_INVEST'; --表名
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_ICL_CMM_IBANK_NON_STD_INVEST'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM RRP_MDL.O_ICL_CMM_IBANK_NON_STD_INVEST T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  --EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_ICL_CMM_IBANK_NON_STD_INVEST';
  ...
  INSERT INTO RRP_MDL.O_ICL_CMM_IBANK_NON_STD_INVEST
    ...
    ,FINAL_DIR_INDUS_MIDDLE_CLASS   --最终投向行业_中类  add by yjy 20241113
  ...
  SELECT TO_DATE(V_P_DATE,'YYYYMMDD') AS ETL_DT  --数据日期
    ...
    FROM (SELECT /*+PARALLEL */ROW_NUMBER() OVER(PARTITION BY OBJ_ID ORDER BY A.ETL_DT DESC) RN,A.*
            FROM ICL.V_CMM_IBANK_NON_STD_INVEST A --视图-同业非标投资
...
END ETL_O_ICL_CMM_IBANK_NON_STD_INVEST;
/
