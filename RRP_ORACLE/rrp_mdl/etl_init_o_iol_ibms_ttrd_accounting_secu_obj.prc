CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_O_IOL_IBMS_TTRD_ACCOUNTING_SECU_OBJ(I_P_DATE IN INTEGER,
                              O_ERRCODE OUT VARCHAR2
                               )
  /**************************************************************************
  *  程序名称：ETL_INIT_O_IOL_IBMS_TTRD_ACCOUNTING_SECU_OBJ
  *  功能描述：中国债券信用评级表
  *  创建日期：20220820
  *  开发人员：梅炜
  *  来源表： IOL.V_WIND_CBONDRATING
  *  目标表： O_IOL_IBMS_TTRD_ACCOUNTING_SECU_OBJ
  *  配置表：
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_INIT_O_IOL_IBMS_TTRD_ACCOUNTING_SECU_OBJ'; -- 程序名称
...
EXECUTE IMMEDIATE 'TRUNCATE TABLE '|| V_TABLE_NAME;
 ...
 --清理当天数据
  V_STEP_DESC  := '清理当天数据';
   -- EXECUTE IMMEDIATE ' TRUNCATE TABLE RRP_MDL.O_IOL_IBMS_TTRD_ACCOUNTING_SECU_OBJ';

  V_STEP_DESC  := '装入目标表';
  INSERT /*+APPEND*/ INTO RRP_MDL.O_IOL_IBMS_TTRD_ACCOUNTING_SECU_OBJ NOLOGGING
    ( OBJ_ID,                       --对象ID
      TSK_ID,                       --任务ID
      BEG_DATE,                     --开始日期
      END_DATE,                     --结束日期
  ...
  SELECT /*+PARALLEL*/
          ...
          PRFT_IR_FUT_AI,               --预收息利息收入
          ETL_DT                        --ETL处理日期
    FROM IOL.V_IBMS_TTRD_ACCOUNTING_SECU_OBJ   --券核算余额表_视图
   ;
 ...
 V_SQLCOUNT := SQL%ROWCOUNT;
  ...
  END ETL_INIT_O_IOL_IBMS_TTRD_ACCOUNTING_SECU_OBJ;
/
