CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_STATE_INSERT_ICL(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_STATE_INSERT_ICL
...
AS
  ...
  V_PROC_NAME  VARCHAR2(50) := 'ETL_STATE_INSERT_ICL'; --程序名称
  CURSOR C_TAB IS SELECT * FROM RRP_MDL.VIEW_LIST_FROM_HDW WHERE SCH_NAME = 'ICL';
BEGIN
  --处理参数及月末等判断逻辑
  ...
  FOR TAB IN C_TAB LOOP
    ...
    FINISH_FLG   := 0;
    M_PROC_NAME  := 'ETL_O_ICL_'||V_TABLE_NAME;
