CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_LOAN_CONT_INFO_BACK20251110(I_P_DATE IN INTEGER, --跑批日期
...
BEGIN
  ...
  SELECT  V_P_DATE                                    AS DATA_DT                    --数据日期
         ...
         ,C.HIGH_TECH_SERV_LOAN_FLG                   AS HIGH_TECH_SERV_LOAN_FLG             --高技术服务业贷款标志       ADD BY YJY 20240829
  ...
  WITH ADD_LOAN_NET_COOP_SUB_TMP AS ( --只拿信用的
        SELECT DISTINCT COOP_AGRT_ID
              ,TO_DATE(AGRT_START_DT||' 000000','YYYYMMDD HH24MISS') AS AGRT_START_DT
              ,CASE WHEN COOP_AGRT_ID = 'XZI32018000164'
                    THEN TO_DATE(ACT_END_DT||' 235959','YYYYMMDD HH24MISS')-1
                    ELSE TO_DATE(ACT_END_DT||' 235959','YYYYMMDD HH24MISS')
                END  AS ACT_END_DT
              ,DATA_SRC
              ,COOP_PROD
              ,SUBSTR(COOP_PROD, 1, INSTR(COOP_PROD, '、') - 1) AS COOP_PROD1
              ,SUBSTR(COOP_PROD, INSTR(COOP_PROD, '、') + 1)    AS COOP_PROD2
         FROM RRP_MDL.ADD_LOAN_NET_COOP_SUB --互联网贷款合作协议静态表
        WHERE DBFS IS NULL),
  ADD_LOAN_NET_COOP_SUB_TMP1 AS (
        SELECT COOP_AGRT_ID,AGRT_START_DT,ACT_END_DT,DATA_SRC,COOP_PROD1,COOP_PROD2
  ...
  SELECT  V_P_DATE                                    AS DATA_DT                    --数据日期
    ...
    INNER JOIN RRP_MDL.O_ICL_CMM_UNITE_WL_LOAN_CONT_INFO A1 --联合网贷贷款合同信息
     ...
     AND A1.STD_PROD_ID = '203050100001'
    ...
    LEFT JOIN RRP_MDL.O_ICL_CMM_UNITE_WL_LMT_INFO C --授信额度信息
     ...
     AND C.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN ADD_LOAN_NET_COOP_SUB_TMP D
      ON D.COOP_PROD1 = A.STD_PROD_ID
     AND D.AGRT_START_DT <= A.DISTR_DT
     AND D.ACT_END_DT >= A.DISTR_DT
     AND D.COOP_PROD1 IS NOT NULL
    LEFT JOIN ADD_LOAN_NET_COOP_SUB_TMP E
      ON E.COOP_PROD2 = A.STD_PROD_ID
     AND E.AGRT_START_DT <= A.DISTR_DT
     AND E.ACT_END_DT >= A.DISTR_DT
     AND E.COOP_PROD2 IS NOT NULL
    LEFT JOIN ADD_LOAN_NET_COOP_SUB_TMP1 DD
      ON DD.COOP_PROD1 = A.STD_PROD_ID
     AND DD.RN = 1
     AND DD.COOP_PROD1 IS NOT NULL
    LEFT JOIN ADD_LOAN_NET_COOP_SUB_TMP1 EE
      ON EE.COOP_PROD2 = A.STD_PROD_ID
     AND EE.RN = 1
     AND EE.COOP_PROD2 IS NOT NULL
   ...
   WHERE A.DUBIL_STATUS_CD NOT IN ('2','5') --失败、撤销
     AND A.STD_PROD_ID = '203050100001' --203050100001-微业贷
  ...
  WITH CMM_RETL_LOAN_DUBIL_INFO_TMP AS ( --ADD BY YJY 20250718 通过借据号关联信贷标签表，判断海大集团、兴采贷（大参林）互联网业务
         SELECT DISTINCT
          ...
          /*LEFT*/ INNER JOIN RRP_MDL.O_IOL_ICMS_TAG_TERM_FINAL_DATA B --标签值最终表
          ...
          /*LEFT*/ INNER JOIN RRP_MDL.O_IOL_ICMS_TAG_CODE_CONFIG C --标签码值配置表
            ON C.TAGID = B.TAGID --标签编号
           AND C.ITEMNO = B.TAGVALUE --标签值
           AND C.ITEMNAME = '是'
           AND C.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
         WHERE /*((A.STD_PROD_ID IN ('201020100062','201020100061') AND B.OBJECTNO IS NOT NULL) --201020100062 饲料e贷-海大集团、201020100061兴采贷（大参林）
               OR (A.STD_PROD_ID IN ('201010300040','201020100059') AND A.DUE_DILIGENCE_FLG <> '1')) */--华兴好易贷（信用） 201010300040,华兴好易贷（经营-担保）201020100059判断尽调标志为否的则为互联网贷款业务
              A.STD_PROD_ID = '201020100062'  --201020100062 饲料e贷-海大集团  --兴采贷（大参林）没有互联网合作协议，不做数据加工
  ...
  SELECT /*+MATERIALIZE*/
         ...
         CASE WHEN SUBSTR(TB.LOAN_TYPE_CD,1,2) = '00' AND SUBSTR(TB.LOAN_TYPE_CD,7,1) = '1' --网商贷（债权直转）
              THEN CASE WHEN T.GUARTOR_NAME = '商诚融资担保有限公司' THEN '2023081859072'
                        ...
                        WHEN T.GUARTOR_NAME = '重庆进出口融资担保有限公司' THEN 'CQEXG融-拓-1-2022-019'
                        WHEN T.GUARTOR_NAME = '中证信用融资担保有限公司' THEN 'GDHX-ZZ-JHZD-2024-01'
                    END --网商贷债权直转数据
              WHEN TB.PROD_ID = '202020200004' AND SUBSTR(TB.LOAN_TYPE_CD,1,2) = '14'
                   AND SUBSTR(TB.LOAN_TYPE_CD,4,1) = '2' AND SUBSTR(TB.LOAN_TYPE_CD,7,1) = '1'
              THEN CASE WHEN T.GUARTOR_NAME = '商诚融资担保有限公司' THEN 'WSD20230920001;WSD20230920004'
                        WHEN T.GUARTOR_NAME = '中国投融资担保股份有限公司' THEN 'WSD20230920001;2024-A01-ZTB-026-DBHZ7'
                        WHEN T.GUARTOR_NAME = '重庆进出口融资担保有限公司' THEN 'WSD20230920001;CQEXG融-拓-1-2022-019'
                        WHEN T.GUARTOR_NAME = '中证信用融资担保有限公司' THEN 'WSD20230920001;GDHX-ZZ-JHZD-2024-01'
                    END --均衡助贷
  ...
  SELECT  V_P_DATE                                    AS DATA_DT                    --数据日期
    ...
    LEFT JOIN WSD_COOP_AGRT_ID T6 --ADD BY LIP 20250731 网商贷保证方式的互联网合作协议
     ...
     AND T6.RN = 1
   WHERE A.DUBIL_STATUS_CD NOT IN ('2','5') --失败、撤销
     AND A.STD_PROD_ID <> '203050100001' --MOD BY YJY 20250219 剔除产品微业贷的数据
     AND A.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  ...
  INSERT /*+APPEND PARALLEL*/ INTO RRP_MDL.M_LOAN_CONT_INFO
    ...
    WITH TMP1 AS (
          SELECT TLR_NO AS TELLER_ID
                ,EMP_ID AS CLERK_ID
                ,TO_DATE(UPDATE_DT,'YYYYMMDD') AS DIMISSION_DT
           FROM RRP_MDL.ADD_EMP_TLR
          UNION ALL
          SELECT TELLER_ID
                ,CLERK_ID
                ,DIMISSION_DT
           FROM RRP_MDL.O_ICL_CMM_CLERK_INFO
          WHERE TRIM(TELLER_ID) IS NOT NULL
...
END ETL_M_LOAN_CONT_INFO_BACK20251110;
/
