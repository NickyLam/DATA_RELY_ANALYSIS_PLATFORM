CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_O_IOL_IBMS_TTRD_INSTRUMENT_EXTEND(I_P_DATE IN INTEGER, --跑批日期
                                                 O_ERRCODE  OUT VARCHAR2 --错误代码
                                                 )
 /*******************************************************************
  **存储过程详细说明： 债券补录信息表
  **存储过程名称：    ETL_INIT_O_IOL_IBMS_TTRD_INSTRUMENT_EXTEND
  **存储过程创建日期：20220707
  **存储过程创建人：  赖海强
  **输入参数：        I_P_DATE
  **输出参数：        O_ERRCODE
 ...
 AS
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_INIT_O_IOL_IBMS_TTRD_INSTRUMENT_EXTEND'; -- 程序名称
...
EXECUTE IMMEDIATE 'TRUNCATE TABLE '|| V_TABLE_NAME;
  ...
  --清理当天数据
  -- EXECUTE IMMEDIATE ' TRUNCATE TABLE RRP_MDL.O_IOL_IBMS_TTRD_INSTRUMENT_EXTEND';

  INSERT /*+APPEND*/ INTO RRP_MDL.O_IOL_IBMS_TTRD_INSTRUMENT_EXTEND NOLOGGING
    (I_CODE  --金融工具代码
    ,A_TYPE  --资产类型
    ,M_TYPE  --市场类型
  ...
  SELECT /*+PARALLEL*/
      ...
      ,MTR_DATE_CONV  --到期日调整规则
      ,ETL_DT  --ETL处理日期
    FROM IOL.V_IBMS_TTRD_INSTRUMENT_EXTEND   --待定_视图
   ;
 -- 如需要分析表，请用如下代码 --
...
END ETL_INIT_O_IOL_IBMS_TTRD_INSTRUMENT_EXTEND;
/
