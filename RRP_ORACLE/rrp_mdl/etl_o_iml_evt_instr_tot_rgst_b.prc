CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IML_EVT_INSTR_TOT_RGST_B(I_P_DATE IN INTEGER, --跑批日期
...
AS
  ...
  V_TAB_NAME  VARCHAR2(50) := 'O_IML_EVT_INSTR_TOT_RGST_B'; --表名
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IML_EVT_INSTR_TOT_RGST_B'; --程序名称
...
BEGIN
  ...
  -- 程序业务逻辑处理主体部分 --
  V_STEP := 2;
  V_STEP_DESC := '数据落地-指令汇总登记簿';
  V_STARTTIME := SYSDATE;
  INSERT /*+APPEND*/ INTO RRP_MDL.O_IML_EVT_INSTR_TOT_RGST_B NOLOGGING
    (EVT_ID                      --事件编号
    ,LP_ID                       --法人编号
    ,SEQ_NUM                     --序号
    ,TRAN_DT                     --交易日期
    ...
    ,DTL_INSTR_TYPE_CD           --明细指令类型代码
    ,ACCT_BILL_FLOW_NUM          --账单流水号
    ,DEBIT_CRDT_DIR_FLG          --借贷方向标志
    ,TRAN_KIND_CD                --交易种类代码
    ,CURR_CD                     --币种代码
    ,AMT                         --金额
    ,INVO_ACCT_ID                --涉及账户编号
    ...
    ,TRAN_CODE                   --交易码
    ,TRAN_ORG_ID                 --交易机构编号
    ,COMN_ENTRY_FLG              --通用记账标志
    ,SUB_TRAN_SEQ_NUM            --子交易序号
    ,BAL_CHG_TYPE_CD             --余额变动类型代码
    ,STRK_BAL_FLG                --冲账标志
    ,ETL_DT                      --ETL处理日期
    ,SRC_TABLE_NAME              --源表名称
    ,JOB_CD                      --任务编码
  ...
  SELECT /*+PARALLEL*/
        ...
        ,SRC_TABLE_NAME              --源表名称
        ,JOB_CD                      --任务编码
    FROM IML.V_EVT_INSTR_TOT_RGST_B   --指令汇总登记簿_视图
...
END ETL_O_IML_EVT_INSTR_TOT_RGST_B;
/
