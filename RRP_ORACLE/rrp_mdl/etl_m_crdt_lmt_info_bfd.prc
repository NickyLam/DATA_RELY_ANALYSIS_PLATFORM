CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_CRDT_LMT_INFO_BFD(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_M_CRDT_LMT_INFO_BFD
  *  功能描述：授信额度主表、授信额度子表
  *  创建日期：20220524
  *  开发人员：梅炜
  *  来源表：
  ...
  *   01    20220524  梅炜     首次创建
      ...
      12    20231228  LYH      网商贷资本新规整改授信额度为失效，改为单笔单批，授信额度、已用额度等于放款金额
      13    20240417  YJY      按业务要求，个人消费信用贷（新兴贷2.0）产品授信额度前期业务确认按照单笔单批报送，授信额度、已用额度等于放款金额
	    14    20250910  YJY      新增零售360借条的授信加工，该产品同一个客户只有一笔有效的授信和批复，会有多笔业务合同和出账，故从零售授信额度审批信息取值
                               业务方芳确认，如果存在借据未结清但其授信编号或者批复编号失效的情况时，取其借据号为授信合同，其借据金额为授信额度（对此数据做兜底）
  *   15    20260408  YJY      参考公共授信表，补充金数授信表相关逻辑
  *   16    20260413  YJY      一表通严希婧反馈：203050100002-微众对公联合贷的取联合贷额度信息表的LMT_RELA_APPL_ID--额度关联申请编号（行内授信编号），其他产品取LMT_CONT_ID--额度合同编号
  *************************************************************************/
  AS
  -- 定义变量 --
  V_STEP      INTEGER := 0; -- 处理步骤
  V_PROC_NAME VARCHAR2(100) := 'ETL_M_CRDT_LMT_INFO_BFD'; -- 程序名称
...
BEGIN
  ...
  V_TAB_NAME  := 'M_CRDT_LMT_INFO_BFD'; --表名
  V_TAB_NAME2 := 'M_CRDT_LMT_SUB_BFD'; --表名
  ...
  SELECT TRUNC(TO_DATE(V_YESTADAY,'YYYYMMDD'), 'MM') INTO V_MONTH_START_DATE FROM DUAL;
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  DELETE FROM M_CRDT_LMT_INFO_BFD T WHERE T.DATA_DT = V_YESTADAY;--普通表的重跑处理
  DELETE FROM M_CRDT_LMT_SUB_BFD T WHERE T.DATA_DT = V_YESTADAY;
  ...
  V_STEP_DESC := '分区处理';
  V_STARTTIME := SYSDATE;
  ETL_PARTITION_ADD(V_YESTADAY, V_TAB_NAME2, '1', O_ERRCODE);
  V_SQLCOUNT := SQL%ROWCOUNT;
  V_SQLMSG := '返回值：['||SQLCODE||'],描述信息：'||SQLERRM;
  V_ENDTIME := SYSDATE;
  COMMIT;
  ...
  SELECT /*+USE_HASH(B1,A,C,D,O)*/
   ...
   INNER JOIN RRP_MDL.O_ICL_CMM_CORP_LOAN_DUBIL_INFO B --对公贷款借据信息
    ...
    LEFT JOIN RRP_MDL.O_IOL_IFRS_VAL_RPT_TRADE O --估值报告表
     ...
     AND O.ETL_DT = TO_DATE(V_YESTADAY,'YYYYMMDD')
  ...
  SELECT /*+USE_HASH(A,C,F2,D,DD,O)*/
         ...
         ,A.CNTPTY_ID                      AS CUST_ID              --客户号 --MOD BY YJY 20260408取交易对手的客户编号
         ,TRIM(E.CUST_ID)                  AS LMT_CUST_ID          --授信客户号
         ,A.ACCT_INSTIT_ID                 AS ORG_ID               --机构号
         ,B.BUS_BREED_ID                   AS LOAN_PROD_ID         --产品号
         ,NVL(A.CURR_CD, 'CNY')            AS CUR                  --币种
         ,A.FAC_VAL_AMT                    AS LOAN_AMT             --放款金额
         ,CASE WHEN B.DISTR_DT < TO_DATE(V_YESTADAY,'YYYYMMDD') AND B.PAYOFF_DT = TO_DATE(V_YESTADAY,'YYYYMMDD') AND J.PAYOFF_FLG = '0'
               THEN ROUND((NVL(A.CURRT_BAL, 0)),2)
               WHEN NVL(A.CURRT_BAL, 0) > 0 AND B.PAYOFF_DT > TO_DATE(V_YESTADAY,'YYYYMMDD')
               THEN ROUND((NVL(A.CURRT_BAL, 0)),2)
               ELSE 0
           END                              AS LOAN_BAL             --贷款余额
         ,CASE WHEN B.DISTR_DT < TO_DATE(V_YESTADAY,'YYYYMMDD') AND B.PAYOFF_DT = TO_DATE(V_YESTADAY,'YYYYMMDD') AND J.PAYOFF_FLG = '0'
               ...
               ELSE 0
           END                              AS INT_ADJ              --利息调整
         ,CASE WHEN V_P_DATE <= '20210630' AND NVL(A.CURRT_BAL, 0) = 0 AND B.PAYOFF_DT <= TO_DATE(V_YESTADAY,'YYYYMMDD') THEN 0
               WHEN B.DISTR_DT < TO_DATE(V_YESTADAY,'YYYYMMDD') AND B.PAYOFF_DT = TO_DATE(V_YESTADAY,'YYYYMMDD') AND J.PAYOFF_FLG = 0
   ...
   INNER JOIN RRP_MDL.O_ICL_CMM_CORP_LOAN_DUBIL_INFO B --对公贷款借据信息
    ...
    LEFT JOIN RRP_MDL.O_ICL_CMM_BILL_CENTER_INFO J --票据中心信息
     ...
     AND J.ETL_DT = TO_DATE(V_YESTADAY,'YYYYMMDD')
  ...
  SELECT /*+USE_HASH(A,B,C,D)*/
         ...
         ,NULL                              AS LMT_CUST_ID          --授信客户号 表外的不用算转授信 */
         ,NVL(TRIM(A.CUST_ID),TRIM(C.CUST_ID))  AS CUST_ID          --客户号 MOD BY YJY 20260408
         ,TRIM(E.CUST_ID)                   AS LMT_CUST_ID          --授信客户号 MOD BY YJY 20260408
    ...
    INNER JOIN RRP_MDL.O_ICL_CMM_BA_ACCT_INFO B --银承账户信息
     ...
     LEFT JOIN RRP_MDL.O_ICL_CMM_CORP_LOAN_CONT_INFO D  --对公贷款合同信息表
      ...
      AND D.ETL_DT = TO_DATE(V_YESTADAY,'YYYYMMDD')
     --ADD BY YJY 20260408 新增对公合同表关联，取额度合同客户号
     LEFT JOIN RRP_MDL.O_ICL_CMM_CORP_LOAN_CONT_INFO E --对公贷款合同信息表
       ON E.CONT_ID = D.LMT_CONT_ID
  ...
  SELECT /*+USE_HASH(A,B,C,D)*/
     ...
     LEFT JOIN RRP_MDL.O_ICL_CMM_CORP_LOAN_CONT_INFO D  --对公贷款合同信息表
       ON D.CONT_ID = A.CONT_ID
      AND D.ETL_DT = TO_DATE(V_YESTADAY,'YYYYMMDD')
    WHERE NVL(A.DISTR_DT,TO_DATE('00010101','YYYYMMDD')) = TO_DATE(V_YESTADAY,'YYYYMMDD') --发放日=当天
      AND A.ETL_DT = TO_DATE(V_YESTADAY,'YYYYMMDD');
  ...
  SELECT  V_YESTADAY                      AS DATA_DT                    --数据日期
    ...
    FROM ( --modify by tangan at 20230214 对公已用额度取借据余额之和
             ...
             WHERE A.ETL_DT = TO_DATE(V_YESTADAY,'YYYYMMDD')
               AND TRIM(C.LMT_CONT_ID) IS NOT NULL ) T
  ...
  INSERT INTO RRP_MDL.M_CRDT_LMT_INFO_BFD_TEMP02
    ...
    ,GROUP_CRDT_FLG                  --36集团授信标志 --ADD BY YJY 20260408
    ,STD_PROD_ID                     --37标准产品编号 --ADD BY YJY 20260408
    ,APPL_WAY_CD                     --38申请方式代码 --ADD BY YJY 20260408
    ,APPROVE_SERIAL_FLOW_NUM         --39被动转授信集团批复流水号 --ADD BY YJY 20260408
    ,APV_FLOW_NUM                    --40批复流水号 --ADD BY YJY 20260408
    ,LMT_UNDER_SELLBL_PROD_ID        --41可售产品 --ADD BY YJY 20260408
    ,APV_AMT                         --42批复金额 --ADD BY YJY 20260408
    ,STATUS_CD                       --43状态代码 --ADD BY YJY 20260408
    ,CRDT_APVED_DT                   --44授信审批日期  --ADD BY YJY 20260408
     )
 ...
 ,LMT_INTO_TEMP07 AS (
  ...
  SELECT  /*+USE_HASH(A,C,D,E,F,TA,B)*/
         ...
         ,D.GROUP_CRDT_FLG                                 AS GROUP_CRDT_FLG             --36集团授信标志  --ADD BY YJY 20260408
         ,A.STD_PROD_ID                                    AS STD_PROD_ID                --37标准产品编号  --ADD BY YJY 20260408
         ,I.APPL_WAY_CD                                    AS APPL_WAY_CD                --38申请方式代码   --ADD BY YJY 20260408
         ,K.APPROVE_SERIAL_FLOW_NUM                        AS APPROVE_SERIAL_FLOW_NUM    --39被动转授信集团批复流水号  --ADD BY YJY 20260408
         ,A.APV_FLOW_NUM                                   AS APV_FLOW_NUM               --40批复流水号  --ADD BY YJY 20260408
         ,I.LMT_UNDER_SELLBL_PROD_ID                       AS LMT_UNDER_SELLBL_PROD_ID   --41可售产品  --ADD BY YJY 20260408
         ,D.CRDT_APV_AMT                                   AS APV_AMT                    --42批复金额  --ADD BY YJY 20260408
   ...
   INSERT INTO RRP_MDL.M_CRDT_LMT_INFO_BFD_TEMP02
    ...
    ,LMT_UNDER_SELLBL_PROD_ID        --41可售产品 --ADD BY YJY 20260408
    ,APV_AMT                         --42批复金额 --ADD BY YJY 20260408
    ,CRDT_CONT_ID_KHFX               --43授信合同编号-客户风险  --ADD BY YJY 20260408
    ,STATUS_CD                       --44状态代码 --ADD BY YJY 20260408
    ,CRDT_APVED_DT                   --45授信审批日期 --ADD BY YJY 20260408
     )
  SELECT  /*+USE_HASH(A,B,C,D,E,F,TA)*/
         ...
         ,CASE WHEN B.MGMT_TELLER_ID = 'system' AND B.CONT_ID LIKE 'UPL%' 
               THEN '03000063'
         ...
         ,NULL                                                AS GROUP_CRDT_FLG             --36集团授信标志 --ADD BY YJY 20260408
         ,B.STD_PROD_ID                                       AS STD_PROD_ID                --37标准产品编号 --ADD BY YJY 20260408
         ,E.APPL_WAY_CD                                       AS APPL_WAY_CD                --38申请方式代码 --ADD BY YJY 20260408
         ,K.APPROVE_SERIAL_FLOW_NUM                           AS APPROVE_SERIAL_FLOW_NUM    --39被动转授信集团批复流水号 --ADD BY YJY 20260408
         ,B.APV_FLOW_NUM                                      AS APV_FLOW_NUM               --40批复流水号 --ADD BY YJY 20260408
         ,E.LMT_UNDER_SELLBL_PROD_ID                          AS LMT_UNDER_SELLBL_PROD_ID   --41可售产品 --ADD BY YJY 20260408
         ,F.CRDT_APV_AMT                                      AS APV_AMT                    --42批复金额 --ADD BY YJY 20260408
  ...
  INSERT INTO RRP_MDL.M_CRDT_LMT_INFO_BFD_TEMP02
    ...
    ,LOAN_HAPP_TYPE_CD               --34合同发生类型
    ,RELA_CONT_ID                    --35原合同编号
    ,STD_PROD_ID                     --36标准产品编号 --ADD BY YJY 20260408
    ,STATUS_CD                       --37状态代码 --ADD BY YJY 20260408
    ,CRDT_APVED_DT                   --38授信审批日期  --ADD BY YJY 20260408
     )
 WITH LMT_INTO_TEMP06 AS (
      SELECT /*+MATERIALIZE*/ 
             T6.LMT_CONT_ID  --物化视图
  ...
  SELECT  /*+USE_HASH(A B M C D)*/
         ...
         ,CASE WHEN T1.PROD_ID IN ('602030100002') THEN 'N' --剔除委托贷款
               ...
               WHEN T1.STATUS_CD = '2' AND NVL(T1.EXP_DT,TO_DATE('99991231','YYYYMMDD')) >= TO_DATE(V_YESTADAY,'YYYYMMDD') -- 合同有效且未到期
    ...
    LEFT JOIN RRP_MDL.O_ICL_CMM_RETL_LOAN_APPL_INFO TE --零售贷款申请信息 --modify by tangan at 20230129
     ...
     AND TE.ETL_DT = TO_DATE(V_YESTADAY,'YYYYMMDD')
   WHERE T1.PROD_ID NOT IN ('202020200007','201010300005','202010200012')  
         --剔除新心金融小微贷    --modify by yjy 20240417 剔除201010300005-新兴贷 --MOD BY YJY 20250910 剔除360借条202010200012
 ...
 WITH LMT_INTO_TEMP06 AS (
  ...
  SELECT  /*+USE_HASH(A B M C D)*/
         ...
         ,CASE WHEN T1.PROD_ID IN ('602030100002') THEN 'N' --不含委托贷款
               ...
               WHEN T1.LOAN_HAPP_TYPE_CD IN ('0102','0202','0204') THEN 'N' --CD04031 贷款发放类型代码
               --零售的 原额度续作/借新还旧/债务重组置为失效，如果项下有借据余额再置为有效
               WHEN T1.CONT_STATUS_CD = '2' AND NVL(T1.EXP_DT,TO_DATE('99991231','YYYYMMDD')) >= TO_DATE(V_YESTADAY,'YYYYMMDD') -- 合同有效且未到期
               THEN 'Y'--有效 --CD04022 合同状态代码
               ELSE 'N'
          END                                             AS CRDT_STAT                   --17授信状态 Z0002
         ...
         ,CASE WHEN T1.PROD_ID IN ('201020100032','201020100033','201020100034','201020100035','202020200008','201020100036'
                                  ...
                                  ,'201010300012') 
               THEN '9901' --其他-敞口授信  --助贷和微贷 modify by tangan at 20230129
               ELSE '02'
 ...
 ,TMP_DUBIL AS (
  ...
  (  DATA_DT                         --01数据日期
    ...
    ,RELA_CONT_ID                    --35原合同编号
    ,STD_PROD_ID                     --36标准产品编号  --ADD BY YJY 20260408
    ,CRDT_APVED_DT                   --37授信审批日期  --ADD BY YJY 20260408
     )
  SELECT  V_YESTADAY                             AS DATA_DT                      --01数据日期
         ,A.LP_ID                                AS LGL_REP_ID                   --02法人编号
         ,NULL                                   AS PRIM_CRDT_CONT_ID            --03主授信合同编号
         ,A.CONT_ID                              AS CRDT_CONT_ID                 --04授信合同编号
         ,A.CONT_ID                              AS CRDT_CONT_NM                 --05授信合同名称
         ,A.CUST_ID                              AS CUST_ID                      --06客户编号
  ...
  --零售360借条，同一个客户只有一笔有效的授信和批复，会有多笔业务合同和出账，除了额度项下的都算单笔单批 ADD BY YJY 20250910
  V_STEP := V_STEP + 1;
 ...
 WITH TMP_CRDT_LMT AS (
  ...
  INSERT INTO RRP_MDL.M_CRDT_LMT_INFO_BFD_TEMP02
    ...
    ,STD_PROD_ID                     --标准产品编号  --ADD BY YJY 20260408
    ,CRDT_APVED_DT                   --授信审批日期  --ADD BY YJY 20260408
  )
     ...
     SELECT T.CUST_ID                            AS CUST_ID --客户号
      ...
      WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
        ...
        AND ((T.BUS_BREED_ID IN ('202010100008','202020100003') AND T.STATUS_CD = '01')
             OR T.BUS_BREED_ID NOT IN ('202020100001','202020200004','02001006135011','02001006160048','202010100008','202020100003')) --微粒贷只取审批通过 剔除网商贷 --20240105
        AND (  (NVL(T.BEGIN_DT,TO_DATE('00010101','YYYYMMDD')) <= TO_DATE(V_P_DATE,'YYYYMMDD') - 1 AND T.BUS_BREED_ID NOT IN ('202010200011','202010200010','201020100063'))
            OR (NVL(T.BEGIN_DT,TO_DATE('00010101','YYYYMMDD')) <= TO_DATE(V_P_DATE,'YYYYMMDD')  AND T.BUS_BREED_ID IN ('202010200011','202010200010','201020100063')) --MOD BY YJY 20251104 分期乐、好企贷-数据贷（微业贷3.0）的放款日期特殊处理
  ...
  SELECT  V_YESTADAY                             AS DATA_DT                      --数据日期
         ...
         --ADD BY LYH 20231228，网商贷资本新规整改授信额度为失效，改为单笔单批，授信额度、已用额度等于放款金额
         ...
         ,SUM(CASE WHEN NVL(A.STD_PROD_ID,TB.BUS_BREED_ID1) IN ('202020100001','202020200004','02001006135011','02001006160048') 
                   THEN A.DUBIL_AMT
                   ELSE NVL(A.NOMAL_PRIC,0) + NVL(A.OVDUE_PRIC,0) + NVL(A.IDLE_PRIC,0) + NVL(A.BAD_DEBT_PRIC,0)
         ...
         ,CASE WHEN NVL(A.NOMAL_PRIC,0) + NVL(A.OVDUE_PRIC,0) + NVL(A.IDLE_PRIC,0) + NVL(A.BAD_DEBT_PRIC,0) > 0
                AND A.WRT_OFF_FLG <> '1'  
               THEN 'Y'
               /*WHEN A.WRT_OFF_FLG = '1' AND B.FIR_WRT_OFF_DT >= V_MONTH_START_DATE THEN 'N'
               WHEN A.PAYOFF_DT >= V_MONTH_START_DATE THEN 'N'*/
               ELSE 'N'
   ...
   WHERE /*((NVL(A.NOMAL_PRIC,0) + NVL(A.OVDUE_PRIC,0) + NVL(A.IDLE_PRIC,0) + NVL(A.BAD_DEBT_PRIC,0) > 0)--取余额大于0的借据
          OR (A.WRT_OFF_FLG = '1' AND B.FIR_WRT_OFF_DT >= V_MONTH_START_DATE)
          OR A.PAYOFF_DT >= V_MONTH_START_DATE )*/
         NVL(A.NOMAL_PRIC,0) + NVL(A.OVDUE_PRIC,0) + NVL(A.IDLE_PRIC,0) + NVL(A.BAD_DEBT_PRIC,0) > 0
     AND A.WRT_OFF_FLG <> '1'
   ...
   --网商贷根据资本新规，调整为单笔单批 ADD BY YJY 20260408
  V_STEP := V_STEP + 1;
  V_STEP_DESC := '插入授信额度主表-联合网贷-网商贷'; --18
  V_STARTTIME := SYSDATE;
  INSERT INTO RRP_MDL.M_CRDT_LMT_INFO_BFD_TEMP02
    (DATA_DT                         --数据日期
    ,LGL_REP_ID                      --法人编号
  ...
  SELECT  V_YESTADAY                               AS DATA_DT                      --数据日期
    ...
    LEFT JOIN RRP_MDL.O_ICL_CMM_UNITE_WL_WRT_OFF_INFO B --联合网贷核销表
      ON B.DUBIL_ID = A.DUBIL_ID
     AND B.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   WHERE NVL(A.NOMAL_PRIC,0) + NVL(A.OVDUE_PRIC,0) + NVL(A.IDLE_PRIC,0) + NVL(A.BAD_DEBT_PRIC,0) > 0
     AND A.WRT_OFF_FLG <> '1'       
     AND A.DUBIL_STATUS_CD NOT IN ('2','5') --失败、撤销 --ADD BY LIP 20220722过滤没发放成功的数据
  ...
  ETL_YUSYS_LOG(V_P_DATE,V_SYSTEM,V_PROC_NAME,V_STARTTIME,V_ENDTIME,V_STEP,V_STEP_DESC,V_SQLCOUNT,O_ERRCODE,'');
   ...
   INSERT INTO RRP_MDL.M_CRDT_LMT_INFO_BFD_TEMP04
    ...
    ,BUS_BREED_ID                    --业务品种编号
    ,ORG_ID_ORI                      --合同原始机构编号
    ,GROUP_CRDT_FLG                  --集团授信标志 --ADD BY YJY 20260408
    ,STD_PROD_ID                     --标准产品编号 --ADD BY YJY 20260408
    ,APPL_WAY_CD                     --申请方式代码 --ADD BY YJY 20260408
    ,APPROVE_SERIAL_FLOW_NUM         --被动转授信集团批复流水号 --ADD BY YJY 20260408
    ,APV_FLOW_NUM                    --批复流水号 --ADD BY YJY 20260408
    ,LMT_UNDER_SELLBL_PROD_ID        --可售产品 --ADD BY YJY 20260408
    ,APV_AMT                         --批复金额 --ADD BY YJY 20260408
    ,CRDT_CONT_ID_KHFX               --授信合同编号-客户风险 --ADD BY YJY 20260408
    ,STATUS_CD                       --状态代码 --ADD BY YJY 20260408
    ,CRDT_APVED_DT                   --授信审批日期 --ADD BY YJY 20260408
     )
  WITH TMP1 AS (
  SELECT V_YESTADAY                      --数据日期
        ,LGL_REP_ID                      --法人编号
        ,PRIM_CRDT_CONT_ID               --主授信合同编号
        ,CRDT_CONT_ID                    --授信合同编号
        ...
        ,CRDT_SUBJ_CL                    --授信主体种类
        ,BANK_TAX_COOP_LOAN_CRDT_FLG     --银税合作贷款授信标志
        ,NVL(TRIM(DSN_SHT_OPN),'同意')            AS DSN_SHT_OPN --决策单意见
        ,APRV_PSN_NO                     --审批人工号
        ,CRDT_EMP_NO                     --授信员工号
        ,DEPT_LINE                       --部门条线
        ,DATA_SRC                        --数据来源
        ...
        ,T.GROUP_CRDT_FLG                         AS GROUP_CRDT_FLG    --集团授信标志 --ADD BY YJY 20260408
        ,T.STD_PROD_ID                            AS STD_PROD_ID       --标准产品编号 --ADD BY YJY 20260408
        ,T.APPL_WAY_CD                            AS APPL_WAY_CD       --申请方式代码 --ADD BY YJY 20260408
        ,T.APPROVE_SERIAL_FLOW_NUM                AS APPROVE_SERIAL_FLOW_NUM --被动转授信集团批复流水号 --ADD BY YJY 20260408
        ,T.APV_FLOW_NUM                           AS APV_FLOW_NUM      --批复流水号 --ADD BY YJY 20260408
        ,T.LMT_UNDER_SELLBL_PROD_ID               AS LMT_UNDER_SELLBL_PROD_ID --可售产品 --ADD BY YJY 20260408
        ,T.APV_AMT                                AS APV_AMT           --批复金额 --ADD BY YJY 20260408
        ,T.CRDT_CONT_ID_KHFX                      AS CRDT_CONT_ID_KHFX --授信合同编号-客户风险 --ADD BY YJY 20260408
        ,T.STATUS_CD                              AS STATUS_CD         --状态代码 --ADD BY YJY 20260408
        ,T.CRDT_APVED_DT                          AS CRDT_APVED_DT     --授信审批日期  --ADD BY YJY 20260408
        /*,ROW_NUMBER() OVER(PARTITION BY CRDT_CONT_ID,PRIM_CRDT_CONT_ID ORDER BY CRDT_STAT DESC,CRDT_APP_DT DESC NULLS LAST) RN --联合网贷去重 */
   ...
   INSERT /*+APPEND*/ INTO RRP_MDL.M_CRDT_LMT_INFO_BFD
    ...
    SELECT A.CRDT_CONT_ID AS CRDT_CONT_ID
      ...
      LEFT JOIN RRP_MDL.O_ICL_CMM_RETL_LOAN_CRDT_LMT_INFO TB --零售贷款授信额度信息
       ...
       AND TB.ETL_DT = TO_DATE(V_YESTADAY,'YYYYMMDD')
  ...
  SELECT  V_YESTADAY                                     AS DATA_DT                            --数据日期
         ...
         ,'CNY'                                          AS CUR                                --币种
         ,CASE WHEN SUM(NVL(A.CRDT_LMT,0)) < SUM(NVL(A.ALDY_USE_LMT,0)) THEN SUM(NVL(A.ALDY_USE_LMT,0) * NVL(D.EXRT,1))
               ELSE SUM(NVL(A.CRDT_LMT,0) * NVL(D.EXRT,1) )
          END                                            AS CRDT_TOTAL_LMT                     --授信总额度
         ,SUM(NVL(A.ALDY_USE_LMT,0) * NVL(D.EXRT,1))     AS ALDY_USE_LMT                       --已用额度
         ,NULL                                           AS EXP_CRDT_LMT                       --敞口授信额度
         ,NULL                                           AS EXP_ALDY_USE_LMT                   --敞口已用额度
         ...
         ,SUM(CASE WHEN C.LOAN_BIZ_TYP LIKE '0102%' THEN NVL(A.ALDY_USE_LMT,0) * NVL(D.EXRT,1) ELSE 0 END)
         ...
         ,SUM(CASE WHEN C.LOAN_BIZ_TYP LIKE '0101%' THEN NVL(A.ALDY_USE_LMT,0) * NVL(D.EXRT,1) ELSE 0 END)
         ...
         ,SUM(CASE WHEN C.LOAN_BIZ_TYP = '010301' THEN NVL(A.ALDY_USE_LMT,0) * NVL(D.EXRT,1) ELSE 0 END)
         ...
         ,SUM(CASE WHEN C.LOAN_BIZ_TYP LIKE '0104%' THEN NVL(A.ALDY_USE_LMT,0) * NVL(D.EXRT,1) ELSE 0 END)
         ...
         ,SUM(CASE WHEN C.LOAN_BIZ_TYP = '010399' THEN NVL(A.ALDY_USE_LMT,0) * NVL(D.EXRT,1) ELSE 0 END)
  ...
  IF V_SQLCOUNT > 0 THEN
     O_ERRCODE   := '1';
     V_SQLMSG   := 'M_CRDT_LMT_SUB_BFD数据重复';
  ...
  IF V_SQLCOUNT > 0 THEN
     O_ERRCODE   := '1';
     V_SQLMSG   := 'M_CRDT_LMT_INFO_BFD数据重复';
     ETL_YUSYS_LOG(V_P_DATE,V_SYSTEM,V_PROC_NAME,V_STARTTIME,V_ENDTIME,V_STEP,V_STEP_DESC,V_SQLCOUNT,O_ERRCODE,V_SQLMSG);
     RETURN;
  END IF;
