CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_ICL_CMM_AM_INVEST_UNDERLY_PROD_INFO(I_P_DATE IN INTEGER,
                                                                      O_ERRCODE OUT VARCHAR2
                                                                      )
  /**************************************************************************
  *  程序名称：ETL_O_ICL_CMM_AM_INVEST_UNDERLY_PROD_INFO
  *  功能描述：资管投资标的产品信息
  *  创建日期：20221008
  *  开发人员：MW
  *  来源表：
...
AS
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_ICL_CMM_AM_INVEST_UNDERLY_PROD_INFO'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  DELETE FROM RRP_MDL.O_ICL_CMM_AM_INVEST_UNDERLY_PROD_INFO T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');--普通表的重跑处理
...
END ETL_O_ICL_CMM_AM_INVEST_UNDERLY_PROD_INFO;
/
