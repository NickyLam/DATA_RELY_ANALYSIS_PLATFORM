CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IOL_ICMS_DEPOSIT_APPLY_INFO (I_P_DATE IN INTEGER,
                              O_ERRCODE OUT VARCHAR2
                               )
  /**************************************************************************
  *  程序名称：ETL_O_IOL_ICMS_DEPOSIT_APPLY_INFO
  *  功能描述：解冻保证金申请详情
  *  创建日期：20221208
  *  开发人员：梅炜
  ...
  **************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IOL_ICMS_DEPOSIT_APPLY_INFO'; -- 程序名称
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM O_IOL_ICMS_DEPOSIT_APPLY_INFO  T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_IOL_ICMS_DEPOSIT_APPLY_INFO';
  ...
  (     SERIALNO              --申请流水号
    ...
    SELECT
        ...
        ,ID_MARK              --增删标志
        ,ETL_TIMESTAMP        --ETL处理时间戳
    FROM IOL.V_ICMS_DEPOSIT_APPLY_INFO  --解冻保证金申请详情
   WHERE ID_MARK <> 'D';
  ...
  END ETL_O_IOL_ICMS_DEPOSIT_APPLY_INFO ;
/
