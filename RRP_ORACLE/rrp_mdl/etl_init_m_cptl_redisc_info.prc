CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_M_CPTL_REDISC_INFO(I_P_DATE IN INTEGER,
                                                O_ERRCODE OUT VARCHAR2
                                                )
  /**************************************************************************
  *  程序名称：ETL_INIT_M_CPTL_REDISC_INFO
  *  功能描述：再贴现信息表
  *  创建日期：20220609
  *  开发人员：梅炜
  *  来源表：
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(200) := 'ETL_INIT_M_CPTL_REDISC_INFO'; -- 程序名称
  ...
  V_TAB_NAME := 'M_CPTL_REDISC_INFO'; --表名
 ...
 -- 支持重跑 --
  ...
  -- ,NULL                        AS REDISC_ORG_NM      --再贴现机构名称
    ,D.ORG_NAME                 AS REDISC_ORG_NM      --再贴现机构名称  --modify by laihaiqiang 20220602 修改再贴现机构取值口径
    ,A.CNTPTY_NAME                 AS CNTPR_NM           --交易对手名称
     ,NVL(
          CASE WHEN LENGTH(TRIM(TY.FIN_INST_CODE))=14 THEN TRIM(TY.FIN_INST_CODE)
               ELSE NULL
           END,
          PBC.FIN_ORG_CODE
       )AS CNTPR_FIN_ORG_ID   --交易对手金融机构编码 --更新 202200722 xucx
    ...
    ,A.PAYER_BANK_NO--A.PAYER_TRUST_ACCT_NUM        AS CPTL_OUT_ACC_NO    --资金划出账号
    ,A.RECVER_BANK_NO--A.RECVER_TRUST_ACCT_NUM       AS CPTL_IN_ACC_NO     --资金划入账号
    ,PBC.FIN_NAME                  AS PBC_ORG_PIC_NM     --人行机构负责人
    ,'800935'   /*票据业务事业部'06'*/                          AS DEPT_LINE          --部门条线
    ...
    ,CAST(D.PHONE AS VARCHAR2(100)) AS ORG_PHONE          --再贴现机构联系电话 --ADD BY 20220628 XUCX
  ...
  FROM O_ICL_CMM_BILL_REDCST_INFO A  --票据再贴现信息
  /*LEFT JOIN O_ICL_CMM_CORP_LOAN_DUBIL_INFO C --对公贷款借据信息
  ...
  LEFT JOIN RRP_MDL.O_IML_PTY_IBANK_CUST_CHAT_INFO TY   --取同业机构的金融机构编码   add by 20220722 xucx
        ...
        AND TY.START_DT <= TO_DATE(V_P_DATE,'YYYYMMDD')/*A.ETL_DT*/
        AND TY.END_DT > TO_DATE(V_P_DATE,'YYYYMMDD')/*A.ETL_DT*/
  ...
  END ETL_INIT_M_CPTL_REDISC_INFO
;
/
