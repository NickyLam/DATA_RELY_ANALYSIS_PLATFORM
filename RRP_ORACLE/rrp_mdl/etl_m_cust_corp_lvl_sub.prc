CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_CUST_CORP_LVL_SUB(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_M_CUST_CORP_LVL_SUB
...
AS
  ...
  V_TAB_NAME  VARCHAR2(100) := 'M_CUST_CORP_LVL_SUB'; --表名
  V_PROC_NAME VARCHAR2(100) := 'ETL_M_CUST_CORP_LVL_SUB'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM M_CUST_CORP_LVL_SUB T WHERE T.DATA_DT = V_P_DATE;--普通表的重跑处理
  --EXECUTE IMMEDIATE ('ALTER TABLE '||'M_CUST_CORP_LVL_SUB'||' TRUNCATE PARTITION '||'写上分区名');--分区表的重跑处理
  ...
  WITH TMP1 AS (
    SELECT DATA_DT, CUST_ID,COUNT(1)
      FROM RRP_MDL.M_CUST_CORP_LVL_SUB T
