CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_ICL_CMM_CORP_LOAN_CONT_INFO(I_P_DATE IN INTEGER,
                                                              O_ERRCODE OUT VARCHAR2
                                                              )
/**************************************************************************
 *  程序名称：ETL_O_ICL_CMM_CORP_LOAN_CONT_INFO
 *  功能描述：对公贷款合同信息
 *  创建日期：20220525
 *  开发人员：梅炜
...
AS
  ...
  V_TAB_NAME  VARCHAR2(100) := 'O_ICL_CMM_CORP_LOAN_CONT_INFO';--表名
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_ICL_CMM_CORP_LOAN_CONT_INFO'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  DELETE FROM RRP_MDL.O_ICL_CMM_CORP_LOAN_CONT_INFO  
  ...
  --EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_ICL_CMM_CORP_LOAN_CONT_INFO';
  ...
  INSERT INTO RRP_MDL.O_ICL_CMM_CORP_LOAN_CONT_INFO
    ...
    ,REGROUP_TYPE_CD                       --重组类型代码  ADD BY YJY 20251226
    )
  SELECT 
     ETL_DT                                --数据日期
    ,LP_ID                                 --法人编号
...
END ETL_O_ICL_CMM_CORP_LOAN_CONT_INFO;
/
