CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IOL_CTMS_TBS_V_BONDSDEALS(I_P_DATE IN INTEGER, --跑批日期
                                                 O_ERRCODE  OUT VARCHAR2 --错误代码
                                                 )
 /*******************************************************************
  **存储过程详细说明： 现券交易
  **存储过程名称：    ETL_O_IOL_CTMS_TBS_V_BONDSDEALS
  **存储过程创建日期：20220707
  **存储过程创建人：  赖海强
  **输入参数：        I_P_DATE
  **输出参数：        O_ERRCODE
 ...
 AS
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IOL_CTMS_TBS_V_BONDSDEALS'; -- 程序名称
 ...
 EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_IOL_CTMS_TBS_V_BONDSDEALS';
  ...
  INSERT /*+APPEND*/ INTO RRP_MDL.O_IOL_CTMS_TBS_V_BONDSDEALS NOLOGGING
    ...
    ,END_DT	             --结束时间
    ,ID_MARK	           --增删标志
    ,CURRENCY            --币种  --ADD BY YJY 20250217
    ,TRADETIME           --交易时间 ADD BY YJY 20250919
    )
  SELECT /*+PARALLEL*/
     DEAL_ID             --引用表ID
    ,DEAL_TABLENAME      --引用表名
   ...
   WHERE START_DT <= TO_DATE(V_P_DATE,'YYYYMMDD')
     ...
     AND ID_MARK <> 'D';--MOD BY YJY 20241217
...
END ETL_O_IOL_CTMS_TBS_V_BONDSDEALS;
/
