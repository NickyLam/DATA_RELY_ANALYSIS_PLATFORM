CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_M_CPTL_INVEST_INFO_BAL0(I_P_DATE IN INTEGER,
                                                   O_ERRCODE  OUT VARCHAR2)
/**************************************************************************
  *  程序名称：ETL_INIT_M_CPTL_INVEST_INFO_BAL0
  *  功能描述：投资业务信息
  *  创建日期：20220608
  *  开发人员：梅炜
  *  来源表：
 ...
 AS
  ...
  V_PROC_NAME VARCHAR2(100) := 'ETL_INIT_M_CPTL_INVEST_INFO_BAL0'; -- 程序名称
...
BEGIN
  ...
  V_TAB_NAME := 'M_CPTL_INVEST_INFO_BAL0'; --表名
  ...
  INSERT INTO RRP_MDL.M_CPTL_INVEST_INFO_BAL0
    ...
    ON NVL(TRIM(B.CUST_ID), NVL(TRIM(B.SRC_PARTY_ID), '-')) = D.CUST_ID*/
     WHERE TO_CHAR(A.ETL_DT, 'YYYYMMDD') = V_P_DATE
      AND NVL(A.ACTL_BAL, 0) + NVL(A.EVHA_VAL_CHAG, 0) = 0
  ...
  WITH TMP1 AS (
    SELECT DATA_DT, ACC_ID,ID,COUNT(1)
      FROM M_CPTL_INVEST_INFO_BAL0 T
...
END ETL_INIT_M_CPTL_INVEST_INFO_BAL0;
/
