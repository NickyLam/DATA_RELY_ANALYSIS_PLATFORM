CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_M_LOAN_IN_DUBILL_INFO(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_INIT_M_LOAN_IN_DUBILL_INFO
...
AS
  ...
  V_PROC_NAME VARCHAR2(30) := 'ETL_INIT_M_LOAN_IN_DUBILL_INFO'; --程序名称
...
BEGIN
  ...
  WITH TMP1 AS (
    ...
    SELECT CUST_ID,DUBIL_NUM AS RCPT_ID,CASE WHEN DISTR_DT = DATE '0001-01-01' THEN NULL ELSE DISTR_DT END AS LOAN_ACT_DSTR_DT
      FROM IOL.V_CMM_RETL_LOAN_ACCT_INFO WHERE ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
     UNION ALL
    SELECT CUST_ID,DUBIL_ID AS RCPT_ID,CASE WHEN DISTR_DT = DATE '0001-01-01' THEN NULL ELSE DISTR_DT END AS LOAN_ACT_DSTR_DT
      FROM IOL.V_CMM_UNITE_WL_DUBIL_INFO WHERE ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
     UNION ALL
    SELECT CUST_ID,DUBIL_ID AS RCPT_ID,CASE WHEN DISTR_DT = DATE '0001-01-01' THEN NULL ELSE DISTR_DT END  AS LOAN_ACT_DSTR_DT
      FROM IOL.V_CMM_UNITE_WL_DUBIL_INFO_CLEAR
     UNION ALL
    SELECT CUST_ID,DUBIL_NUM AS RCPT_ID,CASE WHEN DISTR_DT = DATE '0001-01-01' THEN NULL ELSE DISTR_DT END AS LOAN_ACT_DSTR_DT
      FROM IOL.V_CMM_CORP_LOAN_ACCT_INFO WHERE ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')),
  TMP2 AS (
  ...
  SELECT  A.MAIN_ACCT_ID        AS CUST_ACCT_ID       --账户编号
         ...
         ,B.BKNAME                                    --银行机构名称
         ,COALESCE(TRIM(C.COUNTY_CD),TRIM(C.CITY_CD),TRIM(C.PROV_CD)) AS COUNTY_CD --机构地区
    FROM IOL.V_CMM_INTNAL_ACCT A            --内部账户
    LEFT JOIN RRP_MDL.M_LOAN_IN_DUBILL_INFO_TEMP02 D
  ...
  SELECT  A.CARD_NO               AS CUST_ACCT_ID        --账户编号
         ...
         ,B.BKNAME                                       --银行机构名称
         ,COALESCE(TRIM(C.COUNTY_CD),TRIM(C.CITY_CD),TRIM(C.PROV_CD)) AS COUNTY_CD --机构地区
    FROM IOL.V_CMM_BANK_CARD_BASIC_INFO A  --银行卡基本信息
    LEFT JOIN RRP_MDL.M_LOAN_IN_DUBILL_INFO_TEMP02 D
  ...
  SELECT P1.CUST_ID      --01 客户号
    ...
    FROM IOL.V_CMM_INDV_CUST_BASIC_INFO P1 --个人客户基本信息表
...
WITH DK_LOAN_DUBIL_INFO AS (
        SELECT A.DUBIL_ID,A.STD_PROD_ID,TO_CHAR(A.PAYOFF_DT,'YYYYMMDD') PAYOFF_DT,B.DUBIL_ID RELA_DUBIL_ID,B.BILL_ID,B.BILL_NUM
          FROM IOL.V_CMM_CORP_LOAN_DUBIL_INFO A
          LEFT JOIN IOL.V_CMM_CORP_LOAN_DUBIL_INFO B
...
BILL_INFO AS (
        SELECT TA.BILL_ID,TA.BILL_NUM,TA.BILL_SUB_INTRV_ID,TA.DRAWER_ACCT_NUM,TA.DRAWER_NAME,TA.DRAWER_OPEN_BANK_NAME
          FROM IOL.V_CMM_BILL_CENTER_INFO TA
...
BILL_INFO_JF AS (
        SELECT TA.BILL_ID,TA.BILL_NUM,TA.BILL_SUB_INTRV_ID,TA.DRAWER_ACCT_NUM,TA.DRAWER_NAME,TA.DRAWER_OPEN_BANK_NAME,
               ...
               ROW_NUMBER() OVER(PARTITION BY TA.BILL_NUM ORDER BY
                      ...
                      TA.BILL_ID DESC) RN
          FROM BILL_INFO TA
          LEFT JOIN IML.V_EVT_SUGST_PAY_APPL_EVT TB
          ...
          LEFT JOIN IML.V_AGT_BA_EXP_CASH_APPL_H TC
          ...
          LEFT JOIN IML.V_AGT_BILL_BUS_STL_INFO TD
  ...
  SELECT  TA.DUBIL_ID                             AS DUBIL_ID                 --借据号
    ...
    FROM IML.V_AGT_LOAN_OUT_ACCT_CORP_LOAN_ATTACH_INFO_H T
   INNER JOIN IOL.V_CMM_CORP_LOAN_DUBIL_INFO TA
  ...
  SELECT TB.DUBIL_ID                             AS DUBIL_ID,                 --借据号
   ...
   INNER JOIN IML.V_AGT_FFT_TRAN_DTL TB
    ...
    LEFT JOIN RRP_MDL.O_IOL_MPCS_A08TBANKINFO TC
     ...
     AND TC.START_DT <= TO_DATE(V_P_DATE,'YYYYMMDD')
     AND TC.END_DT > TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN IOL.V_CMM_CORP_LOAN_DUBIL_INFO TD
  ...
  WITH CL_INVOICE_OD_DETAIL AS (
    SELECT ACCT_ID,CURR_PD
      ...
      FROM IML.V_EVT_PNLT_COMP_INT_NOMAL_REPAY_DTL OV   --罚息复利正常还款明细
  ...
  CORP_LOAN_REPAY_PLAN AS (
    SELECT N.ACCT_ID,N.DUBIL_ID,N.TOT_PERDS,N.REPAY_PERDS,N.REPAYBL_DT,N.ETL_DT,
      ...
      FROM IOL.V_CMM_CORP_LOAN_REPAY_PLAN N  --对公贷款还款计划
  ...
  SELECT /*PRALLEL(4)*/ V_P_DATE                                         AS DATA_DT                     --数据日期
        ...
        --MOD BY YJY 20250103 无还本续贷通过借据取信贷系统打的标签，其他仍通过合同的贷款形式映射码值
        ,CASE WHEN WHBXD.DUBIL_ID IS NOT NULL
              THEN '05'  --05-无还本续贷
        ...
        /*,CASE WHEN E.LOAN_HAPP_TYPE_CD IN ('0201','0204','0202') THEN 'Y' --0201展期 0204债务重组 0202借新还旧
              ELSE 'N'
          END*/
        --MOD BY YJY 20250303 优先关联信贷系统贷款重组关联表的借据
        ,CASE WHEN CZ.OBJECTNO  IS NOT NULL
              THEN 'Y'
              WHEN E.LOAN_HAPP_TYPE_CD IN ('0201','0204','0202')
              THEN 'Y' --0201展期 0204债务重组 0202借新还旧
              ELSE 'N'
   ...
   FROM IOL.V_CMM_CORP_LOAN_DUBIL_INFO A --对公贷款借据信息表
  INNER JOIN IOL.V_CMM_CORP_LOAN_ACCT_INFO B --对公贷款账户信息表
     ON B.DUBIL_NUM = A.DUBIL_ID
    AND B.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   LEFT JOIN IOL.V_CMM_STD_PROD_INFO C --标准产品信息表
     ON C.PROD_ID = A.STD_PROD_ID
    AND C.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   LEFT JOIN IOL.V_CMM_LOAN_WRT_OFF_INFO D --贷款核销信息
     ON D.DUBIL_ID = A.DUBIL_ID
    AND D.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   LEFT JOIN IOL.V_CMM_CORP_LOAN_CONT_INFO E --对公贷款合同信息
     ON E.CONT_ID = A.CONT_ID
    AND E.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   LEFT JOIN IOL.V_CMM_CORP_LOAN_CONT_INFO E1 --对公贷款合同信息
     ON E1.CONT_ID = E.LMT_CONT_ID
    AND E1.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   LEFT JOIN IML.V_AGT_LOAN_OUT_ACCT_APPL_H T3 --贷款出账申请历史
     ON T3.DUBIL_ID = A.DUBIL_ID
    AND T3.START_DT <= TO_DATE(V_P_DATE,'YYYYMMDD')
    AND T3.END_DT > TO_DATE(V_P_DATE,'YYYYMMDD')
   LEFT JOIN IML.V_AGT_LOAN_OUT_ACCT_CORP_LOAN_ATTACH_INFO_H A8 --贷款出账对公贷款附属信息历史add by hulj20230728
   ...
   LEFT JOIN (SELECT DUBIL_ID
                ...
                FROM IOL.V_CMM_CORP_LOAN_REPAY_DTL
   ...
   LEFT JOIN IOL.V_CMM_CORP_LOAN_DUBIL_INFO L1 --对公贷款借据信息表
   ...
   LEFT JOIN IML.V_PTY_PARTY_PHYS_ADDR_H F --当事人物理地址历史
   ...
   LEFT JOIN IOL.V_CMM_CORP_CUST_BASIC_INFO G --对公客户基本信息
   ...
   LEFT JOIN (SELECT DUBIL_ID
                FROM (SELECT B.DUBIL_ID
                        ...
                        FROM IOL.V_CMM_CORP_LOAN_DUBIL_INFO B
   ...
   LEFT JOIN IOL.V_CMM_CLERK_INFO T18 --行员信息表      ADD BY HULJ20221123
     ON T18.CLERK_ID = A.RGST_TELLER_ID
    AND T18.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   LEFT JOIN IOL.V_CMM_TELLER_INFO T19
   ...
   /*根据与业务确认的口径，imas对公贷款部分取借据发放时的基准利率（信贷登记的就是发放时的基准利率），如果基准利率为0
     ...
     外币的按C0的计算  4000*/
   LEFT JOIN IML.V_REF_BASE_RAT_H TBA --基准利率 --ADD BY LIP 20230811
   ...
   LEFT JOIN IML.V_REF_BASE_RAT_H TBAB --基准利率 --ADD BY LIP 20230811 取不到起息日时的基准利率时，取最新LPR的
  ...
  ,RETL_LOAN_REPAY_PLAN AS (
        SELECT /*+MATERIALIZE*/N.ACCT_ID,N.DUBIL_ID,N.TOT_PERDS,N.REPAY_PERDS,N.REPAYBL_DT,N.ETL_DT,MAX(N.VALUE_DT) AS VALUE_DT
          ...
          FROM IOL.V_CMM_RETL_LOAN_REPAY_PLAN N  --零售贷款还款计划
         WHERE N.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
         GROUP BY N.ACCT_ID,N.DUBIL_ID,N.TOT_PERDS,N.REPAY_PERDS,N.REPAYBL_DT,N.ETL_DT)
  ...
  ,CORP_CUST AS (
        SELECT TRIM(T.PBC_PAY_BANK_NO) AS SYS_PRTCPTR_BIGAMT_BANK_NO--系统参与者大额行号
          ...
          FROM IOL.V_CMM_CORP_CUST_BASIC_INFO T   --对公客户基本信息
  ...
  SELECT TO_CHAR(A.ETL_DT,'YYYYMMDD')                            AS DATA_DT                    --数据日期
        ...
        ,CASE WHEN B.STD_PROD_ID IN ('201010300040','201020100060') AND B.DUE_DILIGENCE_FLG <> '1' THEN 'Y' --201010300040华兴易贷（信用）201020100060华兴好易贷（经营-信用） --mod by yjy 20250805
              ...
              WHEN B.STD_PROD_ID IN ('201020100062','201020100061') AND HD.DUBIL_ID IS NOT NULL THEN 'Y' --饲料e贷-海大集团\兴采贷 ADD BY YJY 20250717 关联标签表判断是否互联网业务
        ...
        ,CASE WHEN TA.LOAN_HAPP_TYPE_CD = '0102' THEN '05' --无还本续贷
        ...
        ,CASE WHEN B.STD_PROD_ID IN ('201020100014','201020100012','201020100024'
                                     ...
                                     ,'201020100062','201020100061') THEN 'Y' --modify by lwb 新增两个饲料e贷 --mod by yjy 20250718 新增饲料e贷-海大集团  201020100062、兴采贷  201020100061
   ...
   FROM IOL.V_CMM_RETL_LOAN_ACCT_INFO A --零售贷款账户信息
  INNER JOIN IOL.V_CMM_RETL_LOAN_DUBIL_INFO B --零售贷款借据信息
     ON B.DUBIL_ID = A.DUBIL_NUM
    AND B.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   LEFT JOIN IOL.V_CMM_RETL_LOAN_CONT_INFO TA --零售贷款合同信息表
     ON TA.CONT_ID = A.CONT_ID
    AND TA.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   LEFT JOIN IML.V_AGT_LOAN_CONT_INDV_LOAN_ATTACH_INFO_H TB --贷款合同个人贷款附属信息历史 --MODIFY BY TANGAN AT 20230107 取助贷的入账和出账账号
     ON TB.CONT_ID = A.CONT_ID
    AND TB.START_DT <= TO_DATE(V_P_DATE,'YYYYMMDD')
    AND TB.END_DT > TO_DATE(V_P_DATE,'YYYYMMDD')
   LEFT JOIN IOL.V_CMM_RETL_LOAN_BUS_CONT_ATTACH_INFO T1 --零售贷款业务合同补充信息 --ADD BY LIP 20250310
   ...
   LEFT JOIN CORP_CUST TCC
    ...
    AND TCC.RN = 1
   LEFT JOIN IOL.V_CMM_RETL_LOAN_APPL_INFO M --零售贷款申请信息
     ON M.LOAN_APPL_FLOW_NUM = TA.APV_FLOW_NUM
    AND M.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   LEFT JOIN IOL.V_CMM_STD_PROD_INFO C --标准产品信息
     ON C.PROD_ID = A.STD_PROD_ID
    AND C.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   LEFT JOIN IOL.V_CMM_LOAN_WRT_OFF_INFO D --贷款核销信息
     ON D.DUBIL_ID = A.DUBIL_NUM
    AND D.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   LEFT JOIN IOL.V_CMM_INDV_CUST_BASIC_INFO F --个人客户基本信息
  ...
  WITH UNITE_WL_REPAY_PLAN AS (
    SELECT N.DUBIL_ID,N.INIT_TOT_PERDS TOT_PERDS,N.INIT_REPAY_PERDS REPAY_PERDS,
      ...
      FROM IOL.V_CMM_UNITE_WL_REPAY_PLAN N --联合网贷还款计划
     WHERE N.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
  ...
  SELECT V_P_DATE                                        AS DATA_DT                             --数据日期
        ...
        ,CASE WHEN A.STD_PROD_ID IN ('202020100001','202020200004')
              ...
              WHEN A.STD_PROD_ID IN ('202020200001','202010200009','202010200010','202010200011','202010100007')--mod by yjy 20250717 新增分期乐系列产品、唯品消金
        ...
        ,A.DISTR_AMT                                     AS DISTR_AMT                           --放款金额
        ,TO_CHAR(A.DISTR_DT,'YYYYMMDD')                  AS DISTR_DT                            --放款日期
        ,CASE WHEN A.STD_PROD_ID IN ('202010100004')
   ...
   FROM IOL.V_CMM_UNITE_WL_DUBIL_INFO A --联合网贷借据信息
   LEFT JOIN IOL.V_CMM_STD_PROD_INFO M --标准产品信息
     ON M.PROD_ID = STD_PROD_ID
    AND M.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   LEFT JOIN UNITE_WL_REPAY_PLAN_1 B --还款计划 取连续/累计欠款期数
     ON B.DUBIL_ID = A.DUBIL_ID
   LEFT JOIN IOL.V_CMM_INDV_CUST_BASIC_INFO E --个人客户基本信息
     ON E.CUST_ID = A.CUST_ID
    AND E.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   LEFT JOIN IML.V_PTY_PARTY_PHYS_ADDR_H C --当事人物理地址历史
   ...
   LEFT JOIN IML.V_AGT_WLD_ACCT T8 --微粒贷账户 --MOD BY LIP 20230527
     ON T8.ACCT_ID = T7.ACCT_ID
    AND T8.ACCT_TYPE_CD = T7.ACCT_TYPE_CD
   LEFT JOIN IML.V_AGT_MYLOAN_DUBIL T9 --网商贷借据 --MOD BY LIP 20230527
     ON T9.DUBIL_ID = A.DUBIL_ID
    AND T9.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   LEFT JOIN IOL.V_CMM_UNITE_WL_WRT_OFF_INFO TFF --联合网贷核销信息
   ...
   LEFT JOIN IML.V_REF_PUB_CD PUB --公共代码表 取贷款用途
     ON PUB.CD_ID = 'CD1274'
   ...
   LEFT JOIN (SELECT DUBIL_ID
                FROM (SELECT B.DUBIL_ID,
                             ROW_NUMBER() OVER(PARTITION BY B.CUST_ID ORDER BY B.DISTR_DT,B.DUBIL_ID ASC) AS RN
                        FROM IOL.V_CMM_UNITE_WL_DUBIL_INFO B
  ...
  WHERE A.DUBIL_STATUS_CD NOT IN ('2','5') --失败、撤销
    AND A.DUBIL_ID IS NOT NULL
    AND A.STD_PROD_ID <> '203050100001' --MOD BY YJY 20250213 剔除对公产品203050100001微业贷
    AND A.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  ...
  SELECT /*PRALLEL(4)*/ V_P_DATE                                           AS DATA_DT                      --数据日期
        ...
        ,NULL                                                              AS SUIT_FEE_BAL                --诉讼费余额     --ADD BY YJY 20241217
        ,NULL                                                              AS GREEN_CRDT_CLS_NEW          --绿色信贷分类_新版代码 --ADD BY YJY 20250508
   FROM IOL.V_CMM_IBANK_NON_STD_INVEST A  --同业非标投资表
   LEFT JOIN IOL.V_CMM_IBANK_SECU_POST A1 -- 同业证券持仓表  总分核对，根据数仓和财务集市总分校验校验，使用的金额是同业证券持仓的数据
   ...
   LEFT JOIN IOL.V_CMM_IBANK_CAP_BAL B  --同业资金余额表
     ON B.INTNAL_CAP_ACCT_ID = A.INTNAL_SECU_ACCT_ID
    AND B.EXT_CAP_ACCT_ID = A.EXT_SECU_ACCT_ID
    AND B.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   LEFT JOIN IOL.V_CMM_IBANK_FIN_INSTM C  --同业金融工具表
  ...
  SELECT  TO_CHAR(A.ETL_DT,'YYYYMMDD')                AS DATA_DT                     --数据日期
         ...
         ,CASE WHEN A.BILL_KIND_CD = 'AC01' THEN TRIM(A.DRAWER_ACCT_NUM)--030101 银行承兑汇票贴现
           ...
           END                                        AS REPY_ACC                    --还款账号
         ,CASE WHEN A.BILL_KIND_CD = 'AC01' THEN TRIM(A.DRAWER_OPEN_BANK_NAME)--030101 银行承兑汇票贴现
               WHEN A.BILL_KIND_CD = 'AC02'
         ...
         ,CASE WHEN (B.PAYOFF_DT >= V_YEAR_START_DATE
               ...
               ELSE 'N'
           END                                        AS YBT_FLG                   --一表通口径标识 --ADD BY PSF 20250916
    FROM IOL.V_CMM_BILL_DISCNT_INFO A --票据贴现信息
   INNER JOIN IOL.V_CMM_CORP_LOAN_DUBIL_INFO B --对公贷款借据信息
     ...
     AND B.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN IOL.V_CMM_CORP_LOAN_ACCT_INFO C  --对公贷款账户信息
      ON C.DUBIL_NUM = B.DUBIL_ID
     AND C.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN IOL.V_CMM_CORP_LOAN_CONT_INFO H --对公贷款合同信息
      ON H.CONT_ID = B.CONT_ID
     AND H.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN IOL.V_CMM_CORP_CUST_BASIC_INFO E --对公客户基本信息
      ON E.CUST_ID = A.CUST_ID
     AND E.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN IOL.V_CMM_STD_PROD_INFO M --标准产品信息表
    ...
    LEFT JOIN RRP_MDL.O_IOL_IFRS_VAL_RPT_TRADE O --估值报告表
     ...
     AND O.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN IML.V_PTY_CPES_MEM TTA --票交所会员 只有一天数据
      ON TTA.SYS_PRTCPTR_BIGAMT_BANK_NO = TRIM(A.DISCNT_APPLIT_BANK_NO)
     AND TTA.RANK = 1
    LEFT JOIN IML.V_PTY_CPES_MEM MEMA --票交所会员 ADD BY LIP 20231103 部分数据的行号登记的是会员机构号
  ...
  SELECT  TO_CHAR(A.ETL_DT,'YYYYMMDD')                AS DATA_DT                     --数据日期
         ...
         ,CASE WHEN (B.PAYOFF_DT >= V_YEAR_START_DATE OR B.PAYOFF_DT = TO_DATE('00010101','YYYYMMDD') OR NVL(A.CURRT_BAL,0) >0)
               ...
               ELSE 'N'
           END                                        AS YBT_FLG                    --一表通口径标识--ADD BY PSF 20250916
   FROM IOL.V_CMM_BILL_DISCOUNT_INFO A --票据转贴现信息
   INNER JOIN IOL.V_CMM_CORP_LOAN_DUBIL_INFO B --对公贷款借据信息
    ...
    LEFT JOIN IOL.V_CMM_CORP_LOAN_ACCT_INFO C --对公贷款账户信息
      ON C.DUBIL_NUM = B.DUBIL_ID
     AND C.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN IOL.V_CMM_BILL_CENTER_INFO J --票据中心信息
    ...
    LEFT JOIN IML.V_PTY_CPES_MEM TTA --票交所会员 只有一天数据
     ...
     AND TTA.RANK = 1
    LEFT JOIN IML.V_PTY_CPES_MEM TTB --票交所会员 只有一天数据
    ...
    LEFT JOIN (SELECT MIN(G.CUST_ID) AS CUST_ID
                 ...
                 FROM IOL.V_CMM_CORP_CUST_BASIC_INFO G --对公客户基本信息 mod by 调整客户号取值20230803
  ...
  INSERT/*+APPEND PARALLEL*/ INTO RRP_MDL.M_LOAN_IN_DUBILL_INFO_TMP
    ...
    ,YBT_FLG                     --一表通口径标识 --ADD BY PSF 20250916
     ...
     WITH UNITE_WL_REPAY_PLAN AS (
        SELECT A.DUBIL_ID
               ,A.INIT_TOT_PERDS TOT_PERDS
               ,A.INIT_REPAY_PERDS REPAY_PERDS
               ,A.REPAYBL_DT
               ,MAX(A.INIT_VALUE_DT) AS VALUE_DT
               ,A.ETL_DT
               ,A.PROD_ID
               ,SUM(CASE WHEN A.OVDUE_FLG = '1' AND A.REPAY_PERDS <> 0
                         THEN 1
                         ELSE 0 END ) AS OVDUE_FLG    --逾期标志1为是 0为否
               ,MIN(CASE WHEN A.REPAY_FLG = '0' AND A.REPAY_PERDS <> 0
                         THEN 0
                         ELSE 1 END ) AS REPAY_FLG    --未偿还
          FROM IOL.V_CMM_UNITE_WL_REPAY_PLAN A --联合网贷还款计划
         WHERE A.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
  ...
  UNITE_WL_REPAY_PLAN_1 AS (
        SELECT B.DUBIL_ID
               ...
               ,MAX(CASE WHEN B.VALUE_DT >= TO_DATE(V_P_DATE,'YYYYMMDD')
                         THEN 0
                         ELSE B.REPAY_PERDS
                         END )              AS CURR_PERDS
               ,MIN(B.TOT_PERDS)            AS WLD_CURR_PERDS
               ,SUM(CASE WHEN B.OVDUE_FLG >= 1 --逾期标志为是
                          AND B.REPAYBL_DT <= TO_DATE(V_P_DATE,'YYYYMMDD') - 1
                          AND B.REPAY_FLG = 0 --未偿还
                         THEN 1
                         ELSE 0 END)        AS LXQKQS --连续欠款期数
               ,SUM(CASE WHEN B.OVDUE_FLG >= 1 --逾期标志为是
                          AND B.REPAYBL_DT <= TO_DATE(V_P_DATE,'YYYYMMDD') - 1 --未偿还
                         THEN 1
                         ELSE 0 END)        AS LJQKQS --累计欠款期数
          FROM UNITE_WL_REPAY_PLAN B --联合网贷还款计划
         WHERE B.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
         GROUP BY B.DUBIL_ID ),
   CMM_UNITE_WL_LMT_INFO_QC AS ( --MOD BY YJY 20250425 取微业贷的额度合同号
         SELECT T.CUST_ID                            AS CUST_ID          --客户号
           ...
           FROM IOL.V_CMM_UNITE_WL_LMT_INFO T   --联合网贷额度信息
          WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
            ...
            AND T.BUS_BREED_ID IN ('203050100001'))--203050100001-微业贷
  SELECT /*PRALLEL(4)*/ V_P_DATE                                         AS DATA_DT                     --数据日期
        ,A.LP_ID                                                         AS LGL_REP_ID                  --法人编号
        ,A.DUBIL_ID                                                      AS ACC_ID                      --账户编号
        --MOD BY 20250725 回退核算改造前版本
        ,A.DUBIL_ID                                                      AS RCPT_ID                     --借据编号
        --,A.CORE_DUBIL_ID                                                 AS RCPT_ID                     --借据编号 MOD BY YJY 20250521 取核心借据号
        --,A.DUBIL_ID                                                      AS THIRD_RCPT_ID               --第三方借据编号 ADD BY YJY 20250521
        ,A.CORE_DUBIL_ID                                                 AS THIRD_RCPT_ID               --内部借据号 --MOD BY LIP 20250725 IMAS发放日期20250703-20250723的借据使用
        ...
        ,'Y'                                                             AS NET_LOAN_FLG               --互联网贷款标志
        ,'1'                                                             AS NET_LOAN_PROD_TYP          --网贷产品类别   暂定
        ,NULL                                                            AS CR_CRD_BIZ_OD_TYP          --类信用卡业务透支类型
        ...
        /*,A.DUBIL_ID                                                        AS LMT_CONT_ID                --额度合同编号 暂定*/
        ...
        ,NULL                                                              AS LOAN_MGR_ID                --借据主办客户经理号
        ,NULL                                                              AS LOAN_TELLER_ID             --借据主办柜员号
    ...
    FROM IOL.V_CMM_UNITE_WL_DUBIL_INFO A --联合网贷借据信息
    LEFT JOIN IOL.V_CMM_STD_PROD_INFO B --标准产品信息表
      ON B.PROD_ID = A.STD_PROD_ID
     AND B.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN IML.V_REF_PUB_CD C  --公共代码表 取贷款用途
      ON C.CD_ID = 'CD1274'
     AND C.CD_VAL = A.LOAN_USAGE_CD
    LEFT JOIN IOL.V_CMM_UNITE_WL_WRT_OFF_INFO D --联合网贷核销信息
      ON D.DUBIL_ID = A.DUBIL_ID
     AND D.STD_PROD_ID = '203050100001'
     AND D.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN IOL.V_CMM_UNITE_WL_LOAN_CONT_INFO E --联合网贷贷款合同信息
      ON E.CONT_ID = A.CONT_ID
     AND E.STD_PROD_ID = '203050100001'
     AND E.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN IML.V_PTY_PARTY_PHYS_ADDR_H F --当事人物理地址历史
      ON F.PARTY_ID = A.CUST_ID
     AND F.PHYS_ADDR_TYPE_CD = '001001'
    ...
    LEFT JOIN CMM_UNITE_WL_LMT_INFO_QC SX
     ...
     AND SX.RN = 1
   WHERE A.STD_PROD_ID = '203050100001' --微业贷
     AND A.DUBIL_ID IS NOT NULL
     AND A.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  ...
  SELECT B.DUBIL_ID            AS RCPT_ID     --借据编号
        ...
        ,'票据贴现'            AS DATA_SRC    --数据来源
  FROM IOL.V_CMM_BILL_DISCNT_INFO A
 INNER JOIN IOL.V_CMM_CORP_LOAN_DUBIL_INFO B --对公贷款借据信息
    ON B.BILL_UNIQ_MARK_ID = NVL(TRIM(A.BILL_ENTRY_ID),A.BILL_ID)
   AND B.STD_PROD_ID IN ('203020600001','203020400001','204010200001','204010200002')
   AND B.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
  LEFT JOIN (SELECT T.*
              ...
              WHERE T.RECV_OPINION_TYPE_CD = 'SU00'--签收意见类型代码：SU00：同意签收 SU01拒绝签收 其它:未知
                ...
                AND T.START_DT <= TO_DATE(V_P_DATE,'YYYYMMDD')
                AND T.END_DT > TO_DATE(V_P_DATE,'YYYYMMDD')) C
 ...
 WHERE A.DISCNT_STATUS_CD IN ('06')
  ...
  SELECT T3.DUBIL_ID           AS RCPT_ID     --借据编号
        ...
        ,'票据转贴现'          AS DATA_SRC    --数据来源
    FROM IOL.V_CMM_BILL_DISCOUNT_INFO T2 --票据转贴现信息
   INNER JOIN IOL.V_CMM_CORP_LOAN_DUBIL_INFO T3
      ON T3.BILL_ID = T2.BILL_ID
     AND T3.STD_PROD_ID IN ('204010100001', '204010100002') --新一代
     AND T3.ETL_DT = TO_DATE(V_P_DATE, 'YYYYMMDD')
    LEFT JOIN IML.V_EVT_SUGST_PAY_APPL_EVT T1 --提示付款申请事件
    ...
    LEFT JOIN (SELECT T.*
     ...
     AND C.RN = 1
    LEFT JOIN IOL.V_CMM_BILL_DISCOUNT_INFO D
  ...
  INSERT/*+APPEND PARALLEL*/ INTO RRP_MDL.M_LOAN_IN_DUBILL_INFO
    ...
    SELECT TA.CONT_ID
      ...
      FROM IML.V_AGT_LOAN_CONT_RELA_TAB_INFO_H TA  --贷款合同关联表信息历史 --MOD BY LIP 20240807
  ...
  SELECT T.DATA_DT                            --数据日期
        ...
        ,T.YBT_FLG                             --一表通口径标识     --ADD BY PSF 20250916
    FROM RRP_MDL.M_LOAN_IN_DUBILL_INFO_TMP T
    --MOD BY 20240319 取借新还旧的上笔借据号
    /*LEFT JOIN IML.V_AGT_LOAN_CONT_RELA_TAB_INFO_H TA --贷款合同关联表信息历史
...
END ETL_INIT_M_LOAN_IN_DUBILL_INFO;
/
