CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_MRPT_DEP_ACCT_FTP_REFLOW(I_P_DATE IN INTEGER,
                                                           O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_M_MRPT_DEP_ACCT_FTP_REFLOW
  *  功能描述：存款账户_FTP回流-手工报表专用
  *  创建日期：20221229
  *  开发人员：CYK
  *  来源表：RRP_MDL.O_IOL_FTPS_RPT_RST_FTP261  ftp初级报表账户明细结果集
             RRP_MDL.O_ICL_CMM_DEP_ACCT_INFO  存款账户信息表
             RRP_MDL.O_ICL_CMM_IFS_ACCT_INFO  联合存款账户信息表
  *  目标表：RRP_MDL.M_MRPT_DEP_ACCT_FTP_REFLOW 存款账户_FTP回流-手工报表专用
  *  配置表：
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME   VARCHAR2(30) := 'ETL_M_MRPT_DEP_ACCT_FTP_REFLOW'; -- 程序名称
...
BEGIN
  ...
  --清空目标表
  V_SQL :='TRUNCATE TABLE M_MRPT_DEP_ACCT_FTP_REFLOW';
  ...
  V_STEP_DESC := '插入存款账户活期/定期、同业存单';
  D_STARTTIME := SYSDATE;

  INSERT /*+ APPEND */ INTO RRP_MDL.M_MRPT_DEP_ACCT_FTP_REFLOW NOLOGGING
  (
			DATA_DT, 		         --数据日期
			ACCT_NO,                 --账号
			SUB_ACCT_NO,             --子账号
  ...
  SELECT  /*+ PARALLEL */
    ...
    INNER JOIN  RRP_MDL.O_ICL_CMM_DEP_ACCT_INFO T2  --存款账户信息表
       ON T1.T_ACCT_NO = T2.ACCT_ID
    WHERE T1.DATA_SRC IN ('ftp_curr_dep_acct',--存款账户活期
                         'ftp_time_dep_acct', --存款账户定期
                         'ftp_ibank_store_info')--同业存单
  ...
  SELECT  /*+ PARALLEL */
          ...
          ,T1.EXERCISE_INTEREST_RATE         -- 执行利率
     FROM RRP_MDL.O_IOL_FTPS_RPT_RST_FTP261 T1  --ftp初级报表账户明细结果集
    INNER JOIN RRP_MDL.O_ICL_CMM_IFS_ACCT_INFO T2  --联合存款账户信息表
       ON T1.T_ACCT_NO = T2.CUST_ACCT_ID
    WHERE T1.DATA_SRC = 'ftp_time_dep_acct'
      AND T1.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
...
END ETL_M_MRPT_DEP_ACCT_FTP_REFLOW;
/
