CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_O_ICL_CMM_DEP_CUST_ACCT_INFO(I_P_DATE IN INTEGER,
                              O_ERRCODE OUT VARCHAR2
                               )
  /**************************************************************************
  *  程序名称：ETL_INIT_O_ICL_CMM_DEP_CUST_ACCT_INFO
  *  功能描述：存款主账户信息
  *  创建日期：20220525
  *  开发人员：梅炜
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(500) := 'ETL_INIT_O_ICL_CMM_DEP_CUST_ACCT_INFO'; -- 程序名称
...
EXECUTE IMMEDIATE 'TRUNCATE TABLE '|| V_TABLE_NAME;
  ...
  V_TAB_NAME := 'O_ICL_CMM_DEP_CUST_ACCT_INFO'; --表名写
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
   -- EXECUTE IMMEDIATE ' TRUNCATE TABLE  O_ICL_CMM_DEP_CUST_ACCT_INFO ;
  -- -- EXECUTE IMMEDIATE ' TRUNCATE TABLE O_ICL_CMM_DEP_CUST_ACCT_INFO';
  ...
  (      ETL_DT  --数据日期
    ...
    SELECT
      ...
      ,DRAWDOWN_WAY_CD  --支取方式代码
      ,CASE WHEN
      ACCT_STATUS_CD = '0' THEN 'C'
      WHEN  ACCT_STATUS_CD = '1' THEN 'A' END --账户状态代码
      ,ACCT_DRAWDOWN_WAY_STATUS  --账户支取方式状态
      ,FROZ_STATUS_CD  --冻结状态代码
      ,STOP_PAY_STATUS_CD  --止付状态代码
      ,ACPT_PAY_STATUS_CD  --收付状态代码
      ...
      ,CUST_ACCT_CARD_NO  --客户账户卡号
      ,CURR_CD
    FROM ICL.V_CMM_DEP_CUST_ACCT_INFO@LINK_SIT2
    WHERE ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')  --视图-存款主账户信息
    ;
  ...
  END ETL_INIT_O_ICL_CMM_DEP_CUST_ACCT_INFO;
/
