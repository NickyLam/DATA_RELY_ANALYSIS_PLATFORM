CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IOL_IFRS_VAL_RPT_TRADE(I_P_DATE IN INTEGER,
                              O_ERRCODE OUT VARCHAR2
                               )
  /**************************************************************************
  *  程序名称：ETL_O_IOL_IFRS_VAL_RPT_TRADE
  *  功能描述：估值报告表
  *  创建日期：20220525
  *  开发人员：梅炜
  *  来源表：
  ...
  *             1    20220525  梅炜     首次创建
  *             2    20240102  hulj     调整判断数据日期
  ***************************************************************************/
  AS
  -- 定义变量 --
  V_STEP      INTEGER := 0; -- 处理步骤
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IOL_IFRS_VAL_RPT_TRADE'; -- 程序名称
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM O_IOL_IFRS_VAL_RPT_TRADE T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  ...
  SELECT --ETL_DT  --ETL处理日期
         /*CASE WHEN SUBSTR(TO_CHAR(D_DATA_DT,'YYYYMMDD'),5) = '1231' THEN D_DATA_DT
              ELSE D_DATA_DT +1
          END         --数据日期  --MODIFY BY XUXIAOBIN 20220927减估值的日期是对ETL_DT+1*/
          CASE WHEN SUBSTR(TO_CHAR(D_DATA_DT,'YYYYMMDD'),5) = '1231' AND SUBSTR(V_P_DATE,5) = '1231' THEN D_DATA_DT
               ELSE D_DATA_DT + 1 END
         ,D_DATA_DT  --数据日期
         ,V_CONTRACT_NO  --资产代码
         ,V_BUSINESSTYPE  --业务品种代码
         ,V_BUSINESSSUBTYPE  --业务子类型
    ...
    WHERE D_DATA_DT = CASE WHEN SUBSTR(V_P_DATE,5)='1231' THEN TO_DATE(V_P_DATE,'YYYYMMDD')
                           ELSE TO_DATE(V_P_DATE,'YYYYMMDD') -1 END
       OR D_DATA_DT = TO_DATE(V_P_DATE,'YYYYMMDD');--20220927 XUXIAOBIN MODIFY
...
END ETL_O_IOL_IFRS_VAL_RPT_TRADE;
/
