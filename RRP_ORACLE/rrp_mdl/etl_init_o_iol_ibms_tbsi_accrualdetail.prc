CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_O_IOL_IBMS_TBSI_ACCRUALDETAIL(I_P_DATE IN INTEGER, --跑批日期
                                                 O_ERRCODE  OUT VARCHAR2 --错误代码
                                                 )
 /*******************************************************************
  **存储过程详细说明： 债券补录信息表
  **存储过程名称：    ETL_INIT_O_IOL_IBMS_TBSI_ACCRUALDETAIL
  **存储过程创建日期：20220707
  **存储过程创建人：  赖海强
  **输入参数：        I_P_DATE
  **输出参数：        O_ERRCODE
 ...
 AS
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_INIT_O_IOL_IBMS_TBSI_ACCRUALDETAIL'; -- 程序名称
...
EXECUTE IMMEDIATE 'TRUNCATE TABLE '|| V_TABLE_NAME;
  ...
  --清理当天数据
  -- EXECUTE IMMEDIATE ' TRUNCATE TABLE RRP_MDL.O_IOL_IBMS_TBSI_ACCRUALDETAIL';

 INSERT /*+APPEND*/ INTO RRP_MDL.O_IOL_IBMS_TBSI_ACCRUALDETAIL NOLOGGING
    (I_CODE  --
    ,A_TYPE  --
    ,M_TYPE  --
    ,TG_CODE  --
  ...
  SELECT /*+PARALLEL*/
      ...
      ,END_DT  --结束时间
      ,ID_MARK  --增删标志
    FROM IOL.V_IBMS_TBSI_ACCRUALDETAIL   --待定_视图
   ;
...
END ETL_INIT_O_IOL_IBMS_TBSI_ACCRUALDETAIL;
/
