CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IOL_CTMS_TBS_V_COUPONDEALS(I_P_DATE IN INTEGER, --跑批日期
                                                 O_ERRCODE  OUT VARCHAR2 --错误代码
                                                 )
 /*******************************************************************
  **存储过程详细说明： 还本付息交易
  **存储过程名称：    ETL_O_IOL_CTMS_TBS_V_COUPONDEALS
  **存储过程创建日期：20220707
  **存储过程创建人：  赖海强
  **输入参数：        I_P_DATE
  **输出参数：        O_ERRCODE
 ...
 AS
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IOL_CTMS_TBS_V_COUPONDEALS'; -- 程序名称
  ...
  EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_IOL_CTMS_TBS_V_COUPONDEALS';
  ...
  -- 程序业务逻辑处理主体部分 --
  V_STEP := V_STEP + 1;
  V_STEP_DESC := '数据落地-还本付息交易';
  V_STARTTIME := SYSDATE;
  INSERT /*+APPEND*/ INTO RRP_MDL.O_IOL_CTMS_TBS_V_COUPONDEALS NOLOGGING
    (DEAL_ID	           --引用表ID
    ,DEAL_NAME	         --引用表名
    ,ASPCLIENT_ID	       --部门编号
    ,BALANCE_ID	         --引用表2ID
  ...
  SELECT /*+PARALLEL*/
    ...
    ,END_DT	             --结束时间
    ,ID_MARK	           --增删标志
    FROM IOL.V_CTMS_TBS_V_COUPONDEALS   --还本付息交易_视图
...
END ETL_O_IOL_CTMS_TBS_V_COUPONDEALS;
/
