CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IML_EVT_PRIV_ONL_BANK_TRAN_FLOW(I_P_DATE IN INTEGER, --跑批日期
                                                                  O_ERRCODE  OUT VARCHAR2 --错误代码
                                                                  )
 /*******************************************************************
  **存储过程详细说明： 对私网上银行交易流水表
  **存储过程名称：    ETL_O_IML_EVT_PRIV_ONL_BANK_TRAN_FLOW
  **存储过程创建日期：20221130
  **存储过程创建人：  HULIJUAN
  **输入参数：        I_P_DATE
  **输出参数：        O_ERRCODE
...
AS
  ...
  V_TAB_NAME  VARCHAR2(100) := 'O_IML_EVT_PRIV_ONL_BANK_TRAN_FLOW'; --表名
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IML_EVT_PRIV_ONL_BANK_TRAN_FLOW'; --程序名称
...
END ETL_O_IML_EVT_PRIV_ONL_BANK_TRAN_FLOW;
/
