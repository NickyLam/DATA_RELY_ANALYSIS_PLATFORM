CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_O_ICL_CMM_IBANK_NV_TYPE_PROD_INVEST(I_P_DATE IN INTEGER,
                              O_ERRCODE OUT VARCHAR2
                               )
  /**************************************************************************
  *  程序名称：ETL_INIT_O_ICL_CMM_IBANK_NV_TYPE_PROD_INVEST
  *  功能描述：同业净值型产品投资
  *  创建日期：20220525
  *  开发人员：梅炜
  *  来源表：
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_INIT_O_ICL_CMM_IBANK_NV_TYPE_PROD_INVEST'; -- 程序名称
...
EXECUTE IMMEDIATE 'TRUNCATE TABLE '|| V_TABLE_NAME;
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
   -- EXECUTE IMMEDIATE ' TRUNCATE TABLE  O_ICL_CMM_IBANK_NV_TYPE_PROD_INVEST ;
  -- -- EXECUTE IMMEDIATE ' TRUNCATE TABLE O_ICL_CMM_IBANK_NV_TYPE_PROD_INVEST';
  ...
  FROM (SELECT /*+ PARALLEL */ROW_NUMBER() OVER(PARTITION BY OBJ_ID
           ORDER BY A.ETL_DT DESC) RN,A.*
            FROM ICL.V_CMM_IBANK_NV_TYPE_PROD_INVEST/*@LINK_EDW*/ A
            )--视图-同业净值型产品投资
  ...
  END ETL_INIT_O_ICL_CMM_IBANK_NV_TYPE_PROD_INVEST;
/
