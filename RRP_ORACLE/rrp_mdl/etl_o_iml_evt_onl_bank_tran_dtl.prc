CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IML_EVT_ONL_BANK_TRAN_DTL(I_P_DATE IN INTEGER, --跑批日期
                                                            O_ERRCODE OUT VARCHAR2 --错误代码
                                                            )
 /*******************************************************************
  **存储过程详细说明： 网上银行转账明细表
  **存储过程名称：    ETL_O_IML_EVT_ONL_BANK_TRAN_DTL
  **存储过程创建日期：20221201
  **存储过程创建人：  HULIJUAN
  **输入参数：        I_P_DATE
  **输出参数：        O_ERRCODE
...
AS
  ...
  V_TAB_NAME  VARCHAR2(50) := 'O_IML_EVT_ONL_BANK_TRAN_DTL'; --表名
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IML_EVT_ONL_BANK_TRAN_DTL'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_IML_EVT_ONL_BANK_TRAN_DTL';
  ...
  V_STARTTIME := SYSDATE;
  INSERT /*+APPEND*/ INTO RRP_MDL.O_IML_EVT_ONL_BANK_TRAN_DTL NOLOGGING
    (EVT_ID                              --事件编号
    ,LP_ID                               --法人编号
    ,TRAN_FLOW_NUM                       --转账流水号
  ...
  SELECT /*+PARALLEL*/
        ...
        ,JOB_CD                              --任务代码
        ,ETL_TIMESTAMP                       --数据处理时间
    FROM IML.V_EVT_ONL_BANK_TRAN_DTL   --视图网上银行转账明细表
...
END ETL_O_IML_EVT_ONL_BANK_TRAN_DTL;
/
