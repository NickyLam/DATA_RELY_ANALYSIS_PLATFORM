CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_A_FGB_FIVELEVEL_CLASS
(I_P_DATE IN INTEGER,
 O_ERRCODE OUT VARCHAR2
)
  /**************************************************************************
  *  程序名称：ETL_A_FGB_FIVELEVEL_CLASS
  *  功能描述：
  *  创建日期：20221107
  *  开发人员：孙满洋
  *  来源表：
  *  目标表：A_FGB_FIVELEVEL_CLASS --五级分类变动台帐
  *  配置表：CODE_MAP
  *  修改情况：
     序号  修改日期   修改人          修改原因
  *   1    20230306   sunmanyang      首次创建
  *   2    20230411   liuyu           贷款产品编号改为取贷款产品名称
      3    20250520   lwb              新增变动标志等两个字段
  ***************************************************************************/
  AS
  -- 定义变量 --
  V_STEP       INTEGER := 0;     -- 处理步骤
  V_PROC_NAME  VARCHAR2(30) := 'ETL_A_FGB_FIVELEVEL_CLASS';
                                 -- 程序名称
  ...
  V_TAB_NAME := 'A_FGB_FIVELEVEL_CLASS'; --表名,写目标表表名
  ...
  ETL_PARTITION_ADD(I_P_DATE, 'A_FGB_FIVELEVEL_CLASS', '1', O_ERRCODE);
  ...
  -- 程序业务逻辑处理主体部分 --
  V_STEP      := V_STEP + 1;
  V_STEP_DESC := '插入G12五级分类变动信息';
  V_STARTTIME := SYSDATE;

  INSERT INTO A_FGB_FIVELEVEL_CLASS
   (
     BGRQ,                     -- 1. 报告日期
     DATA_SRC                  -- 2. 数据来源
    ,JYWYM                     -- 3.交易唯一码
    ...
    SELECT V_P_DATE                  AS BGRQ -- 1.报告日期
      ...
      FROM S_FIVELEVEL_CLASS_CHANGE A --五级分类变动台账
...
-- 数据重复校验 --
  WITH TMP1 AS (
    SELECT BGRQ,JYWYM,COUNT(1)
      FROM RRP_MDL.A_FGB_FIVELEVEL_CLASS T
     WHERE BGRQ = V_P_DATE
     GROUP BY BGRQ,JYWYM
    HAVING COUNT(1) > 1)
