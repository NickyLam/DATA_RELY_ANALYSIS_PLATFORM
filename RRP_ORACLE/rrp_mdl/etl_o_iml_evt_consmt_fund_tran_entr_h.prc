CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IML_EVT_CONSMT_FUND_TRAN_ENTR_H(I_P_DATE IN INTEGER, --跑批日期
                                                                  O_ERRCODE OUT VARCHAR2 --错误代码
                                                                  )
 /*******************************************************************
  **存储过程详细说明： 代销基金交易委托历史表
  **存储过程名称：    ETL_O_IML_EVT_CONSMT_FUND_TRAN_ENTR_H
  **存储过程创建日期：20221130
  **存储过程创建人：  HULIJUAN
  **输入参数：        I_P_DATE
  **输出参数：        O_ERRCODE
...
AS
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IML_EVT_CONSMT_FUND_TRAN_ENTR_H'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_IML_EVT_CONSMT_FUND_TRAN_ENTR_H';
  ...
  -- 程序业务逻辑处理主体部分 --
  V_STEP := 2;
  V_STEP_DESC := '数据落地-代销基金交易委托历史表';
  V_STARTTIME := SYSDATE;
  INSERT /*+APPEND*/ INTO RRP_MDL.O_IML_EVT_CONSMT_FUND_TRAN_ENTR_H NOLOGGING
    (EVT_ID                         --事件编号
    ,LP_ID                          --法人编号
    ,ENTR_FLOW_NUM                  --委托流水号
  ...
  SELECT /*+PARALLEL*/
        ...
        ,REDEM_ACCT_ID	                --赎回账户编号  ADD BY YJY 20251010
        --,ETL_TIMESTAMP                  --数据处理时间
    FROM IML.V_EVT_CONSMT_FUND_TRAN_ENTR_H   --代销基金交易委托历史表--视图
  ...
  ETL_DBMS_STATS(V_P_DATE, 'O_IML_EVT_CONSMT_FUND_TRAN_ENTR_H','', O_ERRCODE);
...
END ETL_O_IML_EVT_CONSMT_FUND_TRAN_ENTR_H;
/
