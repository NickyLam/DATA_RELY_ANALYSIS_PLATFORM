CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IOL_ICMS_MYBKZQ_CS_EXTENT_INFO(I_P_DATE IN INTEGER,
                                                                         O_ERRCODE OUT VARCHAR2
                                                                         )
  /**************************************************************************
  *  程序名称：ETL_O_IOL_ICMS_MYBKZQ_CS_EXTENT_INFO
  *  功能描述：网商贷债权直转初审数据信息
  *  创建日期：20241105
  *  开发人员：YJY
  *  来源表： IOL.V_ICMS_MYBKZQ_CS_EXTENT_INFO    数仓每天增量提供，下午2点起批，数据日期为T-2
  *  目标表： O_IOL_ICMS_MYBKZQ_CS_EXTENT_INFO
  *  配置表：
...
AS
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IOL_ICMS_MYBKZQ_CS_EXTENT_INFO'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  DELETE FROM RRP_MDL.O_IOL_ICMS_MYBKZQ_CS_EXTENT_INFO T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  --EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_IOL_ICMS_MYBKZQ_CS_EXTENT_INFO';
  ...
  -- 程序业务逻辑处理主体部分 --
  V_STEP := V_STEP + 1;
  V_STEP_DESC := '数据落地-网商贷债权直转初审数据信息';
  V_STARTTIME := SYSDATE;
  INSERT INTO RRP_MDL.O_IOL_ICMS_MYBKZQ_CS_EXTENT_INFO
    (
      SERIALNO                                     --信贷流水号              
     ,APPLYNO                                      --债转申请单据号          
     ,BUSINESSSCENE                                --业务类型                
  ...
  SELECT  
     ...
     ,ETL_DT+1                                      --ETL处理日期          
     ,ETL_TIMESTAMP                                --ETL处理时间戳                
    FROM IOL.V_ICMS_MYBKZQ_CS_EXTENT_INFO  --视图-网商贷债权直转初审数据信息
...
END ETL_O_IOL_ICMS_MYBKZQ_CS_EXTENT_INFO;
/
