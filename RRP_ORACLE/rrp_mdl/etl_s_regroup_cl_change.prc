CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_S_REGROUP_CL_CHANGE(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_S_REGROUP_CL_CHANGE
  *  功能描述：五级分类变动表
  *  创建日期：20240425
  *  开发人员：lwb
  *  来源表：
  *  目标表：  S_REGROUP_CL_CHANGE
  *  配置表：
  *  修改情况：序号  修改日期  修改人   修改原因
  *              1    20240425  lwb      新增重组贷款变动表
  ***************************************************************************/
  AS
  -- 定义变量 --
  V_STEP        INTEGER := 0;                                  -- 处理步骤
  V_PROC_NAME   VARCHAR2(100) := 'ETL_S_REGROUP_CL_CHANGE';     -- 程序名称
  ...
  V_TAB_NAME := 'S_REGROUP_CL_CHANGE'; --表名
  ...
  V_STEP := 3;
  V_STEP_DESC := '零售五级分类变动表';
  V_STARTTIME := SYSDATE;
  INSERT /*+APPEND*/ INTO S_REGROUP_CL_CHANGE NOLOGGING
  (
        DATA_DT                                               --数据日期
        ,RID                                                    --流水号RID
        ,CUST_ID                                              --客户号
        ...
        ,ADJ_DT                                                --调整日期
        ,DATA_SRC                                             --数据来源
        ,REGROUP_LOAN_FLG_BF                                  --重组贷款标志前
        ,REGROUP_LOAN_FLG_AF                                  --重组贷款标志后
        ,REGROUP_LOAN_TYPE_CD_BF                              --重组贷款类型代码前
        ,REGROUP_LOAN_TYPE_CD_AF                              --重组贷款类型代码后
          )
       SELECT
            ...
            ,T1.REGROUP_LOAN_FLG            AS REGROUP_LOAN_FLG_BF    --重组贷款标志前
            ,T3.REGROUP_LOAN_FLG            AS REGROUP_LOAN_FLG_AF    --重组贷款标志后
            ,T1.REGROUP_LOAN_TYPE_CD        AS REGROUP_LOAN_TYPE_CD_BF   --重组贷款类型代码前
            ,T3.REGROUP_LOAN_TYPE_CD        AS REGROUP_LOAN_TYPE_CD_AF   --重组贷款类型代码后
 ...
 INNER JOIN RRP_MDL.M_LOAN_IN_DUBILL_INFO  T3
       ...
       AND T3.DATA_SRC IN ('对公信贷','零售贷款','联合网贷')
     WHERE T1.DATA_DT = V_LDAY
       AND T1.DATA_SRC IN ('对公信贷','零售贷款','联合网贷')
       AND T1.REGROUP_LOAN_FLG='N'
       AND T3.REGROUP_LOAN_FLG='Y'
   ;
