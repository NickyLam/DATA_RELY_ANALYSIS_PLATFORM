CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IML_EVT_CORP_RGST_BILL_CCUTION_EVT(I_P_DATE IN INTEGER,
                                                                     O_ERRCODE OUT VARCHAR2
                                                                     )
  /**************************************************************************
  *  程序名称：ETL_O_IML_EVT_CORP_RGST_BILL_CCUTION_EVT
  *  功能描述：企业登记中心票据流转事件
  *  创建日期：20230215
  *  开发人员：MW
  *  来源表：
...
AS
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IML_EVT_CORP_RGST_BILL_CCUTION_EVT'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM RRP_MDL.O_IML_EVT_CORP_RGST_BILL_CCUTION_EVT T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_IML_EVT_CORP_RGST_BILL_CCUTION_EVT';
  ...
  ETL_DBMS_STATS(V_P_DATE, 'O_IML_EVT_CORP_RGST_BILL_CCUTION_EVT','', O_ERRCODE);
...
END ETL_O_IML_EVT_CORP_RGST_BILL_CCUTION_EVT;
/
