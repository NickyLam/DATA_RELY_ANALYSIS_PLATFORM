CREATE OR REPLACE PROCEDURE RRP_MDL."ETL_INIT_O_IML_REF_TRAN_BANK_CODE_PARA" (I_P_DATE IN INTEGER, --跑批日期
                                                 O_ERRCODE  OUT VARCHAR2 --错误代码
                                                 )
 /*******************************************************************
  **存储过程详细说明： 交易银行交易码参数
  **存储过程名称：    ETL_INIT_O_IML_REF_TRAN_BANK_CODE_PARA
  **存储过程创建日期：20221129
  **存储过程创建人：  HULIJUAN
  **输入参数：        I_P_DATE
  **输出参数：        O_ERRCODE
 ...
 AS
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_INIT_O_IML_REF_TRAN_BANK_CODE_PARA'; -- 程序名称
...
EXECUTE IMMEDIATE 'TRUNCATE TABLE '|| V_TABLE_NAME;
 ...
 INSERT /*+APPEND*/ INTO RRP_MDL.O_IML_REF_TRAN_BANK_CODE_PARA NOLOGGING
  ...
  SELECT /*+PARALLEL*/
    ...
    ,JOB_CD        --任务代码
    ,ETL_TIMESTAMP --数据处理时间
    FROM IML.V_REF_TRAN_BANK_CODE_PARA   交易银行交易码参数--视图
...
END ETL_INIT_O_IML_REF_TRAN_BANK_CODE_PARA;
/
