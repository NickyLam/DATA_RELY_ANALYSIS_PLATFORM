CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_M_CPTL_REPO_LBY_INFO(I_P_DATE IN INTEGER,
                                                O_ERRCODE OUT VARCHAR2
                                                )
  /**************************************************************************
  *  程序名称：ETL_INIT_M_CPTL_REPO_LBY_INFO
  *  功能描述：回购业务（负债方）信息
  *  创建日期：20220608
  *  开发人员：梅炜
  *  来源表：
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(200) := 'ETL_INIT_M_CPTL_REPO_LBY_INFO'; -- 程序名称
  ...
  V_TAB_NAME := 'M_CPTL_REPO_LBY_INFO'; --表名
  ...
  FROM (
    SELECT
        ...
        ,CASE WHEN A.BUS_TYPE_CD ='BT02' THEN A.DISCNT_INT_RAT
              WHEN A.BUS_TYPE_CD ='BT03' THEN A.REDEM_INT_RAT  END
                                                 AS APPT_RESL_OR_REPO_RATE               --约定返售或回购利率
                                                 --modify 20220607 修改买断式和质押式回购利率取值 laihaiqiang
        ,CASE WHEN A.BUS_TYPE_CD ='BT02' THEN A.INT_AMT
              WHEN A.BUS_TYPE_CD ='BT03' THEN A.REPO_INT_AMT  END
                                                 AS APPT_RESL_OR_REPO_INT                --约定返售或回购利息
    ...
    AND A.TRAN_DIR_CD = '02'  --卖出 待确认是否02是卖出  --MODIFY BY MW 20221207上游调整码值
    AND A.BUS_TYPE_CD IN ('BT02' ,'BT03') -- BT00-未知 BT01-转贴现 BT02-质押式回购 BT03-买断式回购 BT06-央行卖票
    AND A.ENTRY_STATUS_CD = '03'  --筛选记账成功的票据
   /* AND (A.ACTL_REPO_DT >= V_START_DT OR A.ACTL_REPO_DT IS NULL)*/ --去除条件筛选，报表自行筛选
  ) T
 -- WHERE T.NUM = 1 --modify by laihaiqiang 202206011 根据业务口径修改，显示所有回购交易记录
  ;
  ...
  SELECT TO_CHAR(A.ETL_DT,'YYYYMMDD')  AS DATA_DT    --数据日期
    ,A.LP_ID AS LGL_REP_ID                           --法人编号
    ,CASE WHEN A.CUST_ID=' ' THEN A.CNTPTY_ID ELSE A.CUST_ID
      END AS CUST_ID                                 --客户编号
    ,A.ENTRY_ORG_ID AS ORG_ID                             --机构编号
    ,--A.BUS_ID AS ACC_ID                             --账户编号
    /*SUBSTR(A.BAG_ID || '.' || A.BOND_ID_COMB,1,INSTR(A.BAG_ID || '.' || A.BOND_ID_COMB,'.')-1)*/
  ...
  SELECT TO_CHAR(A.ETL_DT,'YYYYMMDD') AS DATA_DT     --数据日期
      ,A.LP_ID AS LGL_REP_ID                         --法人编号
     -- ,CASE WHEN A.CUST_ID=' ' THEN A.CNTPTY_ID ELSE A.CUST_ID END
      ,A.CUST_ID AS CUST_ID
      ,A.ENTRY_ORG_ID                                  --机构编号
      ,/*A.BUS_ID*/A.BAG_ID  AS ACC_ID                --账户编号 20221108 XUXIAOBIN MODIFY
      ,NULL AS ACC_TYP                                 --账户类型
      ,A.CURR_CD AS CUR                                --币种
      ...
      ,NULL AS RATE_FLT_FREQ                           --利率浮动频率
      ,NULL AS PRC_BASE_TYP                            --定价基准类型
      ,'07' AS INT_CALC_MODE   --20220929 XUXIAOBIN ADD--计息方式
      ,NULL AS ACT_END_DT                              --实际终止日期
...
/*  LEFT JOIN RRP_MDL.O_IML_PTY_FX_CAP_CNTPTY E --外汇资金交易对手表  --MODIFY BY XUXIAOBIN 2022/07/12
  ...
  SELECT TO_CHAR(A.ETL_DT,'YYYYMMDD') AS DATA_DT     --数据日期
      ...
      DECODE(NVL(TRIM(D.CUST_ID), NVL(TRIM(D.SRC_PARTY_ID), '-')),
        '-',
        '9620007313',
        NVL(TRIM(D.CUST_ID), NVL(TRIM(D.SRC_PARTY_ID), '-'))) AS CUST_ID                               --客户编号  --存款保险用CUST_ID_DIIS填报客户编号
      ,A.BELONG_ORG_ID    AS ORG_ID                            --机构编号
      ,A.BUS_ID AS ACC_ID                             --账户编号
      ,NULL AS ACC_TYP                                --账户类型
      ,A.CURR_CD AS CUR                               --币种
      --,'20101' AS REPO_BIZ_TYP                        --回购业务类型
      ,CASE WHEN A.STD_PROD_ID IN ('401030100001','401030300001','401030300002') THEN '20202'
            WHEN A.STD_PROD_ID IN ('401030200001','401030400001') THEN '20201'
       END AS REPO_BIZ_TYP                        --回购业务类型
      ,'1' AS ULYG_AST_TYP                            --标的资产类型 1.债券 2：商业汇票 3:其他票据 4:信贷资产 5:股票及其他股权 6 黄金 0 其他标的物
      ...
      ,A.BUS_ID  AS BUS_ID                            --业务编号
      ,A.ACRU_INT ASCURRT_ACRU_INT                    --当期应计利息 add by 20221124 xucx
  ...
  FROM RRP_MDL.O_ICL_CMM_IBANK_CASH_DEBIT_CRDT A  --同业现金借贷
   LEFT JOIN O_IML_PTY_IBANK_CNTPTY_INFO D --同业交易对手信息表  ---MODIFY BY XUXIAOBIN 2022/07/12
    ON A.CNTPTY_ID = D.SRC_PARTY_ID --MODIFY BY ZM 20200903 A.CNTPTY_ID = D.PARTY_ID
   AND D.ETL_DT = A.ETL_DT

  WHERE --A.SUBJ_ID LIKE '2003%'   --同业现金借贷卖出回购
        A.STD_PROD_ID LIKE '40103%'  --卖出回购
 ...
 -- 如需要分析表，请用如下代码 --
  ...
  END ETL_INIT_M_CPTL_REPO_LBY_INFO;
/
