CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_ICL_CMM_INTNAL_ORG_INFO(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_O_ICL_CMM_INTNAL_ORG_INFO
  *  功能描述：内部机构信息表
  *  创建日期：20220525
  *  开发人员：梅炜
  *  来源表：
...
AS
  ...
  --V_TAB_NAME  VARCHAR2(100):= 'O_ICL_CMM_INTNAL_ORG_INFO'; --表名
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_ICL_CMM_INTNAL_ORG_INFO'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  DELETE FROM RRP_MDL.O_ICL_CMM_INTNAL_ORG_INFO T WHERE ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  --EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_ICL_CMM_INTNAL_ORG_INFO';
...
END ETL_O_ICL_CMM_INTNAL_ORG_INFO;
/
