CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IOL_LESS_LES_PEER_CUSTOMER_STATE(I_P_DATE IN INTEGER,
                              O_ERRCODE OUT VARCHAR2
                               )
  /**************************************************************************
  *  程序名称：ETL_O_IOL_LESS_LES_PEER_CUSTOMER_STATE
  *  功能描述：同业单一客户大额风险暴露情况表
  *  创建日期：20230607
  *  开发人员：MW
  *  来源表： IOL.V_LESS_LES_PEER_CUSTOMER_STATE
  *  目标表： O_IOL_LESS_LES_PEER_CUSTOMER_STATE
  *  配置表：
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IOL_LESS_LES_PEER_CUSTOMER_STATE'; -- 程序名称
  ...
  V_SYSTEM := '监管报送'; -- 默认写监管报送系统，有真实来源的按实际写
	V_TAB_NAME := 'O_IOL_LESS_LES_PEER_CUSTOMER_STATE';
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM O_IOL_LESS_LES_PEER_CUSTOMER_STATE T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  EXECUTE IMMEDIATE 'TRUNCATE TABLE O_IOL_LESS_LES_PEER_CUSTOMER_STATE';
  ...
  V_STEP_DESC := '数据落地-同业单一客户大额风险暴露情况表';
  V_STARTTIME := SYSDATE;
  INSERT INTO RRP_MDL.O_IOL_LESS_LES_PEER_CUSTOMER_STATE
  (
					DATADATE   --数据日期
					,SEQITEM   --序号
					,CUSTOMERTYPE   --客户类型
    ...
    SELECT
					...
					,SUMMOVERISK   --不考虑风险缓释作用的风险暴露总和
					--,ETL_DT   --ETL处理日期
    FROM IOL.V_LESS_LES_PEER_CUSTOMER_STATE  --视图-同业单一客户大额风险暴露情况表
;
  ...
  END ETL_O_IOL_LESS_LES_PEER_CUSTOMER_STATE;
/
