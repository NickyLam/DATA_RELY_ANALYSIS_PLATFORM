CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_LOAN_POV_ALLE_SUB(I_P_DATE IN INTEGER,
                                                    O_ERRCODE OUT VARCHAR2
                                                    )
/**************************************************************************
 *  程序名称：ETL_M_LOAN_POV_ALLE_SUB
 *  功能描述：精准扶贫贷款有关信息
 *  创建日期：20220615
 *  开发人员：梅炜
 *  来源表：
 *  目标表：  ETL_M_LOAN_POV_ALLE_SUB
 *  配置表：  CODE_MAP
 *  修改情况：序号  修改日期  修改人   修改原因
 *             1    20220523  梅炜      首次创建
 *             2    20250521  YJY       修改联合网贷部分的借据号，取联合网贷借据表的核心借据编号
 *             3    20250725  YJY       回退联合网贷部分的借据号
 *             4    20250821  YJY       调整联合网贷中借据金额的逻辑加工，对内部结转标志进行优化
 ************************************************************************/
AS
  -- 定义变量 --
  ...
  V_TAB_NAME  VARCHAR2(100) := 'M_LOAN_POV_ALLE_SUB'; --表名
  V_PROC_NAME VARCHAR2(100) := 'ETL_M_LOAN_POV_ALLE_SUB'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM RRP_MDL.M_LOAN_POV_ALLE_SUB T WHERE T.DATA_DT = V_P_DATE;--普通表的重跑处理
  /*EXECUTE IMMEDIATE ('ALTER TABLE '||'ETL_M_LOAN_POV_ALLE_SUB'||' TRUNCATE PARTITION '||'写上分区名');--分区表的重跑处理*/
  ...
  INSERT INTO RRP_MDL.M_LOAN_IN_DUBILL_INFO_TEMP03  --表内借据信息--精准扶贫按证件
    (CERT_NO      --01 证件号
    ,TPZT         --02 脱贫状态
  ...
  SELECT CERT_NO     --01 证件号
    ...
    FROM (SELECT P1.PKHSFZH         AS CERT_NO     --01 证件号
                ,'已脱贫'           AS TPZT        --02 脱贫状态
                ,'2021-04'          AS ACCT_DURAN  --03 扶贫名录期间
  ...
  SELECT V_YESTADAY   		               AS DATA_DT                --数据日期
        ...
        ,B.DUBIL_NUM/*A.DUBIL_ID */      AS RCPT_ID                --借据编号  
        ,'N'                             AS POV_ALLE_LOAN_FLG      --精准扶贫贴息贷款标志  
        ,'A01'                           AS POV_ALLE_LOAN_BIZ_VRTY --扶贫贷款业务品种
        ,NULL                            AS POV_ALLE_DRV_NUM       --扶贫带动人数
        ,NULL                            AS POV_ALLE_DRV_SHK_NUM   --扶贫带动已脱贫人数
        ,NULL                            AS POV_ALLE_SML_LOAN_FLG  --扶贫小额信贷标志
        ,NULL                            AS POV_ALLE_LOAN_IDNT_TYP --扶贫贷款认定类型
        ,NULL                            AS SPCL_FIN_DEBT_LOAN_FLG --专项金融债贷款标志
        ,NULL                            AS DEPT_LINE              --部门条线
        ,'零售部分'                      AS DATA_SRC               --数据来源
        ...
        ,B.STD_PROD_ID                   AS STD_PROD_ID
    FROM RRP_MDL.O_ICL_CMM_RETL_LOAN_ACCT_INFO B --零售贷款账户信息
    LEFT JOIN RRP_MDL.O_ICL_CMM_RETL_LOAN_DUBIL_INFO A --零售贷款借据信息
      ON A.DUBIL_ID = B.DUBIL_NUM
     AND A.ETL_DT = TO_DATE(V_YESTADAY,'YYYYMMDD')/*B.ETL_DT*/
    LEFT JOIN RRP_MDL.ADD_POVERTY_RELIF J -- 改关联业务20211231报送表为基表取精准扶贫名单
      ON J.LOAN_NUM = B.DUBIL_NUM
    LEFT JOIN RRP_MDL.M_LOAN_IN_DUBILL_INFO_TEMP04 J1
      ON J1.CUST_ID = B.CUST_ID
  ...
  SELECT V_YESTADAY                      AS DATA_DT                --数据日期
        ...
        ,CASE WHEN J.POVERTY_LOAN_FLG LIKE '%返贫%' OR J.POVERTY_LOAN_FLG LIKE '%未脱贫%' THEN '101'
              ...
              ELSE ''
          END                            AS REC_POOR_TYP           --建档立卡贫困户类型
        ,CASE WHEN REPLACE(A.INTNAL_CARR_FLG,'-','0') = '1'  --MOD BY YJY 20250821 
               AND A.ACCT_INSTIT_ID LIKE '897%' THEN 0
              ELSE A.DUBIL_AMT
          END                            AS AMT                    --借款金额
        ,A.STD_PROD_ID                   AS STD_PROD_ID            --标准产品号
    FROM RRP_MDL.O_ICL_CMM_UNITE_WL_DUBIL_INFO A  --联合网贷借据信息
    LEFT JOIN RRP_MDL.ADD_POVERTY_RELIF J -- 修改关联业务20211231报送表为基表取精准扶贫名单
      ON J.LOAN_NUM = A.DUBIL_ID
    LEFT JOIN RRP_MDL.M_LOAN_IN_DUBILL_INFO_TEMP04 J1
      ON J1.CUST_ID = A.CUST_ID
     AND J1.ACCT_DURAN = '2021-04'
   WHERE (J.LOAN_NUM IS NOT NULL OR J1.CUST_ID IS NOT NULL)
     AND ((A.DISTR_DT < DATE'2021-05-01') OR (A.DISTR_DT >= DATE'2021-05-01' AND A.STD_PROD_ID <> '202010100003'))
  ...
  WITH TMP1 AS (
    SELECT DATA_DT, RCPT_ID,COUNT(1)
      FROM RRP_MDL.M_LOAN_POV_ALLE_SUB T
