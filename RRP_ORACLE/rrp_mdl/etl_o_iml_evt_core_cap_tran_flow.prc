CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IML_EVT_CORE_CAP_TRAN_FLOW(I_P_DATE IN INTEGER, --跑批日期
                                                             O_ERRCODE OUT VARCHAR2 --错误代码
                                                             )
 /*******************************************************************
  **存储过程详细说明： 核心资金交易流水
  **存储过程名称：    ETL_O_IML_EVT_CORE_CAP_TRAN_FLOW
  **存储过程创建日期：20220315
  **存储过程创建人：  严唯正
  **输入参数：        I_P_DATE
  **输出参数：        O_ERRCODE
...
AS
  ...
  V_TAB_NAME  VARCHAR2(50) := 'O_IML_EVT_CORE_CAP_TRAN_FLOW'; --表名
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IML_EVT_CORE_CAP_TRAN_FLOW'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_IML_EVT_CORE_CAP_TRAN_FLOW';
  ...
  -- 程序业务逻辑处理主体部分 --
  V_STEP := 2;
  V_STEP_DESC := '数据落地-核心资金交易流水';
  V_STARTTIME := SYSDATE;
  INSERT /*+APPEND*/ INTO RRP_MDL.O_IML_EVT_CORE_CAP_TRAN_FLOW NOLOGGING
    (EVT_ID                              --事件编号
    ,LP_ID                               --法人编号
    ,TRAN_DT                             --交易日期
    ,TRAN_FLOW_NUM                       --交易流水号
  ...
  SELECT /*+PARALLEL*/
        ...
        ,SRC_TABLE_NAME                      --源表名称
        ,JOB_CD                              --任务代码
    FROM IML.V_EVT_CORE_CAP_TRAN_FLOW  --核心资金交易流水_视图
...
END ETL_O_IML_EVT_CORE_CAP_TRAN_FLOW;
/
