CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_O_ICL_CMM_IBANK_SECU_POST(I_P_DATE IN INTEGER,
                              O_ERRCODE OUT VARCHAR2
                               )
  /**************************************************************************
  *  程序名称：ETL_INIT_O_ICL_CMM_IBANK_SECU_POST
  *  功能描述：同业证券持仓
  *  创建日期：20220525
  *  开发人员：梅炜
  *  来源表：
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_INIT_O_ICL_CMM_IBANK_SECU_POST'; -- 程序名称
  ...
  V_TAB_NAME := 'O_ICL_CMM_IBANK_SECU_POST'; --表名
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM O_ICL_CMM_IBANK_SECU_POST T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  --EXECUTE IMMEDIATE 'TRUNCATE TABLE O_ICL_CMM_IBANK_SECU_POST';
  ...
  (      ETL_DT  --数据日期
    ...
    SELECT
      ...
      ,CURR_ISSUE_BUILD_UP_POS_DT  --本期建仓日期
    ,JOB_CD --任务代码
    /*FROM ICL.V_CMM_IBANK_SECU_POST  --视图-同业证券持仓
    WHERE ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')*/
    FROM (SELECT /*+ PARALLEL */ROW_NUMBER() OVER(PARTITION BY OBJ_ID
           ORDER BY A.ETL_DT DESC) RN,A.*
            FROM O_ICL_CMM_IBANK_SECU_POST_20231117/*@LINK_EDW*/ A --20230112 XUXIAOBIN特殊处理
           WHERE TRUNC(A.ETL_DT,'MM') = TRUNC(TO_DATE(V_P_DATE,'YYYYMMDD'),'MM')
             AND A.ETL_DT <= TO_DATE(V_P_DATE,'YYYYMMDD'))--同业证券持仓_视图
  ...
  END ETL_INIT_O_ICL_CMM_IBANK_SECU_POST;
/
