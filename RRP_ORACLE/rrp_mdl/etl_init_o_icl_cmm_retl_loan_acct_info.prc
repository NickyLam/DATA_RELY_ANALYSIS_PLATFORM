CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_O_ICL_CMM_RETL_LOAN_ACCT_INFO(I_P_DATE IN INTEGER,
                              O_ERRCODE OUT VARCHAR2
                               )
  /**************************************************************************
  *  程序名称：ETL_INIT_O_ICL_CMM_RETL_LOAN_ACCT_INFO
  *  功能描述：零售贷款账户信息
  *  创建日期：20220525
  *  开发人员：梅炜
  *  来源表：
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_INIT_O_ICL_CMM_RETL_LOAN_ACCT_INFO'; -- 程序名称
...
EXECUTE IMMEDIATE 'TRUNCATE TABLE '|| V_TABLE_NAME;
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
   -- EXECUTE IMMEDIATE ' TRUNCATE TABLE  O_ICL_CMM_RETL_LOAN_ACCT_INFO ;
  -- -- EXECUTE IMMEDIATE ' TRUNCATE TABLE O_ICL_CMM_RETL_LOAN_ACCT_INFO';
  ...
  (  			ETL_DT  --数据日期
    ...
    SELECT
					...
					,PED_DISTR_FLG  --周期性放款标志
					,DISTR_WAY_CD  --放款方式代码
					,CASE WHEN REPAY_WAY_CD = '1' THEN '15'--自定义组合还款 --还款方式代码
     WHEN REPAY_WAY_CD = '120' THEN '3'
     WHEN REPAY_WAY_CD = '210' THEN '2'
     WHEN REPAY_WAY_CD = '220' THEN '1'
     WHEN REPAY_WAY_CD = '300' THEN '5' ELSE REPAY_WAY_CD END
					,IRR_REPAY_WAY_CD  --不规则还款方式代码
					,REPAY_PED_CORP_CD  --还款周期单位代码
					,REPAY_PED  --还款周期
					,ALLOW_ADV_REPAY_FLG  --允许提前还款标志
					...
					,ETL_TIMESTAMP  --数据处理时间
					,GRACE_PERIOD_PRIC  --宽限期本金
    FROM ICL.V_CMM_RETL_LOAN_ACCT_INFO  --视图-零售贷款账户信息
    WHERE ETL_DT = (SELECT MAX(ETL_DT)FROM ICL.V_CMM_RETL_LOAN_ACCT_INFO)
    ;
  ...
  END ETL_INIT_O_ICL_CMM_RETL_LOAN_ACCT_INFO;
/
