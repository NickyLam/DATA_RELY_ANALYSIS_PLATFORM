CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IOL_RWAS_RPT_REPO(I_P_DATE IN INTEGER, --跑批日期
                                                                 O_ERRCODE  OUT VARCHAR2 --错误代码
                                                               )
 /*******************************************************************
  **存储过程详细说明：内部管理报表_回购
  **存储过程名称：    ETL_O_IOL_RWAS_RPT_REPO
  **存储过程创建日期：20250114
  **存储过程创建人：  YUJINGYI
  **输入参数：        I_P_DATE
  **输出参数：        O_ERRCODE
...
AS
  ...
  V_PROC_NAME VARCHAR2(100) := 'ETL_O_IOL_RWAS_RPT_REPO'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_IOL_RWAS_RPT_REPO';
  ...
  -- 程序业务逻辑处理主体部分 --
  V_STEP      := V_STEP + 1;
  V_STEP_DESC := '数据落地-内部管理报表_回购';
  V_STARTTIME := SYSDATE;
  INSERT /*+APPEND*/ INTO RRP_MDL.O_IOL_RWAS_RPT_REPO NOLOGGING 
  (    DATA_DATE                    --数据日期
      ,LOAN_REF_NO                  --债项编号
      ,LOAN_REF_DESC                --债项描述
      ,CONTRACT_NO                  --合同编号
    ...
    SELECT
      ...
      ,ETL_DT                        --ETL处理日期
      ,ETL_TIMESTAMP                 --ETL处理时间戳
  FROM IOL.V_RWAS_RPT_REPO  --视图-内部管理报表_回购
 WHERE ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  ...
  ETL_DBMS_STATS(V_P_DATE, 'O_IOL_RWAS_RPT_REPO', '', O_ERRCODE);
