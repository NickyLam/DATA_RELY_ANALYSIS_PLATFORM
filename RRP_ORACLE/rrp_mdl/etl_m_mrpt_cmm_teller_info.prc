CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_MRPT_CMM_TELLER_INFO(I_P_DATE IN INTEGER,
                                                       O_ERRCODE OUT VARCHAR2
                               )
  /**************************************************************************
  *  程序名称：ETL_M_MRPT_CMM_TELLER_INFO
  *  功能描述：柜员信息
  *  创建日期：20221228
  *  开发人员：阳娟
  *  来源表：
  *  目标表： M_MRPT_CMM_TELLER_INFO
  *  配置表：
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_M_MRPT_CMM_TELLER_INFO'; -- 程序名称
...
BEGIN
  ...
  V_TAB_NAME := 'M_MRPT_CMM_TELLER_INFO'; --表名称

  -- 支持重跑 --
  I_STEP := 1;
  I_STEP_DESC := '-- 程序跑批开始 --';
  D_STARTTIME := SYSDATE;
  --DELETE FROM M_MRPT_CMM_TELLER_INFO T WHERE T.DATA_DT = V_P_DATE;--普通表的重跑处理
  /*SELECT COUNT(0)
    INTO V_PART_COUNT
  ...
  END IF ;*/
 V_SQL :='TRUNCATE TABLE M_MRPT_CMM_TELLER_INFO';
  EXECUTE IMMEDIATE V_SQL;
 ...
 ETL_PARTITION_ADD(V_P_DATE,V_TAB_NAME,1,O_ERRCODE);--新增分区
  ...
  I_STEP_DESC := '数据落地-柜员信息';
  D_STARTTIME := SYSDATE;
  INSERT INTO RRP_MDL.M_MRPT_CMM_TELLER_INFO
  (    DATA_DT  --数据日期
      ,LP_ID  --法人编号
      ,TELLER_ID  --柜员编号
      ,TELLER_NAME  --柜员名称
    ...
    SELECT
      ...
      ,TELLER_TYPE_SUBCLASS_CD  --柜员明细分类
      ,JOB_CD --任务代码
    FROM RRP_MDL.O_ICL_CMM_TELLER_INFO  --视图-柜员信息
  ...
  END ETL_M_MRPT_CMM_TELLER_INFO;
/
