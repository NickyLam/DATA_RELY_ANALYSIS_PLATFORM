CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_M_PUM_ORG_INFO_EAST(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_INIT_M_PUM_ORG_INFO_EAST
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(200) := 'ETL_INIT_M_PUM_ORG_INFO_EAST'; -- 程序名称
  ...
  V_TAB_NAME := 'M_PUM_ORG_INFO_EAST'; --表名
  V_PART_NAME := 'PARTITION_'||V_P_DATE;

  ---删除前一天历史数据，每月1号不删除历史数据
  IF V_DEL_FLAG != '01' THEN
    DELETE FROM M_PUM_ORG_INFO_EAST WHERE DATA_DT = V_DEL_DATE;
    COMMIT;
  END IF;
  ...
  EXECUTE IMMEDIATE 'TRUNCATE TABLE TMP_M_PUM_ORG_INFO';

  INSERT INTO TMP_M_PUM_ORG_INFO
    (DATA_DT, --数据日期
     LGL_REP_ID, --法人编号
     ORG_ID, --机构编号
     PBC_NO, --人行支付行号
    ...
    WITH YXJG AS
     ( --筛选有效机构
      SELECT ORG_ID --机构编号
             ,
              CASE
                WHEN TRIM(ACCTI_SUPER_ORG_ID) IS NULL OR
                     ACCTI_SUPER_ORG_ID = '800001' THEN
                 ADMIN_SUPER_ORG_ID
                ELSE
                 ACCTI_SUPER_ORG_ID
              END AS SUPER_ORG_ID --上级机构编号
        FROM O_ICL_CMM_INTNAL_ORG_INFO --内部机构信息
       WHERE ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
         AND (ACCTI_ORG_FLG = '1' OR ADMIN_SUPER_ORG_ID = '000000' OR
             ORG_ID IN ('000000', '811'))
     ...
     ( --有效机构去重
      SELECT DISTINCT ORG_ID
        FROM (SELECT ORG_ID
                 FROM YXJG
               UNION ALL
               SELECT SUPER_ORG_ID AS ORG_ID
                 FROM YXJG
                WHERE SUPER_ORG_ID <> '999999'))
    SELECT TO_CHAR(A.ETL_DT, 'YYYYMMDD') AS DATA_DT --数据日期
          ...
          ,
           CASE
             WHEN NVL(A.PBC_PAY_BANK_NO, ' ') <> ' ' THEN
              A.PBC_PAY_BANK_NO
             WHEN NVL(C.PBC_PAY_BANK_NO, ' ') <> ' ' THEN
              C.PBC_PAY_BANK_NO
             WHEN NVL(D.PBC_PAY_BANK_NO, ' ') <> ' ' THEN
              D.PBC_PAY_BANK_NO
             ELSE
              E.PBC_PAY_BANK_NO
          ...
          ,
           CASE
             WHEN NVL(A.FIN_LICS_NUM, ' ') <> ' ' THEN
              A.FIN_LICS_NUM
             WHEN NVL(C.FIN_LICS_NUM, ' ') <> ' ' THEN
              C.FIN_LICS_NUM
             WHEN NVL(D.FIN_LICS_NUM, ' ') <> ' ' THEN
              D.FIN_LICS_NUM
             ELSE
              E.FIN_LICS_NUM
          ...
          ,
           A.PHYS_ADDR AS ORG_ADDR --机构地址
          ,
           B.CLERK_NAME AS PIC_NM --负责人姓名
          ,
           G.POST_NAME AS PIC_JOB --负责人职务
          ,
           CASE
          ...
          ,
           A.FIN_INST_CODE AS FIN_ORG_ID --金融机构编号
          ,
     ...
     INNER JOIN YXJG_ALL AA --有效机构
      ...
      LEFT JOIN O_IML_REF_POSTN_PARA G --职位参数
       -- ON B.POST_CD = G.POST_ID
        ON B.POSTN_CD = G.POST_ID   -- 20221208  LHQ  根据缺陷BUG_063430数仓反馈修改
  ...
  V_STARTTIME := SYSDATE;
  INSERT INTO M_PUM_ORG_INFO_EAST
  (
     DATA_DT, --数据日期
     LGL_REP_ID, --法人编号
     ORG_ID, --机构编号
    ...
    SELECT V_P_DATE AS DATA_DT, --数据日期
           ...
           NVL(T.IS_ENTITY,'N')
      FROM TMP_M_PUM_ORG_INFO T
      LEFT JOIN ADD_M_PUM_ORG_INFO A
  ...
  INSERT INTO M_PUM_ORG_INFO_EAST
    ...
    SELECT V_P_DATE            AS DATA_DT, --数据日期
      ...
      FROM ADD_M_PUM_ORG_INFO T
      LEFT JOIN TMP_M_PUM_ORG_INFO A
        ON T.ORG_ID = A.ORG_ID
  ...
  INSERT INTO ORG_CONFIG
    ...
    WITH JGXX
    (ORG_ID, SUPER_ORG_ID) AS
     (SELECT TT.ORG_ID,
             CASE
               WHEN TT.ORG_ID = TT.SUPER_ORG_ID1 THEN
                TT.SUPER_ORG_ID2
               ELSE
                TT.SUPER_ORG_ID1
             END AS SUPER_ORG_ID
        FROM (SELECT T.ORG_ID,
                     CASE
                       WHEN (T1.ACCTI_ORG_FLG = '1' OR
                            T1.ADMIN_SUPER_ORG_ID = '000000' OR
                            T1.ORG_ID = '000000') THEN
                        T1.ORG_ID
                       ELSE
                        T1.ADMIN_SUPER_ORG_ID
                     END AS SUPER_ORG_ID1,
                     CASE
                       WHEN TRIM(T.ACCTI_SUPER_ORG_ID) IS NULL OR
                            T.ACCTI_SUPER_ORG_ID = '800001' THEN
                        T.ADMIN_SUPER_ORG_ID
                       ELSE
                        T.ACCTI_SUPER_ORG_ID
                     END AS SUPER_ORG_ID2
                FROM O_ICL_CMM_INTNAL_ORG_INFO T
                LEFT JOIN O_ICL_CMM_INTNAL_ORG_INFO T1
                  ON T1.ORG_ID = SUBSTR(T.ORG_ID, 1, 6)
                 AND T.ETL_DT = T1.ETL_DT
               WHERE T.ETL_DT = TO_DATE(V_P_DATE, 'YYYYMMDD')
                 AND T.ORG_ID <> '999999') TT),
    JG
    (ORG_ID, SUPER_ORG_ID) AS
     ( --递归
      SELECT ORG_ID, SUPER_ORG_ID
        FROM JGXX
      UNION ALL
      SELECT A.ORG_ID, B.SUPER_ORG_ID
        FROM JGXX A, JG B
       WHERE A.SUPER_ORG_ID = B.ORG_ID)
    SELECT T.ORG_ID AS ORG_ID --源机构号
      ...
      FROM (SELECT A.ORG_ID,
                   ...
                   B.ORG_ID AS ORG_ID1,
                   --C.ORG_ID AS ORG_ID2,
                    case WHEN substr(C.ORG_ID,-3) ='902' THEN concat(substr(C.ORG_ID,1,3),'001')  else C.ORG_ID  end AS ORG_ID2,
                   ROW_NUMBER() OVER(PARTITION BY A.ORG_ID ORDER BY B.ORG_ID, NVL(case WHEN substr(C.ORG_ID,-3) ='902' THEN concat(substr(C.ORG_ID,1,3),'001')  else C.ORG_ID  end, '1') DESC) AS NUM
              FROM JG A
              LEFT JOIN M_PUM_ORG_INFO_EAST B
                ON A.ORG_ID = B.ORG_ID
               AND B.DATA_DT = V_P_DATE
              LEFT JOIN M_PUM_ORG_INFO_EAST C
                ON A.SUPER_ORG_ID = C.ORG_ID
               AND C.DATA_DT = V_P_DATE) T
      LEFT JOIN M_PUM_ORG_INFO_EAST T1
        ON NVL(TRIM(T.ORG_ID1), ORG_ID2) = T1.ORG_ID
...
/*   V_STEP_DESC := '-- 表分析 --';
  ...
  END ETL_INIT_M_PUM_ORG_INFO_EAST;
/
