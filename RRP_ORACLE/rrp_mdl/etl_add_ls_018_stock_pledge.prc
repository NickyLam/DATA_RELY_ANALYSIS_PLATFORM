CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_ADD_LS_018_STOCK_PLEDGE(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
/**************************************************************************
  *  程序名称：ETL_ADD_LS_018_STOCK_PLEDGE
  *  功能描述：补录表-零售-股权质押模型（G16）。
  *  创建日期：20221221
  *  开发人员：hulijuan
  *  来源表：  ICL.CMM_RETL_LOAN_ACCT_INFO     --零售贷款账户信息
  *            ICL.CMM_RETL_LOAN_DUBIL_INFO   --零售贷款借据信息
  *            ICL.CMM_COL_INFO              --押品信息表
  *            IML.AST_COL_LIST_STOCK_INPWN_INFO  -- 押品上市公司股权质押信息
  *            ICL.CMM_INTNAL_ORG_INFO  --内部机构信息表
  *  目标表：  ADD_LS_018_STOCK_PLEDGE  --股权质押模型（G16）
  *
  *  配置表：  CODE_MAP  --码值映射表
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME   VARCHAR2(30)   := 'ETL_ADD_LS_018_STOCK_PLEDGE';   -- 程序名称
  V_TABLE_NAME  VARCHAR2(30)   := 'ADD_LS_018_STOCK_PLEDGE';       -- 报表名称
...
BEGIN
  ...
  --DELETE FROM ADD_LS_018_STOCK_PLEDGE T WHERE T.DATA_DATE = V_P_DATE;--普通表的重跑处理
  ...
  -- 程序业务逻辑处理主体部分 --
  V_STEP      := 2;
  V_STEP_DESC := '处理数据-插入目标表-当天补录数据';
  V_STARTTIME := SYSDATE;

  INSERT INTO ADD_LS_018_STOCK_PLEDGE
    (
     DATA_DATE           --01 数据日期
    ...
    FROM ADD_LS_018_STOCK_PLEDGE_ETL A
    WHERE A.DATA_DATE = (SELECT MAX(DATA_DATE) FROM ADD_LS_018_STOCK_PLEDGE_ETL)
       ) T
  WHERE T.RN = 1
    ;
  ...
  V_STEP      := 3;
  V_STEP_DESC := '处理数据-插入目标表-继承上一期数据';
  V_STARTTIME := SYSDATE;
  ...
  INSERT INTO ADD_LS_018_STOCK_PLEDGE
    ...
    SELECT /*+ PARALLEL(A,4) */
          ...
          ,SYS_SOURCE          --43 来源系统
          ,SFSC                --44 是否删除
      FROM RRP_MDL.ADD_LS_018_STOCK_PLEDGE T1
     WHERE T1.DATA_DATE = TO_CHAR(TO_DATE(V_P_DATE,'YYYYMMDD') - 1,'YYYYMMDD') --取前一天数据
       AND NOT EXISTS (SELECT 1
                         FROM RRP_MDL.ADD_LS_018_STOCK_PLEDGE T2
...
-- 数据重复校验 --
  WITH TMP1 AS (
    SELECT DATA_DATE,JYWYM,YPWYM,COUNT(1)
      FROM RRP_MDL.ADD_LS_018_STOCK_PLEDGE T
...
END ETL_ADD_LS_018_STOCK_PLEDGE;
/
