CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_MRPT_EVT_PONL_BK_AUTHEN_FLOW(I_P_DATE IN INTEGER,
                                                               O_ERRCODE OUT VARCHAR2
                               )
  /**************************************************************************
  *  程序名称：ETL_M_MRPT_EVT_PONL_BK_AUTHEN_FLOW
  *  功能描述：个人网银鉴权流水
  *  创建日期：20221228
  *  开发人员：阳娟
  *  来源表： O_IML_EVT_PONL_BK_AUTHEN_FLOW
  *  目标表： M_MRPT_EVT_PONL_BK_AUTHEN_FLOW
  *  配置表：
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_M_MRPT_EVT_PONL_BK_AUTHEN_FLOW'; -- 程序名称
...
BEGIN
  ...
  V_TAB_NAME := 'M_MRPT_EVT_PONL_BK_AUTHEN_FLOW'; --表名称

  -- 支持重跑 --
  I_STEP := 1;
  I_STEP_DESC := '-- 程序跑批开始 --';
  D_STARTTIME := SYSDATE;
  --EXECUTE IMMEDIATE 'TRUNCATE TABLE M_MRPT_EVT_PONL_BK_AUTHEN_FLOW '; -- MODIFY 20230704  增加删数重跑，数仓接口表每天跑全量
 
    DELETE FROM RRP_MDL.M_MRPT_EVT_PONL_BK_AUTHEN_FLOW T WHERE  TO_DATE(T.DATA_DT,'YYYYMMDD') <= TO_DATE(V_P_DATE,'YYYYMMDD') 
       AND TO_DATE(T.DATA_DT,'YYYYMMDD')>= TO_DATE(V_P_DATE,'YYYYMMDD')-14 ;  --MODIFY 2031012 XMY 
  
  --DELETE FROM M_MRPT_EVT_PONL_BK_AUTHEN_FLOW T WHERE T.DATA_DT = V_P_DATE;--普通表的重跑处理
  /*SELECT COUNT(0)
    INTO V_PART_COUNT
  ...
  I_STEP_DESC := '数据落地-个人网银鉴权流水';
  D_STARTTIME := SYSDATE;
  INSERT INTO RRP_MDL.M_MRPT_EVT_PONL_BK_AUTHEN_FLOW
  (          DATA_DT  -- 数据日期
						,EVT_ID  --事件编号
						,LP_ID  --法人编号
						,FLOW_NUM  --流水号
    ...
    SELECT  V_P_DATE
            ...
            ,SRC_TABLE_NAME  --源表名称
            ,JOB_CD  --任务编码
     FROM RRP_MDL.O_IML_EVT_PONL_BK_AUTHEN_FLOW --视图-个人网银鉴权流水 IML.V_EVT_PONL_BK_AUTHEN_FLOW
...
;
  ...
  END ETL_M_MRPT_EVT_PONL_BK_AUTHEN_FLOW;
/
