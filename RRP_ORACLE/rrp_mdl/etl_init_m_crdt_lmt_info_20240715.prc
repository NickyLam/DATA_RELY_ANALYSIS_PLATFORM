CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_M_CRDT_LMT_INFO_20240715(I_P_DATE IN INTEGER,
                                                O_ERRCODE OUT VARCHAR2
                                                )
  /**************************************************************************
  *  程序名称：ETL_INIT_M_CRDT_LMT_INFO
  *  功能描述：授信额度主表&授信额度子表
  *  创建日期：20220524
  *  开发人员：梅炜
  *  来源表：
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(200) := 'ETL_INIT_M_CRDT_LMT_INFO'; -- 程序名称
  ...
  V_TAB_NAME := 'M_CRDT_LMT_INFO'; --表名
  V_TAB_NAME2 := 'M_CRDT_LMT_SUB'; --表名
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM M_CRDT_LMT_INFO T WHERE T.DATA_DT = V_P_DATE;--普通表的重跑处理
  --DELETE FROM M_CRDT_LMT_SUB T WHERE T.DATA_DT = V_P_DATE;
  ...
  /*EXECUTE IMMEDIATE ('ALTER TABLE '||'M_CRDT_LMT_INFO'||' TRUNCATE PARTITION '||'写上分区名');--分区表的重跑处理*/
 ...
 -- 分区表分区处理 --
  ...
  SELECT /*+USE_HASH(A,B,C,D)*/
    ...
    WHERE A.ETL_DT = TO_DATE(I_P_DATE,'YYYYMMDD')
      AND NVL(A.DISTR_DT,DATE'0001-01-01') = TO_DATE(I_P_DATE,'YYYYMMDD'); --发放日=当天
  ...
  SELECT V_P_DATE                        AS DATA_DT,                    --数据日期
    ...
    FROM ( --modify by tangan at 20230214 对公已用额度取借据余额之和
      ...
      WHERE A.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
       AND TRIM(C.LMT_CONT_ID) IS NOT NULL
    ) T
  ...
  SELECT  /*+USE_HASH(A,C,D,E,F,TA,B)*/
   ...
   INNER JOIN RRP_MDL.O_ICL_CMM_CORP_CUST_BASIC_INFO C--对公客户基本信息
    ...
    LEFT JOIN RRP_MDL.O_ICL_CMM_CORP_LOAN_APPL_INFO I --对公贷款申请表
      ON I.LOAN_APPL_FLOW_NUM = A.APV_FLOW_NUM
     AND I.ETL_DT = V_DATE
    LEFT JOIN O_IML_AGT_PASCT_TURN_CRDT_INFO_H K
      ON I.LOAN_APPL_FLOW_NUM=K.OBJ_ID
   ...
   INSERT INTO RRP_MDL.M_CRDT_LMT_INFO_TEMP02
    ...
    ,APPL_WAY_CD                     --37申请方式代码
    ,APPROVE_SERIAL_FLOW_NUM         --38被动转授信集团批复流水号
     )
  SELECT  /*+USE_HASH(A,B,C,D,E,F,TA)*/
         ...
         ,CASE WHEN C.CRDT_CUST_TYPE_CD = '2' THEN '05'                        --同业客户
               ...
               WHEN C.CRDT_CUST_TYPE_CD = '3' OR C.CUST_TYPE_CD='2' THEN '01'  --对公客户
               ELSE '99'                                                       --其他
               END                                          CRDT_SUBJ_TYP                --15授信主体类型 mod by hulj 20230220
         ...
         ,B.STD_PROD_ID                                       STD_PROD_ID                --36标准产品编号
         ,E.APPL_WAY_CD                                       APPL_WAY_CD                --37申请方式代码
         ,K.APPROVE_SERIAL_FLOW_NUM                           APPROVE_SERIAL_FLOW_NUM    --38被动转授信集团批复流水号
    ...
    LEFT JOIN RRP_MDL.O_ICL_CMM_CORP_LOAN_APPL_INFO E --对公贷款申请信息
      ON B.APV_FLOW_NUM = E.LOAN_APPL_FLOW_NUM
     AND E.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
...
/*    LEFT JOIN RRP_MDL.CODE_MAP TA --授信主体类型 CD1471-->C0032
     ...
     SELECT T.CUST_ID                            AS CUST_ID --客户号
           ...
           ,MIN(BEGIN_DT) OVER(PARTITION BY CUST_ID,   --modify by tangan at 20230129
                 CASE WHEN T.BUS_BREED_ID IN ('202020100001','202020200004','02001006135011','02001006160048') THEN '202020100001' -- 网商贷
                      WHEN T.BUS_BREED_ID IN ('02001004165051','02001004120222','202010100001') THEN '202010100001' --借呗
  ...
  END ETL_INIT_M_CRDT_LMT_INFO_20240715;
/
