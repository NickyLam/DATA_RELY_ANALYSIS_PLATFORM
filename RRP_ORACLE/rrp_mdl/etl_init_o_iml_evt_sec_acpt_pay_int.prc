CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_O_IML_EVT_SEC_ACPT_PAY_INT(I_P_DATE IN INTEGER,
                                                O_ERRCODE OUT VARCHAR2
                                                )
  /**************************************************************************
  *  程序名称：ETL_INIT_O_IML_EVT_SEC_ACPT_PAY_INT
  *  功能描述：现券收付息
  *  创建日期：20220611
  *  开发人员：梅炜
  *  来源表：
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(500) := 'ETL_INIT_O_IML_EVT_SEC_ACPT_PAY_INT'; -- 程序名称
...
EXECUTE IMMEDIATE 'TRUNCATE TABLE '|| V_TABLE_NAME;
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
   -- EXECUTE IMMEDIATE ' TRUNCATE TABLE  O_IML_EVT_SEC_ACPT_PAY_INT ';
  /*-- EXECUTE IMMEDIATE ('ALTER TABLE '||'O_IML_EVT_SEC_ACPT_PAY_INT'||' TRUNCATE PARTITION '||'写上分区名');--分区表的重跑处理*/
  ...
  */
  ...
  (
     ...
     SELECT
        ...
        ,ACTL_PAY_DT  --实际支付日期
        ,ACTL_RP_CFM_TM  --实收付确认时间
        ,/*ETL_DT*/TO_DATE(V_P_DATE,'YYYYMMDD')  --ETL处理日期
        ,SRC_TABLE_NAME  --源表名称
        ,JOB_CD  --任务编码
  ...
  END ETL_INIT_O_IML_EVT_SEC_ACPT_PAY_INT;
/
