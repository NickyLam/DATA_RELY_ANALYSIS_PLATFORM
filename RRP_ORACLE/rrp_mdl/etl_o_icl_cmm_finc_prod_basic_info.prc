CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_ICL_CMM_FINC_PROD_BASIC_INFO(I_P_DATE IN INTEGER,
                                                               O_ERRCODE OUT VARCHAR2
                                                               )
  /**************************************************************************
  *  程序名称：ETL_O_ICL_CMM_FINC_PROD_BASIC_INFO
  *  功能描述：理财产品基本信息
  *  创建日期：20221008
  *  开发人员：MW
  *  来源表：
...
AS
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_ICL_CMM_FINC_PROD_BASIC_INFO'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  DELETE FROM RRP_MDL.O_ICL_CMM_FINC_PROD_BASIC_INFO T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');--普通表的重跑处理
  /*EXECUTE IMMEDIATE ('ALTER TABLE '||'O_ICL_CMM_FINC_PROD_BASIC_INFO'||' TRUNCATE PARTITION '||'写上分区名');--分区表的重跑处理*/
  ...
  V_STEP_DESC := '分区处理';
  V_STARTTIME := SYSDATE;
  ETL_PARTITION_ADD(V_P_DATE, 'O_ICL_CMM_FINC_PROD_BASIC_INFO', '1', O_ERRCODE);
  V_SQLCOUNT := SQL%ROWCOUNT;
  ...
  INSERT INTO RRP_MDL.O_ICL_CMM_FINC_PROD_BASIC_INFO
    ...
    ,STD_PROD_ID              --标准产品编号   ADD BY YJY 20241017
    )
  SELECT 
     ETL_DT                   --ETL处理日期
    ,LP_ID                    --法人编号
...
END ETL_O_ICL_CMM_FINC_PROD_BASIC_INFO;
/
