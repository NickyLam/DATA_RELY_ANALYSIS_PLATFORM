CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_ICL_CMM_IBANK_NV_TYPE_PROD_INVEST(I_P_DATE IN INTEGER,
                                                                    O_ERRCODE OUT VARCHAR2
                                                                    )
  /**************************************************************************
  *  程序名称：ETL_O_ICL_CMM_IBANK_NV_TYPE_PROD_INVEST
  *  功能描述：同业净值型产品投资
  *  创建日期：20220525
  *  开发人员：梅炜
  *  来源表：
  *  目标表： O_ICL_CMM_IBANK_NV_TYPE_PROD_INVEST
  *  配置表：
...
AS
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_ICL_CMM_IBANK_NV_TYPE_PROD_INVEST'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  DELETE FROM RRP_MDL.O_ICL_CMM_IBANK_NV_TYPE_PROD_INVEST T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  --EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_ICL_CMM_IBANK_NV_TYPE_PROD_INVEST';
  ...
  -- 程序业务逻辑处理主体部分 --
  V_STEP := V_STEP + 1;
  V_STEP_DESC := '数据落地-同业净值型产品投资';
  V_STARTTIME := SYSDATE;
  INSERT INTO RRP_MDL.O_ICL_CMM_IBANK_NV_TYPE_PROD_INVEST
    (ETL_DT                           --数据日期
    ,LP_ID                            --法人编号
  ...
  SELECT 
    ...
    FROM (SELECT /*+PARALLEL*/ ROW_NUMBER() OVER(PARTITION BY OBJ_ID ORDER BY A.ETL_DT DESC) RN,A.*
            FROM ICL.V_CMM_IBANK_NV_TYPE_PROD_INVEST A --20230112 XUXIAOBIN特殊处理
           WHERE TRUNC(A.ETL_DT,'MM') = TRUNC(TO_DATE(V_P_DATE,'YYYYMMDD'),'MM')
             AND A.ETL_DT <= TO_DATE(V_P_DATE,'YYYYMMDD'))--视图-同业净值型产品投资
...
END ETL_O_ICL_CMM_IBANK_NV_TYPE_PROD_INVEST;
/
