CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_MRPT_CMM_CONL_BK_SIGN_INFO(I_P_DATE IN INTEGER,
                                                             O_ERRCODE OUT VARCHAR2
                                                )
  /**************************************************************************
  *  程序名称：ETL_M_MRPT_CMM_CONL_BK_SIGN_INFO
  *  功能描述：企业网银签约信息
  *  创建日期：20230207
  *  开发人员：阳娟
  *  来源表：
  *  目标表：  M_MRPT_CMM_CONL_BK_SIGN_INFO
  *  配置表：  CODE_MAP
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(100) := 'ETL_M_MRPT_CMM_CONL_BK_SIGN_INFO'; -- 程序名称 --XZY 新建一个的时候，批量替换成新的名字就行
...
BEGIN
 ...
 -- 处理参数及月末等判断逻辑 --
  ...
  V_TAB_NAME := 'M_MRPT_CMM_CONL_BK_SIGN_INFO'; --表名称
  ...
  END IF ;*/
  V_SQL :='TRUNCATE TABLE M_MRPT_CMM_CONL_BK_SIGN_INFO';
  EXECUTE IMMEDIATE V_SQL;
  ...
  I_STEP_DESC := '数据落地-企业网银签约信息';
  D_STARTTIME := SYSDATE;
  INSERT INTO RRP_MDL.M_MRPT_CMM_CONL_BK_SIGN_INFO
  (
            DATA_DT  --数据日期
            ,LP_ID  --法人编号
            ,CUST_ID  --客户编号
    ...
    FROM O_ICL_CMM_CONL_BK_SIGN_INFO
  ...
  END ETL_M_MRPT_CMM_CONL_BK_SIGN_INFO;
/
