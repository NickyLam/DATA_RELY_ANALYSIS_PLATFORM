CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IML_EVT_PPPS_PS_REFUND_FLOW(I_P_DATE IN INTEGER,
                                                              O_ERRCODE OUT VARCHAR2
                                                              )
  /**************************************************************************
  *  程序名称：ETL_O_IML_EVT_PPPS_PS_REFUND_FLOW
  *  功能描述：PPPS退款流水
  *  创建日期：20230302
  *  开发人员：梅炜
...
AS
  ...
  V_TAB_NAME  VARCHAR2(100) := 'O_IML_EVT_PPPS_PS_REFUND_FLOW'; --表名
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IML_EVT_PPPS_PS_REFUND_FLOW'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM RRP_MDL.O_IML_EVT_PPPS_PS_REFUND_FLOW T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  --EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_IML_EVT_PPPS_PS_REFUND_FLOW';
...
END ETL_O_IML_EVT_PPPS_PS_REFUND_FLOW;
/
