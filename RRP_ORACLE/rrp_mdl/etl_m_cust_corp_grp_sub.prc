CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_CUST_CORP_GRP_SUB(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_M_CUST_CORP_GRP_SUB
  *  功能描述：监管集市集团客户补充信息
  *  创建日期：20220609
  *  开发人员：hulijuan
  ...
  *             6    20251129  XZY      日期转换导致报错
  *             7    20260209  YJY      调整报表取数逻辑，按照陈俊文业务口径当期末值为空或0时取期初值
  **************************************************************************/
AS
  -- 定义变量 --
  V_STEP      INTEGER := 0; -- 处理步骤
  V_STEP_DESC VARCHAR2(100);-- 处理步骤描述
  V_PROC_NAME VARCHAR2(100) := 'ETL_M_CUST_CORP_GRP_SUB'; -- 程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM M_CUST_CORP_GRP_SUB T WHERE T.DATA_DT = V_P_DATE;--普通表的重跑处理
  ...
  WITH JT_CWBB AS (
    SELECT  /*+MATERIALIZE*/
           ...
           ,CASE WHEN FR.SUBJECTNO IN ('804','1903') THEN '1903'
                 WHEN FR.SUBJECTNO IN ('807','2906') THEN '2906'
             END                         AS FIN_RPT_ID  --财务报表编号
           --,TO_CHAR(LAST_DAY(TO_DATE(REPLACE(C.ACCOUNTINGMONTH,'/',''),'YYYYMM')),'YYYYMMDD') AS FIN_RPT_DT  --财务报表日期
           --20251129 modify by xzy 日期转换导致报错
           ,CASE WHEN LENGTH(C.ACCOUNTINGMONTH)<=7 THEN TO_CHAR(LAST_DAY(TO_DATE(REPLACE(C.ACCOUNTINGMONTH,'/',''),'YYYYMM')),'YYYYMMDD')
                 ELSE TO_CHAR(LAST_DAY(TO_DATE(REPLACE(C.ACCOUNTINGMONTH,'/',''),'YYYYMMDD')),'YYYYMMDD')
             END AS FIN_RPT_DT  --财务报表日期
           ...
           -- MOD BY YJY 调整报表取数逻辑，按照陈俊文业务口径当期末值为空或0时取期初值
           ,CASE WHEN NVL(TO_NUMBER(TRIM(FR.VALUETWO)),0) = 0 THEN FR.VALUEONE
                 ELSE FR.VALUETWO
                 END                     AS AST_TOT_AMT     --资产总额
           ,CASE WHEN NVL(TO_NUMBER(TRIM(FR.VALUETWO)),0) = 0 THEN FR.VALUEONE
                 ELSE FR.VALUETWO  
                 END                      AS LBY_TOT_AMT     --负债总额
      FROM RRP_MDL.O_IOL_ICMS_FINA_ROW FR
      LEFT JOIN (SELECT REPORTNO,SHEETNO,ROW_NUMBER() OVER (PARTITION BY REPORTNO ORDER BY FS.INPUTDATE DESC) RN
                  FROM RRP_MDL.O_IOL_ICMS_FINA_SHEET FS
                 WHERE FS.SHEETTYPE = 'BS'
                   AND FS.START_DT <= TO_DATE(V_P_DATE,'YYYYMMDD')
                   AND FS.END_DT > TO_DATE(V_P_DATE,'YYYYMMDD')
                   AND FS.ID_MARK <> 'D')FS
        ON FS.SHEETNO = FR.SHEETNO
       AND FS.RN = 1
     INNER JOIN (SELECT MAX(REPORTNO) REPORTNO,MAX(ACCOUNTINGMONTH) ACCOUNTINGMONTH,MAX(REPORTSCOPE) REPORTSCOPE,
                        MAX(CURRENCY) CURRENCY,CUSTOMERID
                   FROM RRP_MDL.O_IOL_ICMS_FINA_REPORT FRN
     ...
     WHERE FR.SUBJECTNO IN ('804','807','1903','2906')--804-旧准则总资产 807-旧准则总负债 1903-新准则总资产 2906-新准则总负债
       AND FR.START_DT <= TO_DATE(V_P_DATE,'YYYYMMDD')
       AND FR.END_DT > TO_DATE(V_P_DATE,'YYYYMMDD')
       AND FR.ID_MARK <> 'D'),
  JT_CWBB2 AS --通过集团成员财报汇总
    (SELECT  /*+MATERIALIZE*/T2.GRP_CUST_ID      AS CUST_ID          --客户编号
      ...
      FROM (SELECT C.CUSTOMERID                  AS CUST_ID --客户编号
          ...
          ,CASE WHEN LENGTH(C.ACCOUNTINGMONTH)<=7 THEN TO_CHAR(LAST_DAY(TO_DATE(REPLACE(C.ACCOUNTINGMONTH,'/',''),'YYYYMM')),'YYYYMMDD')
                  ...
                  ,CASE WHEN C.REPORTSCOPE ='1' THEN '02'  --合并报表
                        WHEN C.REPORTSCOPE ='2' THEN '01'  --本部报表
                        WHEN C.REPORTSCOPE ='01' THEN '02'  --合并报表
                        WHEN C.REPORTSCOPE ='02' THEN '01'  --本部报表
                        ELSE '99'  --其他
                    END                          AS RPT_CLBR        --报表口径
                  ,'CNY'                         AS CUR             --币种
             ...
             LEFT JOIN (SELECT REPORTNO,SHEETNO,REPORTPERIOD,ROW_NUMBER() OVER (PARTITION BY REPORTNO ORDER BY FS.INPUTDATE DESC) RN
               ...
               ON FS.SHEETNO = FR.SHEETNO
              AND FS.RN = 1
            INNER JOIN (SELECT MAX(REPORTPERIOD) REPORTPERIOD ,MAX(REPORTNO) REPORTNO,MAX(ACCOUNTINGMONTH) ACCOUNTINGMONTH,
                         ...
                         GROUP BY CUSTOMERID)  C
               ON C.REPORTNO = FS.REPORTNO
            WHERE FR.SUBJECTNO IN ('804','807','1903','2906')
              AND FR.START_DT <= TO_DATE(V_P_DATE,'YYYYMMDD')
              AND FR.END_DT > TO_DATE(V_P_DATE,'YYYYMMDD')
              AND FR.ID_MARK <> 'D')T1 --804-旧准则总资产 807-旧准则总负这 1903-新准则总资产 2906-新准则总负债
     INNER JOIN --关联集团成员，集团的财报根据集团成员财报汇总 --modify by tangan at 20230114
                (SELECT DISTINCT
                  ...
                  INNER JOIN RRP_MDL.O_ICL_CMM_CORP_CUST_BASIC_INFO B
                     ON B.CUST_ID = A.GROUP_CUST_ID
                    AND B.CRDT_CUST_TYPE_CD='5' --集团客户
       ...
       FROM (SELECT  C.CUSTOMERID                      AS CUST_ID --客户编号
            ...
            ,CASE WHEN LENGTH(C.ACCOUNTINGMONTH)<=7 THEN TO_CHAR(LAST_DAY(TO_DATE(REPLACE(C.ACCOUNTINGMONTH,'/',''),'YYYYMM')),'YYYYMMDD')
              ...
              WHERE FR.SUBJECTNO IN ('804','807','1903','2906')
                AND FR.START_DT <= TO_DATE(V_P_DATE,'YYYYMMDD')
                AND FR.END_DT > TO_DATE(V_P_DATE,'YYYYMMDD')
                AND FR.ID_MARK <> 'D')T --804-旧准则总资产 807-旧准则总负这 1903-新准则总资产 2906-新准则总负债
            )
  SELECT TO_CHAR(A.ETL_DT, 'YYYYMMDD')           AS DATA_DT                   --数据日期
    ...
    LEFT JOIN ( --集团成员数量
              SELECT B1.GROUP_CUST_ID ,COUNT(B1.CUST_ID) AS NUM
  ...
  USING (SELECT FR.SUBJECTNO,FR.VALUETWO,C.CUSTOMERID  FROM O_IOL_ICMS_FINA_ROW FR
           ...
           LEFT JOIN (SELECT MAX(REPORTNO) REPORTNO,CUSTOMERID FROM O_IOL_ICMS_FINA_REPORT FRN
                       ...
                       GROUP BY CUSTOMERID)  C
             ON C.REPORTNO = FS.REPORTNO
          WHERE FR.SUBJECTNO = '804') B
     ON (A.CUST_ID = B.CUSTOMERID
    AND A.DATA_DT = V_P_DATE)
  WHEN MATCHED THEN UPDATE SET A.AST_TOT_AMT = B.VALUETWO;
  COMMIT;
  ...
  MERGE INTO M_CUST_CORP_GRP_SUB A
  USING (SELECT FR.SUBJECTNO,FR.VALUETWO,FRN.CUSTOMERID  FROM O_IOL_ICMS_FINA_ROW FR
           LEFT JOIN O_IOL_ICMS_FINA_SHEET FS
             ON FS.SHEETNO = FR.SHEETNO
            AND FS.SHEETTYPE = 'BS'
          ...
          WHERE FR.SUBJECTNO = '807') B
     ON (A.CUST_ID = B.CUSTOMERID
    AND A.DATA_DT = V_P_DATE)
  WHEN MATCHED THEN UPDATE SET A.LBY_TOT_AMT = B.VALUETWO;
  COMMIT;*/
