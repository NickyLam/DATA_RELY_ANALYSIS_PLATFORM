CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_STATE_INSERT_IOL(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
...
AS
  ...
  V_PROC_NAME  VARCHAR2(50) := 'ETL_STATE_INSERT_IOL'; --程序名称
  CURSOR C_TAB IS SELECT * FROM RRP_MDL.VIEW_LIST_FROM_HDW WHERE SCH_NAME = 'IOL';
BEGIN
  --处理参数及月末等判断逻辑
  ...
  FOR TAB IN C_TAB LOOP
    ...
    FINISH_FLG   := 0;
    M_PROC_NAME  := 'ETL_O_IOL_'||V_TABLE_NAME;
