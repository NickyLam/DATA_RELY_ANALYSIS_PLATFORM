CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IOL_IBMS_TTRD_ACCOUNTING_SECU_CHG(I_P_DATE IN INTEGER,
                              O_ERRCODE OUT VARCHAR2
                               )
  /**************************************************************************
  *  程序名称：ETL_O_IOL_IBMS_TTRD_ACCOUNTING_SECU_CHG
  *  功能描述：券核算变动表
  *  创建日期：20220820
  *  开发人员：梅炜
  *  来源表： IOL.V_WIND_CBONDRATING
  *  目标表： O_IOL_IBMS_TTRD_ACCOUNTING_SECU_CHG
  *  配置表：
  ...
  **************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IOL_IBMS_TTRD_ACCOUNTING_SECU_CHG'; -- 程序名称
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_IOL_IBMS_TTRD_ACCOUNTING_SECU_CHG';
  ...
  -- 程序业务逻辑处理主体部分 --
  V_STEP := V_STEP + 1;
  V_STEP_DESC := '数据落地-券核算变动表';
  V_STARTTIME := SYSDATE;
  INSERT /*+APPEND*/ INTO RRP_MDL.O_IOL_IBMS_TTRD_ACCOUNTING_SECU_CHG NOLOGGING
    (  CHG_ID                        --变动ID
  ...
  SELECT /*+PARALLEL*/
      ...
      ,END_DT                        --结束时间
      ,ID_MARK                        --增删标志
    FROM IOL.V_IBMS_TTRD_ACCOUNTING_SECU_CHG   --券核算变动表_视图
  ...
  END ETL_O_IOL_IBMS_TTRD_ACCOUNTING_SECU_CHG;
/
