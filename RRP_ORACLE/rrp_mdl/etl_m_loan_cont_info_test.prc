CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_LOAN_CONT_INFO_TEST(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  ...
  *             46   20260129  LIP     调整合同表的授信合同取数口径
  **********************************************************/
AS
  --定义变量
...
BEGIN
  ...
  SELECT  V_P_DATE                                    AS DATA_DT                    --数据日期
         ...
         ,NVL(TRIM(A.LMT_CONT_ID),A.CONT_ID)          AS CRDT_LMT_ID                --授信额度编号 --MOD BY LIP 20260129 没有综合授信时，用合同号兜底
 ...
 CONNECT BY LEVEL <= LENGTH(PRODUCTID) - LENGTH(REPLACE(PRODUCTID, ',')) + 1
  ...
  SELECT  V_P_DATE                                    AS DATA_DT                    --数据日期
         ...
         ,NVL(C.LMT_CONT_ID,A.DUBIL_ID)               AS CRDT_LMT_ID                --授信额度编号 --MOD BY LIP 20260129 优先取授信申请表中最新的授信，没有授信时用借据号做授信合同号
         ...
         ,NULL                                        AS DIR_RGN_AREA_CD            --投向地区行政区划代码
         --,A.DIR_INDUS_CD                              AS DIR_IDY                    --投向行业 --MOD BY YJY 20250509
         ,A1.DIR_INDUS_CD                             AS DIR_IDY                    --投向行业 --MOD BY YJY 20250509
    ...
    /*LEFT JOIN RRP_MDL.O_ICL_CMM_UNITE_WL_LMT_INFO C --授信额度信息
     ...
     AND C.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')*/
    LEFT JOIN CMM_UNITE_WL_LMT_INFO_QC C --授信表最新申请记录 --ADD BY LIP 20260129
      ON C.CUST_ID = A.CUST_ID
     AND C.BUS_BREED_ID = A.STD_PROD_ID
     AND C.RN = 1
    ...
    LEFT JOIN (SELECT DISTINCT CODE,NAME,SFDX FROM RRP_MDL.M_DICT_G0107_REMAPPING_BL WHERE TYPE_CODE = 'G010703') T9--知识产权密集型产业
      ON T9.CODE = A1.DIR_INDUS_CD
  ...
  SELECT A.CONT_ID AS CONT_ID --合同号
   ...
   INNER JOIN RRP_MDL.O_IOL_ICMS_TAG_CODE_CONFIG C --标签码值配置表
     ...
     AND C.ITEMNAME = '是'
     AND C.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   WHERE TRIM(B.OBJECTNO) IS NOT NULL
     AND A.STD_PROD_ID = '201020100062' --201020100062 饲料e贷-海大集团 --兴采贷（大参林）没有互联网合作协议，不做数据加工
     AND A.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   GROUP BY A.CONT_ID),
 ...
 CONNECT BY LEVEL <= LENGTH(PRODUCTID) - LENGTH(REPLACE(PRODUCTID, ',')) + 1
  ...
  SELECT  V_P_DATE                                    AS DATA_DT                    --数据日期
         ...
         ,CASE WHEN A.PROD_ID NOT IN ('202020200007','201010300005','202010200012') AND TRIM(A.LMT_CONT_ID) IS NOT NULL --新心金融 新兴贷 360借条 的授信单独处理
           ...
           END                                        AS CRDT_LMT_ID                --授信额度编号 --MOD BY LIP 20260129 与授信表中的各产品的授信保持一致
    ...
    LEFT JOIN (SELECT ROW_NUMBER() OVER(PARTITION BY OBJECTNO ORDER BY SERIALNO DESC) AS RN
                      ...
                      ,ORGID
                      ,OBJECTNO
                 FROM RRP_MDL.O_IOL_ICMS_FLOW_TASK  --流程任务
 ...
 CONNECT BY LEVEL <= LENGTH(PRODUCTID) - LENGTH(REPLACE(PRODUCTID, ',')) + 1
  ...
  SELECT  V_P_DATE                                    AS DATA_DT                    --数据日期
         ...
         ,NVL(T3.LMT_CONT_ID,A.DUBIL_ID)              AS CRDT_LMT_ID                --授信额度编号 --MOD BY LIP 20260129 与授信表中的授信合同取数保持一致
    ...
    LEFT JOIN CMM_UNITE_WL_LMT_INFO_QC T3 --MOD BY LIP 20260129
      ON T3.CUST_ID = A.CUST_ID
     AND T3.BUS_BREED_ID1 = (CASE WHEN A.STD_PROD_ID IN ('202010100004','202010100005') THEN '202010100004' --京东白条
  ...
  SELECT TELLER_ID
   ...
   WHERE TRIM(TELLER_ID) IS NOT NULL
    ...
    CMM_CLERK_INFO AS (
  SELECT TC.TELLER_ID,TC.CLERK_ID,ROW_NUMBER() OVER(PARTITION BY TC.TELLER_ID ORDER BY TC.DIMISSION_DT DESC) RN
    FROM TMP1 TC)
  SELECT DISTINCT
