CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_STATE_INSERT_SDM(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_STATE_INSERT_SDM
  *  功能描述：特殊情况下插入跑批完成记录表——汇总层
  *  创建日期：20230417
  *  开发人员：MW
  *  来源表：
  *  目标表：  ETL_STATE
...
AS
  ...
  V_PROC_NAME  VARCHAR2(50) := 'ETL_STATE_INSERT_SDM'; --程序名称
  CURSOR C_TAB IS SELECT * FROM RRP_MDL.VIEW_LIST_FROM_HDW WHERE SUBSTR(ID,0,3) = 'SDM';
BEGIN
  --处理参数及月末等判断逻辑
