CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_ICL_CMM_IMP_FIN_BUS_INFO(I_P_DATE IN INTEGER,
                                                           O_ERRCODE OUT VARCHAR2
                                                           )
  /**************************************************************************
  *  程序名称：ETL_O_ICL_CMM_IMP_FIN_BUS_INFO
  *  功能描述：进口融资业务信息
  *  创建日期：20230104
  *  开发人员：梅炜
  *  来源表：
  ...
  *             1    20230104  梅炜      首次创建
  *             2    20231127  hulj      新增字段代付行客户编号、代付行客户名称、当期利息发生额
  ***************************************************************************/
AS
  -- 定义变量 --
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_ICL_CMM_IMP_FIN_BUS_INFO'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  DELETE FROM RRP_MDL.O_ICL_CMM_IMP_FIN_BUS_INFO T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');--普通表的重跑处理
...
END ETL_O_ICL_CMM_IMP_FIN_BUS_INFO;
/
