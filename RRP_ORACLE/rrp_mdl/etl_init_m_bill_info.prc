CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_M_BILL_INFO(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_INIT_M_BILL_INFO
...
-- 支持重跑 --
  ...
  (
    ...
    ,ORG_CRDL_NO           --顶级机构社会统一信用证
         ...
         SELECT TRIM(TTA.MEM_ORG_CD) MEM_ORG_CD,--会员机构代码
                ...
                TRIM(ORG_LEV_CD) ORG_LEV_CD,   --机构级别代码
  ...
  SELECT
      ...
      ,CASE WHEN A.BILL_TYPE_CD = 'AC01' AND NVL(TRIM(A.BILL_SUB_INTRV_ID),'-') = '-'
            AND A.ACCPTOR_NAME LIKE '%财务%' THEN '03' --传统票据：承兑人名称中有财务公司
           WHEN A.BILL_TYPE_CD = 'AC01' AND NVL(TRIM(A.BILL_SUB_INTRV_ID),'-') = '-'
                AND A.ACCPTOR_OPEN_BANK_NO LIKE '907%' THEN '03' --传统票据：承兑人行号以“907”开头
           WHEN A.BILL_TYPE_CD = 'AC01' AND NVL(TRIM(A.BILL_SUB_INTRV_ID),'-') <> '-'
                AND TA.ORG_LEV_CD = '301' THEN '03'
           ELSE M.TAR_VALUE_CODE  END  AS BILL_TYP                        --票据类型  --MOD BY LIP 20230207 增加财务公司承兑汇票类型
      ...
      ,CASE WHEN C.BILL_ID IS NOT NULL THEN NVL(C.CUST_NAME,C.DSCNT_PROPS_NAME)
            WHEN D.BILL_ID IS NOT NULL THEN
                   CASE WHEN D.DISCNT_PS_NAME IS NOT NULL THEN D.DISCNT_PS_NAME
                        ELSE D.BF_CNTPTY_NAME
                   END
      ...
      ,/*COALESCE(F.TAR_VALUE_CODE,G.TAR_VALUE_CODE,'99')*/
      ...
      THEN '01'
      WHEN  E.STD_PROD_ID IN ('204010100001','204010100002') THEN '02' END AS DISCNT_WAY                      --贴现方式 20220924 xuxiaobin add
      ,CASE WHEN LENGTH(TRIM(A.DRAWER_ORGNZ_CD))=18 THEN '2313'
                 ...
                 WHEN SUBSTR(TRIM(A.DRAWER_SOCI_CRDT_CD),1,8)<>'00000000' AND LENGTH(A.DRAWER_SOCI_CRDT_CD)=18 THEN '2313'
        END
       AS DRAWER_CRDL_TYP                 -- 出票人证件类型20220930 xuxiaobin add
      ,CASE WHEN TRIM(A.DRAWER_ORGNZ_CD) IS NOT NULL THEN REPLACE(TRIM(A.DRAWER_ORGNZ_CD),'-','')
       WHEN SUBSTR(TRIM(A.DRAWER_SOCI_CRDT_CD),1,8)='00000000'  THEN SUBSTR(TRIM(A.DRAWER_SOCI_CRDT_CD),9,9)
       ELSE TRIM(A.DRAWER_SOCI_CRDT_CD) END AS DRAWER_PRIM_CRDL_NO             --出票人主证件号码20220930 xuxiaobin add
      ,CASE WHEN A.Bill_Type_Cd='AC01' AND SUBSTR(NVL(TRIM(TOP.UNIFY_SOCI_CRDT_CD),TRIM(A.ACCPTOR_SOCI_CRDT_CD)),1,8)='00000000' THEN '2020'
                 WHEN A.Bill_Type_Cd='AC01' AND LENGTH(NVL(TRIM(TOP.UNIFY_SOCI_CRDT_CD),TRIM(A.ACCPTOR_SOCI_CRDT_CD)))=18 THEN '2313'
                 WHEN A.Bill_Type_Cd='AC02' AND SUBSTR(TRIM(A.ACCPTOR_SOCI_CRDT_CD),1,8)='00000000' THEN '2020'
                 WHEN A.Bill_Type_Cd='AC02' AND LENGTH(TRIM(A.ACCPTOR_SOCI_CRDT_CD))=18 THEN '2313'
                 WHEN A.Bill_Type_Cd='AC02' AND TRIM(A.ACCPTOR_SOCI_CRDT_CD) IS NULL AND TRIM(A.DRAWER_SOCI_CRDT_CD) IS NOT NULL THEN '2020'
      ...
      ,CASE WHEN A.Bill_Type_Cd='AC01' AND SUBSTR(NVL(TRIM(TOP.UNIFY_SOCI_CRDT_CD),TRIM(A.ACCPTOR_SOCI_CRDT_CD)),1,8)='00000000'
                 ...
                 WHEN A.Bill_Type_Cd='AC01' THEN NVL(TRIM(TOP.UNIFY_SOCI_CRDT_CD),A.ACCPTOR_SOCI_CRDT_CD)
                 WHEN A.Bill_Type_Cd='AC02' AND SUBSTR(TRIM(A.ACCPTOR_SOCI_CRDT_CD),1,8)='00000000' THEN SUBSTR(TRIM(A.ACCPTOR_SOCI_CRDT_CD),9,9)
                 WHEN A.Bill_Type_Cd='AC02' THEN NVL(TRIM(A.ACCPTOR_SOCI_CRDT_CD),SUBSTR(TRIM(A.DRAWER_SOCI_CRDT_CD),9,9))
            END
       AS ACPTR_PRIM_CRDL_NO              --承兑人主证件号码20220930 xuxiaobin add
  ...
  LEFT JOIN (SELECT AA.* ,ROW_NUMBER() OVER(PARTITION BY BILL_ID ORDER BY DUBIL_ID DESC) AS NUM
               ...
               WHERE AA.ETL_DT = V_DATE
                 AND AA.STD_PROD_ID IN ('203020600001','203020400001','204010200001','204010200002','204010100001','204010100002')
            ) E --对公贷款借据信息
           ON E.BILL_ID = A.BILL_ID --20220930 XUXIAOBIN MODIFY
           --ON E.BILL_NUM = A.BILL_NUM --20220930 XUXIAOBIN MODIFY
           AND E.NUM = 1
  LEFT JOIN
        (SELECT A.MEM_ORG_CD AS ACPT_ORG_ID,B.*,ROW_NUMBER()OVER(PARTITION BY A.MEM_ORG_CD ORDER BY A.UPDATE_DT DESC ) RN
        FROM O_IML_PTY_CPES_MEM A
        INNER JOIN O_IML_PTY_CPES_MEM B ON A.MEM_CD=B.MEM_CD AND B.CREATE_DT <= V_DATE
        WHERE B.LP_LEV_CD='CC00' AND B.ID_MARK<>'D' AND A.ID_MARK<>'D' AND A.CREATE_DT <= V_DATE
    ) TOP ON A.ACPT_ORG_ID=TOP.ACPT_ORG_ID
          AND TOP.RN = 1
  LEFT JOIN (SELECT T2.*,ROW_NUMBER() OVER(PARTITION BY T2.BILL_NUM ORDER BY TO_CHAR(T2.VALUE_DT,'YYYYMMDD')) RN
             FROM RRP_MDL.O_ICL_CMM_BILL_DISCNT_INFO T2
             WHERE T2.ETL_DT = V_DATE
            ) C --票据贴现信息
           ON C.BILL_NUM = A.BILL_NUM
          AND C.RN = 1
  /*LEFT JOIN (SELECT T1.*,ROW_NUMBER() OVER(PARTITION BY T1.BILL_NUM ORDER BY TO_CHAR(T1.DISCNT_DT,'YYYYMMDD')) RN
  ...
  WHERE A.ETL_DT = V_DATE
    --AND A.BILL_STATUS_CD NOT IN ('99', 'S29', '53', '99', 'NN', 'S00', '000002') --过滤无效的票据
  ;
   V_SQLCOUNT := SQL%ROWCOUNT;
   V_SQLMSG := '返回值：['||SQLCODE||'],描述信息：'||SQLERRM;
   O_ERRCODE := '0';
