CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_MRPT_CRDT_FTP_REFLOW(I_P_DATE IN INTEGER,
                                                       O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_M_MRPT_CRDT_FTP_REFLOW
  *  功能描述：类信贷-FTP回流-手工报表专用
  *  创建日期：20221230
  *  开发人员：CYK
  *  来源表：RRP_MDL.O_IOL_FTPS_RPT_RST_FTP261 ftp初级报表账户明细结果集
             RRP_MDL.M_MRPT_INVEST_BIZ  非标投资持仓
  *  目标表：RRP_MDL.M_MRPT_CRDT_FTP_REFLOW 类信贷-FTP回流-手工报表专用
  *  配置表：
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME   VARCHAR2(30) := 'ETL_M_MRPT_CRDT_FTP_REFLOW'; -- 程序名称
...
BEGIN
  ...
  --清空目标表
  V_SQL :='TRUNCATE TABLE RRP_MDL.M_MRPT_CRDT_FTP_REFLOW';
  ...
  V_STEP_DESC := '插入类信贷-FTP回流数据';
  D_STARTTIME := SYSDATE;

  INSERT /*+ APPEND */ INTO RRP_MDL.M_MRPT_CRDT_FTP_REFLOW NOLOGGING
  (
           DATA_DT                        -- 数据日期
     ...
     SELECT /*+ PARALLEL */
           V_P_DATE                          --数据日期
          ,T2.EXT_SECU_ACCT_NO               -- 外部证券账户编号
          ,T2.INTNAL_SECU_ACCT_NO            -- 内部证券账户编号
          ,T2.FIN_INSTM_NO                   -- 金融工具编号
          ,T2.ASSET_TYPE_NO                  -- 资产类型编号
          ,T2.MARKET_TYPE_NO                 -- 市场类型编号
          ,T2.OJB_NO                          -- 业务编号(对象编码)
          ,T2.CURR_CD                        -- 币种代码
    ...
    INNER JOIN RRP_MDL.M_MRPT_INVEST_BIZ T2  --非标投资持仓
       ON T1.CUST_ID = T2.EXT_SECU_ACCT_NO
      AND T1.MGR_ID = T2.INTNAL_SECU_ACCT_NO
      AND T1.CUST_NAME = T2.FIN_INSTM_NO
      AND T1.MGR_NAME = T2.ASSET_TYPE_NO
      AND T1.CUST_SIZE_CD = T2.MARKET_TYPE_NO
      AND T1.S_ACCT_NO = T2.OJB_NO
      AND T2.CLASS_CREDIT_IND = '1'
      AND T2.DATA_DT = V_P_DATE
    WHERE T1.DATA_SRC = 'collbl_coll_invest'
...
END ETL_M_MRPT_CRDT_FTP_REFLOW;
/
