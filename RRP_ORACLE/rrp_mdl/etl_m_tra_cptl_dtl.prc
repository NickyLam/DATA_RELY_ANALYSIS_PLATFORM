CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_TRA_CPTL_DTL(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_M_TRA_CPTL_DTL
  ...
  *             6    20240116  LYH      同业借款借出1段增加 0179-银团借款 类型
  *             8    20250314  LIP      调整资金同业拆借的结清日期取数
  *             9    20250428  LAL      为一表通增加交易对手账号的取值逻辑
  *             10   20250515  XZY      增加一表通字段-行内归属部门
  *             11   20260116  LIP      调整同业借款借出1部分的境内同业借款借入的交易金额取数
  ***************************************************************************/
AS
  ...
  V_TAB_NAME  VARCHAR2(100) := 'M_TRA_CPTL_DTL'; --表名
  V_PROC_NAME VARCHAR2(100) := 'ETL_M_TRA_CPTL_DTL'; --程序名称
...
BEGIN
  ...
  INSERT /*+ APPEND*/INTO RRP_MDL.M_TRA_CPTL_DTL NOLOGGING
    ...
    ,SUB_ACC_ID_NEW                  --新一代子户号
    ,TRAN_KIND_CD                    --交易种类代码
    ,ENTRY_ORG_ID                    --行内归属部门 ADD BY XZY 20250515 一表通字段
    )
  SELECT V_P_DATE                                     AS DATA_DT                  --数据日期
        ,A.LP_ID                                      AS LGL_REP_ID               --法人编号
        ,A.TRAN_FLOW_NUM || A.ACCT_BILL_FLOW_NUM      AS SEQ_NO                   --交易流水号
        ,A.CUST_ACCT_ID                               AS ACC_ID                   --账户编号                                                                       
        ,NULL                                         AS TRA_TYP                  --交易类型
        ,A.ACCT_ORG_ID                                AS OPEN_ACC_ORG_ID          --机构编号 --20220916 XUXIAOBIN MODIFY
        ,A.TRAN_ORG_ID                                AS HDL_ORG_ID               --经办机构编号
        --UPDATE BY LYH 20231017，同业存款跨期不过，需要减掉交易金额小于0的数据
        ,A.TRAN_AMT                                   AS TRA_AMT                  --交易金额
        ,DS.BANK_CAP_ACCT_ID                          AS OPP_ACC                  --对方账号
        ,A.CNTPTY_ACCT_NAME                           AS OPP_ACC_NM               --对方户名
        ,DECODE(A.DEBIT_CRDT_DIR_CD,'D','313581092013',TRIM(DS.BANK_CAP_ACCT_OPEN_BANK_NUM)) 
                                                      AS OPP_PBC_NO --对方行号
        ...
        ,CASE WHEN A.CHN_CD IN ('1001','1002','1013','1019') THEN '01' --01 柜面
              ...
              ELSE '99'
          END                                         AS TRA_CHAN                 --交易渠道 --20220916 XUXIAOBIN MODIFY
        ,NVL(TRIM(A.TRAN_CURR_CD),'CNY')              AS CUR                      --币种
        ,DECODE(A.CASH_TRANS_FLG, '1','1','0','2')    AS CASH_TRF_FLG             --现转标志
        ,NULL                                         AS ADV_DRAW_FLG             --提前支取标志
        ,A.AGENT_NAME                                 AS AGT_NM                   --代办人姓名
        --,F.TAR_VALUE_CODE                             AS AGT_CRDL_TYP             --代办人证件类型
        --MOD BY 20250904 现在模型用的是数仓的标准证件类型
        ,A.AGENT_CERT_TYPE_CD                         AS AGT_CRDL_TYP             --代办人证件类型
        ,A.AGENT_CERT_NO                              AS AGT_CRDL_NO              --代办人证件号码
        ,A.TRAN_TELLER_ID                             AS TRA_TLR_NO               --交易柜员号
        ,A.AUTH_TELLER_ID                             AS GRANT_TLR_NO             --授权柜员号
        ,A.MEMO_CD_DESCB                              AS ABSTR                    --摘要
        ...
        ,C.SUBJ_ID                                    AS SUBJ_ID                  --科目编号
        ,DECODE(A.DEBIT_CRDT_DIR_CD,'D','0','1')      AS OCCUR_SETL_TYP           --发生结清类型--20220928XUXIAOBIN ADD
        ...
        ,'存款账户交易明细'                           AS DATA_SRC                 --数据来源
        ,NVL(C.OLD_CUST_ACCT_SUB_ACCT_NUM,D.OLD_SUB_ACCT_NUM) AS SUB_ACC_ID      --子账户编号
        ,NULL                                         AS FIN_INSTM_ID             --金融工具编号
        ,A.DEP_SUB_ACCT_ID                            AS BUS_ID                   --业务编号
        ,NULL                                         AS ASSET_THD_CLS_CD         --资产三分类代码
        ,A.SUB_ACCT_ID                                AS SUB_ACC_ID_NEW           --新一代子户号
        ,A.TRAN_KIND_CD                               AS TRAN_KIND_CD             --交易种类代码
        ,C.BELONG_ORG_ID                              AS ENTRY_ORG_ID             --行内归属部门 ADD BY XZY 20250515 一表通字段
    ...
    LEFT JOIN (SELECT C.CUST_ACCT_ID,T.BANK_CAP_ACCT_ID,T.BANK_CAP_ACCT_OPEN_BANK_NUM
                     ,ROW_NUMBER()OVER (PARTITION BY  C.CUST_ACCT_ID ORDER BY T.BANK_CAP_ACCT_ID ) RN
                 FROM RRP_MDL.O_IML_EVT_IBANK_TRAN_ACCT_INFO C --同业交易账户信息
                 LEFT JOIN RRP_MDL.O_IML_EVT_IBANK_TRAN T --同业交易表
                   ON T.APV_ODD_NO = C.APV_ODD_NO
                  AND T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
                WHERE TRIM(T.BANK_CAP_ACCT_ID) IS NOT NULL 
                  AND TRIM(T.BANK_CAP_ACCT_OPEN_BANK_NUM) IS NOT NULL 
                  AND C.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')) DS
      ON DS.CUST_ACCT_ID = A.CUST_ACCT_ID
     AND DS.RN = 1
    LEFT JOIN RRP_MDL.O_ICL_CMM_INTNAL_ACCT D --内部账户
      ON D.MAIN_ACCT_ID = A.CUST_ACCT_ID
     AND D.SUB_ACCT_NUM = A.SUB_ACCT_ID
     AND D.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    /*LEFT JOIN RRP_MDL.CODE_MAP F --码值表
      ON F.SRC_VALUE_CODE = A.AGENT_CERT_TYPE_CD
     AND F.SRC_CLASS_CODE = 'CD1014'
     AND F.MOD_FLG = 'MDM'*/
    LEFT JOIN RRP_MDL.CODE_MAP JYLX --交易类型
      ON JYLX.SRC_VALUE_CODE = A.TRAN_KIND_CD
  ...
  SELECT V_P_DATE                                   AS DATA_DT                    --数据日期
        ,A.LP_ID                                    AS LGL_REP_ID                 --法人编号
        ,A.MAIN_INSTR_SEQ_NUM                       AS SEQ_NO                     --交易流水号
        ,D.FIN_INSTM_ID                             AS ACC_ID                     --账户编号
        ,NULL                                       AS TRA_TYP                    --交易类型
        ,G.BELONG_ORG_ID                            AS OPEN_ACC_ORG_ID            --机构编号--20220916 XUXIAOBIN MODIFY
        ,G.BELONG_ORG_ID                            AS HDL_ORG_ID                 --经办机构编号
        ,ABS(F.ACTL_NET_PRICE_AMT)                  AS TRA_AMT                    --交易金额
        ,CASE WHEN A.TRAN_TYPE_CD LIKE D.PROD_TYPE_CD||'30_' OR A.TRAN_TYPE_CD IN ('0100531')
              THEN B.CNTPTY_ACCT_NUM
              ELSE TT2.PAYER_ACCT_NUM
          END                                       AS OPP_ACC                     --对方账号
        ...
        ,TO_TIMESTAMP(A.TRAN_DT,'DD-MON-YYHH:MI:SS.FF AM') AS TRA_TM              --交易时间
        ,NULL                                       AS AST_LBY_SIDE_FLG           --资产负债方标志
        ,G.SUBJ_ID                                  AS SUBJ_ID                    --科目编号
        ...
        ,NULL                                       AS BIZ_AMT                    --业务发生金额
        ,'00001'                                    AS DEPT_LINE                  --部门条线--营运管理部
        ,'存放同业定期'                             AS DATA_SRC                   --数据来源
        ,NULL                                       AS SUB_ACC_ID                 --子账户编号
        ,NULL                                       AS FIN_INSTM_ID               --金融工具编号
        ,D.FIN_INSTM_ID                             AS BUS_ID                     --业务编号
        ,NULL                                       AS ASSET_THD_CLS_CD           --资产三分类代码
        ,G.BELONG_ORG_ID                            AS ENTRY_ORG_ID               --行内归属部门 ADD BY XZY 20250515 一表通字段
    ...
    LEFT JOIN RRP_MDL.O_IML_AGT_IBANK_DEP_RCPT E  --同业存单表
      ON E.DEP_RCPT_CD = B.FIN_INSTM_ID
     AND E.ASSET_TYPE_CD = B.ASSET_TYPE_ID
     AND E.MARKET_TYPE_CD = B.TRAN_MARKET_ID
     AND E.VOUCH_ID = '101007'||C.INTNAL_TRAN_NUM
    ...
    LEFT JOIN RRP_MDL.O_IML_EVT_IBANK_TRAN_VCH_INSTR_DTL F --同业券指令明细
     ...
     AND F.SECU_INSTR_SEQ_NUM = A.VCH_INSTR_ID
    ...
    LEFT JOIN RRP_MDL.O_ICL_CMM_PBC_PASS_TRAN_FLOW TT1 --ADD BY wyp 20220124 回收时付款人
      ON TT1.TRAN_DT = A.ACTL_STL_DT
     AND TT1.TRAN_AMT = ABS(F.ACTL_NET_PRICE_AMT)  --金额不一致
     AND TRIM(TT1.PAYER_NAME) = TRIM(B.CNTPTY_NAME)
     AND TRIM(TT1.PAYER_NAME) IS NOT NULL
    LEFT JOIN RRP_MDL.O_ICL_CMM_PBC_PASS_TRAN_FLOW TT2 --ADD BY wyp 20220124  发放时取收款人
      ON TT2.TRAN_DT = A.ACTL_STL_DT
     AND TT2.TRAN_AMT = ABS(F.ACTL_NET_PRICE_AMT)
     AND TRIM(TT2.RECVER_ACCT_NUM) = TRIM(B.CNTPTY_ACCT_NUM)
     AND TRIM(TT2.RECVER_ACCT_NUM) IS NOT NULL
  ...
  SELECT DISTINCT
        ...
        ,A.ENTRY_ORG_ID         AS ENTRY_ORG_ID                    --行内归属部门 ADD BY XZY 20250515 一表通字段
    FROM RRP_MDL.O_ICL_CMM_CAP_IB_LEND A --资金同业拆借 A
    LEFT JOIN RRP_MDL.O_ICL_CMM_CORP_CUST_BASIC_INFO G --对公客户基本信息表
      ON G.CUST_ID = A.CNTPTY_ID
     AND G.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')/*A.ETL_DT*/
    LEFT JOIN RRP_MDL.O_ICL_CMM_INTNAL_ORG_INFO E  --内部机构信息表
      ON E.ORG_ID = A.ENTRY_ORG_ID
     AND E.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
  ...
  WITH CAP_IB_LEND_CLEAR AS ( --ADD BY LIP 20250314
  SELECT /*+MATERIALIZE*/NB.*,
   ...
   WHERE NB.ASSETTYPE = '拆借'
     AND NB.HOLDPOSITION = 0 --持有仓位为0
     AND NVL(NB.BARETRADENAME,'-') <> 'CARRYOVERDEALS'
     AND NB.SETTLEDATE = V_P_DATE --结算日期为跑批日期
  ...
  SELECT DISTINCT
        ...
        ,CASE WHEN TRIM(A.TRAN_DIR_CD) IN ('1','01') THEN 'C'
              WHEN TRIM(A.TRAN_DIR_CD) IN ('2','02') THEN 'D'
          END                   AS TRA_DR_CR_FLG                   --交易借贷标志
        ,TO_DATE(TA.SETTLEDATE,'YYYYMMDD') AS TRA_TM               --交易时间 --MOD BY LIP 20250314
        ,NULL                   AS AST_LBY_SIDE_FLG                --资产负债方标志
        ,A.SUBJ_ID              AS SUBJ_ID                         --科目编号
   ...
   WHERE A.TRAN_CATE_CD = '0'
     ...
     AND TA.SETTLEDATE = V_P_DATE --MOD BY LIP 20250314
     AND A.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  ...
  SELECT DISTINCT
   ...
   WHERE A.SUBJ_ID IS NOT NULL
     ...
     AND A.VALUE_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
     AND A.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');--还需要看其他模块的是否是每天取数再增加条件限制20221018 XUXIAOBIN
  ...
  INSERT INTO RRP_MDL.M_TRA_CPTL_DTL
    ...
    ,ASSET_THD_CLS_CD                --资产三分类代码
    ,ENTRY_ORG_ID                    --行内归属部门 ADD BY XZY 20250515 一表通字段
    ,CNTPTY_ACCT_NUM                 --交易对手账号  ADD BY LAL 20250428 一表通增加字段
    )
  SELECT DISTINCT
         V_P_DATE                                   AS DATA_DT                    --数据日期
        ,A.LP_ID                                    AS LGL_REP_ID                 --法人编号
        ...
        ,CASE WHEN C.PROD_TYPE_CD = '0220' THEN B.TRAN_AMT - B.INT_RECVBL
              ...
              --MOD BY LIP 20260116 因IMAS日报跑数发现有数据的交易金额取数为0，调整取数，参考借出的取数并减去实付利息的金额
              WHEN C.ASSET_TYPE_NAME = '境内同业借款借入' THEN NVL(B.TRAN_AMT,0) - NVL(B.ACTL_INT,0)
              ELSE B.TRAN_AMT
          END                                       AS TRA_AMT                    --交易金额
        ,NVL(TRIM(T1.CUST_ID),A.CNTPTY_ID)          AS OPP_ACC                    --对方账号
        ...
        ,E.BELONG_ORG_ID                            AS ENTRY_ORG_ID               --行内归属部门 ADD BY XZY 20250515 一表通字段
        ,E.CNTPTY_ACCT_NUM                          AS CNTPTY_ACCT_NUM            --交易对手账号  ADD BY LAL 20250428 一表通增加字段
    FROM RRP_MDL.O_IML_EVT_IBANK_TRAN_MAIN_INSTR_DTL A --同业主指令明细
    LEFT JOIN RRP_MDL.O_IML_EVT_IBANK_TRAN B --同业交易表
      ON B.INTNAL_TRAN_NUM = A.INTNAL_TRAN_FLOW_NUM
     AND B.TRAN_STATUS_CD = '4'
     AND B.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN RRP_MDL.O_ICL_CMM_IBANK_FIN_INSTM C --金融工具表
      ON C.FIN_INSTM_ID = B.FIN_INSTM_ID
     AND C.ASSET_TYPE_ID = B.ASSET_TYPE_ID
     AND C.MARKET_TYPE_ID = B.TRAN_MARKET_ID
     AND C.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN RRP_MDL.O_IML_PTY_IBANK_CNTPTY_INFO T1 --同业交易对手信息
      ON T1.SRC_PARTY_ID = A.CNTPTY_ID
     AND T1.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN RRP_MDL.O_ICL_CMM_IBANK_CASH_DEBIT_CRDT E --同业现金借贷
      ON E.FIN_INSTM_ID = B.FIN_INSTM_ID
     AND E.ASSET_TYPE_ID = B.ASSET_TYPE_ID
     AND E.MARKET_TYPE_ID = B.TRAN_MARKET_ID
     AND E.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   WHERE C.PROD_TYPE_CD IN ('0122','0123','0220','0221','0125','0158','0179') --MOD BY LYH 20240116 增加 0179-银团借款 类型
  ...
  SELECT DISTINCT
        ...
        ,E.ASSET_THD_CLS_CD                         AS ASSET_THD_CLS_CD           --资产三分类代码
        ,E.BELONG_ORG_ID                            AS ENTRY_ORG_ID               --行内归属部门 ADD BY XZY 20250515 一表通字段
        ,E.CNTPTY_ACCT_NUM                          AS CNTPTY_ACCT_NUM            --交易对手账号  ADD BY LAL 20250428
    FROM RRP_MDL.O_IML_EVT_IBANK_TRAN_MAIN_INSTR_DTL A --同业主指令明细
    LEFT JOIN RRP_MDL.O_IML_EVT_IBANK_TRAN B --同业交易表
      ON B.INTNAL_TRAN_NUM = A.INTNAL_TRAN_FLOW_NUM
     AND B.TRAN_STATUS_CD = '4'
    ...
    LEFT JOIN RRP_MDL.O_ICL_CMM_IBANK_FIN_INSTM C --金融工具表
     ...
     AND C.MARKET_TYPE_ID = B.TRAN_MARKET_ID
     AND C.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN RRP_MDL.O_IML_EVT_IBANK_TRAN_VCH_INSTR_DTL D --同业券指令明细
      ON D.MAIN_INSTR_SEQ_NUM = A.MAIN_INSTR_SEQ_NUM
     AND D.SECU_INSTR_SEQ_NUM = A.VCH_INSTR_ID
  ...
  SELECT DISTINCT
        ...
        ,NULL                                       AS AST_LBY_SIDE_FLG           --资产负债方标志
        ,E.SUBJ_ID                                  AS SUBJ_ID                    --科目编号
        ,CASE WHEN C.PROD_TYPE_CD = '0150' AND A.TRAN_TYPE_CD LIKE C.PROD_TYPE_CD||'30_\_L' ESCAPE '\'
                   OR A.TRAN_TYPE_CD LIKE C.PROD_TYPE_CD||'301_L' THEN '0'
              ELSE '1'
          END                                       AS OCCUR_SETL_TYP             --发生结清类型
        ,NULL                                       AS IP                         --IP地址
        ,NULL                                       AS MAC                        --MAC地址
    ...
    LEFT JOIN RRP_MDL.O_ICL_CMM_IBANK_CASH_DEBIT_CRDT E --同业现金借贷
      ON E.FIN_INSTM_ID = B.FIN_INSTM_ID
     AND E.ASSET_TYPE_ID = B.ASSET_TYPE_ID
     AND E.INTNAL_SECU_ACCT_ID = B.INTNAL_SECU_ACCT_ID
     AND E.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN RRP_MDL.O_IML_PTY_IBANK_CNTPTY_INFO T1 --同业交易对手信息
      ON T1.SRC_PARTY_ID = A.CNTPTY_ID
     AND T1.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   WHERE C.PROD_TYPE_CD = '0150'
     AND A.TRAN_TYPE_CD NOT LIKE C.PROD_TYPE_CD || '2%'
     AND A.TRAN_TYPE_CD NOT IN ('0150300','0100531') --ADD BY WYP 20220216 取同业现金借贷质押式回购
     AND A.INSTR_STATUS_CD IN ('02','04') --类型可能还有漏的
     AND (A.PARENT_INSTR_ID IN ('0','-1') OR A.PARENT_INSTR_ID = A.MAIN_INSTR_SEQ_NUM)
     AND A.TRAN_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
     AND A.START_DT <= TO_DATE(V_P_DATE,'YYYYMMDD')
  ...
  SELECT DISTINCT
        ...
        ,'1'                                        AS OCCUR_SETL_TYP             --发生结清类型 --20220928 XUXIAOBIN ADD 
        ,NULL                                       AS IP                         --IP地址
        ,NULL                                       AS MAC                        --MAC地址
        ...
        ,NULL                                       AS BUS_ID                     --业务编号
        ,NULL                                       AS ASSET_THD_CLS_CD           --资产三分类代码
        ,B.ACCT_INSTIT_ID                           AS ENTRY_ORG_ID               --行内归属部门 ADD BY XZY 20250515 一表通字段
  ...
  SELECT DISTINCT
        ...
        ,NULL                                       AS AST_LBY_SIDE_FLG           --资产负债方标志
        ,B.SUBJ_ID                                  AS SUBJ_ID                    --科目编号
        , '0'                                       AS OCCUR_SETL_TYP             --发生结清类型 --20220928 XUXIAOBIN ADD
        ,NULL                                       AS IP                         --IP地址
        ,NULL                                       AS MAC                        --MAC地址
        ,B.EXEC_INT_RAT                             AS BIZ_OCCUR_TMPNT_ACT_RATE   --业务发生时点实际利率
        ,NULL                                       AS BIZ_AMT                    --业务发生金额
    ...
    LEFT JOIN RRP_MDL.O_ICL_CMM_LP_OD_SIGN_INFO D --法透签约信息表
     ...
     AND D.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   WHERE D.LP_OD_TYPE_CD IN ('1','2')  --0 对公法透 1 同业日间法透 2 同业隔夜法透
     AND B.EXP_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
     AND B.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  ...
  SELECT DISTINCT
        ...
        ,NULL                                       AS BUS_ID                     --业务编号
        ,NULL                                       AS ASSET_THD_CLS_CD           --资产三分类代码
        ,OIC.OPER_ORG_ID                            AS ENTRY_ORG_ID               --行内归属部门 ADD BY XZY 20250515 一表通字段
    ...
    LEFT JOIN RRP_MDL.O_ICL_CMM_CORP_LOAN_DUBIL_INFO OIC --ADD BY XZY 20250515 一表通字段，参考表内借据关联取数
      ON OIC.DUBIL_ID = A.DUBIL_ID
     AND OIC.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    ...
    LEFT JOIN RRP_MDL.CODE_MAP TB --定价基准类型
      ON TB.SRC_VALUE_CODE = A.BASE_RAT_TYPE_CD
  ...
  SELECT DISTINCT
        ...
        ,CASE WHEN TRUNC(A.VALUE_DT,'MM') = TRUNC(V_DATE,'MM') THEN '1'
              WHEN TRUNC(A.EXP_DT,'MM') = TRUNC(V_DATE,'MM') THEN '0'
          END                         AS OCCUR_SETL_TYP                  --发生结清类型 --20220928 XUXIAOBIN ADD
        ,NULL                         AS IP                              --IP地址
        ,NULL                         AS MAC                             --MAC地址
        ,NULL                         AS BIZ_OCCUR_TMPNT_ACT_RATE        --业务发生时点实际利率
        ,NULL                         AS BIZ_AMT                         --业务发生金额
    ...
    LEFT JOIN RRP_MDL.O_ICL_CMM_IBANK_FIN_INSTM C1 --同业金融工具表
      ON C1.FIN_INSTM_ID = A.FIN_INSTM_ID
     AND C1.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN RRP_MDL.O_ICL_CMM_CORP_CUST_BASIC_INFO G --对公客户基本信息表
      ON G.CUST_ID = A.ISSUER_ID
     AND G.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN RRP_MDL.O_ICL_CMM_CORP_CUST_RELA_PS_INFO D --对公客户关联人信息表
      ON D.CUST_ID = A.ISSUER_ID
     AND D.RELA_TYPE_CD = '30101'
    LEFT JOIN RRP_MDL.O_ICL_CMM_EXCH_RAT_INFO F --汇率信息表
      ON F.CURR_CD = A.CURR_CD
    LEFT JOIN RRP_MDL.O_IML_PTY_IBANK_CUST_CHAT_INFO TI --同业客户特有信息
      ON TI.PARTY_ID = G.CUST_ID
     AND TI.FIN_INST_CATE_CD != '000000'
     AND TI.START_DT <= TO_DATE(V_P_DATE,'YYYYMMDD')
     AND TI.END_DT > TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN RRP_MDL.O_ICL_CMM_INTNAL_ORG_INFO E  --内部机构信息表
      ON E.ORG_ID = A.BELONG_ORG_ID
     AND E.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
   WHERE --AND A.SUBJ_ID LIKE '1011%'
  ...
  SELECT DISTINCT
        ...
        ,NULL                   AS ABSTR                           --摘要
        ,'1'                    AS FLUSH_PATCH_FLG                 --冲补抹标志
        ,DECODE(C.DEBIT_CRDT_DIR_CD,'D', '1', '0') AS TRADE_DIRECT --交易借贷标志
        ,\*A.TRAN_DT*\A.EXP_DT  AS TRA_TM                          --交易时间
        ,NULL                   AS AST_LBY_SIDE_FLG                --资产负债方标志
        ,B.SUBJ_ID              AS SUBJ_ID                         --科目编号
        ,DECODE(C.DEBIT_CRDT_DIR_CD,'D', '1', '0') AS TRADE_DIRECT --发生结清类型
        ,NULL                   AS IP                              --IP地址
        ,NULL                   AS MAC                             --MAC地址
        ,NULL                   AS BIZ_OCCUR_TMPNT_ACT_RATE        --业务发生时点实际利率
        ,NULL                   AS BIZ_AMT                         --业务发生金额
        ...
        ,NULL                   AS ASSET_THD_CLS_CD                --资产三分类代码
    FROM RRP_MDL.O_ICL_CMM_CORP_LOAN_ACCT_INFO B --对公贷款账户信息
    LEFT JOIN RRP_MDL.O_ICL_CMM_CORP_LOAN_CONT_INFO A --对公贷款合同信息表
      ON A.CONT_ID = B.CONT_ID
     AND A.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    LEFT JOIN RRP_MDL.O_ICL_CMM_LP_OD_SIGN_INFO D --法透签约信息表
      ON D.CONT_ID = NVL(A.LMT_CONT_ID,B.CONT_ID)
     AND D.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')
    \*LEFT JOIN RRP_MDL.O_IML_EVT_CORP_LOAN_TRAN_INSTR C--对公贷款交易指令
