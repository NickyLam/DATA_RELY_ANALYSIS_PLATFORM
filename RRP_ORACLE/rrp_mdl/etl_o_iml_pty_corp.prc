CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_IML_PTY_CORP(I_P_DATE IN INTEGER,
                                               O_ERRCODE OUT VARCHAR2
                                               )
  /**************************************************************************
  *  程序名称：ETL_O_IML_PTY_CORP
  *  功能描述：公司当事人
  *  创建日期：20221222
  *  开发人员：梅炜
  *  来源表： IML.V_PTY_CORP
...
AS
  ...
  V_TAB_NAME  VARCHAR2(200) := 'O_IML_PTY_CORP'; --表名
  V_PROC_NAME VARCHAR2(50) := 'ETL_O_IML_PTY_CORP'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  --DELETE FROM RRP_MDL.O_IML_PTY_CORP T WHERE T.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_IML_PTY_CORP';
