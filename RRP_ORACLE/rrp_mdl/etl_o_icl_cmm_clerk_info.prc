CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_O_ICL_CMM_CLERK_INFO(I_P_DATE IN INTEGER,
                                                     O_ERRCODE OUT VARCHAR2
                                                     )
  /**************************************************************************
  *  程序名称：ETL_O_ICL_CMM_CLERK_INFO
  *  功能描述：行员信息表
  *  创建日期：20220525
  *  开发人员：梅炜
  *  来源表： ICL.V_CMM_CLERK_INFO
  *  目标表： O_ICL_CMM_CLERK_INFO
  *  配置表：
  *  修改情况：序号  修改日期  修改人   修改原因
  *             1    20220525  梅炜     首次创建
                2    20220615           修改参数
                3    20240603  YJY      新增离职状态代码
                4    20251125  YJY      新增岗位编号、岗位描述
  ***************************************************************************/
AS
  -- 定义变量 --
  ...
  V_PROC_NAME VARCHAR2(100) := 'ETL_O_ICL_CMM_CLERK_INFO'; --程序名称
...
BEGIN
  ...
  V_STEP_DESC := '-- 程序跑批开始 --';
  V_STARTTIME := SYSDATE;
  DELETE FROM RRP_MDL.O_ICL_CMM_CLERK_INFO T WHERE ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD');
  --EXECUTE IMMEDIATE 'TRUNCATE TABLE RRP_MDL.O_ICL_CMM_CLERK_INFO';
  ...
  V_STARTTIME := SYSDATE;
  INSERT INTO RRP_MDL.O_ICL_CMM_CLERK_INFO
    (ETL_DT                     --数据日期
    ,LP_ID                      --法人编号
    ,CLERK_ID                   --行员编号
    ,CLERK_NAME                 --行员名称
    ...
    ,POST_NAME                  --职务名称
    ,JOB_CD                     --任务代码
    ,DIMISSION_STATUS_CD        --离职状态代码   ADD IN 20240603
    ,JOBS_ID                    --岗位编号  ADD BY YJY 20251125
    ,JOBS_DESCB                 --岗位描述  ADD BY YJY 20251125
    ,JOBS_DESCB2                --岗位描述2  ADD BY YJY 20251125
    )
  SELECT 
     ETL_DT                     --数据日期
    ,LP_ID                      --法人编号
