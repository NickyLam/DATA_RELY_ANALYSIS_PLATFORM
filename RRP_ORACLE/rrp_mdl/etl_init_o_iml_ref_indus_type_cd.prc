CREATE OR REPLACE PROCEDURE RRP_MDL."ETL_INIT_O_IML_REF_INDUS_TYPE_CD" (I_P_DATE IN INTEGER, --跑批日期
                                                 O_ERRCODE  OUT VARCHAR2 --错误代码
                                                 )
 /*******************************************************************
  **存储过程详细说明： 行业类型代码表
  **存储过程名称：    ETL_INIT_O_IML_REF_INDUS_TYPE_CD
  **存储过程创建日期：20221130
  **存储过程创建人：  HULIJUAN
  **输入参数：        I_P_DATE
  **输出参数：        O_ERRCODE
 ...
 AS
  ...
  V_PROC_NAME VARCHAR2(50) := 'ETL_INIT_O_IML_REF_INDUS_TYPE_CD'; -- 程序名称
 ...
 INSERT /*+APPEND*/ INTO RRP_MDL.O_IML_REF_INDUS_TYPE_CD NOLOGGING
    (
     ...
     SELECT /*+PARALLEL*/
      ...
      ,JOB_CD            --任务代码
      ,ETL_TIMESTAMP     --数据处理时间
    FROM IML.V_REF_INDUS_TYPE_CD   行业类型代码表--视图
...
END ETL_INIT_O_IML_REF_INDUS_TYPE_CD;
/
