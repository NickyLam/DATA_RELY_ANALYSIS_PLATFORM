CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_INIT_M_TRA_LOAN_DTL_BFD(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  *  程序名称：ETL_INIT_M_TRA_LOAN_DTL_BFD
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME VARCHAR2(200) := 'ETL_INIT_M_TRA_LOAN_DTL_BFD'; -- 程序名称
...
BEGIN
  ...
  V_P_DATE := SUBSTR(TO_CHAR(I_P_DATE),0,6)||'01'; -- 获取跑批日期
  V_SYSTEM := '监管报送'; -- 默认写监管报送系统，有真实来源的按实际写
  V_TAB_NAME := 'M_TRA_LOAN_DTL_BFD'; --表名
  WHILE TO_DATE(V_P_DATE,'YYYYMMDD') <= TO_DATE('20230530','YYYYMMDD')
    LOOP
 ...
 V_STEP := V_STEP + 1; -- 小于10步骤直接写数字，大于10步用V_STEP := V_STEP + 1;描述每一步骤的作用，如加工处理核心系统个人客户信息。
  ...
  INSERT /*+ append  parallel */ INTO RRP_MDL.M_TRA_LOAN_DTL_BFD
    (DATA_DT,        --数据日期
     LGL_REP_ID,     --法人编号
     TRA_ORG_ID,     --交易机构编号
     TRA_SEQ_NO,     --交易流水号
   ...
   SELECT
     ...
     TO_CHAR(T1.LOAN_REPAY_DT,'YYYYMMDD')||T2.ADVISE_ODD_NO                         AS TRA_SEQ_NO,     --交易流水号
     ...
     /*NVL(T4.ENTER_ACCT_ID,T5.LOAN_DISTR_ACCT_NUM)
                                              AS OPP_ACC,        --对方账号
     NVL(T7.CUST_ACCT_NAME,T8.CUST_ACCT_NAME)
                                               AS OPP_ACC_NM,     --对方户名
     NVL(T7.PBC_PAY_BANK_NO,T8.PBC_PAY_BANK_NO)
                                              AS OPP_PBC_NO,     --对方行号
     ...
     CASE WHEN D.DUBIL_ID IS NOT NULL THEN 'B10'--核销后收回ADD20230203XUXIAOBIN
          WHEN T1.LOAN_REPAY_TYPE_CD = 'NS' OR T1.LOAN_REPAY_TYPE_CD = 'PO'
          THEN 'B01'
 ...
 INNER JOIN  IML.V_EVT_REPAY_DTL           T2                       --还款明细
       ON    T1.EVT_ID = T2.EVT_ID
       --AND   T2.ETL_DT = TO_DATE(V_YESTADAY,'YYYYMMDD')
 INNER JOIN  ICL.V_CMM_RETL_LOAN_ACCT_INFO T3                       --零售贷款账户信息
       ON    T2.ACCT_ID = T3.ACCT_ID
       AND   T3.ETL_DT = TO_DATE(I_P_DATE,'YYYYMMDD') - 1
      -- AND   T3.ETL_DT = TO_DATE('20230502','YYYYMMDD')
  LEFT JOIN  ICL.V_CMM_RETL_LOAN_CONT_INFO T4                       --零售贷款合同信息
       ON    T3.CONT_ID = T4.CONT_ID
       AND   T4.ETL_DT = TO_DATE(I_P_DATE,'YYYYMMDD') - 1
       --AND   T4.ETL_DT = TO_DATE('20230502','YYYYMMDD')
  ...
  V_STARTTIME := SYSDATE;
   ...
   INSERT /*+ append  parallel */INTO RRP_MDL.M_TRA_LOAN_DTL_BFD
    ...
    SELECT
     ...
     V_YESTADAY                                       DATA_DT,        --数据日期
     T1.LP_ID                                       LGL_REP_ID,     --法人编号
     T1.TRAN_ORG_ID                                 TRA_ORG_ID,     --交易机构编号
     TO_CHAR(T1.TRAN_DT,'YYYYMMDD')||T1.TRAN_FLOW_NUM                               TRA_SEQ_NO,     --交易流水号
     T1.ACCT_ID                                     ACC_ID,         --账户编号
     T1.CUST_ID                                     CUST_ID,        --客户编号
     T2.CONT_ID                                     CONT_ID,        --合同编号
     '1'                                            CORP_IND_FLG,   --对公对私标志
     T2.ACCT_INSTIT_ID                               ORG_ID,         --机构编号
     T2.SUBJ_ID                                      SUBJ_ID,        --科目编号
     T2.DUBIL_NUM                                   RCPT_ID,        --借据编号
     T1.CUST_NAME                                   CUST_NM,        --客户名称
     '11'                                           TRA_TYP,        --交易类型 D0121 --11-贷款放款
     T1.DEBIT_CRDT_FLG                              TRA_DR_CR_FLG,  --交易借贷标志 Z0017 D-借，C-贷
     T1.TRAN_AMT                                    TRA_AMT,        --交易金额
     /*T1.BEF_TRAN_BAL-T1.TRAN_AMT*/
     T1.ACTL_BAL                                    ACC_BAL,        --账户余额
     /*modify by tangan at 20230111 调整助贷产品对手方信息逻辑 BEGIN*/
     /*T1.CNTPTY_ACCT_ID                              OPP_ACC,        --对方账号
     T1.CNTPTY_ACCT_NAME                            OPP_ACC_NM,     --对方户名
     T1.CNTPTY_BANK_NO                              OPP_PBC_NO,     --对方行号
     T1.CNTPTY_BANK_NAME                            OPP_BANK_NM,    --对方行名*/
     ...
     CASE WHEN T2.STD_PROD_ID IN  ('203010500001')  --法人透支
          THEN 'A03' --透支
          ELSE 'A01' --贷款发放
 ...
 V_STEP := V_STEP + 1; -- 小于10步骤直接写数字，大于10步用V_STEP := V_STEP + 1;描述每一步骤的作用，如加工处理核心系统个人客户信息。
  ...
  V_STARTTIME := SYSDATE;
   ...
   WHERE A.FIR_WRT_OFF_DT = TO_DATE(V_YESTADAY,'YYYYMMDD')--mod by hulj 20230109
     AND A.ETL_DT = TO_DATE(I_P_DATE,'YYYYMMDD') - 1
   )
    SELECT
     V_YESTADAY                     DATA_DT,        --数据日期
     T1.LP_ID                     LGL_REP_ID,     --法人编号
     T2.ACCT_INSTIT_ID
                                  TRA_ORG_ID,     --交易机构编号
     TO_CHAR(T1.FIR_WRT_OFF_DT,'YYYYMMDD')||T1.DUBIL_ID        TRA_SEQ_NO,     --交易流水号
     T2.ACCT_ID                   ACC_ID,         --账户编号
     T1.CUST_ID                   CUST_ID,        --客户编号
     ...
     T2.CUST_ID                   CUST_NM,        --客户名称
     CASE WHEN T1.AMT_TYPE IN ('PRI')
          THEN '12'   --贷款还本
          WHEN T1.AMT_TYPE IN ('INT')
          THEN '13'   --贷款还息
          ELSE '99'
          END
                                  TRA_TYP,        --交易类型 D0121
     ...
     TO_CHAR(T1.FIR_WRT_OFF_DT,'YYYYMMDD')
                                  TRA_DT,         --交易日期
     NULL                         LOAN_CHG_TYP,   --贷款变动类型
     NULL                         DEPT_LINE,      --部门条线
     '零售贷款核销'                DATA_SRC,       --数据来源
     '1'                          REPAY_PERDS,    --还款期数
...
V_STEP := V_STEP + 1; -- 小于10步骤直接写数字，大于10步用V_STEP := V_STEP + 1;描述每一步骤的作用，如加工处理核心系统个人客户信息。
  ...
  V_STARTTIME := SYSDATE;
   ...
   (SELECT A.DUBIL_NUM,A.CURR_CD,A.LP_ID,A.ACCT_INSTIT_ID,A.ACCT_ID,A.CUST_ID,A.CONT_ID,A.SUBJ_ID,A.CURRT_BAL,
    ...
    WHERE A.ETL_DT = TO_DATE(I_P_DATE,'YYYYMMDD') - 1
    AND A.ASSET_TRAN_DT  = TO_DATE(V_YESTADAY,'YYYYMMDD')
   )
   SELECT
     V_YESTADAY              DATA_DT,        --数据日期
     T1.LP_ID              LGL_REP_ID,     --法人编号
     T1.ACCT_INSTIT_ID     TRA_ORG_ID,     --交易机构编号
     TO_CHAR(T1.ASSET_TRAN_DT,'YYYYMMDD')||T1.DUBIL_NUM          TRA_SEQ_NO,     --交易流水号
     T1.ACCT_ID            ACC_ID,         --账户编号
     T1.CUST_ID            CUST_ID,        --客户编号
     T1.CONT_ID            CONT_ID,        --合同编号
     ...
     TO_CHAR(T1.ASSET_TRAN_DT,'YYYYMMDD')
                            TRA_DT,         --交易日期
     NULL                   LOAN_CHG_TYP,   --贷款变动类型
     NULL                   DEPT_LINE,      --部门条线
...
INNER JOIN ICL.V_CMM_RETL_LOAN_DUBIL_INFO         T4 --零售贷款借据信息
 ...
 COMMIT;
  ...
  V_STARTTIME := SYSDATE;
   ...
   INSERT /*+ append  parallel */ INTO RRP_MDL.M_TRA_LOAN_DTL_BFD NOLOGGING
    (DATA_DT,        --数据日期
     LGL_REP_ID,     --法人编号
     TRA_ORG_ID,     --交易机构编号
     TRA_SEQ_NO,     --交易流水号
  ...
  SELECT DISTINCT
         ...
         TO_CHAR(A.DISTR_DT,'YYYYMMDD')||A.DUBIL_ID               TRA_SEQ_NO,     --交易流水号
         ...
         NVL(B.DISTR_AMT,A.DISTR_AMT)                                              TRA_AMT,        --交易金额
   ...
   LEFT JOIN RRP_MDL.O_ICL_CMM_UNITE_WL_DUBIL_INFO B --联合网贷借据信息
    ...
    LEFT JOIN ICL.V_CMM_INDV_CUST_BASIC_INFO C --个人客户基本信息
     ...
     AND C.ETL_DT = TO_DATE(I_P_DATE,'YYYYMMDD') - 1
   ...
   WHERE  A.ETL_DT = TO_DATE(I_P_DATE,'YYYYMMDD') - 1
      AND NVL(B.DISTR_AMT,A.DISTR_AMT)>0
      --AND A.DISTR_AMT >0
  ...
  INSERT/*+ append  parallel */ INTO RRP_MDL.M_TRA_LOAN_DTL_BFD
   ...
   CASE LVL WHEN 1  THEN 'PRI' --本金
         ...
         END AS AMT_TYPE
         ,
   CASE LVL WHEN 1 THEN (CASE WHEN A.CURRT_REPAY_PRIC = 0 AND T2.ENTER_ACCT_AMT <> 0 THEN
                        T2.ENTER_ACCT_AMT ELSE A.CURRT_REPAY_PRIC
                     END )
 ...
 FROM ICL.V_CMM_UNITE_WL_REPAY_DTL A --联合网贷还款明细
  LEFT JOIN IML.V_EVT_WLD_ACCT_ETY_TRAN T2 --微粒贷会计分录交易事件
                  ON  T2.ETL_DT = TO_DATE(V_YESTADAY,'YYYYMMDD')
                 AND A.REPAY_FLOW_ID = T2.SER_NUM
 ...
 WHERE A.ETL_DT = TO_DATE(V_YESTADAY,'YYYYMMDD')
  -- AND A.REPAY_DT  = TO_DATE(V_YESTADAY,'YYYYMMDD')
   AND A.REPAY_DT <= TO_DATE(V_P_DATE||'235959','YYYYMMDD HH24:MI:SS') - 1
     AND A.REPAY_DT >= TO_DATE(V_P_DATE||'000000','YYYYMMDD HH24:MI:SS') - 1
 ORDER BY A.DUBIL_ID,A.REPAY_DT,A.REPAY_FLOW_ID )
   SELECT
     V_YESTADAY                       DATA_DT,        --数据日期
     T2.LP_ID                       LGL_REP_ID,     --法人编号
     T2.OPEN_ACCT_ORG_ID            TRA_ORG_ID,     --交易机构编号
     TO_CHAR(T1.REPAY_DT,'YYYYMMDD')||T1.REPAY_FLOW_ID||T1.RN               TRA_SEQ_NO,     --交易流水号
     ...
     CASE WHEN T1.AMT_TYPE = 'PRI'
          THEN '12' --贷款还本
          WHEN T1.AMT_TYPE = 'INT'
          THEN '13' --贷款还息
          WHEN T1.AMT_TYPE = 'ODI'
          THEN '18' --贷款还罚息
          WHEN T1.AMT_TYPE = 'FEE'
          THEN '19' --费用
          END                       TRA_TYP,        --交易类型 D0121
     'C'                            TRA_DR_CR_FLG,  --交易借贷标志 Z0017 D-借，C-贷
     T1.TRAN_AMT                    TRA_AMT,        --交易金额
    -- T1.CURR_NOMAL_PRIC_BAL         ACC_BAL,        --账户余额
     ...
     NVL(TRIM(T2.REPAY_NUM),T2.ENTER_ACCT_ACCT_NUM)
                                     OPP_ACC,        --对方账号
 ...
 LEFT JOIN ICL.V_CMM_INDV_CUST_BASIC_INFO            T3    --个人客户基础信息
      ...
      AND T3.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD') - 1
 LEFT JOIN O_ICL_CMM_STD_PROD_INFO                   T4    --标准产品信息表
      ON T2.STD_PROD_ID = T4.PROD_ID
      AND T4.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD') - 1
       ;
  COMMIT;
 ...
 V_STEP := V_STEP + 1; -- 小于10步骤直接写数字，大于10步用V_STEP := V_STEP + 1;描述每一步骤的作用，如加工处理核心系统个人客户信息。
  ...
  V_STARTTIME := SYSDATE;
   ...
   WHERE A.FIR_WRT_OFF_DT = TO_DATE(V_YESTADAY,'YYYYMMDD') - 1  --mod by hulj 20230109
     AND A.ETL_DT = TO_DATE(I_P_DATE,'YYYYMMDD') - 1--联合网贷均为T-2
   )
    SELECT
     V_YESTADAY                     DATA_DT,        --数据日期
     T1.LP_ID                     LGL_REP_ID,     --法人编号
...
LEFT JOIN ICL.V_CMM_UNITE_WL_DUBIL_INFO           T2 --联合网贷账户信息
  ...
  V_P_DATE := TO_CHAR( TO_DATE(V_P_DATE,'YYYYMMDD')+1,'YYYYMMDD');
  END LOOP;
  ...
  END ETL_INIT_M_TRA_LOAN_DTL_BFD;
/
