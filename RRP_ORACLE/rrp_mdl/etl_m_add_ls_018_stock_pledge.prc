CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_ADD_LS_018_STOCK_PLEDGE(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
/**************************************************************************
  *  程序名称：ETL_M_ADD_LS_018_STOCK_PLEDGE
  ...
  *  目标表：  M_ADD_LS_018_STOCK_PLEDGE  --股权质押模型（G16）
  *
  *  配置表：  CODE_MAP  --码值映射表
  ...
  ***************************************************************************/
  ...
  V_PROC_NAME   VARCHAR2(100)  := 'ETL_M_ADD_LS_018_STOCK_PLEDGE'; -- 程序名称
  V_TABLE_NAME  VARCHAR2(30)   := 'M_ADD_LS_018_STOCK_PLEDGE';     -- 报表名称
...
BEGIN
   ...
   EXECUTE IMMEDIATE 'TRUNCATE TABLE TMP_M_ADD_LS_018_STOCK_PLEDGE';

   INSERT INTO TMP_M_ADD_LS_018_STOCK_PLEDGE
    (
     DATA_DATE           --01 数据日期
   ...
   FROM (
      ...
      FROM ADD_LS_018_STOCK_PLEDGE_ETL B
   ...
   INSERT INTO M_ADD_LS_018_STOCK_PLEDGE
    (
     DATA_DATE,           --01 数据日期,
     ACCT_ORG_NUM,        --02 账务机构编号,
     YPWYM,               --03 押品唯一码,
   ...
   SELECT /*+ PARALLEL(A,4) */
     ...
     A.SYS_SOURCE,          --43 来源系统,
     A.SFSC                 --44 是否删除
   FROM TMP_M_ADD_LS_018_STOCK_PLEDGE A
   ;
   ...
   SELECT /*+ PARALLEL(A,4) */
     ...
     A.SYS_SOURCE,          --43 来源系统,
     A.SFSC                 --44 是否删除
   FROM RRP_MDL.ADD_LS_018_STOCK_PLEDGE A
   WHERE A.DATA_DATE = V_P_DATE
   AND NOT EXISTS (SELECT 1 FROM RRP_MDL.M_ADD_LS_018_STOCK_PLEDGE t
...
-- 数据重复校验 --
  WITH TMP1 AS (
    SELECT DATA_DATE,JYWYM,YPWYM,COUNT(1)
      FROM RRP_MDL.M_ADD_LS_018_STOCK_PLEDGE T
...
END ETL_M_ADD_LS_018_STOCK_PLEDGE;
/
