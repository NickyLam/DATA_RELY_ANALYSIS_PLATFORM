CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_PUM_ORG_INFO_EAST(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /**************************************************************************
  ...
  *             4    20250916  LIP      将每天清理前一天数据的逻辑去掉，方便一表通查数
  *             5    20260106  LIP      因一表通不用该模型，增加每天清理前一天数据的逻辑去掉；调整机构类别的取数口径
  ***************************************************************************/
AS
  --定义变量
...
BEGIN
  ...
  SELECT TO_CHAR(A.ETL_DT, 'YYYYMMDD')                    AS DATA_DT,              --数据日期
         ...
         /*CASE WHEN A.ACCTI_ORG_FLG = '1' THEN '01'
              ...
              ELSE '04'
          END                                             AS ORG_TYP,              --机构类型*/
         CASE WHEN A.ORG_HIBCHY_CD IN ('1','2') AND A.ORG_ID NOT LIKE '89%' THEN '01' --01 管理机构 --0101 管理机构
              WHEN (A.ORG_ID = '800001' OR A.ORG_ID LIKE '800%' OR A.ORG_ID LIKE '89%') AND A.ORG_ID <> '800' THEN '04' --04 内设机构 --0401 总行内设机构
              WHEN (A.ORG_ID LIKE '001%' AND A.ORG_ID NOT LIKE '800%') OR A.ORG_NAME LIKE '%分行清算中心' THEN '04' --04 内设机构 --0501 其他内设机构
              ELSE '02' --02 营业机构 --0201 营业机构
          END                                             AS ORG_TYP,              --机构类型 --MOD BY LIP 20260106 调整与一表通口径一致
         ...
         CASE WHEN TRIM(A.PHONE) IS NOT NULL AND TRIM(A.DDD_AREA_CD) IS NOT NULL THEN TRIM(A.DDD_AREA_CD)||'-'||TRIM(A.PHONE)
              ...
              WHEN TRIM(A.SERV_TEL) IS NOT NULL THEN TRIM(A.SERV_TEL) --ADD BY LIP 20260409 联系电话为空时取服务电话
  ...
  SELECT TT.ORG_ID,
    ...
    FROM (SELECT T.ORG_ID,
            ...
            LEFT JOIN RRP_MDL.O_ICL_CMM_INTNAL_ORG_INFO T1
             ...
             AND T1.ETL_DT = TO_DATE(V_P_DATE,'YYYYMMDD')/*T.ETL_DT*/
  ...
  SELECT A.ORG_ID       AS ORG_ID,
         A.SUPER_ORG_ID AS SUPER_ORG_ID,
         B.ORG_ID       AS ORG_ID1,
         C.ORG_ID       AS ORG_ID2,
  ...
  SELECT T.ORG_ID                           AS ORG_ID,        --源机构号
    ...
    LEFT JOIN RRP_MDL.M_PUM_ORG_INFO_EAST T1
      ON T1.ORG_ID = NVL(TRIM(T.ORG_ID1),T.ORG_ID2)
  ...
  V_STEP_DESC := '机构关系配置';
  V_STARTTIME := SYSDATE;
  EXECUTE IMMEDIATE 'TRUNCATE TABLE CONFIG_ORG_REL'; --清数，支持重跑
  INSERT INTO RRP_MDL.CONFIG_ORG_REL(
    ORG_ID,       --内部机构号
    ORG_ID_LEL_0, --总行
    ORG_ID_LEL_1  --分行
    )
  SELECT DISTINCT T.ORG_ID1      AS ORG_ID,       --内部机构号
         '000000'                AS ORG_ID_LEL_0, --总行
         CASE WHEN T.ORG_ID LIKE '805%' THEN 'B1194B244030001'
              ELSE ''
          END                    AS ORG_ID_LEL_1  --分行
  ...
  IF V_SQLCOUNT > 0 THEN
     O_ERRCODE := '1';
     V_SQLMSG  := 'M_PUM_ORG_INFO_EAST表DATA_DT,ORG_ID数据重复';
  ...
  V_STEP_DESC := '程序跑批结束';
  V_STARTTIME := SYSDATE;
  ETL_DBMS_STATS(V_P_DATE,'M_PUM_ORG_INFO_EAST',V_PART_NAME,O_ERRCODE); --表分析
