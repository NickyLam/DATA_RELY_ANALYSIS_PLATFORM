CREATE OR REPLACE PROCEDURE RRP_MDL.ETL_M_GL_BAL_JZ(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
/**************************************************************************
  *  程序名称：ETL_M_GL_BAL
  ...
  *             4    20231110  HYF      取结转后数据，套账期次为：13
  ***************************************************************************/
AS
  -- 定义变量 --
...
BEGIN
  ...
  --EXECUTE IMMEDIATE ('ALTER TABLE '||'M_GL_BAL'||' TRUNCATE PARTITION '||'写上分区名');--分区表的重跑处理
  ...
  SELECT TO_CHAR(T1.ETL_DT,'YYYYMMDD')   AS DATA_DT           --数据日期
        ...
        ,T1.BNDFFSE                      AS YEAR_CR_AMT       --年贷方发生额
         --,T1.YEAR_DLY_AVG_CR_BAL          AS YEAR_DLY_AVG_CR_BAL --本年日均贷方余额
         --,T1.YEAR_DLY_AVG_DR_BAL          AS YEAR_DLY_AVG_DR_BAL --本年日均借方余额
        ...
        ,D.MON_CUM_CR_BAL                AS MON_CUM_CR_BAL    --月累计贷方余额
        ,D.MON_CUM_DR_BAL                AS MON_CUM_DR_BAL    --月累计借方余额
        ...
        ,D.YEAR_CUM_DR_BAL               AS YEAR_CUM_DR_BAL   --年累计借方余额
    FROM (
    SELECT A.ETL_DT                                 AS ETL_DT                        --数据日期
          ...
          ,SUM(CASE WHEN A.SUBJ_DIR_CD = 'D' THEN A.YD_OC_DR_BAL -A.YD_OC_CR_BAL  --轧差
                    WHEN A.SUBJ_DIR_CD = 'B' THEN (CASE WHEN A.YD_OC_DR_BAL-A.YD_OC_CR_BAL > 0
                                                        THEN A.YD_OC_DR_BAL-A.YD_OC_CR_BAL ELSE 0 END)
                    WHEN A.SUBJ_DIR_CD = 'C' THEN 0
                    ELSE A.YD_OC_DR_BAL
                END)                                AS QCJFYE                         --期初借方余额
          ,SUM(CASE WHEN A.SUBJ_DIR_CD = 'C' THEN A.YD_OC_CR_BAL-A.YD_OC_DR_BAL   --轧差
                    WHEN A.SUBJ_DIR_CD = 'B' THEN (CASE WHEN A.YD_OC_CR_BAL-A.YD_OC_DR_BAL > 0
                                                        THEN A.YD_OC_CR_BAL-A.YD_OC_DR_BAL ELSE 0 END)
          ...
          ,SUM(CASE WHEN A.SUBJ_DIR_CD = 'D' THEN A.TD_OC_DR_BAL -A.TD_OC_CR_BAL  --轧差
                    WHEN A.SUBJ_DIR_CD = 'B' THEN (CASE WHEN A.TD_OC_DR_BAL-A.TD_OC_CR_BAL > 0
                                                        THEN A.TD_OC_DR_BAL-A.TD_OC_CR_BAL ELSE 0 END)
                    WHEN A.SUBJ_DIR_CD = 'C' THEN 0
                    ELSE A.TD_OC_DR_BAL
          ...
          ,SUM(CASE WHEN A.SUBJ_DIR_CD = 'C' THEN A.TD_OC_CR_BAL-A.TD_OC_DR_BAL   --轧差
                    WHEN A.SUBJ_DIR_CD = 'B' THEN (CASE WHEN A.TD_OC_CR_BAL-A.TD_OC_DR_BAL > 0
                                                        THEN A.TD_OC_CR_BAL-A.TD_OC_DR_BAL ELSE 0 END)
                    WHEN A.SUBJ_DIR_CD = 'D' THEN 0
                    ELSE A.TD_OC_CR_BAL
  ...
  SELECT DISTINCT
    ...
    SELECT TO_CHAR(A.ETL_DT,'YYYYMMDD')    AS DATA_DT          --01 数据日期
          ...
          ,SUM(NVL(A.TD_OC_CR_BAL,0))      AS YEAR_DLY_AVG_CR_BAL --05 年日均贷方余额
          ,SUM(NVL(A.TD_OC_DR_BAL,0))      AS YEAR_DLY_AVG_DR_BAL --05 年日均借方余额
  ...
  USING RRP_MDL.M_GL_BAL_TMP1 B
     ...
     AND B.CUR = A.CUR
     AND B.STD_PROD_ID = A.STD_PROD_ID
  ...
  /*-- 如需要分析表，请用如下代码 --
  --DBMS_STATS.GATHER_TABLE_STATS(OWNNAME=>'RRP_MDL',TABLENAME => 'M_GL_BAL',PARTNAME => 'P_'||V_P_DATE||'',DEGREE => 16,CASCATE => TRUE);
