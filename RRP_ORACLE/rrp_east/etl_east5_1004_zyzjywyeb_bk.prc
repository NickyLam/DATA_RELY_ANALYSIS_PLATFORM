CREATE OR REPLACE PROCEDURE RRP_EAST.ETL_EAST5_1004_ZYZJYWYEB_BK(I_P_DATE IN INTEGER, --跑批日期
...
BEGIN
  ...
  IF V_FREQ_FLAG = '1' THEN
    ...
    ETL_PARTITION_TRUNCATE(V_P_DATE, V_TABLE_NAME, O_ERRCODE);*/
    ETL_PARTITION_ADD(V_MONTH_END_DATEID, V_TABLE_NAME, '1', O_ERRCODE);
    ETL_PARTITION_TRUNCATE(V_MONTH_END_DATEID, V_TABLE_NAME, O_ERRCODE);
    ...
    SELECT 'B1194H244050001'                                           AS JRXKZH -- 金融许可证号
           ...
           /*,CASE WHEN T2.BUYORSELL = 'B'
                 THEN DECODE(T7.COMMONATTS_SHORTNAME,'拆入境内银行业存款类金融机构','2003010101',
                                                     '拆入境内银行业非存款类金融机构','2003010102',
                                                     '拆入境内证券业金融机构','2003010201',
                                                     '拆入境内保险业金融机构','2003010202',
                                                     '拆入境内交易及结算类金融机构','2003010203',
                                                     '拆入境内金融控股公司','2003010204',
                                                     '拆入境内其他金融机构','2003010205',
                                                     '拆入境外银行款项','20030201',
                                                     '拆入境外非银行款项','20030202','')
                 ELSE DECODE(T7.COMMONATTS_SHORTNAME,'拆放境内证券业金融机构','130201010201',
                                                     '拆放境内保险业金融机构','130201010202',
                                                     '拆放境内金融控股公司','130201010203',
                                                     '拆放境内其他金融机构','130201010204',
                                                     '拆放境内交易及结算类金融机构','130201010205',
                                                     '拆放境外银行款项','1302010201',
                                                     '拆放境外非银行款项','1302010202',
                                                     '拆放境内银行款项','1302010101','')
             END                                                       AS MXKMBH --明细科目编号*/
           ,CASE WHEN T2.BUYORSELL = 'B'
                 THEN DECODE(T7.COMMONATTS_SHORTNAME,'拆入境内银行业存款类金融机构','20030101',
                                                     '拆入境内银行业非存款类金融机构','20030101',
                                                     ...
                                                     '拆入境外银行款项','20030201',
                                                     '拆入境外非银行款项','20030202','')
                 ELSE DECODE(T7.COMMONATTS_SHORTNAME,'拆放境内证券业金融机构','13020102',
           ...
           ,CASE WHEN T2.BUYORSELL = 'B' AND T7.COMMONATTS_SHORTNAME IN ('拆入境内银行业存款类金融机构','拆入境内银行业非存款类金融机构','拆入境外银行款项')
                 THEN '拆入银行金融机构'
    ...
    SELECT 'B1194H244050001'                                           AS JRXKZH -- 金融许可证号
           ...
           /*,CASE WHEN T2.IBO_TYPE = '0' AND T2.DEAL_DIR = '1'
                 THEN DECODE(T3.INTERBANKTYPE, '境内证券业金融机构', '2003010201',
                                               '境内保险业金融机构', '2003010202',
                                               '境内交易及结算类金融机构','2003010203',
                                               '境内金融控股公司','2003010204',
                                               '境内其他金融机构','2003010205',
                                               '境内银行业存款类金融机构','2003010101',
                                               '境内银行业非存款类金融机构','2003010102',
                                               '境外银行','20030201',
                                               '境外非银行','20030202','-')
                 WHEN T2.IBO_TYPE = '0' AND T2.DEAL_DIR = '-1'
                 THEN DECODE(T3.INTERBANKTYPE, '境内证券业金融机构', '130201010201',
                                               '境内保险业金融机构', '130201010202',
                                               '境内金融控股公司', '130201010203',
                                               '境内其他金融机构', '130201010204',
                                               '境内交易及结算类金融机构', '130201010205',
                                               '境内银行业非存款类金融机构', '1302010101',
                                               '境内银行业存款类金融机构', '1302010101',
                                               '境外银行', '1302010201',
                                               '境外非银行', '1302010202', '-')
                 WHEN T2.IBO_TYPE = '4' AND T2.COLLATERAL_METHOD = '质押'
                 THEN DECODE(T2.DEAL_DIR, '1', '2111050102', '1111040102')
                 WHEN T2.IBO_TYPE = '4' AND T2.COLLATERAL_METHOD = '买断'
                 THEN DECODE(T2.DEAL_DIR, '1', '2111050101', '1111040101')
             END                                                        AS MXKMBH --明细科目编号*/
            ,CASE WHEN T2.IBO_TYPE = '0' AND T2.DEAL_DIR = '1'
                 THEN DECODE(T3.INTERBANKTYPE, '境内证券业金融机构', '20030102',
                                               '境内保险业金融机构', '20030102',
                                               '境内交易及结算类金融机构','20030102',
                                               '境内金融控股公司','20030102',
                                               '境内其他金融机构','20030102',
                                               '境内银行业存款类金融机构','20030101',
                                               '境内银行业非存款类金融机构','20030101',
                                               '境外银行','20030201',
                                               '境外非银行','20030202','-')
                 WHEN T2.IBO_TYPE = '0' AND T2.DEAL_DIR = '-1'
                 THEN DECODE(T3.INTERBANKTYPE, '境内证券业金融机构', '13020102',
                                               '境内保险业金融机构', '13020102',
                                               '境内金融控股公司', '13020102',
                                               '境内其他金融机构', '13020102',
                                               '境内交易及结算类金融机构', '13020102',
                                               '境内银行业存款类金融机构', '13020101',
                                               '境内银行业非存款类金融机构', '13020102',
                                               '境外银行', '13020201',
                                               '境外非银行', '13020202', '-')
                 WHEN T2.IBO_TYPE = '4' AND T2.COLLATERAL_METHOD = '质押'
           ...
           /*,CASE WHEN T1.BUZTYPE = '现券' AND T1.ASSETTYPE = '交易性金融资产'
            ...
            ,CASE WHEN T1.BUZTYPE = '现券' AND T1.ASSETTYPE = '交易性金融资产'
                 ...
                 THEN CASE WHEN T2.ISSUER IN ('中华人民共和国铁道部','中国铁路总公司','中国国家铁路集团有限公司')
                           ...
                           WHEN T2.SECURITY_TYPE_NEW = 'S' THEN '15030101' --MOD BY LIP 20241010 15030101 计入其他综合收益的债券面值
                           ELSE '15039901'
                       END
                 WHEN T1.BUZTYPE = '现券' AND T1.ASSETTYPE = '持有至到期投资'
                 THEN CASE WHEN T2.ISSUER IN ('中华人民共和国铁道部','中国铁路总公司','中国国家铁路集团有限公司')
    ...
    WITH INPUTPARAMS AS
     (SELECT TO_CHAR(TO_DATE(V_MONTH_START_DATEID,'YYYYMMDD'),'YYYY-MM-DD') FIRSTDATE,
             ...
             TO_CHAR(TO_DATE(V_P_DATE,'YYYYMMDD'),'YYYY-MM-DD') BEGDATE FROM DUAL), --20250220 TEST
    PARAMS AS
     (SELECT FIRSTDATE,
             BEGDATE,
    ...
    ZHLXVIEW AS
     (SELECT /*+MATERIALIZE*/*
       ...
       WHERE FK = 1)/*,
    SM_TYPE AS ( --modify by tangan at 20221031
    SELECT '301010100001' AS PROD_CODE,'国债' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301020100001' AS PROD_CODE,'地方政府债' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301030100001' AS PROD_CODE,'央票' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301040100001' AS PROD_CODE,'政策性金融债' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301040100002' AS PROD_CODE,'政策性金融债' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301040100003' AS PROD_CODE,'政策性金融债' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301050100001' AS PROD_CODE,'商业性金融债' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301050100002' AS PROD_CODE,'商业性金融债' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301050100003' AS PROD_CODE,'商业性金融债' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301050100004' AS PROD_CODE,'商业性金融债' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301050100005' AS PROD_CODE,'商业性金融债' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301050100006' AS PROD_CODE,'商业性金融债' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301050200001' AS PROD_CODE,'商业性金融债' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301050200002' AS PROD_CODE,'商业性金融债' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301050200003' AS PROD_CODE,'商业性金融债' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301050200004' AS PROD_CODE,'商业性金融债' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301050200005' AS PROD_CODE,'商业性金融债' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301050200006' AS PROD_CODE,'商业性金融债' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301050200007' AS PROD_CODE,'商业性金融债' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301050200008' AS PROD_CODE,'商业性金融债' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301060100001' AS PROD_CODE,'政府支持机构债' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301060100002' AS PROD_CODE,'政府支持机构债' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301070100001' AS PROD_CODE,'非金融企业债券' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301070100002' AS PROD_CODE,'非金融企业债券' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301070100003' AS PROD_CODE,'非金融企业债券' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301070200001' AS PROD_CODE,'非金融企业债券' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301070200002' AS PROD_CODE,'非金融企业债券' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301070200003' AS PROD_CODE,'非金融企业债券' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301070200004' AS PROD_CODE,'非金融企业债券' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301070200005' AS PROD_CODE,'非金融企业债券' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301070300001' AS PROD_CODE,'非金融企业债券' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301070300002' AS PROD_CODE,'非金融企业债券' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301070300003' AS PROD_CODE,'非金融企业债券' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301070300004' AS PROD_CODE,'非金融企业债券' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301070300005' AS PROD_CODE,'非金融企业债券' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301070300006' AS PROD_CODE,'非金融企业债券' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301070300007' AS PROD_CODE,'非金融企业债券' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301080100001' AS PROD_CODE,'资产支持证券（交易所资产支持证券）' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301080100002' AS PROD_CODE,'资产支持证券（信贷资产证券化）' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301080200001' AS PROD_CODE,'资产支持证券（资产支持票据）' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301090100001' AS PROD_CODE,'外国债券' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301090100002' AS PROD_CODE,'外国债券' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '301090100003' AS PROD_CODE,'外国债券' AS STYPE,'债券投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '302010100001' AS PROD_CODE,'同业存单投资' AS STYPE,'同业存单' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '303010100001' AS PROD_CODE,'债券基金' AS STYPE,'公募基金投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '303020100001' AS PROD_CODE,'货币市场基金' AS STYPE,'公募基金投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '303030100001' AS PROD_CODE,'股票基金' AS STYPE,'公募基金投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '303040100001' AS PROD_CODE,'基金中基金' AS STYPE,'公募基金投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '303050100001' AS PROD_CODE,'混合基金' AS STYPE,'公募基金投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '304010100001' AS PROD_CODE,'非保本理财投资' AS STYPE,'资产管理产品投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '304010200001' AS PROD_CODE,'非保本理财投资' AS STYPE,'资产管理产品投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '304020100001' AS PROD_CODE,'信托产品（资金信托）' AS STYPE,'资产管理产品投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '304020100002' AS PROD_CODE,'信托产品（资金信托）' AS STYPE,'资产管理产品投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '304020100003' AS PROD_CODE,'信托产品（资金信托）' AS STYPE,'资产管理产品投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '304020100004' AS PROD_CODE,'信托产品（资金信托）' AS STYPE,'资产管理产品投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '304020200001' AS PROD_CODE,'信托产品（资金信托）' AS STYPE,'资产管理产品投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '304030100001' AS PROD_CODE,'证券业资产管理产品（不含公募基金）' AS STYPE,'资产管理产品投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '304030100002' AS PROD_CODE,'证券业资产管理产品（不含公募基金）' AS STYPE,'资产管理产品投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '304030100003' AS PROD_CODE,'证券业资产管理产品（不含公募基金）' AS STYPE,'资产管理产品投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '304030100004' AS PROD_CODE,'证券业资产管理产品（不含公募基金）' AS STYPE,'资产管理产品投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '304030200001' AS PROD_CODE,'证券业资产管理产品（不含公募基金）' AS STYPE,'资产管理产品投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '304030200002' AS PROD_CODE,'证券业资产管理产品（不含公募基金）' AS STYPE,'资产管理产品投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '304030300001' AS PROD_CODE,'证券业资产管理产品（不含公募基金）' AS STYPE,'资产管理产品投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '304030400001' AS PROD_CODE,'证券业资产管理产品（不含公募基金）' AS STYPE,'资产管理产品投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '304040100001' AS PROD_CODE,'保险业资产管理产品' AS STYPE,'资产管理产品投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '304040200001' AS PROD_CODE,'保险业资产管理产品' AS STYPE,'资产管理产品投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '304050100001' AS PROD_CODE,'其他-其他资产管理产品' AS STYPE,'资产管理产品投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '304050200001' AS PROD_CODE,'其他-其他资产管理产品' AS STYPE,'资产管理产品投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '305010100001' AS PROD_CODE,'其他-标准化票据' AS STYPE,'其他投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '306010100001' AS PROD_CODE,'私募证券投资基金' AS STYPE,'私募基金投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '306020100001' AS PROD_CODE,'私募股权投资基金' AS STYPE,'私募基金投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '306030100001' AS PROD_CODE,'私募创业投资基金' AS STYPE,'私募基金投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '306040100001' AS PROD_CODE,'其他-其他私募基金' AS STYPE,'私募基金投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '307010100001' AS PROD_CODE,'其他债权融资（其他交易平台债权融资工具）' AS STYPE,'其他投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '307020100001' AS PROD_CODE,'其他债权融资（其他交易平台债权融资工具）' AS STYPE,'其他投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '307020100002' AS PROD_CODE,'其他债权融资（其他交易平台债权融资工具）' AS STYPE,'其他投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '307030100001' AS PROD_CODE,'其他债权融资（其他交易平台债权融资工具）' AS STYPE,'其他投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '307030200001' AS PROD_CODE,'其他-其他' AS STYPE,'其他投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '307030200002' AS PROD_CODE,'其他-其他' AS STYPE,'其他投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '308010100001' AS PROD_CODE,'非上市股权' AS STYPE,'权益类投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '308020100001' AS PROD_CODE,'上市股票' AS STYPE,'权益类投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '308020200001' AS PROD_CODE,'上市股票' AS STYPE,'权益类投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '308020300001' AS PROD_CODE,'上市股票' AS STYPE,'权益类投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '308020300002' AS PROD_CODE,'上市股票' AS STYPE,'权益类投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '308030100001' AS PROD_CODE,'上市股票' AS STYPE,'权益类投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '308040100001' AS PROD_CODE,'其他-其他权益类投资' AS STYPE,'权益类投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '308040100002' AS PROD_CODE,'其他-其他权益类投资' AS STYPE,'权益类投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '308040100003' AS PROD_CODE,'其他-其他权益类投资' AS STYPE,'权益类投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '308040100004' AS PROD_CODE,'其他-其他权益类投资' AS STYPE,'权益类投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '308040100005' AS PROD_CODE,'其他-其他权益类投资' AS STYPE,'权益类投资' AS MTYPE,'债券投资与同业投资' AS LTYPE FROM DUAL UNION ALL
    SELECT '401010100001' AS PROD_CODE,'其他-同业存放' AS STYPE,'其他' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '401010100002' AS PROD_CODE,'其他-同业存放' AS STYPE,'其他' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '401010100003' AS PROD_CODE,'其他-同业存放' AS STYPE,'其他' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '401010200001' AS PROD_CODE,'其他-同业存放' AS STYPE,'其他' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '401010200002' AS PROD_CODE,'其他-同业存放' AS STYPE,'其他' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '401010200003' AS PROD_CODE,'其他-同业存放' AS STYPE,'其他' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '401020100001' AS PROD_CODE,'拆入银行金融机构' AS STYPE,'拆入' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '401020200001' AS PROD_CODE,'拆入银行金融机构' AS STYPE,'拆入' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '401020300001' AS PROD_CODE,'其他-同业代付' AS STYPE,'其他' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '401020400001' AS PROD_CODE,'拆入银行金融机构' AS STYPE,'拆入' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '401030100001' AS PROD_CODE,'其他-卖出回购票据' AS STYPE,'卖出回购' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '401030200001' AS PROD_CODE,'其他-卖出回购票据' AS STYPE,'卖出回购' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '401030300001' AS PROD_CODE,'卖出回购证券' AS STYPE,'卖出回购' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '401030300002' AS PROD_CODE,'卖出回购证券' AS STYPE,'卖出回购' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '401030400001' AS PROD_CODE,'卖出回购证券' AS STYPE,'卖出回购' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '401040100001' AS PROD_CODE,'同业存单发行' AS STYPE,'同业存单' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '402010100001' AS PROD_CODE,'非结算性存放同业' AS STYPE,'存放同业' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '402010100002' AS PROD_CODE,'非结算性存放同业' AS STYPE,'存放同业' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '402010200001' AS PROD_CODE,'非结算性存放同业' AS STYPE,'存放同业' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '402010200002' AS PROD_CODE,'非结算性存放同业' AS STYPE,'存放同业' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '402020100001' AS PROD_CODE,'拆出银行金融机构' AS STYPE,'拆出' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '402020100003' AS PROD_CODE,'拆出银行金融机构' AS STYPE,'拆出' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '402020200001' AS PROD_CODE,'境内同业借款' AS STYPE,'同业借款' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '402020200002' AS PROD_CODE,'境内同业借款' AS STYPE,'同业借款' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '402020200003' AS PROD_CODE,'境外同业借款' AS STYPE,'同业借款' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '402020400001' AS PROD_CODE,'其他-同业代付' AS STYPE,'其他' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '402030100001' AS PROD_CODE,'其他-买入返售票据' AS STYPE,'买入返售' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '402030200001' AS PROD_CODE,'其他-买入返售票据' AS STYPE,'买入返售' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '402030300001' AS PROD_CODE,'买入返售证券' AS STYPE,'买入返售' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '402030300002' AS PROD_CODE,'买入返售证券' AS STYPE,'买入返售' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '402030400001' AS PROD_CODE,'买入返售证券' AS STYPE,'买入返售' AS MTYPE,'同业往来' AS LTYPE FROM DUAL UNION ALL
    SELECT '403010100001' AS PROD_CODE,'债券借贷' AS STYPE,'其他' AS MTYPE,'同业往来' AS LTYPE FROM DUAL
     )*/
    SELECT NULL                                                        JRXKZH, --金融许可证号
     ...
     WHERE (T.REAL_CP + T.DUE_CP <> 0 OR
       ...
       AND S.P_TYPE IN ('0000','1100','1200','0201','0202','0158','0150','0700','0703','0706','0170','0125','0121','0179');--ADD BY LIP 20240219 增加'0179'银团同业借款数据
...
END ETL_EAST5_1004_ZYZJYWYEB_BK;
/
