CREATE OR REPLACE PROCEDURE RRP_EAST.ETL_EAST5_402_NBKMDZB(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
  /***********************************************************************
  **  存储过程详细说明：内部科目对照表
  **  存储过程名称:  ETL_EAST5_402_NBKMDZB
  **  存储过程创建日期:2022-03-07
  **  存储过程创建人:蔡正伟
  **
  **  输入参数:   I_P_DATE
  **  输出参数:   O_ERRCODE
  **  返回值:     O_ERRCODE
  **  修改日期     修改人        修改原因
  **  20220601     付善斌        主键变更
  **  20220629     LIP           调整格式、修改字段超长、字段换行问题
    ************************************************************************/
IS
  V_P_DATE           VARCHAR2(8);               --数据日期
  V_MONTH_END_DATEID VARCHAR2(8);               --本月月底日期
  V_PARTITION_NAME   VARCHAR2(100);             --分区名称
  V_FREQ_FLAG        VARCHAR2(10);              --跑批频度
  V_STEP             INTEGER := 0;              --任务号
  V_COUNT            INTEGER := 0;              --数据记录条数
  V_STARTTIME        DATE;                      --处理开始时间
  V_ENDTIME          DATE;                      --处理结束时间
  V_SQLCOUNT         INTEGER := 0;              --更新或删除影响的记录数
  V_SQLMSG           VARCHAR2(300);             --SQL执行描述信息
  V_STEP_DESC        VARCHAR2(100);             --处理步骤描述
  V_PROC_NAME        VARCHAR2(100) := 'ETL_EAST5_402_NBKMDZB'; --存储过程名称
  V_TABLE_NAME       VARCHAR2(100) := 'EAST5_402_NBKMDZB'; --表名称
BEGIN
  V_P_DATE  := TO_CHAR(I_P_DATE);
  O_ERRCODE := '0';
  V_PARTITION_NAME := 'PARTITION_' || V_P_DATE;
  V_FREQ_FLAG      := RRP_EAST.FUN_FREQ(I_P_DATE,V_PROC_NAME);
  V_MONTH_END_DATEID := TO_CHAR(LAST_DAY(TO_DATE(V_P_DATE,'YYYYMMDD')),'YYYYMMDD');

  --判断跑批频度
  IF V_FREQ_FLAG = '1' THEN
    /*增加分区*/
    V_STEP := V_STEP + 1;
    V_STEP_DESC := '删除当日分区数据';
    V_STARTTIME := SYSDATE;
    --删除当日分区数据
    RRP_EAST.ETL_PARTITION_ADD(V_P_DATE, V_TABLE_NAME, 1, O_ERRCODE);
    RRP_EAST.ETL_PARTITION_TRUNCATE(V_P_DATE, V_TABLE_NAME, O_ERRCODE);

    V_SQLCOUNT := SQL%ROWCOUNT;
    V_SQLMSG   := '返回值：['||SQLCODE||'],描述信息：'||SQLERRM;
    O_ERRCODE  := '0';
    V_ENDTIME  := SYSDATE;
    COMMIT;
    ETL_YUSYS_LOG(V_P_DATE,V_TABLE_NAME,V_PROC_NAME,V_STARTTIME,V_ENDTIME,V_STEP,V_STEP_DESC,V_SQLCOUNT,O_ERRCODE,'');

    --程序业务逻辑处理主体部分
    V_STEP := V_STEP + 1;
    V_STEP_DESC := '插入目标表';
    V_STARTTIME := SYSDATE;
    INSERT INTO RRP_EAST.EAST5_402_NBKMDZB(
       RID,        --数据主键
       JRXKZH,     --金融许可证号
       NBJGH,      --内部机构号
       YHJGMC,     --银行机构名称
       KJKMBH,     --会计科目编号
       KJKMMC,     --会计科目名称
       KJKMJC,     --会计科目级次
       SJKMBH,     --上级科目编号
       SJKMMC,     --上级科目名称
       KMJDBZ,     --科目借贷标志
       GSYWDL,     --归属业务大类
       GSYWZL,     --归属业务子类
       BBZ,        --备注
       CJRQ,       --采集日期
       DEPT_NO,    --部门编号
       SRC_SYS_ID, --来源系统ID
       ISSUED_NO,  --填报机构
       ORG_NO,     --报送机构
       ADDRESS,    --归属地
       GSFZJG      --归属分支机构
       )
    SELECT SYS_GUID()                                                AS RID,        --数据主键
           'B1194H244050001'                                         AS JRXKZH,     --金融许可证号
           '800'                                                     AS NBJGH,      --内部机构号
           '广东华兴银行股份有限公司总行'                            AS YHJGMC,     --银行机构名称
           A.SUBJ_ID                                                 AS KJKMBH,     --会计科目编号
           A.SUBJ_NM                                                 AS KJKMMC,     --会计科目名称
           A.SUBJ_LVL                                                AS KJKMJC,     --会计科目级次
           A.UP_SUBJ_ID                                              AS SJKMBH,     --上级科目编号
           CASE WHEN A.UP_SUBJ_ID = '0' THEN '0' ELSE B.SUBJ_NM END  AS SJKMMC,     --上级科目名称
           --CODE.TAR_VALUE_NAME                                       AS KMJDBZ,     --科目借贷标志
           CASE WHEN SUBSTR(A.SUBJ_ID,1,1) IN ('7','8') THEN '借' --根据陈天骏需求，把表外科目（7,8开头的)借贷标识设为借。MOD BY LIP 20250926
                ELSE CODE.TAR_VALUE_NAME
            END                                                      AS KMJDBZ,     --科目借贷标志
           CODE1.TAR_VALUE_NAME                                      AS GSYWDL,     --归属业务大类
           A.BLNG_BIZ_SUM_CL                                         AS GSYWZL,     --归属业务子类
           ''                                                        AS BBZ,        --备注
           V_MONTH_END_DATEID                                        AS CJRQ,       --采集日期
           '000'                                                     AS DEPT_NO,    --部门编号
           '01'                                                      AS SRC_SYS_ID, --来源系统ID
           '000000'                                                  AS ISSUED_NO,  --填报机构
           '000000'                                                  AS ORG_NO,     --报送机构
           'B1194B244030001'                                         AS ADDRESS,    --归属地
           '9999'                                                    AS GSFZJG      --归属分支机构
      FROM RRP_MDL.M_GL_INFO A --总账会计科目信息表
      LEFT JOIN RRP_MDL.M_GL_INFO B --总账会计科目信息表
        ON B.SUBJ_ID = A.UP_SUBJ_ID
       AND B.SUBJ_LVL <= 3 --科目报送到三级
       AND B.DATA_DT = V_P_DATE
      LEFT JOIN RRP_MDL.CODE_MAP CODE --码值配置表
        ON CODE.SRC_VALUE_CODE = A.SUBJ_DR_CR_FLG
       AND CODE.SRC_CLASS_CODE = 'Z0017' --科目借贷标志
       AND CODE.TAR_CLASS_CODE = 'Z0017' --科目借贷标志
       AND CODE.MOD_FLG = 'EAST'
      LEFT JOIN RRP_MDL.CODE_MAP CODE1 --码值配置表
        ON CODE1.SRC_VALUE_CODE = A.BLNG_BIZ_LRG_CL
       AND CODE1.SRC_CLASS_CODE = 'Z0005' --归属业务大类
       AND CODE1.TAR_CLASS_CODE = 'Z0005' --归属业务大类
       AND CODE1.MOD_FLG = 'EAST'
     WHERE A.SUBJ_LVL <= 3 --科目报送到三级
       AND A.DATA_DT = V_P_DATE;

    V_SQLCOUNT := SQL%ROWCOUNT;
    V_SQLMSG   := '返回值：['||SQLCODE||'],描述信息：'||SQLERRM;
    O_ERRCODE  := '0';
    V_ENDTIME  := SYSDATE;
    COMMIT;
    ETL_YUSYS_LOG(V_P_DATE,V_TABLE_NAME,V_PROC_NAME,V_STARTTIME,V_ENDTIME,V_STEP,V_STEP_DESC,V_SQLCOUNT,O_ERRCODE,'');

    V_STEP := V_STEP + 1;
    V_STEP_DESC := '查询数据是否重复';
    V_STARTTIME := SYSDATE;
      WITH TMP1 AS (
    SELECT CJRQ,NBJGH,KJKMBH,COUNT(1)
      FROM RRP_EAST.EAST5_402_NBKMDZB T
     WHERE CJRQ = V_P_DATE
     GROUP BY CJRQ,NBJGH,KJKMBH
    HAVING COUNT(1) > 1)
    SELECT NVL(COUNT(1),0) INTO V_COUNT FROM TMP1;

    O_ERRCODE := '0';
    V_ENDTIME := SYSDATE;
    IF V_COUNT > 0 THEN
       O_ERRCODE := '1';
       V_SQLMSG  := 'EAST5_402_NBKMDZB(CJRQ,NBJGH,KJKMBH)数据重复';
       ETL_YUSYS_LOG(V_P_DATE,V_TABLE_NAME,V_PROC_NAME,V_STARTTIME,V_ENDTIME,V_STEP,V_STEP_DESC,V_COUNT,O_ERRCODE,V_SQLMSG);
       RETURN;
    END IF;
    ETL_YUSYS_LOG(V_P_DATE,V_TABLE_NAME,V_PROC_NAME,V_STARTTIME,V_ENDTIME,V_STEP,V_STEP_DESC,V_COUNT,O_ERRCODE,'');

    --表分析
    V_STEP := V_STEP + 1;
    V_STEP_DESC := '表分析开始';
    V_STARTTIME := SYSDATE;
    RRP_EAST.ETL_DBMS_STATS(V_P_DATE,V_TABLE_NAME,V_PARTITION_NAME,O_ERRCODE);

    V_SQLCOUNT := SQL%ROWCOUNT;
    V_SQLMSG   := '返回值：['||SQLCODE||'],描述信息：'||SQLERRM;
    O_ERRCODE  := '0';
    V_ENDTIME  := SYSDATE;
    COMMIT;
    ETL_YUSYS_LOG(V_P_DATE,V_TABLE_NAME,V_PROC_NAME,V_STARTTIME,V_ENDTIME,V_STEP,V_STEP_DESC,V_SQLCOUNT,O_ERRCODE,'');

  END IF;

  V_STEP := V_STEP + 1;
  V_STEP_DESC := '跑批结束';
  V_STARTTIME := SYSDATE;
  --在过程跑批完成记录表中插入记录，调度查询该表判断过程是是否跑批完成
  INSERT INTO RRP_MDL.ETL_STATE(ETL_DATE,PROC_NAME,END_TIME)
  VALUES (V_P_DATE,V_PROC_NAME,TO_CHAR(SYSTIMESTAMP,'YYYYMMDD HH24:MI:SS'));

  V_SQLCOUNT := SQL%ROWCOUNT;
  V_SQLMSG   := '返回值：['||SQLCODE||'],描述信息：'||SQLERRM;
  O_ERRCODE  := '0';
  V_ENDTIME  := SYSDATE;
  COMMIT;
  ETL_YUSYS_LOG(V_P_DATE,V_TABLE_NAME,V_PROC_NAME,V_STARTTIME,V_ENDTIME,V_STEP,V_STEP_DESC,V_SQLCOUNT,O_ERRCODE,'');

EXCEPTION
  WHEN OTHERS THEN
    O_ERRCODE := '1';
    V_SQLMSG  := '跑批错误：['||SQLCODE||'],描述信息：'||SQLERRM;
    V_ENDTIME := SYSDATE;
    ROLLBACK;
    ETL_YUSYS_LOG(V_P_DATE,V_TABLE_NAME,V_PROC_NAME,V_STARTTIME,V_ENDTIME,V_STEP,V_STEP_DESC,V_SQLCOUNT,O_ERRCODE,V_SQLMSG);

END ETL_EAST5_402_NBKMDZB;
/

