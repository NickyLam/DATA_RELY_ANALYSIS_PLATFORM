CREATE OR REPLACE FUNCTION RRP_EAST.FUN_DESENSITIZATION(I_CUST_NAME IN VARCHAR2,
...
BEGIN
  ...
  SELECT UPPER(VALUE) INTO V_NLS_CHARACTERSET
    FROM NLS_DATABASE_PARAMETERS
   WHERE PARAMETER = 'NLS_CHARACTERSET';

  SELECT CASE /*WHEN REGEXP_LIKE(V_CUST_NAME, '.([a-z]+|[A-Z])')*/
              WHEN LENGTH(V_CUST_NAME) = LENGTHB(V_CUST_NAME) THEN 'Y'
              ELSE 'N'
          END INTO V_EN_FLAG
    FROM DUAL;
  ...
  --  参数为空处理逻辑  --
  IF TRIM(V_CUST_NAME) IS NULL THEN
    V_RESULT := '';
  ...
  ELSIF V_NLS_CHARACTERSET = 'ZHS16GBK' THEN
    ...
    ELSIF V_EN_FLAG = 'Y' AND V_DES_TYP = 0 THEN
      --如果有英文名称，且脱敏类型为最后字段，则截取字段后三位
      SELECT SUBSTR(V_CUST_NAME, LENGTH(V_CUST_NAME) - 2, 3) INTO V_RESULT FROM DUAL;
    ELSIF V_EN_FLAG = 'N' AND V_DES_TYP = 1 THEN
      --如果无英文名称，且脱敏类型为第一个字段，则截取字段前一位
      SELECT SUBSTR(V_CUST_NAME, 1, 1) INTO V_RESULT FROM DUAL;
    ELSIF V_EN_FLAG = 'N' AND V_DES_TYP = 0 THEN
      --如果无英文名称，且脱敏类型为第一个字段，则截取字段后一位
      SELECT SUBSTR(V_CUST_NAME, LENGTH(V_CUST_NAME), 1) INTO V_RESULT FROM DUAL;
    END IF;
  ...
  ELSIF V_NLS_CHARACTERSET IN ('AL32UTF8', 'UTF8') THEN
    ...
    ELSIF V_EN_FLAG = 'Y' AND V_DES_TYP = 0 THEN
      --如果有英文名称，且脱敏类型为最后字段，则截取字段后三位
      SELECT SUBSTRB(V_CUST_NAME, LENGTHB(V_CUST_NAME) - 2, 3) INTO V_RESULT FROM DUAL;
    ELSIF V_EN_FLAG = 'N' AND V_DES_TYP = 1 THEN
      --如果无英文名称，且脱敏类型为第一个字段，则截取字段前一位
      SELECT SUBSTRB(V_CUST_NAME, 1, 3) INTO V_RESULT FROM DUAL;
    ELSIF V_EN_FLAG = 'N' AND V_DES_TYP = 0 THEN
      --如果无英文名称，且脱敏类型为第一个字段，则截取字段后一位
      --SELECT SUBSTRB(V_CUST_NAME, LENGTHB(V_CUST_NAME) - 2, 3) INTO V_RESULT FROM DUAL;
      SELECT SUBSTR(V_CUST_NAME,LENGTH(V_CUST_NAME),1) INTO V_RESULT FROM DUAL; --MOD BY LIP 20250407 调整取数
    END IF;
