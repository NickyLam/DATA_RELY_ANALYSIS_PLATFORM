CREATE OR REPLACE PROCEDURE RRP_EAST.ETL_EAST5_403_GRCKFHZ(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
/***********************************************************************
  **  存储过程详细说明：个人存款分户账
  **  存储过程名称:  ETL_EAST5_403_GRCKFHZ
  **  存储过程创建日期:2022-03-07
  **  存储过程创建人:蔡正伟
  **  输入参数:   I_P_DATE
  **  输出参数:   O_ERRCODE
...
IS
  ...
  V_PROC_NAME        VARCHAR2(100) := UPPER('ETL_EAST5_403_GRCKFHZ'); --存储过程名称
  V_TABLE_NAME       VARCHAR2(100) := UPPER('EAST5_403_GRCKFHZ'); --表名称
BEGIN
  ...
  IF V_FREQ_FLAG = '1' THEN
    ...
    V_STEP_DESC := '插入目标表';
    V_STARTTIME := SYSDATE;
    INSERT /*+APPNED*/ INTO RRP_EAST.EAST5_403_GRCKFHZ NOLOGGING
      (RID,          --数据主键
       JRXKZH,       --金融许可证号
       NBJGH,        --内部机构号
       YHJGMC,       --银行机构名称
    ...
    SELECT SYS_GUID()                                              AS RID,          --数据主键
           ...
           NVL(C.CUST_NM_DESEN,BC.CUST_NM)                         AS ZHMC,         --账户名称--MODIFY BY LAIHAIQIANG AT 20230403
           --A.ACC_ID_EAST                                           AS GRCKZH,       --个人存款账号
           ...
           CASE WHEN A.DEP_PROD_TYP = '0502' THEN '个人活期存款'
                WHEN A.DEP_PROD_TYP LIKE '0102%' OR A.DEP_PROD_TYP = '1401' THEN '个人定期存款'
                WHEN A.DEP_PROD_TYP = '0300' THEN '定活两便存款'
                WHEN A.DEP_PROD_TYP IN ('0401', '0402', '0400') THEN '个人通知存款'
                WHEN A.DEP_PROD_TYP IN ('0201', '0202', '0200') THEN '个人协议存款'
                WHEN A.DEP_PROD_TYP IN ('0601', '0602', '0600') THEN '个人协定存款'
                WHEN A.DEP_PROD_TYP LIKE '07%' THEN '个人保证金存款'
                WHEN A.DEP_PROD_TYP = '1000' THEN '个人结构性存款（不含保本理财）'
                ELSE '个人其他存款'
            END                                                    AS GRCKZHLX,     --个人存款账户类型
      ...
      LEFT JOIN RRP_EAST.M_CUST_IND_INFO_EAST C --个人客户信息
        ON C.CUST_ID = A.CUST_ID
       AND C.DATA_DT = V_P_DATE
      LEFT JOIN RRP_MDL.M_CUST_CORP_INFO BC --对公客户信息
        ON BC.CUST_ID = A.CUST_ID
       AND BC.CUST_CL = 'E'  --个体工商户
       AND BC.DATA_DT = V_P_DATE
     ...
     WHERE A.CORP_IND_FLG = '1' --对私
       ...
       AND (NVL(A.CNL_ACC_DT,'99991231')>= SUBSTR(V_P_DATE,1,6)||'01' OR A.DEP_BAL>0)
    ...
    V_STARTTIME := SYSDATE;
      WITH TMP1 AS (
    SELECT CJRQ,GRCKZH,BZ,CHLB,COUNT(1)
      FROM RRP_EAST.EAST5_403_GRCKFHZ T
     WHERE CJRQ = V_P_DATE
     GROUP BY CJRQ,GRCKZH,BZ,CHLB
    HAVING COUNT(1) > 1)
    ...
    IF V_COUNT > 0 THEN
       O_ERRCODE := '1';
       V_SQLMSG  := 'EAST5_403_GRCKFHZ(CJRQ,GRCKZH,BZ,CHLB)数据重复';
