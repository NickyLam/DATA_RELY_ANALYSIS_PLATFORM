CREATE OR REPLACE PROCEDURE RRP_EAST.ETL_EAST5_513_HLWDKHZXYB(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
 /***********************************************************************
  **  存储过程详细说明：互联网贷款合作协议表
  **  存储过程名称:  ETL_EAST5_513_HLWDKHZXYB
  **  存储过程创建日期:2022-07-14
  **  存储过程创建人:付善斌
  **        M_LOAN_NET_COOP_SUB A --互联网贷款合作协议表
  **        LOAN_CONT_INFO C --贷款合同信息
  **        M_PUM_ORG_INFO_EAST B --机构表
  **  目标表:
  **         EAST5_513_HLWDKHZXYB
  **  修改日期     修改人      修改项目
  ***********************************************************************/
IS
  V_P_DATE           VARCHAR2(8);      --数据日期
  V_MONTH_END_DATEID VARCHAR2(8);      --本月月底日期
  V_PARTITION_NAME   VARCHAR2(100);    --分区名称
  V_FREQ_FLAG        VARCHAR2(10);     --跑批频度
  V_STEP             INTEGER := 0;     --任务号
  V_COUNT            INTEGER := 0;     --数据记录条数
  V_SQLCOUNT         INTEGER := 0;     --更新或删除影响的记录数
  V_STARTTIME        DATE := SYSDATE;  --处理开始时间
  V_ENDTIME          DATE := SYSDATE;  --处理结束时间
  V_SQLMSG           VARCHAR2(300);    --SQL执行描述信息
  V_STEP_DESC        VARCHAR2(100);    --处理步骤描述
  V_TABLE_NAME       VARCHAR2(100) := 'EAST5_513_HLWDKHZXYB'; --表名称
  V_PROC_NAME        VARCHAR2(100) := 'ETL_EAST5_513_HLWDKHZXYB'; --存储过程名称
BEGIN
  V_P_DATE  := TO_CHAR(I_P_DATE);
  O_ERRCODE := '0';
  V_MONTH_END_DATEID := TO_CHAR(LAST_DAY(TO_DATE(V_P_DATE,'YYYYMMDD')),'YYYYMMDD');
  V_PARTITION_NAME   := 'PARTITION_' || V_P_DATE;
  V_FREQ_FLAG        := RRP_EAST.FUN_FREQ(I_P_DATE,V_PROC_NAME);

  --判断跑批频度
  IF V_FREQ_FLAG = '1' THEN
    --增加分区
    V_STEP := 1;
    V_STEP_DESC := '增加分区';
    V_STARTTIME := SYSDATE;
    RRP_EAST.ETL_PARTITION_ADD(V_P_DATE, V_TABLE_NAME, 1, O_ERRCODE);

    V_SQLCOUNT := SQL%ROWCOUNT;
    V_SQLMSG   := '返回值：['||SQLCODE||'],描述信息：'||SQLERRM;
    O_ERRCODE  := '0';
    V_ENDTIME  := SYSDATE;
    COMMIT;
    ETL_YUSYS_LOG(V_P_DATE,V_TABLE_NAME,V_PROC_NAME,V_STARTTIME,V_ENDTIME,V_STEP,V_STEP_DESC,V_SQLCOUNT,O_ERRCODE,'');

    --删除当日分区数据
    V_STEP := V_STEP + 1;
    V_STEP_DESC := '删除当日分区数据';
    V_STARTTIME := SYSDATE;
    RRP_EAST.ETL_PARTITION_TRUNCATE(V_P_DATE,V_TABLE_NAME,O_ERRCODE); --清空当日分区以便重跑

    V_SQLCOUNT := SQL%ROWCOUNT;
    V_SQLMSG   := '返回值：['||SQLCODE||'],描述信息：'||SQLERRM;
    O_ERRCODE  := '0';
    V_ENDTIME  := SYSDATE;
    COMMIT;
    ETL_YUSYS_LOG(V_P_DATE,V_TABLE_NAME,V_PROC_NAME,V_STARTTIME,V_ENDTIME,V_STEP,V_STEP_DESC,V_SQLCOUNT,O_ERRCODE,'');

    V_STEP := V_STEP + 1;
    V_STEP_DESC := '装入目标表';
    V_STARTTIME := SYSDATE;
    INSERT INTO RRP_EAST.EAST5_513_HLWDKHZXYB
      (RID, --数据主键
       JRXKZH, --金融许可证号
       NBJGH, --内部机构号
       YHJGMC, --银行机构名称
       HZXYBH, --合作协议编号
       HZFMC, --合作方名称
       HZFZJLB, --合作方证件类别
       HZFZJHM, --合作方证件号码
       HZFLX, --合作方类型
       HZFS, --合作方式
       XYQSRQ, --协议起始日期
       XYDQRQ, --协议到期日期
       SJZZRQ, --实际终止日期
       XZQHDM, --合作方注册地代码
       XZBZ, --限制标志
       XYZT, --协议状态
       BBZ, --备注
       CJRQ, --采集日期
       DEPT_NO, --部门编号
       SRC_SYS_ID, --来源系统ID
       ISSUED_NO, --填报机构
       ORG_NO, --报送机构
       ADDRESS, --归属地
       HZFZJHM_ORIG, --合作方证件号码（脱敏前）
       GSFZJG --归属分支机构
       )
      WITH LOAN_CONT_INFO AS (
    SELECT C.COOP_AGRT_ID
      FROM RRP_MDL.M_LOAN_CONT_INFO C --贷款合同信息
     WHERE C.COOP_AGRT_ID IS NOT NULL
       AND C.MON_FLG = 'Y' --ADD BY LIP 20221222
       AND C.DATA_DT = V_P_DATE
     GROUP BY C.COOP_AGRT_ID),
    --MOD BY LIP 20250731 合同表的合作协议编号会有多个，用;分割，需先拆分
    LOAN_CONT_INFO_TMP AS (
    SELECT REGEXP_SUBSTR(C.COOP_AGRT_ID, '[^;]+', 1, LEVEL) COOP_AGRT_ID
      FROM LOAN_CONT_INFO C
   CONNECT BY LEVEL <= LENGTH(C.COOP_AGRT_ID) - LENGTH(REPLACE(C.COOP_AGRT_ID, ';')) + 1
     GROUP BY REGEXP_SUBSTR(C.COOP_AGRT_ID, '[^;]+', 1, LEVEL))
    SELECT SYS_GUID()                                               AS RID, ---数据主键
           B.FIN_PERMIT_NO                                          AS JRXKZH, --金融许可证号
           B.ORG_ID                                                 AS NBJGH, --内部机构号
           B.ORG_NM                                                 AS YHJGMC, --银行机构名称
           --A.COOP_AGRT_ID                                           AS HZXYBH, --合作协议编号
           --MOD BY LIP 20231018 因均衡助贷有多个协议，此处改为子合作协议编号
           A.SUB_COOP_AGRT_ID                                       AS HZXYBH, --合作协议编号
           A.PNR_NM                                                 AS HZFMC, --合作方名称
           --CODE.TAR_VALUE_NAME                                     AS HZFZJLB, --合作方证件类别
           --TRIM(SUBSTRB(CODE.TAR_VALUE_NAME,1,40))                  AS HZFZJLB, --合作方证件类别
           TRIM(SUBSTRB(CODE.TAR_VALUE_NAME,1,60))                  AS HZFZJLB, --合作方证件类别 --MODIFY BY LIP 20240409 改为UTF-8的长度
           A.PNR_CRDL_NO                                            AS HZFZJHM, --合作方证件号码
           CODE1.TAR_VALUE_NAME                                     AS HZFLX, --合作方类型
           --CODE2.TAR_VALUE_NAME                                     AS HZFS, --合作方式
           --MOD BY LIP 20251114 根据一表通的码值映射成east码值
           CASE WHEN CODE2.TAR_VALUE_NAME IS NOT NULL THEN CODE2.TAR_VALUE_NAME
                ELSE '其他-'||REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(A.COOP_MODE,
                     '01','营销获客'),
                     '02','联合贷款'),
                     '03','支付结算'),
                     '04','风险分担'),
                     '05','担保增信'),
                     '06','信息科技'),
                     '07','逾期清收'),
                     '08','其他-客户筛选'),
                     '09','其他-部分风险评价'),
                     '10','其他-其他'),
                     '00','其他-无合作方')
            END                                                     AS HZFS, --合作方式
           NVL(A.AGRT_START_DT,'99991231')                          AS XYQSRQ, --协议起始日期
           NVL(A.AGRT_EXP_DT,'99991231')                            AS XYDQRQ, --协议到期日期
           NVL(A.ACT_END_DT,'99991231')                             AS SJZZRQ, --实际终止日期
           A.PNR_REGD_LAND_CD                                       AS XZQHDM, --合作方注册地代码
           CODE3.TAR_VALUE_NAME                                     AS XZBZ, --限制标志
           --CODE4.TAR_VALUE_NAME                                    AS XYZT, --协议状态
           --TRIM(SUBSTRB(CODE4.TAR_VALUE_NAME,1,20))                 AS XYZT, --协议状态
           TRIM(SUBSTRB(CODE4.TAR_VALUE_NAME,1,30))                 AS XYZT, --协议状态 --MODIFY BY LIP 20240409 改为UTF-8的长度
           ''                                                       AS BBZ, --备注
           V_MONTH_END_DATEID                                       AS CJRQ, --采集日期
           '000'                                                    AS DEPT_NO, --部门编号
           '01'                                                     AS SRC_SYS_ID, --来源系统ID
           '000000'                                                 AS ISSUED_NO, --填报机构
           ORG.ORG_ID_LEL_0                                         AS ORG_NO, --报送机构
           CASE WHEN LIST.FLAG = 1 THEN ORG.ORG_ID_LEL_1
                ELSE LIST.REP_ORG_ID
            END                                                     AS ADDRESS, --归属地
           A.PNR_CRDL_NO                                            AS HZFZJHM_ORIG, --合作方证件号码（脱敏前）
           CASE WHEN LIST.FLAG = 1 THEN B.GSFZJG
                ELSE '9999'
            END                                                     AS GSFZJG--归属分支机构
      FROM RRP_MDL.M_LOAN_NET_COOP_SUB A --互联网贷款合作协议表
     --INNER JOIN LOAN_CONT_INFO C --贷款合同信息
     INNER JOIN LOAN_CONT_INFO_TMP C --贷款合同信息
        ON C.COOP_AGRT_ID = A.COOP_AGRT_ID
      LEFT JOIN RRP_MDL.ORG_CONFIG M
        ON M.ORG_ID = A.ORG_ID
      LEFT JOIN RRP_MDL.M_PUM_ORG_INFO_EAST B --机构表
        ON B.ORG_ID = NVL(M.ORG_ID1,'800')
       AND B.DATA_DT = V_P_DATE
      LEFT JOIN RRP_MDL.CODE_MAP CODE --码值配置表
        ON CODE.SRC_VALUE_CODE = A.PNR_CRDL_TYP
       AND CODE.SRC_CLASS_CODE = 'C0001' --合作方证件类别
       AND CODE.TAR_CLASS_CODE = 'C0001' --合作方证件类别
       AND CODE.MOD_FLG = 'EAST'
      LEFT JOIN RRP_MDL.CODE_MAP CODE1 --码值配置表
        ON CODE1.SRC_VALUE_CODE = A.PNR_TYP
       AND CODE1.SRC_CLASS_CODE = 'C0051' --合作方类型
       AND CODE1.TAR_CLASS_CODE = 'C0051' --合作方类型
       AND CODE1.MOD_FLG = 'EAST'
      LEFT JOIN RRP_MDL.CODE_MAP CODE2 --码值配置表
        ON CODE2.SRC_VALUE_CODE = A.COOP_MODE
       AND CODE2.SRC_CLASS_CODE = 'C0052' --合作方式
       AND CODE2.TAR_CLASS_CODE = 'C0052' --合作方式
       AND CODE2.MOD_FLG = 'EAST'
      LEFT JOIN RRP_MDL.CODE_MAP CODE3 --码值配置表
        ON CODE3.SRC_VALUE_CODE = A.RST_FLG
       AND CODE3.SRC_CLASS_CODE = 'Z0001' --限制标志
       AND CODE3.TAR_CLASS_CODE = 'Z0001' --限制标志
       AND CODE3.MOD_FLG = 'EAST'
      LEFT JOIN RRP_MDL.CODE_MAP CODE4 --码值配置表
        ON CODE4.SRC_VALUE_CODE = A.AGRT_STAT
       AND CODE4.SRC_CLASS_CODE = 'C0053' --协议状态
       AND CODE4.TAR_CLASS_CODE = 'C0053' --协议状态
       AND CODE4.MOD_FLG = 'EAST'
      LEFT JOIN RRP_MDL.CONFIG_ORG_REL ORG --机构级次关系表
        ON ORG.ORG_ID = B.ORG_ID
      LEFT JOIN RRP_MDL.CONFIG_TABLE_LIST LIST --分行报送报表配置表：记录分行报送的报表范围是全行数据还是本行数据 1 只报分行 0 全表报送
        ON UPPER(LIST.TABLE_NAME) = V_TABLE_NAME
     WHERE A.DATA_DT = V_P_DATE;

    V_SQLCOUNT := SQL%ROWCOUNT;
    V_SQLMSG   := '返回值：['||SQLCODE||'],描述信息：'||SQLERRM;
    O_ERRCODE  := '0';
    V_ENDTIME  := SYSDATE;
    COMMIT;
    ETL_YUSYS_LOG(V_P_DATE,V_TABLE_NAME,V_PROC_NAME,V_STARTTIME,V_ENDTIME,V_STEP,V_STEP_DESC,V_SQLCOUNT,O_ERRCODE,'');

    --ADD BY LIP 20220603 去掉表的主键，通过语句判断数据是否重复
    V_STEP := V_STEP + 1;
    V_STEP_DESC := '查询数据是否重复';
    V_STARTTIME := SYSDATE;
      WITH TMP1 AS (
    SELECT HZXYBH,HZFZJHM,CJRQ,COUNT(1)
      FROM RRP_EAST.EAST5_513_HLWDKHZXYB T
     WHERE CJRQ = V_MONTH_END_DATEID
     GROUP BY HZXYBH,HZFZJHM,CJRQ
    HAVING COUNT(1) > 1)
    SELECT NVL(COUNT(1),0) INTO V_COUNT FROM TMP1;

    O_ERRCODE := '0';
    V_ENDTIME := SYSDATE;
    IF V_COUNT > 0 THEN
       O_ERRCODE := '1';
       V_SQLMSG  := 'EAST5_513_HLWDKHZXYB(CJRQ,HZXYBH,HZFZJHM)数据重复';
       ETL_YUSYS_LOG(V_P_DATE,V_TABLE_NAME,V_PROC_NAME,V_STARTTIME,V_ENDTIME,V_STEP,V_STEP_DESC,V_COUNT,O_ERRCODE,V_SQLMSG);
       RETURN;
    END IF;
    ETL_YUSYS_LOG(V_P_DATE,V_TABLE_NAME,V_PROC_NAME,V_STARTTIME,V_ENDTIME,V_STEP,V_STEP_DESC,V_COUNT,O_ERRCODE,'');

    --表分析
    V_STEP := V_STEP + 1;
    V_STEP_DESC := '表分析';
    V_STARTTIME := SYSDATE;
    RRP_EAST.ETL_DBMS_STATS(V_P_DATE,V_TABLE_NAME,V_PARTITION_NAME,O_ERRCODE);

    V_SQLCOUNT := SQL%ROWCOUNT;
    V_SQLMSG   := '返回值：['||SQLCODE||'],描述信息：'||SQLERRM;
    O_ERRCODE  := '0';
    V_ENDTIME  := SYSDATE;
    COMMIT;
    ETL_YUSYS_LOG(V_P_DATE,V_TABLE_NAME,V_PROC_NAME,V_STARTTIME,V_ENDTIME,V_STEP,V_STEP_DESC,V_SQLCOUNT,O_ERRCODE,'');
  END IF;

  V_STEP := V_STEP + 1;
  V_STEP_DESC := '跑批结束';
  V_STARTTIME := SYSDATE;
  --在过程跑批完成记录表中插入记录，调度查询该表判断过程是是否跑批完成
  INSERT INTO RRP_MDL.ETL_STATE(ETL_DATE,PROC_NAME,END_TIME)
  VALUES (V_P_DATE,V_PROC_NAME,TO_CHAR(SYSTIMESTAMP,'YYYYMMDD HH24:MI:SS'));

  V_SQLCOUNT := SQL%ROWCOUNT;
  V_SQLMSG   := '返回值：['||SQLCODE||'],描述信息：'||SQLERRM;
  O_ERRCODE  := '0';
  V_ENDTIME  := SYSDATE;
  COMMIT;
  ETL_YUSYS_LOG(V_P_DATE,V_TABLE_NAME,V_PROC_NAME,V_STARTTIME,V_ENDTIME,V_STEP,V_STEP_DESC,V_SQLCOUNT,O_ERRCODE,'');

EXCEPTION
  WHEN OTHERS THEN
    O_ERRCODE := '1';
    V_SQLMSG  := '跑批错误：['||SQLCODE||'],描述信息：'||SQLERRM;
    V_ENDTIME := SYSDATE;
    ROLLBACK;
    ETL_YUSYS_LOG(V_P_DATE,V_TABLE_NAME,V_PROC_NAME,V_STARTTIME,V_ENDTIME,V_STEP,V_STEP_DESC,V_SQLCOUNT,O_ERRCODE,V_SQLMSG);

END ETL_EAST5_513_HLWDKHZXYB;
/

