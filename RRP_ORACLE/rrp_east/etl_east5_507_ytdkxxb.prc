CREATE OR REPLACE PROCEDURE RRP_EAST.ETL_EAST5_507_YTDKXXB(I_P_DATE IN INTEGER,O_ERRCODE OUT VARCHAR2)
/***********************************************************************
  **  存储过程详细说明：银团贷款信息表
  **  存储过程名称:  ETL_EAST5_507_YTDKXXB
  **  存储过程创建日期:2022-03-07
  **  存储过程创建人:蔡正伟
  **  来源表:
  **        M_LOAN_IN_DUBILL_INFO --表内借据信息
  **        M_LOAN_SYN_SUB C      --银团贷款子表
...
IS
  ...
  V_PROC_NAME        VARCHAR2(100) := UPPER('ETL_EAST5_507_YTDKXXB'); --存储过程名称
  V_TABLE_NAME       VARCHAR2(100) := UPPER('EAST5_507_YTDKXXB'); --表名称
BEGIN
  ...
  IF V_FREQ_FLAG = '1' THEN
    ...
    V_STEP_DESC := '插入目标表';
    V_STARTTIME := SYSDATE;
    INSERT INTO RRP_EAST.EAST5_507_YTDKXXB
      (RID, --数据主键
       JRXKZH, --金融许可证号
       NBJGH, --内部机构号
       YHJGMC, --银行机构名称
    ...
    SELECT SYS_GUID()                                       AS RID, --数据主键
           ...
           CASE WHEN REGEXP_REPLACE(TRIM(D.CUST_NM),'[0-9a-zA-Z[:punct:][:space:]]','') IS NOT NULL
                ...
                ELSE TRIM(D.CUST_NM)
            END                                             AS JKRMC, --借款人名称
           C.LEAD_BANK_PBC_NO                               AS QTHHH, --牵头行行号
           C.LEAD_BANK_NM                                   AS QTHHM, --牵头行行名
           C.ATND_BANK_PBC_NO                               AS CJHHH, --参加行行号
           C.ATND_BANK_NM                                   AS CJHHM, --参加行行名
           C.AGCY_BANK_PBC_NO                               AS DLHHH, --代理行行号
           C.AGCY_BANK_NM                                   AS DLHHM, --代理行行名
           CODE.TAR_VALUE_NAME                              AS YTCYLX, --银团成员类型
           A.CUR                                            AS BZ, --币种
           NVL(C.SYN_LOAN_TOT_AMT,0)                        AS YTDKZJE, --银团贷款总金额
           NVL(C.DSTR_SYN_LOAN_AMT,0)                       AS YFFDKJE, --已发放银团贷款金额
           NVL(C.BEAR_LOAN_AMT,0)                           AS CDDKJE, --承担贷款金额
           NVL(C.DSTR_BEAR_LOAN_AMT,0)                      AS YFFCDDKJE, --已发放承担贷款金额
           NVL(A.LOAN_AMT,0)                                AS JJJE, --借据金额
           NVL(A.LOAN_BAL,0)                                AS JJYE, --借据余额
           ...
           CASE WHEN LIST.FLAG = 1 THEN ORG.ORG_ID_LEL_1
            ...
            END                                             AS ADDRESS, --归属地
           --D.CUST_NM                                        AS JKRMC_ORIG, --借款人名称（脱敏前）
           --MOD BY LIP 20230504 当对公客户的名称都是中文名时，将其中的()改为（）
           CASE WHEN REGEXP_REPLACE(TRIM(D.CUST_NM),'[0-9a-zA-Z[:punct:][:space:]]','') IS NOT NULL
                THEN REPLACE(REPLACE(REPLACE(TRIM(D.CUST_NM),'(','（'),')','）'),' ','')
                ELSE TRIM(D.CUST_NM)
            END                                             AS JKRMC_ORIG, --借款人名称（脱敏前）
           '否'                                             AS JKRMC_OTH, --借款人是否对私客户
      ...
      LEFT JOIN RRP_MDL.M_LOAN_SYN_SUB C --银团贷款子表
      ...
      LEFT JOIN RRP_MDL.CODE_MAP CODE --码值配置表
        ON CODE.SRC_VALUE_CODE = C.AGCY_PART_LOAN_FLG
       AND CODE.SRC_CLASS_CODE = 'D0026' --银团成员类型
       AND CODE.TAR_CLASS_CODE = 'D0026' --银团成员类型
    ...
    SELECT CJRQ,XDHTH,XDJJH,COUNT(1)
      FROM RRP_EAST.EAST5_507_YTDKXXB T
     WHERE CJRQ = V_P_DATE
    ...
    IF V_COUNT > 0 THEN
       O_ERRCODE := '1';
       V_SQLMSG  := 'EAST5_507_YTDKXXB(CJRQ,XDHTH,XDJJH)数据重复';
